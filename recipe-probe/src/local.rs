		...
		if let Ok(entries) = fs::read_dir(&node_root) {
			...
			for path in paths {
				...
				domains.push(RamDomain {
					...
					maximum_inflight_transfers: TransferLaneCount::new(1)
						.map_err(|error| ProbeError::Discovery(error.to_string()))?,
				});
			}
		}
		if domains.is_empty() {
	...
	fn discover_storage(&self, host_memory_key: &Label) -> ProbeResult<Vec<StorageDomain>> {
		...
		for group in groups.into_values() {
			...
			result.push(StorageDomain {
				...
				duplex: group.duplex,
				maximum_concurrent_reads: TransferLaneCount::new(1)
					.map_err(|error| ProbeError::Discovery(error.to_string()))?,
				maximum_concurrent_writes: TransferLaneCount::new(1)
					.map_err(|error| ProbeError::Discovery(error.to_string()))?,
...
impl HostBenchmarkIo for LocalHostBenchmarks {
	fn benchmark_ram(&self, domain: &RamDomain, plan: BoundedBenchmarkPlan) -> ProbeResult<RamMeasurement> {
		...
		let bytes = usize::try_from(plan.buffer_bytes.get())
...
fn label(value: impl Into<String>) -> ProbeResult<Label> {
	Label::new(value).map_err(|error| ProbeError::Discovery(error.to_string()))
}
