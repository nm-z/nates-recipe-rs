...
impl MathFunction {
	...
	const fn domain(self) -> FiniteDomain {
		match self {
			...
			Self::LogOnePlus => FiniteDomain {
				inputs: LOG1P_X,
				relation: "1 + x is strictly positive",
			},
			Self::Floor | Self::Ceil | Self::RoundNearestEven | Self::Trunc | Self::Sign => FiniteDomain {
				inputs: ANY_X,
				relation: "every finite binary32 value is accepted",
			},
			Self::Fmod => FiniteDomain {
				inputs: FMOD_INPUTS,
