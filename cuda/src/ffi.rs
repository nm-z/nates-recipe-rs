...
pub(crate) type CuLaunchKernel = unsafe extern "C" fn(CuFunction, c_uint, c_uint, c_uint, c_uint, c_uint, c_uint, c_uint, CuStream, *mut *mut c_void, *mut *mut c_void) -> CuResult;
...
pub const REQUIRED_DRIVER_SYMBOLS: &[DriverSymbol] = &[
	DriverSymbol::Init,
	DriverSymbol::DriverGetVersion,
	DriverSymbol::DeviceGetCount,
	DriverSymbol::DeviceGet,
	DriverSymbol::DeviceGetName,
	DriverSymbol::DeviceGetUuid,
	DriverSymbol::DeviceGetPciBusId,
	DriverSymbol::DeviceTotalMemV2,
	DriverSymbol::DeviceGetAttribute,
	DriverSymbol::CtxCreateV2,
	DriverSymbol::CtxDestroyV2,
	DriverSymbol::CtxPushCurrentV2,
	DriverSymbol::CtxPopCurrentV2,
	DriverSymbol::ModuleLoadData,
	DriverSymbol::ModuleUnload,
	DriverSymbol::ModuleGetFunction,
	DriverSymbol::MemAllocV2,
	DriverSymbol::MemFreeV2,
	DriverSymbol::MemGetInfoV2,
	DriverSymbol::MemHostAlloc,
	DriverSymbol::MemFreeHost,
	DriverSymbol::StreamCreate,
	DriverSymbol::StreamDestroyV2,
	DriverSymbol::StreamQuery,
	DriverSymbol::EventCreate,
	DriverSymbol::EventRecord,
	DriverSymbol::EventQuery,
	DriverSymbol::EventDestroyV2,
	DriverSymbol::MemcpyHtoDAsyncV2,
	DriverSymbol::MemcpyDtoHAsyncV2,
	DriverSymbol::MemcpyDtoDAsyncV2,
	DriverSymbol::LaunchKernel,
];

pub const OPTIONAL_DRIVER_SYMBOLS: &[DriverSymbol] = &[
	DriverSymbol::DeviceGetUuidV2,
	DriverSymbol::ModuleGetLoadingMode,
	DriverSymbol::GetErrorName,
	DriverSymbol::GetErrorString,
];

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct DriverCapabilities {
	available: BTreeSet<DriverSymbol>,
}
...
pub(crate) struct Api {
	pub(crate) init: CuInit,
	pub(crate) driver_get_version: CuDriverGetVersion,
	pub(crate) device_get_count: CuDeviceGetCount,
	pub(crate) device_get: CuDeviceGet,
	pub(crate) device_get_name: CuDeviceGetName,
	pub(crate) device_get_uuid: CuDeviceGetUuid,
	pub(crate) device_get_uuid_v2: Option<CuDeviceGetUuid>,
	pub(crate) device_get_pci_bus_id: CuDeviceGetPciBusId,
	pub(crate) device_total_mem_v2: CuDeviceTotalMemV2,
	pub(crate) device_get_attribute: CuDeviceGetAttribute,
	pub(crate) ctx_create_v2: CuCtxCreateV2,
	pub(crate) ctx_destroy_v2: CuCtxDestroyV2,
	pub(crate) ctx_push_current_v2: CuCtxPushCurrentV2,
	pub(crate) ctx_pop_current_v2: CuCtxPopCurrentV2,
	pub(crate) module_load_data: CuModuleLoadData,
	pub(crate) module_unload: CuModuleUnload,
	pub(crate) module_get_function: CuModuleGetFunction,
	pub(crate) mem_alloc_v2: CuMemAllocV2,
	pub(crate) mem_free_v2: CuMemFreeV2,
	pub(crate) mem_get_info_v2: CuMemGetInfoV2,
	pub(crate) mem_host_alloc: CuMemHostAlloc,
	pub(crate) mem_free_host: CuMemFreeHost,
	pub(crate) stream_create: CuStreamCreate,
	pub(crate) stream_destroy_v2: CuStreamDestroyV2,
	pub(crate) stream_query: CuStreamQuery,
	pub(crate) event_create: CuEventCreate,
	pub(crate) event_record: CuEventRecord,
	pub(crate) event_query: CuEventQuery,
	pub(crate) event_destroy_v2: CuEventDestroyV2,
	pub(crate) memcpy_htod_async_v2: CuMemcpyHtoDAsyncV2,
	pub(crate) memcpy_dtoh_async_v2: CuMemcpyDtoHAsyncV2,
	pub(crate) memcpy_dtod_async_v2: CuMemcpyDtoDAsyncV2,
	pub(crate) launch_kernel: CuLaunchKernel,
	pub(crate) get_error_name: Option<CuGetErrorName>,
	pub(crate) get_error_string: Option<CuGetErrorString>,
...
impl Api {
	fn load(resolver: &impl SymbolResolver) -> Result<Self> {
		let init = required(resolver, DriverSymbol::Init)?;
		...
		let mut available = BTreeSet::from_iter(REQUIRED_DRIVER_SYMBOLS.iter().copied());
		for (symbol, present) in [
			(DriverSymbol::DeviceGetUuidV2, device_get_uuid_v2.is_some()),
			(
				DriverSymbol::ModuleGetLoadingMode,
				module_get_loading_mode.is_some(),
			),
			(DriverSymbol::GetErrorName, get_error_name.is_some()),
			(DriverSymbol::GetErrorString, get_error_string.is_some()),
		] {
			if present {
				available.insert(symbol);
			}
		}
		...
		Ok(Self {
			...
			event_query,
			event_destroy_v2,
			memcpy_htod_async_v2,
			memcpy_dtoh_async_v2,
			memcpy_dtod_async_v2,
			launch_kernel,
			get_error_name,
			get_error_string,
...
fn optional<T: Copy>(resolver: &impl SymbolResolver, symbol: DriverSymbol) -> Option<T> {
	let pointer = unsafe { resolver.lookup(symbol) }.ok().flatten()?;
	Some(unsafe { pointer_as_function(pointer) })
}

unsafe fn pointer_as_function<T: Copy>(pointer: NonNull<c_void>) -> T {
	assert_eq!(mem::size_of::<T>(), mem::size_of::<*mut c_void>());
	unsafe { mem::transmute_copy(&pointer.as_ptr()) }
}
...
impl SymbolResolver for DynamicLibrary {
	unsafe fn lookup(&self, symbol: DriverSymbol) -> core::result::Result<Option<NonNull<c_void>>, String> {
		unsafe {
			clear_dlerror();
			let pointer = libc::dlsym(self.handle.as_ptr(), symbol.c_name().as_ptr());
			let detail = take_dlerror();
