...
pub(crate) fn build(function: MathFunction) -> LanguageResult<ScalarProgram> {
	...
	for _ in 0..function.arity() {
		inputs.push(builder.input(DType::F32)?);
	}
	require_finite_inputs(&mut builder, &inputs)?;
	let output = emit_function(&mut builder, function, &inputs)?;
	let program = builder.finish(&[output])?;
	Ok(program)
}

fn emit_function(builder: &mut ScalarProgramBuilder, function: MathFunction, inputs: &[ScalarExpression]) -> LanguageResult<ScalarExpression> {
	let x = inputs[0];
	match function {
		MathFunction::Reciprocal => {
			require_nonzero(builder, x)?;
			let one = builder.f32(1.0)?;
			builder.binary(ScalarOpcode::Divide, one, x)
		}
		MathFunction::ReciprocalSquareRoot => {
			require_positive(builder, x)?;
			let root = builder.unary(ScalarOpcode::SquareRoot, x)?;
			let one = builder.f32(1.0)?;
			builder.binary(ScalarOpcode::Divide, one, root)
		}
		MathFunction::Sin => {
			require_closed_interval(builder, x, -8192.0, 8192.0)?;
			let (sine, _) = emit_sine_cosine_core(builder, x)?;
			preserve_zero(builder, x, sine)
		}
		MathFunction::Cos => {
			require_closed_interval(builder, x, -8192.0, 8192.0)?;
			let (_, cosine) = emit_sine_cosine_core(builder, x)?;
			Ok(cosine)
		}
		MathFunction::Tan => {
			require_closed_interval(builder, x, -1.4, 1.4)?;
			let (sine, cosine) = emit_sine_cosine_core(builder, x)?;
			let tangent = builder.binary(ScalarOpcode::Divide, sine, cosine)?;
			preserve_zero(builder, x, tangent)
		}
		MathFunction::Atan2 => {
			let y = x;
			let x = inputs[1];
			require_atan2_nonzero_pair(builder, y, x)?;
		...
		MathFunction::Log => {
			require_positive(builder, x)?;
			emit_log_core(builder, x)
		}
		MathFunction::LogOnePlus => {
			let negative_one = builder.f32(-1.0)?;
			let valid = builder.binary(ScalarOpcode::GreaterThan, x, negative_one)?;
			require_condition(builder, valid)?;
			let result = emit_log1p_core(builder, x)?;
			preserve_zero(builder, x, result)
		}
		...
		MathFunction::Trunc => emit_trunc(builder, x),
		MathFunction::Fmod => {
			let divisor = inputs[1];
			require_nonzero(builder, divisor)?;
			let result = builder.binary(ScalarOpcode::Remainder, x, divisor)?;
			preserve_zero(builder, x, result)
		}
		MathFunction::Pow => {
		...
		MathFunction::Sigmoid => {
			require_closed_interval(builder, x, -80.0, 80.0)?;
			emit_sigmoid_core(builder, x)
		}
		MathFunction::Tanh => {
			require_closed_interval(builder, x, -40.0, 40.0)?;
			let result = emit_tanh_core(builder, x)?;
			preserve_zero(builder, x, result)
		}
		MathFunction::Softplus => {
			require_closed_interval(builder, x, -80.0, 80.0)?;
			emit_softplus_core(builder, x)
		}
		MathFunction::Erf => {
			require_closed_interval(builder, x, -6.0, 6.0)?;
			let result = emit_erf_core(builder, x)?;
			preserve_zero(builder, x, result)
		}
	}
}
...
fn require_finite_inputs(builder: &mut ScalarProgramBuilder, inputs: &[ScalarExpression]) -> LanguageResult<()> {
	for input in inputs {
		let finite = builder.unary(ScalarOpcode::IsFinite, *input)?;
		require_condition(builder, finite)?;
	}
	Ok(())
}
...
fn require_closed_interval(builder: &mut ScalarProgramBuilder, value: ScalarExpression, lower: f32, upper: f32) -> LanguageResult<()> {
	let lower = builder.f32(lower)?;
	let at_least_lower = builder.binary(ScalarOpcode::GreaterThanOrEqual, value, lower)?;
	require_condition(builder, at_least_lower)?;
	let upper = builder.f32(upper)?;
...
fn require_condition(builder: &mut ScalarProgramBuilder, condition: ScalarExpression) -> LanguageResult<()> {
	builder.unary(ScalarOpcode::Require, condition)?;
	Ok(())
}

fn require_atan2_nonzero_pair(builder: &mut ScalarProgramBuilder, y: ScalarExpression, x: ScalarExpression) -> LanguageResult<()> {
	let zero = builder.f32(0.0)?;
...
fn preserve_zero(builder: &mut ScalarProgramBuilder, input: ScalarExpression, result: ScalarExpression) -> LanguageResult<ScalarExpression> {
	let zero = builder.f32(0.0)?;
...
fn horner(builder: &mut ScalarProgramBuilder, x: ScalarExpression, first: f32, remaining: &[f32]) -> LanguageResult<ScalarExpression> {
	let mut accumulator = builder.f32(first)?;
...
fn emit_sine_cosine_core(builder: &mut ScalarProgramBuilder, x: ScalarExpression) -> LanguageResult<(ScalarExpression, ScalarExpression)> {
	let reciprocal = builder.f32(TWO_OVER_PI)?;
	...
	let one = builder.f32(1.0)?;
	let cosine = builder.ternary(ScalarOpcode::Fma, squared, cosine_inner, one)?;

	let quadrant = builder.unary(ScalarOpcode::ConvertF32ToI32, nearest)?;
	let three = builder.i32(3)?;
	let quadrant = builder.binary(ScalarOpcode::BitAnd, quadrant, three)?;
	let zero = builder.i32(0)?;
	let two = builder.i32(2)?;
	let is_zero = builder.binary(ScalarOpcode::Equal, quadrant, zero)?;
	...
	let selected_cosine = builder.ternary(
		ScalarOpcode::Select,
		is_lower_half,
		cosine_lower,
		cosine_upper,
...
fn emit_atan2_core(builder: &mut ScalarProgramBuilder, y: ScalarExpression, x: ScalarExpression) -> LanguageResult<ScalarExpression> {
	let absolute_y = builder.unary(ScalarOpcode::Absolute, y)?;
	...
	let transformed_region = builder.binary(ScalarOpcode::GreaterThan, ratio, threshold)?;
	let one = builder.f32(1.0)?;
	...
	let reduced_angle = builder.ternary(
		ScalarOpcode::Select,
		transformed_region,
		transformed_angle,
		direct_angle,
	...
	let zero = builder.f32(0.0)?;
	let x_negative = builder.binary(ScalarOpcode::LessThan, x, zero)?;
...
fn emit_exp_with_underflow(builder: &mut ScalarProgramBuilder, x: ScalarExpression) -> LanguageResult<ScalarExpression> {
...
fn emit_exp_reconstruction(builder: &mut ScalarProgramBuilder, x: ScalarExpression, support_subnormals: bool) -> LanguageResult<ScalarExpression> {
	...
	let nearest = builder.unary(ScalarOpcode::RoundNearestEven, scaled)?;
	let negative_high = builder.f32(-LOG_TWO_HIGH)?;
	let reduced_high = builder.ternary(ScalarOpcode::Fma, nearest, negative_high, x)?;
...
fn emit_log_core(builder: &mut ScalarProgramBuilder, x: ScalarExpression) -> LanguageResult<ScalarExpression> {
	...
	let bits = builder.unary(ScalarOpcode::BitcastF32ToI32, normalized_input)?;
	let exponent_shift = builder.i32(23)?;
	let exponent = builder.binary(ScalarOpcode::ShiftRightLogical, bits, exponent_shift)?;
	let exponent_mask = builder.i32(0xff)?;
	let exponent = builder.binary(ScalarOpcode::BitAnd, exponent, exponent_mask)?;
	let exponent_bias = builder.i32(127)?;
	let exponent = builder.binary(ScalarOpcode::Subtract, exponent, exponent_bias)?;
	let subnormal_adjustment = builder.i32(-23)?;
	let no_adjustment = builder.i32(0)?;
	let adjustment = builder.ternary(
		ScalarOpcode::Select,
		subnormal,
		subnormal_adjustment,
		no_adjustment,
	)?;
	let exponent = builder.binary(ScalarOpcode::Add, exponent, adjustment)?;

	let mantissa_mask = builder.i32(0x007f_ffff)?;
	let mantissa = builder.binary(ScalarOpcode::BitAnd, bits, mantissa_mask)?;
	let one_bits = builder.i32(0x3f80_0000)?;
	...
	let halved_mantissa = builder.binary(ScalarOpcode::Multiply, mantissa, half)?;
	let mantissa = builder.ternary(
		ScalarOpcode::Select,
		reduce_mantissa,
		halved_mantissa,
		mantissa,
	)?;
	let one_integer = builder.i32(1)?;
	let zero_integer = builder.i32(0)?;
	let exponent_increment = builder.ternary(
		ScalarOpcode::Select,
		reduce_mantissa,
		one_integer,
		zero_integer,
	...
	let one = builder.f32(1.0)?;
	let numerator = builder.binary(ScalarOpcode::Subtract, mantissa, one)?;
	...
	let log_two_high = builder.f32(LOG_TWO_HIGH)?;
	let high = builder.ternary(ScalarOpcode::Fma, exponent, log_two_high, mantissa_log)?;
	let log_two_low = builder.f32(LOG_TWO_LOW)?;
	builder.ternary(ScalarOpcode::Fma, exponent, log_two_low, high)
}
...
fn emit_log1p_core(builder: &mut ScalarProgramBuilder, x: ScalarExpression) -> LanguageResult<ScalarExpression> {
	...
	let regular = emit_log_core(builder, shifted)?;
	builder.ternary(ScalarOpcode::Select, use_series, series, regular)
}

fn emit_trunc(builder: &mut ScalarProgramBuilder, x: ScalarExpression) -> LanguageResult<ScalarExpression> {
	let floor = builder.unary(ScalarOpcode::Floor, x)?;
	let ceiling = builder.unary(ScalarOpcode::Ceiling, x)?;
	let zero = builder.f32(0.0)?;
	let negative = builder.binary(ScalarOpcode::LessThan, x, zero)?;
...
fn emit_sign(builder: &mut ScalarProgramBuilder, x: ScalarExpression) -> LanguageResult<ScalarExpression> {
	let zero = builder.f32(0.0)?;
	let negative = builder.binary(ScalarOpcode::LessThan, x, zero)?;
	let positive = builder.binary(ScalarOpcode::GreaterThan, x, zero)?;
	let one = builder.f32(1.0)?;
	let negative_one = builder.f32(-1.0)?;
	let positive_or_zero = builder.ternary(ScalarOpcode::Select, positive, one, x)?;
	builder.ternary(
		ScalarOpcode::Select,
		negative,
		negative_one,
...
fn emit_sigmoid_core(builder: &mut ScalarProgramBuilder, x: ScalarExpression) -> LanguageResult<ScalarExpression> {
	let zero = builder.f32(0.0)?;
	let nonnegative = builder.binary(ScalarOpcode::GreaterThanOrEqual, x, zero)?;
	let negative_x = builder.unary(ScalarOpcode::Negate, x)?;
	let exp_negative = emit_exp_core(builder, negative_x)?;
	let exp_positive = emit_exp_core(builder, x)?;
	let one = builder.f32(1.0)?;
...
fn emit_tanh_core(builder: &mut ScalarProgramBuilder, x: ScalarExpression) -> LanguageResult<ScalarExpression> {
	...
	let magnitude = builder.binary(ScalarOpcode::Divide, numerator, denominator)?;
	copy_sign_from(builder, magnitude, x)
}
...
fn emit_softplus_core(builder: &mut ScalarProgramBuilder, x: ScalarExpression) -> LanguageResult<ScalarExpression> {
	...
	let exponential = emit_exp_core(builder, negative_absolute)?;
	let correction = emit_log1p_core(builder, exponential)?;
	let zero = builder.f32(0.0)?;
	let positive = builder.binary(ScalarOpcode::GreaterThan, x, zero)?;
...
fn emit_erf_core(builder: &mut ScalarProgramBuilder, x: ScalarExpression) -> LanguageResult<ScalarExpression> {
	let absolute = builder.unary(ScalarOpcode::Absolute, x)?;
	let p = builder.f32(0.327_591_1)?;
	let scaled = builder.binary(ScalarOpcode::Multiply, p, absolute)?;
	let one = builder.f32(1.0)?;
	...
	let magnitude = builder.binary(ScalarOpcode::Subtract, one, tail)?;
	copy_sign_from(builder, magnitude, x)
}

fn copy_sign_from(builder: &mut ScalarProgramBuilder, magnitude: ScalarExpression, sign_source: ScalarExpression) -> LanguageResult<ScalarExpression> {
	let bits = builder.unary(ScalarOpcode::BitcastF32ToI32, sign_source)?;
	let shift = builder.i32(31)?;
	let sign = builder.binary(ScalarOpcode::ShiftRightLogical, bits, shift)?;
