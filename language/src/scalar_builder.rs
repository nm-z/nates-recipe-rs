...
impl ScalarProgramBuilder {
	...
	pub fn binary(&mut self, opcode: ScalarOpcode, left: ScalarExpression, right: ScalarExpression) -> LanguageResult<ScalarExpression> { self.apply(opcode, &[left, right]) }

	pub fn ternary(&mut self, opcode: ScalarOpcode, first: ScalarExpression, second: ScalarExpression, third: ScalarExpression) -> LanguageResult<ScalarExpression> { self.apply(opcode, &[first, second, third]) }
