...
use recipe_core::{AliasPermission, ByteCount, DType, KernelTemplateId, ScalarConstant, ScalarInput, ScalarInstruction, ScalarLiteral, ScalarOpcode, ScalarProgram, ScalarValueId, ValueId};
use recipe_ogdl::{Graph, NodeId};
use write::Errored;

use crate::{AtomicOperation, AtomicOrdering, AxisSet, CalculationGraph, CalculationNode, Contraction, Elementwise, Gather, Histogram, IndexBounds, IndexMap, LanguageError, PrimitiveAliasRule, PrimitiveKernel, PrimitiveKind, RandomDistribution, RandomKey, RandomMap, Reduce, ReduceOperator, ReduceResult, Scan, ScanMode, Scatter, ScatterConflict, Shape, Sort, SortDirection, Tensor, TensorLayout};
...
#[non_exhaustive]
pub enum OgdlDocumentErrorKind {
	InvalidRoot,
	MissingField,
	DuplicateField,
	UnknownField,
...
#[non_exhaustive]
pub enum OgdlCodecError {
	Ogdl(Errored),
	Document {
		kind: OgdlDocumentErrorKind,
		path: String,
		detail: String,
	},
...
impl OgdlCodecError {
	fn document(kind: OgdlDocumentErrorKind, path: impl Into<String>, detail: impl Into<String>) -> Self {
		Self::Document {
			kind,
			path: path.into(),
			detail: detail.into(),
...
impl fmt::Display for OgdlCodecError {
	fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			Self::Ogdl(error) => write!(formatter, "invalid OGDL: {error}"),
...
impl From<Errored> for OgdlCodecError {
	fn from(error: Errored) -> Self { Self::Ogdl(error) }
}
...
impl CalculationGraph {
	...
	pub fn from_ogdl(input: &str) -> Result<Self, OgdlCodecError> {
...
fn encode_graph(source: &CalculationGraph) -> Result<Graph, OgdlCodecError> {
	...
	for tensor in &source.tensors {
		encode_tensor(&mut graph, tensors, tensor)?;
	}
...
fn field_value(graph: &mut Graph, parent: NodeId, name: &str, value: impl Into<String>) -> Result<NodeId, OgdlCodecError> {
...
fn encode_tensor(graph: &mut Graph, parent: NodeId, tensor: &Tensor) -> Result<(), OgdlCodecError> {
	...
	field_value(
		...
		target,
		"storage_bytes",
		tensor.storage_bytes.get().to_string(),
	)?;
	field_value(
		graph,
		target,
		"external_input",
		bool_name(tensor.external_input),
	)?;
	field_value(
		graph,
		target,
		"external_output",
		bool_name(tensor.external_output),
	)?;
	Ok(())
}
	...
	match kind {
		...
		PrimitiveKind::Histogram(spec) => {
			...
			field_value(
				...
				variant,
				"ordering",
				atomic_ordering_name(spec.ordering),
			)?;
			Ok(())
		}
...
fn encode_axis_pairs(graph: &mut Graph, parent: NodeId, name: &str, pairs: &[(usize, usize)]) -> Result<(), OgdlCodecError> {
	let list = graph.append_child(parent, name)?;
...
fn encode_random_distribution(graph: &mut Graph, parent: NodeId, distribution: RandomDistribution) -> Result<(), OgdlCodecError> {
	match distribution {
...
fn encode_scalar_program(graph: &mut Graph, parent: NodeId, program: &ScalarProgram) -> Result<(), OgdlCodecError> {
	...
	for instruction in &program.instructions {
		...
		let opcode = scalar_opcode_name(instruction.opcode).ok_or_else(|| {
...
fn decode_graph(source: &Graph) -> Result<CalculationGraph, OgdlCodecError> {
	...
	let nodes = decode_list(
		...
		"RecipeIR.nodes",
		"node",
		decode_calculation_node,
	)?;
	let graph = CalculationGraph { tensors, nodes };
	graph.validate()?;
	Ok(graph)
}
...
fn decode_tensor(source: &Graph, node: NodeId, path: &str) -> Result<Tensor, OgdlCodecError> {
	...
	Ok(Tensor {
		...
		storage_bytes: ByteCount::new(parse_number(
			source,
			fields[4],
...
fn decode_kernel(source: &Graph, node: NodeId, path: &str) -> Result<PrimitiveKernel, OgdlCodecError> {
	...
	let outputs = decode_values(source, fields[2], &outputs_path, "value")?
		.into_iter()
		.enumerate()
		.map(|(index, value)| parse_number_text(value, &format!("{outputs_path}.value[{index}]")).map(ValueId::new))
		.collect::<Result<Vec<_>, _>>()?;
	let alias_rules = decode_list(
		source,
		fields[3],
	...
	match node_text(source, variant) {
		...
		"Reduce" => {
			...
			Ok(PrimitiveKind::Reduce(Reduce {
				...
				axes: AxisSet::new(axes)?,
				keep_dimensions: parse_bool(
					value_text(
						source,
						fields[2],
		...
		"Random" => {
			...
			Ok(PrimitiveKind::Random(RandomMap {
				distribution: decode_random_distribution(source, fields[0], &format!("{variant_path}.distribution"))?,
				key: RandomKey {
...
fn decode_scatter_conflict(source: &Graph, node: NodeId, path: &str) -> Result<ScatterConflict, OgdlCodecError> {
	...
	match node_text(source, variant) {
		...
		"Atomic" => {
			...
			Ok(ScatterConflict::Atomic {
				operation: parse_atomic_operation(
...
fn decode_random_distribution(source: &Graph, node: NodeId, path: &str) -> Result<RandomDistribution, OgdlCodecError> {
	...
	match node_text(source, variant) {
		...
		"BernoulliI32" => {
			...
			Ok(RandomDistribution::BernoulliI32 {
				probability_bits: parse_number(
					source,
					fields[0],
...
fn decode_scalar_program(source: &Graph, node: NodeId, path: &str) -> Result<ScalarProgram, OgdlCodecError> {
	...
	let inputs = decode_list(
		...
		|source, input, input_path| {
			...
			Ok(ScalarInput {
				id: ScalarValueId::new(parse_number(
					source,
					fields[0],
	...
	let outputs = decode_values(source, fields[3], &outputs_path, "value")?
		.into_iter()
		.enumerate()
		.map(|(index, value)| parse_number_text(value, &format!("{outputs_path}.value[{index}]")).map(ScalarValueId::new))
		.collect::<Result<Vec<_>, _>>()?;
	Ok(ScalarProgram {
		inputs,
		constants,
...
fn decode_scalar_instruction(source: &Graph, node: NodeId, path: &str) -> Result<ScalarInstruction, OgdlCodecError> {
	...
	let operands = decode_values(source, fields[3], &operands_path, "value")?
		.into_iter()
		.enumerate()
		.map(|(index, value)| parse_number_text(value, &format!("{operands_path}.value[{index}]")).map(ScalarValueId::new))
		.collect::<Result<Vec<_>, _>>()?;
...
fn node_text(graph: &Graph, node: NodeId) -> &str {
	graph.node(node)
		.expect("NodeId obtained from this immutable graph")
		.text()
}

fn node_children(graph: &Graph, node: NodeId) -> &[NodeId] {
	graph.node(node)
		.expect("NodeId obtained from this immutable graph")
		.children()
}
...
fn required_fields(graph: &Graph, parent: NodeId, path: &str, names: &[&str]) -> Result<Vec<NodeId>, OgdlCodecError> {
	...
	for child in node_children(graph, parent) {
		let name = node_text(graph, *child);
		...
		if found[index].replace(*child).is_some() {
			return Err(OgdlCodecError::document(
...
fn require_leaf(graph: &Graph, node: NodeId, path: &str) -> Result<(), OgdlCodecError> {
	let count = node_children(graph, node).len();
	if count == 0 {
		Ok(())
	} else {
		Err(OgdlCodecError::document(
...
fn value_text<'a>(graph: &'a Graph, field: NodeId, path: &str) -> Result<&'a str, OgdlCodecError> {
	let value = only_child(graph, field, path)?;
	require_leaf(graph, value, path)?;
	Ok(node_text(graph, value))
}

fn decode_values<'a>(graph: &'a Graph, parent: NodeId, path: &str, item_name: &str) -> Result<Vec<&'a str>, OgdlCodecError> {
	node_children(graph, parent)
		.iter()
		.enumerate()
		.map(|(index, child)| {
			let actual = node_text(graph, *child);
			if actual != item_name {
				return Err(OgdlCodecError::document(
					OgdlDocumentErrorKind::UnknownField,
					format!("{path}.{actual}"),
...
fn decode_list<T>(graph: &Graph, parent: NodeId, path: &str, item_name: &str, decode: impl Fn(&Graph, NodeId, &str) -> Result<T, OgdlCodecError>) -> Result<Vec<T>, OgdlCodecError> {
	node_children(graph, parent)
		.iter()
		.enumerate()
		.map(|(index, child)| {
...
fn parse_bool(text: &str, path: &str) -> Result<bool, OgdlCodecError> {
	match text {
		"true" => Ok(true),
		"false" => Ok(false),
		_ => {
...
fn unknown_variant(path: &str, name: &str) -> OgdlCodecError {
	OgdlCodecError::document(
		OgdlDocumentErrorKind::UnknownVariant,
		path,
...
fn parse_dtype(text: &str, path: &str) -> Result<DType, OgdlCodecError> {
	match text {
		"F32" => Ok(DType::F32),
		"I32" => Ok(DType::I32),
		name => Err(unknown_variant(path, name)),
...
fn parse_alias_permission(text: &str, path: &str) -> Result<AliasPermission, OgdlCodecError> {
	match text {
...
fn parse_atomic_ordering(text: &str, path: &str) -> Result<AtomicOrdering, OgdlCodecError> {
	match text {
...
fn parse_atomic_operation(text: &str, path: &str) -> Result<AtomicOperation, OgdlCodecError> {
	match text {
...
fn parse_index_bounds(text: &str, path: &str) -> Result<IndexBounds, OgdlCodecError> {
	match text {
...
fn parse_reduce_operator(text: &str, path: &str) -> Result<ReduceOperator, OgdlCodecError> {
	match text {
...
fn parse_reduce_result(text: &str, path: &str) -> Result<ReduceResult, OgdlCodecError> {
	match text {
...
fn parse_sort_direction(text: &str, path: &str) -> Result<SortDirection, OgdlCodecError> {
	match text {
...
fn parse_scalar_opcode(text: &str, path: &str) -> Result<ScalarOpcode, OgdlCodecError> {
	match text {
