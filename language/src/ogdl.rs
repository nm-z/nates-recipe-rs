...
use recipe_core::{
	AliasPermission, ByteCount, DType, KernelTemplateId, ScalarConstant, ScalarInput, ScalarInstruction,
...
fn encode_axis_pairs(
	graph: &mut Graph,
	parent: NodeId,
	name: &str,
	pairs: &[(usize, usize)],
) -> Result<(), OgdlCodecError> {
...
fn decode_list<T>(
	...
	path: &str,
	item_name: &str,
	decode: impl Fn(&Graph, NodeId, &str) -> Result<T, OgdlCodecError>,
) -> Result<Vec<T>, OgdlCodecError> {
	node_children(graph, parent)
		.iter()
		.enumerate()
		.map(|(index, child)| {
