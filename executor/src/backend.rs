...
use recipe_core::{ArenaLayout, ArtifactId, ByteCount, DeviceId, FinalizedBundle, KernelTemplateId, LinkId, LoopIteration, MetricId, MetricPurpose, MetricSlotId, ResolvedTransferEndpoint, ResolvedValueLocation, RunId, RunPhase, SubmissionSlots, TaskId, TransferLaneClaim};
...
pub trait Backend: sealed::Sealed {
	...
	fn submit_loop_iteration(&mut self, resource: &mut Self::Resource, arenas: ArenaSet<'_, Self::Arena>, pending: &mut Self::Pending, iteration: LoopIteration, work: BackendWork<'_>, physical_calls: &mut PhysicalCallBatch) -> std::result::Result<(), Self::Error> {
		let _ = iteration;
