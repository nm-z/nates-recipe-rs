...
pub enum WorkClass { InitAdmission, Calculation, InternalTransfer, Metric, ExitTransfer, }

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct InitAdmissionWork<'a> {
	pub task: TaskId, pub destination: ResolvedValueLocation, pub bytes: ByteCount, pub submission: SubmissionSlots,
	pub image: &'a [u8],
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct CalculationWork<'a> {
	pub task: TaskId, pub run: RunId,
	/// Zero-based finalized loop iteration for recurrent-state and schedule addressing.
	pub iteration: LoopIteration, pub device: DeviceId, pub kernel_template: KernelTemplateId, pub artifact: ArtifactId,
...
pub struct TransferWork<'a> {
	pub task: TaskId, pub source: ResolvedTransferEndpoint, pub destination: ResolvedTransferEndpoint,
	pub bytes: ByteCount,
	pub route: &'a [LinkId],
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct MetricWork { pub task: TaskId,
	/// Zero-based finalized loop iteration whose value is being emitted.
	pub iteration: LoopIteration, pub purpose: MetricPurpose, pub metric: MetricId, pub slot: MetricSlotId,
	pub value: ResolvedValueLocation, pub submission: SubmissionSlots, }
...
impl BackendWork<'_> {
	#[must_use]
	pub const fn task(&self) -> TaskId { match self { Self::InitAdmission(work) => work.task,
			Self::Calculation(work) => work.task, Self::InternalTransfer(work) | Self::ExitTransfer(work) => work.task,
			Self::Metric(work) => work.task, } }

	#[must_use]
	pub const fn class(&self) -> WorkClass { match self { Self::InitAdmission(_) => WorkClass::InitAdmission,
			Self::Calculation(_) => WorkClass::Calculation, Self::InternalTransfer(_) => WorkClass::InternalTransfer,
			Self::Metric(_) => WorkClass::Metric, Self::ExitTransfer(_) => WorkClass::ExitTransfer, } } }
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct PendingRequest { pub task: TaskId, pub phase: RunPhase, pub class: WorkClass,
...
impl<'a, A> ArenaSet<'a, A> {
	...
	pub fn iter(&self) -> impl ExactSizeIterator<Item = (DeviceId, &A)> { self.arenas.iter().map(|(device, arena)| (*device, arena)) }
...
pub enum BackendPoll { Pending, Complete { metric: Option<MetricValue> }, }

/// Status attached to a physical poll record.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum PhysicalPollStatus { Pending, Complete, Failed, }
...
/// chunks here.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum PhysicalCall { BindResources, PreparePending { task: TaskId, }, PrepareExternal { task: TaskId, },
	AllocateArena { device: DeviceId, bytes: ByteCount, }, AdmissionChunk { task: TaskId, device: DeviceId,
		bytes: ByteCount, chunk_index: u32, }, SubmitCalculation { task: TaskId, }, SubmitInternalTransfer { task: TaskId, },
	SubmitMetric { task: TaskId, slot: MetricSlotId, }, SubmitExitTransfer { task: TaskId, }, SubmitExternalIngress {
		task: TaskId, bytes: ByteCount, }, SubmitExternalEgress { task: TaskId, bytes: ByteCount, },
	AcknowledgeExternalEgress { task: TaskId, }, QuiesceWorker, CollectExit { task: TaskId, bytes: ByteCount, }, Poll {
		task: TaskId, status: PhysicalPollStatus, }, ReleaseArena { device: DeviceId, }, DestroyResources, }
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct PhysicalCallBatch { calls: [Option<PhysicalCall>; MAX_PHYSICAL_CALLS_PER_OPERATION], len: u8, }

impl PhysicalCallBatch {
	#[must_use]
	pub const fn new() -> Self { Self { calls: [None; MAX_PHYSICAL_CALLS_PER_OPERATION], len: 0, } }

	#[must_use]
	pub const fn single(call: PhysicalCall) -> Self { let mut calls = [None; MAX_PHYSICAL_CALLS_PER_OPERATION];
		calls[0] = Some(call); Self { calls, len: 1 } }

	pub fn try_push(&mut self, call: PhysicalCall) -> Result<(), PhysicalCallBatchOverflow> {
		let index = usize::from(self.len); match index == MAX_PHYSICAL_CALLS_PER_OPERATION {
			true => Err(PhysicalCallBatchOverflow), false => { self.calls[index] = Some(call); self.len += 1; Ok(()) } } }


	pub fn iter(&self) -> impl Iterator<Item = PhysicalCall> + '_ {
		self.calls[..self.len()].iter().map(|call| { match call { Some(call) => *call,
...
impl Default for PhysicalCallBatch { fn default() -> Self { Self::new() } }
...
/// before `init`.
pub trait Backend: sealed::Sealed { type Arena: Debug; type Resource: Debug; type Pending: Debug;
	...
	fn submit_loop_iteration( &mut self, resource: &mut Self::Resource,
		arenas: ArenaSet<'_, Self::Arena>,
		pending: &mut Self::Pending, iteration: LoopIteration,
		work: BackendWork<'_>,
		physical_calls: &mut PhysicalCallBatch, ) -> std::result::Result<(), Self::Error> { let _ = iteration;
