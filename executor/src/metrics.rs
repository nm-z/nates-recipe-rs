...
impl MetricMailbox {
	pub(crate) fn new(slots: &[MetricSlot], tasks: &[Task]) -> Self {
		Self {
			slots: slots
				...
				.filter(|slot| {
					tasks.iter().any(|task| {
						matches!( &task.kind, TaskKind::Metric(metric)
	...
	pub(crate) fn publish(&mut self, iteration: LoopIteration, task: TaskId, slot: MetricSlotId, metric: MetricId, value: MetricValue) -> Result<bool> {
		let state = self
			.slots
			.get_mut(&slot)
