...
use recipe_core::{ArtifactId, BundleIdentity, ByteCount, DeviceId, FinalizedBundle, IterationDomain, KernelTemplateId, LinkId, LoopIteration, MetricId, MetricPurpose, MetricSlotId, ResolvedTransferEndpoint, ResolvedValueLocation, RunId, RunPhase, ScheduleWindow, SubmissionSlots, Task, TaskId, TaskKind, TransferEndpoint, TransferLaneClaim, ValueId};

use crate::{
	backend::{ArenaSet, Backend, BackendPoll, BackendWork, CalculationWork, InitAdmissionWork, MAX_PHYSICAL_CALLS_PER_OPERATION, MetricWork, PendingRequest, PhysicalCall, PhysicalCallBatch, TransferWork, WorkClass},
...
impl JournalCapacity {
	...
	pub(crate) fn for_bundle_retaining<B: Backend>(bundle: &FinalizedBundle, retained_loop_iterations: u64) -> Result<Self> {
		debug_assert!(B::MAX_NON_POLL_PHYSICAL_CALLS != 0 && B::MAX_NON_POLL_PHYSICAL_CALLS <= MAX_PHYSICAL_CALLS_PER_OPERATION);
...
impl RunJournal {
	pub(crate) fn with_loop_detail(declared: JournalCapacity, tasks: &[Task], retain_all_loop_iterations: bool) -> Self {
	...
	fn is_repeated_loop_physical(&self, call: PhysicalCall) -> bool {
		let task = match call {
			PhysicalCall::SubmitCalculation { task } | PhysicalCall::SubmitInternalTransfer { task } | PhysicalCall::SubmitMetric { task, .. } | PhysicalCall::SubmitExternalIngress { task, .. } | PhysicalCall::SubmitExternalEgress { task, .. } | PhysicalCall::AcknowledgeExternalEgress { task } | PhysicalCall::Poll { task, .. } => Some(task),
			PhysicalCall::BindResources | PhysicalCall::PreparePending { .. } | PhysicalCall::PrepareExternal { .. } | PhysicalCall::AllocateArena { .. } | PhysicalCall::AdmissionChunk { .. } | PhysicalCall::SubmitExitTransfer { .. } | PhysicalCall::QuiesceWorker | PhysicalCall::CollectExit { .. } | PhysicalCall::ReleaseArena { .. } | PhysicalCall::DestroyResources => None,
		};
...
impl<B: Backend> PreparedRun<B> {
	...
	pub fn prepare_recoverable(run_id: RunId, bundle: FinalizedBundle, backend: B, watchdog: Watchdog) -> std::result::Result<Self, RunFailure<B>> {
	...
	pub fn prepare_with_journal_capacity_recoverable(run_id: RunId, bundle: FinalizedBundle, mut backend: B, watchdog: Watchdog, journal_capacity: JournalCapacity) -> std::result::Result<Self, RunFailure<B>> {
		...
		journal
			.record_logical(LogicalEvent::Prepared {
				run: run_id,
				bundle: bundle_identity,
			})
			.unwrap();
	...
	pub fn initialize_recoverable(mut self, images: impl IntoIterator<Item = DeviceImage>) -> std::result::Result<InitializedRun<B>, RunFailure<B>> {
	...
	pub fn poll_with_progress_or_stop(&mut self, stop_requested: impl FnOnce() -> bool) -> Result<(LoopStatus, bool)> {
		...
		match poll_phase_once(&mut self.core, &mut self.phase, None) {
			...
			Err(error) => {
				self.core
					.journal
					.record_logical(LogicalEvent::LoopFailed {
						run: self.core.run_id,
					})
					.unwrap();
...
fn unstarted_failure<B: Backend>(run_id: RunId, bundle: BundleIdentity, backend: B, error: ExecutorError) -> RunFailure<B> {
	RunFailure {
		parts: Box::new(RunFailureParts {
			run_id,
			bundle,
...
fn prepared_resource_failure<B: Backend>(run_id: RunId, bundle: BundleIdentity, mut backend: B, resource: B::Resource, mut journal: RunJournal, error: ExecutorError) -> RunFailure<B> {
	let mut physical_calls = PhysicalCallBatch::new();
...
fn run_failure<B: Backend>(core: RunCore<B>, error: ExecutorError, cleanup_error: Option<ExecutorError>) -> RunFailure<B> {
	RunFailure {
		parts: Box::new(RunFailureParts {
			run_id: core.run_id,
			bundle: core.bundle.identity(),
...
fn record_teardown_error(error: &mut Option<ExecutorError>, cleanup_error: &mut Option<ExecutorError>, reported: ExecutorError) {
	if error.is_none() {
		*error = Some(reported);
...
impl PreparedTask {
	fn new(bundle: &FinalizedBundle, task: &Task) -> Self {
		...
		let work = match (&task.kind, task.phase) {
			...
			(TaskKind::Calculation(_), _) | (TaskKind::Metric(_), _) | (TaskKind::Transfer(_), RunPhase::Init | RunPhase::Loop | RunPhase::Exit) => unreachable!(),
		};
		Self {
			id: task.id,
			window: task.window,
	...
	const fn submission(&self) -> Option<SubmissionSlots> {
		match self.work {
			PreparedWork::InitAdmission { submission, .. } | PreparedWork::Calculation { submission, .. } | PreparedWork::Transfer { submission, .. } | PreparedWork::Metric { submission, .. } => Some(submission),
		}
	}

	fn backend_work<'a>(&'a self, run: RunId, iteration: Option<LoopIteration>, images: Option<&'a BTreeMap<InitImageKey, Vec<u8>>>) -> BackendWork<'a> {
		match &self.work {
			PreparedWork::InitAdmission {
				device,
				destination,
...
fn realize_phase<B: Backend>(prepared: PreparedPhase, phase: RunPhase, backend: &mut B, resource: &mut B::Resource, journal: &mut RunJournal) -> Result<PhaseState<B::Pending>> {
...
fn validate_images(bundle: &FinalizedBundle, images: impl IntoIterator<Item = DeviceImage>, fault_resets: &[FaultReset]) -> Result<BTreeMap<InitImageKey, Vec<u8>>> {
...
fn fault_reset_range(image: ResolvedValueLocation, fault: ResolvedValueLocation, image_bytes: ByteCount) -> core::ops::Range<usize> {
	...
	let end = start.checked_add(core::mem::size_of::<i32>()).unwrap();
	start..end
}

fn run_phase_blocking<B: Backend>(core: &mut RunCore<B>, state: &mut PhaseState<B::Pending>, images: Option<&BTreeMap<InitImageKey, Vec<u8>>>) -> Result<()> {
	let mut backoff = BlockingPollBackoff::new();
	loop {
...
fn poll_phase_once<B: Backend>(core: &mut RunCore<B>, state: &mut PhaseState<B::Pending>, images: Option<&BTreeMap<InitImageKey, Vec<u8>>>) -> Result<PhasePoll> {
	...
	for index in 0..state.slots.len() {
		let runnable = {
			...
			slot.status == SlotStatus::Remaining
				...
				.slots
				.iter()
				.all(|active| active.status != SlotStatus::Pending || active.task.window.overlaps(slot.task.window) || active.task.submission().map(|submission| submission.queue) == pipeline_queue)
		};
...
fn submit_slot<B: Backend>(core: &mut RunCore<B>, phase: RunPhase, iteration: Option<LoopIteration>, slot: &mut TaskSlot<B::Pending>, images: Option<&BTreeMap<InitImageKey, Vec<u8>>>) -> Result<()> {
	...
	match slot.task.work {
		PreparedWork::InitAdmission { device, bytes, .. } => {
			core.journal
				.record_logical(LogicalEvent::InitAdmission {
					task: slot.task.id,
					device,
					bytes,
				})
...
fn complete_slot<B: Backend>(core: &mut RunCore<B>, phase: RunPhase, iteration: Option<LoopIteration>, slot: &mut TaskSlot<B::Pending>, metric_value: Option<crate::MetricValue>) -> Result<()> {
		...
		(
			...
			match purpose {
				MetricPurpose::User => {
					...
					core.journal
						.record_logical(LogicalEvent::MetricPublished {
							task: slot.task.id,
							slot: *metric_slot,
				...
				MetricPurpose::FaultReadback => {
					match metric_value {
						crate::MetricValue::I32(0) => {
							core.journal
								.record_logical(LogicalEvent::FaultChecked {
...
fn collect_exit_image<B: Backend>(core: &mut RunCore<B>, slot: &mut TaskSlot<B::Pending>, source: ResolvedValueLocation, bytes: ByteCount) -> Result<()> {
...
fn resolve_transfer_endpoints(bundle: &FinalizedBundle, task: TaskId) -> recipe_core::ResolvedTransferEndpoints { bundle.transfer_endpoints(task).unwrap() }

fn resolve_value(bundle: &FinalizedBundle, value: ValueId) -> ResolvedValueLocation { *bundle.value_location(value).unwrap() }

fn backend_value<T, E: std::error::Error>(journal: &mut RunJournal, operation: BackendOperation, physical_calls: PhysicalCallBatch, result: std::result::Result<T, E>) -> Result<T> {
...
fn checked_capacity_sum(values: &[usize]) -> usize {
	values.iter()
		.fold(0_usize, |sum, value| sum.checked_add(*value).unwrap())
}
...
fn iteration_domain_activations_before(domain: IterationDomain, retained_end: u64) -> usize {
	...
	let span = end_exclusive.checked_sub(domain.first_iteration()).unwrap();
