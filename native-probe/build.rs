...
fn main() -> Result<(), Box<dyn Error>> {
	...
	let inputs = [
		...
		("recipe-primitives", manifest.join("../primitives/src")),
	...
	for (name, path) in [
		...
		("native-probe/Cargo.toml", manifest.join("Cargo.toml")),
		("core/Cargo.toml", manifest.join("../core/Cargo.toml")),
		(
			"executor/Cargo.toml",
			manifest.join("../executor/Cargo.toml"),
		),
		("host/Cargo.toml", manifest.join("../host/Cargo.toml")),
		("kernel/Cargo.toml", manifest.join("../kernel/Cargo.toml")),
		(
			"language/Cargo.toml",
		...
		("planner/Cargo.toml", manifest.join("../planner/Cargo.toml")),
		(
		...
		("cuda/Cargo.toml", manifest.join("../cuda/Cargo.toml")),
		("hsa/Cargo.toml", manifest.join("../hsa/Cargo.toml")),
		("probe/Cargo.toml", manifest.join("../probe/Cargo.toml")),
	] {
...
fn collect_rust_files(domain: &str, root: &Path, directory: &Path, output: &mut Vec<(String, PathBuf)>) -> Result<(), Box<dyn Error>> {
