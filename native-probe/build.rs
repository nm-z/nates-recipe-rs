...
fn main() -> Result<(), Box<dyn Error>> {
	...
	]; let mut files = Vec::new(); for (domain, root) in inputs { collect_rust_files(domain, &root, &root, &mut files)?; }
	files.push((
	...
	for (name, path) in files {
		...
		let bytes = fs::read(&path)?; hash_field(&mut digest, name.as_bytes()); hash_field(&mut digest, &bytes); }
	for name in [
		"HOST",
		"TARGET",
		"OPT_LEVEL",
		...
		hash_field(&mut digest, name.as_bytes()); match std::env::var_os(name) {
			Some(value) => hash_field(&mut digest, value.as_encoded_bytes()), None => hash_field(&mut digest, &[]), } }
	hash_rustc_identity(&mut digest)?; let source_digest = digest .finalize() .iter()
		.map(|byte| format!("{byte:02x}"))
		.collect::<String>();
	println!("cargo:rustc-env=RECIPE_NATIVE_PROBE_SOURCE_DIGEST={source_digest}");
	Ok(()) }

fn hash_field(digest: &mut Sha256, value: &[u8]) { digest.update((value.len() as u64).to_le_bytes());
	digest.update(value); }
...
fn hash_rustc_identity(digest: &mut Sha256) -> Result<(), Box<dyn Error>> {
	...
	if !output.status.success() { return Err(format!(
			"{} -Vv failed with {}",
			Path::new(&rustc).display(), output.status ) .into()); }
	hash_field(digest, b"RUSTC");
	hash_field(digest, rustc.as_encoded_bytes()); hash_field(digest, &output.stdout); hash_field(digest, &output.stderr);
	Ok(()) }

fn collect_rust_files( domain: &str, root: &Path, directory: &Path, output: &mut Vec<(String, PathBuf)>,
) -> Result<(), Box<dyn Error>> { let mut entries = fs::read_dir(directory)?.collect::<Result<Vec<_>, _>>()?;
	entries.sort_by_key(|entry| entry.file_name()); for entry in entries { let path = entry.path(); if path.is_dir() {
