...
impl HsaBackend {
	...
	pub(crate) fn with_runtime<T>(
		&self,
		operation: impl FnOnce(&PinnedLibrary, &Runtime) -> ProbeResult<T>,
		...
		if state.is_none() {
			let runtime = Runtime::open(library.path.as_os_str())
	...
	pub(crate) fn descriptor(
	...
	) -> ProbeResult<Option<GpuDescriptor>> {
		...
		Ok(Some(GpuDescriptor {
			...
			runtime_abi: label(
				format!(
					"hsa-{}.{}-amdext-{}.{}:{runtime}",
					system.hsa_version_major,
					system.hsa_version_minor,
					system.amd_extension_version_major,
	...
	fn benchmark_device(&self, expected: &GpuDescriptor, plan: BoundedBenchmarkPlan) -> ProbeResult<GpuMeasurement> {
		let result =
			self.with_runtime(|library, runtime| self.benchmark_with_runtime(library, runtime, expected, plan))?;
