...
use recipe_hsa::{AgentDescription, AgentUuidBody, DeviceType, DiscoveredAgent, DispatchGeometry, IsaTarget, MemoryLocation, MemoryPoolFlags, MemorySegment, Pending, PollStatus, QueueConfig, QueueKind, Runtime, Session, SystemDescription};
use recipe_kernel::{AmdTarget, ArtifactBuilder, BuildPhase, BuiltArtifact, KernelTarget, LoweringOptions, lower_elementwise};
use recipe_probe::{BoundedBenchmarkPlan, GpuDescriptor, GpuMeasurement, LinkDuplex, ProbeError, ProbeResult};

use crate::{
	benchmark::{calculation_rate, capacity, compute_buffer_bytes, fill_input, fma_template, measured, plan_bytes, time_bounded, transfer_rate, verify_compute_output},
	config::{BackendLibrary, KernelBuildConfig, NativeProbeConfig},
	identity::{PinnedLibrary, backend_toolchain_identity, label, library_identity, pci_accelerator_present, pci_surface, selected_library},
	native::Backend,
};
...
pub(crate) struct HsaBackend {
	...
	runtime: RefCell<Option<HsaRuntimeState>>,
}

struct HsaRuntimeState {
...
impl HsaBackend {
	pub(crate) fn new(config: &NativeProbeConfig) -> ProbeResult<Self> {
		if config.hsa.code_object_version == 0 {
			return Err(ProbeError::Discovery(
				"HSA code-object version must be nonzero".to_owned(),
			));
		}
		Ok(Self {
			library: config.hsa.library.clone(),
			host_memory_key: config.host_memory_key.clone(),
			pci_sysfs_root: config.pci_sysfs_root.clone(),
			code_object_version: config.hsa.code_object_version,
			kernels: config.kernels.clone(),
			runtime: RefCell::new(None),
		})
	}

	pub(crate) fn with_runtime<T>(&self, operation: impl FnOnce(&PinnedLibrary, &Runtime) -> ProbeResult<T>) -> ProbeResult<Option<T>> {
		if !pci_accelerator_present(&self.pci_sysfs_root, 0x1002)? {
		...
		let mut state = self.runtime.borrow_mut();
		...
		if state.is_none() {
			let runtime = Runtime::open(library.path.as_os_str()).map_err(|error| ProbeError::Discovery(format!("load ROCr/HSA runtime: {error}")))?;
	...
	pub(crate) fn descriptor(&self, library: &PinnedLibrary, system: &SystemDescription, description: &AgentDescription) -> ProbeResult<Option<GpuDescriptor>> {
		if description.device_type != DeviceType::Gpu {
			return Ok(None);
		}
		if !description.supports_kernel_dispatch() {
			return Err(ProbeError::Discovery(format!(
				"HSA GPU {} lacks kernel-dispatch capability",
				description.identity.uuid
			)));
		}
		if !matches!(description.identity.uuid.body(), AgentUuidBody::Value(_)) {
			return Err(ProbeError::Discovery(format!(
				"HSA GPU at agent name {:?} did not report a stable UUID",
				description.identity.name
			)));
		}
		let pci = description.identity.pci_address.ok_or_else(|| {
		...
		let surface = pci_surface(&self.pci_sysfs_root, &pci)?;
		let target = exact_target(description)?;
		let target_tail = hsa_target_tail(target)?;
		...
		let name = if gpu.product_name.trim().is_empty() {
			description.identity.name.clone()
		} else {
		...
		let maximum_shared_memory_per_workgroup = driver_lds_capacity(description.identity.driver_node_id.ok_or_else(|| {
			ProbeError::Discovery(format!(
		...
		Ok(Some(GpuDescriptor {
			...
			capacity_hint: capacity(hsa_capacity(description)?)?,
			driver: label(
				format!(
					"amdgpu-kfd-node-{}:{}",
					description.identity.driver_node_id.ok_or_else(|| {
						ProbeError::Discovery(format!(
			...
			runtime_abi: label(
				format!(
					"hsa-{}.{}-amdext-{}.{}:{runtime}",
					system.hsa_version_major, system.hsa_version_minor, system.amd_extension_version_major, system.amd_extension_version_minor
				),
			...
			host_to_device_maximum_inflight: TransferLaneCount::new(1).map_err(|error| ProbeError::Discovery(format!("HSA host-to-device lane count: {error}")))?,
			device_to_host_maximum_inflight: TransferLaneCount::new(1).map_err(|error| ProbeError::Discovery(format!("HSA device-to-host lane count: {error}")))?,
	...
	fn benchmark_device(&self, expected: &GpuDescriptor, plan: BoundedBenchmarkPlan) -> ProbeResult<GpuMeasurement> {
		...
		let Some(measurement) = result else {
	...
	fn benchmark_with_runtime(&self, library: &PinnedLibrary, runtime: &Runtime, expected: &GpuDescriptor, plan: BoundedBenchmarkPlan) -> ProbeResult<GpuMeasurement> {
		...
		let selected_uuid = discovery
			...
			.filter_map(|agent| {
				match self.descriptor(library, &system, agent.description()) {
					Ok(Some(descriptor)) if descriptor == *expected => Some(Ok(agent.description().identity.uuid.as_str().to_owned())),
		...
		let selected_uuid = &selected_uuid[0];
		let agents = discovery.into_agents();
		let gpu_numa = agents
			.iter()
		...
		let capacity_bytes = hsa_capacity(gpu.description())?;
		let target = exact_target(gpu.description())?.clone();
		let session = gpu
			.into_session()
			.map_err(|error| ProbeError::Benchmark(format!("create HSA GPU session: {error}")))?;
		let bytes = plan_bytes(plan)?;
		if u64::try_from(bytes).map_err(|_| ProbeError::Benchmark("HSA allocation size does not fit u64".to_owned()))? > capacity_bytes {
			return Err(ProbeError::Benchmark(format!(
		...
		let mut source_bytes = vec![0_u8; bytes];
		fill_input(&mut source_bytes);
		...
		let mut observed = vec![0_u8; bytes];
		copy_hsa_to_host(&host_destination, &mut observed)?;
		...
		let bytes_u64 = u64::try_from(bytes).map_err(|_| ProbeError::Benchmark("HSA transfer size does not fit u64".to_owned()))?;
		Ok(GpuMeasurement {
			capacity: measured(capacity(capacity_bytes)?),
			calculation_rate: measured(calculation),
			memory_rate: measured(transfer_rate(bytes_u64, d2d)?),
			host_to_device_rate: measured(transfer_rate(bytes_u64, h2d)?),
			device_to_host_rate: measured(transfer_rate(bytes_u64, d2h)?),
		})
	}

	#[allow(clippy::too_many_arguments)]
	fn benchmark_calculation(&self, cpu: &DiscoveredAgent<'_>, session: &Session<'_>, isa_target: &IsaTarget, host_output: &recipe_hsa::Allocation<'_>, input: &recipe_hsa::Allocation<'_>, output: &recipe_hsa::Allocation<'_>, input_bytes: &[u8], output_bytes: &mut [u8], plan: BoundedBenchmarkPlan) -> ProbeResult<recipe_core::FlopsPerSecond> {
		let bytes = compute_buffer_bytes(plan)?;
		let elements = u32::try_from(bytes / 4).map_err(|_| ProbeError::Benchmark("HSA element count does not fit u32".to_owned()))?;
		let lanes = hsa_workgroup_lanes(session.description(), elements)?;
		let target = KernelTarget::Amd(AmdTarget {
			target_id: hsa_target_tail(isa_target)?.to_owned(),
		...
		let kernel = executable
			.kernel(&inspection.kernel.symbol)
		...
		let kernarg_size = usize::try_from(inspection.kernel.kernarg_segment_size).map_err(|_| ProbeError::Benchmark("HSA kernarg size does not fit usize".to_owned()))?;
		let kernarg = cpu
			.allocate_kernarg(kernarg_size)
			.map_err(|error| hsa_benchmark_error("allocate HSA kernarg", error))?;
		session
			.grant_access(&kernarg)
			.map_err(|error| hsa_benchmark_error("grant GPU kernarg access", error))?;
		let mut arguments = vec![0_u8; kernarg_size];
		write_u64(&mut arguments, 0, pointer_address(input)?)?;
		write_u64(&mut arguments, 8, pointer_address(output)?)?;
		write_u64(&mut arguments, 16, lowered.abi.elements.get())?;
		// SAFETY: `allocate_kernarg` selected a CPU host-accessible kernarg
		// pool and dispatch has not yet been published.
		unsafe {
			kernarg
				.copy_from_host(0, &arguments)
				.map_err(|error| hsa_benchmark_error("initialize HSA kernarg", error))?;
		}
		let queue_capability = session
			.description()
			.queue
			.ok_or_else(|| ProbeError::Benchmark("HSA GPU has no user-mode queue".to_owned()))?;
		let queue = session
			.create_queue(QueueConfig::new(
				queue_capability.minimum_packets,
				QueueKind::SingleProducer,
			))
			.map_err(|error| hsa_benchmark_error("create HSA queue", error))?;
		let geometry = DispatchGeometry::one_dimensional(elements, lanes);
		let timed = time_bounded(plan, |remaining| {
			let mut pending = queue
				.dispatch(&kernel, Some(&kernarg), geometry)
		...
		verify_compute_output(&input_bytes[..bytes], &output_bytes[..bytes])?;
		queue.close()
...
fn driver_lds_capacity(node: u32) -> ProbeResult<recipe_core::ByteCount> {
	let path = PathBuf::from("/sys/class/kfd/kfd/topology/nodes")
		.join(node.to_string())
		.join("properties");
	let properties = fs::read_to_string(&path).map_err(|error| ProbeError::io("read KFD GPU properties", &path, error))?;
...
impl Backend for HsaBackend {
	fn discover(&self) -> ProbeResult<Vec<GpuDescriptor>> {
		Ok(self
			.with_runtime(|library, runtime| {
				let discovery = runtime
					.discover()
					.map_err(|error| ProbeError::Discovery(format!("exhaustive HSA discovery: {error}")))?;
				let mut devices = Vec::new();
				for agent in discovery.agents() {
					if let Some(descriptor) = self.descriptor(library, discovery.system(), agent.description())? {
...
pub(crate) fn exact_target(description: &AgentDescription) -> ProbeResult<&IsaTarget> {
	let targets = description
		.isas
		.iter()
		.map(|isa| {
			isa.amd_target.as_ref().ok_or_else(|| {
				ProbeError::Discovery(format!(
	...
	let identities = targets
		...
		.collect::<BTreeSet<_>>();
	let specific = targets
		.iter()
		.copied()
		.filter(|target| !target.architecture().ends_with("-generic"))
		.collect::<Vec<_>>();
	if specific.len() == 1 {
		return Ok(specific[0]);
	}
	if specific.is_empty() && identities.len() == 1 {
		return targets.into_iter().next().ok_or_else(|| {
			ProbeError::Discovery(format!(
...
fn hsa_capacity(description: &AgentDescription) -> ProbeResult<u64> {
	let local_pool_bytes = description
		.memory_pools
		.iter()
		.filter(|pool| {
			pool.segment == MemorySegment::Global
				&& pool.location == Some(MemoryLocation::Gpu)
				&& pool.runtime_allocation.is_some()
				&& pool
					.global_flags
					.is_some_and(|flags| flags.contains(MemoryPoolFlags::COARSE_GRAINED))
		})
		.map(|pool| pool.size_bytes)
		.max();
	match local_pool_bytes {
		Some(bytes) => u64::try_from(bytes).map_err(|_| ProbeError::Discovery("HSA local-memory pool size does not fit u64".to_owned())),
		None => {
...
fn hsa_target_tail(target: &IsaTarget) -> ProbeResult<&str> {
	target.as_str()
...
fn select_agents<'runtime>(agents: Vec<DiscoveredAgent<'runtime>>, gpu_uuid: &str, gpu_numa: u32) -> ProbeResult<(DiscoveredAgent<'runtime>, DiscoveredAgent<'runtime>)> {
	let mut gpu = None;
	let mut matching_cpu = None;
	let mut fallback_cpu = None;
	for agent in agents {
		let description = agent.description();
		if description.identity.uuid.as_str() == gpu_uuid {
			if gpu.is_some() {
				return Err(ProbeError::Benchmark(
			...
			gpu = Some(agent);
		} else if description.device_type == DeviceType::Cpu {
			if description.identity.numa_node_id == gpu_numa && matching_cpu.is_none() {
				matching_cpu = Some(agent);
			} else if fallback_cpu.is_none() {
				fallback_cpu = Some(agent);
			}
		}
	}
	let gpu = gpu.ok_or_else(|| ProbeError::Benchmark("selected HSA GPU is missing".to_owned()))?;
	let cpu = matching_cpu
		.or(fallback_cpu)
		.ok_or_else(|| ProbeError::Benchmark("ROCr exposed no CPU memory agent for GPU transfers".to_owned()))?;
	Ok((cpu, gpu))
}

fn hsa_workgroup_lanes(description: &AgentDescription, elements: u32) -> ProbeResult<u16> {
...
fn copy_hsa(session: &Session<'_>, destination: &recipe_hsa::Allocation<'_>, source: &recipe_hsa::Allocation<'_>, bytes: usize, timeout: Duration, operation: &'static str) -> ProbeResult<()> {
	let mut pending = session
		.copy_async(destination, 0, source, 0, bytes)
...
fn pointer_address(allocation: &recipe_hsa::Allocation<'_>) -> ProbeResult<u64> { u64::try_from(allocation.as_ptr() as usize).map_err(|_| ProbeError::Benchmark("HSA pointer does not fit u64".to_owned())) }

fn write_u64(destination: &mut [u8], offset: usize, value: u64) -> ProbeResult<()> {
	let end = offset
		.checked_add(8)
...
fn hsa_benchmark_error(operation: &'static str, error: recipe_hsa::Error) -> ProbeError { ProbeError::Benchmark(format!("{operation}: {error}")) }

fn kernel_benchmark_error(error: recipe_kernel::LoweringError) -> ProbeError { ProbeError::Benchmark(format!("Recipe-owned GPU artifact: {error}")) }
