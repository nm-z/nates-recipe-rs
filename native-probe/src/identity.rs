...
pub(crate) fn selected_library(library: &BackendLibrary, backend: &str) -> ProbeResult<Option<PinnedLibrary>> {
	...
	for candidate in &library.candidates {
		...
		let canonical = fs::canonicalize(candidate).map_err(|error| ProbeError::io("canonicalize native backend library", candidate, error))?;
		if !seen.insert(canonical.clone()) {
			continue;
		}
		let canonical_metadata = fs::metadata(&canonical).map_err(|error| ProbeError::io("inspect native backend library target", &canonical, error))?;
		...
		let bytes = fs::read(&canonical).map_err(|error| ProbeError::io("read native backend library", &canonical, error))?;
...
pub(crate) fn label(value: impl Into<String>, field: &str) -> ProbeResult<Label> { Label::new(value).map_err(|error| ProbeError::Discovery(format!("{field}: {error}"))) }
...
pub(crate) fn backend_toolchain_identity(toolchain: &OfflineToolchain, release: &Label, backend: &str, target_configuration: &str) -> ProbeResult<ToolchainIdentity> {
...
fn required_tools<'a>(toolchain: &'a OfflineToolchain, backend: &str) -> ProbeResult<Vec<&'a PinnedTool>> {
	...
	match backend {
		"amd-hsa" => {
			tools.push(toolchain
				.elf_linker
				.as_ref()
				.ok_or_else(|| ProbeError::Discovery("AMD probing requires an explicitly pinned ELF linker".to_owned()))?)
		}
		"nvidia-cuda" => {
			tools.push(toolchain
				.ptx_assembler
				.as_ref()
				.ok_or_else(|| ProbeError::Discovery("NVIDIA probing requires an explicitly pinned PTX assembler".to_owned()))?)
		}
		_ => {
			return Err(ProbeError::Discovery(format!(
...
pub(crate) fn pci_surface(root: &Path, bdf: &str) -> ProbeResult<PciSurface> {
	...
	let metadata = fs::metadata(&device_root).map_err(|error| ProbeError::io("inspect PCI device", &device_root, error))?;
	if !metadata.is_dir() {
		return Err(ProbeError::Discovery(format!(
	...
	let driver_target = fs::canonicalize(device_root.join("driver")).map_err(|error| ProbeError::io("resolve PCI driver", device_root.join("driver"), error))?;
	let driver_files = [
...
fn surface_digest<'a>(domain: &str, prefix: Option<&[u8]>, paths: impl IntoIterator<Item = &'a PathBuf>, require_one_file: bool) -> ProbeResult<String> {
	let mut digest = Sha256::new();
