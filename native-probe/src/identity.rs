...
use recipe_kernel::{ArtifactDigest, OfflineToolchain, PinnedTool}; use recipe_probe::{ProbeError, ProbeResult};
...
pub(crate) struct PinnedLibrary { pub path: PathBuf, pub digest: [u8; 32], }

pub(crate) fn selected_library(library: &BackendLibrary, backend: &str) -> ProbeResult<Option<PinnedLibrary>> {
	let mut seen = BTreeSet::new(); let mut selected = None; for candidate in &library.candidates {
		if !candidate.is_absolute() { return Err(ProbeError::Discovery(format!(
				"{backend} library candidate {} is not absolute",
				candidate.display() ))); }
		let metadata = match fs::symlink_metadata(candidate) { Ok(metadata) => metadata,
			Err(error) if error.kind() == std::io::ErrorKind::NotFound => continue, Err(error) => { return Err(ProbeError::io(
					"inspect native backend library",
					candidate, error, )); } }; if !(metadata.file_type().is_file() || metadata.file_type().is_symlink()) {
		...
		if !canonical_metadata.is_file() { return Err(ProbeError::Discovery(format!(
				"{backend} library target {} is not a regular file",
				canonical.display() ))); }
		let bytes = fs::read(&canonical)
			.map_err(|error| ProbeError::io("read native backend library", &canonical, error))?;
		let pinned = PinnedLibrary { path: canonical, digest: Sha256::digest(bytes).into(), }; if selected.is_none() {
...
pub(crate) fn hex(bytes: &[u8]) -> String { let mut output = String::with_capacity(bytes.len() * 2); for byte in bytes {
		use std::fmt::Write as _;
		write!(output, "{byte:02x}").expect("writing to an in-memory String cannot fail");
	}
	output }

pub(crate) fn backend_toolchain_identity( toolchain: &OfflineToolchain, release: &Label, backend: &str,
	target_configuration: &str, ) -> ProbeResult<ToolchainIdentity> { let mut digest = Sha256::new();
	digest.update(b"recipe-native-probe-toolchain-and-benchmark-v2");
	digest.update((backend.len() as u64).to_le_bytes()); digest.update(backend.as_bytes());
	digest.update((release.as_str().len() as u64).to_le_bytes()); digest.update(release.as_str().as_bytes());
	digest.update((target_configuration.len() as u64).to_le_bytes()); digest.update(target_configuration.as_bytes());
	digest.update((PROBE_SOURCE_DIGEST.len() as u64).to_le_bytes()); digest.update(PROBE_SOURCE_DIGEST.as_bytes());
	for tool in required_tools(toolchain, backend)? { hash_tool(&mut digest, tool); }
	let bytes: [u8; 32] = digest.finalize().into(); Ok(ToolchainIdentity {
		name: label(format!("recipe-owned-llvm-{backend}"), "toolchain name")?,
		version: release.clone(), digest: CoreDigest::new(bytes), }) }

fn required_tools<'a>(toolchain: &'a OfflineToolchain, backend: &str) -> ProbeResult<Vec<&'a PinnedTool>> {
	let mut tools = vec![&toolchain.verifier, &toolchain.llvm_codegen]; match backend {
		"amd-hsa" => {
		...
		_ => { return Err(ProbeError::Discovery(format!(
				"unknown native toolchain backend {backend}"
			))); } }
	Ok(tools) }

fn hash_tool(digest: &mut Sha256, tool: &PinnedTool) { let path = tool.path.as_os_str().as_encoded_bytes();
	digest.update((path.len() as u64).to_le_bytes()); digest.update(path);
	digest.update(artifact_digest_bytes(tool.digest)); }

fn artifact_digest_bytes(digest: ArtifactDigest) -> [u8; 32] { let text = digest.to_hex(); let mut bytes = [0_u8; 32];
	for (index, chunk) in text.as_bytes().chunks_exact(2).enumerate() {
		bytes[index] = hex_nibble(chunk[0]) * 16 + hex_nibble(chunk[1]); }
	bytes }

fn hex_nibble(value: u8) -> u8 { match value {
		b'0'..=b'9' => value - b'0',
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct PciSurface { pub driver: String, pub firmware: String, pub link: String, }
...
pub(crate) fn pci_accelerator_present(root: &Path, vendor: u16) -> ProbeResult<bool> { if !root.is_absolute() {
		return Err(ProbeError::Discovery(format!(
	...
	for entry in entries {
		...
		if actual_vendor == u32::from(vendor) && is_accelerator_class(class) { return Ok(true); } }
	Ok(false) }
...
fn read_pci_hex(path: &Path, field: &str) -> ProbeResult<u32> {
	...
	u32::from_str_radix(digits, 16).map_err(|error| { ProbeError::Discovery(format!(
			"PCI {field} at {} is not hexadecimal: {error}",
			path.display() )) }) }

const fn is_accelerator_class(class: u32) -> bool { matches!(class >> 16, 0x03 | 0x12) }

pub(crate) fn pci_surface(root: &Path, bdf: &str) -> ProbeResult<PciSurface> { if !root.is_absolute() {
		return Err(ProbeError::Discovery(format!(
			"PCI sysfs root {} is not absolute",
			root.display() ))); }
	let device_root = root.join(bdf); let metadata =
		fs::metadata(&device_root).map_err(|error| ProbeError::io("inspect PCI device", &device_root, error))?;
	if !metadata.is_dir() { return Err(ProbeError::Discovery(format!(
	...
	let driver_files = [
		...
		"driver",
		Some(driver_target.as_os_str().as_encoded_bytes()), driver_files.iter(), true, )?; let firmware_files = [
	...
	Ok(PciSurface { driver, firmware, link, }) }

fn surface_digest<'a>(
	domain: &str, prefix: Option<&[u8]>,
	paths: impl IntoIterator<Item = &'a PathBuf>,
	require_one_file: bool, ) -> ProbeResult<String> { let mut digest = Sha256::new();
	digest.update(b"recipe-native-probe-surface-v1");
	digest.update(domain.as_bytes()); if let Some(prefix) = prefix { digest.update((prefix.len() as u64).to_le_bytes());
		digest.update(prefix); }
	let mut found = 0_u64; for path in paths { let bytes = match fs::read(path) { Ok(bytes) => bytes, Err(error)
		...
		}; found += 1; let encoded = path.as_os_str().as_encoded_bytes(); digest.update((encoded.len() as u64).to_le_bytes());
		digest.update(encoded); digest.update((bytes.len() as u64).to_le_bytes()); digest.update(bytes); }
	if require_one_file && found == 0 { return Err(ProbeError::Discovery(format!(
...
pub(crate) fn library_identity(domain: &str, library: &PinnedLibrary) -> String { format!(
		"{domain}:{}:sha256={}",
		library.path.display(), hex(&library.digest) ) }
