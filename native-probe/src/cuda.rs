use core::ffi::c_void; use std::{ path::PathBuf, time::{Duration, Instant}, };

use recipe_core::{Label, TargetIdentity, TransferLaneCount, TransportKind}; use recipe_cuda::{
	CompletionStatus, Context, ContextFlags, DeviceBuffer, DeviceInfo, Dim3, Discovery, Driver, Event, LaunchConfig,
	Module, Pending, PinnedHostBuffer, SchedulingPolicy, Stream, }; use recipe_kernel::{
...
#[derive(Clone, Debug)]
pub(crate) struct CudaBackend { library: BackendLibrary, host_memory_key: Label, pci_sysfs_root: PathBuf, ptx_isa: u16,
	kernels: KernelBuildConfig, }

impl CudaBackend { pub(crate) fn new(config: &NativeProbeConfig) -> ProbeResult<Self> { Ok(Self {
			library: config.cuda.library.clone(), host_memory_key: config.host_memory_key.clone(),
			pci_sysfs_root: config.pci_sysfs_root.clone(), ptx_isa: config.cuda.ptx_isa, kernels: config.kernels.clone(), }) }

	pub(crate) fn open(&self) -> ProbeResult<Option<(PinnedLibrary, Driver, Discovery)>> {
		if !pci_accelerator_present(&self.pci_sysfs_root, 0x10de)? { return Ok(None); }
		let Some(library) = selected_library(&self.library, "CUDA Driver")? else {
			...
			)); }; let path = library.path.to_str().ok_or_else(|| { ProbeError::Discovery(format!(
				"CUDA Driver path {} is not valid UTF-8",
				library.path.display() )) })?; let driver = Driver::load_from_path(path)
			.map_err(|error| ProbeError::Discovery(format!("load CUDA Driver: {error}")))?;
		let discovery = driver .discover()
			.map_err(|error| ProbeError::Discovery(format!("exhaustive CUDA discovery: {error}")))?;
		Ok(Some((library, driver, discovery))) }

	pub(crate) fn descriptor( &self, library: &PinnedLibrary, discovery: &Discovery, device: &DeviceInfo,
	) -> ProbeResult<GpuDescriptor> { let pci = parse_pci_bus_id(&device.pci_bus_id)?;
		if pci.domain != device.attributes.pci_domain_id || pci.bus != device.attributes.pci_bus_id
		...
		{ return Err(ProbeError::Discovery(format!(
				...
				device.pci_bus_id, device.attributes.pci_domain_id, device.attributes.pci_bus_id, device.attributes.pci_device_id
			))); }
		...
		let sysfs_bdf = pci.sysfs_bdf(); let surface = pci_surface(&self.pci_sysfs_root, &sysfs_bdf)?;
		let target = NvidiaTarget { sm_major: u8::try_from(device.compute_capability.major).map_err(|_| {
				ProbeError::Discovery(format!(
					"CUDA compute capability major {} does not fit u8",
					device.compute_capability.major )) })?, sm_minor: u8::try_from(device.compute_capability.minor).map_err(|_| {
				ProbeError::Discovery(format!(
					"CUDA compute capability minor {} does not fit u8",
					device.compute_capability.minor )) })?, ptx_isa: self.ptx_isa, }; target.validate()
			.map_err(|error| ProbeError::Discovery(format!("CUDA artifact target: {error}")))?;
		let architecture = target.architecture(); let toolchain = backend_toolchain_identity( &self.kernels.toolchain,
		...
		Ok(GpuDescriptor {
			...
			}, capacity_hint: capacity(device.total_memory_bytes)?, driver: label( format!(
					"cuda-kernel-driver:{}:{}",
					discovery.driver_version.raw(), surface.driver ),
			...
			asynchronous_submission: true, maximum_submission_queues: CUDA_MAXIMUM_SUBMISSION_QUEUES,
			maximum_concurrent_tasks: 1, subgroup_lanes: device.attributes.warp_size,
			...
			transfer_overlaps_calculation: device.attributes.async_engine_count != 0 && device.attributes.concurrent_kernels, })
	}

	fn matching_device<'a>(
		&self, library: &PinnedLibrary,
		discovery: &'a Discovery,
		expected: &GpuDescriptor,
	) -> ProbeResult<&'a DeviceInfo> {
		let mut matched = None; for device in &discovery.devices {
			if self.descriptor(library, discovery, device)? == *expected { if matched.is_some() {
					return Err(ProbeError::Benchmark(format!(
						"CUDA descriptor {} is not unique",
						expected.key ))); }
				matched = Some(device); } }
		matched.ok_or_else(|| { ProbeError::Benchmark(format!(
	...
	fn benchmark_open(&self, expected: &GpuDescriptor) -> ProbeResult<(Driver, Discovery, DeviceInfo)> {
		let Some((library, driver, discovery)) = self.open()? else { return Err(ProbeError::Benchmark(
				"CUDA backend disappeared before benchmark".to_owned(),
			)); }; let device = self .matching_device(&library, &discovery, expected)? .clone(); Ok((driver, discovery, device))
	}
	...
	fn benchmark_device(&self, device: &GpuDescriptor, plan: BoundedBenchmarkPlan) -> ProbeResult<GpuMeasurement> {
		...
		let context = Context::create(&driver, &info, ContextFlags::new(SchedulingPolicy::Yield))
			.map_err(cuda_benchmark_error)?; let stream = Stream::create_nonblocking(&context).map_err(cuda_benchmark_error)?;
		...
		})?; if host_source.as_slice() != host_destination.as_slice() { return Err(ProbeError::Benchmark(
		...
		})?; copy_cuda_to_host( &stream, &context, &mut host_destination, &device_destination, bytes, plan.maximum_duration,
		)?; if host_source.as_slice() != host_destination.as_slice() { return Err(ProbeError::Benchmark(
				"CUDA device-memory transfer verification failed".to_owned(),
			)); }

		let calculation = self.benchmark_calculation( &context, &stream, &device_source, &device_destination, &host_source,
			&mut host_destination, plan, )?;

		Ok(GpuMeasurement { capacity: measured(capacity(info.total_memory_bytes)?), calculation_rate: measured(calculation),
			memory_rate: measured(transfer_rate( u64::try_from(bytes)
					.map_err(|_| ProbeError::Benchmark("CUDA transfer size does not fit u64".to_owned()))?,
				d2d, )?), host_to_device_rate: measured(transfer_rate( u64::try_from(bytes)
					.map_err(|_| ProbeError::Benchmark("CUDA transfer size does not fit u64".to_owned()))?,
				h2d, )?), device_to_host_rate: measured(transfer_rate( u64::try_from(bytes)
					.map_err(|_| ProbeError::Benchmark("CUDA transfer size does not fit u64".to_owned()))?,
				d2h, )?), }) }
	...
	fn benchmark_calculation<'ctx>(
		...
		plan: BoundedBenchmarkPlan, ) -> ProbeResult<recipe_core::FlopsPerSecond> { let bytes = compute_buffer_bytes(plan)?;
		...
		let workgroup_lanes = cuda_workgroup_lanes( context.device().attributes.maximum_threads_per_block, elements, )?;
		let target = KernelTarget::Nvidia(NvidiaTarget { sm_major: u8::try_from(context.device().compute_capability.major)
			...
			ptx_isa: self.ptx_isa, }); let lowered = lower_elementwise(&template, &target, &LoweringOptions {
			entry_symbol: CUDA_KERNEL_ENTRY.to_owned(), workgroup_lanes, }) .map_err(kernel_benchmark_error)?;
		...
		let artifact = builder .build( BuildPhase::Realize, &lowered, &target, &self.kernels.scratch_parent, )
			.map_err(kernel_benchmark_error)?; let BuiltArtifact::Cubin { bytes: cubin, inspection, .. } = artifact
		else { return Err(ProbeError::Benchmark(
				"CUDA benchmark builder returned a non-cubin artifact".to_owned(),
			)); }; if inspection.entry_symbol != CUDA_KERNEL_ENTRY { return Err(ProbeError::Benchmark(
				"CUDA benchmark cubin entry identity changed".to_owned(),
			)); }
		let module = Module::load_cubin(context, &cubin).map_err(cuda_benchmark_error)?; let function = module
			.function(CUDA_KERNEL_ENTRY) .map_err(cuda_benchmark_error)?; let elements = lowered.abi.elements.get();
		let blocks = elements .checked_add(u64::from(workgroup_lanes) - 1)
			.ok_or_else(|| ProbeError::Benchmark("CUDA grid calculation overflowed".to_owned()))?
			/ u64::from(workgroup_lanes); let grid_x =
			u32::try_from(blocks).map_err(|_| ProbeError::Benchmark("CUDA grid does not fit u32".to_owned()))?;
		let launch = LaunchConfig::new( Dim3::new(grid_x, 1, 1).map_err(cuda_benchmark_error)?,
			Dim3::new(workgroup_lanes, 1, 1).map_err(cuda_benchmark_error)?, ); let timed = time_bounded(plan, |remaining| {
			let mut input_address = input.device_ptr().map_err(cuda_benchmark_error)?;
			let mut output_address = output.device_ptr().map_err(cuda_benchmark_error)?; let mut element_count = elements;
			let mut parameters = [ (&raw mut input_address).cast::<c_void>(), (&raw mut output_address).cast::<c_void>(),
				(&raw mut element_count).cast::<c_void>(), ]; let keepalive = [input, output];
			let event = Event::create_completion(context).map_err(cuda_benchmark_error)?;
		...
		})?; copy_cuda_to_host( stream, context, host_output, output, bytes, plan.maximum_duration, )?; verify_compute_output(
			&host_input.as_slice()[..bytes], &host_output.as_slice()[..bytes], )?; calculation_rate(lowered.work, timed) } }

impl Backend for CudaBackend { fn discover(&self) -> ProbeResult<Vec<GpuDescriptor>> {
		let Some((library, _driver, discovery)) = self.open()? else { return Ok(Vec::new()); }; discovery .devices .iter()
			.map(|device| self.descriptor(&library, &discovery, device)) .collect() }
...
fn copy_cuda_to_host<'op, 'ctx>(
	...
	bytes: usize, timeout: Duration, ) -> ProbeResult<()> {
	let event = Event::create_completion(context).map_err(cuda_benchmark_error)?;
	...
	let pending = unsafe { stream.copy_d2h(destination, 0, source, 0, bytes, event) }.map_err(cuda_benchmark_error)?;
...
fn complete_cuda(mut pending: Pending<'_, '_>, timeout: Duration, operation: &'static str) -> ProbeResult<()> {
	let start = Instant::now(); let mut backoff = CompletionPollBackoff::benchmark(); loop {
		match pending.poll().map_err(cuda_benchmark_error)? { CompletionStatus::Complete => {
				drop(pending.recycle_event().map_err(cuda_benchmark_error)?); return Ok(()); }
	...
	let mut cleanup_backoff = CompletionPollBackoff::timed_out_cleanup(); loop {
		cleanup_backoff.wait(TIMED_OUT_CLEANUP_MAXIMUM_DELAY); match pending.poll().map_err(cuda_benchmark_error)? {
			CompletionStatus::Complete => { drop(pending.recycle_event().map_err(cuda_benchmark_error)?);
				return Err(ProbeError::Benchmark(format!(
...
fn cuda_workgroup_lanes(maximum_workgroup_lanes: u32, elements: u64) -> ProbeResult<u32> {
	...
	let maximum = maximum_workgroup_lanes.min(elements); if maximum == 0 { return Err(ProbeError::Benchmark(
			"CUDA workgroup limit or benchmark element count is zero".to_owned(),
		)); }
	Ok(1_u32 << (31 - maximum.leading_zeros())) }

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct ParsedPci { domain: u32, bus: u32, device: u32, function: u32, }

impl ParsedPci { fn sysfs_bdf(self) -> String { format!(
			"{:04x}:{:02x}:{:02x}.{}",
			self.domain, self.bus, self.device, self.function ) } }

fn parse_pci_bus_id(value: &str) -> ProbeResult<ParsedPci> { let (domain, tail) = value
		.split_once(':')
	...
	let parse_hex = |component: &str, field: &str| { u32::from_str_radix(component, 16).map_err(|error| {
			ProbeError::Discovery(format!(
				"CUDA PCI {field} {component:?} is invalid: {error}"
			)) }) }; let function = function.parse::<u32>().map_err(|error| { ProbeError::Discovery(format!(
			"CUDA PCI function {function:?} is invalid: {error}"
		)) })?; if function > 7 { return Err(ProbeError::Discovery(format!(
