...
use recipe_cuda::{CompletionStatus, Context, ContextFlags, DeviceBuffer, DeviceInfo, Dim3, Discovery, Driver, Event, LaunchConfig, Module, Pending, PinnedHostBuffer, SchedulingPolicy, Stream};
use recipe_kernel::{ArtifactBuilder, BuildPhase, BuiltArtifact, KernelTarget, LoweringOptions, NvidiaTarget, lower_elementwise};
...
impl CudaBackend {
	...
	pub(crate) fn open(&self) -> ProbeResult<Option<(PinnedLibrary, Driver, Discovery)>> {
		...
		let driver = Driver::load_from_path(path).map_err(|error| ProbeError::Discovery(format!("load CUDA Driver: {error}")))?;
	...
	pub(crate) fn descriptor(&self, library: &PinnedLibrary, discovery: &Discovery, device: &DeviceInfo) -> ProbeResult<GpuDescriptor> {
		let pci = parse_pci_bus_id(&device.pci_bus_id)?;
		if pci.domain != device.attributes.pci_domain_id || pci.bus != device.attributes.pci_bus_id || pci.device != device.attributes.pci_device_id {
		...
		Ok(GpuDescriptor {
			...
			host_to_device_maximum_inflight: TransferLaneCount::new(1).map_err(|error| ProbeError::Discovery(format!("CUDA host-to-device lane count: {error}")))?,
			device_to_host_maximum_inflight: TransferLaneCount::new(1).map_err(|error| ProbeError::Discovery(format!("CUDA device-to-host lane count: {error}")))?,
	...
	fn matching_device<'a>(&self, library: &PinnedLibrary, discovery: &'a Discovery, expected: &GpuDescriptor) -> ProbeResult<&'a DeviceInfo> {
	...
	fn benchmark_device(&self, device: &GpuDescriptor, plan: BoundedBenchmarkPlan) -> ProbeResult<GpuMeasurement> {
		...
		let context = Context::create(&driver, &info, ContextFlags::new(SchedulingPolicy::Yield)).map_err(cuda_benchmark_error)?;
		let stream = Stream::create_nonblocking(&context).map_err(cuda_benchmark_error)?;
		let bytes = plan_bytes(plan)?;
		if u64::try_from(bytes).map_err(|_| ProbeError::Benchmark("CUDA allocation size does not fit u64".to_owned()))? > info.total_memory_bytes {
			return Err(ProbeError::Benchmark(format!(
		...
		let h2d = time_bounded(plan, |remaining| {
			...
			let pending = unsafe { stream.copy_h2d(&device_source, 0, &host_source, 0, bytes, event) }.map_err(cuda_benchmark_error)?;
		...
		let d2h = time_bounded(plan, |remaining| {
			...
			// SAFETY: all ranges cover live same-context allocations; the
			// returned token is synchronously driven to terminal completion.
			let pending = unsafe { stream.copy_d2h(&mut host_destination, 0, &device_source, 0, bytes, event) }.map_err(cuda_benchmark_error)?;
		...
		let d2d = time_bounded(plan, |remaining| {
			...
			// SAFETY: all ranges cover live same-context allocations; the
			// returned token is synchronously driven to terminal completion.
			let pending = unsafe { stream.copy_d2d(&device_destination, 0, &device_source, 0, bytes, event) }.map_err(cuda_benchmark_error)?;
		...
		Ok(GpuMeasurement {
			...
			memory_rate: measured(transfer_rate(
				u64::try_from(bytes).map_err(|_| ProbeError::Benchmark("CUDA transfer size does not fit u64".to_owned()))?,
				d2d,
			)?),
	...
	fn benchmark_calculation<'ctx>(&self, context: &'ctx Context, stream: &Stream<'ctx>, input: &DeviceBuffer<'ctx>, output: &DeviceBuffer<'ctx>, host_input: &PinnedHostBuffer<'ctx>, host_output: &mut PinnedHostBuffer<'ctx>, plan: BoundedBenchmarkPlan) -> ProbeResult<recipe_core::FlopsPerSecond> {
		let bytes = compute_buffer_bytes(plan)?;
		let template = fma_template(bytes, self.kernels.fma_chain_length)?;
		let elements = u64::try_from(bytes / core::mem::size_of::<f32>()).map_err(|_| ProbeError::Benchmark("CUDA benchmark element count does not fit u64".to_owned()))?;
		...
		let target = KernelTarget::Nvidia(NvidiaTarget {
			sm_major: u8::try_from(context.device().compute_capability.major).map_err(|_| ProbeError::Benchmark("CUDA SM major does not fit u8".to_owned()))?,
			sm_minor: u8::try_from(context.device().compute_capability.minor).map_err(|_| ProbeError::Benchmark("CUDA SM minor does not fit u8".to_owned()))?,
			ptx_isa: self.ptx_isa,
		});
		...
		let grid_x = u32::try_from(blocks).map_err(|_| ProbeError::Benchmark("CUDA grid does not fit u32".to_owned()))?;
		...
		let timed = time_bounded(plan, |remaining| {
			...
			let pending = unsafe { stream.launch(&function, launch, &mut parameters, &keepalive, event) }.map_err(cuda_benchmark_error)?;
...
fn copy_cuda_to_host<'op, 'ctx>(stream: &'op Stream<'ctx>, context: &'ctx Context, destination: &'op mut PinnedHostBuffer<'ctx>, source: &'op DeviceBuffer<'ctx>, bytes: usize, timeout: Duration) -> ProbeResult<()> {
	let event = Event::create_completion(context).map_err(cuda_benchmark_error)?;
...
fn cuda_benchmark_error(error: recipe_cuda::CudaError) -> ProbeError { ProbeError::Benchmark(format!("CUDA native operation: {error}")) }
...
fn cuda_workgroup_lanes(maximum_workgroup_lanes: u32, elements: u64) -> ProbeResult<u32> {
	let elements = u32::try_from(elements.min(u64::from(u32::MAX))).map_err(|_| ProbeError::Benchmark("CUDA benchmark element count does not fit u32".to_owned()))?;
