...
use recipe_core::{DeviceId, MachineId};
...
use recipe_probe::{GpuDiscovery as _, HostInventory, MeasuredProfile, ProbeError, ProbeResult, ResolvedGpuDevice};
...
pub fn with_native_execution_bindings<T>(probe: &NativeGpuProbe, profile: &MeasuredProfile, host: &HostInventory, operation: impl for<'cuda, 'hsa> FnOnce(NativeExecutionBindings<'cuda, 'hsa>) -> ProbeResult<T>) -> ProbeResult<T> {
...
fn partition_expected(probe: &NativeGpuProbe, resolved: &[ResolvedGpuDevice<'_>]) -> ProbeResult<(BTreeMap<String, ExpectedGpu>, BTreeMap<String, ExpectedGpu>)> {
...
fn realize_hsa<'runtime>(backend: &HsaBackend, library: &crate::identity::PinnedLibrary, system: &recipe_hsa::SystemDescription, agents: Vec<DiscoveredAgent<'runtime>>, expected: &BTreeMap<String, ExpectedGpu>) -> ProbeResult<RealizedHsaSet<'runtime>> {
	...
	for agent in gpu_agents {
		...
		let allocator_numa_nodes = host_allocators
			...
			.collect::<Vec<_>>();
		let host_allocator = select_host_allocator(&allocator_numa_nodes, gpu_numa).map_err(|error| {
			match error {
				HostAllocatorSelectionError::Missing => {
					binding_error(format!(
						"measured HSA device {} has no CPU agent with allocatable fine-grained and kernarg pools",
						expected.key
					))
				}
				HostAllocatorSelectionError::AmbiguousSameNuma { count } => {
					binding_error(format!(
						"measured HSA device {} has {count} same-NUMA CPU host allocators; exact binding is ambiguous",
						expected.key
					))
				}
				HostAllocatorSelectionError::AmbiguousFallback { count } => {
					binding_error(format!(
...
fn select_host_allocator(allocator_numa_nodes: &[u32], gpu_numa_node: u32) -> std::result::Result<usize, HostAllocatorSelectionError> {
...
fn require_all_reopened(backend: &str, expected: &BTreeMap<String, ExpectedGpu>, seen: &BTreeSet<String>) -> ProbeResult<()> {
