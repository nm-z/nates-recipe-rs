...
description: When the spec or a stated rule already determines the answer, implement it — don't surface it as a decision/option. Two recurring forms — (1) never offer spec-violating "alternatives" as a choice; (2) fixing swallowed/unchecked errors is settled policy, never a question to raise.
metadata:
  node_type: memory
  type: feedback
...
- Swallowed errors, ignored return codes, unchecked statuses: fixing them is settled policy — do it as part of the task. Generalize [[feedback_no_cya_optimizer]] / [[feedback_sampler_owns_failures]]. Related: [[feedback_direct_execution]], [[feedback_no_form_when_frustrated]].
