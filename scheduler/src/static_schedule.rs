...
use recipe_core::{CalculationTask, DeviceId, DiscoveryProfile, DuplexMode, DuplexResourceId, LinkId, Nanoseconds, RunPhase, ScheduleWindow, SubmissionSlots, Task, TaskId, TaskKind, Topology, TransferEndpoint, TransferLaneClaim, TransferTask, calculation_time_ceil, transfer_time_ceil};
...
pub fn schedule(topology: &Topology, discovery: &DiscoveryProfile, tasks: &[UnscheduledTask]) -> Result<StaticSchedule, ScheduleError> {
	topology
		.validate()
...
fn prepare_tasks(topology: &Topology, discovery: &DiscoveryProfile, tasks: &[UnscheduledTask]) -> Result<BTreeMap<TaskId, PreparedTask>, ScheduleError> {
...
fn prepare_calculation(topology: &Topology, discovery: &DiscoveryProfile, task: TaskId, calculation: &CalculationTask) -> Result<(Nanoseconds, Vec<Resource>), ScheduleError> {
	...
	let duration = calculation_time_ceil(calculation.work, capability.rate.value)
		.map_err(|error| ScheduleError::new(ScheduleErrorKind::ArithmeticOverflow, error.to_string()).for_task(task))?
		.get()
		.max(1);
...
fn prepare_transfer(topology: &Topology, discovery: &DiscoveryProfile, task: TaskId, transfer: &mut TransferTask) -> Result<(Nanoseconds, Vec<Resource>), ScheduleError> {
	...
	let duration = match (transfer.source, transfer.destination) {
		(
			...
			topology
				...
				.map_err(|errors| ScheduleError::new(ScheduleErrorKind::InvalidTransfer, errors.to_string()).for_task(task))?;
...
fn persist_transfer_lane_claims(task: TaskId, kind: &mut TaskKind, selected_resources: &[Resource]) -> Result<(), ScheduleError> {
	...
	let mut claims = selected_resources
		...
		.filter_map(|resource| {
			match resource {
				...
				Resource::Queue(_) | Resource::Completion(_) | Resource::ComputeLane(_, _) | Resource::HalfDuplexDirection(_, _) | Resource::NoComputeTransferOverlap(_) => None,
			}
		})
		.collect::<Vec<_>>();
	claims.sort();
...
fn critical_path_lengths(tasks: &BTreeMap<TaskId, PreparedTask>, successors: &BTreeMap<TaskId, Vec<TaskId>>, indegrees: &BTreeMap<TaskId, usize>) -> Result<BTreeMap<TaskId, u64>, ScheduleError> {
...
fn reserve_earliest(earliest: Nanoseconds, duration: Nanoseconds, resources: &[Resource], reservations: &BTreeMap<Resource, Vec<ScheduleWindow>>) -> Result<(ScheduleWindow, Vec<Resource>), ScheduleError> {
	let fixed = resources
		.iter()
...
const fn resource_group(resource: &Resource) -> Option<ResourceGroup> {
	match resource {
		...
		Resource::Queue(_) | Resource::Completion(_) | Resource::HalfDuplexDirection(_, _) | Resource::NoComputeTransferOverlap(_) => None,
	}
}

fn first_gap(earliest: Nanoseconds, duration: Nanoseconds, fixed: &[Resource], groups: &BTreeSet<ResourceGroup>, resources: &[Resource], reservations: &BTreeMap<Resource, Vec<ScheduleWindow>>) -> Result<(ScheduleWindow, Vec<Resource>), ScheduleError> {
		...
		for group in groups {
			...
			for lane in lanes {
				match resource_conflict_end(*lane, candidate, reservations) {
					Some(end) => {
						earliest_lane_release = Some(earliest_lane_release.map_or(end, |known: Nanoseconds| known.min(end)));
					}
					None => {
			...
			match first_available {
				...
				None => {
					...
					unavailable_until = Some(unavailable_until.map_or(release, |known: Nanoseconds| known.min(release)));
				}
			}
		}
...
fn resource_conflict_end(resource: Resource, candidate: ScheduleWindow, reservations: &BTreeMap<Resource, Vec<ScheduleWindow>>) -> Option<Nanoseconds> {
	match resource {
		Resource::HalfDuplexDirection(capacity, direction) => {
			reservations
				...
				.filter_map(|(reserved, windows)| {
					match reserved {
						Resource::HalfDuplexDirection(reserved_capacity, reserved_direction) if *reserved_capacity == capacity && *reserved_direction != direction => Some(windows),
						_ => None,
					}
				})
				.flatten()
