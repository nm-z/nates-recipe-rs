...
pub(crate) fn time_bounded(
	plan: BoundedBenchmarkPlan,
	mut submit_and_complete: impl FnMut(Duration) -> ProbeResult<()>,
) -> ProbeResult<TimedWork> {
	if !plan.is_bounded() {
...
pub(crate) fn measured<T>(value: T) -> recipe_core::Property<T> {
	recipe_core::Property::new(value, recipe_core::PropertyProvenance::Measured)
}

pub(crate) fn plan_bytes(plan: BoundedBenchmarkPlan) -> ProbeResult<usize> {
	usize::try_from(plan.buffer_bytes.get())
...
mod tests {
	...
	#[test]
	fn theoretical_size_only_bounds_workload() {
		let large = BoundedBenchmarkPlan {
			buffer_bytes: ByteCount::new(MAXIMUM_COMPUTE_BYTES * 4),
			..plan()
		};
		assert_eq!(
			compute_buffer_bytes(large).expect("bounded compute bytes"),
			usize::try_from(MAXIMUM_COMPUTE_BYTES).expect("test constant fits usize")
		);
		assert_eq!(
			fma_template(4096, 32)
	...
	#[test]
	fn rate_uses_completed_work_not_estimate() {
		let timed = TimedWork {
			iterations: 2,
			elapsed: Duration::from_secs(1),
		};
		assert_eq!(transfer_rate(100, timed).expect("transfer rate").get(), 200);
		assert_eq!(
			calculation_rate(FlopCount::new(50), timed)
				.expect("calculation rate")
				.get(),
			100
		);
	}
	...
	#[test]
	fn timing_counts_only_completed_submissions() {
		let mut calls = 0_u32;
		let timed = time_bounded(plan(), |_remaining| {
			calls += 1;
			Ok(())
		})
		.expect("timed fake work");
		assert_eq!(calls, 3);
		assert_eq!(timed.iterations, 3);
	}
}
