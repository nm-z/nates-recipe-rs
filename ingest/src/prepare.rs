...
use crate::{
	...
	semantic::{TemporalInstant, VectorSemantic, VectorSemanticRule, fit_ordinal_vocabulary, infer_table_vectors_with_semantics, parse_temporal_instant},
};
...
impl VectorSchema {
	#[must_use]
	pub const fn new(source_index: usize, name: Vec<u8>, role: VectorRole, semantic_type: SemanticType, encoding: VectorEncoding, metadata: VectorMetadata) -> Self {
		Self {
			source_index,
			name,
			role,
...
pub fn prepare_table(table: &RawTable, request: &PreparationRequest, model: &impl AmbiguousVectorModel) -> PrepareResult<PreparedDataset> { prepare_table_with_semantics(table, request, model, &[]) }

pub(crate) fn prepare_table_with_semantics(table: &RawTable, request: &PreparationRequest, model: &impl AmbiguousVectorModel, semantics: &[VectorSemanticRule]) -> PrepareResult<PreparedDataset> {
	let (target_indices, excluded_columns, retained_source_rows, excluded_source_rows) = select_rows_and_columns_before_fit(table, request)?;
...
pub fn prepare_inferred_table(table: &RawTable, inferred: &InferredVectorList, request: &PreparationRequest) -> PrepareResult<PreparedDataset> {
	validate_inference(table, inferred)?;
	let (target_indices, excluded_columns, retained_source_rows, excluded_source_rows) = select_rows_and_columns(table, inferred, request)?;
	let train_rows = request
		.train_fraction
		.train_rows(retained_source_rows.len())?;
	prepare_preselected_table(
...
pub fn select_table(table: &RawTable, excluded_columns: impl IntoIterator<Item = ColumnPattern>, excluded_rows: impl IntoIterator<Item = RowPredicate>) -> PrepareResult<RawTable> {
...
fn select_rows_and_columns(table: &RawTable, inferred: &InferredVectorList, request: &PreparationRequest) -> PrepareResult<SelectedRowsAndColumns> {
...
fn select_rows_and_columns_before_fit(table: &RawTable, request: &PreparationRequest) -> PrepareResult<SelectedRowsAndColumns> {
	let names = build_header_name_index(table)?;
	let targets = resolve_targets(request, &names)?;
...
fn fit_semantics_with_predicate_constraints(fit_table: &RawTable, request: &PreparationRequest, semantics: &[VectorSemanticRule]) -> PrepareResult<Vec<VectorSemanticRule>> {
...
fn prepare_preselected_table(table: &RawTable, inferred: &InferredVectorList, targets: &ResolvedTargets, excluded_columns: &BTreeSet<usize>, retained_source_rows: Vec<usize>, excluded_source_rows: Vec<usize>, train_rows: usize) -> PrepareResult<PreparedDataset> {
...
fn resolve_column_exclusions(request: &PreparationRequest, inferred: &InferredVectorList, targets: &BTreeSet<usize>) -> PrepareResult<BTreeSet<usize>> {
...
fn resolve_column_exclusions_from_headers(request: &PreparationRequest, headers: &[Vec<u8>], targets: &BTreeSet<usize>) -> PrepareResult<BTreeSet<usize>> {
	let mut excluded = BTreeSet::new();
	for pattern in &request.excluded_columns {
		let matches = headers
			.iter()
...
fn resolve_predicates<'a>(request: &'a PreparationRequest, inferred: &InferredVectorList, names: &BTreeMap<Vec<u8>, usize>) -> PrepareResult<Vec<ResolvedPredicate<'a>>> {
	request
		.excluded_rows
		.iter()
		.map(|predicate| {
...
fn resolve_predicates_before_fit<'a>(request: &'a PreparationRequest, names: &BTreeMap<Vec<u8>, usize>) -> PrepareResult<Vec<ResolvedPredicate<'a>>> {
	request
		.excluded_rows
		.iter()
		.map(|predicate| {
...
fn validate_predicate_type(predicate: &RowPredicate, semantic_type: SemanticType, encoding: VectorEncoding) -> PrepareResult<()> {
	let compatible = match predicate.literal {
		PredicateLiteral::Signed(_) | PredicateLiteral::Unsigned(_) => semantic_type == SemanticType::Numeric && encoding == VectorEncoding::I32,
...
fn parse_predicate_text<'a>(value: &'a [u8], source_row: usize, resolved: &ResolvedPredicate<'_>) -> PrepareResult<&'a str> { core::str::from_utf8(value).map_err(|error| predicate_value_error(source_row, resolved, error)) }

fn predicate_value_error(source_row: usize, resolved: &ResolvedPredicate<'_>, error: impl fmt::Display) -> PrepareError {
	PrepareError::new(
...
fn fit_vector_schema(table: &RawTable, inferred: &InferredVector, role: VectorRole, fit_source_rows: &[usize]) -> PrepareResult<VectorSchema> {
...
fn apply_vector_schema(table: &RawTable, schema: &VectorSchema, retained_source_rows: &[usize]) -> PrepareResult<PreparedVector> {
...
fn vector_values<'a>(table: &'a RawTable, source_index: usize, name: &[u8], source_rows: &[usize]) -> PrepareResult<Vec<&'a [u8]>> {
	source_rows
		.iter()
		.map(|source_row| {
			table.rows()
...
fn fit_temporal_origin(inferred: &InferredVector, source_rows: &[usize], values: &[&[u8]]) -> PrepareResult<TemporalOrigin> {
	let parsed = values
		.iter()
		.zip(source_rows)
		.map(|(value, source_row)| {
...
fn encode_temporal(schema: &VectorSchema, source_rows: &[usize], values: &[&[u8]], origin: TemporalOrigin) -> PrepareResult<Vec<Option<i32>>> {
...
fn temporal_delta(schema: &VectorSchema, origin: TemporalInstant, value: TemporalInstant, source_row: usize) -> PrepareResult<i32> {
...
fn validate_categorical_alignment(schema: &VectorSchema, dictionary_len: usize, codes: &[Option<i32>], observations: &[CategoricalObservation]) -> PrepareResult<()> {
...
fn encode_dictionary(schema: &VectorSchema, source_rows: &[usize], values: &[&[u8]], dictionary: &[Vec<u8>]) -> PrepareResult<AppliedDictionary> {
...
fn encode_ordinal(schema: &VectorSchema, source_rows: &[usize], values: &[&[u8]], vocabulary: &[Vec<u8>]) -> PrepareResult<Vec<Option<i32>>> {
	values.iter()
		.zip(source_rows)
		.map(|(value, source_row)| {
			if value.is_empty() {
...
fn encode_variable(schema: &VectorSchema, source_rows: &[usize], values: &[&[u8]]) -> PrepareResult<VariableWidthVector> {
...
fn inspect_image_variants(inferred: &InferredVector, source_rows: &[usize], values: &[&[u8]]) -> PrepareResult<Vec<EncodedImageMetadata>> {
	let variants = values
		.iter()
		.zip(source_rows)
...
fn inspect_image_variants_for_schema(schema: &VectorSchema, source_rows: &[usize], values: &[&[u8]]) -> PrepareResult<()> {
	for (value, source_row) in values.iter().zip(source_rows) {
		if !value.is_empty() {
			inspect_encoded_image(value).map_err(|error| {
				encoding_error(
...
fn encoding_error_parts(name: &[u8], encoding: VectorEncoding, source_row: usize, error: impl fmt::Display) -> PrepareError {
	PrepareError::new(
		PrepareErrorKind::EncodingFailure,
...
fn partition(kind: PartitionKind, positions: core::ops::Range<usize>, retained_source_rows: &[usize]) -> PreparedPartition {
