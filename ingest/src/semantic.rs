...
pub(crate) fn infer_table_vectors_with_semantics(table: &RawTable, model: &impl AmbiguousVectorModel, semantics: &[VectorSemanticRule]) -> SemanticResult<InferredVectorList> {
...
fn classify_vector(values: &[&[u8]], evidence: VectorEvidence, model: &impl AmbiguousVectorModel) -> (SemanticType, VectorEncoding) {
...
pub(crate) fn parse_temporal_instant(value: &[u8]) -> Option<TemporalInstant> {
	...
	if bytes.len() < 10 || bytes.get(4) != Some(&b'-') || bytes.get(7) != Some(&b'-') || !valid_decimal_range(&bytes[0..4], 1, 9_999) || !valid_decimal_range(&bytes[5..7], 1, 12) {
		return None;
	}
	...
	if bytes.len() == 10 {
		...
		if bytes.get(13) != Some(&b':') || bytes.get(16) != Some(&b':') || !valid_decimal_range(&bytes[11..13], 0, 23) || !valid_decimal_range(&bytes[14..16], 0, 59) || !valid_decimal_range(&bytes[17..19], 0, 60) {
			return None;
		}
		...
		match bytes.get(cursor) {
			...
			Some(sign @ (b'+' | b'-')) if cursor + 6 == bytes.len() && bytes.get(cursor + 3) == Some(&b':') && valid_decimal_range(&bytes[cursor + 1..cursor + 3], 0, 23) && valid_decimal_range(&bytes[cursor + 4..cursor + 6], 0, 59) => {
