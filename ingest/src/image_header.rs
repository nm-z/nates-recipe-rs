...
impl EncodedImageMetadata {
	#[must_use]
	pub const fn new(format: EncodedImageFormat, width: u32, height: u32, channels: Option<u8>, color_model: Option<ImageColorModel>, sample_bits: Option<u8>) -> Self {
		Self {
			format,
			width,
			height,
...
pub(crate) fn has_recognized_image_signature(bytes: &[u8]) -> bool { bytes.starts_with(b"\x89PNG\r\n\x1a\n") || bytes.starts_with(b"\xff\xd8\xff") || bytes.starts_with(b"GIF87a") || bytes.starts_with(b"GIF89a") || bytes.starts_with(b"BM") || bytes.starts_with(b"RIFF") && bytes.get(8..12) == Some(b"WEBP") }

const fn metadata(format: EncodedImageFormat, width: u32, height: u32, channels: Option<u8>, color_model: Option<ImageColorModel>, sample_bits: Option<u8>) -> EncodedImageMetadata {
...
fn jpeg_frame_metadata(payload: &[u8], jfif: bool, adobe_transform: Option<u8>) -> Result<EncodedImageMetadata, ImageHeaderError> {
	let frame = payload
		.get(..6)
...
fn inspect_bmp(bytes: &[u8]) -> Result<EncodedImageMetadata, ImageHeaderError> {
	...
	let dib_size = usize::try_from(read_le_u32(bytes, 14)?).map_err(|error| ImageHeaderError::new(format!("BMP DIB size cannot address host memory: {error}")))?;
	if dib_size < 12 {
		return Err(ImageHeaderError::new(format!(
	...
	let (width, height, planes, bits_per_pixel) = if dib_size == 12 {
		...
		let width = if windows_header {
			u32::try_from(i32::from_le_bytes(width_raw.to_le_bytes())).map_err(|_| ImageHeaderError::new("BMP width is not positive"))?
		} else {
			width_raw
		};
		let height = if windows_header {
			...
			signed.checked_abs()
				...
				.ok_or_else(|| ImageHeaderError::new("BMP height cannot be represented as a positive dimension"))?
		} else {
			height_raw
		};
		(
