...
use crate::{GgufEndian, GgufError, GgufLimits, GgufMetadataType, GgufMetadataValue, GgufTensorType, parse_gguf};
...
/// Architecture identity recovered from one fully validated GGUF v3 file.
///
/// This is deliberately only container-level identity. It does not claim that
/// Recipe has lowered or can execute the named architecture.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct GgufModelDescriptor {
	architecture: String,
	endian: GgufEndian,
	alignment: u32,
	metadata_count: u64,
...
#[derive(Clone, Debug, PartialEq, Eq)]
struct OwnedMetadataArray {
	element_type: GgufMetadataType,
	values: Vec<OwnedMetadataValue>,
}

impl Drop for OwnedMetadataArray {
	fn drop(&mut self) {
		let mut pending = core::mem::take(&mut self.values);
		while let Some(mut value) = pending.pop() {
			if let OwnedMetadataValue::Array(array) = &mut value {
...
impl OwnedMetadataValue {
	const fn value_type(&self) -> GgufMetadataType {
		match self {
...
/// Convert one complete current-version GGUF image into structural OGDL.
///
/// Tensor bytes are decoded into typed floating components, signed integers,
/// scale codes, quantization codes, masks, and grid indexes. No source byte
/// image, base64 text, hexadecimal text, or opaque payload is emitted.
pub fn gguf_to_structural_ogdl(bytes: &[u8], limits: GgufLimits) -> GgufOgdlResult<String> {
	let archive = parse_gguf(bytes, limits)?;
	if archive.version() != 3 {
		return Err(GgufOgdlError::new(
			GgufOgdlErrorKind::InvalidValue,
			"gguf.version",
			format!(
				"structural conversion requires GGUF v3, found v{}",
				archive.version()
			),
		));
	}
	...
	line(
		&mut output,
		1,
		&format!("alignment {}", archive.alignment()),
	);
	line(&mut output, 1, &format!("file_bytes {}", bytes.len()));
	line(&mut output, 1, "metadata");
	for entry in archive.metadata() {
		line(&mut output, 2, "entry");
		let key = json_string(entry.key(), "metadata key")?;
		line(&mut output, 3, &format!("key {key}"));
		write_metadata_value(&mut output, 3, "value", entry.value())?;
	}
	line(&mut output, 1, "tensors");
	for tensor in archive.tensors() {
		if tensor.dimensions().len() > 4 {
			return Err(invalid_value(
				format!("tensors.{}.dimensions", tensor.name()),
				"current GGUF v3 permits at most four tensor dimensions",
			));
		}
		line(&mut output, 2, "tensor");
		let name = json_string(tensor.name(), "tensor name")?;
		line(&mut output, 3, &format!("name {name}"));
		let mut dimensions = String::from("dimensions");
		for dimension in tensor.dimensions() {
			write!(dimensions, " {dimension}").expect("writing to String cannot fail");
		}
		line(&mut output, 3, &dimensions);
		line(
			&mut output,
			3,
			&format!("type {}", tensor_type_name(tensor.tensor_type())),
		);
		line(&mut output, 3, &format!("offset {}", tensor.data_offset()));
		line(&mut output, 3, "payload");
		let data = archive.raw_tensor(tensor.name()).ok_or_else(|| {
			GgufOgdlError::new(
				GgufOgdlErrorKind::InvalidStructure,
				format!("tensors.{}", tensor.name()),
				"validated tensor span is unavailable",
			)
		})?;
		write_tensor_payload(&mut output, tensor.tensor_type(), archive.endian(), data, 4)?;
	}
	Ok(output)
}
...
impl<'a, R: Read> StreamBinaryReader<'a, R> {
	...
	fn metadata_value<F>(
		...
		label: &'static str,
		mut emit: F,
	) -> GgufOgdlResult<StreamMetadataRoot>
	where
		F: FnMut(usize, &str) -> GgufOgdlResult<()>,
	{
...
/// Recover the declared model architecture from one seekable GGUF v3 file.
///
/// The complete container layout is validated without retaining tensor payloads
...
where R: Read + Seek {
	...
	let architecture_type = header.architecture_type.ok_or_else(|| {
		invalid_structure(
			"gguf.metadata.general.architecture",
			"executable GGUF model is missing general.architecture",
		)
	})?;
	if architecture_type != GgufMetadataType::String {
		return Err(invalid_value(
			"gguf.metadata.general.architecture",
			format!(
				"executable GGUF model architecture must be string, found {}",
				metadata_type_name(architecture_type)
			),
		));
	}
	let architecture = header.architecture.ok_or_else(|| {
		invalid_structure(
			"gguf.metadata.general.architecture",
			"string architecture metadata was not retained",
		)
	})?;
	if architecture.is_empty() {
		return Err(invalid_value(
			"gguf.metadata.general.architecture",
			"executable GGUF model architecture is empty",
		));
	}
	let tensor_count = host_u64(header.tensors.len(), "gguf.tensor_count")?;
	Ok(GgufModelDescriptor {
		architecture,
...
fn inspect_stream_gguf<R: Read + Seek>(
	input: &mut R,
	file_bytes: u64,
	limits: GgufLimits,
) -> GgufOgdlResult<StreamGgufHeader> {
...
fn write_tensor_payload_stream<R: Read, W: IoWrite>(
	input: &mut R,
	output: &mut W,
	tensor: &StreamTensor,
	endian: GgufEndian,
...
fn stream_io_error(path: &str, error: std::io::Error) -> GgufOgdlError {
	GgufOgdlError::new(GgufOgdlErrorKind::Io, path, error.to_string())
}
	...
	W: Read + IoWrite + Seek,
{
	...
	if second_endian != header.endian
		|| second_alignment != header.alignment
		|| second_file_bytes != header.file_bytes
	{
		return Err(invalid_structure(
			"gguf",
...
fn inspect_structural_stream<R: BufRead + Seek>(
	lines: &mut CanonicalLines<'_, R>,
	limits: GgufLimits,
) -> GgufOgdlResult<StructuralStreamHeader> {
	...
	let mut budgets = StructuralBudgets::new(limits);
	let metadata = write_structural_metadata::<R, std::io::Cursor<Vec<u8>>>(
		lines,
		&mut budgets,
		limits.metadata_pairs().get(),
		None,
...
fn parse_structural_metadata_declaration(
	line: &CanonicalLine,
	label: &str,
	path: &str,
...
fn parse_structural_metadata_value<R, W>(
	lines: &mut CanonicalLines<'_, R>,
	budgets: &mut StructuralBudgets,
	root_line: &CanonicalLine,
	root_type: GgufMetadataType,
	root_payload: String,
	writer: &mut Option<&mut SeekBinaryWriter<'_, W>>,
	path: &str,
) -> GgufOgdlResult<Option<u32>>
where
	R: BufRead + Seek,
	W: IoWrite + Seek,
{
...
fn open_structural_array<W: IoWrite + Seek>(
	frames: &mut Vec<StructuralArrayFrame>,
	depth: usize,
	element_type: GgufMetadataType,
	budgets: &StructuralBudgets,
	writer: &mut Option<&mut SeekBinaryWriter<'_, W>>,
	path: &str,
) -> GgufOgdlResult<()> {
...
fn close_structural_array<W: IoWrite + Seek>(
	frames: &mut Vec<StructuralArrayFrame>,
	writer: &mut Option<&mut SeekBinaryWriter<'_, W>>,
) -> GgufOgdlResult<()> {
	let frame = frames
...
fn write_structural_metadata_scalar<W: IoWrite + Seek>(
	value_type: GgufMetadataType,
	payload: &str,
	budgets: &mut StructuralBudgets,
	writer: &mut Option<&mut SeekBinaryWriter<'_, W>>,
	...
	let u32_value = match value_type {
		...
		GgufMetadataType::F32 => {
			...
			let bits = u32::try_from(join_float(parts, FloatFormat::F32)?)
				.map_err(|error| invalid_value(path, error.to_string()))?;
		...
		GgufMetadataType::String => {
			let value: String = serde_json::from_str(payload)
...
/// Reconstruct one GGUF v3 image from Recipe's structural GGUF OGDL.
///
/// The resulting image is parsed again through the bounded GGUF validator
/// before it is returned.
pub fn structural_ogdl_to_gguf(text: &str, limits: GgufLimits) -> GgufOgdlResult<Vec<u8>> {
	let graph = Graph::parse(text).map_err(|error| {
		GgufOgdlError::new(
			GgufOgdlErrorKind::InvalidSyntax,
			"<ogdl>",
			error.to_string(),
		)
	})?;
	let roots = graph.roots();
	if roots.len() != 1 || node_text(&graph, roots[0], "<root>")? != "gguf" {
		return Err(invalid_structure(
			"<root>",
			"expected exactly one `gguf` root",
		));
	}
	let root = roots[0];
	let children = node_children(&graph, root, "gguf")?;
	if children.len() != 7 {
		return Err(invalid_structure(
			"gguf",
			format!("expected seven ordered fields, found {}", children.len()),
		));
	}
	expect_exact(
		&graph,
	...
	expect_exact(&graph, children[1], "gguf.version", "version 3")?;
	let endian = match node_text(&graph, children[2], "gguf.endian")? {
		"endian little" => GgufEndian::Little,
		"endian big" => GgufEndian::Big,
		other => {
			return Err(invalid_value(
	...
	let alignment = parse_single_u32(
		strip_prefix(
			node_text(&graph, children[3], "gguf.alignment")?,
			"alignment ",
			"gguf.alignment",
		)?,
		"gguf.alignment",
	...
	let file_bytes = parse_number::<u64>(
		strip_prefix(
			node_text(&graph, children[4], "gguf.file_bytes")?,
			"file_bytes ",
			"gguf.file_bytes",
		)?,
		"gguf.file_bytes",
	...
	if file_bytes == 0 || file_bytes > limits.file_bytes().get() {
		return Err(invalid_value(
			"gguf.file_bytes",
			format!(
				"declared {file_bytes} bytes, configured limit is {}",
	...
	expect_exact(&graph, children[5], "gguf.metadata", "metadata")?;
	expect_exact(&graph, children[6], "gguf.tensors", "tensors")?;

	let metadata = parse_metadata(&graph, children[5])?;
	let tensors = parse_tensors(&graph, children[6])?;
	if host_u64(metadata.len(), "metadata count")? > limits.metadata_pairs().get() {
		return Err(invalid_value(
			"gguf.metadata",
			format!(
				"metadata count exceeds configured limit {}",
	...
	if host_u64(tensors.len(), "tensor count")? > limits.tensors().get() {
		return Err(invalid_value(
			"gguf.tensors",
			format!("tensor count exceeds configured limit {}", limits.tensors()),
		));
	...
	writer.bytes.extend_from_slice(b"GGUF");
	writer.u32(3);
	writer.u64(host_u64(tensors.len(), "tensor count")?);
	writer.u64(host_u64(metadata.len(), "metadata count")?);
	for (key, value) in &metadata {
		writer.string(key, "metadata key")?;
		writer.u32(value.value_type().code());
		writer.metadata_value(value)?;
	}
	for tensor in &tensors {
		writer.string(&tensor.name, "tensor name")?;
		writer.u32(host_u32(tensor.dimensions.len(), "tensor rank")?);
		for dimension in &tensor.dimensions {
			writer.u64(*dimension);
		}
	...
	let header_end = host_u64(writer.bytes.len(), "header length")?;
	let aligned_header_end = align_up(header_end, u64::from(alignment))?;
	let data_start = if tensors.is_empty() {
		if file_bytes == header_end || file_bytes == aligned_header_end {
			file_bytes
			...
			return Err(invalid_value(
				"gguf.file_bytes",
				format!(
					"tensor-free GGUF declares {file_bytes} bytes; expected unpadded header end {header_end} or padded end {aligned_header_end}"
				),
			));
		}
	} else {
	...
	if data_start > header_end {
		if data_start > limits.file_bytes().get() {
			return Err(invalid_value(
				"gguf",
				format!(
					"aligned header requires {data_start} bytes, configured limit is {}",
					limits.file_bytes()
				),
			));
		}
		writer.bytes
			.resize(host_usize(data_start, "aligned tensor data start")?, 0);
	}
	let data_start = writer.bytes.len();
	let mut data_bytes = 0_u64;
	for tensor in &tensors {
		if tensor.offset % u64::from(alignment) != 0 {
			return Err(invalid_value(
				format!("tensors.{}.offset", tensor.name),
				format!("offset {} is not aligned to {alignment}", tensor.offset),
			));
		}
		let encoded = tensor_encoded_bytes(&tensor.name, &tensor.dimensions, tensor.tensor_type)?;
		let end = tensor.offset.checked_add(encoded).ok_or_else(|| {
			GgufOgdlError::new(
				GgufOgdlErrorKind::ArithmeticOverflow,
				format!("tensors.{}.offset", tensor.name),
				"tensor end overflowed u64",
			)
		})?;
		data_bytes = data_bytes.max(end);
	}
	let data_start_u64 = host_u64(data_start, "tensor data start")?;
	let tensor_end = data_start_u64
		.checked_add(data_bytes)
		.ok_or_else(|| invalid_overflow("gguf", "output length overflowed u64"))?;
	let padded_data_bytes = align_up(data_bytes, u64::from(alignment))?;
	let padded_end = data_start_u64
		.checked_add(padded_data_bytes)
		.ok_or_else(|| invalid_overflow("gguf", "padded output length overflowed u64"))?;
	if file_bytes != tensor_end && file_bytes != padded_end {
		return Err(invalid_value(
			"gguf.file_bytes",
			format!(
				"declared {file_bytes} bytes, but the structural layout requires exact end {tensor_end} or aligned end {padded_end}"
			),
		));
	}
	let total = file_bytes;
	if total > limits.file_bytes().get() {
		return Err(invalid_value(
			"gguf",
			format!(
				"output requires {total} bytes, configured limit is {}",
				limits.file_bytes()
			),
		));
	}
	writer.bytes.resize(host_usize(total, "output length")?, 0);
	let mut occupied = Vec::with_capacity(tensors.len());
	for (index, tensor) in tensors.iter().enumerate() {
		let encoded = tensor_encoded_bytes(&tensor.name, &tensor.dimensions, tensor.tensor_type)?;
		let end = tensor
			.offset
			.checked_add(encoded)
			.ok_or_else(|| invalid_overflow("tensors", "tensor end overflow"))?;
		for (prior_start, prior_end, prior_name) in &occupied {
			if tensor.offset < *prior_end && *prior_start < end {
				return Err(invalid_value(
					format!("tensors[{index}].offset"),
					format!("tensor {:?} overlaps tensor {prior_name:?}", tensor.name),
				));
			}
		}
		occupied.push((tensor.offset, end, tensor.name.clone()));
		let begin = data_start
			.checked_add(host_usize(tensor.offset, "tensor offset")?)
			.ok_or_else(|| invalid_overflow("tensors", "tensor output start overflowed usize"))?;
		let end_host = begin
			.checked_add(host_usize(encoded, "tensor encoded bytes")?)
			.ok_or_else(|| invalid_overflow("tensors", "tensor output end overflowed usize"))?;
		let destination = writer.bytes.get_mut(begin..end_host).ok_or_else(|| {
			invalid_overflow(
				"tensors",
				"tensor output range lies outside allocated image",
			)
		})?;
		read_tensor_payload(&graph, tensor, endian, destination, index)?;
	}

	let reparsed = parse_gguf(&writer.bytes, limits)?;
	if reparsed.alignment() != alignment {
		return Err(invalid_value(
			"gguf.alignment",
			format!(
				"declared alignment {alignment} disagrees with metadata-derived alignment {}",
				reparsed.alignment()
			),
		));
	}
...
fn write_metadata_value(
	...
	depth: usize,
	label: &str,
	value: &GgufMetadataValue<'_>,
) -> GgufOgdlResult<()> {
	let mut pending = vec![(depth, label, value)];
	while let Some((depth, label, value)) = pending.pop() {
		let text = match value {
			GgufMetadataValue::U8(value) => format!("{label} u8 {value}"),
			GgufMetadataValue::I8(value) => format!("{label} i8 {value}"),
			GgufMetadataValue::U16(value) => format!("{label} u16 {value}"),
			GgufMetadataValue::I16(value) => format!("{label} i16 {value}"),
			GgufMetadataValue::U32(value) => format!("{label} u32 {value}"),
			GgufMetadataValue::I32(value) => format!("{label} i32 {value}"),
			GgufMetadataValue::F32Bits(bits) => {
				let parts = split_float(u64::from(*bits), FloatFormat::F32);
				format!(
					"{label} f32 {} {} {}",
					parts.sign, parts.exponent, parts.fraction
				)
			}
			GgufMetadataValue::Bool(value) => format!("{label} bool {value}"),
			GgufMetadataValue::String(value) => {
				format!("{label} string {}", json_string(value, label)?)
			}
			GgufMetadataValue::Array(array) => {
				line(
					output,
					depth,
					&format!("{label} array {}", metadata_type_name(array.element_type())),
				);
				pending.extend(
					array.values()
						.iter()
						.rev()
						.map(|element| (depth + 1, "element", element)),
				);
				continue;
			}
			GgufMetadataValue::U64(value) => format!("{label} u64 {value}"),
			GgufMetadataValue::I64(value) => format!("{label} i64 {value}"),
			GgufMetadataValue::F64Bits(bits) => {
				let parts = split_float(*bits, FloatFormat::F64);
				format!(
					"{label} f64 {} {} {}",
					parts.sign, parts.exponent, parts.fraction
				)
...
fn parse_metadata(graph: &Graph, metadata: NodeId) -> GgufOgdlResult<Vec<(String, OwnedMetadataValue)>> {
	let entries = node_children(graph, metadata, "gguf.metadata")?;
	let mut parsed = Vec::with_capacity(entries.len());
	let mut keys = BTreeSet::new();
	for (index, entry) in entries.iter().copied().enumerate() {
		let path = format!("gguf.metadata[{index}]");
		expect_exact(graph, entry, &path, "entry")?;
		let fields = node_children(graph, entry, &path)?;
		if fields.len() != 2 {
			return Err(invalid_structure(
				&path,
				format!(
					"expected ordered key and value fields, found {}",
					fields.len()
				),
			));
		}
		require_no_children(graph, fields[0], &format!("{path}.key"))?;
		let key_text = strip_prefix(
			node_text(graph, fields[0], &format!("{path}.key"))?,
			"key ",
			&format!("{path}.key"),
		)?;
		let key: String = serde_json::from_str(key_text).map_err(|error| {
		...
		let value = parse_metadata_value(graph, fields[1], "value", &format!("{path}.value"))?;
...
fn parse_metadata_value(
	...
	label: &'static str,
	path: &str,
) -> GgufOgdlResult<OwnedMetadataValue> {
	#[derive(Debug)]
	enum Action {
		Visit {
			node: NodeId,
	...
	let mut pending = vec![Action::Visit {
		node,
		label,
		path: path.to_owned(),
	}];
	...
	while let Some(action) = pending.pop() {
		match action {
			Action::Visit { node, label, path } => {
				let text = strip_prefix(node_text(graph, node, &path)?, &format!("{label} "), &path)?;
				let (kind, payload) = text.split_once(' ').unwrap_or((text, ""));
				if kind != "array" {
					completed.push(parse_metadata_scalar(
						graph, node, label, &path, kind, payload,
					)?);
					continue;
				}
				if payload.is_empty() || payload.contains(char::is_whitespace) {
				...
				let element_type = parse_metadata_type(payload, &path)?;
				let children = node_children(graph, node, &path)?;
				pending.push(Action::FinishArray {
					element_type,
					elements: children.len(),
					path: path.clone(),
				});
				for (index, child) in children.iter().copied().enumerate().rev() {
					pending.push(Action::Visit {
						node: child,
						label: "element",
			...
			Action::FinishArray {
				...
				let start = completed
					...
					.ok_or_else(|| invalid_structure(&path, "metadata array completion stack underflowed"))?;
				let values = completed.split_off(start);
				for (index, value) in values.iter().enumerate() {
					if value.value_type() != element_type {
						return Err(invalid_value(
							...
							format!(
								"array declares {} elements but found {}",
								metadata_type_name(element_type),
								metadata_type_name(value.value_type())
							),
						));
					}
				}
				completed.push(OwnedMetadataValue::Array(OwnedMetadataArray {
					element_type,
					values,
				}));
			}
...
fn parse_metadata_scalar(
	...
	let text = strip_prefix(node_text(graph, node, path)?, &format!("{label} "), path)?;
	let (parsed_kind, parsed_payload) = text.split_once(' ').unwrap_or((text, ""));
	if parsed_kind != kind || parsed_payload != payload {
		return Err(invalid_structure(
			path,
			"metadata scalar changed during parsing",
		));
	}
	let scalar = |expected: &str| -> GgufOgdlResult<&str> {
		if kind != expected || payload.is_empty() {
			return Err(invalid_value(
				path,
				format!("expected {label} {expected} VALUE"),
			));
		}
		require_no_children(graph, node, path)?;
		Ok(payload)
	};
	match kind {
		"u8" => {
		...
		"string" => {
			...
			serde_json::from_str(value)
				.map(OwnedMetadataValue::String)
				.map_err(|error| invalid_value(path, format!("string is not valid JSON text: {error}")))
		}
		"f32" => {
			let parts = parse_float_parts(scalar("f32")?, FloatFormat::F32, path)?;
			Ok(OwnedMetadataValue::F32Bits(
				join_float(parts, FloatFormat::F32)? as u32,
			))
		}
		"f64" => {
			let parts = parse_float_parts(scalar("f64")?, FloatFormat::F64, path)?;
			Ok(OwnedMetadataValue::F64Bits(join_float(
				parts,
				FloatFormat::F64,
			)?))
		}
...
fn parse_tensors(graph: &Graph, tensors: NodeId) -> GgufOgdlResult<Vec<ParsedTensor>> {
	let nodes = node_children(graph, tensors, "gguf.tensors")?;
	let mut parsed = Vec::with_capacity(nodes.len());
	let mut names = BTreeSet::new();
	for (index, tensor) in nodes.iter().copied().enumerate() {
		let path = format!("gguf.tensors[{index}]");
		expect_exact(graph, tensor, &path, "tensor")?;
		let fields = node_children(graph, tensor, &path)?;
		if fields.len() != 5 {
			return Err(invalid_structure(
				&path,
				format!(
					"expected name, dimensions, type, offset, and payload, found {} fields",
					fields.len()
				),
			));
		}
		for (field, name) in fields[..4]
			.iter()
			.copied()
			.zip(["name", "dimensions", "type", "offset"])
		{
			require_no_children(graph, field, &format!("{path}.{name}"))?;
		}
		let name_text = strip_prefix(
			node_text(graph, fields[0], &format!("{path}.name"))?,
			"name ",
			&format!("{path}.name"),
		)?;
		let name: String = serde_json::from_str(name_text).map_err(|error| {
		...
		let dimensions = dimensions_text
			.split_whitespace()
			.enumerate()
			.map(|(axis, value)| parse_number::<u64>(value, &format!("{path}.dimensions[{axis}]"), "u64"))
			.collect::<GgufOgdlResult<Vec<_>>>()?;
		if dimensions.len() > 4 {
			return Err(invalid_value(
				format!("{path}.dimensions"),
				"current GGUF v3 tensors permit zero through four dimensions",
			));
		}
		let type_name = strip_prefix(
			node_text(graph, fields[2], &format!("{path}.type"))?,
			"type ",
			&format!("{path}.type"),
		)?;
		let tensor_type = parse_tensor_type(type_name, &format!("{path}.type"))?;
		let offset_text = strip_prefix(
			node_text(graph, fields[3], &format!("{path}.offset"))?,
			"offset ",
			&format!("{path}.offset"),
		)?;
		let offset = parse_number(offset_text, &format!("{path}.offset"), "u64")?;
		expect_exact(graph, fields[4], &format!("{path}.payload"), "payload")?;
		parsed.push(ParsedTensor {
			name,
			dimensions,
			tensor_type,
			offset,
...
impl BinaryWriter {
	...
	fn string(&mut self, value: &str, path: &str) -> GgufOgdlResult<()> {
		self.u64(host_u64(value.len(), path)?);
		self.bytes.extend_from_slice(value.as_bytes());
		Ok(())
	}

	fn metadata_value(&mut self, value: &OwnedMetadataValue) -> GgufOgdlResult<()> {
		let mut pending = vec![value];
		while let Some(value) = pending.pop() {
			match value {
				OwnedMetadataValue::U8(value) => self.u8(*value),
				OwnedMetadataValue::I8(value) => self.i8(*value),
				OwnedMetadataValue::U16(value) => self.u16(*value),
				OwnedMetadataValue::I16(value) => self.i16(*value),
				OwnedMetadataValue::U32(value) | OwnedMetadataValue::F32Bits(value) => self.u32(*value),
				OwnedMetadataValue::I32(value) => self.i32(*value),
				OwnedMetadataValue::Bool(value) => self.u8(u8::from(*value)),
				OwnedMetadataValue::String(value) => self.string(value, "metadata string")?,
				OwnedMetadataValue::Array(array) => {
					self.u32(array.element_type.code());
					self.u64(host_u64(array.values.len(), "metadata array length")?);
					pending.extend(array.values.iter().rev());
				}
				OwnedMetadataValue::U64(value) | OwnedMetadataValue::F64Bits(value) => self.u64(*value),
				OwnedMetadataValue::I64(value) => self.i64(*value),
			}
		}
		Ok(())
	}
...
fn write_tensor_payload(
	...
	if !data.len().is_multiple_of(block_bytes) {
		return Err(invalid_structure(
			"tensor.payload",
			format!(
				"{} bytes do not form complete {block_bytes}-byte blocks",
				data.len()
			),
		));
	}
	if tensor_type.block_elements() == 1 {
		for (chunk_index, blocks) in data
			.chunks(block_bytes.saturating_mul(SCALAR_CHUNK_VALUES))
			.enumerate()
		{
			let start = chunk_index
				.checked_mul(SCALAR_CHUNK_VALUES)
				.ok_or_else(|| invalid_overflow("tensor.payload", "scalar chunk index overflowed usize"))?;
			line(output, depth, &format!("chunk {start}"));
			let mut combined: Option<StructuralField> = None;
			for block in blocks.chunks_exact(block_bytes) {
				let mut decoded = decode_block(tensor_type, endian, block)?;
				if decoded.len() != 1 {
					return Err(invalid_structure(
						"tensor.payload",
						"scalar layout did not produce one value field",
					));
				}
				let field = decoded.remove(0);
				match &mut combined {
			...
			let field = combined.ok_or_else(|| invalid_structure("tensor.payload", "empty scalar chunk"))?;
			line(output, depth + 1, &field_text(&field));
		}
	} else {
		for (block_index, block) in data.chunks_exact(block_bytes).enumerate() {
			line(output, depth, &format!("block {block_index}"));
			for field in decode_block(tensor_type, endian, block)? {
				line(output, depth + 1, &field_text(&field));
			}
		}
	}
...
fn signed_field(name: &'static str, kind: &'static str, value: i64, minimum: i64, maximum: i64) -> StructuralField {
	signed_values(name, kind, minimum, maximum, vec![value])
}
...
fn unpack_nvfp4(bytes: &[u8]) -> Vec<u64> {
	...
	for subblock in bytes.chunks_exact(8) {
...
fn read_tensor_payload(
	...
	let block_count = destination.len() / block_bytes;
	let template_bytes = vec![0_u8; block_bytes];
	let templates = decode_block(tensor.tensor_type, endian, &template_bytes)?;
	if tensor.tensor_type.block_elements() == 1 {
		if templates.len() != 1 {
			return Err(invalid_structure(
				&path,
		...
		for (chunk_index, node) in nodes.iter().copied().enumerate() {
			let chunk_path = format!("{path}.chunks[{chunk_index}]");
			let start_text = strip_prefix(node_text(graph, node, &chunk_path)?, "chunk ", &chunk_path)?;
			let start = parse_number::<usize>(start_text, &chunk_path, "usize")?;
			if start != next {
				return Err(invalid_value(
					&chunk_path,
					format!("chunk starts at {start}, expected contiguous scalar index {next}"),
				));
			}
			let fields = node_children(graph, node, &chunk_path)?;
			if fields.len() != 1 {
				return Err(invalid_structure(
					&chunk_path,
					"scalar chunk must contain exactly one values field",
				));
			}
			let combined = parse_field(
				graph,
				fields[0],
				&templates[0],
				None,
				&format!("{chunk_path}.values"),
			)?;
			let count = field_len(&combined);
			if count == 0
				|| count > SCALAR_CHUNK_VALUES
				|| next.checked_add(count).is_none_or(|end| end > block_count)
			{
				return Err(invalid_value(
					&chunk_path,
					format!(
						"chunk contains {count} values outside remaining scalar count {}",
						block_count - next
					),
				));
			}
			for value_index in 0..count {
				...
				let begin = next
					...
					.ok_or_else(|| invalid_overflow(&chunk_path, "scalar output offset overflowed usize"))?;
				destination[begin..begin + block_bytes].copy_from_slice(&encoded);
				next += 1;
			}
		}
		if next != block_count {
			return Err(invalid_structure(
				&path,
				format!("payload supplies {next} scalar values, expected {block_count}"),
			));
		}
	} else {
		if nodes.len() != block_count {
			return Err(invalid_structure(
				&path,
				format!("payload has {} blocks, expected {block_count}", nodes.len()),
			));
		}
		for (block_index, node) in nodes.iter().copied().enumerate() {
			let block_path = format!("{path}.blocks[{block_index}]");
			expect_exact(graph, node, &block_path, &format!("block {block_index}"))?;
			let field_nodes = node_children(graph, node, &block_path)?;
			if field_nodes.len() != templates.len() {
				return Err(invalid_structure(
					&block_path,
					format!(
						"block has {} fields, expected {}",
						field_nodes.len(),
						templates.len()
					),
				));
			...
			let fields = field_nodes
				...
				.zip(&templates)
				.enumerate()
				.map(|(field_index, (field_node, template))| {
					parse_field(
						graph,
						field_node,
						template,
				...
				.collect::<GgufOgdlResult<Vec<_>>>()?;
			let encoded = encode_block(tensor.tensor_type, endian, &fields)?;
			let begin = block_index
				.checked_mul(block_bytes)
				.ok_or_else(|| invalid_overflow(&block_path, "block output offset overflowed usize"))?;
			destination[begin..begin + block_bytes].copy_from_slice(&encoded);
		}
	}
	Ok(())
...
fn parse_field(
	graph: &Graph,
	node: NodeId,
	template: &StructuralField,
	expected_count: Option<usize>,
	path: &str,
) -> GgufOgdlResult<StructuralField> {
	require_no_children(graph, node, path)?;
	let text = node_text(graph, node, path)?;
	parse_field_text(text, template, expected_count, path)
}
	...
	match tensor_type {
		...
		GgufTensorType::Iq2Xxs => {
			...
			for group in 0..8 {
				...
				let mut sign_scale = u32::try_from(scales[group])
					.map_err(|error| invalid_value("scale_codes", error.to_string()))?
					<< 28;
				for lane in 0..4 {
					sign_scale |= u32::try_from(signs[group * 4 + lane])
		...
		GgufTensorType::Iq2Xs => {
			...
			for (grid, sign) in grids.iter().copied().zip(signs.iter().copied()) {
				writer.u16(u16::try_from(grid | (sign << 9))
		...
		GgufTensorType::Iq1S => {
			...
			for group in 0..8 {
				let mut code = u16::try_from(scales[group] | (shifts[group] << 3))
					.map_err(|error| invalid_value("scale and shift code", error.to_string()))?
					<< 12;
				for lane in 0..4 {
					code |= u16::try_from(grid_high[group * 4 + lane])
...
fn pack_q3k_scales(values: &[u64]) -> GgufOgdlResult<Vec<u8>> {
	...
	for index in 0..8 {
		bytes[index] = u8::try_from(values[index] & 0x0f)
			.map_err(|error| invalid_value("tensor.payload", error.to_string()))?
...
fn pack_k_scale_min(scales: &[u64], minimums: &[u64]) -> GgufOgdlResult<Vec<u8>> {
	...
	let mut bytes = vec![0_u8; 12];
	for index in 0..4 {
		bytes[index] = u8::try_from(scales[index] & 0x3f)
			.map_err(|error| invalid_value("tensor.payload", error.to_string()))?;
		...
		bytes[index + 8] = u8::try_from(scales[index + 4] & 0x0f)
			...
			| (u8::try_from(minimums[index + 4] & 0x0f)
				.map_err(|error| invalid_value("tensor.payload", error.to_string()))?
				<< 4);
		bytes[index] |= u8::try_from((scales[index + 4] >> 4) & 0x03)
			.map_err(|error| invalid_value("tensor.payload", error.to_string()))?
			<< 6;
		bytes[index + 4] |= u8::try_from((minimums[index + 4] >> 4) & 0x03)
			.map_err(|error| invalid_value("tensor.payload", error.to_string()))?
			<< 6;
	}
	Ok(bytes)
...
fn pack_iq4xs_scales(values: &[u64]) -> GgufOgdlResult<(u16, Vec<u8>)> {
	...
	for (index, value) in values.iter().copied().enumerate() {
		low[index / 2] |= u8::try_from(value & 0x0f)
			.map_err(|error| invalid_value("tensor.payload", error.to_string()))?
			<< (4 * (index % 2));
		high |= u16::try_from((value >> 4) & 0x03)
			.map_err(|error| invalid_value("tensor.payload", error.to_string()))?
			<< (2 * index);
	}
	Ok((high, low))
...
fn pack_nvfp4(values: &[u64]) -> GgufOgdlResult<Vec<u8>> {
	...
	for subblock in values.chunks_exact(16) {
		for index in 0..8 {
...
fn node_text<'a>(graph: &'a Graph, node: NodeId, path: &str) -> GgufOgdlResult<&'a str> {
	graph.node(node)
		.map(|node| node.text())
		.ok_or_else(|| invalid_structure(path, "node identifier does not exist"))
}

fn node_children<'a>(graph: &'a Graph, node: NodeId, path: &str) -> GgufOgdlResult<&'a [NodeId]> {
	graph.node(node)
		.map(|node| node.children())
		.ok_or_else(|| invalid_structure(path, "node identifier does not exist"))
}

fn expect_exact(graph: &Graph, node: NodeId, path: &str, expected: &str) -> GgufOgdlResult<()> {
	let actual = node_text(graph, node, path)?;
	if actual != expected {
		return Err(invalid_structure(
			path,
...
fn require_no_children(graph: &Graph, node: NodeId, path: &str) -> GgufOgdlResult<()> {
	let children = node_children(graph, node, path)?;
	if children.is_empty() {
		...
		Err(invalid_structure(
			path,
			format!("scalar node has {} unexpected children", children.len()),
		))
	}
}
...
fn host_u64(value: usize, path: &str) -> GgufOgdlResult<u64> {
	u64::try_from(value).map_err(|error| invalid_overflow(path, error.to_string()))
}

fn host_u32(value: usize, path: &str) -> GgufOgdlResult<u32> {
	u32::try_from(value).map_err(|error| invalid_overflow(path, error.to_string()))
}

fn host_usize(value: u64, path: &str) -> GgufOgdlResult<usize> {
	usize::try_from(value).map_err(|error| invalid_overflow(path, error.to_string()))
}

fn invalid_structure(path: impl Into<String>, detail: impl Into<String>) -> GgufOgdlError {
	GgufOgdlError::new(GgufOgdlErrorKind::InvalidStructure, path, detail)
}

fn invalid_value(path: impl Into<String>, detail: impl Into<String>) -> GgufOgdlError {
	GgufOgdlError::new(GgufOgdlErrorKind::InvalidValue, path, detail)
}
