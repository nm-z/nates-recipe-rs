...
/// overflow.
pub fn prepare_inference_table(table: &RawTable, schema: &[InferenceFeatureSchema]) -> InferencePrepareResult<PreparedInferenceDataset> {
	if schema.is_empty() {
...
fn encode_feature(table: &RawTable, source_column: usize, schema: &InferenceFeatureSchema, feature: usize) -> InferencePrepareResult<(PreparedInferenceValues, Option<Vec<CategoricalObservation>>)> {
	match &schema.encoding {
		...
		InferenceFeatureEncoding::CategoricalDictionary { dictionary } => {
			...
			let reserved = i32::try_from(dictionary.len()).map_err(|error| invalid_value(schema, feature, 0, error))?;
			let encoded = table
				.rows()
				.iter()
				.enumerate()
...
fn validate_categorical_alignment(schema: &InferenceFeatureSchema, feature: usize, reserved: i32, codes: &[i32], observations: &[CategoricalObservation]) -> InferencePrepareResult<()> {
...
fn source_value<'a>(row: &'a [Vec<u8>], source_column: usize, schema: &InferenceFeatureSchema, feature: usize, source_row: usize) -> InferencePrepareResult<&'a [u8]> {
...
fn invalid_value(schema: &InferenceFeatureSchema, feature: usize, source_row: usize, error: impl fmt::Display) -> InferencePrepareError {
	InferencePrepareError::new(
		InferencePrepareErrorKind::InvalidValue,
