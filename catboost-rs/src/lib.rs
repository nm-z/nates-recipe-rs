...
fn validate_train(
	...
	if params.n_permutations == 0 {
		return Err(Error::InvalidInput("n_permutations must be > 0".into()));
	}
	for &c in &params.cat_features {
		if c >= p {
			return Err(Error::InvalidInput(format!("cat_feature {c} >= p={p}")));
		}
	}
	if y.iter().any(|&v| v >= nc) {
		return Err(Error::InvalidInput("y contains value >= n_classes".into()));
	}
	for (i, &v) in x.iter().enumerate() {
		if v.is_nan() {
			return Err(Error::InvalidInput(format!("x[{i}] is NaN")));
		}
	}
	for &cat_j in &params.cat_features {
		for i in 0..n {
			...
			if v != (v as i64) as f64 {
				return Err(Error::InvalidInput(format!(
					"categorical x[{},{}]={v} is not an exact integer",
					i, cat_j
				)));
			}
		}
	...
	for j in 0..p {
		if cat_set.contains(&j) {
			...
			for i in 0..n {
				...
				let id = *cat_mappings[j].entry(v).or_insert_with(|| {
					let id = next_id;
					next_id += 1;
					id
				});
			...
			while (idx as usize) < n {
				...
				if b.is_empty() || (v - b[b.len() - 1]).abs() > 1e-12_f64 {
					b.push(v);
				}
				idx += step;
			}
			for i in 0..n {
				bins[j][i] = b.partition_point(|&t| t <= x[i * p + j]) as u32;
			}
			borders[j] = b;
		}
	}
...
fn quantize_predict(
	...
	(0..p).map(|j| {
		if cat_set.contains(&j) {
			...
			(0..n).map(|i| *map.get(&(x[i * p + j] as i64)).unwrap_or(&fallback))
				.collect()
		} else {
			let b = &borders[j];
			(0..n).map(|i| b.partition_point(|&t| t <= x[i * p + j]) as u32)
				.collect()
		}
	})
	.collect()
...
fn greedy_target_stats(
	...
	(0..=n_cats as u32)
		...
		.map(|c| {
			(
				c,
				(sum[c as usize] + prior * class_prior) / (count[c as usize] + prior),
			)
		})
		.collect()
}
...
fn quantize_float_col(values: &[f64], n: usize, border_count: usize) -> (Vec<u32>, Vec<f64>) {
	...
	let binned = values
		.iter()
		.map(|&v| b.partition_point(|&t| t <= v) as u32)
		.collect();
	(binned, b)
}

fn leaf_weight(g: f64, h: f64, l2: f64) -> f64 {
	-g / (h + l2)
}
...
fn compute_ts_floats(
	...
	for &cat_j in cat_features {
		...
		for k in 0..n_classes {
			ts_floats.push(ordered_target_stats(
				y,
				&bins[cat_j],
				perm,
				n_cats,
...
fn bin_ts_floats(ts_floats: &[Vec<f64>], borders: &[Vec<f64>]) -> Vec<Vec<u32>> {
	ts_floats
		...
		.map(|(vals, bord)| {
			vals.iter()
				.map(|&v| bord.partition_point(|&t| t <= v) as u32)
				.collect()
		})
		.collect()
}
...
fn compute_ts_bins_fresh(
	...
	let ts_floats = compute_ts_floats(
		y,
		bins,
		perm,
		cat_features,
...
fn build_oblivious_tree_gpu(
	...
	for d in 0..depth {
		...
		for k in 0..n_classes {
			...
			let g_gpu = GpuBuffer::upload_f32(&grad_k)
				.map_err(|e| Errored::new(format!("upload grad class {k}: {e}")))?;
			...
			gain_k.download_f32(&mut g_host)
				.map_err(|e| Errored::new(format!("download gain_k class {k}: {e}")))?;
			for i in 0..gain_elems {
				gain_sum[i] += g_host[i];
			}
		}

		let best_idx = gain_sum
			.iter()
			.enumerate()
	...
	for k in 0..n_classes {
		...
		let hess_k: Vec<f32> = (0..n).map(|i| hess_f32[i * n_classes + k]).collect();
		let g_gpu = GpuBuffer::upload_f32(&grad_k)
			.map_err(|e| Errored::new(format!("upload leaf grad class {k}: {e}")))?;
		let h_gpu = GpuBuffer::upload_f32(&hess_k)
		...
		leaf_val
			.download_f32(&mut lv_f32)
			.map_err(|e| Errored::new(format!("download leaf_val class {k}: {e}")))?;
		leaf_values.push(lv_f32.iter().map(|&v| v as f64).collect());
	}

	let mut leaf_idx = vec![0u8; n];
	node_a.download_u8(&mut leaf_idx)
		.map_err(|e| Errored::new(format!("download leaf indices: {e}")))?;
...
fn route_full_gpu(
	...
	leaf_gpu
		.download_u8(&mut out)
		.map_err(|e| Errored::new(format!("download leaf routing: {e}")))?;
	Ok(out)
}
...
pub fn train(
	...
	for t in 0..params.iterations {
		...
		for col in &ts_columns {
			all_bin_refs.push(col.as_slice());
		}
		...
		for k in 0..n_classes {
			...
			new_lv.push((0..n_leaves)
				.map(|j| leaf_weight(lg[j], lh[j], params.l2_reg))
				.collect());
		}

		let tree = ObliviousTree {
			split_features: sf.clone(),
			split_bins: sb.clone(),
			leaf_values: new_lv,
			ts_borders: ts_bord,
		};
		...
		for r in 0..s {
			let rperm = &perms[r];
			let r_ts_floats = compute_ts_floats(
				y,
				&bins,
				rperm,
				&params.cat_features,
			...
			for col in &r_ts_cols {
				r_all_refs.push(col.as_slice());
			}
		...
		Write::line(
			epoch,
			format!("      cb iter={}/{}", t + 1, params.iterations),
		);
	}
	...
	Ok(Model {
		...
		n_orig_features: p,
		borders,
		cat_features: params.cat_features.clone(),
		cat_mappings,
		ts_infos,
	})
}
...
pub fn predict(model: &Model, x: &[f64], n: usize) -> Result<Vec<f64>, Error> {
	...
	let orig_bins = quantize_predict(
		x,
		n,
		p,
		&model.borders,
	...
	let ts_float_vals: Vec<Vec<f64>> = model
		.ts_infos
		.iter()
		.map(|info| {
			...
			cat_bins
				.iter()
				.map(|&c| *info.greedy_ts.get(&c).unwrap_or(&info.default_ts))
				.collect()
		})
		.collect();
	...
	for tree in &model.trees {
		...
		for col in &ts_columns {
			all_refs.push(col.as_slice());
		}

		let rm = bins_row_major_u8(&all_refs, n_total, n);
		let bins_rm_gpu = upload_u8(&rm).map_err(|e| Error::InvalidInput(e.to_string()))?;

		let leaf_u8 = route_full_gpu(
			&bins_rm_gpu,
			&tree.split_features,
			&tree.split_bins,
			n,
...
mod tests {
	...
	fn test_sanity_depth1() {
		...
		for i in 0..4 {
			let pred = if probs[i * 2] > probs[i * 2 + 1] {
				0
			} else {
				1
			};
	...
	fn test_ts_feature_split_roundtrip() {
		...
		assert!(
			model.trees
				.iter()
				.any(|t| t.split_features.iter().any(|&f| f >= 2)),
			"no TS split"
		);
