...
fn main() -> Result<(), Errored> {
	...
	if p != p_te {
		Write::err(format!(
			"feature count mismatch: train p={p} vs test p={p_te}"
		))?;
	}
	...
	let n_classes = *y_tr
		.iter()
		.max()
		.ok_or_else(|| Errored::new("y_tr is empty"))?
		+ 1;
	Write::line(
		data,
	...
	Write::line(
		...
		format!(
			...
			params.iterations, params.depth, params.learning_rate, params.l2_reg
		),
	);
	...
	Write::line(
		...
		format!(
			...
			train_time.as_secs_f64() * 1000.0 / params.iterations as f64
		),
	);
