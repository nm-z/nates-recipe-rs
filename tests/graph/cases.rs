...
fn arity_forms_dispatch() {
	...
	let _: Node = g.itnl(()).itnl("a");
	let a = g.itnl("a");
	drop(g.del(&a[0]));
	assert!(g.itnl("a").children.iter().all(|c| return c.name != "x"));
	drop(g.del(NamedChild::new("y", &a)));
...
fn add_del() {
	...
	assert!(
		g.snapshot()
			...
			.children
			.iter()
			.any(|c| return c.name == "c")
	);
	let _: Graph = del!(g, a.b {});
	assert!(
		!g.snapshot()
			.select("a")
			.expect("a")
...
fn typed_serialize_roundtrip() {
	let g = Graph::empty();
	let weights = vec![0.01f64, -0.02f64, 0.03f64];
	let labels = vec!["cat".to_owned(), "dog".to_owned()];
	drop(g.add(weights.clone(), "z1.w")
		.add(0.5f64, "z1.b")
		.add(42i64, "z1.neurons")
		.add(true, "z1.bias")
		.add("relu", "z1.act")
	...
	let w: Vec<f64> = g
		...
		.children
		.iter()
		.filter_map(|c| return c.name.parse().ok())
		.collect();
	assert_eq!(w, weights);
