...
const CASES: &[Case] = &[
	...
	Case { name: "hyper-attention", shape: "hyper-attention", precision: "fp32", rows: 131, columns: 18 },
...
fn build(case: &Case) -> Model {
	match case.shape {
		...
		"hyper-attention" => recipe.model().pool(1).layer(4).hyper(2, 2, &recipe.model().attn(2).qk(rms).relu()).layer(1).loss(mse),
