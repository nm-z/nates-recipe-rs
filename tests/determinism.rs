...
const CASES: &[Case] = &[
	...
	Case { name: "attention-rope", shape: "attention-rope", precision: "fp32", rows: 131, columns: 17 },
	Case { name: "attention-rope-fp16", shape: "attention-rope", precision: "fp16", rows: 131, columns: 17 },
	Case { name: "attention-sparse", shape: "attention-sparse", precision: "fp32", rows: 131, columns: 17 },
	Case { name: "attention-sparse-fp16", shape: "attention-sparse", precision: "fp16", rows: 131, columns: 17 },
...
fn build(case: &Case) -> Model {
	match case.shape {
		...
		"hyper" => recipe.model().layer(8).hyper(4, 6, &recipe.model().layer(8).relu()).hyper(4, 6, &recipe.model().layer(8).gelu()).layer(1).loss(mse),
		"hyper-static" => recipe.model().layer(8).hyper(2, 0, &recipe.model().layer(8).relu()).layer(1).loss(mse),
		...
		"attention-rope" => recipe.model().conv(4, 3).attn(2).rope(2, 10000.0).relu().layer(1).loss(mse),
		"attention-sparse" => recipe.model().conv(8, 3).attn(4).kv(2).qk(rms).rope(2, 10000.0).index(1, 2, 4, 2).gate().relu().layer(1).loss(mse),
