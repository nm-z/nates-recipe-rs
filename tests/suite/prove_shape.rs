...
fn prove_pad() -> bool {
	...
	unsafe {
		launch_shapex_pad_constant(
			gx.ptr_raw() as *const c_void,
			o.ptr_raw(),
			r as i32,
			c as i32,
			pt as i32,
...
fn prove_fliplr() -> bool {
	...
	for i in 0..r {
		for j in 0..c {
			want[i * c + j] = x[i * c + (c - 1 - j)];
		}
	}
	veq(&got, &want)
}
fn prove_flip() -> bool {
	prove_flipud() && prove_fliplr()
}
...
fn prove_diagonal() -> bool {
	...
	for k in [-1i32, 0, 1] {
		let len = if k >= 0 {
			(c as i32 - k).min(r as i32)
		} else {
			(r as i32 + k).min(c as i32)
		} as usize;
		let o = GpuBuffer::alloc(len).unwrap();
		unsafe {
			launch_shapex_diagonal(
				gx.ptr_raw() as *const c_void,
				o.ptr_raw(),
				r as i32,
				c as i32,
	...
	for e in fs::read_dir(&dir).expect("no recipe.db").flatten() {
		...
		if p.extension().is_some_and(|x| x == "json") {
			...
			if let Some(ks) = v.get("kernels").and_then(|k| k.as_array()) {
				for k in ks {
					if k.get("category").and_then(|c| c.as_str()) != Some("shape") {
						continue;
					}
...
fn prove_shape() {
	...
	assert!(
		failures.is_empty(),
		"registered shape op(s) FAILED oracle: {:?}",
		failures
	);
