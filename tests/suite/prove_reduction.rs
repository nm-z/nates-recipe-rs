...
fn g_std_samp(x: &[f64]) -> f64 {
	g_var_samp(x).sqrt()
}
fn g_argmax(x: &[f64]) -> f64 {
	scalar(launch_reductionx_argmax, x)
}
...
fn registry() -> HashMap<&'static str, Op> {
	...
	op!("mean", g_mean, |x: &[f64]| x.iter().sum::<f64>()
		/ x.len() as f64);
	op!("max", g_max, |x: &[f64]| x
		.iter()
		.cloned()
		.fold(f64::NEG_INFINITY, f64::max));
	op!("min", g_min, |x: &[f64]| x
		.iter()
		.cloned()
	...
	op!("mean_abs", g_mean_abs, |x: &[f64]| x
		.iter()
		.map(|v| v.abs())
		.sum::<f64>()
		/ x.len() as f64);
	op!(
		"count_nonzero",
		g_count_nonzero,
		|x: &[f64]| x.iter().filter(|v| **v != 0.0).count() as f64
	);
	op!("any", g_any, |x: &[f64]| if x.iter().any(|v| *v != 0.0) {
		1.0
	} else {
		0.0
	});
	op!("all", g_all, |x: &[f64]| if x.iter().all(|v| *v != 0.0) {
		1.0
	} else {
		0.0
	});
	...
	op!("var_pop", g_var_pop, |x: &[f64]| {
		let mu = x.iter().sum::<f64>() / x.len() as f64;
		x.iter().map(|v| (v - mu).powi(2)).sum::<f64>() / x.len() as f64
	});
	op!("var_samp", g_var_samp, |x: &[f64]| {
		let mu = x.iter().sum::<f64>() / x.len() as f64;
		x.iter().map(|v| (v - mu).powi(2)).sum::<f64>() / (x.len() as f64 - 1.0)
	});
	op!("std_pop", g_std_pop, |x: &[f64]| {
		let mu = x.iter().sum::<f64>() / x.len() as f64;
		(x.iter().map(|v| (v - mu).powi(2)).sum::<f64>() / x.len() as f64).sqrt()
	});
	op!("std_samp", g_std_samp, |x: &[f64]| {
		let mu = x.iter().sum::<f64>() / x.len() as f64;
...
fn canon(name: &str) -> String {
	...
	let alias: &[(&str, &str)] = &[
		...
		("reducemean", "mean"),
		("meanforward", "mean"),
		("miopenmeanforward", "mean"),
		("cudnnreduce_avg", "mean"),
		("average", "mean"),
		...
		("reducemax", "max"),
		("maxforward", "max"),
		("miopenmaxforward", "max"),
		("cudnnreduce_max", "max"),
		("nppimax_8u_c1r", "max"),
		("vector_reduce_max", "max"),
		// ── min family ──
		("min", "min"),
		("amin", "min"),
		("reduce_min", "min"),
		("reducemin", "min"),
		("minforward", "min"),
		("miopenminforward", "min"),
		("cudnnreduce_min", "min"),
		("nppimin_8u_c1r", "min"),
		("vector_reduce_min", "min"),
		// ── prod family ──
		("prod", "prod"),
		("product", "prod"),
		("reduce_prod", "prod"),
		...
		("reduceproduct", "prod"),
		("prodforward", "prod"),
		("miopenprodforward", "prod"),
		("cudnnreduce_mul", "prod"),
		("vector_reduce_prod", "prod"),
		// ── l2 / euclidean norm family ──
		("norm", "l2"),
		("l2_norm", "l2"),
		("euclideannorm", "l2"),
		("frobenius_norm", "l2"),
		("multi_tensor_l2norm", "l2"),
		("cudnnreduce_norm2", "l2"),
		("nppinorm_l2_8u_c1r", "l2"),
		// ── asum / L1 norm family ──
		("abssum", "asum"),
		("cudnnreduce_norm1", "asum"),
		("nppinorm_l1_8u_c1r", "asum"),
		// ── sum-of-squares ──
		...
		("argmax_dim", "argmax"),
		("reduceargmax", "argmax"),
		("reduce_argmax", "argmax"),
		("idxmax", "argmax"),
		("vector_reduce_argmax", "argmax"),
		("maximumindices", "argmax"),
		// ── argmin family ──
		("argmin", "argmin"),
		("arg_min", "argmin"),
		("argmin_dim", "argmin"),
	...
	for e in fs::read_dir(&dir).expect("no recipe.db").flatten() {
		...
		if p.extension().is_some_and(|x| x == "json") {
			...
			if let Some(ks) = v.get("kernels").and_then(|k| k.as_array()) {
				for k in ks {
					if k.get("category").and_then(|c| c.as_str()) != Some("reduction") {
						continue;
					}
...
fn prove_reduction() {
	...
	for (k, op) in reg.iter() {
		...
		if !ok {
			failures.push(format!("{}: gpu={} oracle={}", k, got, want));
		}
		op_ok.insert(*k, ok);
	}

	{
		...
		if all_zero != 0.0 {
			failures.push(format!("any(all-zero)={} != 0", all_zero));
		}
		let has_zero = g_all(&[1.0, 0.0, 2.0]);
		if has_zero != 0.0 {
			failures.push(format!("all(has-zero)={} != 0", has_zero));
		}
	}
	...
	assert!(
		failures.is_empty(),
		"registered reduction op(s) FAILED oracle: {:?}",
		failures
	);
