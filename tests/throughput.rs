...
fn natural(name: &str, fallback: usize) -> usize { std::env::var(name).ok().map(|value| value.parse().unwrap_or_else(|_| panic!("{name} must be a positive integer"))).unwrap_or(fallback) }
...
fn training_throughput() {
	...
	for (precision, tflops, per_epoch, extents) in &results {
		let (peak_text, share) = match peak(precision) {
			Some(peak) => (format!("{peak:.2}"), format!("{:.4}%", tflops / peak * 100.0)),
			None => ("-".to_owned(), "-".to_owned()),
		};
		let _ = writeln!(table, "{precision:<9} {tflops:>10.3} {per_epoch:>10.3} {peak_text:>12} {share:>10}  {extents:?}");
	}
	report(table);
}
