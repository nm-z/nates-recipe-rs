...
pub fn register_composites(lua: &Lua) -> LuaResult<()> {
      ...
      g.set("exp", lua.create_function(|_, x: LuaGpuBuffer| {
            let buf = kernels::gpu_exp(&x.buf, x.len()).map_err(hip_err)?;
            Ok(LuaGpuBuffer::new(buf, x.rows, x.cols))
      })?)?;

      g.set("log", lua.create_function(|_, x: LuaGpuBuffer| {
            let buf = kernels::gpu_log(&x.buf, x.len()).map_err(hip_err)?;
            Ok(LuaGpuBuffer::new(buf, x.rows, x.cols))
      })?)?;

      g.set("sqrt", lua.create_function(|_, x: LuaGpuBuffer| {
            let buf = kernels::gpu_sqrt(&x.buf, x.len()).map_err(hip_err)?;
            Ok(LuaGpuBuffer::new(buf, x.rows, x.cols))
      })?)?;

      g.set("abs", lua.create_function(|_, x: LuaGpuBuffer| {
            let buf = kernels::gpu_abs(&x.buf, x.len()).map_err(hip_err)?;
            Ok(LuaGpuBuffer::new(buf, x.rows, x.cols))
      })?)?;

      g.set("neg", lua.create_function(|_, x: LuaGpuBuffer| {
            let buf = kernels::gpu_neg(&x.buf, x.len()).map_err(hip_err)?;
            Ok(LuaGpuBuffer::new(buf, x.rows, x.cols))
      })?)?;

      g.set("pow", lua.create_function(|_, (x, p): (LuaGpuBuffer, f64)| {
            let buf = kernels::gpu_pow(&x.buf, x.len(), p).map_err(hip_err)?;
            Ok(LuaGpuBuffer::new(buf, x.rows, x.cols))
      })?)?;

      g.set("clamp", lua.create_function(|_, (x, lo, hi): (LuaGpuBuffer, f64, f64)| {
      ...
      g.set("transpose", lua.create_function(|_, x: LuaGpuBuffer| {
            let buf = kernels::gpu_transpose(&x.buf, x.rows, x.cols).map_err(hip_err)?;
            Ok(LuaGpuBuffer::new(buf, x.cols, x.rows))
      })?)?;

      g.set("eye", lua.create_function(|_, n: usize| {
            let buf = kernels::gpu_eye(n).map_err(hip_err)?;
            Ok(LuaGpuBuffer::new(buf, n, n))
      })?)?;

      g.set("copy", lua.create_function(|_, x: LuaGpuBuffer| {
            let buf = kernels::gpu_copy(&x.buf, x.len()).map_err(hip_err)?;
            Ok(LuaGpuBuffer::new(buf, x.rows, x.cols))
      })?)?;

      g.set("where_mask", lua.create_function(|_, (cond, a, b): (LuaGpuBuffer, LuaGpuBuffer, LuaGpuBuffer)| {
            let buf = kernels::gpu_where_mask(&cond.buf, &a.buf, &b.buf, a.len()).map_err(hip_err)?;
            Ok(LuaGpuBuffer::new(buf, a.rows, a.cols))
      })?)?;

      g.set("slice_rows", lua.create_function(|_, (x, start, count): (LuaGpuBuffer, usize, usize)| {
