...
fn validate_source_aliases(
	aliases: &[SourceAliasContract],
	input_count: usize,
	output_count: usize,
...
fn validate_kind(stage: &ProgramStage, path: &str, errors: &mut Vec<ProgramValidationError>) {
	match &stage.kind {
		StageKind::ScalarMap { template } => {
			errors.extend(template.validate().err().map(|core_errors| {
		...
		StageKind::TiledContraction(contraction) => {
			...
			require(
				contraction.tile.output_x != 0
					&& contraction.tile.output_y != 0
		...
		StageKind::StableSortCompareExchange(compare) => {
			...
			require(
				compare.merge_width.is_power_of_two()
					&& compare.merge_width <= compare.network.padded_axis_length
...
fn expected_stage_resources(stage: &ProgramStage) -> Option<StageResourceBounds> {
	...
	let (flops, integer, atomic, shared, private) = match &stage.kind {
		StageKind::ScalarMap { template } => {
			...
			let slots = template.program.inputs.len()
				+ template.program.constants.len()
				+ template.program.instructions.len();
