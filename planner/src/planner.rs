...
use recipe_core::{
	AliasPermission, AliasRule, ArenaLayout, ArenaObject, ArenaObjectId, ArenaRelease, ArtifactBuildAccess, ArtifactBuildBinding, ArtifactBuildProvenance, ArtifactBuildRecipe, ArtifactBuildView, ArtifactDispatchGeometry, ArtifactId, ArtifactIdentity, ArtifactWorkBounds, ByteCount, ByteOffset, CalculationTask, CandidateIdentity, CapacityLedger, CompletionSlot, CompletionSlotId, DType, DeviceBytes, DeviceId, DeviceKind, Digest, DiscoveryProfile, DraftIdentity, DraftPlan, InitDataImage, InitDataImageMember, IterationDomain, KernelResourceBounds, KernelTemplate, KernelTemplateId, LinkId,
	LoopIterations, LoopTaskDomain, MetricId, MetricPurpose, MetricSlot, MetricSlotId, MetricTask, Nanoseconds, QueueSlot, QueueSlotId, ReservationLedger, ResourceManifest, RunPhase, ScalarLiteral, ScheduleWindow, StaticBufferAccess, SubmissionSlots, Task, TaskId, TaskKind, Topology, TransferEndpoint, TransferLaneClaim, TransferTask, ValueAliasContract, ValueBinding, ValueId, ValueSpec,
};
use recipe_language::{CalculationGraph, Tensor};
use recipe_primitives::{AccessMode, BufferLifetime, BufferOrigin, LoweredProgram, LoweringHardware, ProgramBuffer, ProgramStage, StageKind};
...
type PlacementOption = DeviceId;
type Assignment = Vec<PlacementOption>;
...
pub fn plan_program_candidates(program: &StaticCalculationProgram, topology: &Topology, discovery: &DiscoveryProfile, artifacts: &[ArtifactIdentity], reservations: &ReservationLedger, capacity: &CapacityLedger) -> PlannerResult<ProgramPlannerSearch> {
	program
		.validate()
	...
	enumerate_assignments(&choices, &mut Vec::new(), &mut |options| {
		...
		match lower_candidate(&context, &assignment, candidate) {
			...
			Err(error)
				if matches!(
					error.kind,
					PlannerErrorKind::NoRoute | PlannerErrorKind::DependencyConflict | PlannerErrorKind::CandidateInfeasible | PlannerErrorKind::InvalidCapacity
				) =>
			{
...
fn legal_choices(order: &[KernelTemplateId], topology: &Topology, discovery: &DiscoveryProfile) -> PlannerResult<Vec<Vec<PlacementOption>>> {
	if order.is_empty() {
		return Ok(Vec::new());
	}
	let gpus = topology
...
fn enumerate_assignments(choices: &[Vec<PlacementOption>], current: &mut Vec<PlacementOption>, visit: &mut impl FnMut(&[PlacementOption]) -> PlannerResult<()>) -> PlannerResult<()> {
...
fn graph_digest(order: &[KernelTemplateId], nodes: &BTreeMap<KernelTemplateId, &recipe_language::CalculationNode>, tensors: &BTreeMap<ValueId, &Tensor>, programs: &BTreeMap<KernelTemplateId, LoweredKernelPlan>, loop_iterations: LoopIterations, source_domains: &BTreeMap<KernelTemplateId, IterationDomain>, metrics: &[MetricEmission]) -> PlannerResult<recipe_core::Digest> {
...
fn candidate_identity(graph: recipe_core::Digest, topology: &Topology, discovery: &DiscoveryProfile, order: &[KernelTemplateId], assignment: &Assignment) -> CandidateIdentity {
	...
	digest.length(order.len());
	for (kernel, option) in order.iter().zip(assignment) {
		digest.u64(kernel.get());
...
fn lower_candidate(context: &PlanningContext<'_>, assignment: &Assignment, candidate: CandidateIdentity) -> PlannerResult<PlannedProgramCandidate> {
	...
	let mut scheduled = schedule(context.topology, context.discovery, &state.tasks).map_err(|error| {
		let kind = match error.kind {
			ScheduleErrorKind::DependencyCycle | ScheduleErrorKind::InvalidLifecycleDependency => PlannerErrorKind::DependencyConflict,
			ScheduleErrorKind::ArithmeticOverflow | ScheduleErrorKind::InsufficientCapacity => PlannerErrorKind::CandidateInfeasible,
	...
	let arena_layouts = pack_arenas(context.topology, &arena_objects, context.capacity).map_err(|error| PlannerError::new(PlannerErrorKind::InvalidCapacity, error.to_string()))?;
...
fn add_user_metrics(context: &PlanningContext<'_>, selected: &BTreeMap<KernelTemplateId, &PlacementOption>, state: &mut LoweringState) -> PlannerResult<()> {
	for emission in context.metrics {
		...
		let device = selected.get(&producer).copied().ok_or_else(|| {
			PlannerError::new(
				PlannerErrorKind::InvalidDraft,
...
fn lower_program_invocation(context: &PlanningContext<'_>, node: &recipe_language::CalculationNode, lowered: &LoweredKernelPlan, device: DeviceId, domain: IterationDomain, state: &mut LoweringState, artifacts: &mut LoweredArtifacts<'_>) -> PlannerResult<()> {
...
fn materialize_program_buffer(context: &PlanningContext<'_>, node: &recipe_language::CalculationNode, program: &LoweredProgram, buffer: &ProgramBuffer, device: DeviceId, domain: IterationDomain, state: &mut LoweringState) -> PlannerResult<(ValueId, Option<TaskId>)> {
	match buffer.origin {
...
fn lower_build_binding(binding: &recipe_primitives::BufferBinding, values: &BTreeMap<u32, ValueId>) -> PlannerResult<ArtifactBuildBinding> {
...
fn select_or_defer_artifact(context: &PlanningContext<'_>, device: DeviceId, build: &ArtifactBuildRecipe, chosen_artifacts: &mut Vec<ArtifactIdentity>, artifact_builds: &mut Vec<ArtifactBuildRecipe>) -> PlannerResult<()> {
	...
	match catalog_entry {
		Some(artifact) => {
			...
			if artifact.kernel_template != build.kernel_template || artifact.build != Some(build.provenance) || artifact.resources != build.resources || !target_matches {
				return Err(PlannerError::new(
					PlannerErrorKind::InvalidArtifact,
					format!(
...
fn program_tensor_value(program: &LoweredProgram, logical: ValueId, values: &BTreeMap<u32, ValueId>) -> PlannerResult<ValueId> {
	let buffer = program_tensor_buffer(program, logical)?;
...
fn first_output_writer(program: &LoweredProgram, node: &recipe_language::CalculationNode, dispatches: &BTreeMap<u32, (TaskId, SubmissionSlots)>) -> PlannerResult<Option<TaskId>> {
...
fn source_buffer<'program>(program: &'program LoweredProgram, node: &recipe_language::CalculationNode, input: bool, index: usize) -> PlannerResult<&'program ProgramBuffer> {
...
fn trial_timing(topology: &Topology, discovery: &DiscoveryProfile, tasks: &[UnscheduledTask], trial: UnscheduledTask) -> PlannerResult<Option<(Nanoseconds, Nanoseconds)>> {
	let trial_id = trial.id;
	trial_chain_timing(topology, discovery, tasks, &[trial], trial_id)
}

fn trial_chain_timing(topology: &Topology, discovery: &DiscoveryProfile, tasks: &[UnscheduledTask], trials: &[UnscheduledTask], completion: TaskId) -> PlannerResult<Option<(Nanoseconds, Nanoseconds)>> {
	...
	let scheduled = match schedule(topology, discovery, &tasks) {
		...
		Err(error)
			if matches!(
				error.kind,
				ScheduleErrorKind::ArithmeticOverflow | ScheduleErrorKind::InsufficientCapacity | ScheduleErrorKind::NoRoute
			) =>
		{
			return Ok(None);
		}
...
fn enumerate_directed_routes(current: DeviceId, destination: DeviceId, outgoing: &BTreeMap<DeviceId, Vec<(LinkId, DeviceId)>>, visited: &mut BTreeSet<DeviceId>, path: &mut Vec<LinkId>, routes: &mut Vec<Vec<LinkId>>) {
...
fn build_transfer_chain(source: RuntimeCopy, destination: DeviceId, route: &[LinkId], tensor: &Tensor, topology: &Topology, allocation: TransferAllocationStart, domain: IterationDomain) -> PlannerResult<TransferChain> {
...
fn initialize_data_images(context: &PlanningContext<'_>, selected: &BTreeMap<KernelTemplateId, &PlacementOption>, state: &mut LoweringState) -> PlannerResult<()> {
...
fn ensure_copy(logical: ValueId, destination: DeviceId, tensors: &BTreeMap<ValueId, &Tensor>, topology: &Topology, discovery: &DiscoveryProfile, consumer_domain: IterationDomain, state: &mut LoweringState) -> PlannerResult<RuntimeCopy> {
		...
		.filter(|source| source.transfer_domain.is_none() || source.transfer_domain == Some(consumer_domain))
	{
		for route in directed_routes(topology, source.device, destination) {
			...
			if best
				.as_ref()
				.is_none_or(|known| (&key.0, &key.1, &key.2, &key.3, &key.4) < (&known.0, &known.1, &known.2, &known.3, &known.4))
			{
				best = Some(key);
			}
		}
	...
	let (_, _, _, _, route, source) = best.ok_or_else(|| {
		...
		PlannerError::new(
			kind,
			format!("tensor {logical} has {route_count} directed routes to device {destination}, but {schedulable_count} are schedulable"),
		)
	})?;
	let chain = build_transfer_chain(
		source,
...
fn add_external_outputs(tensors: &BTreeMap<ValueId, &Tensor>, topology: &Topology, discovery: &DiscoveryProfile, aliases: &[AliasInvocation], state: &mut LoweringState) -> PlannerResult<()> {
...
fn compact_submission_resources(tasks: &mut [Task], values: &[ValueSpec], resources: &mut ResourceManifest, discovery: &DiscoveryProfile) -> PlannerResult<()> {
...
fn compact_submission_resources_with_limits(tasks: &mut [Task], values: &[ValueSpec], resources: &mut ResourceManifest, maximum_queues: &BTreeMap<DeviceId, u32>) -> PlannerResult<()> {
...
fn task_submission_device(task: &Task, value_devices: &BTreeMap<ValueId, DeviceId>) -> PlannerResult<(DeviceId, SubmissionOwnerClass)> {
	match &task.kind {
		...
		TaskKind::Transfer(transfer) => {
			match (transfer.source, transfer.destination) {
				...
				(TransferEndpoint::External, TransferEndpoint::Device { device, .. }) => Ok((device, SubmissionOwnerClass::DeviceLocal)),
...
fn finalize_auxiliary_resources(resources: &mut ResourceManifest, tasks: &[Task], artifacts: &[ArtifactIdentity], artifact_builds: &[ArtifactBuildRecipe]) -> PlannerResult<Vec<DeviceBytes>> {
		...
		match &task.kind {
			TaskKind::Transfer(transfer) => {
				let device = match (transfer.source, transfer.destination) {
					(TransferEndpoint::External, TransferEndpoint::Device { device, .. }) | (TransferEndpoint::Device { device, .. }, TransferEndpoint::External) => Some(device),
...
fn push_usage(events: &mut BTreeMap<DeviceId, Vec<UsageEvent>>, device: DeviceId, window: ScheduleWindow, bytes: ByteCount) {
...
fn build_arena_contract(values: &[ValueSpec], tasks: &[Task], images: &[ImageRegion], aliases: &[AliasInvocation], loop_iterations: LoopIterations, loop_domains: &BTreeMap<TaskId, IterationDomain>) -> PlannerResult<(Vec<ArenaObject>, Vec<ValueBinding>)> {
	let specs = values
		.iter()
	...
	for mut blueprint in blueprints {
		...
		if crosses_phase {
			...
			let crosses_iteration = blueprint.members.iter().try_fold(false, |crosses, value| {
				Ok(crosses || value_crosses_iteration_boundary(*value, specs[value], tasks, &task_index, loop_domains)?)
			})?;
...
fn value_crosses_phase_boundary(value: ValueId, spec: &ValueSpec, tasks: &[Task], task_index: &BTreeMap<TaskId, &Task>) -> PlannerResult<bool> {
	let producer = spec
		.producer
...
fn value_crosses_iteration_boundary(value: ValueId, spec: &ValueSpec, tasks: &[Task], task_index: &BTreeMap<TaskId, &Task>, loop_domains: &BTreeMap<TaskId, IterationDomain>) -> PlannerResult<bool> {
	let producer = spec
		.producer
		.and_then(|producer| task_index.get(&producer).copied());
	for reader in tasks
...
fn task_reads_value(task: &Task, value: ValueId) -> bool {
	match &task.kind {
		TaskKind::Calculation(calculation) => calculation.inputs.contains(&value) || calculation.fault_flag == Some(value),
		TaskKind::Transfer(transfer) => {
			matches!( transfer.source, TransferEndpoint::Device { value: candidate, ..
				} if candidate == value )
		}
...
fn task_kind_references(kind: &TaskKind, value: ValueId) -> bool {
	match kind {
		TaskKind::Calculation(calculation) => calculation.inputs.contains(&value) || calculation.outputs.contains(&value) || calculation.fault_flag == Some(value),
		TaskKind::Transfer(transfer) => {
			matches!( transfer.source, TransferEndpoint::Device { value: candidate, ..
				} if candidate == value )
				|| matches!( transfer.destination, TransferEndpoint::Device { value: candidate, ..
				} if candidate == value )
		}
		TaskKind::Metric(metric) => metric.value == value,
	}
...
fn require_total_capacity(capacity: &CapacityLedger, layouts: &[ArenaLayout], auxiliary_peak: &[DeviceBytes]) -> PlannerResult<()> {
