...
#[rustfmt::skip] fn iq1_s(values:&[f32],importance:&[f32])->Vec<u8>{
	let mut output=Vec::new();for(chunk,values)in values.chunks(256).enumerate(){let value=|index|values.get(index).copied().unwrap_or(0.0);let importance=|index|importance.get(chunk*256+index).copied().unwrap_or(0.0);let sigma2=2.0*(0..256).map(|index|value(index)*value(index)).sum::<f32>()/256.0;let mut packed=[0_u8;48];let mut scales=[0.0_f32;8];let mut shifts=[1_i8;8];let mut maximum=0.0_f32;
		for block in 0..8{let x=(0..32).map(|offset|value(block*32+offset)).collect::<Vec<_>>();let weights=(0..32).map(|offset|importance(block*32+offset)*(sigma2+x[offset]*x[offset]).sqrt()).collect::<Vec<_>>();let max=x.iter().map(|value|value.abs()).fold(0.0_f32,f32::max);let mut levels=[1_i8;32];if max<1.0e-12{continue}let mut pairs=x.iter().copied().enumerate().map(|(index,value)|(value,index)).collect::<Vec<_>>();pairs.sort_by(|left,right|left.0.total_cmp(&right.0));let(mut sumx,mut sumw)=([0.0_f32;33],[0.0_f32;33]);for j in 0..32{let index=pairs[j].1;sumx[j+1]=sumx[j]+weights[index]*x[index];sumw[j+1]=sumw[j]+weights[index]}let(mut best,mut scale,mut split,mut shift)=(f32::NEG_INFINITY,max,(0,0),0_i8);
			for first in 0..=32{for second in first..=32{for candidate in [1_i8,-1_i8]{let q=[-1.0+0.125*f32::from(candidate),0.125*f32::from(candidate),1.0+0.125*f32::from(candidate)];let qx=(sumx[first]-sumx[0])*q[0]+(sumx[second]-sumx[first])*q[1]+(sumx[32]-sumx[second])*q[2];let q2=(sumw[first]-sumw[0])*q[0]*q[0]+(sumw[second]-sumw[first])*q[1]*q[1]+(sumw[32]-sumw[second])*q[2]*q[2];if q2>0.0&&qx*qx>best*q2{scale=qx/q2;best=scale*qx;split=(first,second);shift=candidate}}}}if shift==0{continue}for(index,pair)in pairs.iter().enumerate(){levels[pair.1]=if index<split.0{0}else if index<split.1{1}else{2}}if scale<0.0{for level in &mut levels{*level=2-*level}scale=-scale;shift=-shift}
			let mut indices=[0_usize;4];let mut changed=false;for group in 0..4{let key=(0..8).fold(0_u16,|key,lane|key|(levels[group*8+lane]as u16)<<(2*lane));changed|=!IQ1.contains(&key);indices[group]=iq1_nearest(&mut levels[group*8..group*8+8],&x[group*8..group*8+8],&weights[group*8..group*8+8],scale,shift)}if changed{let(mut qx,mut q2)=(0.0,0.0);for lane in 0..32{let quant=f32::from(levels[lane])-1.0+0.125*f32::from(shift);qx+=weights[lane]*quant*x[lane];q2+=weights[lane]*quant*quant}if qx>0.0&&q2>0.0{scale=qx/q2}}let mut high=0_u16;for group in 0..4{packed[block*4+group]=indices[group]as u8;high|=((indices[group]>>8)as u16)<<(3*group)}packed[32+2*block..34+2*block].copy_from_slice(&high.to_le_bytes());scales[block]=scale;shifts[block]=shift;maximum=maximum.max(scale)}
		if maximum==0.0{put_half(&mut output,0.0);output.extend(packed);continue}let scale=maximum/15.0;for block in 0..8{let code=qround(0.5*(scales[block]/scale-1.0)).max(0.0).min(7.0)as u16|u16::from(shifts[block]<0)<<3;let mut high=u16::from_le_bytes(packed[32+2*block..34+2*block].try_into().unwrap());high|=code<<12;packed[32+2*block..34+2*block].copy_from_slice(&high.to_le_bytes())}put_half(&mut output,scale*1.125);output.extend(packed)}output
}
...
impl Integer for IntegerFormat {
	...
	fn compress(self, weights: &[f64], importance: &[f64], config: Config) -> Result<(Vec<u8>, Vec<f64>)> {
		...
		if family == 1 && variant == 3 && bits == 1 { require(importance.len()==weights.len()&&importance.iter().all(|value|value.is_finite()&&*value>=0.0)&&importance.iter().any(|value|*value>0.0),"GGML IQ1_S requires trained importance weights")?;return Ok((iq1_s(&weights.iter().map(|value|*value as f32).collect::<Vec<_>>(),&importance.iter().map(|value|*value as f32).collect::<Vec<_>>()),Vec::new())) }
		...
		Err(RecipeError::new(format!("quantization code {} is unavailable; available GGML formats: Q2_0, Q4_0, Q4_1, Q5_0, Q5_1, Q8_0, Q8_1, Q2_K, Q3_K, Q4_K, Q5_K, Q6_K, Q8_K, Q4_NF, IQ1_S, IQ2_XXS, IQ2_XS, IQ2_S, IQ3_XXS, IQ3_S, IQ4_NL, and IQ4_XS", self.0)))
	}
	fn decompress(self, data: &[u8], codebook: &[f64], count: usize) -> Result<Vec<f64>> {
