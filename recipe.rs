...
mod bundle { use super::*; use std::{collections::BTreeMap, io::Write as _, str::FromStr}; #[derive(Clone)] pub(super) struct StoredGraph { pub graph: Graph, pub inputs: Vec<String>, pub outputs: Vec<String>, } struct Builder { inputs: Vec<String>, outputs: Vec<String>, input: Option<Shape>, output: Option<Shape>, nodes: Vec<Node>, arguments: usize, parameters: Vec<f64>, frozen: Vec<u8>, programs: Vec<f64>, } impl Builder { fn new() -> Self { Self { inputs: Vec::new(), outputs: Vec::new(), input: None, output: None, nodes: Vec::new(), arguments: 0, parameters: Vec::new(), frozen: Vec::new(), programs: Vec::new(), } } fn finish(self) -> Result<StoredGraph> { let input = self.input.ok_or_else(|| RecipeError::new("model graph has no input shape"))?; let output = self.output.ok_or_else(|| RecipeError::new("model graph has no output shape"))?; require(!self.nodes.is_empty(), "model graph has no nodes")?; require(self.arguments == self.nodes.len(), "model graph node arguments are incomplete")?;
require(self.parameters.len() == self.frozen.len(), "model graph frozen weights are incomplete")?; require(self.inputs.len() == input.elements(), "model graph input schema has the wrong width")?; require(self.outputs.len() == output.elements(), "model graph output schema has the wrong width")?; let source = self.nodes.len() as i32 - 1; Ok(StoredGraph { graph: Graph { nodes: self.nodes, parameters: self.parameters, frozen: self.frozen, programs: self.programs, input, output, source, }, inputs: self.inputs, outputs: self.outputs, }) } } fn parse_values<T>(text: &str, role: &str) -> Result<Vec<T>> where T: FromStr, T::Err: fmt::Display, { text.split_whitespace() .map(|value| value.parse().map_err(|error| RecipeError::new(format!("invalid {role}: {error}")))) .collect() } fn primitive(value: i32) -> Result<Primitive> { match value { 0 => Ok(Primitive::Contraction), 2 => Ok(Primitive::Pool), 3 => Ok(Primitive::Gather), 4 => Ok(Primitive::Attention), 5 => Ok(Primitive::Scan), 6 => Ok(Primitive::Elementwise), 8 => Ok(Primitive::Normalize),
_ => Err(RecipeError::new(format!("model graph primitive {value} is invalid"))), } } fn node(values: &[i32]) -> Result<Node> { require(values.len() == 11, "model graph node descriptor has the wrong width")?; Ok(Node { op: primitive(values[0])?, source: values[1], second: values[2], input: Shape { channels: values[3] as usize, length: values[4] as usize }, output: Shape { channels: values[5] as usize, length: values[6] as usize }, offset: values[7] as usize, parameters: values[8] as usize, argument: [0.0; 9], program_offset: values[9] as usize, program_count: values[10] as usize, }) } pub(super) fn load(path: &str) -> Result<(String, Vec<StoredGraph>)> { require(path.ends_with(".ogdl"), "model path requires .ogdl")?; let document = fs::read_to_string(path).map_err(|error| RecipeError::new(format!("cannot read {path}: {error}")))?; let mut schema = String::new(); let mut builders = Vec::new(); let mut current: Option<Builder> = None; for line in document.lines() { let line = line.trim(); if line == "recipe-model" { continue; } if line == "graph" {
if let Some(builder) = current.take() { builders.push(builder.finish()?); } current = Some(Builder::new()); continue; } if let Some(value) = line.strip_prefix("schema ") { schema = value.to_owned(); continue; } let builder = current.as_mut().ok_or_else(|| RecipeError::new("model value precedes graph"))?; if let Some(value) = line.strip_prefix("in ") { builder.inputs.push(value.to_owned()); } else if let Some(value) = line.strip_prefix("out ") { builder.outputs.push(value.to_owned()); } else if let Some(value) = line.strip_prefix("shape ") { let shape = parse_values::<usize>(value, "model shape")?; require(shape.len() == 4, "model graph shape has the wrong width")?; builder.input = Some(Shape { channels: shape[0], length: shape[1] }); builder.output = Some(Shape { channels: shape[2], length: shape[3] }); } else if let Some(value) = line.strip_prefix("node ") { builder.nodes.push(node(&parse_values::<i32>(value, "node descriptor")?)?); } else if let Some(value) = line.strip_prefix("arguments ") { let values = parse_values::<f64>(value, "node argument")?;
require(values.len() == 9, "model graph node argument has the wrong width")?; builder .nodes .last_mut() .ok_or_else(|| RecipeError::new("model graph argument precedes node"))? .argument .copy_from_slice(&values); builder.arguments += 1; } else if let Some(value) = line.strip_prefix("programs") { builder.programs = parse_values(value, "scalar program")?; } else if let Some(value) = line.strip_prefix("weights") { builder.parameters = parse_values(value, "weight")?; } else if let Some(value) = line.strip_prefix("frozen") { builder.frozen = parse_values(value, "frozen weight")?; } else if !line.is_empty() { return Err(RecipeError::new(format!("model graph value is invalid: {line}"))); } } if let Some(builder) = current { builders.push(builder.finish()?); } require(!builders.is_empty(), "model has no graphs")?; Ok((schema, builders)) } fn join<T: ToString>(values: &[T]) -> String { values.iter().map(ToString::to_string).collect::<Vec<_>>().join(" ") } pub(super) fn save(path: &str, schema: &str, graphs: &[StoredGraph]) -> Result<()> {
require(path.ends_with(".ogdl"), "save requires an .ogdl model")?; require(!graphs.is_empty(), "model bundle has no graphs")?; let mut document = format!("recipe-model\n    schema {schema}\n"); for stored in graphs { document.push_str("    graph\n"); for name in &stored.inputs { document.push_str(&format!("        in {name}\n")); } for name in &stored.outputs { document.push_str(&format!("        out {name}\n")); } let graph = &stored.graph; document.push_str(&format!( "        shape {} {} {} {}\n", graph.input.channels, graph.input.length, graph.output.channels, graph.output.length )); for node in &graph.nodes { let descriptor = [ node.op as i32, node.source, node.second, node.input.channels as i32, node.input.length as i32, node.output.channels as i32, node.output.length as i32, node.offset as i32, node.parameters as i32, node.program_offset as i32, node.program_count as i32, ]; document.push_str(&format!("        node {}\n", join(&descriptor))); document.push_str(&format!("            arguments {}\n", join(&node.argument))); }
document.push_str(&format!("        programs {}\n", join(&graph.programs))); document.push_str(&format!("        weights {}\n", join(&graph.parameters))); document.push_str(&format!("        frozen {}\n", join(&graph.frozen))); } fs::write(path, document).map_err(|error| RecipeError::new(format!("cannot write {path}: {error}")))?; eprintln!("saved: {path}"); Ok(()) } fn same_node(left: &Node, right: &Node) -> bool { left.op == right.op && left.source == right.source && left.second == right.second && left.input == right.input && left.output == right.output && left.offset == right.offset && left.parameters == right.parameters && left.argument.map(f64::to_bits) == right.argument.map(f64::to_bits) && left.program_offset == right.program_offset && left.program_count == right.program_count } fn same_graph(left: &StoredGraph, right: &StoredGraph) -> bool { left.inputs == right.inputs && left.outputs == right.outputs && left.graph.input == right.graph.input && left.graph.output == right.graph.output && left.graph.frozen == right.graph.frozen && left.graph.programs == right.graph.programs
&& left.graph.parameters.len() == right.graph.parameters.len() && left.graph.nodes.len() == right.graph.nodes.len() && left.graph.nodes.iter().zip(&right.graph.nodes).all(|(a, b)| same_node(a, b)) } pub(super) fn restore(path: &str, schema: &str, graphs: &mut [StoredGraph]) -> Result<()> { if !fs::exists(path).map_err(|error| RecipeError::new(format!("cannot inspect {path}: {error}")))? { return save(path, schema, graphs); } let (stored_schema, stored) = load(path)?; let matches = stored_schema == schema && stored.len() == graphs.len() && stored.iter().zip(graphs.iter()).all(|(a, b)| same_graph(a, b)); if matches { for (current, saved) in graphs.iter_mut().zip(stored) { current.graph.parameters = saved.graph.parameters; } return Ok(()); } eprint!("mismatch: overwrite {path}? Y/n "); std::io::stderr() .flush() .map_err(|error| RecipeError::new(format!("cannot prompt: {error}")))?; let mut answer = String::new(); std::io::stdin() .read_line(&mut answer) .map_err(|error| RecipeError::new(format!("cannot read answer: {error}")))?;
require(answer.trim().is_empty() || answer.trim().eq_ignore_ascii_case("y"), "model mismatch not overwritten")?; save(path, schema, graphs) } pub(super) fn infer(path: &str, input: &[f64]) -> Result<Vec<f64>> { let (_, graphs) = load(path)?; let first = graphs.first().ok_or_else(|| RecipeError::new("model has no graph"))?; require(input.len() == first.inputs.len(), "model input has the wrong width")?; let mut values = first.inputs.iter().cloned().zip(input.iter().copied()).collect::<BTreeMap<_, _>>(); let backend = device_backend()?; let mut result = Vec::new(); for stored in graphs { let samples = stored .inputs .iter() .map(|name| values.get(name).copied().ok_or_else(|| RecipeError::new(format!("input {name:?} is absent")))) .collect::<Result<Vec<_>>>()?; let mut tape = DeviceTape::new(&stored.graph, &samples, &[], backend)?; tape.forward()?; result = tape.predictions()?; require(result.len() == stored.outputs.len(), "model output has the wrong width")?; for (name, value) in stored.outputs.iter().cloned().zip(result.iter().copied()) { values.insert(name, value); } } Ok(result) } }
mod rat { use super::*; use std::{ collections::{BTreeMap, BTreeSet}, io::{BufRead, BufReader, Write}, process::{Child, ChildStdin, ChildStdout, Command, Stdio}, }; struct Process { child: Child, input: Option<ChildStdin>, output: BufReader<ChildStdout>, } impl Process { fn spawn(command: &str) -> Result<Self> { require(!command.trim().is_empty(), ".every() requires a command")?; let mut child = Command::new(command) .stdin(Stdio::piped()) .stdout(Stdio::piped()) .spawn() .map_err(|error| RecipeError::new(format!("cannot start {command:?}: {error}")))?; let input = child.stdin.take().ok_or_else(|| RecipeError::new("RAT command stdin is absent"))?; let output = child.stdout.take().ok_or_else(|| RecipeError::new("RAT command stdout is absent"))?; Ok(Self { child, input: Some(input), output: BufReader::new(output) }) } fn read(&mut self) -> Result<BTreeMap<String, f64>> { let mut values = BTreeMap::new(); loop { let mut line = String::new(); let bytes = self .output .read_line(&mut line) .map_err(|error| RecipeError::new(format!("cannot read RAT command: {error}")))?;
require(bytes != 0, "RAT command exited before a blank-line frame terminator")?; let line = line.trim(); if line.is_empty() { require(!values.is_empty(), "RAT command returned an empty frame")?; return Ok(values); } let Some((name, value)) = line.split_once(char::is_whitespace) else { continue; }; let value = value .trim() .parse::<f64>() .map_err(|error| RecipeError::new(format!("RAT value {name:?} is invalid: {error}")))?; require(value.is_finite(), format!("RAT value {name:?} must be finite"))?; require(values.insert(name.to_owned(), value).is_none(), format!("RAT value {name:?} is duplicated"))?; } } fn write(&mut self, names: &[String], values: &[f64]) -> Result<()> { require(names.len() == values.len(), "RAT proposal has the wrong width")?; let input = self.input.as_mut().ok_or_else(|| RecipeError::new("RAT command stdin is closed"))?; writeln!(input, "proposal").map_err(|error| RecipeError::new(format!("cannot write RAT command: {error}")))?; for (name, value) in names.iter().zip(values) { writeln!(input, "    {name} {value}")
.map_err(|error| RecipeError::new(format!("cannot write RAT command: {error}")))?; } writeln!(input).map_err(|error| RecipeError::new(format!("cannot write RAT command: {error}")))?; input.flush().map_err(|error| RecipeError::new(format!("cannot flush RAT command: {error}"))) } } impl Drop for Process { fn drop(&mut self) { drop(self.input.take()); let _ = self.child.wait(); } } struct State { graphs: Vec<bundle::StoredGraph>, tapes: Vec<DeviceTape>, steps: Vec<usize>, schema: String, } pub struct RatTrain<const N: usize> { train: Train, models: [Model; N], command: Option<String>, process: Option<Process>, context: Option<BTreeMap<String, f64>>, state: Option<State>, } pub struct RatReport { proposal: Vec<f64>, prediction: Vec<f64>, measurement: Vec<f64>, } impl RatReport { pub fn proposal(&self) -> &[f64] { &self.proposal } pub fn prediction(&self) -> &[f64] { &self.prediction } pub fn measurement(&self) -> &[f64] { &self.measurement } } fn rat<const N: usize>(train: Train, models: [Model; N]) -> RatTrain<N> {
RatTrain { train, models, command: None, process: None, context: None, state: None } } impl Train { pub fn rat(self, proposer: Model, predictor: Model) -> RatTrain<2> { rat(self, [proposer, predictor]) } pub fn rats<const N: usize>(self, models: [Model; N]) -> RatTrain<N> { rat(self, models) } } fn values(names: &[String], source: &BTreeMap<String, f64>) -> Result<Vec<f64>> { names .iter() .map(|name| source.get(name).copied().ok_or_else(|| RecipeError::new(format!("RAT value {name:?} is absent")))) .collect() } fn schema(data: &Data) -> String { data.routes .iter() .map(|route| format!("{}->{}", route.inputs.join("|"), route.outputs.join("|"))) .chain(std::iter::once(format!("target->{}", data.target.join("|")))) .collect::<Vec<_>>() .join("/") } fn build<const N: usize>( models: &[Model; N], train: &Train, data: &Data, backend: Backend, config: Config, ) -> Result<State> { require(N >= 2, "RAT requires an intermediate model and a predictor")?; require(data.routes.len() + 1 == N, "RAT requires one .r#in().out() pair per intermediate model")?;
require(!data.target.is_empty(), "RAT requires .target()")?; let mut available = data.routes[0].inputs.iter().cloned().collect::<BTreeSet<_>>(); let mut graphs = Vec::with_capacity(N); for (index, route) in data.routes.iter().enumerate() { if let Some(downstream) = &models[index].downstream { require(downstream == &models[index + 1].blocks, "model-valued loss must name the next RAT model")?; } require(!route.inputs.is_empty() && !route.outputs.is_empty(), "RAT route names must not be empty")?; require(route.inputs.iter().all(|name| available.contains(name)), "RAT route input is not yet available")?; let sample = vec![ 0.0; route.inputs.len() ]; let prepared = Prepared { samples: sample, targets: vec![0.0], rows: 1, features: route.inputs.len(), schema: schema(data), }; let graph = compile_output(&models[index], &prepared, 1, backend, config, route.outputs.len())?; graphs.push(bundle::StoredGraph { graph, inputs: route.inputs.clone(), outputs: route.outputs.clone() }); available.extend(route.outputs.iter().cloned()); }
let route = data.routes.last().ok_or_else(|| RecipeError::new("RAT route is absent"))?; require(models[N - 1].downstream.is_none(), "the final RAT model requires a scalar loss")?; let mut inputs = route.inputs.clone(); inputs.extend(route.outputs.iter().cloned()); let prepared = Prepared { samples: vec![0.0; inputs.len()], targets: vec![0.0; data.target.len()], rows: 1, features: inputs.len(), schema: schema(data), }; let graph = compile_output(&models[N - 1], &prepared, 1, backend, config, data.target.len())?; graphs.push(bundle::StoredGraph { graph, inputs, outputs: data.target.clone() }); let schema = schema(data); if let Some(path) = &train.resume { bundle::restore(path, &schema, &mut graphs)?; eprintln!("resumed: {path}"); } let mut tapes = Vec::with_capacity(N); for stored in &graphs { let samples = vec![ 0.0; stored.inputs.len() ]; let targets = vec![ 0.0; stored.outputs.len() ]; tapes.push(DeviceTape::new(&stored.graph, &samples, &targets, backend)?); } Ok(State { graphs, tapes, steps: vec![0; N], schema }) }
fn forward(state: &mut State, context: &BTreeMap<String, f64>) -> Result<(BTreeMap<String, f64>, Vec<f64>)> { let mut fields = context.clone(); let mut proposal = Vec::new(); for index in 0..state.graphs.len() - 1 { let sample = values(&state.graphs[index].inputs, &fields)?; state.tapes[index].set_samples(&sample)?; state.tapes[index].forward()?; let output = state.tapes[index].predictions()?; proposal.extend_from_slice(&output); for (name, value) in state.graphs[index].outputs.iter().cloned().zip(output) { fields.insert(name, value); } } Ok((fields, proposal)) } fn train( state: &mut State, train: &Train, predictor: &Model, context: &BTreeMap<String, f64>, measurement: &[f64], config: Config, ) -> Result<Vec<f64>> { let run = RUN.fetch_add(1, Ordering::Relaxed) + 1; let mut prediction = Vec::new(); for epoch in 1..=train.epochs { let started = Instant::now(); let (fields, _) = forward(state, context)?; let last = state.graphs.len() - 1; let sample = values(&state.graphs[last].inputs, &fields)?; state.tapes[last].set_samples(&sample)?; state.tapes[last].set_targets(measurement)?;
state.tapes[last].set_frozen(&state.graphs[last].graph.frozen)?; state.steps[last] += 1; let (loss, _) = state.tapes[last].epoch( state.steps[last], train.learning_rate, predictor.loss, 0.0, config, false, )?; prediction = state.tapes[last].predictions()?; train.print(predictor, run, epoch, loss, measurement, &prediction, started, false); let seed = prediction.iter().map(|value| 2.0 * value / prediction.len() as f64).collect::<Vec<_>>(); state.tapes[last].set_targets(&seed)?; let frozen = vec![ 1; state.graphs[last].graph.parameters.len() ]; state.tapes[last].set_frozen(&frozen)?; state.tapes[last].epoch(state.steps[last], train.learning_rate, mse, 0.0, config, true)?; let mut gradient = state.graphs[last] .inputs .iter() .cloned() .zip(state.tapes[last].input_gradients()?) .collect::<BTreeMap<_, _>>(); for index in (0..last).rev() { let seed = state.graphs[index] .outputs .iter() .map(|name| gradient.get(name).copied().unwrap_or(0.0)) .collect::<Vec<_>>(); state.tapes[index].set_targets(&seed)?; state.tapes[index].set_frozen(&state.graphs[index].graph.frozen)?;
state.steps[index] += 1; state.tapes[index].epoch(state.steps[index], train.learning_rate, mse, 0.0, config, true)?; for (name, value) in state.graphs[index] .inputs .iter() .cloned() .zip(state.tapes[index].input_gradients()?) { *gradient.entry(name).or_insert(0.0) += value; } } } capture(state)?; Ok(prediction) } fn capture(state: &mut State) -> Result<()> { for (stored, tape) in state.graphs.iter_mut().zip(&state.tapes) { stored.graph.parameters = tape.weights(false)?; } Ok(()) } impl<const N: usize> RatTrain<N> { pub fn every(mut self, command: impl Into<String>) -> Self { self.command = Some(command.into()); self } pub fn save(mut self, path: impl Into<String>) -> Self { self.train.save = Some(path.into()); self } pub fn resume(mut self, path: impl Into<String>) -> Self { self.train.resume = Some(path.into()); self } fn process(&mut self) -> Result<&mut Process> { if self.process.is_none() { let command = self.command.as_deref().ok_or_else(|| RecipeError::new("RAT requires .every()"))?; self.process = Some(Process::spawn(command)?); }
self.process.as_mut().ok_or_else(|| RecipeError::new("RAT command is absent")) } fn check_interrupt(&mut self, state: Option<&mut State>) -> Result<()> { if !INTERRUPTED.load(Ordering::Acquire) { return Ok(()); } if let Some(state) = state { capture(state)?; if let Some(path) = &self.train.save { bundle::save(path, &state.schema, &state.graphs)?; } } drop(self.process.take()); std::process::exit(INTERRUPTED_EXIT) } pub fn run(&mut self, data: &Data) -> RatReport { SIGNAL.get_or_init(|| unsafe { signal(SIGINT, interrupt) }); self.try_run(data).unwrap_or_else(|error| panic!("{error}")) } fn try_run(&mut self, data: &Data) -> Result<RatReport> { if INTERRUPTED.load(Ordering::Acquire) { let mut state = self.state.take(); self.check_interrupt(state.as_mut())?; } let context_result = match self.context.take() { Some(context) => context, None => self.process()?.read()?, }; self.check_interrupt(None)?; let context = context_result; let backend = device_backend()?; let config = Config::load()?; if self.state.is_none() {
self.state = Some(build(&self.models, &self.train, data, backend, config)?); } let mut state = self.state.take().ok_or_else(|| RecipeError::new("RAT state is absent"))?; let proposed = forward(&mut state, &context); self.check_interrupt(Some(&mut state))?; let (fields, proposal) = proposed?; let names = state.graphs[..N - 1] .iter() .flat_map(|graph| graph.outputs.iter().cloned()) .collect::<Vec<_>>(); let written = self.process()?.write(&names, &proposal); self.check_interrupt(Some(&mut state))?; written?; let result = self.process()?.read(); self.check_interrupt(Some(&mut state))?; let result = result?; let measurement = values(&data.target, &result)?; let trained = train(&mut state, &self.train, &self.models[N - 1], &fields, &measurement, config); self.check_interrupt(Some(&mut state))?; let prediction = trained?; self.context = Some(result); if let Some(path) = &self.train.save { bundle::save(path, &state.schema, &state.graphs)?; } self.state = Some(state); Ok(RatReport { proposal, prediction, measurement }) } } }
mod driver { use super::*; use super::tile::{Geometry, Resources}; #[cfg(feature = "amd")] use super::tile::amd; #[cfg(feature = "nvidia")] use super::tile::nvidia; pub(super) struct Buffer { runtime: &'static Gpu, 	pub(super) pointer: u64, } impl Buffer { 	pub(super) fn new(runtime: &'static Gpu, bytes: usize) -> Result<Self> { Ok(Self { runtime, pointer: runtime.allocate(bytes)? }) 	} pub(super) fn upload<T>(runtime: &'static Gpu, values: &[T]) -> Result<Self> { let buffer = Self::new(runtime, size_of_val(values))?; runtime.upload(buffer.pointer, values.as_ptr().cast(), size_of_val(values))?; 		Ok(buffer) 	} pub(super) fn download<T: Copy + Default>(&self, count: usize) -> Result<Vec<T>> { 		self.runtime.synchronize()?; let mut values = std::iter::repeat_n(T::default(), count).collect::<Vec<_>>(); self.runtime.download(values.as_mut_ptr().cast(), self.pointer, size_of_val(&*values))?; 		Ok(values) 	} } impl Drop for Buffer { 	fn drop(&mut self) { 		self.runtime.free(self.pointer); 	} } #[derive(Clone, Copy)]
pub(super) struct Kernel { 	object: u64, 	#[cfg(feature = "amd")] 	kernarg: usize, 	#[cfg(feature = "amd")] 	group: u32, #[cfg(feature = "amd")] 	private: u32, 	#[cfg(feature = "amd")] 	layout: &'static [u8], } const FORWARD_ARGS: &[u8] = b"888888444"; const EPOCH_ARGS: &[u8] = b"8888888888888888444488888888844"; #[cfg(feature = "nvidia")] struct Cuda { allocate: unsafe extern "C" fn(*mut u64, usize) -> i32, 	free: unsafe extern "C" fn(u64) -> i32, upload: unsafe extern "C" fn(u64, *const c_void, usize) -> i32, download: unsafe extern "C" fn(Ptr, u64, usize) -> i32, 	synchronize: unsafe extern "C" fn() -> i32, launch: unsafe extern "C" fn(usize, u32, u32, u32, u32, u32, u32, u32, Ptr, *mut Ptr) -> i32, } #[cfg(feature = "nvidia")] impl Kernel { 	const fn cuda(object: usize, _layout: &'static [u8]) -> Self { 		Self { object: object as u64, 			#[cfg(feature = "amd")] 			kernarg: 0, 			#[cfg(feature = "amd")] 			group: 0, #[cfg(feature = "amd")] 			private: 0, 			#[cfg(feature = "amd")] 			layout: _layout, 		} 	} } #[cfg(feature = "amd")] #[allow(dead_code)] struct Hsa {
allocate: unsafe extern "C" fn(u64, usize, u32, *mut Ptr) -> i32, 	free: unsafe extern "C" fn(Ptr) -> i32, allow: unsafe extern "C" fn(u32, *const u64, *const u32, *const c_void) -> i32, copy: unsafe extern "C" fn(Ptr, *const c_void, usize) -> i32, 	store: unsafe extern "C" fn(u64, i64), wait: unsafe extern "C" fn(u64, i32, i64, u64, i32) -> i64, 	write: unsafe extern "C" fn(*const HsaQueue, u64) -> u64, queue: Ptr, 	signal: u64, 	cpu_agent: u64, 	vram_pool: u64, 	kernarg_pool: u64, 	kernarg_size: usize, 	kernarg: Ptr, _code: fs::File, } enum Driver { 	#[cfg(feature = "amd")] 	Hsa(Hsa), 	#[cfg(feature = "nvidia")] 	Cuda(Cuda), } #[allow(dead_code)] pub(super) struct Gpu { backend: Backend, 	driver: Driver, 	geometry: Geometry, 	pub(super) forward: Kernel, pub(super) epoch: Kernel, 	dispatch: Mutex<()>, } unsafe impl Send for Gpu {} unsafe impl Sync for Gpu {} #[cfg(feature = "amd")] #[repr(C)] struct HsaQueue { 	kind: u32, 	features: u32, 	base: Ptr, 	doorbell: u64, 	size: u32, 	reserved: u32,
id: u64, } #[cfg(feature = "amd")] #[repr(C)] struct HsaPacket { 	header: u16, 	setup: u16, 	workgroup_x: u16, workgroup_y: u16, 	workgroup_z: u16, 	reserved0: u16, 	grid_x: u32, 	grid_y: u32, 	grid_z: u32, 	private: u32, group: u32, 	object: u64, 	kernarg: Ptr, 	reserved1: u64, 	completion: u64, } #[cfg(feature = "nvidia")] type Count = unsafe extern "C" fn(*mut i32) -> i32; #[cfg(feature = "nvidia")] type Attribute = unsafe extern "C" fn(*mut i32, i32, i32) -> i32; #[cfg(feature = "nvidia")] type Device = unsafe extern "C" fn(*mut i32, i32) -> i32; #[cfg(feature = "nvidia")] type Context = unsafe extern "C" fn(*mut Ptr, u32, i32) -> i32; #[cfg(feature = "nvidia")] type Module = unsafe extern "C" fn(*mut Ptr, *const u8) -> i32; #[cfg(feature = "nvidia")] type Function = unsafe extern "C" fn(*mut usize, Ptr, *const u8) -> i32; #[cfg(feature = "nvidia")] type FunctionAttribute = unsafe extern "C" fn(*mut i32, i32, usize) -> i32; #[cfg(feature = "nvidia")] type Occupancy = unsafe extern "C" fn(*mut i32, usize, i32, usize) -> i32;
#[cfg(any(feature = "amd", feature = "nvidia"))] struct Library(Ptr); #[cfg(any(feature = "amd", feature = "nvidia"))] impl Library { 	fn open(name: &str) -> Result<Self> { 		let name = format!("{name}\0"); let handle = unsafe { dlopen(name.as_ptr().cast(), 2) }; require(!handle.is_null(), format!("cannot load {name:?}"))?; 		Ok(Self(handle)) 	} fn function<F: Copy>(&self, name: &[u8]) -> Result<F> { 		let pointer = unsafe { dlsym(self.0, name.as_ptr().cast()) }; require(!pointer.is_null(), format!("runtime symbol {:?} is absent", name))?; Ok(unsafe { std::mem::transmute_copy(&pointer) }) 	} } fn driver_status(backend: Backend, status: i32, action: &str) -> Result<()> { (status == 0).then_some(()).ok_or_else(|| RecipeError::new(format!("{backend:?} {action} failed: {status}"))) } impl Gpu { 	fn status(&self, status: i32, action: &str) -> Result<()> { 		driver_status(self.backend, status, action) 	} pub(super) fn threads(&self) -> Result<u32> { 		self.geometry.threads() 	} fn allocate(&self, bytes: usize) -> Result<u64> {
unsafe { 			match &self.driver { 				#[cfg(feature = "nvidia")] 				Driver::Cuda(driver) => { let mut pointer = 0; self.status((driver.allocate)(&mut pointer, bytes), "allocation")?; 					Ok(pointer) 				} #[cfg(feature = "amd")] 				Driver::Hsa(driver) => { 					let mut pointer = ptr::null_mut(); self.status((driver.allocate)(driver.vram_pool, bytes, 0, &mut pointer), "allocation")?; 					self.status( (driver.allow)(1, &driver.cpu_agent, ptr::null(), pointer), 						"CPU allocation access", 					)?; Ok(pointer as u64) 				} 			} 		} 	} 	fn free(&self, pointer: u64) { 		unsafe { 			match &self.driver { #[cfg(feature = "nvidia")] 				Driver::Cuda(driver) => { 					(driver.free)(pointer); 				} #[cfg(feature = "amd")] 				Driver::Hsa(driver) => { 					(driver.free)(pointer as Ptr); 				} 			} 		} 	} fn upload(&self, dst: u64, src: *const c_void, bytes: usize) -> Result<()> { 		unsafe { 			match &self.driver { #[cfg(feature = "nvidia")] 				Driver::Cuda(driver) => self.status((driver.upload)(dst, src, bytes), "upload"),
#[cfg(feature = "amd")] 				Driver::Hsa(driver) => self.status((driver.copy)(dst as Ptr, src, bytes), "upload"), } 		} 	} 	fn download(&self, dst: Ptr, src: u64, bytes: usize) -> Result<()> { 		unsafe { 			match &self.driver { #[cfg(feature = "nvidia")] 				Driver::Cuda(cuda) => self.status((cuda.download)(dst, src, bytes), "download"), #[cfg(feature = "amd")] Driver::Hsa(driver) => self.status((driver.copy)(dst, src as *const c_void, bytes), "download"), 			} 		} 	} fn synchronize(&self) -> Result<()> { 		unsafe { 			match &self.driver { 				#[cfg(feature = "nvidia")] Driver::Cuda(driver) => self.status((driver.synchronize)(), "synchronization"), 				#[cfg(feature = "amd")] Driver::Hsa(driver) => require((driver.wait)(driver.signal, 0, 0, u64::MAX, 1) == 0, "AMD synchronization failed"), } 		} 	} 	pub(super) fn launch(&self, kernel: Kernel, arguments: &mut [Ptr]) -> Result<()> { require(!INTERRUPTED.load(Ordering::Acquire), "interrupted before GPU dispatch")?; let _guard = self.dispatch.lock().map_err(|_| RecipeError::new("GPU dispatch lock is poisoned"))?; 		unsafe {
match &self.driver { 				#[cfg(feature = "nvidia")] 				Driver::Cuda(driver) => { 					let stream = ptr::null_mut(); self.status( 						(driver.launch)( 							kernel.object as usize, self.geometry.groups, 							1, 							1, 							self.geometry.block, 1, 							1, 							0, 							stream, 							arguments.as_mut_ptr(), 						), "dispatch", 					) 				} 				#[cfg(feature = "amd")] 				Driver::Hsa(driver) => { require(arguments.len() == kernel.layout.len(), "kernel argument count is invalid")?; ptr::write_bytes(driver.kernarg.cast::<u8>(), 0, driver.kernarg_size); let mut offset = 0; 					for (argument, kind) in arguments.iter().zip(kernel.layout) { let bytes = usize::from(*kind - b'0'); 						ptr::copy_nonoverlapping( 							(*argument).cast::<u8>(), driver.kernarg.cast::<u8>().add(offset), 							bytes, 						); offset += bytes; 					} 					require( 						offset <= kernel.kernarg && kernel.kernarg <= driver.kernarg_size, "kernarg layout is invalid", 					)?; (driver.store)(driver.signal, 1); let queue = &mut *(driver.queue as *mut HsaQueue);
let index = (driver.write)(queue, 1); 					let packet = queue.base.cast::<HsaPacket>().add(index as usize & (queue.size as usize - 1)); 					packet.write(HsaPacket { header: 0, 						setup: 1, 						workgroup_x: self.geometry.block as u16, workgroup_y: 1, 						workgroup_z: 1, reserved0: 0, 						grid_x: self.threads()?, 						grid_y: 1, 						grid_z: 1, 						private: kernel.private, group: kernel.group, 						object: kernel.object, 						kernarg: driver.kernarg, 						reserved1: 0, completion: driver.signal, 					}); std::sync::atomic::fence(Ordering::Release); let header = &*(&mut (*packet).header as *mut u16 as *mut std::sync::atomic::AtomicU16); header.store(2 | 2 << 9 | 2 << 11, Ordering::Release); (driver.store)(queue.doorbell, index as i64); require((driver.wait)(driver.signal, 0, 0, u64::MAX, 1) == 0, "AMD dispatch failed") 				} 			} 		} 	} } static AMD: OnceLock<Result<Gpu>> = OnceLock::new(); static NVIDIA: OnceLock<Result<Gpu>> = OnceLock::new(); pub(super) fn device_backend() -> Result<Backend> {
let mut failures = Vec::new(); 	for backend in [Backend::Amd, Backend::Nvidia] { 		match gpu(backend) { Ok(_) => return Ok(backend), 			Err(error) => failures.push(error.to_string()), 		} 	} Err(RecipeError::new(failures.join("; "))) } pub(super) fn gpu(backend: Backend) -> Result<&'static Gpu> { let loaded = match backend { 		Backend::Amd => AMD.get_or_init(load_amd), Backend::Nvidia => NVIDIA.get_or_init(load_nvidia), 	}; 	loaded.as_ref().map_err(Clone::clone) } #[cfg(feature = "nvidia")] fn discrete(count: i32, mut probe: impl FnMut(i32) -> Result<Option<i32>>) -> Result<i32> { (0..count) 		.map(&mut probe) 		.find_map(|result| result.transpose()) 		.transpose()? .ok_or_else(|| RecipeError::new("Nvidia has no discrete GPU")) } #[cfg(feature = "amd")] type HsaInfo = unsafe extern "C" fn(u64, i32, Ptr) -> i32; #[cfg(feature = "amd")] struct HsaQuery { 	info: HsaInfo, attribute: i32, 	expected: u32, 	secondary: i32, 	mask: u32, 	found: u64, } #[cfg(feature = "amd")] extern "C" fn collect_hsa(handle: u64, pointer: Ptr) -> i32 { 	unsafe { 		let query = &mut *pointer.cast::<HsaQuery>();
let mut value = 0; let mut status = (query.info)(handle, query.attribute, (&mut value as *mut u32).cast()); if status != 0 || value != query.expected { 			return status; 		} 		if query.secondary >= 0 { status = (query.info)(handle, query.secondary, (&mut value as *mut u32).cast()); if status != 0 || value & query.mask == 0 { 				return status; 			} 		} 		if query.found == 0 { query.found = handle; 		} 		0 	} } #[cfg(feature = "amd")] struct HsaGpuQuery { 	info: HsaInfo, 	found: u64, } #[cfg(feature = "amd")] extern "C" fn collect_discrete_hsa(handle: u64, pointer: Ptr) -> i32 { 	unsafe { let query = &mut *pointer.cast::<HsaGpuQuery>(); let mut device = 0_u32; let mut status = (query.info)(handle, 17, (&mut device as *mut u32).cast()); 		if status != 0 || device != 1 { return status; 		} let mut properties = 0_u64; status = (query.info)(handle, 0xA114, (&mut properties as *mut u64).cast()); 		if status != 0 || properties & 1 != 0 { return status; 		} 		if query.found == 0 { query.found = handle; 		} 		0 	} } #[cfg(feature = "amd")]
type HsaSymbol = unsafe extern "C" fn(u64, *const u8, *const u64, *mut u64) -> i32; #[cfg(feature = "amd")] type HsaSymbolInfo = unsafe extern "C" fn(u64, i32, Ptr) -> i32; #[cfg(feature = "amd")] unsafe fn hsa_kernel( symbol: HsaSymbol, 	info: HsaSymbolInfo, 	executable: u64, 	agent: u64, 	name: &'static [u8], 	layout: &'static [u8], ) -> Result<Kernel> { 	let mut handle = 0; driver_status(Backend::Amd, unsafe { symbol(executable, name.as_ptr(), &agent, &mut handle) }, "kernel lookup")?; let mut kernel = Kernel { object: 0, kernarg: 0, group: 0, private: 0, layout }; 	for (attribute, output) in [ (22, (&mut kernel.object as *mut u64).cast()), 		(11, (&mut kernel.kernarg as *mut usize).cast()), (13, (&mut kernel.group as *mut u32).cast()), 		(14, (&mut kernel.private as *mut u32).cast()), 	] { driver_status(Backend::Amd, unsafe { info(handle, attribute, output) }, "kernel metadata")?; 	} 	Ok(kernel) } fn load_amd() -> Result<Gpu> { 	#[cfg(not(feature = "amd"))] return Err(RecipeError::new("AMD support is not compiled into this build")); 	#[cfg(feature = "amd")] 	unsafe {
let runtime = Library::open(env!("RECIPE_HSA_RUNTIME"))?; let init: unsafe extern "C" fn() -> i32 = runtime.function(b"hsa_init\0")?; let iterate: unsafe extern "C" fn(extern "C" fn(u64, Ptr) -> i32, Ptr) -> i32 = runtime.function(b"hsa_iterate_agents\0")?; let info: HsaInfo = runtime.function(b"hsa_agent_get_info\0")?; driver_status(Backend::Amd, init(), "initialization")?; let mut cpu = HsaQuery { info, attribute: 17, expected: 0, secondary: -1, mask: 0, found: 0 }; let mut gpu = HsaGpuQuery { info, found: 0 }; driver_status(Backend::Amd, iterate(collect_hsa, (&mut cpu as *mut HsaQuery).cast()), "CPU agent")?; 		driver_status( Backend::Amd, 			iterate(collect_discrete_hsa, (&mut gpu as *mut HsaGpuQuery).cast()), 			"GPU agent", 		)?; require(cpu.found != 0 && gpu.found != 0, "AMD CPU or discrete GPU agent is absent")?; let pool_info: HsaInfo = runtime.function(b"hsa_amd_memory_pool_get_info\0")?; let pool_iterate: unsafe extern "C" fn(u64, extern "C" fn(u64, Ptr) -> i32, Ptr) -> i32 = runtime.function(b"hsa_amd_agent_iterate_memory_pools\0")?;
let mut vram = HsaQuery { info: pool_info, attribute: 0, expected: 0, secondary: 1, mask: 4, found: 0 }; let mut kernarg = HsaQuery { info: pool_info, attribute: 0, expected: 0, secondary: 1, mask: 1, found: 0 }; driver_status( 			Backend::Amd, 			pool_iterate(gpu.found, collect_hsa, (&mut vram as *mut HsaQuery).cast()), "VRAM pools", 		)?; 		driver_status( 			Backend::Amd, pool_iterate(cpu.found, collect_hsa, (&mut kernarg as *mut HsaQuery).cast()), 			"KERNARG pools", 		)?; require(vram.found != 0 && kernarg.found != 0, "AMD VRAM or KERNARG pool is absent")?; let (mut wave, mut workgroup, mut available, mut node, mut waves, mut simds, mut cus) = (0, 0, 0, 0, 0, 0, 0); for (attribute, output, action) in [ (6, (&mut wave as *mut u32).cast(), "wave query"), (8, (&mut workgroup as *mut u32).cast(), "workgroup query"), (0xA002, (&mut available as *mut u32).cast(), "CU query"), (0xA004, (&mut node as *mut u32).cast(), "KFD node query"), (0xA00A, (&mut waves as *mut u32).cast(), "wave occupancy query"), (0xA00B, (&mut simds as *mut u32).cast(), "SIMD query"),
(0xA014, (&mut cus as *mut u32).cast(), "cooperative CU query"), ] { driver_status(Backend::Amd, info(gpu.found, attribute, output), action)?; } require(cus <= available, "AMD cooperative CU count exceeds available CUs")?; let code = fs::File::open(env!("RECIPE_HSA_CODE_OBJECT")) .map_err(|error| RecipeError::new(format!("cannot open HSA code object: {error}")))?; let reader_create: unsafe extern "C" fn(i32, *mut u64) -> i32 = runtime.function(b"hsa_code_object_reader_create_from_file\0")?; let executable_create: unsafe extern "C" fn(i32, i32, Ptr, *mut u64) -> i32 = runtime.function(b"hsa_executable_create_alt\0")?; let executable_load: unsafe extern "C" fn(u64, u64, u64, Ptr, Ptr) -> i32 = runtime.function(b"hsa_executable_load_agent_code_object\0")?; let executable_freeze: unsafe extern "C" fn(u64, Ptr) -> i32 = 			runtime.function(b"hsa_executable_freeze\0")?; let symbol: HsaSymbol = runtime.function(b"hsa_executable_get_symbol_by_name\0")?; let symbol_info: HsaSymbolInfo = runtime.function(b"hsa_executable_symbol_get_info\0")?; let (mut reader, mut executable) = (0, 0);
let descriptor = std::os::fd::AsRawFd::as_raw_fd(&code); driver_status(Backend::Amd, reader_create(descriptor, &mut reader), "code-object reader")?; 		driver_status( Backend::Amd, 			executable_create(1, 0, ptr::null_mut(), &mut executable), 			"executable creation", 		)?; driver_status( 			Backend::Amd, 			executable_load(executable, gpu.found, reader, ptr::null_mut(), ptr::null_mut()), "code-object load", 		)?; driver_status(Backend::Amd, executable_freeze(executable, ptr::null_mut()), "executable freeze")?; let forward = hsa_kernel(symbol, symbol_info, executable, gpu.found, b"forward_graph.kd\0", FORWARD_ARGS)?; let epoch = hsa_kernel(symbol, symbol_info, executable, gpu.found, b"tape_epoch_graph.kd\0", EPOCH_ARGS)?; let compiled = |name, text| -> Result<u32> { Ok(narrow(natural(name, text)?, name)? as u32) }; let resources = Resources { registers: compiled("HSA forward VGPRs", env!("RECIPE_HSA_FORWARD_VGPRS"))? .max(compiled("HSA epoch VGPRs", env!("RECIPE_HSA_EPOCH_VGPRS"))?), shared: forward.group.max(epoch.group),
max_block: compiled("HSA forward workgroup", env!("RECIPE_HSA_FORWARD_MAX_BLOCK"))? .min(compiled("HSA epoch workgroup", env!("RECIPE_HSA_EPOCH_MAX_BLOCK"))?), }; let geometry = amd(cus, wave, workgroup, waves, simds, node, resources)?; let queue_create: unsafe extern "C" fn(u64, u32, u32, Ptr, Ptr, u32, u32, *mut Ptr) -> i32 = runtime.function(b"hsa_queue_create\0")?; let signal_create: unsafe extern "C" fn(i64, u32, *const u64, *mut u64) -> i32 = runtime.function(b"hsa_signal_create\0")?; 		let allocate: unsafe extern "C" fn(u64, usize, u32, *mut Ptr) -> i32 = runtime.function(b"hsa_amd_memory_pool_allocate\0")?; let allow: unsafe extern "C" fn(u32, *const u64, *const u32, *const c_void) -> i32 = runtime.function(b"hsa_amd_agents_allow_access\0")?; let (ka_size, mut ka) = (forward.kernarg.max(epoch.kernarg), ptr::null_mut()); let (mut queue, mut completion) = (ptr::null_mut(), 0); 		driver_status( 			Backend::Amd, queue_create(gpu.found, 256, 2, ptr::null_mut(), ptr::null_mut(), u32::MAX, u32::MAX, &mut queue), "queue creation", 		)?;
driver_status(Backend::Amd, signal_create(1, 0, ptr::null(), &mut completion), "signal creation")?; driver_status(Backend::Amd, allocate(kernarg.found, ka_size, 0, &mut ka), "KERNARG allocation")?; driver_status(Backend::Amd, allow(1, &gpu.found, ptr::null(), ka), "GPU KERNARG access")?; eprintln!("AMD grid {} block {}", geometry.groups, geometry.block); 		Ok(Gpu { backend: Backend::Amd, 			driver: Driver::Hsa(Hsa { allocate, 				free: runtime.function(b"hsa_amd_memory_pool_free\0")?, 				allow, copy: runtime.function(b"hsa_memory_copy\0")?, 				store: runtime.function(b"hsa_signal_store_screlease\0")?, wait: runtime.function(b"hsa_signal_wait_scacquire\0")?, write: runtime.function(b"hsa_queue_add_write_index_scacq_screl\0")?, 				queue, 				signal: completion, cpu_agent: cpu.found, 				vram_pool: vram.found, 				kernarg_pool: kernarg.found, 				kernarg_size: ka_size, kernarg: ka, 				_code: code, 			}), 			geometry, 			forward, 			epoch, dispatch: Mutex::new(()), 		}) 	} } fn load_nvidia() -> Result<Gpu> { 	#[cfg(not(feature = "nvidia"))]
return Err(RecipeError::new("NVIDIA support is not compiled into this build")); 	#[cfg(feature = "nvidia")] 	unsafe { const MAX_BLOCK: i32 = 1; const BLOCK_LDS: i32 = 8; const WAVE: i32 = 10; const CUS: i32 = 16; const INTEGRATED: i32 = 18; const THREADS_PER_SM: i32 = 39; const SM_LDS: i32 = 81; const REGISTERS_PER_SM: i32 = 82; const COOPERATIVE: i32 = 95; let runtime = Library::open(env!("RECIPE_NV_RUNTIME"))?; let init: unsafe extern "C" fn(u32) -> i32 = runtime.function(b"cuInit\0")?; let count_devices: Count = runtime.function(b"cuDeviceGetCount\0")?; let get_device: Device = runtime.function(b"cuDeviceGet\0")?; let attribute: Attribute = runtime.function(b"cuDeviceGetAttribute\0")?; let create: Context = runtime.function(b"cuCtxCreate_v2\0")?; let load: Module = runtime.function(b"cuModuleLoad\0")?; let function: Function = runtime.function(b"cuModuleGetFunction\0")?; let function_attribute: FunctionAttribute = runtime.function(b"cuFuncGetAttribute\0")?; let occupancy: Occupancy = runtime.function(b"cuOccupancyMaxActiveBlocksPerMultiprocessor\0")?;
let (mut count, mut forward, mut epoch) = (0, 0, 0); let (mut context, mut module) = (ptr::null_mut(), ptr::null_mut()); driver_status(Backend::Nvidia, init(0), "initialization")?; driver_status(Backend::Nvidia, count_devices(&mut count), "device enumeration")?; let device = discrete(count, |ordinal| { 			let (mut device, mut integrated) = (0, 0); driver_status(Backend::Nvidia, get_device(&mut device, ordinal), "device enumeration")?; driver_status(Backend::Nvidia, attribute(&mut integrated, INTEGRATED, device), "device probe")?; Ok((integrated == 0).then_some(device)) 		})?; let (mut cus, mut wave, mut workgroup, mut block_lds, mut sm_lds, mut registers, mut threads, mut cooperative) = (0, 0, 0, 0, 0, 0, 0, 0); for (kind, output, action) in [ (CUS, &mut cus, "SM query"), 			(WAVE, &mut wave, "warp query"), (MAX_BLOCK, &mut workgroup, "workgroup query"), 			(BLOCK_LDS, &mut block_lds, "workgroup LDS query"), (SM_LDS, &mut sm_lds, "SM LDS query"), 			(REGISTERS_PER_SM, &mut registers, "register query"), (THREADS_PER_SM, &mut threads, "resident thread query"),
(COOPERATIVE, &mut cooperative, "cooperative launch query"), ] { driver_status(Backend::Nvidia, attribute(output, kind, device), action)?; } require(cooperative != 0, "Nvidia device does not support cooperative launch")?; driver_status(Backend::Nvidia, create(&mut context, 0, device), "context creation")?; 		driver_status( Backend::Nvidia, 			load(&mut module, concat!(env!("RECIPE_NV_MODULE"), "\0").as_ptr()), 			"module load", 		)?; driver_status( 			Backend::Nvidia, 			function(&mut forward, module, b"forward_graph\0".as_ptr()), 			"forward load", )?; driver_status(Backend::Nvidia, function(&mut epoch, module, b"tape_epoch_graph\0".as_ptr()), "epoch load")?; let resource = |kernel| -> Result<Resources> { let (mut max_block, mut shared, mut used_registers) = (0, 0, 0); for (kind, output, action) in [ (0, &mut max_block, "kernel workgroup query"), 				(1, &mut shared, "kernel LDS query"), (4, &mut used_registers, "kernel register query"), ] { driver_status(Backend::Nvidia, function_attribute(output, kind, kernel), action)?; }
require(max_block > 0 && shared >= 0 && used_registers > 0, "Nvidia kernel resources are invalid")?; Ok(Resources { registers: used_registers as u32, shared: shared as u32, max_block: max_block as u32 }) }; let forward_resource = resource(forward)?; let epoch_resource = resource(epoch)?; let resources = Resources { registers: forward_resource.registers.max(epoch_resource.registers), shared: forward_resource.shared.max(epoch_resource.shared), max_block: forward_resource.max_block.min(epoch_resource.max_block), }; let register_wave = resources.registers.checked_mul(wave as u32) .ok_or_else(|| RecipeError::new("Nvidia wave register count overflows"))?; let observed = (registers as u32 / register_wave).min(threads as u32 / wave as u32); let geometry = nvidia( cus as u32, 			wave as u32, 			workgroup as u32, 			block_lds as u32, 			sm_lds as u32, observed, 			resources, 		)?; for (kernel, action) in [(forward, "forward occupancy"), (epoch, "epoch occupancy")] { let mut active = 0; driver_status(Backend::Nvidia, occupancy(&mut active, kernel, geometry.block as i32, 0), action)?;
require(active > 0, format!("Nvidia {action} has no resident workgroup"))?; } let cuda = Cuda { 			allocate: runtime.function(b"cuMemAlloc_v2\0")?, free: runtime.function(b"cuMemFree_v2\0")?, 			upload: runtime.function(b"cuMemcpyHtoD_v2\0")?, download: runtime.function(b"cuMemcpyDtoH_v2\0")?, 			synchronize: runtime.function(b"cuCtxSynchronize\0")?, launch: runtime.function(b"cuLaunchCooperativeKernel\0")?, 		}; eprintln!("Nvidia grid {} block {}", geometry.groups, geometry.block); 		Ok(Gpu { backend: Backend::Nvidia, 			driver: Driver::Cuda(cuda), 			geometry, forward: Kernel::cuda(forward, FORWARD_ARGS), 			epoch: Kernel::cuda(epoch, EPOCH_ARGS), dispatch: Mutex::new(()), 		}) 	} } #[cfg(any(feature = "amd", feature = "nvidia"))] #[link(name = "dl")] unsafe extern "C" { 	fn dlopen(name: *const std::ffi::c_char, flags: i32) -> Ptr; fn dlsym(handle: Ptr, name: *const std::ffi::c_char) -> Ptr; } }
use driver::{Buffer, Gpu, device_backend, gpu};
...
fn graph_inputs(graph: &Graph, samples: &[f64], targets: &[f64], rows: usize, backend: Backend) -> Result<Vec<f64>> {
	...
	let mut tape = DeviceTape::new(graph, samples, &targets[..rows], backend)?; tape.forward()?; tape.predictions()
}
fn fit_surrogate(input: Shape, samples: &[f64], targets: &[f64], hidden: usize, backend: Backend, config: Config) -> Result<Vec<f64>> {
	...
	let prepared = Prepared { samples: samples.to_vec(), targets: targets.to_vec(), rows: targets.len(), features: input.elements(), schema: String::new() };
	...
	for step in 1..=config.surrogate_epochs { tape.epoch(step, config.surrogate_rate, mse, 0.0, config, false)?; }
	tape.weights(false)
}
fn lower_estimator(
	graph: &mut Graph,
...
impl Train { 	pub const fn stop(mut self, value: f64) -> Self { 		self.stop = Some(value); 		self 	}
	...
	fn finish_dispatch<T>(
		...
		if let Some(best) = interrupted.then_some(self.stop.is_some()).or(save) && let Some(path) = &self.save {
