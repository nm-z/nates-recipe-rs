...
impl InterfaceClassification {
	...
	pub const fn is_prohibited(self) -> bool {
		matches!(self, Self::Prohibited(_))
	}
}
...
mod tests {
	...
	#[test]
	fn exact_policy_has_no_ordinary_word_false_positives() {
		for word in [
			"architecture",
			"relationship",
			"scholarship",
			...
			assert_eq!(
				classify_interface_symbol(word),
				InterfaceClassification::Unknown,
				"{word}"
			);
			assert_eq!(
				classify_library(word),
				InterfaceClassification::Unknown,
				"{word}"
			);
			assert_eq!(
				classify_dependency(word),
				InterfaceClassification::Unknown,
				"{word}"
			);
		}
	...
	#[test]
	fn allowed_interfaces_are_explicit() {
		assert_eq!(
			classify_interface_symbol("hsa_queue_create"),
			InterfaceClassification::Allowed(NativeInterface::RocrHsa)
		);
		assert_eq!(
			classify_interface_symbol("cuLaunchKernel"),
			InterfaceClassification::Allowed(NativeInterface::CudaDriver)
		);
		assert_eq!(
			classify_interface_symbol("cuDeviceGetPCIBusId"),
			InterfaceClassification::Allowed(NativeInterface::CudaDriver)
		);
		assert_eq!(
			classify_interface_symbol("cuUnreviewedThing"),
			InterfaceClassification::Prohibited(NativeInterface::CudaDriverOutsideAllowlist)
		);
		assert_eq!(
			classify_library("libcuda.so.1"),
			InterfaceClassification::Allowed(NativeInterface::CudaDriver)
		);
		assert_eq!(
			classify_library("libhsa-runtime64.so.1"),
			InterfaceClassification::Allowed(NativeInterface::RocrHsa)
		);
	}

	#[test]
	fn forbidden_interfaces_require_complete_tokens() {
		for symbol in [
			"hipMalloc",
			...
			assert!(
				classify_interface_symbol(symbol).is_prohibited(),
				"{symbol}"
			);
		}
		for library in [
			...
			"libMIOpen.so",
			"librccl.so",
			"/opt/lib/libcublas_static.a",
			"libcusolver.so",
			"libcufft.so",
			"libcudnn.so",
			"libnccl.so",
		] {
			assert!(classify_library(library).is_prohibited(), "{library}");
		}
		for dependency in [
			"hip-sys",
			"cuda-runtime-sys",
			...
			assert!(
				classify_dependency(dependency).is_prohibited(),
				"{dependency}"
			);
		}
	}
