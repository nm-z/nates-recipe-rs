...
fn allocate_from<'runtime>(
	...
	owner: HsaAgent,
	raw_pools: &[RawPool],
	descriptions: &[MemoryPoolDescription],
	pool_index: usize,
	size: usize,
...
fn matching_pool(
	descriptions: &[MemoryPoolDescription],
	kind: &'static str,
	predicate: impl Fn(MemoryPoolFlags) -> bool,
) -> Result<usize> {
	descriptions
		.iter()
		.position(|pool| {
			pool.segment == MemorySegment::Global
				...
				&& pool.global_flags.is_some_and(&predicate)
		})
...
fn executable_symbol_info<T: Copy>(
	api: &Api,
...
impl<'session, 'runtime> Pending<'session, 'runtime> {
	...
	pub fn wait(&mut self, timeout: Duration) -> Result<WaitOutcome> {
		...
		loop {
			...
			unsafe {
				(self.pool.inner.api.signal_wait_scacquire)(
					signal,
					SIGNAL_CONDITION_LT,
					1,
					ticks,
...
impl<'runtime> Session<'runtime> {
	...
	pub fn copy_async_prepared(
		...
		if !source.inner.is_accessible_by(destination.inner.owner)
			|| !destination.inner.is_accessible_by(source.inner.owner)
		{
			return Err(Error::InvalidDispatch {
...
fn kernel_packet_header(barrier: bool) -> u16 {
...
fn barrier_and_packet_header() -> u16 {
...
fn validate_geometry(description: &AgentDescription, geometry: DispatchGeometry) -> Result<()> {
	...
	for isa in &description.isas {
		if workgroup_total > isa.maximum_workgroup_size
			|| grid_total > isa.maximum_grid_size
			|| (0..3).any(|dimension| {
				geometry.workgroup[dimension] > isa.maximum_workgroup_dimensions[dimension]
					|| geometry.grid[dimension] > isa.maximum_grid_dimensions[dimension]
			}) {
			return Err(Error::InvalidDispatch {
	...
	pub fn dispatch_prepared(
		...
		let kernarg_pointer = match (kernel.metadata.kernarg_segment_size, kernarg) {
			...
			(_, Some(allocation)) => {
				...
				if !(allocation.as_ptr() as usize)
					.is_multiple_of(kernel.metadata.kernarg_segment_alignment as usize)
				{
					return Err(Error::InvalidDispatch {
