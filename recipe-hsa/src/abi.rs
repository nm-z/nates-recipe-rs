...
mod tests {
	...
	#[test]
	fn opaque_handles_match_the_rocr_abi() {
		assert_eq!(size_of::<HsaAgent>(), 8);
		assert_eq!(size_of::<HsaIsa>(), 8);
		assert_eq!(size_of::<HsaSignal>(), 8);
		assert_eq!(size_of::<HsaMemoryPool>(), 8);
		assert_eq!(size_of::<HsaCodeObjectReader>(), 8);
		assert_eq!(size_of::<HsaExecutable>(), 8);
		assert_eq!(size_of::<HsaExecutableSymbol>(), 8);
		assert_eq!(align_of::<HsaAgent>(), 8);
	}

	#[test]
	fn dim3_matches_the_rocr_abi() {
		assert_eq!(size_of::<HsaDim3>(), 12);
		assert_eq!(align_of::<HsaDim3>(), 4);
	}

	#[test]
	fn large_model_queue_matches_the_rocr_abi() {
		assert_eq!(size_of::<HsaQueue>(), 40);
		assert_eq!(align_of::<HsaQueue>(), 8);
		assert_eq!(offset_of!(HsaQueue, kind), 0);
		assert_eq!(offset_of!(HsaQueue, features), 4);
		assert_eq!(offset_of!(HsaQueue, base_address), 8);
		assert_eq!(offset_of!(HsaQueue, doorbell_signal), 16);
		assert_eq!(offset_of!(HsaQueue, size), 24);
		assert_eq!(offset_of!(HsaQueue, reserved1), 28);
		assert_eq!(offset_of!(HsaQueue, id), 32);
	}

	#[test]
	fn kernel_dispatch_packet_matches_the_rocr_abi() {
		assert_eq!(size_of::<HsaKernelDispatchPacket>(), 64);
		assert_eq!(align_of::<HsaKernelDispatchPacket>(), 8);
		assert_eq!(offset_of!(HsaKernelDispatchPacket, header), 0);
		assert_eq!(offset_of!(HsaKernelDispatchPacket, setup), 2);
		assert_eq!(offset_of!(HsaKernelDispatchPacket, grid_size_x), 12);
		assert_eq!(offset_of!(HsaKernelDispatchPacket, kernel_object), 32);
		assert_eq!(offset_of!(HsaKernelDispatchPacket, kernarg_address), 40);
		assert_eq!(offset_of!(HsaKernelDispatchPacket, completion_signal), 56);
	}

	#[test]
	fn barrier_and_packet_matches_the_rocr_abi() {
		assert_eq!(size_of::<HsaBarrierAndPacket>(), 64);
		assert_eq!(align_of::<HsaBarrierAndPacket>(), 8);
		assert_eq!(offset_of!(HsaBarrierAndPacket, header), 0);
		assert_eq!(offset_of!(HsaBarrierAndPacket, dep_signal), 8);
		assert_eq!(offset_of!(HsaBarrierAndPacket, reserved2), 48);
		assert_eq!(offset_of!(HsaBarrierAndPacket, completion_signal), 56);
	}
	...
	fn reviewed_constant_sentinels_match_headers() {
		...
		assert_eq!(AMD_MEMORY_POOL_INFO_LOCATION, 17);
	}
}
