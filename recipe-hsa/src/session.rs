...
impl Session<'_> {
	...
	pub fn create_queue(&self, config: QueueConfig) -> Result<Queue<'_, '_>> {
		...
		if config.size_packets < capabilities.minimum_packets
			|| config.size_packets > capabilities.maximum_packets
		...
		let invalid = base_is_null
			|| actual_size == 0
			|| !actual_size.is_power_of_two()
