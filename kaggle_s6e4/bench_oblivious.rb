...
MODE = ARGV.find { |a| a.start_with?("--mode=") }&.split("=", 2)&.last || "regression"
raise "Unknown mode: #{MODE}" unless %w[regression binary multiclass].include?(MODE)

# ── Load dataset ──────────────────────────────────────────────────────────────
if MODE == "multiclass"
      lines = File.readlines("/tmp/letter.data").map(&:chomp).reject(&:empty?)
      label_map = {}
      rows = lines.map do |l|
            f = l.split(",")
            label_map[f[0]] ||= label_map.size
            [label_map[f[0]].to_f] + f[1..].map(&:to_f)
      end
      n = rows.size; nf = rows[0].size - 1
      nc = label_map.size
      x = rows.map { |r| r[1..] }.flatten
      y = rows.map { |r| r[0] }
      $stderr.puts "Letters: n=#{n} features=#{nf} classes=#{nc}"
...
if MODE == "multiclass"
      srand(0)
      all_idx = (0...n).to_a.shuffle
      split = (n * 0.8).to_i
      tr_idx = all_idx[0...split]
      te_idx = all_idx[split..]
else
...
if MODE == "regression"
      ...
      $stderr.puts "Config: mode=regression depth=#{DEPTH} lr=#{LR} lambda=#{LAM} iters=#{ITERS}"
      ...
      $stderr.puts "\n# ── results ──────────────────────────────────────────────────────"
      $stderr.puts "  dataset:          #{dataset_name} (n=#{n}, features=#{nf})"
      $stderr.puts "  split:            sklearn train_test_split(test_size=0.2, random_state=0)"
      $stderr.puts "  mode:             regression"
...
elsif MODE == "multiclass"
      pred      = zeros_f32(n_tr, nc)
      te_pred   = zeros_f32(n_te, nc)
      grad_all  = zeros_f32(n_tr, nc)
      hess_all  = zeros_f32(n_tr, nc)
      node_a    = zeros_u8(n_tr); node_b = zeros_u8(n_tr)
      leaf_idx  = zeros_u8(n_tr); te_leaf = zeros_u8(n_te)
      grad_hist = zeros_f32(n_leaves * nf, NB)
      hess_hist = zeros_f32(n_leaves * nf, NB)
      ...
      $stderr.puts "Config: mode=multiclass depth=#{DEPTH} lr=#{LR} lambda=#{LAM} iters=#{ITERS} classes=#{nc}"
      $stderr.puts "Training..."
      ...
      for t in 0...ITERS
            ...
            if (t + 1) % 100 == 0
                  ...
                  $stderr.puts "  iter=%d  acc=%.4f  wall=%.2fms  kernel=%.2fms" % [t + 1, acc, dt_wall * 1000, dt_kern * 1000]
            end
      end
      ...
      $stderr.puts "\n# ── results ──────────────────────────────────────────────────────"
      $stderr.puts "  dataset:          #{dataset_name} (n=#{n}, features=#{nf}, classes=#{nc})"
      $stderr.puts "  mode:             multiclass"
      $stderr.puts "  depth=#{DEPTH}  lr=#{LR}  lambda=#{LAM}  iters=#{ITERS}"
      $stderr.puts "  Accuracy:         %.4f" % acc
      $stderr.puts "  total wall:       %.2fs" % total
      $stderr.puts "  median wall:      %.3fms" % (med_wall * 1000)
      $stderr.puts "  median kernel:    %.3fms  (gpu_sync bookended)" % (med_kern * 1000)
end
