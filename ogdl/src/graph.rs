...
impl Graph {
	...
	pub fn append_root(&mut self, text: impl Into<String>) -> Result<NodeId, GraphError> {
		let text = text.into();
		validate_text(&text)?;
...
fn validate_text(text: &str) -> Result<(), GraphError> {
	if text.is_empty() {
		return Err(GraphError::InvalidNodeText {
