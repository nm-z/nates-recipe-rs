...
struct Config {
	...
	triage_path: PathBuf,
	issue_reader: PathBuf,
	dsh_binary: PathBuf,
	dsh_profile: String,
	dsh_socket: PathBuf,
	dsh_start_seconds: u64,
	dsh_poll_milliseconds: u64,
	dsh_read_poll_milliseconds: u64,
	dsh_turn_seconds: u64,
	dsh_max_prompt_bytes: usize,
	dsh_max_response_bytes: usize,
	dsh_max_turns: usize,
	dsh_context_window: usize,
	dsh_max_tokens: usize,
	dsh_parallel_tool_calls: usize,
	dsh_node_memory_mib: usize,
	dsh_opencode_auth_path: PathBuf,
	dsh_opencode_credential: String,
	dsh_opencode_base_url: String,
	dsh_openrouter_base_url: String,
	dsh_openrouter_auth_provider: String,
	dsh_models: Vec<String>,
	...
	resolver_excluded_issues: BTreeSet<u64>,
...
fn value(text: &str, name: &str) -> String {
	text.lines().find_map(|line| line.split_once('=').filter(|(key, _)| key.trim() == name).map(|(_, value)| value.trim().trim_matches('"').to_owned())).unwrap_or_else(|| panic!("machine.toml has no {name}"))
}
...
fn values(text: &str, name: &str) -> Vec<String> {
	value(text, name).split(',').map(str::trim).filter(|value| !value.is_empty()).map(str::to_owned).collect()
}
...
fn discovered_devices(nvidia_topology_root: &Path, amd_topology_root: &Path) -> Vec<String> {
	...
	if amd_topology_root.is_dir() {
		let mut nodes = std::fs::read_dir(amd_topology_root).expect("cannot read AMD topology").map(|entry| entry.expect("cannot read AMD topology entry").path()).collect::<Vec<_>>();
		nodes.sort();
		let count = nodes.iter().filter(|node| std::fs::read_to_string(node.join("gpu_id")).ok().and_then(|value| value.trim().parse::<u64>().ok()).is_some_and(|gpu| gpu != 0)).count();
	...
	if nvidia_topology_root.is_dir() {
		let mut nvidia = std::fs::read_dir(nvidia_topology_root)
			...
			.map(|device| {
				...
				information.lines().find_map(|line| line.strip_prefix("Device Minor:").map(str::trim)).expect("NVIDIA device has no minor").parse::<usize>().expect("NVIDIA device minor is invalid")
			})
			.collect::<Vec<_>>();
...
fn config(path: &Path) -> Config {
	...
	let dsh_models = values(&text, "dsh_models");
	...
	let configuration = Config {
		...
		issue_reader: value(&text, "issue_reader").into(),
		dsh_binary: value(&text, "dsh_binary").into(),
		dsh_profile: value(&text, "dsh_profile"),
		dsh_socket: value(&text, "dsh_socket").into(),
		dsh_start_seconds: positive(&text, "dsh_start_seconds"),
		dsh_poll_milliseconds: positive(&text, "dsh_poll_milliseconds"),
		dsh_read_poll_milliseconds: positive(&text, "dsh_read_poll_milliseconds"),
		dsh_turn_seconds: positive(&text, "dsh_turn_seconds"),
		dsh_max_prompt_bytes: positive(&text, "dsh_max_prompt_bytes"),
		dsh_max_response_bytes: positive(&text, "dsh_max_response_bytes"),
		dsh_max_turns: positive(&text, "dsh_max_turns"),
		dsh_context_window: positive(&text, "dsh_context_window"),
		dsh_max_tokens: positive(&text, "dsh_max_tokens"),
		dsh_parallel_tool_calls: positive(&text, "dsh_parallel_tool_calls"),
		dsh_node_memory_mib: positive(&text, "dsh_node_memory_mib"),
		dsh_opencode_auth_path: value(&text, "dsh_opencode_auth_path").into(),
		dsh_opencode_credential: value(&text, "dsh_opencode_credential"),
		dsh_opencode_base_url: value(&text, "dsh_opencode_base_url"),
		dsh_openrouter_base_url: value(&text, "dsh_openrouter_base_url"),
		dsh_openrouter_auth_provider: value(&text, "dsh_openrouter_auth_provider"),
		dsh_models,
		...
		resolver_excluded_issues: values(&text, "resolver_excluded_issues").into_iter().map(|issue| issue.parse().unwrap_or_else(|_| panic!("machine.toml has invalid resolver_excluded_issues"))).collect(),
		...
		trial_limit: number(&text, "trial_limit"),
		device_worker_floor: number(&text, "device_worker_floor"),
		review_limit: number(&text, "review_limit"),
		review_worker_floor: number(&text, "review_worker_floor"),
...
// The handle a review worker publishes so its live DSH turn can be cancelled.
#[derive(Clone)]
struct Cancel {
	retire: Arc<AtomicBool>,
...
impl Cancel {
	fn new() -> Self {
		Self { retire: Arc::new(AtomicBool::new(false)), stream: Arc::new(Mutex::new(None)) }
	}
	...
	fn opened(&self, stream: &UnixStream) {
		*self.stream.lock().expect("review cancellation lock is poisoned") = Some(stream.try_clone().expect("cannot clone DSH socket"));
	}

	fn send(&self, bytes: &[u8]) -> std::io::Result<()> {
		let mut stream = self.stream.lock().expect("review cancellation lock is poisoned");
		let stream = stream.as_mut().ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotConnected, "DSH review has no live socket"))?;
	...
	fn closed(&self) {
		*self.stream.lock().expect("review cancellation lock is poisoned") = None;
	}
}
...
impl ReviewEndpoint {
	fn new(config: &Config) -> Self {
		Self { derating: Mutex::new(1.0), recovery: config.review_recovery, overload_divisor: config.review_overload_divisor as f64 }
	}
	...
	fn derating(&self) -> f64 {
		*self.derating.lock().expect("review endpoint lock is poisoned")
	}
	...
	fn claim(self: &Arc<Self>, config: &Config) -> Option<ModelLease> {
		let first = DSH_NEXT.fetch_add(1, Ordering::Relaxed);
		(0..config.dsh_models.len()).find_map(|offset| {
			let model = &config.dsh_models[(first + offset) % config.dsh_models.len()];
			available(model).then(|| ModelLease { endpoint: Arc::clone(self), model: model.clone() })
		})
	}

	fn reachable(&self, config: &Config) -> usize {
		config.dsh_models.iter().filter(|model| available(model)).count()
	}
...
impl Gains {
	fn ziegler_nichols(name: &str, ultimate_gain: f64, ultimate_period_seconds: f64) -> Self {
		...
		Self { proportional: ZN_PROPORTIONAL * ultimate_gain, integral: ZN_INTEGRAL * ultimate_gain / ultimate_period_seconds, derivative: ZN_DERIVATIVE * ultimate_gain * ultimate_period_seconds }
	}
}
...
impl Control {
	fn summary(&self) -> String {
		...
		format!("CONTROL {actuators} memory={:.3}/{:.3} vram={:.3}/{:.3} cpu={:.3}/{:.3} gpu={:.3}/{:.3} disk={:.3}<{:.3} ready={}/{} residual={}", self.memory, self.memory_setpoint, self.vram, self.vram_setpoint, self.cpu, self.cpu_setpoint, self.gpu, self.gpu_setpoint, self.disk, self.disk_maximum, self.queue, self.queue_setpoint, self.residual,)
	}
}
...
impl Regulator {
	...
	fn step(&mut self, config: &Config, plant: &Plant, occupancy: &Occupancy, seconds: f64) -> Control {
		...
		let reviews = queue_request.min((memory_budget - trial_budget).max(0.0)).max(review_floor.min(memory_budget));
		...
		let accelerator_total = gpu_request.min(vram_request).min(accelerator_ceiling).max(accelerator_floor.min(accelerator_ceiling));
		...
		for device in &config.discovery_devices {
			...
			if plant.memory >= config.memory_maximum {
				command = command.min(retire_below(live))
			}
			if device != "cpu" && plant.vram >= config.vram_maximum {
				command = command.min(retire_below(live))
			}
			if disk_blocked {
				command = command.min(live as f64)
			}
			if disk_exceeded {
				command = command.min(retire_below(live))
			}
		...
		if plant.memory >= config.memory_maximum {
			review_command = review_command.min(retire_below(occupancy.reviews))
		}
		...
		Control { memory: plant.memory, vram: plant.vram, cpu: plant.cpu, gpu: plant.gpu, disk: plant.disk, queue: plant.queue, memory_setpoint: config.memory_setpoint, vram_setpoint: config.vram_setpoint, cpu_setpoint: config.cpu_setpoint, gpu_setpoint: config.gpu_setpoint, disk_maximum: config.disk_maximum, queue_setpoint: config.queue_setpoint, actuators, residual }
	}
	...
	fn residual(&self, config: &Config, plant: &Plant, demand: &Demand) -> String {
		...
		record("memory", config.memory_setpoint - plant.memory, config.convergence_tolerance, (demand.trial_budget + demand.reviews < demand.memory_budget).then(|| if self.cpu.ceiling { "trial-limit" } else { "cpu-budget" }));
		record(
			"cpu",
			config.cpu_setpoint - plant.cpu,
			config.convergence_tolerance,
			if demand.trial_budget < demand.cpu_budget {
				Some("memory-budget")
			} else if self.cpu.ceiling {
				Some("trial-limit")
			} else {
				None
			},
		...
		record(
			"vram",
			config.vram_setpoint - plant.vram,
			config.convergence_tolerance,
			if demand.accelerator_total < demand.vram_request {
				Some("trial-budget")
			} else if self.vram.ceiling {
				Some("trial-limit")
			} else {
				None
			},
		...
		record(
			"queue",
			config.queue_setpoint as f64 - plant.queue as f64,
			config.queue_tolerance as f64,
			if demand.reviews < demand.queue_request {
				Some("memory-budget")
			} else if self.queue.ceiling {
				Some("review-limit")
			} else {
				None
			},
		);
		if demand.disk_blocked {
			residuals.push(format!("disk:{:+.3}<headroom", config.disk_maximum - plant.disk))
		}
		if residuals.is_empty() { "none".to_owned() } else { residuals.join(",") }
	}
}

fn retire_below(live: usize) -> f64 {
	(live as f64 - 1.0).max(0.0)
}

impl Admission {
	fn new(config: &Config) -> Self {
		Self { trials: config.discovery_devices.iter().cloned().map(|device| (device, AtomicI64::new(0))).collect(), reviews: AtomicI64::new(0), live: Mutex::new(LiveWorkers::default()), next_worker: AtomicUsize::new(0) }
	}
	...
	fn retire_trials(&self, device: &str, excess: usize) {
		...
		for trial in candidates.iter().take(excess) {
			trial.retire.store(true, Ordering::Relaxed)
		}
	}
	...
	fn audit_reviews(&self, config: &Config) -> usize {
		...
		if overrun != 0 {
			event(config, &format!("RETIRE review overrun={overrun} grace={}s", config.review_grace_seconds))
		}
		overrun
	}
}
...
impl Drop for TrialSlot {
	fn drop(&mut self) {
		self.admission.release_trial(self.worker)
	}
}
...
impl Drop for ReviewSlot {
	fn drop(&mut self) {
		self.admission.release_review(self.worker)
	}
}
...
fn control_lines(control: &Control, width: usize) -> Vec<String> {
	...
	vec![fit(&format!("├─ command   {commands}"), width), fit(&format!("├─ resource  memory {:.3}/{:.3}  vram {:.3}/{:.3}  cpu {:.3}/{:.3}  gpu {:.3}/{:.3}  disk {:.3}<{:.3}  ready {}/{}", control.memory, control.memory_setpoint, control.vram, control.vram_setpoint, control.cpu, control.cpu_setpoint, control.gpu, control.gpu_setpoint, control.disk, control.disk_maximum, control.queue, control.queue_setpoint,), width), fit(&format!("└─ residual  {}", control.residual), width)]
}
...
impl CursorLease {
	...
	fn stop(&mut self) {
		...
		if let Some(heartbeat) = self.heartbeat.take() {
			heartbeat.join().expect("cursor heartbeat failed");
		}
	}
...
impl Drop for TrialDirectory {
	fn drop(&mut self) {
		if self.0.exists() {
			std::fs::remove_dir_all(&self.0).expect("cannot remove trial directory")
		}
	}
}
...
static DSH_NEXT: AtomicUsize = AtomicUsize::new(0);
...
fn identity(provider: &str, model: &str) -> String {
	format!("{provider}/{model}")
}
fn epoch() -> u64 {
	SystemTime::now().duration_since(UNIX_EPOCH).expect("system time precedes Unix epoch").as_secs()
}

fn memory_used() -> f64 {
	let text = std::fs::read_to_string("/proc/meminfo").expect("cannot read /proc/meminfo");
	let kib = |name: &str| text.lines().find_map(|line| line.strip_prefix(name)).and_then(|value| value.split_whitespace().next()).and_then(|value| value.parse::<f64>().ok()).unwrap_or_else(|| panic!("/proc/meminfo has no {name}"));
...
fn accelerator_used(config: &Config) -> (f64, f64) {
	...
	for device in config.discovery_devices.iter().filter_map(|device| device.strip_prefix("nv")) {
		let result = output(Command::new(&config.timeout_binary).arg(config.hardware_query_seconds.to_string()).arg(&config.nvidia_smi_binary).args(["-i", device, "--query-gpu=utilization.gpu,memory.used,memory.total", "--format=csv,noheader,nounits"]), None);
	...
	if config.discovery_devices.iter().any(|device| device.starts_with("amd")) {
		...
		for entry in std::fs::read_dir(&config.amd_drm_root).expect("cannot read AMD DRM resources") {
			...
			if !name.strip_prefix("card").is_some_and(|index| !index.is_empty() && index.bytes().all(|byte| byte.is_ascii_digit())) {
				continue;
			}
			...
			if !(busy.is_file() && used.is_file() && total.is_file()) {
				continue;
			}
...
fn regulate(config: Arc<Config>, work: Arc<Mutex<Work>>, admission: Arc<Admission>) {
	std::thread::spawn(move || {
		...
		loop {
			...
			let plant = Plant { memory: memory_used(), vram, cpu: 1.0 - idle_ticks as f64 / period_ticks as f64, gpu, disk: disk_used(&config), queue: ready_packets(&work.lock().expect("failure queue lock is poisoned")) };
...
fn reset_after(error: &str) -> Option<Duration> {
	...
	for token in tail.split_whitespace() {
		...
		seconds = seconds.saturating_add(value.saturating_mul(match unit {
			"d" => 86_400,
			"h" => 3_600,
			"m" => 60,
			"s" => 1,
...
fn usage_exhausted(error: &str) -> bool {
	...
	(error.contains(r#""type":"rate_limit_event""#) && error.contains(r#""status":"rejected""#)) || ["usage limit", "session limit", "quota", "out of credits", "credit balance"].iter().any(|message| error.contains(message))
}
...
fn available(model: &str) -> bool {
	...
	match cooldowns.get(model).copied() {
		Some(reset) if Instant::now() >= reset => {
...
fn dsh_models(config: &Config, provider: &str) -> String {
	let prefix = format!("{provider}/");
	config.dsh_models.iter().filter_map(|model| model.strip_prefix(&prefix)).collect::<Vec<_>>().join(",")
}

fn dsh_server(config: &Config) -> DshServer {
	let mut child = Command::new(&config.dsh_binary)
		.args(["--profile", &config.dsh_profile])
		.current_dir(&config.repository)
		.env("NODE_OPTIONS", format!("--max-old-space-size={}", config.dsh_node_memory_mib))
		.env("DSH_TELEMETRY_DISABLED", "1")
		.env("RECIPE_MACHINE_REPOSITORY", &config.repository)
		.env("RECIPE_MACHINE_DSH_SOCKET", &config.dsh_socket)
		.env("RECIPE_MACHINE_OPENCODE_AUTH_PATH", &config.dsh_opencode_auth_path)
		.env("RECIPE_MACHINE_ISSUE_READER", &config.issue_reader)
		.env("RECIPE_MACHINE_DSH_OPENCODE_CREDENTIAL", &config.dsh_opencode_credential)
		.env("RECIPE_MACHINE_DSH_OPENCODE_BASE_URL", &config.dsh_opencode_base_url)
		.env("RECIPE_MACHINE_DSH_OPENROUTER_BASE_URL", &config.dsh_openrouter_base_url)
		.env("RECIPE_MACHINE_DSH_OPENROUTER_AUTH_PROVIDER", &config.dsh_openrouter_auth_provider)
		.env("RECIPE_MACHINE_DSH_CONTEXT_WINDOW", config.dsh_context_window.to_string())
		.env("RECIPE_MACHINE_DSH_MAX_TOKENS", config.dsh_max_tokens.to_string())
		.env("RECIPE_MACHINE_DSH_MAX_PROMPT_BYTES", config.dsh_max_prompt_bytes.to_string())
		.env("RECIPE_MACHINE_DSH_MAX_TURNS", config.dsh_max_turns.to_string())
		.env("RECIPE_MACHINE_DSH_PARALLEL_TOOL_CALLS", config.dsh_parallel_tool_calls.to_string())
		.env("RECIPE_MACHINE_DSH_OPENCODE_MODELS", dsh_models(config, "opencode"))
		.env("RECIPE_MACHINE_DSH_OPENROUTER_MODELS", dsh_models(config, "openrouter"))
		.stdout(Stdio::piped())
		.stderr(Stdio::piped())
		.spawn()
		.expect("cannot start DSH server");
	let stdout = child.stdout.take().expect("DSH stdout is absent");
	let stdout_config = config.clone();
	std::thread::spawn(move || {
		for line in BufReader::new(stdout).lines().map_while(Result::ok) {
			log(&stdout_config, &format!("DSH {line}"));
		}
	});
	let stderr = child.stderr.take().expect("DSH stderr is absent");
	let stderr_config = config.clone();
	std::thread::spawn(move || {
		for line in BufReader::new(stderr).lines().map_while(Result::ok) {
			log(&stderr_config, &format!("DSH {line}"));
		}
	});
	let deadline = Instant::now() + Duration::from_secs(config.dsh_start_seconds);
	loop {
		if let Ok(stream) = UnixStream::connect(&config.dsh_socket) {
			drop(stream);
			break;
		}
		assert!(child.try_wait().expect("cannot inspect DSH server").is_none(), "DSH server exited during startup");
		assert!(Instant::now() < deadline, "DSH server did not create {}", config.dsh_socket.display());
		std::thread::sleep(Duration::from_millis(config.dsh_poll_milliseconds));
	}
	DshServer(child)
}
...
impl<'a> DshSession<'a> {
	fn connect(config: &Config, cancel: &'a Cancel) -> std::result::Result<Self, String> {
		let stream = UnixStream::connect(&config.dsh_socket).map_err(|error| format!("cannot connect to DSH server: {error}"))?;
		stream.set_read_timeout(Some(Duration::from_millis(config.dsh_read_poll_milliseconds))).map_err(|error| format!("cannot configure DSH socket: {error}"))?;
		cancel.opened(&stream);
		Ok(Self { reader: BufReader::new(stream), cancel, turn_timeout: Duration::from_secs(config.dsh_turn_seconds), max_prompt_bytes: config.dsh_max_prompt_bytes, max_response_bytes: config.dsh_max_response_bytes })
	}

	fn wait(&self, deadline: Instant, cancel_sent: &mut bool) -> std::result::Result<(), String> {
		if self.cancel.retiring() && !*cancel_sent {
			self.cancel.cancel();
			*cancel_sent = true;
		}
		if Instant::now() >= deadline {
			self.cancel.cancel();
			return Err(format!("DSH turn exceeded {} seconds", self.turn_timeout.as_secs()));
		}
		Ok(())
	}

	fn prompt(&mut self, provider: &str, model: &str, prompt: &str) -> std::result::Result<Option<String>, String> {
		assert!(!provider.contains(char::is_whitespace) && !model.contains(char::is_whitespace), "DSH route contains whitespace");
		let prompt = prompt.as_bytes();
		if prompt.len() > self.max_prompt_bytes {
			return Err(format!("DSH prompt is {} bytes, exceeding {}", prompt.len(), self.max_prompt_bytes));
		}
		let mut request = format!("PROMPT {provider} {model} {}\n", prompt.len()).into_bytes();
		request.extend_from_slice(prompt);
		self.cancel.send(&request).map_err(|error| format!("cannot send DSH prompt: {error}"))?;
		let deadline = Instant::now() + self.turn_timeout;
		let mut cancel_sent = false;
		let mut header = String::new();
		loop {
			match self.reader.read_line(&mut header) {
				Ok(0) => return Err("DSH server closed the review socket".to_owned()),
				Ok(_) => break,
				Err(error) if matches!(error.kind(), std::io::ErrorKind::WouldBlock | std::io::ErrorKind::TimedOut) => self.wait(deadline, &mut cancel_sent)?,
				Err(error) => return Err(format!("cannot read DSH response header: {error}")),
			}
		}
		let fields = header.trim_end().split_whitespace().collect::<Vec<_>>();
		if fields.len() != 3 || fields[0] != "RESULT" {
			return Err(format!("DSH returned an invalid response header: {}", header.trim_end()));
		}
		let length = fields[2].parse::<usize>().map_err(|_| format!("DSH returned an invalid response length: {}", fields[2]))?;
		if length > self.max_response_bytes {
			return Err(format!("DSH response is {length} bytes, exceeding {}", self.max_response_bytes));
		}
		let mut body = vec![0; length];
		let mut read = 0;
		while read < length {
			match self.reader.read(&mut body[read..]) {
				Ok(0) => return Err("DSH server closed an incomplete response".to_owned()),
				Ok(bytes) => read += bytes,
				Err(error) if matches!(error.kind(), std::io::ErrorKind::WouldBlock | std::io::ErrorKind::TimedOut) => self.wait(deadline, &mut cancel_sent)?,
				Err(error) => return Err(format!("cannot read DSH response: {error}")),
			}
		}
		let body = String::from_utf8(body).map_err(|error| format!("DSH response is not UTF-8: {error}"))?;
		match fields[1] {
			"COMPLETED" => Ok(Some(body)),
			"ABORTED" => Ok(None),
			"ERROR" => Err(body),
			status => Err(format!("DSH returned unknown status {status}")),
		}
	}
}

impl Drop for DshSession<'_> {
	fn drop(&mut self) {
		self.cancel.closed();
	}
}
...
fn duration(elapsed: Duration) -> String {
	...
	if seconds < 1.0 { format!("{:>7.3} ms", seconds * 1000.0) } else { format!("{seconds:>8.4} s") }
}
...
fn pane_size() -> (usize, usize) {
	...
	(values.next().expect("tmux pane width is absent").saturating_sub(1), values.next().expect("tmux pane height is absent"))
}
...
fn display_render(display: &mut Display) {
	...
	for active in display.active.values() {
		*trials.entry(active.device.clone()).or_default() += 1
	}
	...
	let resolving = display.resolvers.values().map(|resolver| format!("{:<28}  issue #{:<5}  time {}", resolver.model, resolver.issue, elapsed(resolver.started))).collect::<Vec<_>>();
	if let Some(control) = display.control.clone() {
		display_line(&mut frame, display, &format!("{BLUE}control{RESET}"));
		for line in control_lines(&control, width) {
			display_line(&mut frame, display, &line)
		}
	}
	if queued != 0 {
	...
	for line in &display.history[history_start..] {
...
fn display_queue(packet: &str) {
	let key = packet_key(packet);
	let mut display = display().lock().expect("display lock is poisoned");
	display.reviews.entry(key).or_insert(ReviewNode { model: None, started: None });
}
...
fn display_clock(model: String) {
	...
	std::thread::spawn(move || {
		loop {
			...
			for (issue, started) in running {
				display.resolvers.entry(issue).or_insert_with(|| ResolverNode { issue, model: model.clone(), started });
			}
...
fn queued(text: &str) -> VecDeque<String> {
	let mut packets = text.split("RECIPE QUEUED FAILURE BEGIN\n").skip(1).filter_map(|tail| tail.split_once("\nRECIPE QUEUED FAILURE END").map(|(packet, _)| packet.to_owned())).collect::<VecDeque<_>>();
...
fn persist_queue(path: &Path, packets: &VecDeque<String>) {
	let text = packets.iter().map(|packet| format!("RECIPE QUEUED FAILURE BEGIN\n{packet}\nRECIPE QUEUED FAILURE END\n")).collect::<String>();
...
fn setting(path: &Path, key: &str, value: impl std::fmt::Display) {
	...
	let text = std::fs::read_to_string(path).expect("cannot read machine.toml");
	let updated = text.lines().map(|line| if line.split_once('=').is_some_and(|(candidate, _)| candidate.trim() == key) { format!("{key} = {value}") } else { line.to_owned() }).collect::<Vec<_>>().join("\n") + "\n";
	let temporary = path.with_extension("next");
...
fn output(command: &mut Command, input: Option<&str>) -> std::process::Output {
	...
	let mut child = command.stdout(Stdio::piped()).stderr(Stdio::piped()).spawn().expect("cannot start command");
	if let Some(input) = input {
		child.stdin.take().expect("command has no input").write_all(input.as_bytes()).expect("cannot write command input");
	}
	child.wait_with_output().expect("cannot read command output")
}
...
fn event(config: &Config, message: &str) {
	log(config, message)
}
...
fn failure(result: &std::process::Output) -> String {
	...
	String::from_utf8_lossy(&result.stdout).lines().rev().find(|line| !line.trim().is_empty()).unwrap_or("no diagnostic").trim().to_owned()
}

fn packets(text: &str) -> Vec<&str> {
	text.split("RECIPE FAILURE BEGIN\n").skip(1).filter_map(|tail| tail.split_once("RECIPE FAILURE END").map(|(packet, _)| packet.trim())).collect()
}

fn field<'a>(packet: &'a str, name: &str) -> &'a str {
	packet.lines().find_map(|line| line.strip_prefix(name)).unwrap_or_else(|| panic!("failure packet has no {name}"))
}

fn packet_cursor(packet: &str) -> u64 {
	field(packet, "cursor=").split_whitespace().find_map(|value| value.strip_prefix("cursor:")).expect("failure packet has no cursor").parse().expect("failure packet cursor is invalid")
}
...
fn packet_composition(packet: &str) -> &str {
	field(packet, "cursor=").split_whitespace().find_map(|value| value.strip_prefix("composition:")).expect("failure cursor has no composition")
}
...
fn packet_for_device(packet: &str, device: &str) -> String {
	let selector = if device == "cpu" { "RECIPE_FORCE_CPU=1".to_owned() } else { format!("RECIPE_DEVICE={device}") };
	packet.lines().map(|line| if let Some(command) = line.strip_prefix("command=") { format!("command={selector} {command}") } else { line.to_owned() }).chain(std::iter::once(format!("backend={device}"))).collect::<Vec<_>>().join("\n")
}

fn same_failure(left: &str, right: &str) -> bool {
	field(left, "id=") == field(right, "id=") && packet_backend(left) == packet_backend(right)
}
...
fn prepare_storage(config: &Config) {
	...
	for entry in std::fs::read_dir(&config.trial_directory).expect("cannot read trial root") {
		...
		let valid = entry.file_type().is_ok_and(|kind| kind.is_dir()) && entry.file_name().to_str().and_then(|name| name.strip_prefix("recipe-trial-")).and_then(|name| name.split_once('-')).is_some_and(|(worker, cursor)| worker.parse::<usize>().is_ok() && cursor.parse::<u64>().is_ok());
		if valid {
			std::fs::remove_dir_all(entry.path()).expect("cannot remove interrupted trial")
		}
	}
}

fn trial(config: &Config, device: &str, cursor: u64, directory: &Path, reproduction: &Path, retire: &AtomicBool) -> (std::process::Output, Termination, Duration) {
	let started = Instant::now();
	let mut command = Command::new("cargo");
	command.args(["run", "--bin", "recipe", "--", "test.rs"]).env("RECIPE_COMPOSITION_SEED", config.seed.to_string()).env("RECIPE_COMPOSITION_CURSOR", cursor.to_string()).env("RECIPE_COMPOSITION_COUNT", config.compositions_per_batch.to_string()).env("RECIPE_COMPOSITION_REPRO", reproduction).env("RECIPE_TRIAL_DIRECTORY", directory).env("RECIPE_NATIVE_CACHE_DIRECTORY", &config.native_cache_directory).env("TMPDIR", directory).env_remove("RECIPE_DEVICE").env_remove("RECIPE_FORCE_CPU").current_dir(&config.repository);
	if device == "cpu" {
	...
	let output = std::process::Output { status, stdout: stdout.join().expect("Recipe traversal stdout reader failed"), stderr: stderr.join().expect("Recipe traversal stderr reader failed") };
...
fn signal_process_group(group: u32, signal: &str) -> bool {
	Command::new("kill").args([signal, &format!("-{group}")]).stdout(Stdio::null()).stderr(Stdio::null()).status().expect("cannot inspect Recipe traversal process group").success()
}
...
fn crash_packet(trial: &Trial, text: &str, observed: i32, replayed: i32) -> String {
	...
	format!(
		...
		repository_base(&trial.config.repository),
		trial.config.seed,
		trial.cursor,
		trial.cursor + 1,
		configuration.trim(),
		trial.reproduction.display()
	)
}
...
fn slow_packet(trial: &Trial) -> String {
	...
	let composition = source.lines().find_map(|line| line.trim().strip_prefix(".seed(")?.strip_suffix(')')).expect("slow traversal reproduction has no composition seed");
	let mut fingerprint = 1_469_598_103_934_665_603_u64;
...
fn jq(json: &str, filter: &str) -> std::result::Result<String, String> {
	...
	String::from_utf8(result.stdout).map(|value| value.trim().to_owned()).map_err(|error| error.to_string())
}
...
fn object(text: String) -> std::result::Result<String, String> {
	let start = text.find('{').ok_or_else(|| "classifier returned no JSON object".to_owned())?;
	let end = text.rfind('}').ok_or_else(|| "classifier returned an incomplete JSON object".to_owned())?;
...
fn dsh(config: &Config, route: &str, prompt: &str, cancel: &Cancel) -> std::result::Result<Option<Decision>, String> {
	let (provider, model) = route.split_once('/').expect("DSH model has no provider");
	let mut session = DshSession::connect(config, cancel)?;
	let response = match session.prompt(provider, model, prompt) {
		Ok(Some(response)) => response,
		Ok(None) => return Ok(None),
		Err(_) if cancel.retiring() => return Ok(None),
		Err(error) => return Err(error),
	};
	let mut json = object(response);
	if json.is_err() {
		trace(config, &format!("model={route} repairing structured output in the same DSH session"));
		let repaired = match session.prompt(provider, model, "Return only one corrected JSON object matching the required schema. Do not repeat the investigation.") {
	...
	let provider = match provider {
		...
		_ => unreachable!("DSH profile admitted an unsupported provider"),
	};
	Ok(Some(Decision { provider, model: model.to_owned(), effort: "default".to_owned(), json }))
}
...
fn classify(config: &Config, prompt: &str, packet: &str, model: &str, cancel: &Cancel) -> std::result::Result<Decision, Review> {
	...
	match dsh(config, model, prompt, cancel) {
		Ok(None) => {
...
fn triage(config: &Config, instructions: &str, packet: &str, model: &str, cancel: &Cancel) -> Review {
	...
	let schema = std::fs::read_to_string(&config.decision_schema).expect("cannot read decision schema");
	...
	event(config, &format!("CLASSIFY model={classifier} composition={composition}"));
	...
	if !config.publish {
		event(config, &format!("DECISION model={} verdict={verdict} issue={issue} title={title}", classifier));
		display_reviewed(packet, &classifier, &verdict, None);
		return Review::Stop;
	}
	assert!(!title.is_empty() && !body.is_empty(), "classifier returned empty issue text");
...
```"#,
	...
	if verdict == "new" {
		...
		let existing = output(Command::new("gh").args(["issue", "list", "--state", "all", "--limit", &limit, "--json", "number,title,body"]).current_dir(&config.repository), None);
		...
		let duplicate = output(Command::new("jq").args(["-er", "--arg", "title", &title, "--arg", "cause", cause, r#"[.[] | select((.title | ascii_downcase) == ($title | ascii_downcase) or ($cause != "" and ((.body // "") | contains($cause))))] | sort_by(.number) | .[0].number // empty"#]), Some(&existing));
	...
	let published = match verdict.as_str() {
		"new" => output(Command::new("gh").args(["issue", "create", "--label", "bug", "--title", &title, "--body", &body]).current_dir(&config.repository), None),
		"comment" => output(Command::new("gh").args(["issue", "comment", &issue, "--body", &body]).current_dir(&config.repository), None),
		_ => unreachable!(),
	};
	assert!(published.status.success(), "GitHub publication failed: {}", failure(&published));
	let url = String::from_utf8_lossy(&published.stdout).trim().to_owned();
	let published_issue = if verdict == "new" { url.rsplit('/').next().unwrap_or("unknown") } else { &issue };
	let action = if verdict == "new" { "created" } else { "commented on" };
	event(config, &format!("ISSUE model={} {action} issue=#{published_issue} url={url}", classifier));
	display_reviewed(packet, &classifier, if verdict == "new" { "issue" } else { "comment" }, Some(&url));
	Review::Done
}

fn review_loop(work: Arc<Mutex<Work>>, config: Arc<Config>, instructions: Arc<String>, packet: String, lease: ModelLease, cancel: Cancel) {
	...
	match result {
		...
		Review::Unavailable | Review::Transport => {
			state.ready.remove(&key);
		}
	}
...
fn enqueue_review(work: &Arc<Mutex<Work>>, config: &Config, packet: &str) -> Option<usize> {
	let depth = {
		let mut state = work.lock().expect("failure queue lock is poisoned");
...
fn resolver_units() -> std::result::Result<BTreeMap<u64, Instant>, String> {
	...
	if !resolvers.status.success() {
		return Err(failure(&resolvers));
	}
	...
	let now = Instant::now();
	Ok(String::from_utf8_lossy(&resolvers.stdout)
		.split("\n\n")
		.filter_map(|unit| {
...
fn resolver_issue(config: &Config, active: &BTreeSet<u64>, running: &BTreeMap<u64, Instant>) -> std::result::Result<Option<(u64, String)>, String> {
	let issues = output(Command::new("gh").args(["issue", "list", "--state", "open", "--search", "sort:created-asc", "--limit", "100", "--json", "number,url", "--jq", ".[] | [.number, .url] | @tsv"]).current_dir(&config.repository), None);
	if !issues.status.success() {
		return Err(failure(&issues));
	}
	let pull_requests = output(Command::new("gh").args(["pr", "list", "--state", "open", "--limit", "100", "--json", "closingIssuesReferences", "--jq", ".[].closingIssuesReferences[].number"]).current_dir(&config.repository), None);
	if !pull_requests.status.success() {
		return Err(failure(&pull_requests));
	}
	let mut claimed = String::from_utf8_lossy(&pull_requests.stdout).lines().filter_map(|number| number.parse::<u64>().ok()).collect::<BTreeSet<_>>();
	claimed.extend(config.resolver_excluded_issues.iter().copied());
...
fn resolver_claim(config: &Config, active: &Arc<Mutex<BTreeSet<u64>>>) -> std::result::Result<Option<ResolverClaim>, String> {
	...
	if issues.len() + running.keys().filter(|issue| !issues.contains(issue)).count() >= limit as usize {
		return Ok(None);
	}
...
fn resolver_pr(config: &Config, issue: u64) -> std::result::Result<Option<String>, String> {
	let pull_requests = output(Command::new("gh").args(["pr", "list", "--state", "open", "--limit", "100", "--json", "closingIssuesReferences,url", "--jq", ".[] | [.url, ([.closingIssuesReferences[].number] | map(tostring) | join(\",\"))] | @tsv"]).current_dir(&config.repository), None);
	if !pull_requests.status.success() {
		return Err(failure(&pull_requests));
	}
...
fn resolver_loop(config: Arc<Config>, path: PathBuf, active: Arc<Mutex<BTreeSet<u64>>>) {
	loop {
		...
		let goal = format!(
			r#"/goal Read GitHub issue #{number} and every comment in full, then resolve that one issue completely and create one pull request that closes it.
...
Commit and push the coherent fix, then create one pull request targeting {base}. Keep the pull request body concise and include only `Fixes #{number}`, the exact reproduction, the root cause, the change, and measured end-to-end evidence. Omit issue history, implementation tours, alternatives, tables, and exhaustive edge-case narratives. Do not stop until the pull request exists and its URL is visible. Target issue: {url}"#,
		...
		let result = output(
			Command::new("systemd-run")
				.args(["--user", "--wait", "--pipe", "--collect", "--quiet", "--unit", &unit, "--working-directory", config.repository.to_str().expect("repository path is not UTF-8"), "--property", &memory, "--property", &environment, "--", config.claude_binary.to_str().expect("Claude binary path is not UTF-8"), "-p", &goal, "--model", &config.resolver_model, "--effort", &config.resolver_effort, "--dangerously-skip-permissions", "--strict-mcp-config", "--output-format", "json", "--name", &format!("Resolve Recipe issue #{}", issue.number)])
				.current_dir(&config.repository),
			None,
		);
		...
		if usage_exhausted(&transcript) {
			if let Some(reset) = cooldown(&config, &model, &transcript) {
				setting(&path, "resolver_reset_unix", reset)
			}
...
fn discovery_loop(worker: usize, device: String, config: Arc<Config>, work: Arc<Mutex<Work>>, cluster: Arc<cluster::Client>, send: mpsc::Sender<Discovery>, retire: Arc<AtomicBool>) {
	if work.lock().expect("failure queue lock is poisoned").halted {
		return;
	}
	let replay = {
		let mut state = work.lock().expect("failure queue lock is poisoned");
		...
		if let Some(packet) = &packet {
			state.active.insert(packet_key(packet));
		}
		packet
	};
	...
	if send.send(Discovery::Start { worker, device: device.clone(), cursor: start }).is_err() {
		return;
	}
	...
	std::thread::spawn(move || {
		loop {
			...
			for cancel in admission.retire_reviews(live.saturating_sub(target)) {
				// Cancel the live DSH turn immediately so the endpoint stops working on a
			...
			// rather than retried.
			if admitted < target {
				event(&config, &format!("ENDPOINT leases={admitted}/{target} derating={:.3} reachable={}/{}", endpoint.derating(), endpoint.reachable(&config), config.dsh_models.len()));
			}
...
fn manage_trials(config: Arc<Config>, work: Arc<Mutex<Work>>, cluster: Arc<cluster::Client>, admission: Arc<Admission>, send: mpsc::Sender<Discovery>) {
	std::thread::spawn(move || {
		loop {
			let occupancy = admission.occupancy();
...
fn main() {
	...
	assert!(!initial.dsh_models.is_empty(), "dsh_models must contain at least one model");
	let digest = format!("{}{}", cluster::binary_digest(&executable, &initial.sha256_binary), cluster::repository_digest(&initial.repository, &initial.cluster_workload_paths, &initial.git_binary, &initial.sha256_binary),);
	let (cluster_send, cluster_receive) = mpsc::channel();
	if initial.cluster_coordinator {
		cluster::start_server(cluster::ServerConfig { listen: initial.cluster_listen, state_path: initial.cluster_state_path.clone(), secret: initial.cluster_secret.clone(), digest: digest.clone(), initial_cursor: initial.cursor, cursor_step: initial.compositions_per_batch, batches: initial.batches, node_limit: initial.cluster_node_limit, node_timeout: Duration::from_secs(initial.cluster_node_timeout_seconds), cursor_timeout: Duration::from_secs(initial.cursor_lease_seconds) }, cluster_send);
	}
	let cluster = Arc::new(cluster::Client::new(initial.cluster_address, initial.cluster_secret.clone(), initial.cluster_node_id.clone(), digest.clone(), &initial.discovery_devices, Duration::from_secs(initial.cluster_request_seconds)));
	cluster.register();
	...
	let _dsh_server = initial.cluster_coordinator.then(|| dsh_server(&initial));
	...
	let pending = if initial.cluster_coordinator { std::fs::read_to_string(&initial.queue_path).map(|text| queued(&text)).unwrap_or_default() } else { VecDeque::new() };
	...
	for discovery in receive {
		let mut trial = match discovery {
			...
			Discovery::Retired { worker, device, cursor, replay, lease } => {
				match replay {
					Some(packet) => {
						work.lock().expect("failure queue lock is poisoned").active.remove(&packet_key(&packet));
					}
		...
		let text = format!("{}{}", String::from_utf8_lossy(&trial.output.stderr), String::from_utf8_lossy(&trial.output.stdout));
		let signal = (!trial.timed_out).then(|| termination_signal(trial.output.status)).flatten();
		assert!(trial.timed_out || trial.output.status.success() || signal.is_some(), "Recipe traversal failed outside a failure packet on {} with status {:?}: {}", trial.device, trial.output.status, failure(&trial.output));
		let mut failures = if trial.timed_out { vec![slow_packet(&trial)] } else { packets(&text).into_iter().map(|packet| packet_for_device(packet, &trial.device)).collect::<Vec<_>>() };
			...
			.enumerate()
		{
			...
			let failed = failures.iter().any(|packet| packet_cursor(packet) == analyzed);
			let status = if failed { "FAIL" } else { "PASS" };
			event(&trial.config, &format!("{status} device={} cursor={analyzed} composition={composition}", trial.device));
		}
		...
		if let Some(replayed) = &trial.replay {
			...
			if removed {
				display_drop(replayed)
			}
			continue;
		}
		let next = if trial.timed_out || signal.is_some() { trial.cursor + trial.config.compositions_per_batch } else { text.lines().find_map(|line| line.strip_prefix("composition cursor=")).expect("Recipe traversal emitted no next cursor").parse().expect("Recipe next cursor is invalid") };
