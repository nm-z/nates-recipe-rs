...
fn epoch() -> u64 {
	SystemTime::now().duration_since(UNIX_EPOCH).expect("system time precedes the Unix epoch").as_secs()
}

fn token(value: &str, name: &str) {
	assert!(!value.is_empty() && value.bytes().all(|byte| byte.is_ascii_alphanumeric() || b"._-".contains(&byte)), "cluster {name} contains an unsupported character");
}

pub fn binary_digest(binary: &Path, sha256_binary: &Path) -> String {
	let output = Command::new(sha256_binary).arg(binary).output().expect("cannot start the configured SHA-256 command");
	assert!(output.status.success(), "cannot hash the machine binary");
	let digest = String::from_utf8(output.stdout).expect("SHA-256 output is not UTF-8").split_whitespace().next().expect("SHA-256 output is empty").to_owned();
...
fn content_digest(content: &[u8], sha256_binary: &Path) -> String {
	let mut child = Command::new(sha256_binary).stdin(std::process::Stdio::piped()).stdout(std::process::Stdio::piped()).spawn().expect("cannot start the configured SHA-256 command");
...
pub fn repository_digest(repository: &Path, paths: &[String], git_binary: &Path, sha256_binary: &Path) -> String {
	...
	let listed = Command::new(git_binary).args(["ls-files", "-z", "--"]).args(paths).current_dir(repository).output().expect("cannot start workload file discovery");
	...
	for path in files {
		...
		let hashed = Command::new(git_binary).args(["hash-object", "--", path]).current_dir(repository).output().expect("cannot start workload hashing");
...
fn load_map(value: &str) -> BTreeMap<u64, u64> {
	value.split(',')
		...
		.map(|entry| {
			...
			(key.parse().expect("cluster state map key is invalid"), value.parse().expect("cluster state map value is invalid"))
		})
		.collect()
}

fn load_set(value: &str) -> BTreeSet<u64> {
	value.split(',').filter(|entry| !entry.is_empty()).map(|entry| entry.parse().expect("cluster state set entry is invalid")).collect()
}

impl State {
	fn new(cursor: u64) -> Self {
		Self { frontier: cursor, next: cursor, next_lease: 1, returned: BTreeSet::new(), completed: BTreeMap::new(), leases: BTreeMap::new(), nodes: BTreeMap::new() }
	}
	...
	fn load(path: &Path, cursor: u64) -> Self {
		...
		let value = |name: &str| text.lines().find_map(|line| line.split_once('=').filter(|(key, _)| *key == name).map(|(_, value)| value)).unwrap_or_else(|| panic!("cluster state has no {name}"));
		let mut state = Self { frontier: value("frontier").parse().expect("cluster frontier is invalid"), next: value("next").parse().expect("cluster next cursor is invalid"), next_lease: value("next_lease").parse().expect("cluster next lease is invalid"), returned: load_set(value("returned")), completed: load_map(value("completed")), leases: BTreeMap::new(), nodes: BTreeMap::new() };
		for line in text.lines() {
			if let Some(value) = line.strip_prefix("lease=") {
				let fields = value.split('|').collect::<Vec<_>>();
				assert_eq!(fields.len(), 5, "cluster lease state has the wrong width");
				state.leases.insert(fields[0].parse().expect("cluster lease id is invalid"), Held { cursor: fields[1].parse().expect("cluster lease cursor is invalid"), node: fields[2].to_owned(), device: fields[3].to_owned(), deadline: fields[4].parse().expect("cluster lease deadline is invalid") });
			}
			if let Some(value) = line.strip_prefix("node=") {
				let fields = value.split('|').collect::<Vec<_>>();
				assert_eq!(fields.len(), 4, "cluster node state has the wrong width");
				state.nodes.insert(fields[0].to_owned(), Node { digest: fields[1].to_owned(), devices: fields[2].to_owned(), deadline: fields[3].parse().expect("cluster node deadline is invalid") });
			}
		}
		state
	}
	...
	fn text(&self) -> String {
		let mut text = format!("frontier={}\nnext={}\nnext_lease={}\nreturned={}\ncompleted={}\n", self.frontier, self.next, self.next_lease, set(&self.returned), map(&self.completed),);
		for (id, lease) in &self.leases {
			text.push_str(&format!("lease={id}|{}|{}|{}|{}\n", lease.cursor, lease.node, lease.device, lease.deadline));
		}
...
impl Coordinator {
	...
	fn expire(&mut self, now: u64) {
		...
		let expired = self.state.leases.iter().filter_map(|(id, lease)| (lease.deadline <= now).then_some((*id, lease.cursor))).collect::<Vec<_>>();
	...
	fn register(&mut self, node: &str, digest: &str, devices: &str, now: u64) -> Result<(), String> {
		...
		self.state.nodes.insert(node.to_owned(), Node { digest: digest.to_owned(), devices: devices.to_owned(), deadline: now + self.config.node_timeout.as_secs() });
		Ok(())
	}
	...
	fn claim(&mut self, node: &str, digest: &str, devices: &str, device: &str) -> Result<Option<Lease>, String> {
		...
		let cursor = if let Some(cursor) = self.state.returned.pop_first() {
			...
			let end = self.config.batches.checked_mul(self.config.cursor_step).and_then(|span| self.config.initial_cursor.checked_add(span)).expect("cluster cursor range overflows");
			if self.config.batches != 0 && self.state.next >= end {
				self.persist();
				return Ok(None);
			}
			let cursor = self.state.next;
			self.state.next = self.state.next.checked_add(self.config.cursor_step).expect("cluster cursor overflows");
			cursor
		};
		let id = self.state.next_lease;
		self.state.next_lease = self.state.next_lease.checked_add(1).expect("cluster lease id overflows");
		self.state.leases.insert(id, Held { cursor, node: node.to_owned(), device: device.to_owned(), deadline: now + self.config.cursor_timeout.as_secs() });
		self.persist();
	...
	fn complete(&mut self, node: &str, id: u64, cursor: u64, next: u64, device: &str, packets: Vec<String>) -> Result<u64, String> {
		...
		self.persist();
		self.completions.send(Completion { node: node.to_owned(), device: device.to_owned(), cursor, packets }).map_err(|_| "cluster completion receiver is absent".to_owned())?;
	...
	fn handle(&mut self, request: &str) -> Result<String, String> {
		...
		match command {
			...
			"CLAIM" if fields.len() == 6 => self.claim(fields[2], fields[3], fields[4], fields[5]).map(|lease| lease.map_or_else(|| "EMPTY".to_owned(), |lease| format!("LEASE\t{}\t{}", lease.id, lease.cursor))),
			"RENEW" if fields.len() == 5 => {
				self.renew(fields[2], fields[3].parse().map_err(|_| "lease id is invalid")?, fields[4].parse().map_err(|_| "lease cursor is invalid")?)?;
				Ok("OK".to_owned())
			}
			"COMPLETE" if fields.len() == 7 || fields.len() == 8 => self.complete(fields[2], fields[3].parse().map_err(|_| "lease id is invalid")?, fields[4].parse().map_err(|_| "lease cursor is invalid")?, fields[5].parse().map_err(|_| "next cursor is invalid")?, fields[6], unpack_bundle(fields.get(7).copied().unwrap_or_default())?).map(|frontier| format!("FRONTIER\t{frontier}")),
			"RETURN" if fields.len() == 5 => {
				self.release(fields[2], fields[3].parse().map_err(|_| "lease id is invalid")?, fields[4].parse().map_err(|_| "lease cursor is invalid")?)?;
				Ok("OK".to_owned())
			}
...
fn serve(stream: &mut TcpStream, coordinator: &Arc<Mutex<Coordinator>>) {
	...
	let response = coordinator.lock().expect("cluster coordinator lock is poisoned").handle(&request).unwrap_or_else(|error| format!("ERROR\t{error}"));
...
impl Client {
	pub fn new(address: SocketAddr, secret: String, node: String, digest: String, devices: &[String], timeout: Duration) -> Self {
		token(&node, "node id");
		token(&digest, "binary digest");
		...
		Self { address, secret, node, digest, devices: devices.join(","), timeout, exhausted: Arc::new(AtomicBool::new(false)) }
	}

	fn request(&self, command: &str) -> String {
		let mut stream = TcpStream::connect_timeout(&self.address, self.timeout).expect("cannot connect to the cluster coordinator");
	...
	pub fn register(&self) {
		let response = self.request(&format!("REGISTER\t{}\t{}\t{}\t{}", self.secret, self.node, self.digest, self.devices));
		assert_eq!(response, "OK", "cluster registration response is invalid");
	}

	pub fn claim(&self, device: &str) -> Option<Lease> {
		let response = self.request(&format!("CLAIM\t{}\t{}\t{}\t{}\t{device}", self.secret, self.node, self.digest, self.devices));
		if response == "EMPTY" {
		...
		Some(Lease { id: fields[1].parse().expect("cluster lease id is invalid"), cursor: fields[2].parse().expect("cluster lease cursor is invalid") })
	}
	...
	pub fn renew(&self, lease: Lease) {
		let response = self.request(&format!("RENEW\t{}\t{}\t{}\t{}", self.secret, self.node, lease.id, lease.cursor));
	...
	pub fn complete(&self, lease: Lease, next: u64, device: &str, packets: &[String]) -> u64 {
		let response = self.request(&format!("COMPLETE\t{}\t{}\t{}\t{}\t{next}\t{device}\t{}", self.secret, self.node, lease.id, lease.cursor, packet_bundle(packets)));
		let fields = response.split('\t').collect::<Vec<_>>();
	...
	pub fn release(&self, lease: Lease) {
		let response = self.request(&format!("RETURN\t{}\t{}\t{}\t{}", self.secret, self.node, lease.id, lease.cursor));
