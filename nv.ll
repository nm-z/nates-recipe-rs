...
sum.step:
  ...
  %exponential = call double @__nv_exp(double %centered)
...
block.gru:
  ...
  %gru.exp = call double @__nv_exp(double %gru.negative)
  %gru.denominator = fadd double 1.0, %gru.exp
  %gru.gate = fdiv double 1.0, %gru.denominator
  %gru.state = call double @__nv_tanh(double %value)
...
block.lstm:
  ...
  %lstm.exp = call double @__nv_exp(double %lstm.negative)
  %lstm.denominator = fadd double 1.0, %lstm.exp
  %lstm.gate = fdiv double 1.0, %lstm.denominator
  %lstm.cell = call double @__nv_tanh(double %value)
  %lstm.state = call double @__nv_tanh(double %lstm.cell)
