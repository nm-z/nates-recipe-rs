	...
	fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			Self::InvalidPhaseTask {
				...
				task,
				detail,
			} => write!(formatter, "task {task} is invalid in {phase:?}: {detail}"),
			...
			Self::SchedulerStalled { phase } => {
				write!(
					formatter,
					"{phase:?} scheduler has no runnable or pending task"
				)
			}
			...
			Self::JournalCapacityExceeded { stream, capacity } => {
				write!(
					formatter,
					"{stream:?} journal exhausted its declared capacity of {capacity}"
				)
			}
