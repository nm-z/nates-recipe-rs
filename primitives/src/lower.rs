...
use recipe_core::{AliasRule, ByteCount, DType, ElementCount, FlopCount, IndexSpace, KernelInput, KernelInputId, KernelOutput, KernelOutputId, KernelTemplate, ScalarLiteral, StaticBufferAccess, ValueId};
use recipe_language::{Elementwise, Gather, Histogram, IndexBounds, IndexMap, PrimitiveKernel, PrimitiveKind, RandomMap, Reduce, ReduceOperator, ReduceResult, Scan, ScanMode, Scatter, ScatterConflict, Sort, Tensor};
...
macro_rules! push_stage {
	( $builder:expr, $geometry:expr, $bindings:expr, $synchronization:expr, $atomics:expr,
		$fault:expr, $resources:expr, $kind:expr $(,)? ) => {
...
pub fn lower(kernel: &PrimitiveKernel, tensors: &BTreeMap<ValueId, &Tensor>, hardware: LoweringHardware) -> LoweringResult<LoweredProgram> {
...
fn validate_hardware(kernel: &PrimitiveKernel, hardware: LoweringHardware) -> LoweringResult<()> {
	let valid = hardware.subgroup_lanes != 0 && hardware.subgroup_lanes.is_power_of_two() && hardware.maximum_workgroup_lanes >= hardware.subgroup_lanes && hardware.maximum_shared_memory_per_workgroup.get() != 0;
	if valid {
		Ok(())
	} else {
...
fn tensor<'a>(tensors: &'a BTreeMap<ValueId, &Tensor>, value: ValueId, kernel: recipe_core::KernelTemplateId) -> LoweringResult<&'a Tensor> {
...
impl ProgramBuilder {
	...
	fn checked_fault(&mut self, reason: FaultReason, code: i32) -> LoweringResult<(FaultContract, BufferBinding, AtomicContract)> {
		let flag = self.fault()?;
	...
	fn finish(self, source_aliases: Vec<SourceAliasContract>, source_input_count: usize, source_output_count: usize) -> LoweringResult<LoweredProgram> {
...
fn aggregate_resources(buffers: &[ProgramBuffer], stages: &[ProgramStage], kernel: recipe_core::KernelTemplateId) -> LoweringResult<ProgramResourceBounds> {
...
fn basic_resources(flops: u64, integer_operations: u64, atomic_operations: u64, shared_bytes: u64, private_bytes: u64, workgroup_lanes: u32) -> StageResourceBounds {
	StageResourceBounds {
...
fn lower_elementwise(builder: &mut ProgramBuilder, kernel: &PrimitiveKernel, spec: &Elementwise, tensors: &BTreeMap<ValueId, &Tensor>) -> LoweringResult<()> {
...
fn lower_reduce(builder: &mut ProgramBuilder, kernel: &PrimitiveKernel, spec: &Reduce, tensors: &BTreeMap<ValueId, &Tensor>) -> LoweringResult<()> {
...
fn lower_empty_reduction(builder: &mut ProgramBuilder, kernel: &PrimitiveKernel, spec: &Reduce, tensors: &BTreeMap<ValueId, &Tensor>) -> LoweringResult<()> {
...
fn lower_scan(builder: &mut ProgramBuilder, kernel: &PrimitiveKernel, spec: &Scan, tensors: &BTreeMap<ValueId, &Tensor>) -> LoweringResult<()> {
	let input = tensor(tensors, kernel.inputs[0], kernel.id)?;
...
fn lower_contraction(builder: &mut ProgramBuilder, kernel: &PrimitiveKernel, spec: &recipe_language::Contraction, tensors: &BTreeMap<ValueId, &Tensor>) -> LoweringResult<()> {
...
fn contraction_physical_plan(builder: &ProgramBuilder, left: &Tensor, spec: &recipe_language::Contraction, output_elements: u64, contracted_elements: u64) -> LoweringResult<ContractionPhysicalPlan> {
	let free_extent = left
		...
		.copied()
		.enumerate()
		.filter(|(axis, _)| !spec.batch_axes.iter().any(|(left, _)| left == axis) && !spec.contract_axes.iter().any(|(left, _)| left == axis))
...
fn lower_gather(builder: &mut ProgramBuilder, kernel: &PrimitiveKernel, spec: Gather, tensors: &BTreeMap<ValueId, &Tensor>) -> LoweringResult<()> {
	let output = tensor(tensors, kernel.outputs[0], kernel.id)?;
	let false = output.shape.is_empty() else {
		return Ok(());
	};
...
fn lower_scatter(builder: &mut ProgramBuilder, kernel: &PrimitiveKernel, spec: Scatter, tensors: &BTreeMap<ValueId, &Tensor>) -> LoweringResult<()> {
	...
	let flops_per_atomic = match spec.conflict {
		ScatterConflict::Atomic {
			operation: recipe_language::AtomicOperation::Add | recipe_language::AtomicOperation::Minimum | recipe_language::AtomicOperation::Maximum,
			..
		} => 1,
		ScatterConflict::Atomic {
...
fn lower_histogram(builder: &mut ProgramBuilder, kernel: &PrimitiveKernel, spec: Histogram, tensors: &BTreeMap<ValueId, &Tensor>) -> LoweringResult<()> {
	let input = tensor(tensors, kernel.inputs[0], kernel.id)?;
	let output = tensor(tensors, kernel.outputs[0], kernel.id)?;
	let output_buffer = builder.tensor_buffer(kernel.outputs[0])?;
...
fn lower_sort(builder: &mut ProgramBuilder, kernel: &PrimitiveKernel, spec: Sort, tensors: &BTreeMap<ValueId, &Tensor>) -> LoweringResult<()> {
	let input = tensor(tensors, kernel.inputs[0], kernel.id)?;
	let false = input.shape.is_empty() else {
		return Ok(());
	};
...
fn lower_random(builder: &mut ProgramBuilder, kernel: &PrimitiveKernel, spec: RandomMap, tensors: &BTreeMap<ValueId, &Tensor>) -> LoweringResult<()> {
	let output = tensor(tensors, kernel.outputs[0], kernel.id)?;
	let false = output.shape.is_empty() else {
		return Ok(());
	};
...
fn lower_index_map(builder: &mut ProgramBuilder, kernel: &PrimitiveKernel, spec: IndexMap, tensors: &BTreeMap<ValueId, &Tensor>) -> LoweringResult<()> {
	let output = tensor(tensors, kernel.outputs[0], kernel.id)?;
	let false = output.shape.is_empty() else {
		return Ok(());
	};
