...
const USAGE: &str = "usage: recipe [--device <name>[,<name>]] <source.rs> [export] | recipe --worker <address> | recipe --scheduler <address>";
...
fn main() {
	...
	while let Some(argument) = arguments.next() {
		if argument == "--scheduler" {
			recipe::scheduler(&arguments.next().unwrap_or_else(|| invalid(USAGE)))
		}
		if argument == "--worker" {
