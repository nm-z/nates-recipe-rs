...
fn main() {
	...
	let selection = match arguments.next().as_deref() {
		None => ArtifactSet::Auto,
		Some("--amd") => ArtifactSet::Amd,
		Some("--nvidia") => ArtifactSet::Nvidia,
		_ => panic!("usage: recipe export <source.rs> [--amd|--nvidia]"),
	};
