...
fn run(source: &Path, device: Option<&str>) {
	...
	command.env("RECIPE_BINARY", directory.join("recipe"));
...
fn main() {
	...
	while let Some(argument) = arguments.next() {
		if argument == "--worker" {
			let device_name = arguments.next().unwrap_or_else(|| invalid("--worker requires a device name"));
			recipe::worker_serve(&device_name).unwrap_or_else(|error| { eprintln!("{error}"); std::process::exit(1) });
			return;
		}
		if argument == "--device" {
