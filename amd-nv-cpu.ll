...
define internal void @contraction_accumulate(ptr addrspace(5) %sums, ptr addrspace(5) %biases, i1 %bias.enable, i1 %lane.active, i32 %output.m.base, i32 %output.n.base, i32 %m.count, i32 %n.count, i32 %m.tile, i32 %n.tile, i32 %k.tile, i32 %k.count) #1 { entry: br label %k.loop k.loop:
...
%a.row = mul i32 %k, %m.tile %a.index = add i32 %a.row, %output.m %b.base = mul i32 %m.tile, %k.tile %b.row = mul i32 %k, %n.tile %b.local = add i32 %b.row, %output.n %b.index = add i32 %b.base, %b.local
...
%a.value = call double @contraction_input( ptr addrspace(1) %input, i32 %a.row.base, i32 %a.position, i32 %a.term, i32 %span, i32 %in.length, i1 %is.conv ) %a.tile.row = mul i32 %a.k, %m.tile %a.tile.index = add i32 %a.tile.row, %a.m br label %load.store
...
%b.index = add i32 %b.channel.base, %b.term %b.ptr = getelementptr inbounds double, ptr addrspace(1) %weights, i32 %b.index %b.value = load double, ptr addrspace(1) %b.ptr, align 8 %b.tile.base = mul i32 %m.tile, %k.tile %b.tile.row = mul i32 %b.k, %n.tile %b.tile.local = add i32 %b.tile.row, %b.n %b.tile.index = add i32 %b.tile.base, %b.tile.local br label %load.store
...
call void @recipe.local.barrier() call void @contraction_accumulate(ptr addrspace(5) %sums, ptr addrspace(5) null, i1 false, i1 %lane.active, i32 %output.m.base, i32 %output.n.base, i32 %m.count, i32 %n.count, i32 %m.tile, i32 %n.tile, i32 %k.tile, i32 %k.count) call void @recipe.local.barrier()
