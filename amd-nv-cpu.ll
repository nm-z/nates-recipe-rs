...
; The model compiler maps IQ1_S and IQ2_XXS nodes to their block-native dot
; products. Zero keeps every other packed format on the scalar decoder.
define internal i32 @recipe.model.iq.kind(i32 %node) #1 { entry: unreachable }
; One weight of a node-relative span: a dense node loads it, a packed node decodes it.
define internal double @recipe.model.weight(ptr addrspace(1) %weights, i32 %index, i32 %decode) #1 { entry:
%packed = icmp ne i32 %decode, 0 br i1 %packed, label %decoded, label %direct
direct: %ptr = getelementptr inbounds double, ptr addrspace(1) %weights, i32 %index %loaded = load double, ptr addrspace(1) %ptr, align 8 ret double %loaded
decoded: %value = call double @recipe.model.decode(ptr addrspace(1) %weights, i32 %index, i32 %decode) ret double %value }
; One expert is implicit. A routed mixture reads the compact selected ID that
; TopK stored once for this row, position, and slot.
define internal i32 @expert_selected(ptr addrspace(1) %context, i32 %pair, i32 %slot, i32 %experts, i32 %top) #1 { entry:
%single = icmp eq i32 %experts, 1 br i1 %single, label %one, label %many
one: ret i32 0
many: %base = mul i32 %pair, %top %index = add i32 %base, %slot %pointer = getelementptr inbounds i32, ptr addrspace(1) %context, i32 %index %expert = load i32, ptr addrspace(1) %pointer, align 4 ret i32 %expert }
; Dot one contiguous, 32-aligned IQ1_S row span without re-decoding block
; metadata for every scalar. The input remains in its declared model precision,
; and the sum remains in the arithmetic precision.
...
group.step:
%weight.absolute = add i32 %weight.begin, %k
%block.index = udiv i32 %weight.absolute, RECIPE_IQ1S_BLOCK
%block.local = urem i32 %weight.absolute, RECIPE_IQ1S_BLOCK
...
%factor.odd = or i16 %factor.twice, 1
%factor.i32 = zext i16 %factor.odd to i32
%factor = call double @recipe.from.u32(i32 %factor.i32)
%scale = call double @recipe.mul(double %scale.base, double %factor)
%delta.bit = and i16 %qh, -32768
%delta.negative = icmp ne i16 %delta.bit, 0
%delta = select i1 %delta.negative, double -1.125, double -0.875
br label %grid.loop
grid.loop:
%grid = phi i32 [ 0, %group.step ], [ %grid.next, %lane.done ]
%grid.sum = phi RECIPE_STATE [ %sum, %group.step ], [ %lane.sum, %lane.done ]
%grid.more = icmp ult i32 %grid, 4
br i1 %grid.more, label %grid.step, label %grid.done
grid.step:
%qs.group = mul i32 %block.group, 4
%qs.offset.0 = add i32 %block.byte, 2
...
%qh.high = and i16 %qh.shifted, 7
%qh.high.shifted = shl i16 %qh.high, 8
%qs.low.wide = zext i8 %qs.low to i16
...
%lane.sum = phi RECIPE_STATE [ %grid.sum, %grid.step ], [ %lane.sum.next, %lane.step ]
%lane.more = icmp ult i32 %lane, 8
br i1 %lane.more, label %lane.step, label %lane.done
lane.step:
%code.shift.i32 = mul i32 %lane, 2
%code.shift = trunc i32 %code.shift.i32 to i16
%code.shifted = lshr i16 %table, %code.shift
%code.i16 = and i16 %code.shifted, 3
%code.i32 = zext i16 %code.i16 to i32
%code = call double @recipe.from.u32(i32 %code.i32)
%level = call double @recipe.add(double %code, double %delta)
%weight = call double @recipe.mul(double %scale, double %level)
%grid.channel = mul i32 %grid, 8
%channel.0 = add i32 %input.channel.begin, %k
%channel.1 = add i32 %channel.0, %grid.channel
%channel = add i32 %channel.1, %lane
%channel.local = mul i32 %channel, %input.length
%input.local = add i32 %channel.local, %input.position
%input.index = add i32 %input.row, %input.local
...
%input.wide = call RECIPE_STATE @recipe.state.from.model(double %input.value)
%weight.wide = call RECIPE_STATE @recipe.state.from.model(double %weight)
%product = call RECIPE_STATE @recipe.state.mul(RECIPE_STATE %input.wide, RECIPE_STATE %weight.wide)
%lane.sum.next = call RECIPE_STATE @recipe.state.add(RECIPE_STATE %lane.sum, RECIPE_STATE %product)
...
; IQ2_XXS uses four codebook grids and one shared sign-and-scale word for each
; 32-value group. Decode that group once, then accumulate its 32 products.
...
group.step:
%weight.absolute = add i32 %weight.begin, %k
%block.index = udiv i32 %weight.absolute, RECIPE_IQ2XXS_BLOCK
%block.local = urem i32 %weight.absolute, RECIPE_IQ2XXS_BLOCK
%block.group = udiv i32 %block.local, 32
...
%factor = call double @recipe.from.u32(i32 %factor.odd)
%scale.unadjusted = call double @recipe.mul(double %scale.base, double %factor)
%scale = call double @recipe.mul(double %scale.unadjusted, double 0.125)
br label %grid.loop
grid.loop:
%grid = phi i32 [ 0, %group.step ], [ %grid.next, %lane.done ]
%grid.sum = phi RECIPE_STATE [ %sum, %group.step ], [ %lane.sum, %lane.done ]
...
%parity.4.shift = lshr i32 %signs, 4
%parity.4 = xor i32 %signs, %parity.4.shift
%parity.2.shift = lshr i32 %parity.4, 2
%parity.2 = xor i32 %parity.4, %parity.2.shift
%parity.1.shift = lshr i32 %parity.2, 1
%parity.1 = xor i32 %parity.2, %parity.1.shift
...
%code.i16 = and i16 %code.shifted, 3
%code.i32 = zext i16 %code.i16 to i32
%code.twice = shl i32 %code.i32, 1
%magnitude.i32 = or i32 %code.twice, 1
%magnitude = call double @recipe.from.u32(i32 %magnitude.i32)
%direct.shifted = lshr i32 %signs, %lane
%direct = and i32 %direct.shifted, 1
%last = icmp eq i32 %lane, 7
%sign = select i1 %last, i32 %parity, i32 %direct
%negative = icmp ne i32 %sign, 0
%negative.magnitude = call double @recipe.neg(double %magnitude)
%signed = select i1 %negative, double %negative.magnitude, double %magnitude
%weight = call double @recipe.mul(double %scale, double %signed)
%grid.channel = mul i32 %grid, 8
%channel.0 = add i32 %input.channel.begin, %k
%channel.1 = add i32 %channel.0, %grid.channel
%channel = add i32 %channel.1, %lane
...
; A packed Q8_0 contraction quantizes each 32-value input block exactly once per
; output accumulator, multiplies the signed integer codes, and applies the two
; stored half scales once to the block sum. The surrounding contraction owns the
; output schedule and epilogue, so this changes only the representation-aware K
; reduction.
; IQ4_NL shares one half scale across 32 weights and packs two 4-bit codes
; into each payload byte.
...
br label %lane.loop
lane.loop:
%lane = phi i32 [ 0, %block.step ], [ %lane.next, %lane.step ]
%lane.sum = phi RECIPE_STATE [ %sum, %block.step ], [ %lane.sum.next, %lane.step ]
%lane.more = icmp ult i32 %lane, 32
br i1 %lane.more, label %lane.step, label %lane.done
lane.step:
%payload.lane = urem i32 %lane, 16
...
%weight = call double @recipe.mul(double %scale, double %level)
%channel.0 = add i32 %input.channel.begin, %k
%channel = add i32 %channel.0, %lane
%channel.local = mul i32 %channel, %input.length
%input.local = add i32 %channel.local, %input.position
%input.index = add i32 %input.row, %input.local
%input.ptr = getelementptr inbounds double, ptr addrspace(1) %input, i32 %input.index
...
; Reduce one 32-thread output group through workgroup memory. Every thread enters
; both barriers, including a group outside the short final output round.
define internal void @expert_iq_wave_store(
ptr addrspace(1) %output, i32 %p, i1 %active, i32 %lane,
RECIPE_STATE %partial ) #1 {
entry:
%lid = call i32 @recipe.local.id.x()
%partial.ptr = getelementptr RECIPE_STATE, ptr addrspace(3) @contraction_tile, i32 %lid
store RECIPE_STATE %partial, ptr addrspace(3) %partial.ptr, align RECIPE_STATE_ALIGN
call void @recipe.local.barrier()
%first = icmp eq i32 %lane, 0
%store = and i1 %active, %first
br i1 %store, label %reduce.loop, label %finish
reduce.loop:
%part = phi i32 [ 0, %entry ], [ %part.next, %reduce.step ]
%sum = phi RECIPE_STATE [ zeroinitializer, %entry ], [ %sum.next, %reduce.step ]
%more = icmp ult i32 %part, 32
br i1 %more, label %reduce.step, label %reduce.done
reduce.step:
%group.base = sub i32 %lid, %lane
%source.index = add i32 %group.base, %part
...
define internal void @expert_iq_in_wave_forward_body(
...
i1 %active, i32 %lane, i32 %p, i32 %in.channels, i32 %out.channels, i32 %length,
i32 %experts, i32 %top, i32 %hidden, i32 %kind, i32 %decode ) #1 {
entry:
%zero = call RECIPE_STATE @recipe.state.from.u1(i1 false)
...
br label %term.loop
term.loop:
%term = phi i32 [ %lane, %prepare ], [ %term.next, %term.step ]
%sum = phi RECIPE_STATE [ %zero, %prepare ], [ %sum.next, %term.step ]
%more = icmp ult i32 %term, %in.channels
br i1 %more, label %term.step, label %finish
term.step:
%weight.index = add i32 %weight.begin, %term
%weight.value = call double @recipe.model.decode(ptr addrspace(1) %weights, i32 %weight.index, i32 %decode)
%input.channel.base = mul i32 %term, %length
%input.local = add i32 %input.channel.base, %position
%input.index = add i32 %input.row, %input.local
%input.ptr = getelementptr inbounds double, ptr addrspace(1) %input, i32 %input.index
%input.value = load double, ptr addrspace(1) %input.ptr, align 8
%weight.wide = call RECIPE_STATE @recipe.state.from.model(double %weight.value)
%input.wide = call RECIPE_STATE @recipe.state.from.model(double %input.value)
...
%term.next = add i32 %term, 32
br label %term.loop
finish:
%partial = phi RECIPE_STATE [ %zero, %entry ], [ %sum, %term.loop ]
call void @expert_iq_wave_store(ptr addrspace(1) %output, i32 %p, i1 %active, i32 %lane, RECIPE_STATE %partial)
ret void
}
define internal void @expert_iq_out_wave_forward_body(
ptr addrspace(1) %input, ptr addrspace(1) %weights, ptr addrspace(1) %output,
ptr addrspace(1) %routing, ptr addrspace(1) %routing.context,
i1 %active, i32 %lane, i32 %p, i32 %in.channels, i32 %out.channels, i32 %length,
i32 %experts, i32 %top, i32 %hidden, i32 %kind, i32 %decode ) #1 {
...
br label %slot.loop
slot.loop:
%slot = phi i32 [ 0, %prepare ], [ %slot.next, %slot.done ]
%total = phi RECIPE_STATE [ %zero, %prepare ], [ %total.next, %slot.done ]
%slot.more = icmp ult i32 %slot, %top
br i1 %slot.more, label %slot.step, label %finish
slot.step:
%expert = call i32 @expert_selected(ptr addrspace(1) %routing.context, i32 %routing.pair, i32 %slot, i32 %experts, i32 %top)
...
br label %term.loop
term.loop:
%term = phi i32 [ %lane, %slot.step ], [ %term.next, %term.step ]
%dot = phi RECIPE_STATE [ %zero, %slot.step ], [ %dot.next, %term.step ]
%term.more = icmp ult i32 %term, %hidden
br i1 %term.more, label %term.step, label %slot.done
term.step:
%weight.index = add i32 %weight.begin, %term
%weight.value = call double @recipe.model.decode(ptr addrspace(1) %weights, i32 %weight.index, i32 %decode)
%input.channel.local = add i32 %input.channel, %term
%input.channel.base = mul i32 %input.channel.local, %length
%input.local = add i32 %input.channel.base, %position
%input.index = add i32 %input.row, %input.local
%input.ptr = getelementptr inbounds double, ptr addrspace(1) %input, i32 %input.index
%input.value = load double, ptr addrspace(1) %input.ptr, align 8
...
%input.wide = call RECIPE_STATE @recipe.state.from.model(double %input.value)
%product = call RECIPE_STATE @recipe.state.mul(RECIPE_STATE %weight.wide, RECIPE_STATE %input.wide)
%dot.next = call RECIPE_STATE @recipe.state.add(RECIPE_STATE %dot, RECIPE_STATE %product)
%term.next = add i32 %term, 32
br label %term.loop
slot.done:
...
%total.next = call RECIPE_STATE @recipe.state.add(RECIPE_STATE %total, RECIPE_STATE %scaled)
%slot.next = add i32 %slot, 1
br label %slot.loop
finish:
%partial = phi RECIPE_STATE [ %zero, %entry ], [ %total, %slot.loop ]
call void @expert_iq_wave_store(ptr addrspace(1) %output, i32 %p, i1 %active, i32 %lane, RECIPE_STATE %partial)
ret void
}
...
register.loop:
%register = phi i32 [ 0, %entry ], [ %register.next, %register.done ]
%register.more = icmp ult i32 %register, RECIPE_REGISTER_COUNT
br i1 %register.more, label %register.test, label %exit
register.test:
%local.m = call i32 @contraction_output_m(i32 %lid, i32 %register, i32 %m.lanes)
%local.n = call i32 @contraction_output_n(i32 %lid, i32 %register, i32 %m.lanes)
%m.valid = icmp ult i32 %local.m, %m.count
%n.valid = icmp ult i32 %local.n, %n.count
%valid = and i1 %m.valid, %n.valid
br i1 %valid, label %register.begin, label %register.done
register.begin:
...
register.done:
%register.next = add i32 %register, 1
br label %register.loop
exit:
ret void
}
define internal void @contraction_q8_0_accumulate(
ptr addrspace(5) %sums, ptr addrspace(1) %input, ptr addrspace(1) %weights,
i1 %lane.active, i32 %lid, i32 %m.lanes, i32 %m.base, i32 %n.base,
i32 %m.count, i32 %n.count, i32 %out.begin, i32 %out.span,
i32 %in.channels, i32 %in.length, i32 %weight.base ) #1 {
entry:
br i1 %lane.active, label %register.loop, label %exit
register.loop:
%register = phi i32 [ 0, %entry ], [ %register.next, %register.done ]
...
%row.elements = mul i32 %in.channels, %in.length
%input.row = mul i32 %row, %row.elements
%weight.row = mul i32 %global.n, %in.channels
%sum.zero = call RECIPE_STATE @recipe.state.from.u1(i1 false)
br label %block.loop
block.loop:
%block.begin = phi i32 [ 0, %register.begin ], [ %block.next, %block.done ]
%sum = phi RECIPE_STATE [ %sum.zero, %register.begin ], [ %sum.next, %block.done ]
%block.more = icmp ult i32 %block.begin, %in.channels
br i1 %block.more, label %maximum.loop, label %register.store
maximum.loop:
%maximum.j = phi i32 [ 0, %block.loop ], [ %maximum.j.next, %maximum.step ]
%maximum = phi double [ 0.0, %block.loop ], [ %maximum.next, %maximum.step ]
%maximum.more = icmp ult i32 %maximum.j, RECIPE_Q8_0_BLOCK
br i1 %maximum.more, label %maximum.step, label %scale
maximum.step:
%maximum.channel = add i32 %block.begin, %maximum.j
%maximum.channel.base = mul i32 %maximum.channel, %in.length
%maximum.local = add i32 %maximum.channel.base, %position
%maximum.index = add i32 %input.row, %maximum.local
%maximum.ptr = getelementptr inbounds double, ptr addrspace(1) %input, i32 %maximum.index
%maximum.raw = load double, ptr addrspace(1) %maximum.ptr, align 8
%maximum.absolute = call double @recipe.abs(double %maximum.raw)
%maximum.greater = call i1 @recipe.ogt(double %maximum.absolute, double %maximum)
%maximum.next = select i1 %maximum.greater, double %maximum.absolute, double %maximum
%maximum.j.next = add i32 %maximum.j, 1
br label %maximum.loop
scale:
%scale.raw = call double @recipe.div(double %maximum, double RECIPE_Q8_0_MAX)
%scale.nonzero = call i1 @recipe.one(double %scale.raw, double 0.0)
%scale.inverse.raw = call double @recipe.div(double 1.0, double %scale.raw)
%scale.inverse = select i1 %scale.nonzero, double %scale.inverse.raw, double 0.0
%scale.half = call half @recipe.to.f16(double %scale.raw)
%scale.stored = call double @recipe.from.f16(half %scale.half)
%weight.element = add i32 %weight.row, %block.begin
%weight.element.absolute = add i32 %weight.base, %weight.element
%weight.block = udiv i32 %weight.element.absolute, RECIPE_Q8_0_BLOCK
%weight.byte = mul i32 %weight.block, RECIPE_Q8_0_STRIDE
%weight.scale.ptr = getelementptr inbounds i8, ptr addrspace(1) %weights, i32 %weight.byte
%weight.scale.half = load half, ptr addrspace(1) %weight.scale.ptr, align 2
%weight.scale = call double @recipe.from.f16(half %weight.scale.half)
br label %dot.loop
dot.loop:
%dot.j = phi i32 [ 0, %scale ], [ %dot.j.next, %dot.step ]
%dot = phi i32 [ 0, %scale ], [ %dot.next, %dot.step ]
%dot.more = icmp ult i32 %dot.j, RECIPE_Q8_0_BLOCK
br i1 %dot.more, label %dot.step, label %block.done
dot.step:
%dot.channel = add i32 %block.begin, %dot.j
%dot.channel.base = mul i32 %dot.channel, %in.length
%dot.local = add i32 %dot.channel.base, %position
%dot.input.index = add i32 %input.row, %dot.local
%dot.input.ptr = getelementptr inbounds double, ptr addrspace(1) %input, i32 %dot.input.index
%dot.input = load double, ptr addrspace(1) %dot.input.ptr, align 8
%dot.scaled = call double @recipe.mul(double %dot.input, double %scale.inverse)
%dot.absolute = call double @recipe.abs(double %dot.scaled)
%dot.shifted = call double @recipe.add(double %dot.absolute, double 0.5)
%dot.rounded.absolute = call double @recipe.floor(double %dot.shifted)
%dot.negative = call i1 @recipe.olt(double %dot.scaled, double 0.0)
%dot.rounded.negative = call double @recipe.neg(double %dot.rounded.absolute)
%dot.rounded = select i1 %dot.negative, double %dot.rounded.negative, double %dot.rounded.absolute
%dot.input.code = call i32 @recipe.to.s32(double %dot.rounded)
%dot.weight.local = add i32 %weight.byte, RECIPE_Q8_0_HEADER
%dot.weight.byte = add i32 %dot.weight.local, %dot.j
%dot.weight.ptr = getelementptr inbounds i8, ptr addrspace(1) %weights, i32 %dot.weight.byte
%dot.weight.i8 = load i8, ptr addrspace(1) %dot.weight.ptr, align 1
%dot.weight.code = sext i8 %dot.weight.i8 to i32
%dot.product = mul i32 %dot.input.code, %dot.weight.code
%dot.next = add i32 %dot, %dot.product
%dot.j.next = add i32 %dot.j, 1
br label %dot.loop
block.done:
%dot.value = call double @recipe.from.s32(i32 %dot)
%block.scales = call double @recipe.mul(double %scale.stored, double %weight.scale)
%block.value = call double @recipe.mul(double %dot.value, double %block.scales)
%block.state = call RECIPE_STATE @recipe.state.from.model(double %block.value)
%sum.next = call RECIPE_STATE @recipe.state.add(RECIPE_STATE %sum, RECIPE_STATE %block.state)
%block.next = add i32 %block.begin, RECIPE_Q8_0_BLOCK
br label %block.loop
register.store:
%sum.ptr = getelementptr RECIPE_STATE, ptr addrspace(5) %sums, i32 %register
store RECIPE_STATE %sum, ptr addrspace(5) %sum.ptr, align RECIPE_STATE_ALIGN
...
ptr addrspace(1) %input, ptr addrspace(1) %weights, ptr addrspace(1) %output, ptr addrspace(1) %activation, ptr addrspace(1) %routing, ptr addrspace(1) %routing.context, i32 %rows, i32 %in.channels, i32 %in.length, i32 %out.channels, i32 %out.length, i32 %out.begin, i32 %out.span, i32 %kernel,
i1 %has.bias, i1 %relu, i1 %transpose, i1 %reverse, i1 %accumulate, i32 %tile.m, i32 %tile.n, i32 %tile.k, i32 %threads, i32 %weight.base, i32 %decode, i32 %expert.mode, i32 %expert.experts, i32 %expert.top, i32 %expert.hidden ) #1 { entry:
...
%weight.iq.kind = call i32 @recipe.model.iq.kind(i32 %decode)
%weight.iq.present = icmp ne i32 %weight.iq.kind, 0
...
%in.elements = mul i32 %in.channels, %in.length %expert.out.channels = mul i32 %out.channels, %expert.top %stored.out.channels = select i1 %expert.in, i32 %expert.out.channels, i32 %out.channels %out.elements = mul i32 %stored.out.channels, %out.length %is.conv = icmp ne i32 %kernel, 0 %span = select i1 %is.conv, i32 %kernel, i32 1 %terms = mul i32 %in.channels, %span %m.positions = mul i32 %rows, %out.span %m.expert.in = mul i32 %m.positions, %expert.top %m.total = select i1 %expert.in, i32 %m.expert.in, i32 %m.positions
...
sum.init.done:
br i1 %weight.q8_0, label %q8_0, label %iq.test
q8_0:
call void @contraction_q8_0_accumulate(ptr addrspace(5) %sums, ptr addrspace(1) %input, ptr addrspace(1) %weights, i1 %lane.active, i32 %lid, i32 %m.lanes, i32 %m.base, i32 %n.base, i32 %m.count, i32 %n.count, i32 %out.begin, i32 %out.span, i32 %in.channels, i32 %in.length, i32 %weight.base)
br label %store.loop
iq.test:
...
%b.fragment.full = icmp eq i32 %b.fragment.remainder, 0 %b.direct = xor i1 %transpose, true %b.contiguous = and i1 %b.fragment.full, %b.direct %b.vector.dense = and i1 %b.contiguous, %weight.dense %b.vector = and i1 %b.vector.dense, %expert.none
...
load.a.step: %a.m = udiv i32 %load, %a.columns %a.column = urem i32 %load, %a.columns %a.k = mul i32 %a.column, %a.width %a.global = add i32 %m.base, %a.m %a.expert.pair = udiv i32 %a.global, %expert.top.safe %a.pair = select i1 %expert.in, i32 %a.expert.pair, i32 %a.global %a.row = udiv i32 %a.pair, %out.span %a.position.local = urem i32 %a.pair, %out.span %a.position = add i32 %a.position.local, %out.begin %a.row.base = mul i32 %a.row, %in.elements %a.term = add i32 %term.base, %a.k
...
br label %load.advance
load.b.scalar:
br i1 %expert.any, label %load.b.expert, label %load.b.regular
load.b.regular:
...
br label %load.b.ready
load.b.expert:
%b.expert.in.pair = udiv i32 %m.base, %expert.top.safe %b.expert.in.slot = urem i32 %m.base, %expert.top.safe %b.expert.pair = select i1 %expert.in, i32 %b.expert.in.pair, i32 %m.base
%b.expert.row = udiv i32 %b.expert.pair, %out.span %b.expert.position.local = urem i32 %b.expert.pair, %out.span %b.expert.position = add i32 %b.expert.position.local, %out.begin
%b.expert.row.position = mul i32 %b.expert.row, %in.length %b.expert.routing.pair = add i32 %b.expert.row.position, %b.expert.position
%b.expert.out.slot = udiv i32 %b.term, %expert.hidden %b.expert.out.term = urem i32 %b.term, %expert.hidden
%b.expert.slot = select i1 %expert.in, i32 %b.expert.in.slot, i32 %b.expert.out.slot
%b.expert = call i32 @expert_selected(ptr addrspace(1) %routing.context, i32 %b.expert.routing.pair, i32 %b.expert.slot, i32 %expert.experts, i32 %expert.top)
%b.expert.plane.terms = select i1 %expert.in, i32 %in.channels, i32 %expert.hidden %b.expert.plane = mul i32 %out.channels, %b.expert.plane.terms %b.expert.base = mul i32 %b.expert, %b.expert.plane
%b.expert.channel.base = mul i32 %b.channel, %b.expert.plane.terms %b.expert.term = select i1 %expert.in, i32 %b.term, i32 %b.expert.out.term %b.expert.local = add i32 %b.expert.channel.base, %b.expert.term %b.expert.index.local = add i32 %b.expert.base, %b.expert.local %b.expert.index = add i32 %weight.base, %b.expert.index.local
%b.expert.weight = call double @recipe.model.weight(ptr addrspace(1) %weights, i32 %b.expert.index, i32 %decode)
%b.routing.row.span = mul i32 %expert.experts, %in.length %b.routing.row = mul i32 %b.expert.row, %b.routing.row.span %b.routing.expert = mul i32 %b.expert, %in.length %b.routing.local = add i32 %b.routing.expert, %b.expert.position %b.routing.index = add i32 %b.routing.row, %b.routing.local
%b.routing.ptr = getelementptr inbounds double, ptr addrspace(1) %routing, i32 %b.routing.index %b.routing.weight = load double, ptr addrspace(1) %b.routing.ptr, align 8 %b.expert.scaled = call double @recipe.mul(double %b.expert.weight, double %b.routing.weight) %b.expert.value = select i1 %expert.out, double %b.expert.scaled, double %b.expert.weight
br label %load.b.ready
load.b.ready:
%b.value = phi double [ %b.loaded, %load.b.direct ], [ %b.decoded, %load.b.packed ], [ %b.expert.value, %load.b.expert ]
br label %load.store
...
%store.register = phi i32 [ 0, %accumulate.done ], [ 0, %q8_0 ], [ 0, %iq ], [ %store.register.next, %store.next ] %store.more = icmp ult i32 %store.register, RECIPE_REGISTER_COUNT br i1 %store.more, label %store.test, label %job.done
...
store: %store.channel = add i32 %n.base, %store.output.n.raw %store.m.global = add i32 %m.base, %store.output.m.raw %store.expert.pair = udiv i32 %store.m.global, %expert.top.safe %store.pair = select i1 %expert.in, i32 %store.expert.pair, i32 %store.m.global %store.position.local = urem i32 %store.pair, %out.span %store.position = add i32 %store.position.local, %out.begin %store.row = udiv i32 %store.pair, %out.span %store.output.row.base = mul i32 %store.row, %out.elements
%store.expert.slot = urem i32 %store.m.global, %expert.top.safe %store.expert.channel.base = mul i32 %store.expert.slot, %out.channels %store.expert.channel = add i32 %store.expert.channel.base, %store.channel %store.output.channel = select i1 %expert.in, i32 %store.expert.channel, i32 %store.channel
%store.output.channel.base = mul i32 %store.output.channel, %out.length %store.output.local = add i32 %store.output.channel.base, %store.position %store.output.index = add i32 %store.output.row.base, %store.output.local %store.output.ptr = getelementptr inbounds double, ptr addrspace(1) %output, i32 %store.output.index
%store.bias.base = mul i32 %out.channels, %terms %store.bias.index = add i32 %store.bias.base, %store.channel
br i1 %has.bias, label %store.bias.choose, label %store.unbiased
store.bias.choose: br i1 %weight.packed, label %store.bias.packed, label %store.bias.direct
store.bias.direct:
...
%store.biased = call double @recipe.add(double %store.sum, double %store.bias) br label %store.epilogue
store.unbiased: %store.sum.unbiased.ptr = getelementptr [RECIPE_REGISTER_COUNT x RECIPE_STATE], ptr addrspace(5) %sums, i32 0, i32 %store.register %store.sum.unbiased.wide = load RECIPE_STATE, ptr addrspace(5) %store.sum.unbiased.ptr, align RECIPE_STATE_ALIGN %store.unbiased.value = call double @recipe.encode(RECIPE_STATE %store.sum.unbiased.wide) br label %store.epilogue
store.epilogue: %store.raw = phi double [ %store.biased, %store.bias.ready ], [ %store.unbiased.value, %store.unbiased ] %store.forward = xor i1 %reverse, true %store.activate = and i1 %relu, %store.forward %store.positive = call i1 @recipe.ogt(double %store.raw, double 0.0) %store.activated = select i1 %store.positive, double %store.raw, double 0.0 %store.result = select i1 %store.activate, double %store.activated, double %store.raw %store.prior = load double, ptr addrspace(1) %store.output.ptr, align 2 %store.accumulated = call double @recipe.add(double %store.prior, double %store.result) %store.value = select i1 %accumulate, double %store.accumulated, double %store.result store double %store.value, ptr addrspace(1) %store.output.ptr, align 8 br label %store.next
...
define internal void @fold_forward_body( ptr addrspace(1) %input, ptr addrspace(1) %output, i32 %p, i32 %groups, i32 %width, i32 %length ) #1 { entry:
%narrow = mul i32 %groups, %length %row = udiv i32 %p, %narrow %within = urem i32 %p, %narrow %group = udiv i32 %within, %length %position = urem i32 %within, %length
%channels = mul i32 %groups, %width %wide = mul i32 %channels, %length %row.base = mul i32 %row, %wide %first = mul i32 %group, %width %first.base = mul i32 %first, %length
%base.row = add i32 %row.base, %first.base %base = add i32 %base.row, %position br label %loop loop:
%c = phi i32 [ 0, %entry ], [ %c.next, %step ] %sum = phi double [ 0.0, %entry ], [ %sum.next, %step ] %more = icmp ult i32 %c, %width br i1 %more, label %step, label %done
step: %offset = mul i32 %c, %length %index = add i32 %base, %offset %input.ptr = getelementptr inbounds double, ptr addrspace(1) %input, i32 %index %value = load double, ptr addrspace(1) %input.ptr, align 8
%sum.next = call double @recipe.add(double %sum, double %value) %c.next = add i32 %c, 1 br label %loop
...
define internal void @fold_reverse_body( ptr addrspace(1) %delta, ptr addrspace(1) %adjoint, i32 %p, i32 %groups, i32 %width, i32 %length ) #1 { entry:
%channels = mul i32 %groups, %width %wide = mul i32 %channels, %length %row = udiv i32 %p, %wide %within = urem i32 %p, %wide %channel = udiv i32 %within, %length %position = urem i32 %within, %length
%group = udiv i32 %channel, %width %narrow = mul i32 %groups, %length %row.base = mul i32 %row, %narrow %group.base = mul i32 %group, %length %index.row = add i32 %row.base, %group.base %index = add i32 %index.row, %position
%delta.ptr = getelementptr inbounds double, ptr addrspace(1) %delta, i32 %index %value = load double, ptr addrspace(1) %delta.ptr, align 8
%adjoint.ptr = getelementptr inbounds double, ptr addrspace(1) %adjoint, i32 %p %prior = load double, ptr addrspace(1) %adjoint.ptr, align 8
...
define internal void @dconv_forward_body( ptr addrspace(1) %input, ptr addrspace(1) %weights, ptr addrspace(1) %output, i32 %p, i32 %channels, i32 %length, i32 %kernel, i32 %dilation, i32 %decode ) #1 { entry:
...
%more = icmp ult i32 %tap, %kernel br i1 %more, label %step, label %done step: %back = sub i32 %reach, %tap %back.scaled = mul i32 %back, %dilation %source = sub i32 %position, %back.scaled
...
%tap.index = add i32 %tap.base, %tap %weight = call double @recipe.model.weight(ptr addrspace(1) %weights, i32 %tap.index, i32 %decode)
...
%row.base = mul i32 %row, %narrow %channel.base = mul i32 %channel, %length %base = add i32 %row.base, %channel.base %tap.base = mul i32 %channel, %kernel
%reach = sub i32 %kernel, 1 br label %loop loop: %tap = phi i32 [ 0, %entry ], [ %tap.next, %step ] %sum = phi double [ 0.0, %entry ], [ %sum.next, %step ]
%more = icmp ult i32 %tap, %kernel br i1 %more, label %step, label %done step: %ahead.taps = sub i32 %reach, %tap %ahead = mul i32 %ahead.taps, %dilation %target = add i32 %position, %ahead
...
%channel = udiv i32 %p, %kernel %tap = urem i32 %p, %kernel %narrow = mul i32 %channels, %length %channel.base = mul i32 %channel, %length %reach = sub i32 %kernel, 1 %shift.taps = sub i32 %reach, %tap %shift = mul i32 %shift.taps, %dilation
...
%row.base = mul i32 %row, %narrow %base = add i32 %row.base, %channel.base %input.index = add i32 %base, %source.clamped %delta.index = add i32 %base, %position
%input.ptr = getelementptr inbounds double, ptr addrspace(1) %input, i32 %input.index %loaded = load double, ptr addrspace(1) %input.ptr, align 8 %value = select i1 %valid, double %loaded, double 0.0
...
%column.adjoint.index = add i32 %adjoint.base, %column.cell
%column.adjoint.pointer = getelementptr inbounds double, ptr addrspace(1) %context, i32 %column.adjoint.index
%column.adjoint = load double, ptr addrspace(1) %column.adjoint.pointer, align 8
%column.i.offset.row = mul i32 %column.i, %length %column.i.offset = add i32 %column.i.offset.row, %time
...
column.store: %column.row.offset = mul i32 %column, %length %column.offset = add i32 %column.row.offset, %time
%column.value.index = add i32 %v.base, %column.offset
%column.value.pointer = getelementptr inbounds double, ptr addrspace(1) %input, i32 %column.value.index
...
%error.index = add i32 %vector.base, %column
%error.pointer = getelementptr inbounds double, ptr addrspace(1) %context, i32 %error.index
store double %error, ptr addrspace(1) %error.pointer, align 8
%weight.index = add i32 %weight.base, %column
%weight.pointer = getelementptr inbounds double, ptr addrspace(1) %context, i32 %weight.index
...
%value.adjoint.pointer = getelementptr inbounds double, ptr addrspace(1) %input.adjoint, i32 %column.value.index
...
%row.adjoint.index = add i32 %adjoint.base, %row.cell
%row.adjoint.pointer = getelementptr inbounds double, ptr addrspace(1) %context, i32 %row.adjoint.index
%row.adjoint = load double, ptr addrspace(1) %row.adjoint.pointer, align 8
%row.error.index = add i32 %vector.base, %row.j
%row.error.pointer = getelementptr inbounds double, ptr addrspace(1) %context, i32 %row.error.index
...
%row.j.offset.row = mul i32 %row.j, %length %row.j.offset = add i32 %row.j.offset.row, %time
%row.delta.index = add i32 %o.base, %row.j.offset
%row.delta.pointer = getelementptr inbounds double, ptr addrspace(1) %delta, i32 %row.delta.index
...
%decay.adjoint.pointer = getelementptr inbounds double, ptr addrspace(1) %gate.adjoint, i32 %decay.index
...
%write.adjoint.pointer = getelementptr inbounds double, ptr addrspace(1) %gate.adjoint, i32 %write.index
...
restore.loop: %restore.i = phi i32 [ 0, %chunk.loop ], [ %restore.next, %restore.step ] %restore.more = icmp ult i32 %restore.i, %state
br i1 %restore.more, label %restore.step, label %replay.loop
restore.step: %restore.entry = add i32 %chunk.entry.base, %restore.i
%restore.entry.pointer = getelementptr inbounds double, ptr addrspace(1) %context, i32 %restore.entry
%restore.value = load double, ptr addrspace(1) %restore.entry.pointer, align 8 %restore.work = add i32 %work.base, %restore.i
%restore.work.pointer = getelementptr inbounds double, ptr addrspace(1) %context, i32 %restore.work
...
%replay.slot.offset = mul i32 %replay.i, %state %replay.slot = add i32 %replay.base, %replay.slot.offset
br i1 %replay.more, label %replay.save, label %backward.loop
replay.save: %save.i = phi i32 [ 0, %replay.loop ], [ %save.next, %save.step ] %save.more = icmp ult i32 %save.i, %state
br i1 %save.more, label %save.step, label %replay.step
save.step: %save.work = add i32 %work.base, %save.i
%save.work.pointer = getelementptr inbounds double, ptr addrspace(1) %context, i32 %save.work
%save.value = load double, ptr addrspace(1) %save.work.pointer, align 8 %save.slot = add i32 %replay.slot, %save.i
%save.slot.pointer = getelementptr inbounds double, ptr addrspace(1) %context, i32 %save.slot
...
backward.step: %backward.index = sub i32 %backward.i, 1 %backward.time = add i32 %chunk.start, %backward.index
...
step: %pair.base = mul i32 %r, %heads %pair = add i32 %pair.base, %p %index = add i32 %partials, %pair
%pointer = getelementptr inbounds double, ptr addrspace(1) %context, i32 %index
...
done: %gradient.index = add i32 %offset, %p
%gradient.pointer = getelementptr inbounds double, ptr addrspace(1) %gradient, i32 %gradient.index
...
define internal void @topk_forward_body( ptr addrspace(1) %scores, ptr addrspace(1) %weights, ptr addrspace(1) %context, i32 %p, i32 %experts, i32 %length, i32 %top, i32 %scoring, i32 %renormalize ) #1 { entry:
...
write.entry: %denominator = select i1 %divide, double %total, double 1.0 %compact.base = mul i32 %p, %top br label %write.loop
write.loop: %w = phi i32 [ 0, %write.entry ], [ %w.next, %write.advance ] %selected = phi i32 [ 0, %write.entry ], [ %selected.next, %write.advance ] %write.more = icmp ult i32 %w, %experts br i1 %write.more, label %write.step, label %exit
...
%w.value = select i1 %w.marked, double %w.probability, double 0.0 store double %w.value, ptr addrspace(1) %w.mark.ptr, align 8 br i1 %w.marked, label %compact.write, label %write.advance
compact.write: %compact.index = add i32 %compact.base, %selected %compact.ptr = getelementptr inbounds i32, ptr addrspace(1) %context, i32 %compact.index store i32 %w, ptr addrspace(1) %compact.ptr, align 4 %selected.grown = add i32 %selected, 1 br label %write.advance
write.advance: %selected.next = phi i32 [ %selected.grown, %compact.write ], [ %selected, %write.step ] %w.next = add i32 %w, 1 br label %write.loop
exit: ret void }
...
cached:
%cached.block = mul i32 %block.index, %index.width
%cached.row = add i32 %representative.start, %cached.block
%cached.index = add i32 %cached.row, %d
%cached.ptr = getelementptr inbounds double, ptr addrspace(1) %context, i32 %cached.index
%cached.value = load double, ptr addrspace(1) %cached.ptr, align 8
br label %done
prefix.loop:
%key = phi i32 [ %start, %entry ], [ %key.next, %prefix.step ]
%sum = phi double [ %model.zero, %entry ], [ %sum.next, %prefix.step ]
%more = icmp ult i32 %key, %query.next
br i1 %more, label %prefix.step, label %done
prefix.step:
%key.position = add i32 %key.origin, %key
%key.index = add i32 %key.position, %d.offset
%key.ptr = getelementptr inbounds double, ptr addrspace(1) %indexer, i32 %key.index
...
%stop.over = icmp ugt i32 %stop.full, %end
%stop = select i1 %stop.over, i32 %end, i32 %stop.full
%first.before = icmp ult i32 %start, %begin
%first = select i1 %first.before, i32 %begin, i32 %start
%fresh = icmp uge i32 %start, %begin
%extends = icmp ult i32 %first, %stop
...
br label %clear.loop
key.loop:
%key = phi i32 [ %first, %entry ], [ %first, %clear.loop ], [ %key.advance, %key.step ]
...
%dim.ptr = getelementptr inbounds double, ptr addrspace(1) %indexer, i32 %dim.index
...
i32 %p, i32 %keep, i1 %derivatives, i32 %rows, i32 %from, i32 %heads, i32 %channels, i32 %kv.heads, i32 %index.heads,
...
%row.base = mul i32 %row, %row.stride
%key.origin = add i32 %row.base, %index.key.base
%query.position = add i32 %row.base, %query
%count.less = udiv i32 %query, %select.block
%count = add i32 %count.less, 1
%p.wide = zext i32 %p to i64
%score.stride.wide = zext i32 %score.stride to i64
%score.base.wide = zext i32 %score.base to i64
%score.query = mul i64 %p.wide, %score.stride.wide
%score.start = add i64 %score.base.wide, %score.query
%score.row.ptr = getelementptr inbounds double, ptr addrspace(1) %context, i64 %score.start
%representative.row = mul i32 %row, %representative.stride
%representative.start = add i32 %representative.base, %representative.row
%score.count = mul i32 %rows, %length
%score.count.wide = zext i32 %score.count to i64
%score.plane = mul i64 %score.count.wide, %score.stride.wide
%derivative.base = add i64 %score.base.wide, %score.plane
%length.wide = zext i32 %length to i64
%blocks.wide = zext i32 %blocks to i64
%heads.wide = zext i32 %heads to i64
%derivative.head.stride = mul i64 %length.wide, %blocks.wide
%derivative.row.stride = mul i64 %derivative.head.stride, %heads.wide
%row.wide = zext i32 %row to i64
%derivative.row = mul i64 %row.wide, %derivative.row.stride
%derivative.query = mul i32 %query, %blocks
%derivative.query.wide = zext i32 %derivative.query to i64
%derivative.row.start = add i64 %derivative.base, %derivative.row
%derivative.start = add i64 %derivative.row.start, %derivative.query.wide
%derivative.ptr = getelementptr inbounds double, ptr addrspace(1) %context, i64 %derivative.start
%derivative.total = mul i32 %heads, %blocks
%derivative.count = select i1 %derivatives, i32 %derivative.total, i32 0
%state.zero = call RECIPE_STATE @recipe.state.from.u1(i1 false)
%model.zero = call double @recipe.encode(RECIPE_STATE %state.zero)
br label %clear.loop
clear.loop:
...
clear.step:
%clear.ptr = getelementptr inbounds double, ptr addrspace(1) %score.row.ptr, i32 %clear.b
store double %model.zero, ptr addrspace(1) %clear.ptr, align 8
...
%derivative.clear.head.wide = zext i32 %derivative.clear.head to i64
%derivative.clear.plane = mul i64 %derivative.clear.head.wide, %derivative.head.stride
%derivative.clear.block.wide = zext i32 %derivative.clear.block to i64
%derivative.clear.index = add i64 %derivative.clear.plane, %derivative.clear.block.wide
%derivative.clear.ptr = getelementptr inbounds double, ptr addrspace(1) %derivative.ptr, i64 %derivative.clear.index
...
br label %score.dim.loop
score.store:
%score.ptr = getelementptr inbounds double, ptr addrspace(1) %score.row.ptr, i32 %score.b
...
rank.body:
%rank.b.ptr = getelementptr inbounds double, ptr addrspace(1) %score.row.ptr, i32 %rank.b
...
rank.inner.step:
%rank.c.ptr = getelementptr inbounds double, ptr addrspace(1) %score.row.ptr, i32 %rank.c
...
%rank.flag.index = add i32 %blocks, %rank.b
%rank.flag.ptr = getelementptr inbounds double, ptr addrspace(1) %score.row.ptr, i32 %rank.flag.index
...
ptr addrspace(1) %precompute.context, ptr addrspace(1) %input, ptr addrspace(1) %input, ptr addrspace(1) %context,
...
i32 %tile.m, i32 %tile.n, i32 %tile.k, i32 %threads, i32 %precompute.base, i32 %decode, i32 0, i32 0, i32 0, i32 0 )
