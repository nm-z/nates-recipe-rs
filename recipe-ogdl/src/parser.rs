...
pub(crate) fn parse(input: &str) -> Result<Graph, ParseError> {
	if input.is_empty() {
		return Ok(Graph::new());
	}
		...
		match bytes[cursor] {
			b'\r' if bytes.get(cursor + 1) != Some(&b'\n') => {
				...
				return Err(ParseError::new(
					ParseErrorKind::BareCarriageReturn,
					SourceLocation::new(line, column, cursor),
				));
			}
			b'\n' => {
...
fn parse_line(
	...
	line_offset: usize,
	graph: &mut Graph,
	ancestors: &mut Vec<crate::NodeId>,
) -> Result<(), ParseError> {
	...
	if indentation > ancestors.len() {
		return Err(ParseError::new(
			ParseErrorKind::IndentationJump {
				found: indentation,
				maximum: ancestors.len(),
			},
			SourceLocation::new(line, 1, line_offset),
		));
	}
	...
	if segment_start == source.len() {
		return Err(empty_node_error(source, line, line_offset, segment_start));
	}
