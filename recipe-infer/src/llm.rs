...
/// One dense causal transformer layer (gemma3 / llama-style), reusing the shared
/// kernels. Applies only the norms present in `m.norms[l]`, scales Q by
/// `1/sqrt(hd)` (the gqa kernel does no scaling), and masks fully causally.
fn layer_causal(
	m: &Model,
	l: usize,
	h_in: &GpuBuffer,
	...
	if let Some(qn) = nm.get("q_norm") {
		gpu_rmsnorm_f64(&ar.q, qn, &m.eps, t * nqh, hd, &ar.q)?;
	}
	if let Some(kn) = nm.get("k_norm") {
		gpu_rmsnorm_f64(&ar.k, kn, &m.eps, t * nkv, hd, &ar.k)?;
	}
	...
	if let Some(pa) = nm.get("post_attn") {
		gpu_rmsnorm_f64(&ar.o, pa, &m.eps, t, ne, &ar.o)?;
	}
	gpu_add_into(&ar.o, h_in, t * ne, &ar.attn_out)?;
	gpu_rmsnorm_f64(&ar.attn_out, &nm["pre_ff"], &m.eps, t, ne, &ar.cms)?;
	gpu_gemm_bt_f64(
	...
	if let Some(pf) = nm.get("pfw") {
		gpu_rmsnorm_f64(&ar.mlp0, pf, &m.eps, t, ne, &ar.mlp0)?;
	}
