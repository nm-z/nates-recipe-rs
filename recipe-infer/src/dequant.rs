...
fn codec(d: Dtype) -> Result<Cx> {
	let cx = match d {
		...
		Dtype::F8e4m3fnuz => Cx::Own(Box::new(FpCodec::e4m3("f8_e4m3_fnuz", 1, true))),
		Dtype::F8e5m2fnuz => Cx::Own(Box::new(FpCodec::e5m2("f8_e5m2_fnuz", 1, true))),
		...
		Dtype::F8x2e4m3fnuz => Cx::Own(Box::new(FpCodec::e4m3("f8x2_e4m3_fnuz", 2, true))),
		Dtype::F8x4e4m3fnuz => Cx::Own(Box::new(FpCodec::e4m3("f8x4_e4m3_fnuz", 4, true))),
		Dtype::F8x2e5m2fnuz => Cx::Own(Box::new(FpCodec::e5m2("f8x2_e5m2_fnuz", 2, true))),
		Dtype::F8x4e5m2fnuz => Cx::Own(Box::new(FpCodec::e5m2("f8x4_e5m2_fnuz", 4, true))),
		Dtype::F16x2 => Cx::Own(Box::new(WideCodec::new("f16x2", WideKind::F16, 2))),
		Dtype::BF16x2 => Cx::Own(Box::new(WideCodec::new("bf16x2", WideKind::Bf16, 2))),
...
fn minifloat_to_f32(bits: u32, ebits: u32, mbits: u32, bias: i32, special: FpSpecial, signed: bool) -> f32 {
	let sign = if signed {
		(bits >> (ebits + mbits)) & 1
	} else {
		0
...
fn f32_to_minifloat(x: f32, ebits: u32, mbits: u32, bias: i32, special: FpSpecial, signed: bool) -> u32 {
	let emax = (1u32 << ebits) - 1;
	let maxmant = (1u32 << mbits) - 1;
	...
	let a = x.abs() as f64;
	if a == 0.0 {
		return if special == FpSpecial::Fnuz {
			0
		} else {
			sfield
		};
...
impl Codec for FpCodec {
	...
	fn encode_block(&self, vals: &[f32], out: &mut Vec<u8>) -> Result<()> {
		...
		let mut blk = vec![0u8; self.block_bytes()];
		for (j, &x) in vals.iter().enumerate() {
			let bits = f32_to_minifloat(x, self.ebits, self.mbits, self.bias, self.special, true);
	...
	fn decode_block(&self, raw: &[u8], out: &mut Vec<f32>) -> Result<()> {
		...
		for j in 0..self.lanes {
			...
			let v = match self.kind {
				...
				WideKind::Bf16 => f32::from_bits((u16::from_le_bytes([s[0], s[1]]) as u32) << 16),
				WideKind::F32 => f32::from_le_bytes([s[0], s[1], s[2], s[3]]),
				WideKind::Tf32 => {
					let word = u32::from_le_bytes([s[0], s[1], s[2], s[3]]);
					f32::from_bits(word & 0xFFFF_E000)
				}
				WideKind::F64 => f64::from_le_bytes([s[0], s[1], s[2], s[3], s[4], s[5], s[6], s[7]]) as f32,
			};
			out.push(v);
		}
		return Ok(());
	...
	fn encode_block(&self, vals: &[f32], out: &mut Vec<u8>) -> Result<()> {
		for &x in vals {
			match self.kind {
				...
				WideKind::Bf16 => out.extend_from_slice(&f32_to_bf16(x).to_le_bytes()),
				WideKind::F32 => out.extend_from_slice(&x.to_le_bytes()),
				WideKind::Tf32 => out.extend_from_slice(&(x.to_bits() & 0xFFFF_E000).to_le_bytes()),
...
fn img_channel_encode(x: f32, width: u32, fmt: ImgFmt, idx: usize, count: usize) -> Result<u64> {
	...
	let v = match fmt {
		...
		ImgFmt::Snorm => ((x.clamp(-1.0, 1.0) as f64 * smax).round() as i64 as u64) & mask,
		ImgFmt::Srgb => {
