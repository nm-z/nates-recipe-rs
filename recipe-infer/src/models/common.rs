...
use gpu_core::infer_ops::{
	gpu_flash_gqa, gpu_flash_mla, gpu_gemm_bt, gpu_glu_silu, gpu_rmsnorm_f64, gpu_rmsnorm_f64_nogamma,
	gpu_rope_partial_factors_pos, gpu_rope_partial_pos, gpu_scale_f64_inplace,
};
use gpu_core::k_gapact::gpu_softplus;
use gpu_core::kernels::{
	gpu_add_into, gpu_bias_add, gpu_broadcast_mul, gpu_concat_into, gpu_copy_into, gpu_gated_delta_scan,
	gpu_gelu_into, gpu_l2norm_rows, gpu_layernorm_opt_into, gpu_mul_inplace, gpu_relu_into, gpu_row_scale,
	gpu_sigmoid_into, gpu_silu_into, gpu_slice_cols, gpu_slice_lead_into, gpu_ssm_conv_causal,
...
fn norm_of<'m>(m: &'m Model, l: usize, key: Nk) -> Result<&'m GpuBuffer> {
	m.norms[l][key as usize].as_ref().ok_or_else(|| {
		anyhow!(
			"{}: layer {l} has no {:?} norm weight",
			m.hp.arch,
			key.name()
		)
	})
...
fn cached_gqa(
	...
	gpu_copy_into(
		new_k,
		t * kd,
		&s.k.view((d.cached - d.win_base) * kd, t * kd),
	)?;
	gpu_copy_into(
		new_v,
		t * vd,
		&s.v.view((d.cached - d.win_base) * vd, t * vd),
	)?;
...
fn cached_mla(
	...
	gpu_copy_into(
		new_k,
		t * kw,
		&s.k.view((d.cached - d.win_base) * kw, t * kw),
	)?;
	gpu_copy_into(
		new_v,
		t * kvlr,
		&s.v.view((d.cached - d.win_base) * kvlr, t * kvlr),
	)?;
	let stage_k = [&d.stage.sk[0], &d.stage.sk[1]];
	let stage_v = [&d.stage.sv[0], &d.stage.sv[1]];
	let mut scratch = Vec::new();
	...
	let res_n = total - d.win_base;
	gpu_flash_mla(
		q, &s.k, &s.v, t, res_n, nqh, 1, kw, kvlr, d.cached, total, out, &ar.cm, &ar.cl, &ar.cacc, d.win_base, true,
	)?;
	return Ok(());
}
fn attn_block(
	...
	cached_gqa(
		&dec, ar, &ar.q, &ar.k, &ar.v, t, nqh, nkv, hd, kd, kd, max_bias, sp.bidir, &ar.attn,
	)?;
	gpu_gemm_bt(
		&ar.attn,
...
fn mla_attn_block(m: &Model, l: usize, sp: &Spec, h_in: &GpuBuffer, t: usize, ar: &Arena, dec: &DecCtx) -> Result<()> {
	...
	if nqh != 1 {
		return Err(anyhow!(
			"{}: MLA absorbed path supports n_head == 1, got {nqh}",
			hp.arch
		));
	}
	...
	gpu_gemm_bt(
		&ar.x,
		&m.stream(&layer_name(l, "self_attn.q_a_proj.weight"))?,
		t,
		qlr,
		ne,
		&ar.mqa,
	...
	gpu_gemm_bt(
		&ar.mqa,
		&m.stream(&layer_name(l, "self_attn.q_b_proj.weight"))?,
		t,
		nqh * hdk,
		qlr,
		&ar.mqb,
	...
	gpu_gemm_bt(
		&ar.mqn,
		&m.stream(&layer_name(l, "self_attn.k_b_proj.weight"))?,
		t,
		kvlr,
		nope,
		&ar.mqx,
	...
	gpu_gemm_bt(
		&ar.x,
		&m.stream(&layer_name(l, "self_attn.kv_a_proj.weight"))?,
		t,
		kvlr + rot,
		ne,
		&ar.mkv,
	...
	let kw = kvlr + rot;
	cached_mla(
		&dec, ar, &ar.mqc, &ar.mkk, &ar.mkc, t, nqh, kw, kvlr, &ar.mrw,
	)?;
	gpu_gemm_bt(
		&ar.mrw,
		&m.stream(&layer_name(l, "self_attn.v_b_proj.weight"))?,
		t,
		hdv,
		kvlr,
		&ar.mav,
...
pub(super) fn layer_minicpm3(
	...
	gpu_gemm_bt(
		&ar.mkc,
		&m.stream(&layer_name(l, "self_attn.kv_b_proj.weight"))?,
		t,
		nqh * hdv,
		kvlr,
		&ar.v,
	...
	cached_gqa(
		&dec, ar, &ar.mqb, &ar.k, &ar.v, t, nqh, nqh, hdk, kwid, vwid, 0.0, false, &ar.attn,
	)?;
	gpu_gemm_bt(
		&ar.attn,
		&m.stream(&layer_name(l, "self_attn.o_proj.weight"))?,
...
pub(super) fn layer_talkie(
	...
	gpu_scale_f64_inplace(attn_scale, t * qd, &ar.q)?;
	cached_gqa(
		&dec, ar, &ar.q, &ar.k, &ar.v, t, nqh, nkv, hd, kd, kd, 0.0, false, &ar.attn,
	)?;
	gpu_gemm_bt(
		&ar.attn,
		&m.stream(&layer_name(l, "self_attn.o_proj.weight"))?,
...
fn ssm_of<'m>(buf: &'m [Option<GpuBuffer>], l: usize, arch: &str, name: &str) -> Result<&'m GpuBuffer> {
	buf.get(l)
		.and_then(|o| o.as_ref())
...
fn mamba2_mix(
	...
	if nh == 0 || di % nh != 0 {
		return Err(anyhow!(
			"{arch}: mamba-2 d_inner={di} not divisible by n_head={nh}"
		));
	}
	if ng == 0 || di % ng != 0 {
		return Err(anyhow!(
			"{arch}: mamba-2 d_inner={di} not divisible by n_group={ng}"
		));
	}
	...
	gpu_gemm_bt(
		&ar.x,
		&w_in.view((di + conv_dim) * ne, nh * ne),
		t,
		nh,
		ne,
		&ar.ss_dtlr,
...
fn plamo2_mix(
	...
	if nh == 0 || di % nh != 0 {
		return Err(anyhow!(
			"{arch}: plamo2 d_inner={di} not divisible by n_head={nh}"
		));
	}
	let head_dim = di / nh;
	let bcd = dtd + 2 * ds;
	...
	deinterleave_heads(
		&ar.ss_zx, t, head_dim, nh, head_dim, &ar.ss_x, &ar.ss_dt, &ar.ss_y,
	)?;
	...
	gpu_rmsnorm_f64(
		&ar.ss_bb,
		ssm_of(&m.ssm_b_norm, l, arch, "ssm_b_norm")?,
		&m.eps,
		t,
		ds,
		&ar.ss_bb,
	...
	gpu_rmsnorm_f64(
		&ar.ss_cc,
		ssm_of(&m.ssm_c_norm, l, arch, "ssm_c_norm")?,
		&m.eps,
		t,
		ds,
		&ar.ss_cc,
	...
	gpu_rmsnorm_f64(
		&ar.ss_dtlr,
		ssm_of(&m.ssm_dt_norm, l, arch, "ssm_dt_norm")?,
		&m.eps,
		t,
		dtd,
		&ar.ss_dtlr,
	...
	gpu_bias_add(
		&dt,
		ssm_of(&m.ssm_dt_b, l, arch, "ssm_dt.bias")?,
		t,
		nh,
		&dt,
	)?;
...
pub(super) fn layer_is_recur(m: &Model, l: usize) -> bool {
	return m
		.big
		.contains_key(&layer_name(l, "self_attn.ssm_in.weight"));
}

pub(super) fn layer_is_delta(m: &Model, l: usize) -> bool {
	return m
		.big
		.contains_key(&layer_name(l, "self_attn.ssm_conv1d.weight"))
		|| m.big
			.contains_key(&layer_name(l, "self_attn.q_conv.weight"));
}
...
fn gated_delta_mix(
	...
	gpu_gemm_bt(
		&ar.x,
		&m.stream(&layer_name(l, "self_attn.qkv_proj.weight"))?,
		t,
		conv_dim,
		ne,
		&ar.d_qkv,
	...
	gpu_gemm_bt(
		&ar.x,
		&m.stream(&layer_name(l, "self_attn.z_gate.weight"))?,
		t,
		value_dim,
		ne,
		&ar.d_z,
	...
	if m.big
		.contains_key(&layer_name(l, "self_attn.ssm_ba.weight"))
	{
		gpu_gemm_bt(
			&ar.x,
			&m.stream(&layer_name(l, "self_attn.ssm_ba.weight"))?,
			t,
			2 * hv,
			ne,
			&ar.d_o,
		...
		gpu_gemm_bt(
			&ar.x,
			&m.stream(&layer_name(l, "self_attn.ssm_beta.weight"))?,
			t,
			hv,
			ne,
			&ar.d_bt,
		...
		gpu_gemm_bt(
			&ar.x,
			&m.stream(&layer_name(l, "self_attn.ssm_alpha.weight"))?,
			t,
			hv,
			ne,
			&ar.d_g,
	...
	let conv_w = ssm_of(&m.ssm_conv_w, l, arch, "ssm_conv1d.weight")?;
	gpu_ssm_conv_causal_silu(
		&ar.d_qkv, conv_w, None, t, conv_dim, dc, &ar.d_cv, cin, cout,
	)?;
	...
	gpu_rmsnorm_f64(
		&ar.d_o,
		ssm_of(&m.ssm_norm, l, arch, "ssm_norm.weight")?,
		&m.eps,
		t * hv,
		d,
		&ar.d_o,
	...
	gpu_gemm_bt(
		&ar.d_o,
		&m.stream(&layer_name(l, "self_attn.ssm_out.weight"))?,
		t,
		ne,
		value_dim,
		out,
...
fn kda_mix(m: &Model, l: usize, h_in: &GpuBuffer, out: &GpuBuffer, t: usize, ar: &Arena, dec: &DecCtx) -> Result<()> {
	...
	gpu_gemm_bt(
		&ar.x,
		&m.stream(&layer_name(l, "self_attn.f_a.weight"))?,
		t,
		d,
		ne,
		&ar.d_z,
	...
	gpu_gemm_bt(
		&ar.d_z,
		&m.stream(&layer_name(l, "self_attn.f_b.weight"))?,
		t,
		di,
		d,
		&ar.d_g,
	...
	gpu_gemm_bt(
		&ar.x,
		&m.stream(&layer_name(l, "self_attn.g_a.weight"))?,
		t,
		d,
		ne,
		&ar.d_z,
	...
	gpu_gemm_bt(
		&ar.d_z,
		&m.stream(&layer_name(l, "self_attn.g_b.weight"))?,
		t,
		di,
		d,
		&ar.d_qkv,
...
fn kimi_mla_attn(
	...
	if nqh != 1 {
		return Err(anyhow!(
			"{}: kimi MLA supports n_head == 1, got {nqh}",
			hp.arch
		));
	}
...
fn shared_expert(m: &Model, l: usize, cms: &GpuBuffer, out: &GpuBuffer, t: usize, ar: &Arena) -> Result<()> {
	...
	gpu_gemm_bt(
		cms,
		&m.stream(&layer_name(l, "shexp.gate.weight"))?,
		t,
		nffs,
		ne,
		&ar.g,
	...
	gpu_gemm_bt(
		cms,
		&m.stream(&layer_name(l, "shexp.up.weight"))?,
		t,
		nffs,
		ne,
		&ar.u,
	...
	gpu_gemm_bt(
		&ar.u,
		&m.stream(&layer_name(l, "shexp.down.weight"))?,
		t,
		ne,
		nffs,
		out,
	...
	if m.big.contains_key(&layer_name(l, "shexp.gate_inp.weight")) {
		gpu_gemm_bt(
			cms,
			&m.stream(&layer_name(l, "shexp.gate_inp.weight"))?,
			t,
			1,
			ne,
			&ar.d_bt,
...
fn delta_ffn(m: &Model, l: usize, sp: &Spec, resid: &GpuBuffer, h_out: &GpuBuffer, t: usize, ar: &Arena) -> Result<()> {
	let ne = m.hp.ne;
...
pub(super) fn layer_is_attn(m: &Model, l: usize) -> bool {
	return m
		.big
		.contains_key(&layer_name(l, "self_attn.q_proj.weight"));
}
...
pub(super) fn layer_is_shortconv(m: &Model, l: usize) -> bool {
	return m
		.big
		.contains_key(&layer_name(l, "self_attn.shortconv_in_proj.weight"));
}
...
fn shortconv_mix(
	...
	gpu_gemm_bt(
		&ar.v,
		&m.stream(&layer_name(l, "self_attn.shortconv_out_proj.weight"))?,
		t,
		ne,
		ne,
		out,
