...
fn turn2_h2d_scales_with_suffix_not_total() {
	...
	assert!(
		turn2_h2d * 3 < turn1_h2d,
		"turn-2 H2D {turn2_h2d} B is not a small fraction of turn-1 {turn1_h2d} B: turn-2 work is scaling with total sequence length, not the suffix (prefix K/V not reused)"
	);
}
