...
unsafe extern "C" {
	...
	fn launch_poolx_global_max_pool(
		x: *const c_void,
		out: *mut c_void,
		rows: i32,
		span: i32,
	...
	fn launch_poolx_adaptive_avg_pool(
		x: *const c_void,
		out: *mut c_void,
		rows: i32,
		in_len: i32,
	...
	fn launch_poolx_adaptive_max_pool(
		x: *const c_void,
		out: *mut c_void,
		rows: i32,
		in_len: i32,
...
fn run_poolx_span(
	launch: unsafe extern "C" fn(*const c_void, *mut c_void, i32, i32, i32, *mut c_void),
	x: &[f64],
	rows: usize,
	span: usize,
	ch: usize,
...
fn prove_lp_pool() {
	...
	for r in 0..rows {
		for c in 0..ch {
			let mut acc = 0.0;
			for t in 0..span {
				acc += x[(r * span + t) * ch + c].abs().powf(p);
			}
...
fn prove_adaptive_max_pool() {
	let (rows, in_len, out_len) = (2usize, 7usize, 3usize);
	let x: Vec<f64> = (0..rows * in_len)
		.map(|i| ((i * 29 + 3) % 71) as f64)
		.collect();
