...
unsafe extern "C" {
      fn launch_embeddingx_lookup(table: *const c_void, indices: *const c_void, out: *mut c_void, n: i32, d: i32, s: *mut c_void);
      fn launch_embeddingx_bag(table: *const c_void, indices: *const c_void, offsets: *const c_void, out: *mut c_void, n_bags: i32, d: i32, mode: i32, s: *mut c_void);
      fn launch_embeddingx_one_hot(indices: *const c_void, out: *mut c_void, n: i32, c: i32, s: *mut c_void);
      fn launch_embeddingx_rope(x: *const c_void, cosb: *const c_void, sinb: *const c_void, y: *mut c_void, n: i32, d: i32, s: *mut c_void);
      fn launch_embeddingx_rope_bwd(dy: *const c_void, cosb: *const c_void, sinb: *const c_void, dx: *mut c_void, n: i32, d: i32, s: *mut c_void);
}

const TOL: f64 = 1e-6;

fn chk() { gpu_core::hip::check(unsafe { gpu_core::hip::hipGetLastError() }).unwrap(); }
...
fn gpu_lookup(table: &[f64], indices: &[i32], n: usize, d: usize) -> Vec<f64> {
      ...
      unsafe { launch_embeddingx_lookup(bt.ptr_raw() as *const c_void, bi.ptr_raw() as *const c_void, o.ptr_raw(), n as i32, d as i32, std::ptr::null_mut()); }
      chk();
...
fn gpu_bag(table: &[f64], indices: &[i32], offsets: &[i32], n_bags: usize, d: usize, mode: i32) -> Vec<f64> {
      ...
      unsafe { launch_embeddingx_bag(bt.ptr_raw() as *const c_void, bi.ptr_raw() as *const c_void, bo.ptr_raw() as *const c_void, o.ptr_raw(), n_bags as i32, d as i32, mode, std::ptr::null_mut()); }
      chk();
...
fn gpu_one_hot(indices: &[i32], n: usize, c: usize) -> Vec<f64> {
      ...
      unsafe { launch_embeddingx_one_hot(bi.ptr_raw() as *const c_void, o.ptr_raw(), n as i32, c as i32, std::ptr::null_mut()); }
      chk();
...
fn gpu_rope(x: &[f64], cosb: &[f64], sinb: &[f64], n: usize, d: usize) -> Vec<f64> {
      ...
      let o = GpuBuffer::alloc(n * d).unwrap();
      unsafe { launch_embeddingx_rope(bx.ptr_raw() as *const c_void, bc.ptr_raw() as *const c_void, bs.ptr_raw() as *const c_void, o.ptr_raw(), n as i32, d as i32, std::ptr::null_mut()); }
      chk();
      let mut out = vec![0.0; n * d];
      o.download(&mut out).unwrap();
      out
...
fn gpu_rope_bwd(dy: &[f64], cosb: &[f64], sinb: &[f64], n: usize, d: usize) -> Vec<f64> {
      ...
      let bs = GpuBuffer::upload(sinb).unwrap();
      let o = GpuBuffer::alloc(n * d).unwrap();
      unsafe { launch_embeddingx_rope_bwd(bd.ptr_raw() as *const c_void, bc.ptr_raw() as *const c_void, bs.ptr_raw() as *const c_void, o.ptr_raw(), n as i32, d as i32, std::ptr::null_mut()); }
      chk();
      let mut out = vec![0.0; n * d];
      o.download(&mut out).unwrap();
      out
...
fn prove_lookup() -> bool {
      ...
      for (i, &row) in indices.iter().enumerate() {
            for j in 0..d { want[i * d + j] = table[row as usize * d + j]; }
      }
...
fn prove_bag() -> bool {
      ...
      for &mode in &[0i32, 1i32] {
            ...
            for b in 0..n_bags {
                  ...
                  for j in 0..d {
                        let mut acc = 0.0;
                        for k in s..e { acc += table[indices[k] as usize * d + j]; }
                        if mode == 1 && e > s { acc /= (e - s) as f64; }
                        want[b * d + j] = acc;
                  }
            }
            ok &= close(&got, &want);
...
fn prove_one_hot() -> bool {
      ...
      for (i, &cl) in indices.iter().enumerate() { want[i * c + cl as usize] = 1.0; }
      got == want
}
...
fn prove_embedding_lookup_bwd() -> bool {
      ...
      for (i, &row) in indices.iter().enumerate() {
            for j in 0..cols { want[row as usize * cols + j] += grad_out[i * cols + j]; }
      }
      got.iter().zip(&want).all(|(a, b)| (a - b).abs() <= 1e-5 * (1.0 + b.abs()))
}
...
fn prove_rope_bwd() -> bool {
      ...
      for k in 0..n * d {
            ...
            let mut acc = 0.0;
            for m in 0..n * d { acc += dy[m] * (yp[m] - y0[m]) / h; }
            dx_fd[k] = acc;
      }
      // FD carries O(h) error; loosen tolerance for this op only.
      dx.iter().zip(&dx_fd).all(|(a, b)| (a - b).abs() <= 1e-4 * (1.0 + b.abs()))
}
...
fn canon(name: &str) -> String {
      ...
      let is_bwd = lname.contains("backward") || lname.contains("_bwd") || lname.ends_with("_grad") || lname.contains("grad");

      // RoPE / rotary family
      let is_rope = lname.contains("rope") || lname.contains("rotary");
      if is_rope {
            return if is_bwd { "rope_bwd".to_string() } else { "rope".to_string() };
      }

      // embedding_bag family (pooled). torch._embedding_bag*, F.embedding_bag
      if lname.contains("embedding_bag") || lname.contains("_bag") {
            return if is_bwd { "embedding_bag_bwd".to_string() } else { "embedding_bag".to_string() };
      }

      // one_hot
      if lname.contains("one_hot") { return "one_hot".to_string(); }
      ...
      if (lname.contains("embedding_backward") || lname.contains("embedding_dense_backward"))
            && !lname.contains("sparse") && !lname.contains("_bag") {
      ...
      lname.rsplit(['.', ':', '$']).next().unwrap_or(&lname).to_string()
}
...
fn prove_embedding() {
      ...
      let failures: Vec<&str> = op_ok.iter().filter(|&(_, &v)| !v).map(|(k, _)| *k).collect();
      ...
      eprintln!("\n=== PROVE embedding ===");
      for (k, v) in &op_ok { eprintln!("  op {:<20} {}", k, if *v { "PROVEN" } else { "FAIL" }); }
      ...
      assert!(failures.is_empty(), "registered embedding op(s) FAILED oracle: {:?}", failures);
