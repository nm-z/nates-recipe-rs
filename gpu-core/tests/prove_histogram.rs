...
unsafe extern "C" {
      fn launch_histogramx_histc(
            x: *const c_void, counts: *mut c_void, n: i32, lo: f64, hi: f64, bins: i32, s: *mut c_void);
      fn launch_histogramx_bincount(
            labels: *const c_void, counts: *mut c_void, n: i32, out_len: i32, s: *mut c_void);
}
...
fn gpu_histc(x: &[f64], lo: f64, hi: f64, bins: usize) -> Vec<i32> {
      ...
      unsafe {
            launch_histogramx_histc(
                  bx.ptr_raw() as *const c_void, counts.ptr_raw(),
                  x.len() as i32, lo, hi, bins as i32, std::ptr::null_mut());
      }
...
fn gpu_bincount(labels: &[i32], out_len: usize) -> Vec<i32> {
      ...
      unsafe {
            launch_histogramx_bincount(
                  bl.ptr_raw() as *const c_void, counts.ptr_raw(),
                  labels.len() as i32, out_len as i32, std::ptr::null_mut());
      }
      gpu_core::hip::check(unsafe { gpu_core::hip::hipGetLastError() }).unwrap();
...
fn cpu_histc(x: &[f64], lo: f64, hi: f64, bins: usize) -> Vec<i32> {
      ...
      for &v in x {
            ...
            if b == bins as isize { b = bins as isize - 1; }
            if b < 0 { b = 0; }
            if b >= bins as isize { b = bins as isize - 1; }
            c[b as usize] += 1;
      }
...
fn cpu_bincount(labels: &[i32], out_len: usize) -> Vec<i32> {
      ...
      for &l in labels {
            if l >= 0 && (l as usize) < out_len { c[l as usize] += 1; }
      }
      c
}
...
fn prove_histc() -> bool {
      ...
      let exact = got.iter().zip(&want).all(|(g, w)| ((*g as f64) - (*w as f64)).abs() <= TOL);
...
fn prove_bincount() -> bool {
      ...
      let exact = got == want;
      let in_range = labels.iter().filter(|&&l| l >= 0 && (l as usize) < out_len).count() as i32;
      let conserved = got.iter().sum::<i32>() == in_range;
      exact && conserved
}
...
fn prove_histogram() {
      ...
      for name in &items {
            ...
            match op_ok.get(key.as_str()) {
                  Some(&ok) if ok => { proven += 1; proven_keys.insert(key); }
                  _ => { backlog.insert(canon(name)); }
            }
      }

      eprintln!("\n=== PROVE histogram ===");
      eprintln!("histc proven: {}   bincount proven: {}", histc_ok, bincount_ok);
      eprintln!("PROVE histogram: {} / {}", proven, total);
      eprintln!("proven canonical ops ({}): {}",
            proven_keys.len(), proven_keys.iter().cloned().collect::<Vec<_>>().join(", "));
      eprintln!("backlog canonical ops ({}): {}",
            backlog.len(), backlog.iter().cloned().collect::<Vec<_>>().join(", "));

      assert!(histc_ok, "histc kernel FAILED oracle match / count conservation");
      assert!(bincount_ok, "bincount kernel FAILED oracle match / count conservation");
