...
fn unary_registry() -> HashMap<&'static str, UnaryOp> {
      use gpu_core::k_mathx::{gpu_erf, gpu_erfc, gpu_tgamma, gpu_lgamma};
      let mut m: HashMap<&'static str, UnaryOp> = HashMap::new();
      macro_rules! sx { ($k:literal, $launch:expr, $o:expr, $lo:expr, $hi:expr) => {
            m.insert($k, UnaryOp { run: Box::new(|x| run_specialx($launch, x)), oracle: Box::new($o), lo: $lo, hi: $hi });
      }; }
      ...
      km!("erf",    gpu_erf,    |x| libm::erf(x),    -3.0, 3.0);
      km!("erfc",   gpu_erfc,   |x| libm::erfc(x),   -3.0, 3.0);
      km!("tgamma", gpu_tgamma, |x| libm::tgamma(x),  0.2, 4.0);
      km!("lgamma", gpu_lgamma, |x| libm::lgamma(x),  0.2, 5.0);

      // ── new specialx_ ops ──
      sx!("expit",  launch_specialx_expit, |x| 1.0 / (1.0 + (-x).exp()), -6.0, 6.0);
      sx!("logit",  launch_specialx_logit, |x| (x / (1.0 - x)).ln(), 0.02, 0.98);
      sx!("sinc",   launch_specialx_sinc, |x: f64| if x == 0.0 { 1.0 } else { (std::f64::consts::PI * x).sin() / (std::f64::consts::PI * x) }, -3.0, 3.0);
      sx!("entr",   launch_specialx_entr, |x: f64| if x > 0.0 { -x * x.ln() } else if x == 0.0 { 0.0 } else { f64::NEG_INFINITY }, 0.05, 3.0);
      ...
      sx!("erfcx",  launch_specialx_erfcx, |x: f64| (x * x).exp() * libm::erfc(x), -3.0, 3.0);
      // i0/i1 textbook power series
      sx!("i0",     launch_specialx_i0, |x| bessel_i0_series(x), -5.0, 5.0);
      sx!("i1",     launch_specialx_i1, |x| bessel_i1_series(x), -5.0, 5.0);
      sx!("i0e",    launch_specialx_i0e, |x: f64| (-x.abs()).exp() * bessel_i0_series(x), -5.0, 5.0);
      sx!("i1e",    launch_specialx_i1e, |x: f64| (-x.abs()).exp() * bessel_i1_series(x), -5.0, 5.0);
      sx!("j0",     launch_specialx_j0, |x| libm::j0(x), -8.0, 8.0);
      sx!("j1",     launch_specialx_j1, |x| libm::j1(x), -8.0, 8.0);
      sx!("y0",     launch_specialx_y0, |x| libm::y0(x), 0.5, 8.0);
      sx!("y1",     launch_specialx_y1, |x| libm::y1(x), 0.5, 8.0);
      // ndtr = standard normal CDF = 0.5*erfc(-x/sqrt2) ; ndtri = inverse (round-trip)
      sx!("ndtr",   launch_specialx_ndtr, |x| 0.5 * libm::erfc(-x / std::f64::consts::SQRT_2), -3.0, 3.0);
      sx!("ndtri",  launch_specialx_ndtri, |x| x, 0.02, 0.98); // round-trip handled specially below
      sx!("sinpi",  launch_specialx_sinpi, |x| (std::f64::consts::PI * x).sin(), -2.0, 2.0);
      // digamma oracle = central finite-diff of lgamma (digamma == d/dx ln Gamma)
      sx!("digamma", launch_specialx_digamma, |x| { let h = 1e-5; (libm::lgamma(x + h) - libm::lgamma(x - h)) / (2.0 * h) }, 1.0, 6.0);
      m
}
...
fn run_xlogy(x: &[f64], y: &[f64]) -> Vec<f64> {
      ...
      let o = GpuBuffer::alloc(x.len()).unwrap();
      unsafe { launch_specialx_xlogy(bx.ptr_raw() as *const c_void, by.ptr_raw() as *const c_void, o.ptr_raw(), x.len() as i32, std::ptr::null_mut()); }
      gpu_core::hip::check(unsafe { gpu_core::hip::hipGetLastError() }).unwrap();
      let mut out = vec![0.0; x.len()];
      o.download(&mut out).unwrap();
      out
...
fn canon(name: &str) -> String {
      ...
      let base = base
            .strip_suffix("_2").map(|s| s.to_string())
            .or_else(|| base.strip_suffix("_alias").map(|s| s.to_string()))
            .or_else(|| base.strip_suffix("_aten").map(|s| s.to_string()))
            .unwrap_or(base);
      let alias: &[(&str, &str)] = &[
            // sigmoid family
            ("expit", "expit"), ("sigmoid", "expit"),
            // digamma / psi
            ("digamma", "digamma"), ("psi", "digamma"), ("hpsi", "digamma"),
            // gamma family
            ("gamma", "tgamma"), ("gammaln", "lgamma"), ("lngamma", "lgamma"),
            // error functions
            ("erf", "erf"), ("erfc", "erfc"), ("erfcx", "erfcx"), ("erfinv", "erfinv"),
            // modified bessel I
            ("i0", "i0"), ("i1", "i1"), ("i0e", "i0e"), ("i1e", "i1e"),
            ("modified_bessel_i0", "i0"), ("modified_bessel_i1", "i1"),
            // bessel J / Y
            ("bessel_j0", "j0"), ("bessel_j1", "j1"), ("bessel_y0", "y0"), ("bessel_y1", "y1"),
            ("j0", "j0"), ("j1", "j1"), ("y0", "y0"), ("y1", "y1"),
            // normal CDF / inverse
            ("ndtr", "ndtr"), ("ndtri", "ndtri"),
            // others
            ("logit", "logit"), ("sinc", "sinc"), ("entr", "entr"),
            ("xlogy", "xlogy"), ("sinpi", "sinpi"),
      ];
...
fn check_special_op(key: &str, reg: &UnaryOp) -> bool {
      match key {
            ...
            "erfinv" => {
                  ...
                  let got = (reg.run)(&xs);
                  xs.iter().zip(&got).all(|(x, g)| (libm::erf(*g) - x).abs() <= TOL * (1.0 + x.abs()))
            }
...
fn prove_special() {
      ...
      // entr(0)=0, sinc(0)=1, xlogy(0,y)=0.
      {
            ...
            if e[0] != 0.0 { failures.push(format!("entr(0)={} != 0", e[0])); }
            let s = run_specialx(launch_specialx_sinc, &[0.0]);
            if (s[0] - 1.0).abs() > TOL { failures.push(format!("sinc(0)={} != 1", s[0])); }
            let xl = run_xlogy(&[0.0, 2.0, -1.0], &[5.0, 3.0, 4.0]);
            let want = [0.0, 2.0 * 3.0_f64.ln(), -1.0 * 4.0_f64.ln()];
            if xl.iter().zip(&want).any(|(g, w)| (g - w).abs() > TOL * (1.0 + w.abs())) {
      ...
      assert!(failures.is_empty(), "registered special op(s) FAILED oracle: {:?}", failures);
