use gpu_core::tiered::{Tiered, Budgets, Full, P, human};
use std::ffi::c_void;
use std::path::Path;
...
fn tiled_full_batch_runs_and_matches_whole() {
      ...
      let dl = |buf: &GpuBuffer, m: usize| -> Vec<f64> {
            ...
            unsafe {
                  gpu_core::memory::xfer(h.as_mut_ptr() as *mut c_void, buf.ptr_raw(), m * 8, gpu_core::hip::HIP_MEMCPY_D2H, std::ptr::null_mut())
                        .expect("D2H");
            }
      ...
      for _ in 0..epochs {
            ...
            while r0 < n {
                  ...
                  let yblk = GpuBuffer::borrow(unsafe { (y_dev.ptr_raw() as *mut f64).add(r0 * o) as *mut c_void }, r * o * 8);
