...
use gpu_core::nn_f32::{
      gpu_linear_f32, gpu_relu_f32, gpu_relu_backward_f32,
      gpu_gelu_f32, gpu_gelu_backward_f32,
      gpu_layernorm_f32, gpu_layernorm_backward_f32,
      gpu_bias_add_f32,
      gpu_avg_pool_2d_f32, gpu_max_pool_2d_f32,
      gpu_lstm_cell_f32, gpu_gru_cell_f32,
      gpu_relu_f16, gpu_gelu_f16, gpu_add_f16, gpu_mul_f16,
      gpu_sgd_update_f32,
};
use gpu_core::attention::{
      gpu_scaled_dot_product_attention,
      gpu_causal_softmax_rows,
      gpu_mha_split, gpu_mha_merge,
      gpu_rope,
      gpu_positional_encoding,
      gpu_rmsnorm, gpu_rmsnorm_backward,
      gpu_im2col_2d_ext,
...
fn assert_close(got: &[f32], expected: &[f32], label: &str) {
      ...
      for (i, (&g, &e)) in got.iter().zip(expected.iter()).enumerate() {
            assert!(g.is_finite(), "{}: got[{}] = {} is not finite (expected {})", label, i, g, e);
            let err = abs_err(g, e);
            let t = tol(e);
            assert!(
                  err <= t,
...
fn test_gelu_backward_f32() {
      ...
      for &v in &got { assert!(v.is_finite(), "gelu_backward non-finite: {}", v); }
}
...
fn test_layernorm_backward_f32() {
      ...
      let (grad_x, grad_gamma, grad_beta) = gpu_layernorm_backward_f32(&grad_y, &x, &gamma, rows, cols, eps).unwrap();
      sync();
      ...
      for row in 0..rows {
            ...
            assert!(sum.abs() < 1e-3, "layernorm_backward grad_x row {} sum = {} (should be ~0)", row, sum);
      }
      // grad_beta = sum of grad_y over rows = [2,2,2,2]
      for &v in &gb { assert!(v.is_finite(), "grad_beta non-finite"); }
      // grad_gamma should be finite
      for &v in &gg { assert!(v.is_finite(), "grad_gamma non-finite"); }
}
...
fn test_gelu_f16() {
      ...
      for (i, (&g, &e)) in got.iter().zip(expected.iter()).enumerate() {
            ...
            assert!((g - e).abs() <= 1e-2 * (1.0 + e.abs()),
                  "gelu_f16[{}]: got={} expected={}", i, g, e);
      }
}
...
fn test_sdpa_noncausal() {
      ...
      let expected: Vec<f32> = vec![1.6604769, 2.6604769, 2.3395231, 3.3395231];
...
fn test_sdpa_causal() {
      ...
      let expected: Vec<f32> = vec![1.0, 2.0, 2.3395231, 3.3395231];
...
fn test_causal_softmax_rows() {
      ...
      assert!((got[0] - 1.0).abs() < 1e-4, "row0[0] should be 1.0 got {}", got[0]);
      assert!((got[1]).abs() < 1e-4, "row0[1] should be 0.0 got {}", got[1]);
      assert!((got[2]).abs() < 1e-4, "row0[2] should be 0.0 got {}", got[2]);
      // Row 1: j=0,1 → prob=[0.5, 0.5, 0]
      assert!((got[3] - 0.5).abs() < 1e-4, "row1[0] should be 0.5 got {}", got[3]);
      assert!((got[4] - 0.5).abs() < 1e-4, "row1[1] should be 0.5 got {}", got[4]);
      assert!((got[5]).abs() < 1e-4, "row1[2] should be 0.0 got {}", got[5]);
      // Row 2: all three → prob=[1/3, 1/3, 1/3]
      for j in 6..9 {
            assert!((got[j] - 1.0/3.0).abs() < 1e-4, "row2[{}] should be 1/3 got {}", j-6, got[j]);
      }
}
...
fn test_rope_norm_preserved() {
      ...
      let seq = 3usize;
      let dim = 4usize;
      let data: Vec<f32> = vec![1.0, 0.0, 0.0, 1.0,  // s=0
                                  2.0, 1.0, -1.0, 2.0,  // s=1
                                  0.0, 3.0, 1.0, 0.0];  // s=2
      ...
      for s in 0..seq {
            for d in 0..half {
                  ...
                  assert!((norm_x - norm_y).abs() < 1e-4 * (1.0 + norm_x),
                        "rope s={} d={}: norm_x={} norm_y={}", s, d, norm_x, norm_y);
            }
      }
}
...
fn test_positional_encoding() {
      ...
      for d in (0..dim).step_by(2) {
            assert!((got[d]).abs() < 1e-5, "pos_enc[0,{}] (even, should be sin(0)=0) got {}", d, got[d]);
      }
      // pe[0, odd] = cos(0 / ...) = 1
      for d in (1..dim).step_by(2) {
            assert!((got[d] - 1.0).abs() < 1e-5, "pos_enc[0,{}] (odd, should be cos(0)=1) got {}", d, got[d]);
      }
}
...
fn test_rmsnorm_backward() {
      ...
      let (grad_x, grad_gamma) = gpu_rmsnorm_backward(&grad_out, &x, &gamma, rows, cols, eps).unwrap();
      sync();
      ...
      for &v in &gx { assert!(v.is_finite(), "rmsnorm grad_x non-finite"); }
      for &v in &gg { assert!(v.is_finite(), "rmsnorm grad_gamma non-finite"); }
...
fn test_im2col_2d_ext() {
      ...
      // out_w = 3
      let n = 1usize; let c = 1usize; let h = 4usize; let w = 4usize;
      let kh = 2usize; let kw = 2usize;
      let sh = 2usize; let sw = 2usize;
      let pad_h = 1usize; let pad_w = 1usize;
      let dil_h = 1usize; let dil_w = 1usize;
      ...
      let x = GpuBuffer::upload_f32(&input).unwrap();
      let patches = gpu_im2col_2d_ext(&x, n, c, h, w, kh, kw, sh, sw, pad_h, pad_w, dil_h, dil_w).unwrap();
      sync();
...
fn test_embedding_backward() {
      ...
      let n = 3usize; let cols = 2usize; let vocab = 2usize;
