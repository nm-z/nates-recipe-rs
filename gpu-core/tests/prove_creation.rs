...
fn close(a: &[f64], b: &[f64]) -> bool {
	a.len() == b.len()
		&& a.iter()
			.zip(b)
			.all(|(g, w)| w.is_finite() && (g - w).abs() <= TOL * (1.0 + w.abs()))
}
...
fn run_proofs() -> (HashMap<&'static str, bool>, Vec<String>) {
	...
	// linspace: endpoint-INCLUSIVE. assert out[n-1]==stop and out[0]==start; n==1 edge.
	{
		...
		prove!(
			"linspace",
			close(&got, &want) && edge_first && edge_last && edge_one
		);
	}
	...
	// geomspace: out[i] = start*(stop/start)^(i/(n-1)). assert endpoints exact.
	{
		...
		let want: Vec<f64> = (0..n)
			.map(|i| start * (stop / start).powf(i as f64 / (n - 1) as f64))
			.collect();
...
fn prove_creation() {
	...
	assert!(
		failures.is_empty(),
		"registered creation op(s) FAILED oracle: {:?}",
		failures
	);
