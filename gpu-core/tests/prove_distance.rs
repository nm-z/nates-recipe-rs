...
unsafe extern "C" {
      fn launch_distancex_manhattan(q: *const c_void, t: *const c_void, o: *mut c_void, nq: i32, nt: i32, dim: i32, s: *mut c_void);
      fn launch_distancex_chebyshev(q: *const c_void, t: *const c_void, o: *mut c_void, nq: i32, nt: i32, dim: i32, s: *mut c_void);
      fn launch_distancex_minkowski(q: *const c_void, t: *const c_void, o: *mut c_void, nq: i32, nt: i32, dim: i32, p: f64, s: *mut c_void);
      fn launch_distancex_braycurtis(q: *const c_void, t: *const c_void, o: *mut c_void, nq: i32, nt: i32, dim: i32, s: *mut c_void);
      fn launch_distancex_canberra(q: *const c_void, t: *const c_void, o: *mut c_void, nq: i32, nt: i32, dim: i32, s: *mut c_void);
}
...
fn make_data() -> (Vec<f64>, Vec<f64>, usize, usize, usize) {
      ...
      for (i, v) in q.iter_mut().enumerate() { *v = 0.5 + 4.5 * (((i * 7 + 3) % 11) as f64) / 10.0; }
      for (i, v) in t.iter_mut().enumerate() { *v = 0.5 + 4.5 * (((i * 5 + 1) % 11) as f64) / 10.0; }
      (q, t, nq, nt, dim)
}
...
fn run_flat_f64(f: FlatLaunch, q: &[f64], t: &[f64], nq: usize, nt: usize, dim: usize) -> Vec<f64> {
      ...
      unsafe { f(bq.ptr_raw() as *const c_void, bt.ptr_raw() as *const c_void, o.ptr_raw(), nq as i32, nt as i32, dim as i32, std::ptr::null_mut()); }
...
fn close(a: &[f64], b: &[f64]) -> Option<(usize, f64, f64)> {
      for (i, (x, y)) in a.iter().zip(b).enumerate() {
            if (x - y).abs() > TOL * (1.0 + y.abs()) { return Some((i, *x, *y)); }
      }
      None
}
...
fn canon(name: &str) -> Option<&'static str> {
      ...
      let base = base.strip_suffix("_aten").map(|s| s.to_string()).unwrap_or(base);
      ...
      match base.as_str() {
            ...
            "cdist" | "_cdist_forward" | "cdist_forward" | "pdist" | "_pdist_forward"
            | "pdist_forward" | "native_pdist" | "pairwise_distance" | "pairwise_distances"
            | "pairwise" | "pairwise_point_distance" => return Some("euclidean"),
            _ => {}
      }
      None
}
      ...
      for e in std::fs::read_dir(&dir).expect("no kernel_inventory").flatten() {
            ...
            if p.extension().map_or(false, |x| x == "json") {
                  ...
                  if let Some(ks) = v.get("kernels").and_then(|k| k.as_array()) {
                        for k in ks {
                              if k.get("category").and_then(|c| c.as_str()) != Some("distance") { continue; }
...
fn prove_distance() {
      ...
      {
            let got = gpu_core::kernels::gpu_pairwise_l2(&GpuBuffer::upload(&q).unwrap(), &GpuBuffer::upload(&t).unwrap(), nq, nt, dim).unwrap().download_vec().unwrap();
            let want = oracle_grid(&q, &t, nq, nt, dim, |a, b| a.iter().zip(b).map(|(x, y)| (x - y) * (x - y)).sum());
            assert_op("l2_squared", got, want, &mut failures);
      }
      // pairwise_cosine → cosine SIMILARITY
      {
            let got = gpu_core::encoding::gpu_pairwise_cosine(&GpuBuffer::upload(&q).unwrap(), &GpuBuffer::upload(&t).unwrap(), nq, nt, dim).unwrap().download_vec().unwrap();
      ...
      // pairwise_l1 → L1
      {
            let got = gpu_core::encoding::gpu_pairwise_l1(&GpuBuffer::upload(&q).unwrap(), &GpuBuffer::upload(&t).unwrap(), nq, nt, dim).unwrap().download_vec().unwrap();
            let want = oracle_grid(&q, &t, nq, nt, dim, |a, b| a.iter().zip(b).map(|(x, y)| (x - y).abs()).sum());
      ...
      {
            let qd = dim;
            let nqh = 5usize; let nth = 6usize;
            let mut qu = vec![0u8; nqh * qd];
            let mut tu = vec![0u8; nth * qd];
            for (i, v) in qu.iter_mut().enumerate() { *v = ((i * 3 + 1) % 4) as u8; }
            for (i, v) in tu.iter_mut().enumerate() { *v = ((i * 2 + 2) % 4) as u8; }
            let got = gpu_core::encoding::gpu_pairwise_hamming(&GpuBuffer::upload_u8(&qu).unwrap(), &GpuBuffer::upload_u8(&tu).unwrap(), nqh, nth, qd).unwrap().download_vec().unwrap();
            let mut want = vec![0.0; nqh * nth];
            for qi in 0..nqh {
                  for ti in 0..nth {
                        let mut m = 0usize;
                        for d in 0..qd { if qu[qi * qd + d] != tu[ti * qd + d] { m += 1; } }
      ...
      // chebyshev: Linf
      {
            ...
            let want = oracle_grid(&q, &t, nq, nt, dim, |a, b| a.iter().zip(b).map(|(x, y)| (x - y).abs()).fold(0.0_f64, f64::max));
      ...
      // minkowski(p=3): (Σ|diff|^p)^(1/p)
      {
            ...
            let bt = GpuBuffer::upload(&t).unwrap();
            let o = GpuBuffer::alloc(nq * nt).unwrap();
            unsafe { launch_distancex_minkowski(bq.ptr_raw() as *const c_void, bt.ptr_raw() as *const c_void, o.ptr_raw(), nq as i32, nt as i32, dim as i32, p, std::ptr::null_mut()); }
            gpu_core::hip::check(unsafe { gpu_core::hip::hipGetLastError() }).unwrap();
      ...
      // minkowski(p=2) cross-check == sqrt(l2-squared kernel output)
      {
            ...
            let want = oracle_grid(&q, &t, nq, nt, dim, |a, b| a.iter().zip(b).map(|(x, y)| (x - y) * (x - y)).sum::<f64>().sqrt());
            assert_op("euclidean", mink2.clone(), want, &mut failures);
            // bonus cross-check: GPU minkowski(p=2) == sqrt(GPU squared-L2 kernel)
            let l2sq = gpu_core::kernels::gpu_pairwise_l2(&GpuBuffer::upload(&q).unwrap(), &GpuBuffer::upload(&t).unwrap(), nq, nt, dim).unwrap().download_vec().unwrap();
            let l2root: Vec<f64> = l2sq.iter().map(|v| v.sqrt()).collect();
            if close(&mink2, &l2root).is_some() { failures.push("minkowski(p=2) != sqrt(l2_squared)".into()); }
      }
      ...
      // defining-edge probes: canberra(0,0)-term → 0 ; braycurtis all-zero pair → 0.
      {
            ...
            if c[0] != 0.0 { failures.push(format!("canberra(0,0)={} != 0", c[0])); }
            let bc = run_flat_f64(launch_distancex_braycurtis, &zq, &zt, 1, 1, dim);
            if bc[0] != 0.0 { failures.push(format!("braycurtis(0,0)={} != 0", bc[0])); }
      }
      ...
      let implemented = ["braycurtis", "canberra", "chebyshev", "manhattan", "minkowski"];
      eprintln!("\n=== PROVE distance ===");
      ...
      eprintln!("new kernels implemented ({}): {}", implemented.len(), implemented.join(", "));
      eprintln!("proven canonical ops ({}): {}", proven_keys.len(), proven_keys.iter().copied().collect::<Vec<_>>().join(", "));

      assert!(failures.is_empty(), "registered distance op(s) FAILED oracle: {:?}", failures);
