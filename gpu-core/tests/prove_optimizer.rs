...
unsafe extern "C" {
      fn launch_optimizerx_nesterov(w: *mut c_void, buf: *mut c_void, g: *const c_void, lr: f64, momentum: f64, n: i32, s: *mut c_void);
      fn launch_optimizerx_adadelta(w: *mut c_void, eg: *mut c_void, edx: *mut c_void, g: *const c_void, lr: f64, rho: f64, eps: f64, n: i32, s: *mut c_void);
      fn launch_optimizerx_radam(w: *mut c_void, m: *mut c_void, v: *mut c_void, g: *const c_void, lr: f64, b1: f64, b2: f64, eps: f64, t: i32, n: i32, s: *mut c_void);
      fn launch_optimizerx_lars_phase1(w: *const c_void, g: *const c_void, n: i32, w_norm_sq: *mut c_void, g_norm_sq: *mut c_void, s: *mut c_void);
      fn launch_optimizerx_lars_phase2(w: *mut c_void, buf: *mut c_void, g: *const c_void, lr: f64, momentum: f64, wd: f64, lambda: f64, w_norm_sq: f64, g_norm_sq: f64, eps: f64, n: i32, s: *mut c_void);
}
...
fn wv(n: usize) -> Vec<f64> { (0..n).map(|i| 0.30 - 0.11 * i as f64 + 0.017 * (i * i) as f64).collect() }
fn gv(n: usize) -> Vec<f64> { (0..n).map(|i| -0.20 + 0.13 * i as f64 - 0.009 * (i * i) as f64).collect() }
fn sv(n: usize, base: f64, step: f64) -> Vec<f64> { (0..n).map(|i| base + step * i as f64).collect() }
...
fn prove_momentum() -> bool {
      ...
      let w = wv(n); let g = gv(n); let v0 = sv(n, 0.4, -0.02);
      let bw = GpuBuffer::upload(&w).unwrap();
      let bv = GpuBuffer::upload(&v0).unwrap();
      let bg = GpuBuffer::upload(&g).unwrap();
      gpu_core::optimizers::gpu_momentum_update(&bw, &bv, &bg, lr, mu, n);
      let mut gw = vec![0.0; n]; bw.download(&mut gw).unwrap();
      let mut gvb = vec![0.0; n]; bv.download(&mut gvb).unwrap();
...
fn prove_rmsprop() -> bool {
      ...
      let w = wv(n); let g = gv(n); let c0 = sv(n, 0.5, 0.01);
      let bw = GpuBuffer::upload(&w).unwrap();
      let bc = GpuBuffer::upload(&c0).unwrap();
      let bg = GpuBuffer::upload(&g).unwrap();
      gpu_core::optimizers::gpu_rmsprop_update(&bw, &bc, &bg, lr, decay, eps, n);
      let mut gw = vec![0.0; n]; bw.download(&mut gw).unwrap();
      let mut gc = vec![0.0; n]; bc.download(&mut gc).unwrap();
      let want_c: Vec<f64> = (0..n).map(|i| decay * c0[i] + (1.0 - decay) * g[i] * g[i]).collect();
      let want_w: Vec<f64> = (0..n).map(|i| w[i] - lr * g[i] / (want_c[i].sqrt() + eps)).collect();
...
fn prove_adagrad() -> bool {
      ...
      let w = wv(n); let g = gv(n); let a0 = sv(n, 0.3, 0.02);
      let bw = GpuBuffer::upload(&w).unwrap();
      let ba = GpuBuffer::upload(&a0).unwrap();
      let bg = GpuBuffer::upload(&g).unwrap();
      gpu_core::optimizers::gpu_adagrad_update(&bw, &ba, &bg, lr, eps, n);
      let mut gw = vec![0.0; n]; bw.download(&mut gw).unwrap();
      let mut ga = vec![0.0; n]; ba.download(&mut ga).unwrap();
      let want_a: Vec<f64> = (0..n).map(|i| a0[i] + g[i] * g[i]).collect();
      let want_w: Vec<f64> = (0..n).map(|i| w[i] - lr * g[i] / (want_a[i].sqrt() + eps)).collect();
...
fn prove_lion() -> bool {
      ...
      let w = wv(n); let g = gv(n); let m0 = sv(n, -0.15, 0.02);
      let bw = GpuBuffer::upload(&w).unwrap();
      let bm = GpuBuffer::upload(&m0).unwrap();
      let bg = GpuBuffer::upload(&g).unwrap();
      gpu_core::optimizers::gpu_lion_update(&bw, &bm, &bg, lr, b1, b2, wd, n);
      let mut gw = vec![0.0; n]; bw.download(&mut gw).unwrap();
      let mut gm = vec![0.0; n]; bm.download(&mut gm).unwrap();
      let sign = |x: f64| if x > 0.0 { 1.0 } else if x < 0.0 { -1.0 } else { 0.0 };
      let want_w: Vec<f64> = (0..n).map(|i| { let u = sign(b1 * m0[i] + (1.0 - b1) * g[i]); w[i] - lr * (u + wd * w[i]) }).collect();
...
fn prove_adam() -> bool {
      ...
      let w = wv(n); let g = gv(n); let m0 = sv(n, 0.05, 0.01); let v0 = sv(n, 0.2, 0.005);
      let bw = GpuBuffer::upload(&w).unwrap();
      let bm = GpuBuffer::upload(&m0).unwrap();
      let bv = GpuBuffer::upload(&v0).unwrap();
      let bg = GpuBuffer::upload(&g).unwrap();
      ...
      let mut gw = vec![0.0; n]; bw.download(&mut gw).unwrap();
      let bc1 = 1.0 - b1.powi(t as i32); let bc2 = 1.0 - b2.powi(t as i32);
...
fn prove_nesterov() -> bool {
      let (lr, mu, n) = (0.05, 0.9, N);
      let w = wv(n); let g = gv(n); let b0 = sv(n, 0.1, 0.01);
      let bw = GpuBuffer::upload(&w).unwrap();
      let bb = GpuBuffer::upload(&b0).unwrap();
      let bg = GpuBuffer::upload(&g).unwrap();
      unsafe { launch_optimizerx_nesterov(bw.ptr_raw(), bb.ptr_raw(), bg.ptr_raw() as *const c_void, lr, mu, n as i32, std::ptr::null_mut()); }
      gpu_core::hip::check(unsafe { gpu_core::hip::hipGetLastError() }).unwrap();
      let mut gw = vec![0.0; n]; bw.download(&mut gw).unwrap();
      let mut gb = vec![0.0; n]; bb.download(&mut gb).unwrap();
      let want_b: Vec<f64> = (0..n).map(|i| mu * b0[i] + g[i]).collect();
      let want_w: Vec<f64> = (0..n).map(|i| w[i] - lr * (g[i] + mu * want_b[i])).collect();
...
fn prove_adadelta() -> bool {
      ...
      let w = wv(n); let g = gv(n); let eg0 = sv(n, 0.3, 0.01); let edx0 = sv(n, 0.15, 0.008);
      let bw = GpuBuffer::upload(&w).unwrap();
      let beg = GpuBuffer::upload(&eg0).unwrap();
      let bedx = GpuBuffer::upload(&edx0).unwrap();
      let bg = GpuBuffer::upload(&g).unwrap();
      unsafe { launch_optimizerx_adadelta(bw.ptr_raw(), beg.ptr_raw(), bedx.ptr_raw(), bg.ptr_raw() as *const c_void, lr, rho, eps, n as i32, std::ptr::null_mut()); }
      gpu_core::hip::check(unsafe { gpu_core::hip::hipGetLastError() }).unwrap();
      let mut gw = vec![0.0; n]; bw.download(&mut gw).unwrap();
      let mut geg = vec![0.0; n]; beg.download(&mut geg).unwrap();
      let mut gedx = vec![0.0; n]; bedx.download(&mut gedx).unwrap();
      let want_eg: Vec<f64> = (0..n).map(|i| rho * eg0[i] + (1.0 - rho) * g[i] * g[i]).collect();
      let dx: Vec<f64> = (0..n).map(|i| -(edx0[i] + eps).sqrt() / (want_eg[i] + eps).sqrt() * g[i]).collect();
      let want_edx: Vec<f64> = (0..n).map(|i| rho * edx0[i] + (1.0 - rho) * dx[i] * dx[i]).collect();
...
fn radam_step(t: usize) -> (Vec<f64>, Vec<f64>) {
      ...
      let bv = GpuBuffer::upload(&v0).unwrap();
      let bg = GpuBuffer::upload(&g).unwrap();
      unsafe { launch_optimizerx_radam(bw.ptr_raw(), bm.ptr_raw(), bv.ptr_raw(), bg.ptr_raw() as *const c_void, lr, b1, b2, eps, t as i32, n as i32, std::ptr::null_mut()); }
      gpu_core::hip::check(unsafe { gpu_core::hip::hipGetLastError() }).unwrap();
      let mut gw = vec![0.0; n]; bw.download(&mut gw).unwrap();
      let b1t = b1.powi(t as i32); let b2t = b2.powi(t as i32);
...
fn prove_radam() -> bool {
      ...
      let rho_at = |t: usize| { let b2t = 0.999_f64.powi(t as i32); rho_inf - 2.0 * t as f64 * b2t / (1.0 - b2t) };
      let t_lo = 4usize;  // unrectified branch
      let t_hi = 12usize; // rectified branch
      assert!(rho_at(t_lo) <= 4.0, "radam t_lo branch setup wrong: rho_t={}", rho_at(t_lo));
      assert!(rho_at(t_hi) > 4.0, "radam t_hi branch setup wrong: rho_t={}", rho_at(t_hi));
...
fn prove_lars() -> bool {
      ...
      let w = wv(n); let g = gv(n); let b0 = sv(n, 0.1, 0.005);
      let bw = GpuBuffer::upload(&w).unwrap();
      let bb = GpuBuffer::upload(&b0).unwrap();
      let bg = GpuBuffer::upload(&g).unwrap();
      ...
      unsafe { launch_optimizerx_lars_phase1(bw.ptr_raw() as *const c_void, bg.ptr_raw() as *const c_void, n as i32, wns.ptr_raw(), gns.ptr_raw(), std::ptr::null_mut()); }
      gpu_core::hip::check(unsafe { gpu_core::hip::hipGetLastError() }).unwrap();
      gpu_core::hip::device_synchronize().unwrap();
      let mut wn = [0.0]; let mut gn = [0.0];
      wns.download(&mut wn).unwrap(); gns.download(&mut gn).unwrap();
      unsafe { launch_optimizerx_lars_phase2(bw.ptr_raw(), bb.ptr_raw(), bg.ptr_raw() as *const c_void, lr, mu, wd, lambda, wn[0], gn[0], eps, n as i32, std::ptr::null_mut()); }
      gpu_core::hip::check(unsafe { gpu_core::hip::hipGetLastError() }).unwrap();
      let mut gw = vec![0.0; n]; bw.download(&mut gw).unwrap();
      let mut gb = vec![0.0; n]; bb.download(&mut gb).unwrap();
      // independent oracle
      ...
      if (wn[0] - w.iter().map(|x| x * x).sum::<f64>()).abs() > 1e-9 * (1.0 + wn[0].abs()) { return false; }
      if (gn[0] - g.iter().map(|x| x * x).sum::<f64>()).abs() > 1e-9 * (1.0 + gn[0].abs()) { return false; }
      let trust = if w_norm > 0.0 && g_norm > 0.0 { lambda * w_norm / (g_norm + wd * w_norm + eps) } else { 1.0 };
      let want_b: Vec<f64> = (0..n).map(|i| mu * b0[i] + trust * (g[i] + wd * w[i])).collect();
...
fn canon(name: &str) -> String {
      ...
      while let Some(r) = s.strip_prefix('_') { s = r.to_string(); }
      // strip framework method/op prefixes (longest first; repeat to peel stacks)
      let prefixes = [
            "resourcesparseapply", "resourceapply", "sparseapply", "apply",
            "multi_tensor_", "fused", "deepspeedcpu", "paged_", "scale_by_",
            "keras", "noisy_", "polyak_", "mixedprecision",
            "dp", "mb",
      ];
      loop {
            ...
            for p in prefixes {
                  if let Some(r) = s.strip_prefix(p) {
                        if !r.is_empty() { s = r.to_string(); changed = true; break; }
                  }
            }
            if !changed { break; }
      }
      // strip trailing underscores (torch._fused_adam_)
      while let Some(r) = s.strip_suffix('_') { s = r.to_string(); }
      // strip dtype / variant suffixes
      let suffixes = [
            "_capturable_master", "_capturable", "_distributed", "_cpuoffload",
            "_optimizer", "_update", "_8bit", "_4bit", "_32bit", "_master",
            "_stage1", "_stage2", "_v2",
      ];
      loop {
            let mut changed = false;
      ...
      let alias: &[(&str, &str)] = &[
            ("sgd", "sgd"), ("gradientdescent", "sgd"), ("sgdupdate", "sgd"),
            ("momentum", "momentum"),
            ("nesterov", "nesterov"),
            ("rmsprop", "rmsprop"),
            ("adagrad", "adagrad"),
            ("adadelta", "adadelta"),
            ("adam", "adam"), ("adamoptimizer", "adam"),
            ("adamw", "adamw"), ("adamwupdate", "adamw"),
            ("nadam", "nadam"),
            ("radam", "radam"),
            ("lamb", "lamb"), ("lambupdate", "lamb"),
            ("lars", "lars"),
            ("lion", "lion"),
      ];
      for (a, c) in alias { if s == *a { return (*c).to_string(); } }
      s
}
...
fn prove_optimizer() {
      ...
      assert!(failures.is_empty(), "registered optimizer op(s) FAILED oracle: {:?}", failures);
      ...
      eprintln!("RESULT optimizer: proven={} total={} green=true implemented={}", proven, total, implemented);
}
