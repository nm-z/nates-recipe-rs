...
unsafe extern "C" {
      ...
      fn launch_sortx_searchsorted(sorted: *const c_void, q: *const c_void, out: *mut c_void, n: i32, nq: i32, s: *mut c_void);
}
...
fn data() -> Vec<f64> {
      let raw = [
            3.5, -1.0, 7.25, 0.0, 7.25, -8.0, 2.0, 5.5, -1.0, 9.0,
            4.0, -3.5, 6.0, 1.5, -2.0, 8.0, 0.5, -6.0, 2.75, 10.0,
      ];
      raw.to_vec()
}

fn run_keys(f: unsafe extern "C" fn(*const c_void, *mut c_void, i32, *mut c_void), x: &[f64]) -> Vec<f64> {
...
fn run_argsort(x: &[f64]) -> Vec<i32> {
      ...
      unsafe { launch_sortx_argsort(b.ptr_raw() as *const c_void, o.ptr_raw(), x.len() as i32, std::ptr::null_mut()); }
      lasterr();
...
fn run_argextreme(f: unsafe extern "C" fn(*const c_void, *mut c_void, i32, *mut c_void), x: &[f64]) -> i32 {
      let b = GpuBuffer::upload(x).unwrap();
...
fn run_topk(x: &[f64], k: usize) -> Vec<f64> {
      ...
      unsafe { launch_sortx_topk(b.ptr_raw() as *const c_void, o.ptr_raw(), x.len() as i32, k as i32, std::ptr::null_mut()); }
      lasterr();
...
fn run_scalar(f: unsafe extern "C" fn(*const c_void, *mut c_void, i32, i32, *mut c_void), x: &[f64], k: i32) -> f64 {
      let b = GpuBuffer::upload(x).unwrap();
      let o = GpuBuffer::alloc(1).unwrap();
      unsafe { f(b.ptr_raw() as *const c_void, o.ptr_raw(), x.len() as i32, k, std::ptr::null_mut()); }
      lasterr();
...
fn run_median(x: &[f64]) -> f64 {
      let b = GpuBuffer::upload(x).unwrap();
      let o = GpuBuffer::alloc(1).unwrap();
      unsafe { launch_sortx_median(b.ptr_raw() as *const c_void, o.ptr_raw(), x.len() as i32, std::ptr::null_mut()); }
      lasterr();
      let mut out = vec![0.0; 1];
      o.download(&mut out).unwrap();
      out[0]
...
fn run_searchsorted(sorted: &[f64], q: &[f64]) -> Vec<i32> {
      ...
      unsafe { launch_sortx_searchsorted(bs.ptr_raw() as *const c_void, bq.ptr_raw() as *const c_void, o.ptr_raw(), sorted.len() as i32, q.len() as i32, std::ptr::null_mut()); }
      lasterr();
...
fn prove_ops() -> (HashMap<&'static str, bool>, Vec<String>) {
      ...
      macro_rules! mark { ($k:expr, $pass:expr, $msg:expr) => {{
            let pass = $pass;
            ok.insert($k, pass);
      ...
      // sort descending == reversed CPU sort
      {
            ...
            mark!("sort_desc", pass, format!("sort_desc {:?} != {:?}", g, desc));
      }
      ...
      // matching the stable CPU argsort exactly (radix SortPairs is stable).
      {
            ...
            let perm = g.iter().all(|&i| { let i = i as usize; if i < n && !seen[i] { seen[i] = true; true } else { false } });
      ...
      // kthvalue: k-th smallest (1-based)
      {
            ...
            for k in [1usize, 7, n] {
                  ...
                  let want = asc[k - 1];
                  if (g - want).abs() > TOL { all = false; fail.push(format!("kthvalue k={} {} != {}", k, g, want)); }
            }
      ...
      // median: even-n => mean of two central; verify odd-n branch too
      {
            ...
            mark!("median", pass, format!("median even {} (want {}) / odd {} (want {})", g, want, go, wanto));
      }
      // argmax / argmin (ties: radix pair sort returns the lowest original index)
      {
            let g = run_argextreme(launch_sortx_argmax, &x);
            let want = (0..n).max_by(|&a, &b| x[a].partial_cmp(&x[b]).unwrap()).unwrap() as i32;
            mark!("argmax", g == want, format!("argmax {} != {}", g, want));
            let g2 = run_argextreme(launch_sortx_argmin, &x);
            let want2 = (0..n).min_by(|&a, &b| x[a].partial_cmp(&x[b]).unwrap()).unwrap() as i32;
      ...
      // searchsorted (left): out[j] == count of sorted[i] < q[j]
      {
            ...
            let want: Vec<i32> = q.iter().map(|&k| asc.iter().filter(|&&v| v < k).count() as i32).collect();
            let pass = g == want;
            mark!("searchsorted", pass, format!("searchsorted {:?} != {:?}", g, want));
      }
...
fn canon(name: &str) -> String {
      ...
      let alias: &[(&str, &str)] = &[
            ...
            ("sort", "sort"), ("sortkeys", "sort"), ("stablesortkeys", "sort"),
            ("stable_sort", "sort"), ("msort", "sort"), ("sort_lists", "sort"),
            ("radix_sort_keys", "sort"), ("merge_sort", "sort"), ("sort_complex", "sort"),
            ("sorted", "sort"), ("merge_sorted", "sort"),
            // descending keys
            ("sortkeysdescending", "sort_desc"), ("radix_sort_keys_descending", "sort_desc"),
            // argsort / sorted_order / stable argsort
            ("argsort", "argsort"), ("argsort_stable", "argsort"),
            ("sorted_order", "argsort"), ("stable_sorted_order", "argsort"),
            // pairs / by-key sorts have the SAME key ordering as plain sort (proven via sort/argsort)
            ("sortpairs", "sort"), ("sortkeyscopy", "sort"),
            ("sortpairsdescending", "sort_desc"),
            ("radix_sort_pairs", "sort"), ("radix_sort_pairs_descending", "sort_desc"),
            ("merge_sort_pairs", "sort"), ("sort_by_key", "sort"),
            ("stablesortpairs", "sort"), ("sort_key_val", "sort"),
            ("stable_sort_by_key", "sort"),
            // top-k
            ("topk", "topk"), ("top_k", "topk"), ("moe_top_k", "topk"),
            ("itoplayer", "topk"), ("itopklayer", "topk"),
            // k-th value / nth element
            ("kthvalue", "kthvalue"), ("nthelement", "kthvalue"), ("nth_element", "kthvalue"),
            // argmax / argmin
...
fn prove_sort() {
      ...
      eprintln!("backlog ({}): {}", backlog.len(), backlog.iter().cloned().collect::<Vec<_>>().join(", "));

      assert!(failures.is_empty(), "registered sort op(s) FAILED oracle: {:?}", failures);
