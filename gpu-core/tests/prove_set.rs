...
unsafe extern "C" {
      fn launch_setx_unique(x: *const c_void, out: *mut c_void, out_count: *mut c_void, n: i32, s: *mut c_void);
      fn launch_setx_unique_consecutive(x: *const c_void, out: *mut c_void, out_count: *mut c_void, n: i32, s: *mut c_void);
      fn launch_setx_unique_counts(x: *const c_void, vals_out: *mut c_void, counts_out: *mut c_void, out_count: *mut c_void, n: i32, s: *mut c_void);
      fn launch_setx_isin(a: *const c_void, b: *const c_void, mask: *mut c_void, na: i32, nb: i32, s: *mut c_void);
}
...
fn data() -> Vec<f64> {
      vec![
            3.5, -1.0, 7.25, 2.0, 2.0, 7.25, -8.0, 5.5, 5.5, -1.0,
            4.0, 0.0, 4.0, 9.0,
      ]
}
...
fn run_unique(launch: unsafe extern "C" fn(*const c_void, *mut c_void, *mut c_void, i32, *mut c_void), x: &[f64]) -> Vec<f64> {
      let n = x.len();
      let b = GpuBuffer::upload(x).unwrap();
      let out = GpuBuffer::alloc(n).unwrap();
      let cnt = GpuBuffer::alloc_bytes(4).unwrap();
      unsafe { launch(b.ptr_raw() as *const c_void, out.ptr_raw(), cnt.ptr_raw(), n as i32, std::ptr::null_mut()); }
      lasterr();
      let mut k = [0i32];
...
fn run_unique_counts(x: &[f64]) -> (Vec<f64>, Vec<i32>) {
      ...
      let cnt = GpuBuffer::alloc_bytes(4).unwrap();
      unsafe { launch_setx_unique_counts(b.ptr_raw() as *const c_void, vals.ptr_raw(), counts.ptr_raw(), cnt.ptr_raw(), n as i32, std::ptr::null_mut()); }
      lasterr();
      let mut k = [0i32];
      cnt.download_i32(&mut k).unwrap();
      let k = k[0] as usize;
...
fn run_isin(a: &[f64], b: &[f64]) -> Vec<f64> {
      ...
      unsafe { launch_setx_isin(ba.ptr_raw() as *const c_void, bb.ptr_raw() as *const c_void, mask.ptr_raw(), a.len() as i32, b.len() as i32, std::ptr::null_mut()); }
      lasterr();
...
fn cpu_unique_counts(x: &[f64]) -> (Vec<f64>, Vec<i32>) {
      ...
      for &val in &s {
            if let Some(&last) = vals.last() {
                  if last == val { *counts.last_mut().unwrap() += 1; continue; }
            }
            vals.push(val);
            counts.push(1);
      }
...
fn prove_ops() -> (HashMap<&'static str, bool>, Vec<String>) {
      ...
      // MUST differ from unique here (probe has non-adjacent dups), else vacuous.
      {
            ...
            mark!("unique_consecutive", pass, format!("unique_consecutive {:?} != {:?}", g, w));
      }
      ...
      // unique_counts: sorted distinct values + multiplicities
      {
            ...
            let pass = gv.len() == wv.len()
                  ...
                  && gc == wc;
            mark!("unique_counts", pass, format!("unique_counts ({:?},{:?}) != ({:?},{:?})", gv, gc, wv, wc));
      }
      ...
      // isin: membership mask a[i] in b ; oracle = exact b.contains
      {
            ...
            let g = run_isin(&a, &b);
            let w: Vec<f64> = a.iter().map(|av| if b.iter().any(|bv| bv == av) { 1.0 } else { 0.0 }).collect();
...
fn prove_set() {
      ...
      assert!(failures.is_empty(), "registered set op(s) FAILED oracle: {:?}", failures);
