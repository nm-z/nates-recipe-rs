...
unsafe extern "C" {
      fn launch_paddingx_constant1d(x: *const c_void, o: *mut c_void, l: i32, lpad: i32, n: i32, cval: f64, s: *mut c_void);
      ...
      fn launch_paddingx_constant2d(x: *const c_void, o: *mut c_void, h: i32, w: i32, tpad: i32, lpad: i32, oh: i32, ow: i32, n: i32, cval: f64, s: *mut c_void);
      fn launch_paddingx_reflect2d(x: *const c_void, o: *mut c_void, h: i32, w: i32, tpad: i32, lpad: i32, oh: i32, ow: i32, n: i32, s: *mut c_void);
      fn launch_paddingx_replicate2d(x: *const c_void, o: *mut c_void, h: i32, w: i32, tpad: i32, lpad: i32, oh: i32, ow: i32, n: i32, s: *mut c_void);
      fn launch_paddingx_circular2d(x: *const c_void, o: *mut c_void, h: i32, w: i32, tpad: i32, lpad: i32, oh: i32, ow: i32, n: i32, s: *mut c_void);
}
...
fn replicate_idx(p: i64, l: i64) -> i64 { p.clamp(0, l - 1) }
fn circular_idx(p: i64, l: i64) -> i64 { let mut q = p % l; if q < 0 { q += l; } q }
...
fn oracle_1d(mode: &str, x: &[f64], lpad: i64, rpad: i64, cval: f64) -> Vec<f64> {
      ...
      (0..n).map(|i| {
            let p = i - lpad;
            match mode {
                  "constant" => if p >= 0 && p < l { x[p as usize] } else { cval },
...
fn oracle_2d(mode: &str, x: &[f64], h: i64, w: i64, tpad: i64, bpad: i64, lpad: i64, rpad: i64, cval: f64) -> Vec<f64> {
      ...
      for orr in 0..oh {
            for oc in 0..ow {
                  ...
                  let c = oc - lpad;
                  let v = match mode {
                        "constant" => if r >= 0 && r < h && c >= 0 && c < w { x[(r * w + c) as usize] } else { cval },
...
fn gpu_1d(mode: &str, x: &[f64], lpad: i32, rpad: i32, cval: f64) -> Vec<f64> {
      ...
      unsafe {
            match mode {
                  "constant" => launch_paddingx_constant1d(xp, op, l, lpad, n, cval, std::ptr::null_mut()),
                  "reflect" => launch_paddingx_reflect1d(xp, op, l, lpad, n, std::ptr::null_mut()),
                  "replicate" => launch_paddingx_replicate1d(xp, op, l, lpad, n, std::ptr::null_mut()),
                  "circular" => launch_paddingx_circular1d(xp, op, l, lpad, n, std::ptr::null_mut()),
                  _ => unreachable!(),
            }
      }
...
fn gpu_2d(mode: &str, x: &[f64], h: i32, w: i32, tpad: i32, bpad: i32, lpad: i32, rpad: i32, cval: f64) -> Vec<f64> {
      ...
      unsafe {
            match mode {
                  "constant" => launch_paddingx_constant2d(xp, op, h, w, tpad, lpad, oh, ow, n, cval, std::ptr::null_mut()),
                  "reflect" => launch_paddingx_reflect2d(xp, op, h, w, tpad, lpad, oh, ow, n, std::ptr::null_mut()),
                  "replicate" => launch_paddingx_replicate2d(xp, op, h, w, tpad, lpad, oh, ow, n, std::ptr::null_mut()),
                  "circular" => launch_paddingx_circular2d(xp, op, h, w, tpad, lpad, oh, ow, n, std::ptr::null_mut()),
                  _ => unreachable!(),
            }
      }
      gpu_core::hip::check(unsafe { gpu_core::hip::hipGetLastError() }).unwrap();
...
fn close(a: &[f64], b: &[f64]) -> bool {
      a.len() == b.len() && a.iter().zip(b).all(|(g, w)| (g - w).abs() <= TOL * (1.0 + w.abs()))
}

// Prove one pad mode end-to-end on both 1D and 2D with asymmetric pads.
fn prove_mode(mode: &str) -> bool {
      // deterministic pseudo-random input
      let x1: Vec<f64> = (0..7).map(|i| ((i * 31 + 5) % 17) as f64 * 0.5 - 3.0).collect();
      let cval = 2.5;
      ...
      for &(lp, rp) in &[(3i32, 2i32), (6, 5), (0, 4)] {
            ...
            if !close(&g, &o) { eprintln!("  {mode} 1d pad({lp},{rp}) MISMATCH\n    gpu={g:?}\n    cpu={o:?}"); return false; }
      }
      // 2D: 4x5 input, asymmetric pads on all four sides
      let (h, w) = (4i32, 5i32);
      let x2: Vec<f64> = (0..(h * w)).map(|i| ((i * 13 + 7) % 23) as f64 * 0.25 - 2.0).collect();
      for &(t, b, l, r) in &[(2i32, 1i32, 3i32, 2i32), (3, 3, 4, 4), (0, 2, 1, 0)] {
            let g = gpu_2d(mode, &x2, h, w, t, b, l, r, cval);
            let o = oracle_2d(mode, &x2, h as i64, w as i64, t as i64, b as i64, l as i64, r as i64, cval);
            if !close(&g, &o) { eprintln!("  {mode} 2d pad(t{t},b{b},l{l},r{r}) MISMATCH"); return false; }
      }
      true
}
...
fn canon(name: &str) -> &'static str {
      ...
      if base.contains("grad") || base.contains("backward") { return "DROP_grad"; }
      // tflite DILATE = interior dilation, not boundary fill -> drop
      if base == "dilate" { return "DROP_dilate"; }
      // reflect / mirror family
      if base.contains("reflection") || base.contains("mirror") { return "reflect"; }
      // replicate / edge family
      if base.contains("replication") { return "replicate"; }
      // circular / wrap family
      if base.contains("circular") { return "circular"; }
      // zero_pad = constant with 0
      if base.contains("zero_pad") { return "constant"; }
      ...
      if base.contains("pad") || base.contains("copymakeborder") { return "constant"; }
      "UNMAPPED"
}
      ...
      for e in std::fs::read_dir(&dir).expect("no kernel_inventory").flatten() {
            ...
            if p.extension().map_or(false, |x| x == "json") {
                  ...
                  if let Some(ks) = v.get("kernels").and_then(|k| k.as_array()) {
                        for k in ks {
                              if k.get("category").and_then(|c| c.as_str()) != Some("padding") { continue; }
...
fn prove_padding() {
      ...
      for m in modes {
            let ok = prove_mode(m);
            op_ok.insert(m, ok);
            if !ok { failures.push(m.to_string()); }
      }
      ...
      for name in &items {
            match canon(name) {
                  k @ ("constant" | "reflect" | "replicate" | "circular") => {
                        if *op_ok.get(k).unwrap() { proven += 1; proven_keys.insert(k.to_string()); }
                  }
      ...
      if !dropped.is_empty() {
            eprintln!("dropped ({}, different function — not boundary-fill padding):", dropped.len());
      ...
      assert!(failures.is_empty(), "registered padding op(s) FAILED oracle: {:?}", failures);
      assert!(unmapped.is_empty(), "unmapped padding items (extend canon): {:?}", unmapped);
