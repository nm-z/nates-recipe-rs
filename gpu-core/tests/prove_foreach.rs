...
fn registry() -> HashMap<&'static str, Op> {
      ...
      macro_rules! u { ($k:literal, $l:expr, $o:expr, $lo:expr, $hi:expr) => {
            m.insert($k, Op { run: Box::new(|x, _| run_u($l, x)), oracle: Box::new(|x, _| $o(x)), lo: $lo, hi: $hi, blo: 0.0, bhi: 0.0 });
      }; }
      macro_rules! b { ($k:literal, $l:expr, $o:expr, $alo:expr, $ahi:expr, $blo:expr, $bhi:expr) => {
            m.insert($k, Op { run: Box::new(|a, bb| run_b($l, a, bb)), oracle: Box::new($o), lo: $alo, hi: $ahi, blo: $blo, bhi: $bhi });
      }; }

      // ── core 9 ──
      b!("add", launch_foreachx_add, |x: f64, y: f64| x + y, -3.0, 3.0, -2.0, 4.0);
      b!("sub", launch_foreachx_sub, |x: f64, y: f64| x - y, -3.0, 3.0, -2.0, 4.0);
      b!("mul", launch_foreachx_mul, |x: f64, y: f64| x * y, -3.0, 3.0, -2.0, 4.0);
      b!("div", launch_foreachx_div, |x: f64, y: f64| x / y, -3.0, 3.0, 0.5, 4.0);
      ...
      u!("sigmoid", launch_foreachx_sigmoid, |x: f64| 1.0 / (1.0 + (-x).exp()), -6.0, 6.0);

      // ── extra exact-oracle elementwise foreach ops ──
      u!("reciprocal", launch_foreachx_reciprocal, |x: f64| 1.0 / x, 0.5, 5.0);
      u!("log", launch_foreachx_log, |x: f64| x.ln(), 0.05, 5.0);
      u!("log2", launch_foreachx_log2, |x: f64| x.log2(), 0.05, 5.0);
      u!("log10", launch_foreachx_log10, |x: f64| x.log10(), 0.05, 5.0);
      u!("log1p", launch_foreachx_log1p, |x: f64| x.ln_1p(), -0.9, 5.0);
      u!("expm1", launch_foreachx_expm1, |x: f64| x.exp_m1(), -3.0, 3.0);
      u!("floor", launch_foreachx_floor, |x: f64| x.floor(), -5.0, 5.0);
      u!("ceil", launch_foreachx_ceil, |x: f64| x.ceil(), -5.0, 5.0);
      u!("round", launch_foreachx_round, |x: f64| { let r = x.round(); // ties-to-even (torch/rint)
            if (x - x.trunc()).abs() == 0.5 { 2.0 * (x / 2.0).round() } else { r } }, -5.0, 5.0);
      u!("trunc", launch_foreachx_trunc, |x: f64| x.trunc(), -5.0, 5.0);
      u!("frac", launch_foreachx_frac, |x: f64| x - x.trunc(), -5.0, 5.0);
      u!("sign", launch_foreachx_sign, |x: f64| if x > 0.0 { 1.0 } else if x < 0.0 { -1.0 } else { 0.0 }, -3.0, 3.0);
      ...
      u!("erfc", launch_foreachx_erfc, |x: f64| libm::erfc(x), -3.0, 3.0);
      u!("lgamma", launch_foreachx_lgamma, |x: f64| libm::lgamma(x), 0.2, 5.0);
      b!("maximum", launch_foreachx_maximum, |x: f64, y: f64| x.max(y), -3.0, 3.0, -3.0, 3.0);
      b!("minimum", launch_foreachx_minimum, |x: f64, y: f64| x.min(y), -3.0, 3.0, -3.0, 3.0);
      b!("pow", launch_foreachx_pow, |x: f64, y: f64| x.powf(y), 0.2, 4.0, 0.5, 3.0);
      m
}
...
fn canon(name: &str) -> String {
      ...
      if let Some(p) = s.find("_foreach_") { s = s[p + "_foreach_".len()..].to_string(); }
      // strip overload suffixes (longest first), repeatedly
      let suffixes = ["_scalarlist", "_tensorlist", "_scalar", "_tensor", "_list", "_self", "_2"];
      loop {
      ...
      for e in std::fs::read_dir(&dir).expect("no kernel_inventory").flatten() {
            ...
            if p.extension().map_or(false, |x| x == "json") {
                  ...
                  if let Some(ks) = v.get("kernels").and_then(|k| k.as_array()) {
                        for k in ks {
                              if k.get("category").and_then(|c| c.as_str()) != Some("foreach") { continue; }
...
fn prove_foreach() {
      ...
      for k in ["add", "sub", "mul", "div", "neg", "abs", "sqrt", "exp", "sigmoid"] {
            assert!(*op_ok.get(k).unwrap_or(&false), "core foreach op {} failed oracle", k);
      }
      assert!(failures.is_empty(), "registered foreach op(s) FAILED oracle: {:?}", failures);
