...
fn unary_registry() -> HashMap<&'static str, UnaryOp> {
	...
	use gpu_core::kernels::{gpu_relu_into, gpu_sigmoid_into, gpu_silu_into, gpu_tanh_into};
	km!("relu", gpu_relu_into, |x| x.max(0.0), -3.0, 3.0);
	km!(
		"sigmoid",
		gpu_sigmoid_into,
		|x| 1.0 / (1.0 + (-x).exp()),
	...
	km!("tanh", gpu_tanh_into, |x| x.tanh(), -3.0, 3.0);
	km!("silu", gpu_silu_into, |x| x / (1.0 + (-x).exp()), -6.0, 6.0);
	...
	fn celu1(x: &GpuBuffer, n: usize, out: &GpuBuffer) -> Result<(), HipError> {
		let p = { let __up = &[1.0f64]; let __ub = GpuBuffer::alloc(__up.len())?; __ub.load(__up)?; __ub };
		gpu_core::k_actx::gpu_celu(x, &p, n, out)
	}
	fn hardshrink05(x: &GpuBuffer, n: usize, out: &GpuBuffer) -> Result<(), HipError> {
		let p = { let __up = &[0.5f64]; let __ub = GpuBuffer::alloc(__up.len())?; __ub.load(__up)?; __ub };
	...
	ax!(
		"relu_squared",
		launch_activationx_relu_squared,
		|x: f64| {
			let r = x.max(0.0);
			r * r
		},
...
fn check_squareplus() -> Option<String> {
	...
	let bx = { let __up = &xs; let __ub = GpuBuffer::alloc(__up.len()).unwrap(); __ub.load(__up).unwrap(); __ub };
	let o = GpuBuffer::alloc(xs.len()).unwrap();
	unsafe {
		launch_activationx_squareplus(
	...
	for (x, g) in xs.iter().zip(&got) {
		...
		if !approx(*g, want) {
			return Some(format!("squareplus(b={b}): x={x} got={g} want={want}"));
		}
	}
	None
}
...
fn check_star_relu() -> Option<String> {
	...
	for (x, g) in xs.iter().zip(&got) {
		...
		if !approx(*g, want) {
			return Some(format!(
				"star_relu(s={sc},b={b}): x={x} got={g} want={want}"
			));
		}
	}
	None
...
fn check_prelu() -> Option<String> {
	...
	let alpha: Vec<f64> = (0..xs.len())
		.map(|i| 0.01 + 0.3 * (i as f64) / xs.len() as f64)
		.collect();
	let bx = { let __up = &xs; let __ub = GpuBuffer::alloc(__up.len()).unwrap(); __ub.load(__up).unwrap(); __ub };
	let ba = { let __up = &alpha; let __ub = GpuBuffer::alloc(__up.len()).unwrap(); __ub.load(__up).unwrap(); __ub };
	let o = GpuBuffer::alloc(xs.len()).unwrap();
	unsafe {
		launch_activationx_prelu(
			bx.ptr_raw() as *const c_void,
	...
	for ((x, a), g) in xs.iter().zip(&alpha).zip(&got) {
		...
		if !approx(*g, want) {
			return Some(format!("prelu: x={x} a={a} got={g} want={want}"));
		}
	}
	None
}
...
fn check_glu() -> Option<String> {
	...
	for i in 0..half {
		...
		if !approx(got[i], want) {
			return Some(format!("glu: i={i} got={} want={want}", got[i]));
		}
	}
	None
}
...
fn check_reglu() -> Option<String> {
	...
	for i in 0..half {
		...
		if !approx(got[i], want) {
			return Some(format!("reglu: i={i} got={} want={want}", got[i]));
		}
	}
	None
}
...
fn check_crelu() -> Option<String> {
	...
	for (i, x) in xs.iter().enumerate() {
		...
		if !approx(got[i], want_pos) {
			return Some(format!("crelu pos: i={i} got={} want={want_pos}", got[i]));
		}
		if !approx(got[i + n], want_neg) {
			return Some(format!(
				"crelu neg: i={i} got={} want={want_neg}",
				got[i + n]
			));
		}
	}
...
fn check_softmin() -> Option<String> {
	...
	for r in 0..rows {
		for c in 0..cols {
			x[r * cols + c] = ((r as f64) - 2.0) + 0.5 * (c as f64) - 1.5;
		}
	}
	let bx = { let __up = &x; let __ub = GpuBuffer::alloc(__up.len()).unwrap(); __ub.load(__up).unwrap(); __ub };
	let o = GpuBuffer::alloc(rows * cols).unwrap();
	unsafe {
		launch_activationx_softmin(
			bx.ptr_raw() as *const c_void,
			o.ptr_raw(),
			rows as i32,
			cols as i32,
		...
		.flatten()
	{
		...
		if p.extension().is_some_and(|x| x == "json") {
			...
			if let Some(ks) = v.get("kernels").and_then(|k| k.as_array()) {
				for k in ks {
					if k.get("category").and_then(|c| c.as_str()) != Some("activation") {
						continue;
					}
...
fn prove_activation() {
	...
	for (k, op) in reg.iter() {
		...
		let ok = xs
			.iter()
			.zip(&got)
			.all(|(x, g)| approx(*g, (op.oracle)(*x)));
		op_ok.insert(*k, ok);
		if !ok {
			failures.push(format!("registered op {k} FAILED oracle"));
		}
	}
	...
	// ── row-wise softmax / log_softmax (existing kernels, (rows,cols) sig) ──
	{
		...
		for r in 0..rows {
			for c in 0..cols {
				x[r * cols + c] = ((r as f64) - 2.0) + 0.4 * (c as f64) - 1.2;
			}
		}
		let bx = { let __up = &x; let __ub = GpuBuffer::alloc(__up.len()).unwrap(); __ub.load(__up).unwrap(); __ub };
		...
		unsafe { sm.download_async(&mut gsm, std::ptr::null_mut()) }.unwrap(); gpu_core::hip::device_synchronize().unwrap();
		let lsm = GpuBuffer::alloc(rows * cols).unwrap();
		gpu_log_softmax_rows(&bx, rows, cols, &lsm).unwrap();
		let mut glsm = vec![0.0; rows * cols];
		unsafe { lsm.download_async(&mut glsm, std::ptr::null_mut()) }.unwrap(); gpu_core::hip::device_synchronize().unwrap();
		...
		for r in 0..rows {
			...
			for c in 0..cols {
				...
				if !approx(gsm[r * cols + c], w_sm) {
					sm_ok = false;
				}
				if !approx(glsm[r * cols + c], w_lsm) {
					lsm_ok = false;
				}
			}
		}
		...
		if !sm_ok {
			failures.push("softmax row FAILED oracle".into());
		}
		if !lsm_ok {
			failures.push("log_softmax row FAILED oracle".into());
		}
	}

	// ── leaky_relu (existing, has alpha param) ──
	{
		use gpu_core::kernels::gpu_leaky_relu_into;
		let alpha = 0.01;
		let xs = probes(-3.0, 3.0, 32);
		let bx = { let __up = &xs; let __ub = GpuBuffer::alloc(__up.len()).unwrap(); __ub.load(__up).unwrap(); __ub };
		let ba = { let __up = &[alpha]; let __ub = GpuBuffer::alloc(__up.len()).unwrap(); __ub.load(__up).unwrap(); __ub };
		...
		let ok = xs
			.iter()
			.zip(&got)
			.all(|(x, g)| approx(*g, if *x > 0.0 { *x } else { alpha * x }));
		op_ok.insert("leaky_relu", ok);
		if !ok {
			failures.push("leaky_relu FAILED oracle".into());
		}
	}
	...
	// ── gated swiglu / geglu (existing, binary a,b) ──
	{
		...
		unsafe { sw.download_async(&mut gsw, std::ptr::null_mut()) }.unwrap(); gpu_core::hip::device_synchronize().unwrap();
		let ge = GpuBuffer::alloc(a.len()).unwrap();
		gpu_geglu(&ba, &bb, a.len(), &ge).unwrap();
		let mut gge = vec![0.0; a.len()];
		unsafe { ge.download_async(&mut gge, std::ptr::null_mut()) }.unwrap(); gpu_core::hip::device_synchronize().unwrap();
		let mut sw_ok = true;
		let mut ge_ok = true;
		for i in 0..a.len() {
		...
		if !sw_ok {
			failures.push("swiglu FAILED oracle".into());
		}
		if !ge_ok {
			failures.push("geglu FAILED oracle".into());
		}
	}
	...
	assert!(
		failures.is_empty(),
		"activation op(s) FAILED oracle: {:?}",
		failures
	);
	assert!(proven > 0, "zero activation items proven");

	eprintln!(
		"RESULT activation: proven={} total={} green=true implemented={}",
