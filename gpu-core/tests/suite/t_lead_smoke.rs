...
fn sum_entire() {
	...
	let mut s = [0.0; 1];
	unsafe { out.download_async(&mut s, std::ptr::null_mut()) }.unwrap();
...
fn ddot() {
	...
	let mut d = [0.0; 1];
	unsafe { out.download_async(&mut d, std::ptr::null_mut()) }.unwrap();
	gpu_core::hip::device_synchronize().unwrap();
	eprintln!("ddot = {}", d[0]);
...
fn rsqrt_recip_max() {
	...
	let mut out = [0.0; 2];
	unsafe { r.download_async(&mut out, std::ptr::null_mut()) }.unwrap();
	gpu_core::hip::device_synchronize().unwrap();
	eprintln!("rsqrt = {:?}", out);
	approx(&out, &[0.5, 0.25]);
	...
	unsafe { rc.download_async(&mut out, std::ptr::null_mut()) }.unwrap();
	gpu_core::hip::device_synchronize().unwrap();
	eprintln!("recip = {:?}", out);
	approx(&out, &[0.5, 0.25]);
	...
	unsafe { m.download_async(&mut out, std::ptr::null_mut()) }.unwrap();
	gpu_core::hip::device_synchronize().unwrap();
	eprintln!("max = {:?}", out);
	approx(&out, &[3.0, 5.0]);
}
...
fn sort_pow2() {
	...
	let mut out = [0.0; 4];
	unsafe { s.download_async(&mut out, std::ptr::null_mut()) }.unwrap();
	gpu_core::hip::device_synchronize().unwrap();
...
fn cumsum_rows() {
	...
	let mut out = [0.0; 6];
	unsafe { c.download_async(&mut out, std::ptr::null_mut()) }.unwrap();
	gpu_core::hip::device_synchronize().unwrap();
...
fn one_hot() {
	...
	let mut out = [0.0; 6];
	unsafe { oh.download_async(&mut out, std::ptr::null_mut()) }.unwrap();
	gpu_core::hip::device_synchronize().unwrap();
...
fn discounted_returns() {
	...
	let mut out = [0.0; 3];
	unsafe { g.download_async(&mut out, std::ptr::null_mut()) }.unwrap();
	gpu_core::hip::device_synchronize().unwrap();
