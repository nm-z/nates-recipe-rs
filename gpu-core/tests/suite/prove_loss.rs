...
fn registry() -> HashMap<&'static str, LossOp> {
	...
	op!(
		...
		|logp, t| if t > 0.0 { t * (t.ln() - logp) } else { 0.0 },
		-3.0,
		-0.05,
		0.02,
		0.98
...
fn prove_loss() {
	...
	eprintln!(
		"RESULT loss: proven={} total={} green={} implemented={}",
		proven_items,
		total,
		green,
		ops_sorted.join(",")
	...
	assert!(
		failures.is_empty(),
		"{} registered loss op(s) FAILED oracle:\n{}",
		failures.len(),
		failures.join("\n")
	);
	assert!(
		proven_items > 0,
		"zero loss items proven — canon/registry broken"
	);
}
