...
fn registry() -> BTreeMap<&'static str, Op> {
	...
	macro_rules! e {
		($k:literal, $g:expr, $o:expr, $in:expr) => {
			m.insert(
				$k,
				Op {
					gpu: Gpu::Existing($g),
	...
	nw!(
		"frac",
		launch_elementwise_unaryx_frac,
		|x| x - x.trunc(),
		std_probe
	);
	nw!(
		"heaviside",
		launch_elementwise_unaryx_heaviside,
		|x| if x < 0.0 {
			0.0
		} else if x > 0.0 {
			1.0
	...
	nw!(
		"sinc",
		launch_elementwise_unaryx_sinc,
		|x| {
			if x == 0.0 {
				1.0
			} else {
	...
	nw!(
		"positive",
		launch_elementwise_unaryx_positive,
		|x| x,
		std_probe
	);
	nw!(
		"logit",
		launch_elementwise_unaryx_logit,
		|x| (x / (1.0 - x)).ln(),
		prob01
	);
	nw!(
		"expit",
		launch_elementwise_unaryx_expit,
		|x| 1.0 / (1.0 + (-x).exp()),
		probes(-6.0, 6.0, 64)
	);
	nw!(
		"exp10",
		launch_elementwise_unaryx_exp10,
		|x| 10f64.powf(x),
		std_probe
	);
	...
	nw!(
		"isinf",
		launch_elementwise_unaryx_isinf,
		|x| if x.is_infinite() { 1.0 } else { 0.0 },
		patho
	);
	nw!(
		"isfinite",
		launch_elementwise_unaryx_isfinite,
		|x| if x.is_finite() { 1.0 } else { 0.0 },
		patho
	);
	nw!(
		"signbit",
		launch_elementwise_unaryx_signbit,
		|x| if x.is_sign_negative() { 1.0 } else { 0.0 },
		patho
	);
	m
}
