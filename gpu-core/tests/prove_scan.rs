...
fn o_cumsum(x: &[f64]) -> Vec<f64> { let mut a = 0.0; x.iter().map(|&v| { a += v; a }).collect() }
fn o_cumprod(x: &[f64]) -> Vec<f64> { let mut a = 1.0; x.iter().map(|&v| { a *= v; a }).collect() }
fn o_cummax(x: &[f64]) -> Vec<f64> { let mut a = f64::NEG_INFINITY; x.iter().map(|&v| { a = a.max(v); a }).collect() }
fn o_cummin(x: &[f64]) -> Vec<f64> { let mut a = f64::INFINITY; x.iter().map(|&v| { a = a.min(v); a }).collect() }
fn o_excl_sum(x: &[f64]) -> Vec<f64> { let mut a = 0.0; x.iter().map(|&v| { let p = a; a += v; p }).collect() }
fn o_excl_prod(x: &[f64]) -> Vec<f64> { let mut a = 1.0; x.iter().map(|&v| { let p = a; a *= v; p }).collect() }
fn o_logcumsumexp(x: &[f64]) -> Vec<f64> {
      ...
      let mut acc = 0.0f64;
      x.iter().map(|&v| { acc += v.exp(); acc.ln() }).collect()
}

const TOL: f64 = 1e-7;

fn close(g: &[f64], w: &[f64]) -> bool {
      g.len() == w.len() && g.iter().zip(w).all(|(a, b)| (a - b).abs() <= TOL * (1.0 + b.abs()))
}

// A registered scan op: GPU runner + CPU oracle, both x -> full vector.
struct ScanOp { run: Box<dyn Fn(&[f64]) -> Vec<f64>>, oracle: Box<dyn Fn(&[f64]) -> Vec<f64>> }

fn registry() -> HashMap<&'static str, ScanOp> {
      let mut m: HashMap<&'static str, ScanOp> = HashMap::new();
      macro_rules! reg {
            ($k:literal, $run:expr, $or:expr) => {
                  m.insert($k, ScanOp { run: Box::new($run), oracle: Box::new($or) });
            };
      }
      // existing gpu-core ops (prove they still hold)
      reg!("cumsum",  |x| g_cumsum(x),  |x| o_cumsum(x));
      reg!("cumprod", |x| g_cumprod(x), |x| o_cumprod(x));
      reg!("cummax",  |x| g_cummax(x),  |x| o_cummax(x));
      // new scanx_ ops
      reg!("cummin",       |x| run_scanx(launch_scanx_cummin, x),       |x| o_cummin(x));
      reg!("logcumsumexp", |x| run_scanx(launch_scanx_logcumsumexp, x), |x| o_logcumsumexp(x));
      reg!("exclusive_sum",  |x| run_scanx(launch_scanx_excl_sum, x),  |x| o_excl_sum(x));
      reg!("exclusive_prod", |x| run_scanx(launch_scanx_excl_prod, x), |x| o_excl_prod(x));
      reg!("inclusive_sum",  |x| run_scanx(launch_scanx_cumsum, x),    |x| o_cumsum(x));
      reg!("associative_scan", |x| run_scanx(launch_scanx_assoc_add, x), |x| o_cumsum(x));
      m
}
...
fn canon(name: &str) -> String {
      ...
      let base = base
            .strip_suffix("_aten").map(|s| s.to_string())
      ...
      let alias: &[(&str, &str)] = &[
            ...
            ("cumsum", "cumsum"), ("cumulative_sum", "cumsum"), ("_cumsum", "cumsum"),
            ("chunk_cumsum", "cumsum"),
            ("inclusivesum", "inclusive_sum"), ("inclusive_sum", "inclusive_sum"),
            ("inclusive_scan", "inclusive_sum"), ("inclusivescan", "inclusive_sum"),
            ("inclusivesumbykey", "inclusive_sum"), ("inclusive_scan_by_key", "inclusive_sum"),
            ("inclusivescanbykey", "inclusive_sum"),
            ("segmented_inclusive_scan", "inclusive_sum"),
            ("transform_inclusive_scan", "inclusive_sum"),
            ("block_scan_inclusive", "inclusive_sum"), ("warp_scan_inclusive", "inclusive_sum"),
            ("blockscan", "inclusive_sum"), ("warpscan", "inclusive_sum"), ("threadscan", "inclusive_sum"),
            // exclusive scan family (identity element)
            ("exclusivesum", "exclusive_sum"), ("exclusive_sum", "exclusive_sum"),
            ("exclusive_scan", "exclusive_sum"), ("exclusivescan", "exclusive_sum"),
            ("exclusivesumbykey", "exclusive_sum"), ("exclusive_scan_by_key", "exclusive_sum"),
            ("exclusivescanbykey", "exclusive_sum"),
            ("segmented_exclusive_scan", "exclusive_sum"),
            ("transform_exclusive_scan", "exclusive_sum"),
            ("block_scan_exclusive", "exclusive_sum"), ("warp_scan_exclusive", "exclusive_sum"),
            // cumprod
            ("cumprod", "cumprod"), ("cumulative_product", "cumprod"),
            // cummax / cummin
            ("cummax", "cummax"), ("cumulative_max", "cummax"), ("_cummax", "cummax"),
            ("cummin", "cummin"), ("cumulative_min", "cummin"), ("_cummin", "cummin"),
            // log-cumsum-exp
            ("logcumsumexp", "logcumsumexp"), ("cumlogsumexp", "logcumsumexp"),
            // associative / generic forward scan primitives
            ("associative_scan", "associative_scan"), ("scan", "associative_scan"),
...
fn prove_scan() {
      ...
      let xs: Vec<f64> = {
            let n = 33usize;
            (0..n).map(|i| -4.0 + 8.0 * (i as f64 + 0.5) / n as f64).collect()
      };
      ...
      for (k, op) in reg.iter() {
            let probe = if *k == "cumprod" || *k == "exclusive_prod" { &xs_prod } else { &xs };
            let got = (op.run)(probe);
            let want = (op.oracle)(probe);
            let ok = close(&got, &want);
            op_ok.insert(*k, ok);
            if !ok {
                  failures.push(format!("{}: got {:?} want {:?}", k, &got[..got.len().min(5)], &want[..want.len().min(5)]));
            }
      }
      ...
      // exclusive scan identities: out[0] must equal the identity element.
      {
            ...
            if e[0] != 0.0 { failures.push(format!("excl_sum[0]={} != 0 (identity)", e[0])); }
            let p = run_scanx(launch_scanx_excl_prod, &xs_prod);
            if p[0] != 1.0 { failures.push(format!("excl_prod[0]={} != 1 (identity)", p[0])); }
      }
      ...
      // Prove against an independent CPU recurrence.
      {
            ...
            unsafe { launch_scanx_recur(ba.ptr_raw() as *const c_void, bb.ptr_raw() as *const c_void, o.ptr_raw(), a.len() as i32, std::ptr::null_mut()); }
            gpu_core::hip::check(unsafe { gpu_core::hip::hipGetLastError() }).unwrap();
            let mut got = vec![0.0; a.len()];
            o.download(&mut got).unwrap();
            let mut h = 0.0;
            let want: Vec<f64> = a.iter().zip(&b).map(|(&ai, &bi)| { h = ai * h + bi; h }).collect();
            if !close(&got, &want) {
      ...
      eprintln!("backlog (windowed/groupby/rank/interp, non-prefix-scan): {}", backlog.len());

      assert!(failures.is_empty(), "registered scan op(s) FAILED oracle: {:#?}", failures);
