...
fn cpu_layernorm(
	...
	for r in 0..rows {
		...
		for j in 0..cols {
			o[r * cols + j] = (row[j] - mean) * inv * gamma[j] + beta[j];
		}
	}
	o
}
...
fn cpu_batchnorm(x: &[f64], n: usize, c: usize, gamma: &[f64], beta: &[f64], eps: f64) -> Vec<f64> {
	...
	for ch in 0..c {
		...
		let inv = 1.0 / (var + eps).sqrt();
		for i in 0..n {
			o[i * c + ch] = gamma[ch] * (x[i * c + ch] - mean) * inv + beta[ch];
		}
	}
	o
}
...
fn run_proofs() -> (HashMap<&'static str, bool>, Vec<String>) {
	...
	// ── softmax_backward (Jacobian-vector product: out_j = s_j * (g_j - Σ g·s)) ──
	{
		use gpu_core::kernels::gpu_softmax_backward_into;
		...
		for r in 0..rows {
			let s = &sm[r * cols..(r + 1) * cols];
			let g = &grad[r * cols..(r + 1) * cols];
			let dot: f64 = s.iter().zip(g).map(|(a, b)| a * b).sum();
			for j in 0..cols {
				want[r * cols + j] = s[j] * (g[j] - dot);
			}
		}
		put("softmax_backward", close(&got, &want), &mut fails);
	}
	...
	// ── rmsnorm (NEW normx_rmsnorm, f64 fused, affine) ──
	{
		...
		put(
			...
			close(
				&dl(&out, rows * cols),
				&cpu_rmsnorm(&x, rows, cols, &gamma, eps),
			),
			&mut fails,
		);
	}
	...
	// ── groupnorm (NEW normx_groupnorm, affine) ──
	{
		...
		put(
			...
			close(
				&dl(&out, n * c * l),
				&cpu_groupnorm(&x, n, c, l, g, &gamma, &beta, eps),
			),
			&mut fails,
		);
	}
	...
	// ── instancenorm (NEW normx_instancenorm, affine) ──
	{
		...
		put(
			...
			close(
				&dl(&out, n * c * l),
				&cpu_instancenorm(&x, n, c, l, &gamma, &beta, eps),
			),
			&mut fails,
		);
	}
	...
	// ── l2_normalize (NEW normx_l2_normalize) ──
	{
		...
		put(
			...
			close(
				&dl(&out, rows * cols),
				&cpu_l2_normalize(&x, rows, cols, eps),
			),
			&mut fails,
		);
	}
...
fn canon(name: &str) -> String {
	...
	if base.contains("groupnorm") || base.contains("groupnormalization") {
		return "groupnorm".to_string();
	}
	if base.contains("instancenorm") || base.contains("instancenormalization") {
		return "instancenorm".to_string();
	}
	if base.contains("layernorm") || base.contains("layernormalization") {
		return "layernorm".to_string();
	}
	if base.contains("batchnorm") || base.contains("batchnormalization") {
		return "batchnorm".to_string();
	}
	if base == "logsoftmax" {
		return "log_softmax".to_string();
	}
	if base == "softmax" || base == "sparsesoftmax" {
		return "softmax".to_string();
	}
	if base == "standardize" {
		return "standardize".to_string();
	}
	base
}
...
fn prove_norm() {
	...
	assert!(
		fails.is_empty(),
		"registered norm op(s) FAILED oracle: {:?}",
		fails
	);
