...
unsafe extern "C" {
	fn hsa_init() -> i32;
	fn hsa_iterate_agents(cb: extern "C" fn(Agent, *mut c_void) -> i32, data: *mut c_void) -> i32;
	...
	fn hsa_signal_create(initial_value: i64, num_consumers: u32, consumers: *const Agent, signal: *mut Signal)
	-> i32;
	...
	fn hsa_code_object_reader_create_from_file(file: i32, reader: *mut CodeObjectReader) -> i32;
	...
	fn hsa_executable_symbol_get_info(symbol: Symbol, attribute: i32, value: *mut c_void) -> i32;
	...
	fn hsa_amd_memory_pool_allocate(pool: Pool, size: usize, flags: u32, ptr: *mut *mut c_void) -> i32;
...
extern "C" fn pick_pool(pool: Pool, data: *mut c_void) -> i32 {
	...
	if segment == HSA_SEGMENT_GLOBAL && alloc_ok != 0 && (flags & search.want_flags) == search.want_flags {
...
fn find_agent(want: u32) -> Result<Agent> {
	...
	return search
		.found
		.ok_or_else(|| AsmError(format!("no HSA agent of device type {want}")));
}

fn find_pool(agent: Agent, want_flags: u32) -> Result<Pool> {
	let mut search = PoolSearch {
		want_flags,
		found: None,
	};
...
fn context() -> Result<&'static Ctx> {
	...
	return CTX
		.get()
		.ok_or_else(|| AsmError("asm context vanished after set".to_owned()));
}
...
fn alloc_from(ctx: &Ctx, pool: Pool, bytes: usize, what: &str) -> Result<*mut c_void> {
	...
	let allow = unsafe { hsa_amd_agents_allow_access(1, &raw const ctx.gpu, ptr::null(), ptr_out) };
	if allow != HSA_STATUS_SUCCESS {
		...
		return err(format!(
			"hsa_amd_agents_allow_access({what}): HSA status {allow}"
		));
	}
	return Ok(ptr_out);
}
...
pub unsafe fn free_device(ptr: *mut c_void) -> Result<()> {
	...
	return hsa_check(
		unsafe { hsa_amd_memory_pool_free(ptr) },
		"hsa_amd_memory_pool_free(vram)",
	);
}
...
pub fn load(path: impl AsRef<Path>) -> Result<Program> {
	...
	hsa_check(
		unsafe { hsa_executable_load_agent_code_object(exe, ctx.gpu, reader, ptr::null(), ptr::null_mut()) },
		"hsa_executable_load_agent_code_object",
	)?;
	// SAFETY: exe is a live, loaded executable ready to freeze.
	hsa_check(
		unsafe { hsa_executable_freeze(exe, ptr::null()) },
...
fn resolve(exe: Executable, gpu: Agent, name: &str, hidden: Vec<Hidden>) -> Result<Kernel> {
	...
	hsa_check(
		unsafe { hsa_executable_get_symbol_by_name(exe, symbol_name.as_ptr(), &raw const gpu, &raw mut symbol) },
...
impl Program {
	...
	pub fn launch(&self, name: &str, grid: [u32; 3], block: [u16; 3], args: &KernArgs) -> Result<()> {
	...
	fn dispatch(
		...
		let header =
			HSA_PACKET_TYPE_KERNEL_DISPATCH | (HSA_FENCE_SCOPE_SYSTEM << 9) | (HSA_FENCE_SCOPE_SYSTEM << 11);
		...
		let (base, qsize, doorbell) = unsafe {
			(
				(*ctx.queue).base_address,
				(*ctx.queue).size,
				(*ctx.queue).doorbell_signal,
			)
		};
...
fn fill_hidden(
	...
	for h in hidden {
		...
		unsafe {
			ptr::copy_nonoverlapping(bytes.as_ptr(), kernarg.cast::<u8>().add(h.offset), h.size);
		}
	}
	return Ok(());
}
...
fn alloc_kernarg(ctx: &Ctx, size: u32, bytes: &[u8]) -> Result<*mut c_void> {
	...
	hsa_check(
		unsafe { hsa_amd_memory_pool_allocate(ctx.kernarg_pool, size, 0, &raw mut kernarg) },
	...
	let allow = unsafe { hsa_amd_agents_allow_access(1, &raw const ctx.gpu, ptr::null(), kernarg) };
	if allow != HSA_STATUS_SUCCESS {
		...
		return err(format!(
			"hsa_amd_agents_allow_access(kernarg): HSA status {allow}"
		));
	}
...
fn assemble(path: &Path) -> Result<PathBuf> {
	...
	if ext != "s" {
		return err(format!(
			"unrecognized code-object extension: {}",
			path.display()
		));
	}
