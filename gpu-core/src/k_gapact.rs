...
unsafe extern "C" {
    ...
    fn launch_gapact_elu_backward(g: *const c_void, x: *const c_void, out: *mut c_void, n: i32, alpha: f64, s: *mut c_void);
    fn launch_gapact_selu(x: *const c_void, out: *mut c_void, n: i32, alpha: f64, lambda: f64, s: *mut c_void);
    fn launch_gapact_selu_backward(g: *const c_void, x: *const c_void, out: *mut c_void, n: i32, alpha: f64, lambda: f64, s: *mut c_void);
...
pub fn gpu_elu(x: &GpuBuffer, n: usize, alpha: f64) -> Result<GpuBuffer, HipError> {
    ...
    unsafe { launch_gapact_elu(x.ptr_raw() as *const c_void, o.ptr_raw(), n as i32, alpha, std::ptr::null_mut()); }
    e()?; Ok(o)
}
pub fn gpu_elu_backward(g: &GpuBuffer, x: &GpuBuffer, n: usize, alpha: f64) -> Result<GpuBuffer, HipError> {
    let o = GpuBuffer::alloc(n)?;
    unsafe { launch_gapact_elu_backward(g.ptr_raw() as *const c_void, x.ptr_raw() as *const c_void, o.ptr_raw(), n as i32, alpha, std::ptr::null_mut()); }
    e()?; Ok(o)
}
pub fn gpu_selu(x: &GpuBuffer, n: usize) -> Result<GpuBuffer, HipError> {
    let o = GpuBuffer::alloc(n)?;
    unsafe { launch_gapact_selu(x.ptr_raw() as *const c_void, o.ptr_raw(), n as i32, SELU_ALPHA, SELU_LAMBDA, std::ptr::null_mut()); }
    e()?; Ok(o)
}
pub fn gpu_selu_backward(g: &GpuBuffer, x: &GpuBuffer, n: usize) -> Result<GpuBuffer, HipError> {
    let o = GpuBuffer::alloc(n)?;
    unsafe { launch_gapact_selu_backward(g.ptr_raw() as *const c_void, x.ptr_raw() as *const c_void, o.ptr_raw(), n as i32, SELU_ALPHA, SELU_LAMBDA, std::ptr::null_mut()); }
    e()?; Ok(o)
}

macro_rules! u { ($name:ident, $launch:ident) => {
...
macro_rules! ub { ($name:ident, $launch:ident) => {
    pub fn $name(g: &GpuBuffer, x: &GpuBuffer, n: usize) -> Result<GpuBuffer, HipError> {
        let o = GpuBuffer::alloc(n)?;
        unsafe { $launch(g.ptr_raw() as *const c_void, x.ptr_raw() as *const c_void, o.ptr_raw(), n as i32, std::ptr::null_mut()); }
        e()?; Ok(o)
    }
}; }
macro_rules! gate { ($name:ident, $launch:ident) => {
    pub fn $name(a: &GpuBuffer, b: &GpuBuffer, n: usize) -> Result<GpuBuffer, HipError> {
        let o = GpuBuffer::alloc(n)?;
        unsafe { $launch(a.ptr_raw() as *const c_void, b.ptr_raw() as *const c_void, o.ptr_raw(), n as i32, std::ptr::null_mut()); }
        e()?; Ok(o)
    }
}; }
