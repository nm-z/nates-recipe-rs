...
unsafe extern "C" {
    ...
    // Async transfers
    pub fn hipMemcpyAsync(dst: *mut c_void, src: *const c_void, size: usize, kind: i32, stream: *mut c_void) -> i32;
    ...
    pub fn hipMemcpyPeer(dst: *mut c_void, dst_device: i32, src: *const c_void, src_device: i32, size: usize) -> i32;
...
pub fn memcpy_async(dst: *mut c_void, src: *const c_void, size: usize, kind: i32, stream: *mut c_void) -> Result<(), HipError> {
    check(unsafe { hipMemcpyAsync(dst, src, size, kind, stream) })
}

pub fn memset_async(dst: *mut c_void, value: i32, size: usize, stream: *mut c_void) -> Result<(), HipError> {
    check(unsafe { hipMemsetAsync(dst, value, size, stream) })
}

pub fn memcpy_dtod(dst: *mut c_void, src: *const c_void, size: usize) -> Result<(), HipError> {
...
pub fn host_free(ptr: *mut c_void) -> Result<(), HipError> {
    check(unsafe { hipHostFree(ptr) })
}

pub fn host_register(ptr: *mut c_void, size: usize, flags: u32) -> Result<(), HipError> {
    check(unsafe { hipHostRegister(ptr, size, flags) })
}

pub fn host_unregister(ptr: *mut c_void) -> Result<(), HipError> {
...
pub fn memcpy_peer(dst: *mut c_void, dst_device: i32, src: *const c_void, src_device: i32, size: usize) -> Result<(), HipError> {
    check(unsafe { hipMemcpyPeer(dst, dst_device, src, src_device, size) })
}

pub fn malloc_async(size: usize, stream: *mut c_void) -> Result<*mut c_void, HipError> {
    let mut ptr: *mut c_void = std::ptr::null_mut();
    check(unsafe { hipMallocAsync(&mut ptr, size, stream) })?;
    Ok(ptr)
}

pub fn free_async(ptr: *mut c_void, stream: *mut c_void) -> Result<(), HipError> {
...
impl Drop for Stream {
    fn drop(&mut self) {
        unsafe { hipStreamDestroy(self.raw); }
    }
}
...
impl Event {
    ...
    pub fn record(&self, stream: *mut c_void) -> Result<(), HipError> {
...
impl Drop for Event {
    fn drop(&mut self) {
        unsafe { hipEventDestroy(self.raw); }
    }
}
