...
unsafe extern "C" {
      ...
      fn launch_sum_all(x: *const c_void, out: *mut c_void, workspace: *mut c_void, workspace_bytes: usize, n: i32, stream: *mut c_void);
      fn launch_max_all_workspace_bytes(n: i32) -> usize;
      fn launch_max_all(x: *const c_void, out: *mut c_void, workspace: *mut c_void, workspace_bytes: usize, n: i32, stream: *mut c_void);
      fn launch_min_all_workspace_bytes(n: i32) -> usize;
      fn launch_min_all(x: *const c_void, out: *mut c_void, workspace: *mut c_void, workspace_bytes: usize, n: i32, stream: *mut c_void);
      fn launch_mean_all_workspace_bytes(n: i32) -> usize;
      fn launch_mean_all(x: *const c_void, out: *mut c_void, workspace: *mut c_void, workspace_bytes: usize, n: i32, stream: *mut c_void);
      fn launch_l2_norm_workspace_bytes(n: i32) -> usize;
      fn launch_l2_norm(x: *const c_void, sq: *mut c_void, out: *mut c_void, workspace: *mut c_void, workspace_bytes: usize, n: i32, stream: *mut c_void);
      fn launch_dot_workspace_bytes(n: i32) -> usize;
      fn launch_dot(a: *const c_void, b: *const c_void, prod: *mut c_void, out: *mut c_void, workspace: *mut c_void, workspace_bytes: usize, n: i32, stream: *mut c_void);

      fn launch_fill_sentinel(data: *mut c_void, real_n: i32, padded_n: i32, sentinel: f64, stream: *mut c_void);
      ...
      fn launch_bitonic_step_idx(keys: *mut c_void, vals: *mut c_void, j: i32, k: i32, padded_n: i32, stream: *mut c_void);
      fn launch_bitonic_step_dd(keys: *mut c_void, vals: *mut c_void, j: i32, k: i32, padded_n: i32, stream: *mut c_void);
      ...
      fn launch_cumprod(x: *const c_void, out: *mut c_void, workspace: *mut c_void, workspace_bytes: usize, n: i32, stream: *mut c_void);
      fn launch_cummax_workspace_bytes(n: i32) -> usize;
      fn launch_cummax(x: *const c_void, out: *mut c_void, workspace: *mut c_void, workspace_bytes: usize, n: i32, stream: *mut c_void);
...
fn scalar_reduce(
      ...
      unsafe { f(x.ptr_raw() as *const c_void, out.ptr_raw(), ws.ptr_raw(), wb, ni, std::ptr::null_mut()); }
      check_launch();
      let mut v = [0.0f64];
      out.download(&mut v)?;
      Ok(v[0])
...
pub fn gpu_sort(x: &GpuBuffer, n: usize) -> Result<GpuBuffer, HipError> {
      ...
      if pn > n {
            unsafe { launch_fill_sentinel(work.ptr_raw(), safe_i32(n), safe_i32(pn), f64::MAX, std::ptr::null_mut()); }
      }
      let mut k = 2usize;
      while k <= pn {
            let mut j = k >> 1;
            while j > 0 {
                  unsafe { launch_bitonic_step(work.ptr_raw(), safe_i32(j), safe_i32(k), safe_i32(pn), std::ptr::null_mut()); }
                  j >>= 1;
            }
            k <<= 1;
      }
...
pub fn gpu_argsort(x: &GpuBuffer, n: usize) -> Result<GpuBuffer, HipError> {
      ...
      unsafe { launch_init_idx(vals.ptr_raw(), safe_i32(pn), std::ptr::null_mut()); }
      if pn > n {
            unsafe { launch_fill_sentinel(keys.ptr_raw(), safe_i32(n), safe_i32(pn), f64::MAX, std::ptr::null_mut()); }
      }
      let mut k = 2usize;
      while k <= pn {
            let mut j = k >> 1;
            while j > 0 {
                  unsafe { launch_bitonic_step_idx(keys.ptr_raw(), vals.ptr_raw(), safe_i32(j), safe_i32(k), safe_i32(pn), std::ptr::null_mut()); }
                  j >>= 1;
            }
            k <<= 1;
      }
...
pub fn gpu_sort_by_key(
      ...
      if pn > n {
            unsafe { launch_fill_sentinel(wk.ptr_raw(), safe_i32(n), safe_i32(pn), f64::MAX, std::ptr::null_mut()); }
      }
      let mut k = 2usize;
      while k <= pn {
            let mut j = k >> 1;
            while j > 0 {
                  unsafe { launch_bitonic_step_dd(wk.ptr_raw(), wv.ptr_raw(), safe_i32(j), safe_i32(k), safe_i32(pn), std::ptr::null_mut()); }
                  j >>= 1;
            }
            k <<= 1;
      }
...
pub fn gpu_cumprod(x: &GpuBuffer, n: usize) -> Result<GpuBuffer, HipError> {
      ...
      let ws = GpuBuffer::alloc_bytes(wb)?;
      let out = GpuBuffer::alloc(n)?;
      unsafe { launch_cumprod(x.ptr_raw() as *const c_void, out.ptr_raw(), ws.ptr_raw(), wb, ni, std::ptr::null_mut()); }
      check_launch();
      Ok(out)
}
...
pub fn gpu_cummax(x: &GpuBuffer, n: usize) -> Result<GpuBuffer, HipError> {
      ...
      let ws = GpuBuffer::alloc_bytes(wb)?;
      let out = GpuBuffer::alloc(n)?;
      unsafe { launch_cummax(x.ptr_raw() as *const c_void, out.ptr_raw(), ws.ptr_raw(), wb, ni, std::ptr::null_mut()); }
      check_launch();
      Ok(out)
}
