...
unsafe extern "C" {
      ...
      fn rocblas_dgemm_strided_batched(
            ...
            beta: *const f64,
            C: *mut f64, ldc: i32, stride_c: i64,
            batch_count: i32,
      ) -> i32;
      ...
      fn rocsolver_dgesvd(
            handle: *mut c_void,
            left_svect: u32, right_svect: u32,
            m: i32, n: i32,
            A: *mut f64, lda: i32,
            S: *mut f64,
            U: *mut f64, ldu: i32,
...
pub fn gpu_ddot(a: &GpuBuffer, b: &GpuBuffer, n: usize) -> Result<f64, HipError> {
      ...
      let status = unsafe {
            rocblas_ddot(
                  rocblas_handle(),
                  safe_i32(n),
                  a.ptr_raw() as *const f64, 1,
                  b.ptr_raw() as *const f64, 1,
                  &mut result,
            )
      };
      check(status)?;
...
pub fn gpu_dgemv(a: &GpuBuffer, x: &GpuBuffer, m: usize, n: usize, trans: bool) -> Result<GpuBuffer, HipError> {
      ...
      let status = unsafe {
            rocblas_dgemv(
                  ...
                  rb_m, rb_n,
                  &alpha,
                  a.ptr_raw() as *const f64, rb_m,
                  x.ptr_raw() as *const f64, 1,
                  &beta,
                  out.ptr_raw() as *mut f64, 1,
            )
      };
      check(status)?;
      Ok(out)
...
pub fn gpu_dger(x: &GpuBuffer, y: &GpuBuffer, m: usize, n: usize) -> Result<GpuBuffer, HipError> {
      ...
      let status = unsafe {
            rocblas_dger(
                  rocblas_handle(),
                  n as i32, m as i32,   // m_rb=n, n_rb=m (column-major transposed layout)
                  &alpha,
                  y.ptr_raw() as *const f64, 1,   // "x" operand in rocBLAS
                  x.ptr_raw() as *const f64, 1,   // "y" operand in rocBLAS
                  out.ptr_raw() as *mut f64, n as i32,  // lda=n (stride between col-major cols = n)
            )
      };
      check(status)?;
      Ok(out)
...
pub fn gpu_dsyrk(a: &GpuBuffer, n: usize, k: usize) -> Result<GpuBuffer, HipError> {
      ...
      let status = unsafe {
            rocblas_dsyrk(
                  ...
                  n as i32, k as i32,
                  &alpha,
                  a.ptr_raw() as *const f64, k as i32,  // lda=k: A_rm(k×n) as cm n×k
                  &beta,
                  c.ptr_raw() as *mut f64, n as i32,
            )
      };
      check(status)?;
      Ok(c)
...
pub fn gpu_dgemm_strided_batched(
      ...
      let status = unsafe {
            rocblas_dgemm_strided_batched(
                  ...
                  &alpha,
                  b.ptr_raw() as *const f64, n as i32, stride_b,   // rocBLAS "A" = our B
                  a.ptr_raw() as *const f64, k as i32, stride_a,   // rocBLAS "B" = our A
                  &beta,
                  c.ptr_raw() as *mut f64, n as i32, stride_c,
                  batch as i32,
            )
      };
      check(status)?;
...
pub fn gpu_lu_factor(a: &GpuBuffer, n: usize) -> Result<(GpuBuffer, GpuBuffer), HipError> {
      ...
      let status = unsafe {
            rocsolver_dgetrf(
                  rocblas_handle(),
                  n as i32, n as i32,
                  lu.ptr_raw() as *mut f64, n as i32,
...
pub fn gpu_lu_solve(lu: &GpuBuffer, ipiv: &GpuBuffer, b: &GpuBuffer, n: usize, nrhs: usize) -> Result<GpuBuffer, HipError> {
      ...
      let status = unsafe {
            rocsolver_dgetrs(
                  ...
                  b_copy.ptr_raw() as *mut f64, n as i32,
            )
      };
      check(status)?;
      Ok(b_copy)
...
pub fn gpu_potrs(l: &GpuBuffer, b: &GpuBuffer, n: usize, nrhs: usize) -> Result<GpuBuffer, HipError> {
      let b_copy = gpu_copy_n(b, n * nrhs)?;
      let status = unsafe {
            rocsolver_dpotrs(
                  rocblas_handle(),
...
pub fn gpu_qr(a: &GpuBuffer, m: usize, n: usize) -> Result<(GpuBuffer, GpuBuffer), HipError> {
      ...
      let status = unsafe {
            rocsolver_dgeqrf(
                  rocblas_handle(),
                  m as i32, n as i32,
                  factor.ptr_raw() as *mut f64, m as i32,
      ...
      // factor[j*m+i] holds R[i,j] for i<=j; we pack it into r[j*n+i].
      // Do this on CPU via download/upload — R is tiny (n×n <= 64×64 in practice).
      let mut factor_host = vec![0.0f64; m * n];
      {
            let factor_dl = gpu_copy_n(&factor, m * n)?;
            factor_dl.download(&mut factor_host)?;
      }
      let mut r_host = vec![0.0f64; n * n];
      for j in 0..n {
            for i in 0..=j {
                  r_host[j * n + i] = factor_host[j * m + i];
            }
      }
      let r = GpuBuffer::upload(&r_host)?;
...
pub fn gpu_eigh_sym(a: &GpuBuffer, n: usize) -> Result<(GpuBuffer, GpuBuffer), HipError> {
      ...
      let status = unsafe {
            rocsolver_dsyevd(
                  rocblas_handle(),
                  EVECT_ORIGINAL, FILL_LOWER,
                  n as i32,
                  evecs.ptr_raw() as *mut f64, n as i32,
...
pub fn gpu_svd(a: &GpuBuffer, m: usize, n: usize) -> Result<(GpuBuffer, GpuBuffer, GpuBuffer), HipError> {
      let k   = m.min(n);
      ...
      let status = unsafe {
            rocsolver_dgesvd(
                  ...
                  SVECT_ALL, SVECT_ALL,
                  m as i32, n as i32,
                  a_cm.ptr_raw() as *mut f64, m as i32,
                  s.ptr_raw() as *mut f64,
                  u.ptr_raw() as *mut f64, m as i32,
                  v.ptr_raw() as *mut f64, n as i32,
                  e_work.ptr_raw() as *mut f64,
                  OUTOFPLACE,
                  info.ptr_raw() as *mut i32,
            )
...
// Creates and destroys the plan on every call (no plan cache — compose callers
// that need repeated transforms).
pub fn gpu_fft_c2c_1d(input: &GpuBuffer, n: usize, forward: bool) -> Result<GpuBuffer, HipError> {
      ...
      let transform_type = if forward { ROCFFT_COMPLEX_FORWARD } else { ROCFFT_COMPLEX_INVERSE };
      let lengths = [n];
      ...
      // Query and allocate work buffer if required.
      let mut work_size: usize = 0;
      unsafe { rocfft_plan_get_work_buffer_size(plan as *const c_void, &mut work_size); }

      let work_buf: Option<GpuBuffer>;
      let exec_info_handle: *mut c_void;

      if work_size > 0 {
            let wb = GpuBuffer::alloc_bytes(work_size)?;
            let mut einfo: *mut c_void = std::ptr::null_mut();
            unsafe { rocfft_execution_info_create(&mut einfo); }
            unsafe {
                  rocfft_execution_info_set_work_buffer(einfo, wb.ptr_raw(), work_size);
            }
            work_buf = Some(wb);
            exec_info_handle = einfo;
      } else {
            work_buf = None;
            exec_info_handle = std::ptr::null_mut();
      }
      ...
      if !exec_info_handle.is_null() {
            unsafe { rocfft_execution_info_destroy(exec_info_handle); }
      }
      drop(work_buf);
      unsafe { rocfft_plan_destroy(plan); }
