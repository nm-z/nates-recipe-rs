...
fn main() -> Result<ExitCode, Errored> {
	...
	math_ops::gpu_rsqrt(&x, n, &x).map_err(|e| return Errored::new(format!("probe: kernel: {e}")))?;
	...
	hip::device_synchronize().map_err(|e| return Errored::new(format!("probe: download sync: {e}")))?;
	...
	hip::device_synchronize().map_err(|e| return Errored::new(format!("probe: device sync: {e}")))?;
	set_opt(Opt {
		gpu: true,
		..Opt::default()
	});
