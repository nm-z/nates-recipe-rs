...
fn main() -> Result<(), Errored> {
	...
	x.load(&host)
		.map_err(|e| Errored::new(format!("probe: upload: {e}")))?;
	gpu_core::math_ops::gpu_rsqrt(&x, n, &x)
		.map_err(|e| Errored::new(format!("probe: kernel: {e}")))?;
	...
	gpu_core::hip::device_synchronize()
		.map_err(|e| Errored::new(format!("probe: download sync: {e}")))?;
	for i in 0..back.len() {
		let v = back[i];
	...
	gpu_core::hip::device_synchronize()
		.map_err(|e| Errored::new(format!("probe: device sync: {e}")))?;
	set_opt(Opt {
		gpu: true,
		..Opt::default()
	});
