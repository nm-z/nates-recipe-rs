...
unsafe extern "C" {
      fn launch_bin_edges_uniform(
            x: *const c_void, edges: *mut c_void,
            rows: i32, cols: i32, n_bins: i32,
            stream: *mut c_void,
      );
      fn launch_bin_edges_quantile(
            x: *const c_void, edges: *mut c_void, tmp_col_buf: *mut c_void,
            rows: i32, cols: i32, n_bins: i32,
            stream: *mut c_void,
      );
      fn launch_quantize_features(
            x: *const c_void, edges: *const c_void, out: *mut c_void,
            rows: i32, cols: i32, n_bins: i32,
            stream: *mut c_void,
      );
      fn launch_one_hot(
            labels: *const c_void, out: *mut c_void,
            n: i32, n_classes: i32,
            stream: *mut c_void,
      );
...
unsafe extern "C" {
      fn launch_pairwise_cosine(
            query: *const c_void, train: *const c_void, out: *mut c_void,
            nq: i32, nt: i32, dim: i32,
            stream: *mut c_void,
      );
...
pub fn gpu_bin_edges_uniform(
      ...
      unsafe {
            launch_bin_edges_uniform(
                  ...
                  out.ptr_raw(),
                  rows as i32, cols as i32, n_bins as i32,
                  std::ptr::null_mut(),
            );
      }
      check_launch();
...
pub fn gpu_run_length(
      x: &GpuBuffer,
      n: usize,
) -> Result<(GpuBuffer, GpuBuffer, usize), HipError> {
...
pub fn gpu_pairwise_cosine(
      ...
      unsafe {
            launch_pairwise_cosine(
                  ...
                  out.ptr_raw(),
                  nq as i32, nt as i32, dim as i32,
                  std::ptr::null_mut(),
            );
      }
      check_launch();
