...
impl InferenceBlock for CheckpointLstmImage {
...
impl DeclaredBlock for DenseLstm {
	...
	fn compile_forward(
		...
		let result: TrainingCompileResult<(ValueId, LstmValues)> = {
			...
			let initialize_gate = |compiler: &mut GraphCompiler| {
			...
			let (input_gate_input_weight, input_gate_recurrent_weight, input_gate_bias) =
				initialize_gate(compiler)?;
			let (forget_gate_input_weight, forget_gate_recurrent_weight, forget_gate_bias) =
				initialize_gate(compiler)?;
			let (output_gate_input_weight, output_gate_recurrent_weight, output_gate_bias) =
				initialize_gate(compiler)?;
			...
			let domain = compiler.training_domain;
			let (hidden, steps) = lower_lstm_sequence(
				&mut TrainingForwardGraph { compiler, domain },
				input,
				rows,
				sequence_length.get(),
				width.get(),
		...
		let (output, tape) = result?;
		Ok(BlockForward {
			output,
			logical: context.logical.recurrent(self.width())?,
			routing: None,
			tape: Box::new(tape),
		})
	}
...
impl RealizedBlock for RealizedLstm {
	fn clone_box(&self) -> Box<dyn RealizedBlock> {
		Box::new(self.clone())
	}

	fn visit_parameter_states(&self, visit: &mut dyn FnMut(ParameterState)) {
		visit(self.state.input_gate_input_weight);
		visit(self.state.input_gate_recurrent_weight);
		visit(self.state.input_gate_bias);
		visit(self.state.forget_gate_input_weight);
		visit(self.state.forget_gate_recurrent_weight);
		visit(self.state.forget_gate_bias);
		visit(self.state.output_gate_input_weight);
		visit(self.state.output_gate_recurrent_weight);
		visit(self.state.output_gate_bias);
		visit(self.state.candidate_input_weight);
		visit(self.state.candidate_recurrent_weight);
	...
	fn compile_validation(
		...
		let result: TrainingCompileResult<ValueId> = {
			compiler.require_tensor(
				input,
				DType::F32,
		...
		Ok(BlockValidation {
			output: result?,
			logical: context.logical.recurrent(self.declaration.width())?,
			routing: None,
		})
	}
...
impl CompiledBlock for LstmValues {
	fn backward(
		&self,
		compiler: &mut GraphCompiler,
		...
		let result: TrainingCompileResult<Vec<ParameterGradient>> = {
			compiler.require_tensor(
				gradient,
				DType::F32,
				&[rows, self.width.get()],
			...
			let tensor_shape = shape(&[rows, self.width.get()])?;
			...
			for step in self.steps.iter().rev() {
				let output_gate_gradient = compiler.tensor(DType::F32, tensor_shape.clone())?;
				let cell_activation_gradient = compiler.tensor(DType::F32, tensor_shape.clone())?;
				...
				let cell_from_hidden = compiler.backward_activation(
					cell_activation_gradient,
					step.cell,
					step.cell_activation,
					DenseActivation::Tanh,
				)?;
				let cell_from_hidden =
					compiler.mask_f32_with_zero(cell_from_hidden, validity, compiler.training_domain)?;
				let total_cell_gradient =
				...
				let candidate_gradient = compiler.tensor(DType::F32, tensor_shape.clone())?;
				compiler.emit_elementwise(
					vec![
						total_cell_gradient,
						step.forget_gate,
				...
				let input_gate_preactivation_gradient = compiler.backward_activation(
					input_gate_gradient,
					step.input_gate_preactivation,
					step.input_gate,
					DenseActivation::Sigmoid,
				)?;
				let input_gate_preactivation_gradient = compiler.mask_f32_with_zero(
					input_gate_preactivation_gradient,
					validity,
					compiler.training_domain,
				)?;
				let forget_gate_preactivation_gradient = compiler.backward_activation(
					forget_gate_gradient,
					step.forget_gate_preactivation,
					step.forget_gate,
					DenseActivation::Sigmoid,
				)?;
				let forget_gate_preactivation_gradient = compiler.mask_f32_with_zero(
					forget_gate_preactivation_gradient,
					validity,
					compiler.training_domain,
				)?;
				let output_gate_preactivation_gradient = compiler.backward_activation(
					output_gate_gradient,
					step.output_gate_preactivation,
					step.output_gate,
					DenseActivation::Sigmoid,
				)?;
				let output_gate_preactivation_gradient = compiler.mask_f32_with_zero(
					output_gate_preactivation_gradient,
					validity,
					compiler.training_domain,
				)?;
				let candidate_preactivation_gradient = compiler.backward_activation(
					candidate_gradient,
					step.candidate_preactivation,
					step.candidate,
					DenseActivation::Tanh,
				)?;
				let candidate_preactivation_gradient = compiler.mask_f32_with_zero(
					candidate_preactivation_gradient,
					validity,
					compiler.training_domain,
				)?;

				let input_gate = compiler.recurrent_gate_backward(
					[
						step.input,
						step.previous_hidden,
						input_gate_preactivation_gradient,
						self.input_gate_recurrent_weight.value,
					],
					[rows, self.width.get()],
					tree_lanes,
				)?;
				let forget_gate = compiler.recurrent_gate_backward(
					[
						step.input,
						step.previous_hidden,
						forget_gate_preactivation_gradient,
						self.forget_gate_recurrent_weight.value,
					],
					[rows, self.width.get()],
					tree_lanes,
				)?;
				let output_gate = compiler.recurrent_gate_backward(
					[
						step.input,
						step.previous_hidden,
						output_gate_preactivation_gradient,
						self.output_gate_recurrent_weight.value,
					],
					[rows, self.width.get()],
					tree_lanes,
				)?;
				let candidate = compiler.recurrent_gate_backward(
					[
						step.input,
						step.previous_hidden,
						candidate_preactivation_gradient,
						self.candidate_recurrent_weight.value,
					],
					[rows, self.width.get()],
					tree_lanes,
				)?;

				compiler.accumulate_recurrent_gradient(
				...
				let previous_hidden_gradient = compiler.elementwise_f32(
					tensor_shape.clone(),
					vec![
			...
			Ok([
				(
					...
					input_gate_input_weight_gradient,
				),
				(
					ParameterRole::LstmInputGateRecurrentWeight,
					input_gate_recurrent_weight_gradient,
				),
				(ParameterRole::LstmInputGateBias, input_gate_bias_gradient),
				(
					ParameterRole::LstmForgetGateInputWeight,
					forget_gate_input_weight_gradient,
				),
				(
					ParameterRole::LstmForgetGateRecurrentWeight,
					forget_gate_recurrent_weight_gradient,
				),
				(ParameterRole::LstmForgetGateBias, forget_gate_bias_gradient),
				(
					ParameterRole::LstmOutputGateInputWeight,
					output_gate_input_weight_gradient,
				),
				(
					ParameterRole::LstmOutputGateRecurrentWeight,
					output_gate_recurrent_weight_gradient,
				),
				(ParameterRole::LstmOutputGateBias, output_gate_bias_gradient),
				(
					ParameterRole::LstmCandidateInputWeight,
					candidate_input_weight_gradient,
				),
				(
					ParameterRole::LstmCandidateRecurrentWeight,
					candidate_recurrent_weight_gradient,
				),
				(ParameterRole::LstmCandidateBias, candidate_bias_gradient),
			]
			.into_iter()
			.map(|(role, value)| {
				ParameterGradient::new(
					role,
					value.expect("nonzero LSTM sequence has every gradient"),
				)
			})
			.collect())
		};
	...
	fn optimize(
		...
		Ok(DenseBlockState::from_realized(Box::new(RealizedLstm {
			declaration: self.declaration,
			state: DenseLstmState {
				sequence_length: self.sequence_length,
				width: self.width,
				input_gate_input_weight: updates.apply(
					compiler,
					ParameterRole::LstmInputGateInputWeight,
					self.input_gate_input_weight,
				)?,
				input_gate_recurrent_weight: updates.apply(
					compiler,
					ParameterRole::LstmInputGateRecurrentWeight,
					self.input_gate_recurrent_weight,
				)?,
				input_gate_bias: updates.apply(
					compiler,
					ParameterRole::LstmInputGateBias,
					self.input_gate_bias,
				)?,
				forget_gate_input_weight: updates.apply(
					compiler,
					ParameterRole::LstmForgetGateInputWeight,
					self.forget_gate_input_weight,
				)?,
				forget_gate_recurrent_weight: updates.apply(
					compiler,
					ParameterRole::LstmForgetGateRecurrentWeight,
					self.forget_gate_recurrent_weight,
				)?,
				forget_gate_bias: updates.apply(
					compiler,
					ParameterRole::LstmForgetGateBias,
					self.forget_gate_bias,
				)?,
				output_gate_input_weight: updates.apply(
					compiler,
					ParameterRole::LstmOutputGateInputWeight,
					self.output_gate_input_weight,
				)?,
				output_gate_recurrent_weight: updates.apply(
					compiler,
					ParameterRole::LstmOutputGateRecurrentWeight,
					self.output_gate_recurrent_weight,
				)?,
				output_gate_bias: updates.apply(
					compiler,
					ParameterRole::LstmOutputGateBias,
					self.output_gate_bias,
				)?,
				candidate_input_weight: updates.apply(
					compiler,
					ParameterRole::LstmCandidateInputWeight,
					self.candidate_input_weight,
				)?,
				candidate_recurrent_weight: updates.apply(
					compiler,
					ParameterRole::LstmCandidateRecurrentWeight,
					self.candidate_recurrent_weight,
				)?,
				candidate_bias: updates.apply(
					compiler,
					ParameterRole::LstmCandidateBias,
					self.candidate_bias,
				)?,
			},
		})))
