...
impl InferenceBlock for CheckpointResidualImage {
	fn compile_inference(
		...
		Ok(BlockInference {
			output: compiler.compile_residual(self, context)?,
			width,
...
impl DeclaredBlock for DenseResidual {
	...
	fn compile_forward(
		...
		let residual_input = context.input;
		let residual_input_width = context.logical.width()?;
		...
		for operation in self.branch() {
			match operation {
				DenseResidualOperation::Layer(layer) => {
					let (output, tape) = layer.compile_layer_forward(
						compiler,
						LayerForwardContext {
							input: branch_current,
					...
					branch.push(ResidualBranchValues::Layer(tape));
				}
				DenseResidualOperation::Operation(operation) => {
					let (output, tape) = compiler.compile_training_operation(
						branch_current,
						*operation,
						branch_width,
		...
		let (skip, projection) = match residual_input_width == output_width.get() {
			true => (residual_input, None),
			false => {
				let projection = compiler.initialize_weight(
					shape(&[residual_input_width, output_width.get()])?,
					residual_input_width,
					context.config.random_seed,
				)?;
				let skip = compiler.bias_free_linear(
					residual_input,
					projection.value,
		...
		let mut output = compiler.exact_add(branch_current, skip, compiler.training_domain)?;
		let mut operations = Vec::with_capacity(self.operations().len());
		for operation in self.operations().iter().copied() {
			let (next, tape) = compiler.compile_training_operation(
				output,
				operation,
				output_width.get(),
				(context.partition_rows, context.validity, context.valid_count, context.config),
		...
		Ok(BlockForward {
			output,
			logical: LogicalFeatureShape::from_width(output_width.get())?,
			routing: None,
			tape: Box::new(ResidualValues {
...
impl RealizedBlock for RealizedResidual {
	...
	fn visit_parameter_states(&self, visit: &mut dyn FnMut(ParameterState)) {
		...
		self.state
			...
			.copied()
			.for_each(&mut *visit);
		self.state.projection.iter().copied().for_each(&mut *visit);
	...
	fn compile_validation(
		...
		let mut layer_states = self.state.branch.iter();
		let mut branch_prelu = self.state.branch_prelu.iter();
		for operation in self.declaration.branch() {
			match operation {
				DenseResidualOperation::Layer(layer) => {
					branch = layer.compile_layer_validation(
						compiler,
				...
				DenseResidualOperation::Operation(operation) => {
					...
					branch = compiler.apply_validation_operation(
						branch,
						*operation,
						prelu,
						[
		...
		let output_width = self
			...
			.output_width()
			.expect("validated residual output width");
		let skip = match self.state.projection {
			None => residual_input,
			Some(projection) => compiler.bias_free_linear(
				residual_input,
		...
		let mut output = compiler.exact_add(branch, skip, context.domain)?;
		let mut prelu = self.state.prelu.iter();
		for operation in self.declaration.operations().iter().copied() {
			let parameter = validation_prelu_parameter(operation, &mut prelu);
			output = compiler.apply_validation_operation(
				output,
				operation,
				parameter,
				[context.rows, output_width.get()],
				context.config,
				context.domain,
			)?;
		}
...
impl CompiledBlock for ResidualValues {
	fn backward(
		&self,
		compiler: &mut GraphCompiler,
		...
		let mut post_gradients = vec![None; self.operations.len()];
		...
		let mut branch_parameters = vec![None; self.branch.len()];
		let mut branch_gradient = merge_gradient;
		for branch_index in (0..self.branch.len()).rev() {
			match &self.branch[branch_index] {
				ResidualBranchValues::Operation(operation) => {
					...
					branch_parameters[branch_index] = Some(parameter_gradient
						.into_iter()
						.map(|value| ParameterGradient::new(ParameterRole::PReluSlope, value))
						.collect());
				}
				ResidualBranchValues::Layer(layer) => {
					let result = layer.compile_layer_backward(
						compiler,
						BlockBackwardContext {
							gradient: branch_gradient,
							input_gradient: InputGradientRequirement::Required,
							..context
						},
					)?;
					branch_gradient = result
						.input_gradient
		...
		let (projection_gradient, skip_gradient) = match self.projection {
			Some(projection) => {
				let safe_input =
					compiler.mask_f32_with_zero(self.input, context.validity, compiler.training_domain)?;
				let safe_gradient = compiler.mask_f32_with_zero(
					merge_gradient,
					context.validity,
					compiler.training_domain,
				)?;
				let weight_shape = compiler.tensor_ref(projection.value)?.shape.clone();
				let weight_gradient = compiler.tensor(DType::F32, weight_shape)?;
				compiler.emit(
					vec![safe_input, safe_gradient],
					vec![weight_gradient],
				...
				let input_gradient = compiler.tensor(DType::F32, input_shape)?;
				compiler.emit(
					vec![safe_gradient, projection.value],
					vec![input_gradient],
					PrimitiveKind::Contraction(Contraction {
						batch_axes: Vec::new(),
		...
		let mut parameters = branch_parameters
			.into_iter()
			.flat_map(|parameters| parameters.expect("residual branch emits its parameter tape"))
			.collect::<Vec<_>>();
		parameters.extend(
			projection_gradient
				.map(|value| ParameterGradient::new(ParameterRole::ResidualProjectionWeight, value)),
		);
		parameters.extend(
			post_gradients
				.into_iter()
	...
	fn optimize(
		...
		Ok(DenseBlockState::from_realized(Box::new(RealizedResidual {
