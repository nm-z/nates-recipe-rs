...
impl InferenceBlock for CheckpointConvolutionImage {
	fn compile_inference(
		...
		Ok(BlockInference {
			output: compiler.compile_convolution(
				context.block_index,
				self,
				context.input,
				[
...
impl DeclaredBlock for DenseConvolution {
	...
	fn compile_forward(
		...
		let (logical, geometry) = context.logical.convolved(self)?;
		...
		let bias = compiler.initialize_zero_parameter(shape(&[geometry.filters().get()])?)?;
		let (mut output, preparation, columns, output_group_indices) = compiler.compile_convolution_forward(
			[safe_input, weight.value, bias.value],
			context.partition_rows,
			geometry,
			ExternalInputRole::TrainingConvolutionWindowIndices {
		...
		Ok(BlockForward {
			...
			logical,
			routing: None,
			tape: Box::new(ConvolutionValues {
...
impl RealizedBlock for RealizedConvolution {
	...
	fn compile_validation(
		...
		let output_width = self
			...
			.output_width()
			.expect("validated convolution output width");
		let (mut output, ..) = compiler.compile_convolution_forward(
			[
				context.input,
				self.state.weight.updated_parameter,
				self.state.bias.updated_parameter,
			],
			context.rows,
			self.state.geometry,
			ExternalInputRole::ValidationConvolutionWindowIndices {
				block: context.block_index,
			},
			context.domain,
		)?;
		...
		Ok(BlockValidation {
			output,
			logical: context.logical.convolved(&self.declaration)?.0,
			routing: None,
		})
	}
...
impl CompiledBlock for ConvolutionValues {
	fn backward(
		&self,
		compiler: &mut GraphCompiler,
		...
		let grouped_gradient = compiler.gather(
			...
			shape(&self.preparation.output_shape())?,
			1,
			compiler.training_domain,
		)?;
		let weight_gradient = compiler.tensor(
			DType::F32,
			shape(&[
				self.geometry.kernel().get(),
				self.geometry.input_channels().get(),
				self.geometry.filters().get(),
			])?,
		)?;
		compiler.emit(
			vec![self.columns, grouped_gradient],
			vec![weight_gradient],
		...
		let bias_gradient = compiler.sum_tensor(
			grouped_gradient,
			shape(&[self.geometry.filters().get()])?,
			&[0, 1],
			context.tree_lanes,
			compiler.training_domain,
		)?;
		let input_gradient =
			match context.input_gradient {
				InputGradientRequirement::Required => {
					let (flat_gradient, _) = compiler.pack_matrix_to_flat(
						gradient,
						context.partition_rows,
						output_width.get(),
						compiler.training_domain,
					)?;
					let contribution_shape = shape(&self.preparation.input_gradient_indices_shape())?;
					let contribution_indices = compiler.external_i32_tensor(
						ExternalInputRole::TrainingConvolutionInputGradientIndices {
							block: context.block_index,
						},
						contribution_shape.clone(),
						self.preparation.input_gradient_indices(),
					)?;
					let contribution_validity = compiler.external_f32_tensor(
						ExternalInputRole::TrainingConvolutionInputGradientValidity {
							block: context.block_index,
						},
						contribution_shape.clone(),
						self.preparation.input_gradient_validity(),
					)?;
					let contributions = compiler.gather(
						flat_gradient,
					...
					let valid_contributions = compiler.elementwise_f32(
						contribution_shape,
						vec![contributions, contribution_validity],
						masked_zero_f32_program()?,
						compiler.training_domain,
					)?;
					let grouped_input_gradient = compiler.tensor(
						DType::F32,
						shape(&[
							context.partition_rows,
							self.geometry.input_length().get(),
							self.geometry.input_channels().get(),
						])?,
					)?;
					compiler.emit(
						vec![valid_contributions, self.weight.value],
					...
					let (input_gradient, _) = compiler.unpack_pool_to_matrix(
					...
					Some(compiler.mask_f32_with_zero(
						input_gradient,
						context.validity,
						compiler.training_domain,
					)?)
	...
	fn optimize(
		...
		let weight = updates.apply(compiler, ParameterRole::ConvolutionWeight, self.weight)?;
		let bias = updates.apply(compiler, ParameterRole::ConvolutionBias, self.bias)?;
		let prelu = compiler.update_operation_parameters(&self.operations, updates)?;
		Ok(DenseBlockState::from_realized(Box::new(RealizedConvolution {
