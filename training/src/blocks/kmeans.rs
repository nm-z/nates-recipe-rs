...
impl InferenceBlock for CheckpointKMeansImage {
	fn compile_inference(
		...
		let width = self.clusters().get();
		Ok(BlockInference {
			output: compiler.compile_kmeans(
				context.block_index,
				self,
				context.input,
				context.rows,
...
impl DeclaredBlock for DenseKMeans {
	...
	fn compile_forward(
		...
		let input_width = context.logical.width()?;
		compiler.require_tensor(
			context.input,
			DType::F32,
			&[context.partition_rows, input_width],
		...
		let initialization_first_value = compiler.next_value;
		let initialization_first_kernel = compiler.next_kernel;
		...
		let initialization = materialize_kmeans_initialization(&KMeansInitializationRequest {
			points: compiler.tensor_ref(context.input)?.clone(),
			centroids: compiler.tensor_ref(fresh_centroids)?.clone(),
		...
		let resumed_centroids = compiler.external_f32_zeros(
			ExternalInputRole::ResumeKMeansCentroids {
		...
		let updated_centroids = compiler.tensor(DType::F32, centroid_shape)?;
		let output = compiler.tensor(
			DType::F32,
			shape(&[context.partition_rows, clusters.get()])?,
		)?;
		let lloyd_requirements =
			kmeans_lloyd_requirements(context.partition_rows, input_width.get(), clusters.get())?;
		let lloyd_first_value = compiler.next_value;
		let lloyd_first_kernel = compiler.next_kernel;
		compiler.next_value = compiler
			.next_value
		...
		let lloyd = materialize_kmeans_lloyd_step(&KMeansLloydRequest {
			...
			updated_centroids: compiler.tensor_ref(updated_centroids)?.clone(),
			distances: compiler.tensor_ref(output)?.clone(),
			tree_lanes: context.config.reduction_tree_lanes,
			identity_namespace: IdentityNamespace::new(
		...
		compiler.external_outputs.insert(updated_centroids);
		...
		Ok(BlockForward {
			...
			routing: self.routing().map(|routing| (routing, NonZeroU64::MIN)),
			tape: Box::new(KMeansValues {
				declaration: *self,
				input: context.input,
				distances: output,
				state,
...
impl RealizedBlock for RealizedKMeans {
	...
	fn compile_validation(
		...
		compiler.require_tensor(
			context.input,
			DType::F32,
			&[context.rows, input_width],
			"validation K-means input",
		)?;
		compiler.require_tensor(
			self.state.updated_centroids,
			DType::F32,
			&[self.state.clusters.get(), input_width],
			"validation K-means centroids",
		)?;
		let output = compiler.tensor(
			DType::F32,
			shape(&[context.rows, self.state.clusters.get()])?,
		)?;
		let parameters = [
			...
			(
				"tree_lanes".to_owned(),
				PreparedParameter::U64(u64::from(context.config.reduction_tree_lanes)),
			),
		]
		.into_iter()
		...
		Ok(BlockValidation {
			output,
			logical: LogicalFeatureShape::from_width(self.declaration.clusters().get())?,
			routing: self
				.declaration
				.routing()
...
impl CompiledBlock for KMeansValues {
	fn backward(
		&self,
		compiler: &mut GraphCompiler,
		...
		let input_gradient = match context.input_gradient {
			InputGradientRequirement::Omit => None,
			InputGradientRequirement::Required => {
				let input_width = self.state.input_width.get();
				let clusters = self.state.clusters.get();
				compiler.require_tensor(
					context.gradient,
					DType::F32,
					&[context.partition_rows, clusters],
					"K-means output gradient",
				)?;
				let matrix_shape = shape(&[context.partition_rows, clusters])?;
				let scaled_gradient = compiler.elementwise_f32(
					matrix_shape,
					vec![context.gradient, self.distances],
					kmeans_distance_gradient_scale_program(context.epsilon)?,
					compiler.training_domain,
				)?;
				let row_scales = compiler.sum_tensor(
					scaled_gradient,
					shape(&[context.partition_rows, 1])?,
					&[1],
					context.tree_lanes,
					compiler.training_domain,
				)?;
				let input_term = compiler.elementwise_f32(
					shape(&[context.partition_rows, input_width])?,
					vec![self.input, row_scales],
					multiply_program()?,
					compiler.training_domain,
				)?;
				let centroid_term =
				...
				compiler.emit(
					vec![scaled_gradient, self.state.updated_centroids],
					vec![centroid_term],
				...
				let gradient = compiler.elementwise_f32(
					shape(&[context.partition_rows, input_width])?,
					vec![input_term, centroid_term],
					subtract_program()?,
					compiler.training_domain,
				)?;
				Some(compiler.mask_f32_with_zero(gradient, context.validity, compiler.training_domain)?)
			}
		};
		Ok(BlockBackward {
			input_gradient,
	...
	fn optimize(
		...
		Ok(DenseBlockState::from_realized(Box::new(RealizedKMeans {
