...
impl InferenceBlock for CheckpointGruImage {
...
impl DeclaredBlock for DenseGru {
	...
	fn compile_forward(
		...
		let result: TrainingCompileResult<(ValueId, GruValues)> = {
			...
			let reset_input_weight =
				compiler.initialize_weight(shape(&[1, width.get()])?, 1, config.random_seed)?;
			let reset_recurrent_weight = compiler.initialize_weight(
				shape(&[width.get(), width.get()])?,
				width.get(),
				config.random_seed,
			)?;
			let reset_bias = compiler.initialize_zero_parameter(shape(&[width.get()])?)?;
			let update_input_weight =
				compiler.initialize_weight(shape(&[1, width.get()])?, 1, config.random_seed)?;
			let update_recurrent_weight = compiler.initialize_weight(
				shape(&[width.get(), width.get()])?,
				width.get(),
				config.random_seed,
			)?;
			let update_bias = compiler.initialize_zero_parameter(shape(&[width.get()])?)?;
			let candidate_input_weight =
				compiler.initialize_weight(shape(&[1, width.get()])?, 1, config.random_seed)?;
			let candidate_recurrent_weight = compiler.initialize_weight(
				shape(&[width.get(), width.get()])?,
				width.get(),
				config.random_seed,
			)?;
			let candidate_bias = compiler.initialize_zero_parameter(shape(&[width.get()])?)?;
...
impl RealizedBlock for RealizedGru {
	fn clone_box(&self) -> Box<dyn RealizedBlock> {
		Box::new(self.clone())
	}

	fn visit_parameter_states(&self, visit: &mut dyn FnMut(ParameterState)) {
		visit(self.state.reset_input_weight);
		visit(self.state.reset_recurrent_weight);
		visit(self.state.reset_bias);
		visit(self.state.update_input_weight);
		visit(self.state.update_recurrent_weight);
		visit(self.state.update_bias);
...
impl CompiledBlock for GruValues {
	fn backward(
		&self,
		compiler: &mut GraphCompiler,
		...
		let result: TrainingCompileResult<Vec<ParameterGradient>> = {
			...
			let hidden_shape = shape(&[rows, self.width.get()])?;
			...
			for step in self.steps.iter().rev() {
				let candidate_gradient = compiler.tensor(DType::F32, hidden_shape.clone())?;
				let update_gradient = compiler.tensor(DType::F32, hidden_shape.clone())?;
				let direct_previous_hidden_gradient = compiler.tensor(DType::F32, hidden_shape.clone())?;
				...
				let step_candidate_input_weight_gradient = compiler.gru_matrix_gradient(
					step.input,
					candidate_preactivation_gradient,
					1,
					self.width.get(),
				...
				let step_candidate_recurrent_weight_gradient = compiler.gru_matrix_gradient(
					step.reset_hidden,
					candidate_preactivation_gradient,
					self.width.get(),
					self.width.get(),
				...
				let step_candidate_bias_gradient = compiler.gru_bias_gradient(
					candidate_preactivation_gradient,
					self.width.get(),
					tree_lanes,
				)?;
				compiler.accumulate_recurrent_gradient(
					&mut candidate_bias_gradient,
					step_candidate_bias_gradient,
				)?;

				let reset_hidden_gradient = compiler.gru_projection_input_gradient(
					candidate_preactivation_gradient,
				...
				let reset_preactivation_gradient = compiler.backward_activation(
					reset_gradient,
					step.reset_preactivation,
					step.reset,
					DenseActivation::Sigmoid,
				)?;
				let reset_preactivation_gradient = compiler.mask_f32_with_zero(
					reset_preactivation_gradient,
					validity,
					compiler.training_domain,
				)?;
				let step_reset_input_weight_gradient = compiler.gru_matrix_gradient(
					step.input,
					reset_preactivation_gradient,
					1,
					self.width.get(),
				...
				let step_reset_recurrent_weight_gradient = compiler.gru_matrix_gradient(
					step.previous_hidden,
					reset_preactivation_gradient,
					self.width.get(),
					self.width.get(),
				...
				let update_preactivation_gradient = compiler.backward_activation(
					update_gradient,
					step.update_preactivation,
					step.update,
					DenseActivation::Sigmoid,
				)?;
				let update_preactivation_gradient = compiler.mask_f32_with_zero(
					update_preactivation_gradient,
					validity,
					compiler.training_domain,
				)?;
				let step_update_input_weight_gradient = compiler.gru_matrix_gradient(
					step.input,
					update_preactivation_gradient,
					1,
					self.width.get(),
				...
				let step_update_recurrent_weight_gradient = compiler.gru_matrix_gradient(
					step.previous_hidden,
					update_preactivation_gradient,
					self.width.get(),
					self.width.get(),
				...
				let step_update_bias_gradient = compiler.gru_bias_gradient(
					update_preactivation_gradient,
					self.width.get(),
					tree_lanes,
				)?;
				...
				let reset_previous_hidden_gradient = compiler.gru_projection_input_gradient(
					reset_preactivation_gradient,
					self.reset_recurrent_weight.value,
					rows,
					self.width.get(),
				)?;
				let update_previous_hidden_gradient = compiler.gru_projection_input_gradient(
					update_preactivation_gradient,
					self.update_recurrent_weight.value,
					rows,
					self.width.get(),
				)?;
			...
			Ok([
				(
					...
					reset_input_weight_gradient,
				),
				(
					ParameterRole::GruResetRecurrentWeight,
					reset_recurrent_weight_gradient,
				),
				(ParameterRole::GruResetBias, reset_bias_gradient),
				(
					ParameterRole::GruUpdateInputWeight,
					update_input_weight_gradient,
				),
				(
					ParameterRole::GruUpdateRecurrentWeight,
					update_recurrent_weight_gradient,
				),
				(ParameterRole::GruUpdateBias, update_bias_gradient),
				(
				...
				(ParameterRole::GruCandidateBias, candidate_bias_gradient),
			]
			.into_iter()
			.map(|(role, value)| {
				ParameterGradient::new(
					role,
					value.expect("nonzero GRU sequence has every gradient"),
				)
			})
			.collect())
		};
	...
	fn optimize(
		...
		Ok(DenseBlockState::from_realized(Box::new(RealizedGru {
			declaration: self.declaration,
			state: DenseGruState {
				sequence_length: self.sequence_length,
				width: self.width,
				reset_input_weight: updates.apply(
					compiler,
					ParameterRole::GruResetInputWeight,
					self.reset_input_weight,
				)?,
				reset_recurrent_weight: updates.apply(
					compiler,
					ParameterRole::GruResetRecurrentWeight,
					self.reset_recurrent_weight,
				)?,
				reset_bias: updates.apply(compiler, ParameterRole::GruResetBias, self.reset_bias)?,
				update_input_weight: updates.apply(
					compiler,
					ParameterRole::GruUpdateInputWeight,
					self.update_input_weight,
				)?,
				update_recurrent_weight: updates.apply(
					compiler,
					ParameterRole::GruUpdateRecurrentWeight,
					self.update_recurrent_weight,
				)?,
				update_bias: updates.apply(compiler, ParameterRole::GruUpdateBias, self.update_bias)?,
				...
				candidate_bias: updates.apply(
					compiler,
					ParameterRole::GruCandidateBias,
					self.candidate_bias,
				)?,
			},
		})))
