...
impl InferenceBlock for CheckpointEmbeddingImage {
	...
	fn compile_inference(
		...
		Ok(BlockInference {
			output: compiler.compile_embedding(
				context.block_index,
				self,
				context.input,
				context.rows,
...
impl DeclaredBlock for DenseEmbedding {
	...
	fn compile_forward(
		...
		let logical = context.logical.embedded(*self)?;
		let table = compiler.initialize_weight(
			shape(&[self.vocabulary().get(), self.dimensions().get()])?,
			self.dimensions().get(),
			context.config.random_seed,
		)?;
		let (output, indices) = compiler.compile_embedding_forward(
			context.input,
			context.partition_rows,
			context.logical.length.get(),
			self.dimensions().get(),
			table.value,
		...
		Ok(BlockForward {
			...
			logical,
			routing: None,
			tape: Box::new(EmbeddingValues {
				declaration: *self,
				indices,
				table,
				sequence_length: context.logical.length,
	...
	fn compile_validation(
		...
		let output = compiler
			.compile_embedding_forward(
				context.input,
				context.rows,
				self.state.sequence_length.get(),
				self.state.dimensions.get(),
				self.state.table.updated_parameter,
				context.domain,
			)?
			.0;
		Ok(BlockValidation {
			output,
			logical: context.logical.embedded(self.declaration)?,
			routing: None,
		})
	}
...
impl CompiledBlock for EmbeddingValues {
	fn backward(
		&self,
		compiler: &mut GraphCompiler,
		context: BlockBackwardContext,
	) -> TrainingCompileResult<BlockBackward> {
		let output_width = self.sequence_length.get() * self.dimensions.get();
		compiler.require_tensor(
			context.gradient,
			DType::F32,
			&[context.partition_rows, output_width],
		...
		let row_gradients = compiler.gather_flat_as(
			flat,
			shape(&[token_rows, self.dimensions.get()])?,
			compiler.training_domain,
		)?;
		let base = compiler.zero_f32_tensor(shape(&[self.vocabulary.get(), self.dimensions.get()])?)?;
		let table = compiler.tensor(
			DType::F32,
			shape(&[self.vocabulary.get(), self.dimensions.get()])?,
		)?;
		let parameters = [
			...
			(
				"vocabulary".to_owned(),
				PreparedParameter::U64(self.vocabulary.get()),
			),
		]
		.into_iter()
		...
		Ok(BlockBackward {
			input_gradient: None,
			parameters: vec![ParameterGradient::new(ParameterRole::EmbeddingTable, table)],
		})
	}
	...
	fn optimize(
		...
		Ok(DenseBlockState::from_realized(Box::new(RealizedEmbedding {
			declaration: self.declaration,
			state: DenseEmbeddingState {
				sequence_length: self.sequence_length,
				dimensions: self.dimensions,
				vocabulary: self.vocabulary,
				table: updates.apply(compiler, ParameterRole::EmbeddingTable, self.table)?,
			},
		})))
	}
}
