...
impl DeclaredBlock for DenseTree {
	...
	fn compile_forward(
		...
		let supervision = context.validity;
		let rows = context.partition_rows;
		let input_width = context.logical.width()?;
		let output_width = context.target_width;
		let task = context.task;
		let block_index = context.block_index;
		let config = context.config;
		let result: TrainingCompileResult<(ValueId, TreeValues)> = {
			...
			let split_elements = checked_product(
				&[self.trees().get(), internal_nodes_per_tree.get()],
			...
			let (fresh_split_features, fresh_split_thresholds) = compiler.compile_supervised_tree_structure(
				[input, target_matrix, supervision],
				rows,
				[input_width, output_width, internal_nodes_per_tree],
				*self,
				config.reduction_tree_lanes,
				config.random_seed,
			)?;
			let resumed_split_features = compiler.external_i32_zeros(
				ExternalInputRole::ResumeTreeSplitFeatures { block: block_index },
				split_shape.clone(),
			)?;
			let split_features = compiler.select_resume_i32_tensor(
				fresh_split_features,
				resumed_split_features,
				split_shape.clone(),
			)?;
			compiler.external_outputs.insert(split_features);
			let resumed_split_thresholds = compiler.external_f32_zeros(
				ExternalInputRole::ResumeTreeSplitThresholds { block: block_index },
				split_shape.clone(),
			)?;
			let split_thresholds = compiler.select_resume_tensor(
			...
			let leaf_values = compiler.initialize_zero_parameter(shape(&[leaf_elements])?)?;
			let (flat_input, _) =
			...
			let output = compiler.materialize_tree_predictions(
				[flat_input, split_features, split_thresholds, leaf_values.value],
				rows,
			...
			let leaf_indices = compiler.route_tree_leaf_indices(
				[flat_input, split_features, split_thresholds],
				rows,
				[
					input_width,
					output_width,
		...
		Ok(BlockForward {
			output,
			logical: LogicalFeatureShape::from_width(context.target_width)?,
			routing: None,
			tape: Box::new(tape),
		})
	}
...
impl RealizedBlock for RealizedTree {
	...
	fn compile_validation(
		...
		let (flat, _) = compiler.pack_matrix_to_flat(
			context.input,
			context.rows,
			context.logical.width()?,
			context.domain,
		)?;
		Ok(BlockValidation {
			output: compiler.materialize_tree_predictions(
				[
					flat,
					self.state.split_features,
					self.state.split_thresholds,
					self.state.leaf_values.updated_parameter,
				],
				context.rows,
				[self.state.input_width, self.state.output_width],
				self.declaration,
				context.config.reduction_tree_lanes,
				context.domain,
			)?,
			logical: LogicalFeatureShape::from_width(self.state.output_width.get())?,
			routing: None,
		})
	}
...
impl CompiledBlock for TreeValues {
	fn backward(
		&self,
		compiler: &mut GraphCompiler,
		...
		let result: TrainingCompileResult<ValueId> = {
			let outputs = self.output_width.get();
			let trees = self.declaration.trees().get();
			let leaf_elements = checked_product(
				&[trees, self.leaves_per_tree.get(), outputs],
				"self leaf-gradient element count",
			)?;
			let (flat_gradient, _) =
				compiler.pack_matrix_to_flat(gradient, rows, outputs, compiler.training_domain)?;
			let gradient_indices = compiler.identity_indices(shape(&[rows, 1, outputs])?)?;
			let row_gradients = compiler.gather(
				flat_gradient,
				gradient_indices,
			...
			let contributions = compiler.elementwise_f32(
				shape(&[rows, trees, outputs])?,
				vec![row_gradients, self.leaf_indices],
				repeat_f32_with_i32_program()?,
				compiler.training_domain,
			)?;
			let leaf_gradient = compiler.deterministic_segment_sum(
				self.leaf_indices,
				contributions,
				leaf_elements,
				rows,
		...
		Ok(BlockBackward {
			input_gradient: None,
			parameters: vec![ParameterGradient::new(
	...
	fn optimize(
		...
		Ok(DenseBlockState::from_realized(Box::new(RealizedTree {
			declaration: self.declaration,
			state: DenseTreeState {
				declaration: self.declaration,
				input_width: self.input_width,
				output_width: self.output_width,
				internal_nodes_per_tree: self.internal_nodes_per_tree,
				leaves_per_tree: self.leaves_per_tree,
				split_features: self.split_features,
				split_thresholds: self.split_thresholds,
				leaf_values: updates.apply(compiler, ParameterRole::TreeLeafValue, self.leaf_values)?,
			},
		})))
	}
}
