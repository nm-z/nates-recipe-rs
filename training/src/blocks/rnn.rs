...
impl InferenceBlock for CheckpointRnnImage {
...
impl DeclaredBlock for DenseRnn {
	...
	fn compile_forward(
		...
		let result: TrainingCompileResult<(ValueId, RnnValues)> = {
			...
			let recurrent_weight = compiler.initialize_weight(
				shape(&[width.get(), width.get()])?,
				width.get(),
				config.random_seed,
			)?;
			let bias = compiler.initialize_zero_parameter(shape(&[width.get()])?)?;
...
impl CompiledBlock for RnnValues {
	fn backward(
		&self,
		compiler: &mut GraphCompiler,
		...
		let result: TrainingCompileResult<Vec<ParameterGradient>> = {
			...
			for step in self.steps.iter().rev() {
				let preactivation_gradient = compiler.backward_activation(
					hidden_gradient,
					step.preactivation,
					step.hidden,
					DenseActivation::Tanh,
				)?;
				let preactivation_gradient = compiler.mask_f32_with_zero(
					preactivation_gradient,
					validity,
					compiler.training_domain,
				)?;

				let step_input_weight_gradient = compiler.tensor(DType::F32, shape(&[1, self.width.get()])?)?;
				compiler.emit(
				...
				let step_recurrent_weight_gradient =
					compiler.tensor(DType::F32, shape(&[self.width.get(), self.width.get()])?)?;
				compiler.emit(
				...
				let step_bias_gradient = compiler.sum_tensor(
					preactivation_gradient,
				...
				compiler.accumulate_recurrent_gradient(&mut bias_gradient, step_bias_gradient)?;

				let previous_hidden_gradient =
					compiler.tensor(DType::F32, shape(&[rows, self.width.get()])?)?;
				compiler.emit(
					vec![preactivation_gradient, self.recurrent_weight.value],
	...
	fn optimize(
		...
		Ok(DenseBlockState::from_realized(Box::new(RealizedRnn {
			declaration: self.declaration,
			state: DenseRnnState {
				sequence_length: self.sequence_length,
				width: self.width,
				input_weight: updates.apply(compiler, ParameterRole::RnnInputWeight, self.input_weight)?,
				recurrent_weight: updates.apply(
					compiler,
					ParameterRole::RnnRecurrentWeight,
					self.recurrent_weight,
				)?,
				bias: updates.apply(compiler, ParameterRole::RnnBias, self.bias)?,
			},
		})))
	}
}
