...
use recipe_ingest::{EncodedImageFormat, EncodedImageMetadata, ImageColorModel, ImageValueLayout, ImageValueRange, SemanticType, TemporalOrigin, VectorEncoding, VectorMetadata, VectorRole, VectorSchema};
use recipe_ogdl::{Graph, NodeId};

use crate::{
	CheckpointDecodeErrorKind, CheckpointError, CheckpointPath, CheckpointResult, CompiledFeatureSpan, DataNormalization, DenseFeatureLowering, Function, KnnLabelValue, KnnReferenceOutput, KnnReferenceSet, KnnReferenceValues,
	checkpoint::{atomic_save, decode_error},
};
...
impl KnnModelArtifact {
	...
	/// inconsistent.
	#[inline]
	pub fn new(references: KnnReferenceSet, data_normalization: Option<DataNormalization>, operations: impl IntoIterator<Item = Function>) -> CheckpointResult<Self> {
		let artifact = Self {
	...
	pub fn continue_with(mut self, current: Self) -> CheckpointResult<Self> {
		...
		if self.format_version != current.format_version || saved_references.neighbors != current_references.neighbors || self.data_normalization != current.data_normalization || self.operations != current.operations {
			return Err(incompatible_resume(
				"saved and current KNN neighbor count, normalization, or topology differ",
			));
		}
		if saved_references.vectors != current_references.vectors || saved_references.feature_spans != current_references.feature_spans || saved_references.normalization_mask != current_references.normalization_mask || saved_references.feature_width != current_references.feature_width {
			return Err(incompatible_resume(
			...
			.zip(appended_references.outputs)
		{
			...
			if saved_output.values.is_numeric() {
				...
				saved_bits
					...
					.map_err(|error| {
						return incompatible_resume(format!("reserve combined KNN numeric output: {error}"));
					})?;
				...
				let mut codes_by_label = saved_labels
					...
					.map(|(index, label)| {
						let code = i32::try_from(index).map_err(|error| {
							return incompatible_resume(format!("saved KNN label index does not fit int32: {error}"));
						})?;
				...
				for label in &current_labels {
					...
					saved_labels.try_reserve_exact(1).map_err(|error| {
						return incompatible_resume(format!("reserve combined KNN label dictionary: {error}"));
					})?;
				...
				saved_codes
					...
					.map_err(|error| {
						return incompatible_resume(format!("reserve combined KNN discrete output: {error}"));
					})?;
				for code in current_codes {
					let index = usize::try_from(code).map_err(|error| {
						return incompatible_resume(format!("current KNN class code is negative: {error}"));
					})?;
	...
	pub fn encode(&self) -> CheckpointResult<Vec<u8>> {
		...
		for output in &self.references.outputs {
			...
			if output.values.is_numeric() {
				...
				for label in labels {
					match label {
						KnnLabelValue::I32(value) => field(&mut graph, dictionary, "int32", value)?,
						KnnLabelValue::Bytes(value) => {
							field(&mut graph, dictionary, "bytes", &encode_bytes(value))?;
						}
					}
				}
			}
	...
	#[inline]
	pub fn decode(source: &[u8], limits: KnnModelDecodeLimits) -> CheckpointResult<Self> { return Decoder::new(source, limits)?.decode(); }
...
#[inline]
pub fn decode_knn_model(source: &[u8], limits: KnnModelDecodeLimits) -> CheckpointResult<KnnModelArtifact> { return Decoder::new(source, limits)?.decode(); }
...
impl EncodeKnnNode for VectorSchema {
	fn encode_node(&self, graph: &mut Graph, parent: NodeId) -> CheckpointResult<()> {
		...
		field(graph, node, "role", &match self.role() {
			VectorRole::Feature => "feature",
			VectorRole::Target => "target",
		})?;
		field(graph, node, "semantic-type", &match self.semantic_type() {
		...
		field(graph, node, "encoding", &match self.encoding() {
		...
		match self.metadata() {
			...
			VectorMetadata::Temporal { origin } => {
				...
				field(graph, tag, "unix-seconds", &origin.unix_seconds)?;
				field(graph, tag, "nanoseconds", &origin.nanoseconds)?;
			}
			...
			VectorMetadata::Image { encoded_variants } => {
				...
				for variant in encoded_variants {
					...
					field(graph, item, "format", &match variant.format() {
					...
					field(graph, item, "value-layout", &match variant.value_layout() {
						ImageValueLayout::EncodedFile => "encoded-file",
					})?;
					field(graph, item, "value-range", &match variant.value_range() {
...
fn scalar(graph: &mut Graph, parent: NodeId, value: &impl ToString) -> CheckpointResult<NodeId> { return child(graph, parent, value.to_string()); }
...
fn validate_artifact(artifact: &KnnModelArtifact) -> CheckpointResult<()> {
	...
	for vector in &references.vectors {
		...
		match span.lowering() {
			...
			DenseFeatureLowering::CategoricalOneHot {
				...
				if reserved_index != dictionary_width {
					return Err(CheckpointError::manifest(
						"KNN categorical span reserved index differs from dictionary width",
					));
				}
				if span.width() != dictionary_width.saturating_add(1) {
					return Err(CheckpointError::manifest(
						"KNN categorical span width differs from encoded dictionary width",
					));
				}
			}
			DenseFeatureLowering::NumericScalar => {
	...
	if cursor != references.feature_width || seen_spans != feature_sources {
		return Err(CheckpointError::manifest(
			"KNN feature spans do not exactly cover every saved feature column",
		));
	}
...
fn validate_outputs(references: &KnnReferenceSet) -> CheckpointResult<()> {
	...
	for output in &references.outputs {
		...
		if !seen.insert(source) || vectors.get(&source).copied() != Some(&output.schema) || output.schema.role() != VectorRole::Target {
			return Err(CheckpointError::manifest(
...
impl Decoder {
	...
	fn decode(mut self) -> CheckpointResult<KnnModelArtifact> {
		...
		let format_version = Self::parse_u32(
			self.scalar(fields["format-version"], &format_version_path)?,
			&format_version_path,
		)?;
		...
		let neighbor_count = parse_canonical(
			self.scalar(fields["neighbors"], &neighbors_path)?,
			&neighbors_path,
		)?;
		let neighbors = NonZeroU64::new(neighbor_count).ok_or_else(|| {
			return Self::invalid_value(&neighbors_path, "neighbor count must be nonzero");
		})?;
		...
		let operations = self.parse_operations(fields["operations"], &path.field("operations"))?;
		let vectors = self.parse_vectors(fields["vectors"], &path.field("vectors"))?;
		let feature_spans = self.parse_spans(fields["feature-spans"], &path.field("feature-spans"))?;
		...
		let shape = self.fields(fields["reference-shape"], &shape_path, &[
			"rows",
			"feature-width",
		])?;
		...
		let feature_width = Self::parse_usize(
			self.scalar(shape["feature-width"], &width_path)?,
			&width_path,
		)?;
		...
		validate_artifact(&artifact).map_err(|validation_error| {
			match validation_error {
				CheckpointError::Decode(decode_failure) => return CheckpointError::Decode(decode_failure),
				other @ (CheckpointError::InvalidManifest { .. } | CheckpointError::IncompatibleResume { .. } | CheckpointError::DuplicateOutput { .. } | CheckpointError::MissingOutput { .. } | CheckpointError::UnexpectedOutput { .. } | CheckpointError::OutputDTypeMismatch { .. } | CheckpointError::OutputSizeMismatch { .. } | CheckpointError::NativeKernelUnavailable { .. } | CheckpointError::NativeKernelAmbiguous { .. } | CheckpointError::InvalidTarget { .. } | CheckpointError::InsufficientCapacity { .. } | CheckpointError::Io { .. }) => {
					return decode_error(
	...
	fn parse_operations(&self, node: NodeId, path: &CheckpointPath) -> CheckpointResult<Vec<Function>> {
		...
		for (index, child) in children.iter().copied().enumerate() {
			...
			operations.push(Function::from_token(value).ok_or_else(|| {
	...
	fn parse_vectors(&mut self, node: NodeId, path: &CheckpointPath) -> CheckpointResult<Vec<VectorSchema>> {
	...
	fn parse_metadata(&mut self, node: NodeId, path: &CheckpointPath) -> CheckpointResult<VectorMetadata> {
	...
	fn parse_image_entries(&mut self, children: &[NodeId], path: &CheckpointPath) -> CheckpointResult<Vec<EncodedImageMetadata>> {
		self.reserve_metadata(children.len(), path)?;
		let mut values = Vec::with_capacity(children.len());
		for (index, child) in children.iter().copied().enumerate() {
			let item_path = path.index(index);
			...
			values.push(EncodedImageMetadata::new(
				format,
				width,
				height,
				channels,
	...
	fn parse_outputs(&mut self, node: NodeId, path: &CheckpointPath, vectors: &[VectorSchema], reference_rows: usize) -> CheckpointResult<Vec<KnnReferenceOutput>> {
		let children = self.node(node, path)?.children().to_vec();
		...
		for (index, child) in children.into_iter().enumerate() {
			...
			let values = match self.node(tag, &item_path.field("values"))?.text() {
				"numeric-f32-bits" => {
					...
					KnnReferenceValues::from_numeric_f32_bits(self.parse_u32_hex(&value, reference_rows, &item_path.field("values"))?)
				}
				"discrete-int32" => {
	...
	fn fields(&self, node: NodeId, path: &CheckpointPath, required: &[&str]) -> CheckpointResult<BTreeMap<String, NodeId>> { return self.fields_from(self.node(node, path)?.children(), path, required); }

	/// Decode required named fields from a child-node sequence.
	fn fields_from(&self, children: &[NodeId], path: &CheckpointPath, required: &[&str]) -> CheckpointResult<BTreeMap<String, NodeId>> {
	...
	fn parse_usize_hex(&mut self, value: &str, count: usize, path: &CheckpointPath) -> CheckpointResult<Vec<usize>> {
		...
		return (0..count)
			.map(|index| {
				...
				let source_row = u64::from_str_radix(chunk, 16).map_err(|error| return Self::invalid_value(path, error.to_string()))?;
