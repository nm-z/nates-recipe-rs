...
use recipe_core::{Block, ByteCount, DType, KernelTemplateId, ScalarOpcode, ValueId};
use recipe_ingest::{InferenceFeatureEncoding, InferenceFeatureSchema, InferencePrepareError, PreparedInferenceDataset, PreparedInferenceFeature, PreparedInferenceValues, RawTable, SemanticType, SourceError, SourceLimit, VectorEncoding, VectorMetadata, VectorSchema, read_source_snapshot};
use recipe_language::{AxisSet, CalculationGraph, CalculationNode, Contraction, Elementwise, Gather, IndexBounds, IndexMap, PrimitiveAliasRule, PrimitiveKernel, PrimitiveKind, Reduce, ReduceOperator, ReduceResult, ScalarProgramBuilder, Scatter, ScatterConflict, Shape, Tensor};
use recipe_ops::{CategoricalBayesInferenceRequest, IdentityNamespace, KnnAllOutputRequest, KnnOutputRequest, MaterializationRequest, NamedTensor, PreparedParameter, PreparedParameters, append_categorical_bayes_inference, append_knn_all_outputs, categorical_bayes_inference_requirements, knn_all_output_requirements, lower_scalar, materialize_composition, operation_registry};
use recipe_program::{IterationDomain, KernelIterationDomain, StaticCalculationProgram};

use crate::{
	Activation, BayesModelArtifact, BayesModelDecodeLimits, CheckpointArtifact, CheckpointDecodeErrorKind, CheckpointDecodeLimits, CheckpointError, CheckpointPath, CheckpointTensorImage, CompiledFeatureSpan, DataNormalization, DenseFeatureLowering, DenseOutputAdapter, DenseTask, Function, KnnModelArtifact, KnnModelDecodeLimits, MAXIMUM_REDUCTION_TREE_LANES, Normalization,
	checkpoint::decode_error,
	decode_bayes_model, decode_checkpoint, decode_knn_model,
	forward::{ActivationForward as _, ForwardActivation, RecurrentForwardGraph, add_program, causal_mask_program, divide_constant_program, divide_program, forbidden_aliases, head_major_source_index_program, l2_norm_program, l2_square_program, min_max_program, multiply_program, square_program, subtract_program, z_score_program},
	model::f32_from_u64,
};

const KNN_NORMALIZATION_EPSILON: f32 = 1.0e-6;

#[path = "gguf_llama.rs"]
mod gguf_llama;

pub use gguf_llama::{GgufLlamaArtifact, GgufLlamaError, GgufLlamaErrorKind, GgufLlamaResult, PreparedGgufLlamaInference, compile_prepared_gguf_llama_inference, decode_gguf_llama, load_gguf_llama_model_file, prepare_gguf_llama_inference_table};
...
impl fmt::Display for InferencePreparationError {
	...
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			Self::CheckpointSource(error) => return write!(f, "load checkpoint source: {error}"),
			Self::Checkpoint(error) => return write!(f, "load checkpoint: {error}"),
			Self::GgufLlama(error) => return write!(f, "load GGUF llama model: {error}"),
			Self::Data(error) => return write!(f, "prepare inference data: {error}"),
			...
			Self::ArithmeticOverflow { detail } => return write!(f, "prepare inference data: {detail}"),
		}
	}
}
...
impl core::error::Error for InferencePreparationError {
	...
	fn source(&self) -> Option<&(dyn core::error::Error + 'static)> {
		match self {
			Self::CheckpointSource(error) => return Some(error),
			Self::Checkpoint(error) => return Some(error),
			Self::GgufLlama(error) => return Some(error),
			Self::Data(error) => return Some(error),
			Self::InconsistentCheckpoint { .. } | Self::ArithmeticOverflow { .. } => return None,
		}
	}
}
...
impl From<SourceError> for InferencePreparationError {
	#[inline]
	fn from(error: SourceError) -> Self { return Self::CheckpointSource(error); }
}
...
impl From<InferencePrepareError> for InferencePreparationError {
	#[inline]
	fn from(error: InferencePrepareError) -> Self { return Self::Data(error); }
}

impl From<GgufLlamaError> for InferencePreparationError {
	#[inline]
	fn from(error: GgufLlamaError) -> Self { return Self::GgufLlama(error); }
}
...
impl fmt::Display for InferenceCompileErrorKind {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		return f.write_str(match *self {
			Self::EmptyDataset => "empty dataset",
			Self::InconsistentCheckpoint => "inconsistent checkpoint",
...
impl InferenceCompileError {
	#[must_use]
	#[inline]
	pub const fn kind(&self) -> InferenceCompileErrorKind { return self.kind; }
...
impl From<recipe_language::LanguageError> for InferenceCompileError {
	#[inline]
	fn from(error: recipe_language::LanguageError) -> Self { return Self::new(InferenceCompileErrorKind::Language, error.to_string()); }
}

impl From<recipe_language::OgdlCodecError> for InferenceCompileError {
	#[inline]
	fn from(error: recipe_language::OgdlCodecError) -> Self { return Self::new(InferenceCompileErrorKind::Ogdl, error.to_string()); }
}

impl From<recipe_ops::OperationError> for InferenceCompileError {
	#[inline]
	fn from(error: recipe_ops::OperationError) -> Self { return Self::new(InferenceCompileErrorKind::Operation, error.to_string()); }
}

impl From<recipe_program::ProgramError> for InferenceCompileError {
	#[inline]
	fn from(error: recipe_program::ProgramError) -> Self { return Self::new(InferenceCompileErrorKind::Program, error.to_string()); }
}

pub type InferenceCompileResult<T> = Result<T, InferenceCompileError>;

pub struct BlockInferenceContext<'a> {
	pub input: ValueId,
	pub rows: u64,
	pub width: u64,
...
pub trait InferenceBlock {
	fn token_vocabulary(&self) -> Option<u64> { return None; }

	fn output_width(&self) -> NonZeroU64;

	fn output_operations(&self) -> &[Function] { return &[]; }

	fn compile_inference(&self, compiler: &mut InferenceGraphCompiler, context: BlockInferenceContext<'_>) -> InferenceCompileResult<BlockInference>;
}

impl InferenceBlock for Block {
	fn output_width(&self) -> NonZeroU64 { todo!("derive output width from Block") }

	fn compile_inference(&self, _compiler: &mut InferenceGraphCompiler, _context: BlockInferenceContext<'_>) -> InferenceCompileResult<BlockInference> { todo!("compile inference directly from Block") }
}
...
impl InferenceExternalInput {
	#[must_use]
	#[inline]
	pub const fn role(&self) -> InferenceInputRole { return self.role; }
...
impl InferenceOutputContract {
	...
	#[must_use]
	#[inline]
	pub const fn kind(&self) -> InferencePredictionKind { return self.kind; }
}
...
impl CompiledInference {
	#[must_use]
	#[inline]
	pub const fn graph(&self) -> &CalculationGraph { return self.program.graph(); }

	#[must_use]
	#[inline]
	pub const fn program(&self) -> &StaticCalculationProgram { return &self.program; }

	#[must_use]
	#[inline]
	pub fn external_inputs(&self) -> &[InferenceExternalInput] { return &self.external_inputs; }

	#[must_use]
	#[inline]
	pub const fn output(&self) -> &InferenceOutputContract { return &self.output; }

	#[must_use]
	#[inline]
	pub const fn rows(&self) -> u64 { return self.rows; }

	#[must_use]
	#[inline]
	pub const fn task(&self) -> InferenceTask { return self.task; }
...
impl KnnInferenceOutputContract {
	pub(crate) const fn new(value: ValueId, dtype: DType, shape: Shape, source_vector: usize, kind: KnnInferencePredictionKind) -> Self {
		return Self {
			value,
			dtype,
			shape,
	...
	#[must_use]
	#[inline]
	pub const fn kind(&self) -> KnnInferencePredictionKind { return self.kind; }
}
...
impl CompiledKnnInference {
	...
	#[must_use]
	#[inline]
	pub fn outputs(&self) -> &[KnnInferenceOutputContract] { return &self.outputs; }
...
impl PreparedBayesInference {
	#[must_use]
	#[inline]
	pub const fn artifact(&self) -> &BayesModelArtifact { return &self.artifact; }

	#[must_use]
	#[inline]
	pub const fn data(&self) -> &PreparedInferenceDataset { return &self.data; }

	#[must_use]
	#[inline]
	pub fn into_artifact(self) -> BayesModelArtifact { return self.artifact; }
}

impl PreparedKnnInference {
	#[must_use]
	#[inline]
	pub const fn artifact(&self) -> &KnnModelArtifact { return &self.artifact; }
	...
	#[must_use]
	#[inline]
	pub fn into_artifact(self) -> KnnModelArtifact { return self.artifact; }
}
...
impl PreparedInference {
	#[must_use]
	#[inline]
	pub const fn checkpoint(&self) -> &CheckpointArtifact { return &self.checkpoint; }
	...
	#[must_use]
	#[inline]
	pub fn features(&self) -> &[PreparedInferenceFeature] { return self.data.features(); }

	#[must_use]
	#[inline]
	pub fn feature_spans(&self) -> &[CompiledFeatureSpan] { return self.checkpoint.feature_spans(); }

	#[must_use]
	#[inline]
	pub const fn normalization(&self) -> DataNormalization { return self.checkpoint.data_normalization(); }

	#[must_use]
	#[inline]
	pub fn feature_normalization_mask(&self) -> &[u32] { return self.checkpoint.feature_normalization_mask(); }

	#[must_use]
	#[inline]
	pub const fn normalization_epsilon(&self) -> f32 { return self.checkpoint.normalization_epsilon(); }
}

#[inline]
pub fn load_checkpoint_file(path: impl AsRef<Path>, limits: CheckpointDecodeLimits) -> InferencePreparationResult<CheckpointArtifact> {
	let source_bytes = u64::try_from(limits.source_bytes).map_err(|error| {
		return InferencePreparationError::ArithmeticOverflow {
	...
	return decode_checkpoint(source.bytes(), limits).map_err(Into::into);
}

#[inline]
pub fn load_knn_model_file(path: impl AsRef<Path>, limits: KnnModelDecodeLimits) -> InferencePreparationResult<KnnModelArtifact> {
	let source_bytes = u64::try_from(limits.source_bytes).map_err(|error| {
		return InferencePreparationError::ArithmeticOverflow {
	...
	let source = read_source_snapshot(path.as_ref(), SourceLimit::new(source_bytes)?)?;
	return decode_knn_model(source.bytes(), limits).map_err(Into::into);
}

#[inline]
pub fn load_bayes_model_file(path: impl AsRef<Path>, limits: BayesModelDecodeLimits) -> InferencePreparationResult<BayesModelArtifact> {
	let source_bytes = u64::try_from(limits.source_bytes).map_err(|error| {
		return InferencePreparationError::ArithmeticOverflow {
	...
	let source = read_source_snapshot(path.as_ref(), SourceLimit::new(source_bytes)?)?;
	return decode_bayes_model(source.bytes(), limits).map_err(Into::into);
}

#[inline]
pub fn load_semantic_model_file(path: impl AsRef<Path>, checkpoint_limits: CheckpointDecodeLimits, knn_limits: KnnModelDecodeLimits) -> InferencePreparationResult<SemanticModelArtifact> {
	...
	let root_end = source
		...
		.unwrap_or_else(|| return source.bytes().len());
	let root = &source.bytes()[..root_end];
	...
	match root {
		"recipe" => {
			return decode_checkpoint(source.bytes(), checkpoint_limits)
				.map(|checkpoint| return SemanticModelArtifact::Dense(Box::new(checkpoint)))
				.map_err(Into::into);
		}
		"recipe-knn-model" => {
			return decode_knn_model(source.bytes(), knn_limits)
				.map(SemanticModelArtifact::Knn)
				.map_err(Into::into);
		}
		"recipe-bayes-model" => {
			return decode_bayes_model(source.bytes(), BayesModelDecodeLimits::default())
...
#[inline]
pub fn prepare_knn_inference_table(artifact: KnnModelArtifact, table: &RawTable) -> InferencePreparationResult<PreparedKnnInference> {
	...
	return Ok(PreparedKnnInference { artifact, data });
}

#[inline]
pub fn prepare_bayes_inference_table(artifact: BayesModelArtifact, table: &RawTable) -> InferencePreparationResult<PreparedBayesInference> {
	...
	let data = recipe_ingest::prepare_inference_table(table, &schema)?;
	return Ok(PreparedBayesInference { artifact, data });
}

pub fn prepare_checkpoint_inference_table(checkpoint: CheckpointArtifact, table: &RawTable) -> InferencePreparationResult<PreparedInference> {
	let schema = saved_feature_schema(&checkpoint)?;
	let data = recipe_ingest::prepare_inference_table(table, &schema)?;
	validate_prepared_feature_spans(&data, checkpoint.feature_spans())?;
	return Ok(PreparedInference { checkpoint, data });
}
...
pub fn compile_prepared_inference(prepared: &PreparedInference) -> InferenceCompileResult<CompiledInference> {
	...
	let rows = u64::try_from(prepared.data().rows()).map_err(|error| {
		return InferenceCompileError::new(
	...
	for (block_index, block) in checkpoint.blocks().iter().enumerate() {
		let inference = block.compile_inference(&mut compiler, BlockInferenceContext {
			input: current,
			rows,
			width: current_width,
			logical_length,
			...
			tree_lanes: checkpoint.reduction_tree_lanes(),
		})?;
	...
	let (prediction, kind) = compiler.compile_prediction(
		...
		rows,
		current_width,
		checkpoint.reduction_tree_lanes(),
	)?;
...
#[inline]
pub fn compile_prepared_bayes_inference(prepared: &PreparedBayesInference) -> InferenceCompileResult<CompiledInference> {
	let rows = u64::try_from(prepared.data().rows()).map_err(|error| {
		return InferenceCompileError::new(
			InferenceCompileErrorKind::UnsupportedExtent,
...
fn compile_bayes_conditional(compiler: &mut InferenceGraphCompiler, data: &PreparedInferenceDataset, references: &crate::BayesianCategoricalReferenceSet, smoothing: f32, conditional: usize, rows: u64) -> InferenceCompileResult<ValueId> {
	...
	for row in 0..data.rows() {
		for (parent_index, parent) in references.parents().iter().enumerate() {
			let feature = data
				...
				.iter()
				.find(|feature| {
					return feature.schema().source_vector() == parent.source_index() && feature.schema().name() == parent.name();
				})
				.ok_or_else(|| {
					return InferenceCompileError::new(
						InferenceCompileErrorKind::InconsistentCheckpoint,
						format!("prepared Bayesian conditional {conditional} parent {parent_index} is absent"),
					);
				})?;
			let PreparedInferenceValues::I32(values) = feature.values() else {
				return Err(InferenceCompileError::new(
					InferenceCompileErrorKind::InconsistentCheckpoint,
					format!("prepared Bayesian conditional {conditional} parent {parent_index} is not dictionary-coded I32"),
				));
			};
			let code = values.get(row).copied().ok_or_else(|| {
				return InferenceCompileError::new(
					InferenceCompileErrorKind::InconsistentCheckpoint,
					format!("prepared Bayesian conditional {conditional} parent {parent_index} is missing query row {row}"),
				);
			})?;
			...
			if code < 0 || code >= cardinality {
				return Err(InferenceCompileError::new(
					InferenceCompileErrorKind::InconsistentCheckpoint,
					format!("prepared Bayesian conditional {conditional} parent {parent_index} row {row} has out-of-range code {code}"),
				));
			}
	...
	let reference_parent_codes = compiler.external(
		...
		references
			.parent_codes()
			.iter()
			.flat_map(|value| return value.to_le_bytes())
			.collect(),
	)?;
...
fn concatenate_bayes_probabilities(compiler: &mut InferenceGraphCompiler, probabilities: Vec<(ValueId, u64)>, rows: u64) -> InferenceCompileResult<ValueId> {
...
pub fn compile_prepared_knn_inference(prepared: &PreparedKnnInference) -> InferenceCompileResult<CompiledKnnInference> {
	...
	let reference_features = compiler.external(
		...
		references
			...
			.iter()
			.flat_map(|bits| return bits.to_le_bytes())
			.collect(),
	)?;
	...
	return Ok(CompiledKnnInference {
		program,
...
#[derive(Debug)]
pub struct InferenceGraphCompiler {
...
impl RecurrentForwardGraph for InferenceGraphCompiler {
	...
	fn zero_f32(&mut self, shape: Shape) -> InferenceCompileResult<ValueId> { return self.allocate_zero_f32(shape); }

	fn gather_matrix_column(&mut self, matrix: ValueId, rows: u64, columns: u64, column: u64) -> InferenceCompileResult<ValueId> { return self.extract_matrix_column(matrix, rows, columns, column); }

	fn bias_free_linear(&mut self, input: ValueId, weight: ValueId, rows: u64, output_width: u64) -> InferenceCompileResult<ValueId> { return self.linear_without_bias(input, weight, rows, output_width); }

	fn elementwise_f32(&mut self, shape: Shape, inputs: Vec<ValueId>, program: recipe_core::ScalarProgram) -> InferenceCompileResult<ValueId> {
		let output = self.tensor(DType::F32, shape)?;
		self.emit_elementwise(inputs, vec![output], program)?;
		return Ok(output);
	}

	fn activate(&mut self, input: ValueId, activation: Activation, shape: Shape) -> InferenceCompileResult<ValueId> { return self.apply_activation(input, activation, None, shape); }
}
...
impl InferenceGraphCompiler {
	...
	fn external(&mut self, role: InferenceInputRole, dtype: DType, shape: Shape, bytes: Vec<u8>) -> InferenceCompileResult<ValueId> {
	...
	fn external_checkpoint_tensor(&mut self, role: InferenceInputRole, image: &CheckpointTensorImage) -> InferenceCompileResult<ValueId> {
	...
	fn next_kernel(&mut self) -> InferenceCompileResult<KernelTemplateId> {
		...
		return Ok(KernelTemplateId::new(kernel));
	}

	fn tensor_ref(&self, value: ValueId) -> InferenceCompileResult<&Tensor> {
		return self.tensors.get(&value).ok_or_else(|| {
			return InferenceCompileError::new(
	...
	fn emit(&mut self, inputs: Vec<ValueId>, outputs: Vec<ValueId>, kind: PrimitiveKind, alias_rules: Vec<PrimitiveAliasRule>) -> InferenceCompileResult<KernelTemplateId> {
	...
	fn emit_elementwise(&mut self, inputs: Vec<ValueId>, outputs: Vec<ValueId>, program: recipe_core::ScalarProgram) -> InferenceCompileResult<KernelTemplateId> {
	...
	fn emit_owned_scalar(&mut self, symbol: &str, inputs: Vec<ValueId>, outputs: Vec<ValueId>) -> InferenceCompileResult<KernelTemplateId> {
		let descriptor = operation_registry().resolve_unique(symbol)?;
		let program = lower_scalar(descriptor)?;
		return self.emit_elementwise(inputs, outputs, program);
	}

	fn reduce(&mut self, input: ValueId, output: ValueId, operator: ReduceOperator, axes: &[usize], keep_dimensions: bool, tree_lanes: u32) -> InferenceCompileResult<()> {
		self.emit(
			vec![input],
			vec![output],
	...
	fn materialize(&mut self, symbol: &str, inputs: &[(&'static str, ValueId)], outputs: &[(&'static str, ValueId)], iteration_shape_input: &'static str, parameters: &PreparedParameters) -> InferenceCompileResult<()> {
		let mut input_tensors = inputs
			.iter()
			.copied()
			.map(|(_, value)| return self.tensor_ref(value).cloned())
		...
		let named_inputs = inputs
			...
			.copied()
			.zip(&input_tensors)
			.map(|((name, _), tensor)| return NamedTensor::new(name, tensor))
			.collect::<Vec<_>>();
	...
	fn insert_tensor_contract(&mut self, mut tensor: Tensor) -> InferenceCompileResult<()> {
		...
		match self.tensors.get(&tensor.id) {
			Some(existing) if existing.dtype == tensor.dtype && existing.shape == tensor.shape && existing.layout == tensor.layout && existing.storage_bytes == tensor.storage_bytes => {
				return Ok(());
			}
			Some(_) => {
				return Err(InferenceCompileError::new(
	...
	fn compile_features(&mut self, data: &PreparedInferenceDataset, spans: &[CompiledFeatureSpan], rows: u64, feature_width: u64) -> InferenceCompileResult<ValueId> {
		...
		let mut combined = self.allocate_zero_f32(shape(&[total_elements])?)?;
		...
		for (feature_index, (feature, span)) in data.features().iter().zip(spans).enumerate() {
			...
			if expected_start > feature_width || feature.schema().source_vector() != span.source_vector() || feature.values().len() != data.rows() {
				return Err(InferenceCompileError::new(
					InferenceCompileErrorKind::InconsistentCheckpoint,
			...
			let block = match (span.lowering(), feature.values()) {
				...
				) if dictionary_width.checked_add(1) == Some(span.width()) && reserved_index == dictionary_width => self.one_hot(raw, rows, width)?,
				_ => {
					return Err(InferenceCompileError::new(
						InferenceCompileErrorKind::InconsistentCheckpoint,
		...
		if expected_start != feature_width {
			return Err(InferenceCompileError::new(
				InferenceCompileErrorKind::InconsistentCheckpoint,
				format!("saved feature spans end at width {expected_start}, checkpoint declares {feature_width}"),
			));
		}
	...
	fn compile_token_features(&mut self, data: &PreparedInferenceDataset, spans: &[CompiledFeatureSpan], rows: u64, feature_width: u64, vocabulary: u64) -> InferenceCompileResult<ValueId> {
		if data.features().len() != spans.len() {
			return Err(InferenceCompileError::new(
				InferenceCompileErrorKind::InconsistentCheckpoint,
		...
		for (feature_index, (feature, span)) in data.features().iter().zip(spans).enumerate() {
			...
			if start != feature_index as u64 || span.width() != 1 || span.lowering() != DenseFeatureLowering::NumericScalar || feature.schema().source_vector() != span.source_vector() || feature.values().len() != data.rows() {
				return Err(InferenceCompileError::new(
					InferenceCompileErrorKind::InconsistentCheckpoint,
			...
			if let Some((row, token)) = values
				...
				.copied()
				.enumerate()
				.find(|entry| return entry.1 < 0 || entry.1 >= vocabulary)
			{
				return Err(InferenceCompileError::new(
					InferenceCompileErrorKind::InconsistentCheckpoint,
					format!("inference token {token} at row {row}, position {feature_index} is outside 0..{vocabulary}"),
				));
			}
			let raw = self.external(
				InferenceInputRole::Feature {
	...
	fn allocate_zero_f32(&mut self, output_shape: Shape) -> InferenceCompileResult<ValueId> {
	...
	fn one_hot(&mut self, labels: ValueId, rows: u64, classes: u64) -> InferenceCompileResult<ValueId> {
		...
		let base = self.allocate_zero_f32(shape(&[elements])?)?;
	...
	fn scatter_feature_block(&mut self, base: ValueId, block: ValueId, rows: u64, total_width: u64, start: u64, block_width: u64) -> InferenceCompileResult<ValueId> {
	...
	fn scatter_i32_feature_block(&mut self, base: ValueId, block: ValueId, rows: u64, total_width: u64, start: u64) -> InferenceCompileResult<ValueId> {
	...
	fn apply_data_normalization(&mut self, checkpoint: &CheckpointArtifact, input: ValueId, rows: u64, columns: u64) -> InferenceCompileResult<ValueId> {
		if checkpoint.data_normalization() == DataNormalization::Identity {
		...
		match checkpoint.data_normalization() {
			DataNormalization::Identity => return Ok(input),
			DataNormalization::ZScore => {
				...
				let variance = self.external_checkpoint_tensor(InferenceInputRole::DataNormalizationVariance, variance)?;
				let output = self.tensor(DType::F32, matrix_shape)?;
				self.emit_elementwise(
					vec![input, mean, variance, mask],
					vec![output],
					z_score_program(checkpoint.normalization_epsilon(), true)?,
				)?;
				return Ok(output);
			}
			DataNormalization::MinMax => {
				...
				let minimum = self.external_checkpoint_tensor(InferenceInputRole::DataNormalizationMinimum, minimum)?;
				let maximum = self.external_checkpoint_tensor(InferenceInputRole::DataNormalizationMaximum, maximum)?;
				let output = self.tensor(DType::F32, matrix_shape)?;
				self.emit_elementwise(
					vec![input, minimum, maximum, mask],
					vec![output],
					min_max_program(checkpoint.normalization_epsilon(), true)?,
				)?;
				return Ok(output);
			}
			DataNormalization::L2Norm => {
				if !checkpoint.normalization().is_empty() {
					return Err(InferenceCompileError::new(
						InferenceCompileErrorKind::InconsistentCheckpoint,
				...
				self.emit_elementwise(
					...
					vec![output],
					l2_norm_program(checkpoint.normalization_epsilon(), true)?,
				)?;
				return Ok(output);
			}
		}
	...
	fn normalize_knn_features(&mut self, values: [ValueId; 2], dimensions: [u64; 3], normalization: Option<DataNormalization>, mask_bits: Option<&[u32]>, tree_lanes: u32) -> InferenceCompileResult<(ValueId, ValueId)> {
		...
		let mask = mask_bits
			...
			.transpose()?;
		match normalization {
			DataNormalization::Identity => return Ok((query, reference)),
			DataNormalization::ZScore => {
				...
				self.emit_elementwise(
					vec![sums],
					vec![means],
					divide_constant_program(f32_from_u64(reference_rows))?,
				)?;
				...
				return Ok((
					self.apply_knn_z_score(
						[query, means, variances],
						[query_rows, columns],
						mask,
						KNN_NORMALIZATION_EPSILON,
					)?,
					self.apply_knn_z_score(
	...
	fn apply_knn_z_score(&mut self, values: [ValueId; 3], dimensions: [u64; 2], mask: Option<ValueId>, epsilon: f32) -> InferenceCompileResult<ValueId> {
	...
	fn apply_knn_min_max(&mut self, values: [ValueId; 3], dimensions: [u64; 2], mask: Option<ValueId>, epsilon: f32) -> InferenceCompileResult<ValueId> {
	...
	fn apply_knn_l2(&mut self, input: ValueId, rows: u64, columns: u64, mask: Option<ValueId>, epsilon: f32, tree_lanes: u32) -> InferenceCompileResult<ValueId> {
		let matrix_shape = shape(&[rows, columns])?;
		let squares = self.tensor(DType::F32, matrix_shape.clone())?;
	...
	#[cfg(any())]
	pub(crate) fn compile_embedding(&mut self, block_index: usize, embedding: &Block, input: ValueId, rows: u64, input_width: u64) -> InferenceCompileResult<ValueId> {
	...
	#[cfg(any())]
	pub(crate) fn compile_attention(&mut self, block_index: usize, attention: &Block, input: ValueId, geometry: [u64; 4], tree_lanes: u32) -> InferenceCompileResult<ValueId> {
		...
		if input_width != expected_width {
			return Err(InferenceCompileError::new(
				InferenceCompileErrorKind::InconsistentCheckpoint,
				format!("attention block {block_index} input width {input_width} differs from fixed sequence width {expected_width}"),
			));
		}
		checked_i32(
			checked_product(
	...
	#[cfg(any())]
	pub(crate) fn compile_rnn(&mut self, block_index: usize, rnn: &Block, input: ValueId, rows: u64, input_width: u64) -> InferenceCompileResult<ValueId> {
	...
	#[cfg(any())]
	pub(crate) fn compile_gru(&mut self, block_index: usize, gru: &Block, input: ValueId, rows: u64, input_width: u64) -> InferenceCompileResult<ValueId> {
	...
	#[cfg(any())]
	pub(crate) fn compile_lstm(&mut self, block_index: usize, lstm: &Block, input: ValueId, rows: u64, input_width: u64) -> InferenceCompileResult<ValueId> {
	...
	fn extract_matrix_column(&mut self, matrix: ValueId, rows: u64, columns: u64, column: u64) -> InferenceCompileResult<ValueId> {
		if column >= columns {
			return Err(InferenceCompileError::new(
				InferenceCompileErrorKind::InconsistentCheckpoint,
	...
	fn attention_projection(&mut self, input: ValueId, weight: ValueId, rows: u64, sequence: u64, dimensions: u64) -> InferenceCompileResult<ValueId> {
	...
	#[cfg(any())]
	pub(crate) fn compile_layer(&mut self, layer_index: usize, layer: &Block, input: ValueId, dimensions: [u64; 2], normalization_epsilon: f32, tree_lanes: u32) -> InferenceCompileResult<ValueId> {
		...
		for operation in layer.declaration().operations().iter().copied() {
			current = match operation {
				Function::Activation(activation) => {
					let alpha = if activation.learned_parameters() == 1 {
	...
	#[cfg(any())]
	pub(crate) fn compile_convolution(&mut self, block_index: usize, convolution: &Block, input: ValueId, input_geometry: [u64; 4], normalization_epsilon: f32, tree_lanes: u32) -> InferenceCompileResult<ValueId> {
		let [rows, input_width, logical_length, logical_channels] = input_geometry;
		let geometry = convolution.geometry();
		if geometry.input_length().get() != logical_length || geometry.input_channels().get() != logical_channels || geometry.input_width().map(NonZeroU64::get) != Some(input_width) {
			return Err(InferenceCompileError::new(
				InferenceCompileErrorKind::InconsistentCheckpoint,
				format!("convolution block {block_index} input geometry disagrees with the preceding logical shape"),
			));
		}
	...
	#[cfg(any())]
	pub(crate) fn compile_pool(&mut self, block_index: usize, pool: &Block, input: ValueId, input_geometry: [u64; 4], tree_lanes: u32) -> InferenceCompileResult<ValueId> {
		let [rows, input_width, logical_length, logical_channels] = input_geometry;
		if pool.input_width().get() != input_width || pool.input_length().get() != logical_length || pool.channels().get() != logical_channels {
			return Err(InferenceCompileError::new(
				InferenceCompileErrorKind::InconsistentCheckpoint,
				format!(
	...
	#[cfg(any())]
	pub(crate) fn compile_kmeans(&mut self, block_index: usize, kmeans: &Block, input: ValueId, rows: u64, input_width: u64, tree_lanes: u32) -> InferenceCompileResult<ValueId> {
	...
	#[cfg(any())]
	pub(crate) fn compile_tree(&mut self, block_index: usize, tree: &Block, input: ValueId, rows: u64, input_width: u64, tree_lanes: u32) -> InferenceCompileResult<ValueId> {
		...
		if requirements.internal_nodes_per_tree != tree.internal_nodes_per_tree().get() || requirements.leaves_per_tree != tree.leaves_per_tree().get() {
			return Err(InferenceCompileError::new(
				InferenceCompileErrorKind::InconsistentCheckpoint,
	...
	#[cfg(any())]
	pub(crate) fn compile_residual(&mut self, residual: &Block, context: BlockInferenceContext<'_>) -> InferenceCompileResult<ValueId> {
			...
			match step {
				...
				Block::Residual { .. } => {
					let alpha = if matches!(operation, Function::Activation(activation) if activation.learned_parameters() == 1) {
						...
						validate_checkpoint_parameter_image(parameter.parameter(), &[1], "residual branch PReLU scalar")?;
						Some(self.external_checkpoint_tensor(
		...
		let skip = match residual.skip() {
			...
			Block::Residual { .. } if input_width != output_width => {
				...
				self.linear_without_bias(input, weight, rows, output_width)?
			}
			Block::Layer { .. } => {
				return Err(InferenceCompileError::new(
					InferenceCompileErrorKind::InconsistentCheckpoint,
		...
		for operation in residual.operations().iter().copied() {
			let alpha = if matches!(operation, Function::Activation(activation) if activation.learned_parameters() == 1) {
				...
				validate_checkpoint_parameter_image(parameter.parameter(), &[1], "residual output PReLU scalar")?;
				Some(self.external_checkpoint_tensor(
	...
	fn apply_saved_operation(&mut self, input: ValueId, operation: Function, prelu: Option<ValueId>, dimensions: [u64; 2], normalization_epsilon: f32, tree_lanes: u32) -> InferenceCompileResult<ValueId> {
		let [rows, width] = dimensions;
		match operation {
			Function::Activation(activation) => {
				return self.apply_activation(input, activation, prelu, shape(&[rows, width])?);
			}
			Function::Normalization(normalization) => {
				return self.apply_model_normalization(
					input,
					normalization,
					rows,
					width,
	...
	fn external_i32(&mut self, role: InferenceInputRole, shape: Shape, values: &[i32]) -> InferenceCompileResult<ValueId> {
		return self.external(
			role,
			DType::I32,
			shape,
	...
	fn pack_contiguous_f32_to_flat(&mut self, input: ValueId) -> InferenceCompileResult<ValueId> {
		...
		let indices = self.identity_indices(contract.shape)?;
		let base = self.allocate_zero_f32(shape(&[elements])?)?;
	...
	fn head_major_to_sequence(&mut self, input: ValueId, rows: u64, sequence: u64, heads: u64, head_dimension: u64) -> InferenceCompileResult<ValueId> {
		self.require_tensor(
			input,
			DType::F32,
	...
	fn causal_softmax(&mut self, scores: ValueId, rows: u64, heads: u64, sequence: u64, tree_lanes: u32) -> InferenceCompileResult<ValueId> {
		...
		return self.reinterpret_f32(softmax, shape(&[rows, heads, sequence, sequence])?);
	}
	...
	fn unpack_pool_to_matrix(&mut self, pooled: ValueId, rows: u64, groups: u64, channels: u64, output_width: u64) -> InferenceCompileResult<ValueId> {
		self.require_tensor(
			pooled,
			DType::F32,
		...
		let base = self.allocate_zero_f32(shape(&[rows, output_width])?)?;
	...
	fn linear_without_bias(&mut self, input: ValueId, weight: ValueId, rows: u64, output_width: u64) -> InferenceCompileResult<ValueId> {
		let output = self.tensor(DType::F32, shape(&[rows, output_width])?)?;
		self.emit(
			vec![input, weight],
			vec![output],
	...
	fn apply_activation(&mut self, input: ValueId, activation: Activation, prelu: Option<ValueId>, output_shape: Shape) -> InferenceCompileResult<ValueId> {
		...
		match activation {
			ForwardActivation::Identity => return Ok(input),
	...
	fn apply_model_normalization(&mut self, input: ValueId, normalization: Normalization, rows: u64, columns: u64, epsilon: f32, tree_lanes: u32) -> InferenceCompileResult<ValueId> {
		let (axis, statistic_shape, population, keep_dimensions) = match normalization {
			Normalization::Layer => (1, shape(&[rows, 1])?, f32_from_u64(columns), true),
			Normalization::Batch => (0, shape(&[columns])?, f32_from_u64(rows), false),
		};
		let matrix_shape = shape(&[rows, columns])?;
	...
	fn apply_temperature(&mut self, logits: ValueId, temperature: &CheckpointTensorImage) -> InferenceCompileResult<ValueId> {
	...
	fn compile_prediction(&mut self, values: ValueId, task: DenseTask, rows: u64, columns: u64, tree_lanes: u32) -> InferenceCompileResult<(ValueId, InferencePredictionKind)> {
		match task {
			DenseTask::BinaryClassification { .. } => {
				...
				return Ok((probabilities, InferencePredictionKind::BinaryProbability));
			}
			...
			DenseTask::ScalarRegression { .. } => {
				...
				return Ok((values, InferencePredictionKind::Regression));
			}
			...
			DenseTask::MultiTargetRegression { target_count, .. } => {
				...
				return Ok((values, InferencePredictionKind::MultiTargetRegression));
			}
		}
	}
	...
	fn stable_softmax(&mut self, logits: ValueId, rows: u64, classes: u64, tree_lanes: u32) -> InferenceCompileResult<ValueId> {
	...
	fn finish(mut self, prediction: ValueId, kind: InferencePredictionKind, rows: u64, task: InferenceTask, target_dtypes: Vec<DType>, output_adapter: Option<DenseOutputAdapter>) -> InferenceCompileResult<CompiledInference> {
...
fn validate_checkpoint_parameter_image(image: &CheckpointTensorImage, expected_shape: &[u64], role: &str) -> InferenceCompileResult<()> {
	...
	let expected_bytes = expected_shape
		...
		.try_fold(1u64, |elements, extent| {
			return elements.checked_mul(*extent);
		})
		.and_then(|elements| return elements.checked_mul(u64::from(DType::F32.byte_width())))
		.ok_or_else(|| {
			return InferenceCompileError::new(
				InferenceCompileErrorKind::ArithmeticOverflow,
...
fn shape(extents: &[u64]) -> InferenceCompileResult<Shape> { return Ok(Shape::new(extents.to_vec())?); }

fn checked_product(values: &[u64], name: &str) -> InferenceCompileResult<u64> {
	return values.iter().copied().try_fold(1u64, |product, value| {
		return product.checked_mul(value).ok_or_else(|| {
			return InferenceCompileError::new(
				InferenceCompileErrorKind::ArithmeticOverflow,
...
fn checked_i32(value: u64, name: &str) -> InferenceCompileResult<i32> {
	return i32::try_from(value).map_err(|error| {
		return InferenceCompileError::new(
			InferenceCompileErrorKind::UnsupportedExtent,
...
pub fn identity_exhausted() -> InferenceCompileError {
	return InferenceCompileError::new(
...
fn checked_i32_to_f32_program() -> InferenceCompileResult<recipe_core::ScalarProgram> {
	...
	return Ok(builder.finish(&[converted])?);
}
...
fn stable_sigmoid_exponent_program() -> InferenceCompileResult<recipe_core::ScalarProgram> {
	...
	return Ok(builder.finish(&[exponent_argument])?);
}
...
fn stable_sigmoid_result_program() -> InferenceCompileResult<recipe_core::ScalarProgram> {
	...
	return Ok(builder.finish(&[probability])?);
}
...
fn softmax_exponent_input_program() -> InferenceCompileResult<recipe_core::ScalarProgram> {
	...
	return Ok(builder.finish(&[exponent_input])?);
}
...
fn checked_one_hot_update_program(classes: i32) -> InferenceCompileResult<recipe_core::ScalarProgram> {
	...
	let one = builder.f32(1.0)?;
	return Ok(builder.finish(&[destination, one])?);
}

fn feature_destination_program(total_width: i32, start: i32, block_width: i32) -> InferenceCompileResult<recipe_core::ScalarProgram> {
	let mut builder = ScalarProgramBuilder::new()?;
	...
	return Ok(builder.finish(&[destination])?);
}

fn saved_feature_schema(checkpoint: &CheckpointArtifact) -> InferencePreparationResult<Vec<InferenceFeatureSchema>> { return saved_feature_schema_from_parts(checkpoint.vectors(), checkpoint.feature_spans()); }

fn saved_feature_schema_from_parts(vectors: &[VectorSchema], spans: &[CompiledFeatureSpan]) -> InferencePreparationResult<Vec<InferenceFeatureSchema>> {
	return spans
		.iter()
		.enumerate()
		.map(|(feature, span)| {
			let vector = vectors
				.iter()
				.find(|vector| return vector.source_index() == span.source_vector())
				.ok_or_else(|| return inconsistent_feature(feature, span, "saved feature vector is absent"))?;
			let encoding = match (
				span.lowering(),
				...
				(DenseFeatureLowering::NumericScalar, SemanticType::Numeric, VectorEncoding::I32, VectorMetadata::None) => InferenceFeatureEncoding::NumericI32,
				(DenseFeatureLowering::NumericScalar, SemanticType::Numeric, VectorEncoding::F32, VectorMetadata::None) => InferenceFeatureEncoding::NumericF32,
				(
					DenseFeatureLowering::CategoricalOneHot {
						dictionary_width,
						reserved_index,
				...
				) if dictionary_width == dictionary.len() && reserved_index == dictionary.len() && span.width() == dictionary.len().saturating_add(1) => {
					InferenceFeatureEncoding::CategoricalDictionary {
			...
			return Ok(InferenceFeatureSchema::new(
				span.source_vector(),
				vector.name(),
				encoding,
			));
...
fn validate_prepared_feature_spans(prepared: &PreparedInferenceDataset, spans: &[CompiledFeatureSpan]) -> InferencePreparationResult<()> {
	...
	for (feature, (prepared, span)) in prepared.features().iter().zip(spans).enumerate() {
		...
		match (
			...
			(DenseFeatureLowering::NumericScalar, InferenceFeatureEncoding::NumericI32, PreparedInferenceValues::I32(_)) | (DenseFeatureLowering::NumericScalar, InferenceFeatureEncoding::NumericF32, PreparedInferenceValues::F32Bits(_)) if span.width() == 1 => {}
			(
				DenseFeatureLowering::CategoricalOneHot {
					dictionary_width,
					reserved_index,
...
fn inconsistent_feature(feature: usize, span: &CompiledFeatureSpan, detail: impl Into<String>) -> InferencePreparationError {
	return InferencePreparationError::InconsistentCheckpoint {
		feature,
		source_vector: span.source_vector(),
		detail: detail.into(),
	};
