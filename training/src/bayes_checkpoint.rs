...
use crate::{
	...
	bayes::{BayesianCategoricalReferenceSet, BayesianCategoricalSchema, CATEGORICAL_BAYES_SMOOTHING, validate_categorical_reference_set},
...
impl BayesModelArtifact {
	...
	pub fn continue_with(mut self, current: Self) -> CheckpointResult<Self> {
		...
		for (saved, incoming) in self.conditionals.iter_mut().zip(current.conditionals) {
			saved.append(incoming).map_err(|error| {
	...
	#[inline]
	pub fn decode(source: &[u8], limits: BayesModelDecodeLimits) -> CheckpointResult<Self> { return decode_bayes_model(source, limits); }
	...
	pub fn save(&self, path: impl AsRef<Path>) -> CheckpointResult<()> {
		...
		if target.extension().and_then(|extension| {
...
fn validate_artifact(artifact: &BayesModelArtifact) -> CheckpointResult<()> {
	...
	if artifact.conditionals.is_empty() || (artifact.format_version == BAYES_MODEL_FORMAT_VERSION_ONE && artifact.conditionals.len() != 1) || (artifact.format_version == BAYES_MODEL_FORMAT_VERSION_TWO && artifact.conditionals.len() < 2) {
		return Err(CheckpointError::manifest(
	...
	for references in &artifact.conditionals {
		validate_categorical_reference_set(references).map_err(|error| {
			return CheckpointError::manifest(error.to_string());
		})?;
	}
	...
	for references in &artifact.conditionals {
		if references.reference_rows() != first.reference_rows() || references.reference_source_rows() != first.reference_source_rows() {
			return Err(CheckpointError::manifest(
				"Bayesian conditionals do not retain the same ordered reference partition",
			));
		}
		if !children_by_name.insert(references.child().name()) || !children_by_source.insert(references.child().source_index()) {
			return Err(CheckpointError::manifest(
...
fn encode_reference(graph: &mut Graph, root: NodeId, references: &BayesianCategoricalReferenceSet) -> CheckpointResult<()> {
	...
	field(graph, root, "reference-source-rows", &encoded_source_rows)?;
...
fn encode_schema(graph: &mut Graph, parent: NodeId, tag: &str, schema: &BayesianCategoricalSchema) -> CheckpointResult<()> {
...
fn field(graph: &mut Graph, parent: NodeId, name: impl Into<String>, value: &(impl ToString + ?Sized)) -> CheckpointResult<()> {
...
impl Decoder {
	...
	fn decode_reference_fields(&mut self, fields: &BTreeMap<String, NodeId>) -> CheckpointResult<BayesianCategoricalReferenceSet> {
		...
		let parent_codes = self.decode_i32_hex(
			&encoded_parent_codes,
			parent_elements,
			"reference-parent-codes",
		)?;
		let encoded_child_codes = self.scalar(fields["reference-child-codes"])?.to_owned();
		let child_codes = self.decode_i32_hex(
			&encoded_child_codes,
			reference_rows,
			"reference-child-codes",
		)?;
	...
	fn decode_schema(&mut self, node: NodeId) -> CheckpointResult<BayesianCategoricalSchema> {
		...
		return Ok(BayesianCategoricalSchema::from_parts(
			source_index,
			name,
			dictionary,
		));
