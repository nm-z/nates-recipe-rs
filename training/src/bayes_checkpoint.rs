use alloc::collections::{BTreeMap, BTreeSet}; use core::{fmt::Write as _, iter::once, str::from_utf8};
use std::{io::Write as _, path::Path};

use recipe_ogdl::{Graph, NodeId};

use crate::{ CheckpointError, CheckpointResult, bayes::{
		BayesianCategoricalReferenceSet, BayesianCategoricalSchema, CATEGORICAL_BAYES_SMOOTHING,
		validate_categorical_reference_set, }, checkpoint::atomic_save, };
...
pub struct BayesModelDecodeLimits { pub source_bytes: usize, pub nodes: usize, pub conditionals: usize,
	pub parents: usize, pub labels: usize, pub reference_rows: usize, pub total_payload_bytes: usize, }

impl Default for BayesModelDecodeLimits {
	#[inline]
	fn default() -> Self { return Self { source_bytes: 1 << 30, nodes: 4_000_000, conditionals: 0x0001_0000,
			parents: 0x0001_0000, labels: 1_000_000, reference_rows: 100_000_000, total_payload_bytes: 1 << 30, }; } }
...
impl BayesModelArtifact {
	...
	/// artifact contract.
	#[inline]
	pub fn new(references: BayesianCategoricalReferenceSet) -> CheckpointResult<Self> { let artifact = Self {
			format_version: BAYES_MODEL_FORMAT_VERSION_ONE, smoothing_bits: CATEGORICAL_BAYES_SMOOTHING.to_bits(),
			conditionals: vec![references], }; validate_artifact(&artifact)?; return Ok(artifact); }
	...
	pub fn from_conditionals(conditionals: Vec<BayesianCategoricalReferenceSet>) -> CheckpointResult<Self> {
		let format_version = if conditionals.len() == 1 { BAYES_MODEL_FORMAT_VERSION_ONE } else {
			BAYES_MODEL_FORMAT_VERSION_TWO }; let artifact = Self { format_version,
			smoothing_bits: CATEGORICAL_BAYES_SMOOTHING.to_bits(), conditionals, }; validate_artifact(&artifact)?;
		return Ok(artifact); }
	...
	#[must_use]
	#[inline]
	pub const fn smoothing(&self) -> f32 { return f32::from_bits(self.smoothing_bits); }

	#[must_use]
	#[inline]
	pub fn conditionals(&self) -> &[BayesianCategoricalReferenceSet] { return &self.conditionals; }
	...
	pub fn continue_with(mut self, current: Self) -> CheckpointResult<Self> { validate_artifact(&self)?;
		...
		if self.conditionals.len() != current.conditionals.len() { return Err(CheckpointError::manifest(
				"saved and current categorical Bayesian conditional counts differ",
			)); }
		for (saved, incoming) in self.conditionals.iter_mut().zip(current.conditionals) { saved.append(incoming)
				.map_err(|error| { return CheckpointError::manifest(error.to_string()); })?; }
	...
	pub fn encode(&self) -> CheckpointResult<Vec<u8>> { validate_artifact(self)?; let mut graph = Graph::new();
		...
		if self.format_version == BAYES_MODEL_FORMAT_VERSION_ONE { encode_reference(&mut graph, root, &self.conditionals[0])?;
		} else {
	...
	pub fn save(&self, path: impl AsRef<Path>) -> CheckpointResult<()> { let target = path.as_ref(); if target .extension()
		...
		let encoded = self.encode()?; let bytes = u64::try_from(encoded.len()).map_err(|error| {
			return CheckpointError::invalid_target(target, format!("Bayesian model size exceeds u64: {error}"));
		})?; return atomic_save(target, bytes, |file| { return file.write_all(&encoded); }); } }
...
pub fn decode_bayes_model(source: &[u8], limits: BayesModelDecodeLimits) -> CheckpointResult<BayesModelArtifact> {
	if source.len() > limits.source_bytes { return Err(CheckpointError::manifest(format!(
	...
	})?; if graph.len() > limits.nodes { return Err(CheckpointError::manifest(format!(
			"Bayesian model has {} nodes, limit is {}",
			graph.len(), limits.nodes ))); }
	return Decoder { graph, limits, payload_bytes: 0, labels: 0, parents: 0, reference_rows: 0, } .decode(source); }

/// Validate versioning, smoothing, schemas, partitions, and child-parent roles.
fn validate_artifact(artifact: &BayesModelArtifact) -> CheckpointResult<()> { if !matches!( artifact.format_version,
		BAYES_MODEL_FORMAT_VERSION_ONE | BAYES_MODEL_FORMAT_VERSION_TWO ) { return Err(CheckpointError::manifest(format!(
	...
	if artifact.smoothing_bits != CATEGORICAL_BAYES_SMOOTHING.to_bits() { return Err(CheckpointError::manifest(
			"categorical Bayesian model must use Recipe's Laplace-one smoothing",
		)); }
	for references in &artifact.conditionals { validate_categorical_reference_set(references) .map_err(|error| {
				return CheckpointError::manifest(error.to_string()); })?; }
	let first = &artifact.conditionals[0]; let mut children_by_name = BTreeSet::new();
	let mut children_by_source = BTreeSet::new(); let mut schemas_by_name = BTreeMap::new();
	let mut schemas_by_source = BTreeMap::new(); for references in &artifact.conditionals {
		...
		for schema in references .parents() .iter() .chain(once(references.child()))
		{ if let Some(existing) = schemas_by_name.insert(schema.name(), schema) && existing != schema {
				return Err(CheckpointError::manifest(
					"Bayesian repeated node names have inconsistent saved schemas",
				)); }
			if let Some(existing) = schemas_by_source.insert(schema.source_index(), schema) && existing != schema
			{ return Err(CheckpointError::manifest(
					"Bayesian repeated source identities have inconsistent saved schemas",
				)); } } }
	if artifact.conditionals.iter().any(|references| { return references.parents().iter().any(|parent| {
			return children_by_name.contains(parent.name()) || children_by_source.contains(&parent.source_index()); }); }) {
		return Err(CheckpointError::manifest(
...
fn encode_reference( graph: &mut Graph, root: NodeId, references: &BayesianCategoricalReferenceSet,
) -> CheckpointResult<()> {
	...
	for value in source_rows { let encoded_value = u64::try_from(*value).map_err(|error| {
	...
	field( graph, root,
		...
		&encode_i32_hex(references.parent_codes())?, )?; field( graph, root,
		"reference-child-codes",
		&encode_i32_hex(references.child_codes())?, )?; return Ok(()); }
...
fn encode_schema( graph: &mut Graph, parent: NodeId, tag: &str, schema: &BayesianCategoricalSchema,
) -> CheckpointResult<()> { let node = child(graph, parent, tag)?;
...
fn field( graph: &mut Graph, parent: NodeId, name: impl Into<String>, value: &(impl ToString + ?Sized),
) -> CheckpointResult<()> { let field = child(graph, parent, name)?; child(graph, field, value.to_string())?;
	return Ok(()); }
...
impl Decoder {
	...
	fn decode(mut self, source: &[u8]) -> CheckpointResult<BayesModelArtifact> { let &[root] = self.graph.roots() else {
			return Err(CheckpointError::manifest(format!(
				"Bayesian model requires exactly one root, found {}",
				self.graph.roots().len() ))); }; if self.node(root)?.text() != ROOT { return Err(CheckpointError::manifest(format!(
				"Bayesian model root must be {ROOT:?}"
			))); }
		let format_field = self.unique_field(root, "format-version")?;
		let format_value = self.scalar(format_field)?; let format_version = format_value.parse::<u32>().map_err(|error| {
			return CheckpointError::manifest(format!("Bayesian format-version is invalid: {error}"));
		})?; if format_version.to_string() != format_value { return Err(CheckpointError::manifest(
				"Bayesian format-version is not canonical decimal",
			)); }
		let conditionals = match format_version { BAYES_MODEL_FORMAT_VERSION_ONE => { let fields = self.fields( root, &[
						"format-version",
						"smoothing",
						"reference-rows",
						"reference-source-rows",
			...
			BAYES_MODEL_FORMAT_VERSION_TWO => {
				...
				if nodes.len() < 2 || nodes.len() > self.limits.conditionals { return Err(CheckpointError::manifest(format!(
						"Bayesian conditional count is {}, expected 2..={}",
						nodes.len(), self.limits.conditionals ))); }
				let mut conditionals = Vec::with_capacity(nodes.len()); for node in nodes {
				...
				conditionals }
			_ => { return Err(CheckpointError::manifest(format!(
					"unsupported Bayesian model version {format_version}"
				))); } }; let artifact = BayesModelArtifact { format_version, smoothing_bits: CATEGORICAL_BAYES_SMOOTHING.to_bits(),
			conditionals, }; validate_artifact(&artifact)?; if artifact.encode()?.as_slice() != source {
			return Err(CheckpointError::manifest(
	...
	fn decode_reference_fields( &mut self, fields: &BTreeMap<String, NodeId>,
		...
		self.reference_rows = self .reference_rows .checked_add(reference_rows) .ok_or_else(|| {
		...
		self.parents = self .parents .checked_add(parent_entries.len()) .ok_or_else(|| {
		...
		let mut parents = Vec::with_capacity(parent_entries.len()); for parent in parent_entries {
		...
		return BayesianCategoricalReferenceSet::from_observations( parents, child, reference_source_rows, parent_codes,
			child_codes, ) .map_err(|error| { return CheckpointError::manifest(error.to_string()); }); }
	...
	fn unique_field(&self, node: NodeId, name: &str) -> CheckpointResult<NodeId> { let matches = self .node(node)?
			.children() .iter() .copied() .filter(|child| { return self.node(*child).is_ok_and(|candidate| {
					return candidate.text() == name; }); }) .collect::<Vec<_>>(); let &[field] = matches.as_slice() else {
			return Err(CheckpointError::manifest(format!(
	...
	fn decode_schema(&mut self, node: NodeId) -> CheckpointResult<BayesianCategoricalSchema> {
		...
		self.labels = self .labels .checked_add(label_nodes.len()) .ok_or_else(|| {
				return CheckpointError::manifest("Bayesian total label count overflowed usize");
			})?; if label_nodes.is_empty() || self.labels > self.limits.labels { return Err(CheckpointError::manifest(format!(
				"Bayesian total label count is {}, limit is {}",
				self.labels, self.limits.labels ))); }
		let mut dictionary = Vec::with_capacity(label_nodes.len()); for label in label_nodes {
	...
	fn fields(&self, node: NodeId, required: &[&str]) -> CheckpointResult<BTreeMap<String, NodeId>> {
		let allowed = required.iter().copied().collect::<BTreeSet<_>>(); let mut fields = BTreeMap::new();
		for child in self.node(node)?.children() { let name = self.node(*child)?.text(); if !allowed.contains(name) {
				return Err(CheckpointError::manifest(format!(
					"unknown Bayesian field {name:?}"
				))); }
			if fields.insert(name.to_owned(), *child).is_some() { return Err(CheckpointError::manifest(format!(
					"duplicate Bayesian field {name:?}"
				))); } }
		for name in required { if !fields.contains_key(*name) { return Err(CheckpointError::manifest(format!(
	...
	fn node(&self, id: NodeId) -> CheckpointResult<&recipe_ogdl::Node> { return self.graph .node(id) .ok_or_else(|| {
	...
	fn scalar(&self, node: NodeId) -> CheckpointResult<&str> { let children = self.node(node)?.children();
		let &[value_node] = children else { return Err(CheckpointError::manifest(format!(
				"Bayesian scalar field has {} values",
				children.len() ))); }; let scalar_node = self.node(value_node)?; if !scalar_node.children().is_empty() {
			return Err(CheckpointError::manifest(
	...
	fn parse_usize(value: &str, role: &str) -> CheckpointResult<usize> { let parsed = value .parse::<usize>()
			.map_err(|error| {
				return CheckpointError::manifest(format!("Bayesian {role} is invalid: {error}"));
			})?; if parsed.to_string() != value { return Err(CheckpointError::manifest(format!(
	...
	fn decode_bytes(&mut self, value: &str, role: &str) -> CheckpointResult<Vec<u8>> { let digits = value
			.strip_prefix("0x")
			.ok_or_else(|| {
				return CheckpointError::manifest(format!("Bayesian {role} lacks 0x prefix"));
			})?; if digits.len() % 2 != 0 { return Err(CheckpointError::manifest(format!(
				"Bayesian {role} has odd hex length"
			))); }
		let bytes = digits.len() / 2; self.add_payload(bytes, role)?; return digits .as_bytes() .chunks_exact(2)
			.map(|encoded_chunk| { let hex_byte = from_utf8(encoded_chunk).map_err(|error| {
	...
	fn decode_i32_hex(&mut self, value: &str, count: usize, role: &str) -> CheckpointResult<Vec<i32>> { let digits = value
			.strip_prefix("0x")
			.ok_or_else(|| {
				return CheckpointError::manifest(format!("Bayesian {role} lacks 0x prefix"));
			})?; let expected = count .checked_mul(8) .ok_or_else(|| {
				return CheckpointError::manifest(format!("Bayesian {role} length overflowed usize"));
			})?; if digits.len() != expected { return Err(CheckpointError::manifest(format!(
				"Bayesian {role} has {} hex digits, expected {expected}",
				digits.len() ))); }
		self.add_payload(count.saturating_mul(4), role)?; return digits .as_bytes() .chunks_exact(8) .map(|encoded_chunk| {
				let hex_code = from_utf8(encoded_chunk).map_err(|error| {
					return CheckpointError::manifest(format!("Bayesian {role} has invalid UTF-8: {error}"));
				})?; let code_bits = u32::from_str_radix(hex_code, 16).map_err(|error| {
					return CheckpointError::manifest(format!("Bayesian {role} has invalid hex: {error}"));
				})?; return Ok(code_bits.cast_signed()); }) .collect(); }
	...
	fn decode_usize_hex(&mut self, value: &str, count: usize, role: &str) -> CheckpointResult<Vec<usize>> {
		let digits = value
			...
			.ok_or_else(|| {
				return CheckpointError::manifest(format!("Bayesian {role} lacks 0x prefix"));
			})?; let expected = count .checked_mul(16) .ok_or_else(|| {
				return CheckpointError::manifest(format!("Bayesian {role} length overflowed usize"));
			})?; if digits.len() != expected { return Err(CheckpointError::manifest(format!(
				"Bayesian {role} has {} hex digits, expected {expected}",
				digits.len() ))); }
		self.add_payload(count.saturating_mul(8), role)?; return digits .as_bytes() .chunks_exact(16) .map(|encoded_chunk| {
	...
	fn add_payload(&mut self, bytes: usize, role: &str) -> CheckpointResult<()> {
		self.payload_bytes = self.payload_bytes.checked_add(bytes).ok_or_else(|| { return CheckpointError::manifest(format!(
				"Bayesian {role} payload accounting overflowed usize"
			)); })?; if self.payload_bytes > self.limits.total_payload_bytes { return Err(CheckpointError::manifest(format!(
