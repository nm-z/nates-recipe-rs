...
use recipe_ingest::{PreparedDataset, PreparedVector, SemanticType, VectorEncoding, VectorMetadata, VectorRole, VectorSchema};
...
impl BayesianCategoricalReferenceSet {
	...
	pub(crate) fn from_observations(parents: Vec<BayesianCategoricalSchema>, child: BayesianCategoricalSchema, reference_source_rows: Vec<usize>, parent_codes: Vec<i32>, child_codes: Vec<i32>) -> TrainingCompileResult<Self> {
	...
	pub(crate) fn append(&mut self, current: Self) -> TrainingCompileResult<()> {
		if self.parents != current.parents || self.child != current.child || self.parent_cardinalities != current.parent_cardinalities || self.parent_multipliers != current.parent_multipliers || self.parent_configurations != current.parent_configurations {
...
#[inline]
pub fn prepare_categorical_bayesian_reference_sets(dataset: &PreparedDataset, dependencies: &[BayesianDependency]) -> TrainingCompileResult<Vec<BayesianCategoricalReferenceSet>> {
...
fn prepare_categorical_bayesian_reference_set_for_dependency(dataset: &PreparedDataset, dependency: &BayesianDependency) -> TrainingCompileResult<BayesianCategoricalReferenceSet> {
	let child = find_vector(dataset, dependency.child())?;
...
pub(crate) fn validate_categorical_reference_set(references: &BayesianCategoricalReferenceSet) -> TrainingCompileResult<()> {
	if references.parents.is_empty()
		...
		|| references.parent_codes.len()
			!= references
				.reference_rows
				.saturating_mul(references.parents.len())
	{
		return Err(bayes_compile_error(
			TrainingCompileErrorKind::InvalidNetwork,
	...
	for vector in references.parents.iter().chain(once(&references.child)) {
		if vector.name.is_empty() || vector.dictionary.is_empty() || !names.insert(vector.name.as_slice()) || !sources.insert(vector.source_index) {
			return Err(bayes_compile_error(
				TrainingCompileErrorKind::InvalidNetwork,
		...
		if vector.dictionary.iter().any(|label| {
	...
	if references.child_codes.iter().copied().any(|code| {
...
fn require_known_code(vector: &PreparedVector, values: &[Option<i32>], position: usize, source_row: usize, classes: usize) -> TrainingCompileResult<i32> {
	let code = values
		.get(position)
		.ok_or_else(|| {
		...
		.ok_or_else(|| {
			return bayes_vector_error(
				vector,
				format!("source row {source_row} is missing; the observed categorical slice requires complete rows"),
			);
		})?;
	if usize::try_from(code).map_or(true, |parsed_code| {
...
fn bayes_compile_error(kind: TrainingCompileErrorKind, detail: impl Into<String>) -> TrainingCompileError { return TrainingCompileError::new(kind, detail); }
...
impl BayesianSchemaError {
	...
	fn new(kind: BayesianSchemaErrorKind, path: impl Into<Vec<BayesianSchemaPathSegment>>, detail: impl Into<String>) -> Self {
		return Self {
			kind,
			path: path.into(),
			detail: detail.into(),
...
#[inline]
pub fn resolve_bayesian_schema(dataset: &PreparedDataset, dependencies: &[BayesianDependency]) -> BayesianSchemaResult<ResolvedBayesianSchema> {
...
fn deterministic_topological_order(nodes: &[BayesianNodeSchema], declarations: &[ResolvedBayesianDependency]) -> BayesianSchemaResult<Vec<BayesianNodeId>> {
