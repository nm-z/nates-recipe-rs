...
pub fn lower_rnn_sequence<G: RecurrentForwardGraph>(
	graph: &mut G,
	input: ValueId,
	rows: u64,
	sequence: u64,
...
///
/// Returns an error when the graph cannot emit a required value or operation.
pub fn lower_gru_sequence<G: RecurrentForwardGraph>(
	graph: &mut G,
	input: ValueId,
	rows: u64,
	sequence: u64,
...
///
/// Returns an error when the graph cannot emit a required value or operation.
pub fn lower_lstm_sequence<G: RecurrentForwardGraph>(
	graph: &mut G,
	input: ValueId,
	rows: u64,
	sequence: u64,
	...
	for step in 0..sequence {
		...
		let cell_program = {
			...
			let program_candidate = program_builder.input(DType::F32)?;
			let retained =
				program_builder.binary(ScalarOpcode::Multiply, program_forget, program_previous_cell)?;
			let admitted =
...
/// be constructed.
pub fn lower_activation<E>(activation: DenseActivation) -> Result<ForwardActivation, E>
where
...
pub fn head_major_source_index_program(
	sequence_length: u64,
	head_count: u64,
	head_dimension: u64,
...
pub fn add_program() -> LanguageResult<ScalarProgram> {
	return binary_program(ScalarOpcode::Add);
}
...
///
/// Returns an error when the scalar program cannot be constructed.
pub fn subtract_program() -> LanguageResult<ScalarProgram> {
	return binary_program(ScalarOpcode::Subtract);
}
...
///
/// Returns an error when the scalar program cannot be constructed.
pub fn multiply_program() -> LanguageResult<ScalarProgram> {
	return binary_program(ScalarOpcode::Multiply);
}
...
///
/// Returns an error when the scalar program cannot be constructed.
pub fn divide_program() -> LanguageResult<ScalarProgram> {
	return binary_program(ScalarOpcode::Divide);
}
