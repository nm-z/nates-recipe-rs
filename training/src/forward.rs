...
use recipe_language::{LanguageError, LanguageErrorKind, LanguageResult, PrimitiveAliasRule, ScalarProgramBuilder, Shape};
use recipe_ops::{OperationError, canonical_elu_program, canonical_leaky_relu_program, canonical_prelu_program, canonical_selu_program};
...
pub trait RecurrentForwardGraph {
	...
	fn gather_matrix_column(&mut self, matrix: ValueId, rows: u64, columns: u64, column: u64) -> Result<ValueId, Self::Error>;

	/// Emit a bias-free linear projection.
	fn bias_free_linear(&mut self, input: ValueId, weight: ValueId, rows: u64, output_width: u64) -> Result<ValueId, Self::Error>;

	/// Apply a scalar program elementwise to the supplied inputs.
	fn elementwise_f32(&mut self, shape: Shape, inputs: Vec<ValueId>, program: ScalarProgram) -> Result<ValueId, Self::Error>;

	/// Apply one dense activation to an input value.
	fn activate(&mut self, input: ValueId, activation: Activation, shape: Shape) -> Result<ValueId, Self::Error>;
}
...
pub fn lower_rnn_sequence<G: RecurrentForwardGraph>(graph: &mut G, input: ValueId, rows: u64, sequence: u64, width: u64, parameters: RecurrentGateParameters) -> Result<(ValueId, Vec<RnnStepValues>), G::Error> {
	...
	for step in 0..sequence {
		...
		let previous_hidden = hidden;
		let (preactivation, next_hidden) = lower_recurrent_gate(
			graph,
			step_input,
			previous_hidden,
			rows,
...
///
/// Returns an error when the graph cannot emit a required value or operation.
pub fn lower_gru_sequence<G: RecurrentForwardGraph>(graph: &mut G, input: ValueId, rows: u64, sequence: u64, width: u64, parameters: GruForwardParameters) -> Result<(ValueId, Vec<GruStepValues>), G::Error> {
	let mut hidden = graph.zero_f32(Shape::new(vec![rows, width])?)?;
	let mut steps = Vec::new();
	for step in 0..sequence {
		let step_input = graph.gather_matrix_column(input, rows, sequence, step)?;
		...
		let candidate_input_projection = graph.bias_free_linear(step_input, parameters.candidate.input_weight, rows, width)?;
		...
		let hidden_program = {
			let mut program_builder = ScalarProgramBuilder::new()?;
			let program_candidate = program_builder.input(DType::F32)?;
			let program_update = program_builder.input(DType::F32)?;
			let program_previous = program_builder.input(DType::F32)?;
			let difference = program_builder.binary(ScalarOpcode::Subtract, program_previous, program_candidate)?;
			...
			program_builder.finish(&[interpolated])?
		};
...
///
/// Returns an error when the graph cannot emit a required value or operation.
pub fn lower_lstm_sequence<G: RecurrentForwardGraph>(graph: &mut G, input: ValueId, rows: u64, sequence: u64, width: u64, parameters: LstmForwardParameters) -> Result<(ValueId, Vec<LstmStepValues>), G::Error> {
	...
	for step in 0..sequence {
		...
		let step_shape = Shape::new(vec![rows, width])?;
		let cell_program = {
			let mut program_builder = ScalarProgramBuilder::new()?;
			let program_forget = program_builder.input(DType::F32)?;
			let program_previous_cell = program_builder.input(DType::F32)?;
			let program_input = program_builder.input(DType::F32)?;
			let program_candidate = program_builder.input(DType::F32)?;
			let retained = program_builder.binary(
				ScalarOpcode::Multiply,
				program_forget,
				program_previous_cell,
			)?;
			let admitted = program_builder.binary(ScalarOpcode::Multiply, program_input, program_candidate)?;
			let updated_cell = program_builder.binary(ScalarOpcode::Add, retained, admitted)?;
			program_builder.finish(&[updated_cell])?
		};
		let next_cell = graph.elementwise_f32(
			step_shape.clone(),
...
fn lower_recurrent_gate<G: RecurrentForwardGraph>(graph: &mut G, input: ValueId, previous_hidden: ValueId, rows: u64, width: u64, parameters: RecurrentGateParameters, activation: Activation) -> Result<(ValueId, ValueId), G::Error> {
	...
	return Ok((preactivation, activated));
}
...
/// be constructed.
pub trait ActivationForward {
	fn forward_kernel<E>(self) -> Result<ForwardActivation, E>
	where E: From<LanguageError> + From<OperationError>;
}

impl ActivationForward for Activation {
	fn forward_kernel<E>(self) -> Result<ForwardActivation, E>
	where E: From<LanguageError> + From<OperationError> {
		return Ok(match self {
			Self::Linear => ForwardActivation::Identity,
			Self::Cosine => ForwardActivation::Owned("gpu_cos"),
			Self::Exponential => ForwardActivation::Owned("gpu_exp"),
			Self::Logarithm => ForwardActivation::SignedMagnitude("gpu_log_into"),
			Self::NaturalLogarithm => ForwardActivation::Owned("gpu_log_into"),
			Self::LegacySignedLogOnePlus => ForwardActivation::SignedMagnitude("gpu_log1p"),
			Self::Huber => {
			...
			Self::Tangent => ForwardActivation::Owned("gpu_tan"),
			Self::Relu => ForwardActivation::Owned("gpu_relu_into"),
			Self::LeakyRelu => ForwardActivation::Program(canonical_leaky_relu_program()?),
			Self::Sigmoid => ForwardActivation::Owned("gpu_sigmoid_into"),
			Self::Tanh => ForwardActivation::Owned("gpu_tanh_into"),
			Self::Selu => ForwardActivation::Program(canonical_selu_program()?),
			Self::Gelu => ForwardActivation::Owned("gpu_gelu_into"),
			Self::Silu => ForwardActivation::Owned("gpu_silu_into"),
			Self::Elu => ForwardActivation::Program(canonical_elu_program()?),
			Self::PRelu => ForwardActivation::PRelu(canonical_prelu_program()?),
		});
	}
}
...
pub fn head_major_source_index_program(sequence_length: u64, head_count: u64, head_dimension: u64) -> LanguageResult<ScalarProgram> {
	let mut builder = ScalarProgramBuilder::new()?;
	let position = builder.input(DType::I32)?;
	let sequence = builder.i32(attention_extent(sequence_length, "sequence length")?)?;
	...
	let source_index = builder.binary(ScalarOpcode::Add, channel_base, channel)?;
	return builder.finish(&[source_index]);
}
...
pub fn sum_program(inputs: usize) -> LanguageResult<ScalarProgram> {
	let mut builder = ScalarProgramBuilder::new()?;
	let mut input_values = Vec::with_capacity(inputs);
	for _ in 0..inputs {
		input_values.push(builder.input(DType::F32)?);
	}
...
pub fn forbidden_aliases(inputs: usize, outputs: usize) -> Vec<PrimitiveAliasRule> {
	return (0..inputs)
		.flat_map(|input| {
			return (0..outputs).map(move |output| {
				return PrimitiveAliasRule {
					input,
					output,
...
///
/// Returns an error when the scalar program cannot be constructed.
pub fn square_program() -> LanguageResult<ScalarProgram> {
	let mut builder = ScalarProgramBuilder::new()?;
	let value = builder.input(DType::F32)?;
...
///
/// Returns an error when the scalar program cannot be constructed.
pub fn multiply_constant_program(constant: f32) -> LanguageResult<ScalarProgram> { return constant_binary_program(constant, ScalarOpcode::Multiply); }
...
///
/// Returns an error when the scalar program cannot be constructed.
pub fn divide_constant_program(divisor: f32) -> LanguageResult<ScalarProgram> { return constant_binary_program(divisor, ScalarOpcode::Divide); }
...
fn constant_binary_program(constant_value: f32, opcode: ScalarOpcode) -> LanguageResult<ScalarProgram> {
	...
	let result = builder.binary(opcode, value, constant_expression)?;
	return builder.finish(&[result]);
}
...
pub fn z_score_program(epsilon_value: f32, masked: bool) -> LanguageResult<ScalarProgram> {
	...
	let mask = masked
		.then(|| {
			return builder.input(DType::F32);
		})
		.transpose()?;
	let epsilon_floor = builder.f32(epsilon_value)?;
...
pub fn l2_square_program(masked: bool) -> LanguageResult<ScalarProgram> {
	...
	let mask = masked
		...
		.transpose()?;
	let squared_value = builder.binary(ScalarOpcode::Multiply, value, value)?;
	let masked_square = match mask {
		Some(active_mask) => builder.binary(ScalarOpcode::Multiply, squared_value, active_mask)?,
		None => squared_value,
	};
...
pub fn l2_norm_program(epsilon_value: f32, masked: bool) -> LanguageResult<ScalarProgram> {
	let mut builder = ScalarProgramBuilder::new()?;
	let value = builder.input(DType::F32)?;
	let input_norm_squared = builder.input(DType::F32)?;
	let mask = masked
		.then(|| {
			return builder.input(DType::F32);
		})
...
fn select_normalized(builder: &mut ScalarProgramBuilder, value: recipe_language::ScalarExpression, normalized: recipe_language::ScalarExpression, mask: Option<recipe_language::ScalarExpression>) -> LanguageResult<recipe_language::ScalarExpression> {
	let Some(active_mask) = mask else {
		return Ok(normalized);
	};
	let zero = builder.f32(0.0)?;
	let normalize = builder.binary(ScalarOpcode::GreaterThan, active_mask, zero)?;
