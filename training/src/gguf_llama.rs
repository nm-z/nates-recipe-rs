...
use recipe_ingest::{GgufArchive, GgufEndian, GgufLimits, GgufMetadataValue, GgufTensorType, RawTable, SourceLimit, parse_contract_i32, parse_gguf, read_source_snapshot};
...
use super::{InferenceCompileError, InferenceCompileErrorKind, InferenceCompileResult, InferenceGraphCompiler, InferenceInputRole, InferencePredictionKind, InferencePreparationResult, InferenceTask, checked_i32, checked_product, shape};
use crate::{
	Activation, CompiledInference, MAXIMUM_REDUCTION_TREE_LANES,
...
impl fmt::Display for GgufLlamaErrorKind {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		return f.write_str(match *self {
			...
			Self::MissingTensor => "missing tensor",
			Self::InvalidTensor => "invalid tensor",
			Self::InvalidTokenStream => "invalid token stream",
...
impl GgufLlamaError {
	#[must_use]
	#[inline]
	pub const fn kind(&self) -> GgufLlamaErrorKind { return self.kind; }
...
impl GgufLlamaArtifact {
	#[must_use]
	#[inline]
	pub const fn architecture(&self) -> &'static str { return "llama"; }

	#[must_use]
	#[inline]
	pub const fn vocabulary(&self) -> u64 { return self.vocabulary; }

	#[must_use]
	#[inline]
	pub const fn context_length(&self) -> u64 { return self.context_length; }

	#[must_use]
	#[inline]
	pub const fn embedding_length(&self) -> u64 { return self.embedding_length; }

	#[must_use]
	#[inline]
	pub const fn block_count(&self) -> usize { return self.blocks.len(); }

	fn tensor(&self, index: usize) -> &GgufLlamaTensorImage { return &self.tensors[index]; }
	...
	fn execution_tensor_indices(&self) -> BTreeSet<usize> {
		...
		for block in &self.blocks {
			indices.insert(block.attention_norm);
			indices.insert(block.feed_forward_norm);
			for linear in [
				block.query,
				block.key,
				block.value,
				...
				indices.insert(linear.weight);
				indices.extend(linear.bias);
				indices.extend(linear.scale);
			}
		}
		if let Some(rope_factors) = self.rope_factors {
			indices.remove(&rope_factors);
		}
		return indices;
	}
}
...
impl PreparedGgufLlamaInference {
	#[must_use]
	#[inline]
	pub const fn artifact(&self) -> &GgufLlamaArtifact { return &self.artifact; }

	#[must_use]
	#[inline]
	pub fn tokens(&self) -> &[i32] { return &self.tokens; }
}

#[inline]
pub fn decode_gguf_llama(bytes: &[u8], limits: GgufLimits) -> GgufLlamaResult<GgufLlamaArtifact> {
	let archive = parse_gguf(bytes, limits).map_err(|error| return GgufLlamaError::new(GgufLlamaErrorKind::Container, error.to_string()))?;
	if archive.version() != 3 {
		return Err(GgufLlamaError::new(
	...
	if archive.endian() != GgufEndian::Little {
		return Err(GgufLlamaError::new(
			GgufLlamaErrorKind::UnsupportedVariant,
	...
	if architecture != "llama" {
		return Err(GgufLlamaError::new(
			GgufLlamaErrorKind::UnsupportedArchitecture,
	...
	if heads != key_value_heads {
		return Err(GgufLlamaError::new(
			GgufLlamaErrorKind::UnsupportedVariant,
			format!("grouped-query attention is not in the first llama instrument ({heads} query heads, {key_value_heads} KV heads)"),
		));
	}
	if !embedding_length.is_multiple_of(heads) {
		return Err(GgufLlamaError::new(
			GgufLlamaErrorKind::InvalidMetadata,
	...
	if rotary_dimension != head_dimension || !rotary_dimension.is_multiple_of(2) {
		return Err(GgufLlamaError::new(
			GgufLlamaErrorKind::UnsupportedVariant,
			format!("the first llama instrument requires even full-head RoPE; rotary dimension {rotary_dimension}, head dimension {head_dimension}"),
		));
	}
	...
	if key_length != head_dimension || value_length != head_dimension {
		return Err(GgufLlamaError::new(
			GgufLlamaErrorKind::UnsupportedVariant,
	...
	let effective_ext = if configured_ext < 0.0 {
		0.0
	} else {
		configured_ext
	};
	...
	let yarn_attention_factor = metadata_optional_f32(&archive, "llama.rope.scaling.yarn_attn_factor")?.unwrap_or(1.0);
	...
	if !(effective_attention_factor.is_finite() && effective_attention_factor > 0.0) {
		return Err(GgufLlamaError::new(
			GgufLlamaErrorKind::InvalidMetadata,
	...
	let attention_scale = if configured_attention_scale == 0.0 {
		1.0 / f32_from_u64(head_dimension).sqrt()
	} else {
		configured_attention_scale
	};
	...
	if !(clamp_kqv.is_finite() && clamp_kqv >= 0.0) {
		return Err(GgufLlamaError::new(
			GgufLlamaErrorKind::InvalidMetadata,
	...
	for block in 0..block_count {
		...
		let query = tensors.linear(
			&prefix,
			"attn_q",
			&[embedding_length, embedding_length],
			embedding_length,
		)?;
		let key = tensors.linear(
			&prefix,
			"attn_k",
			&[embedding_length, embedding_length],
			embedding_length,
		)?;
		let value = tensors.linear(
			&prefix,
			"attn_v",
			&[embedding_length, embedding_length],
			embedding_length,
		)?;
		let attention_output = tensors.linear(
			&prefix,
			"attn_output",
			&[embedding_length, embedding_length],
			embedding_length,
		...
		let feed_forward_gate = tensors.linear(
			&prefix,
			"ffn_gate",
			&[embedding_length, feed_forward_length],
			feed_forward_length,
		)?;
		let feed_forward_up = tensors.linear(
			&prefix,
			"ffn_up",
			&[embedding_length, feed_forward_length],
			feed_forward_length,
		)?;
		let feed_forward_down = tensors.linear(
			&prefix,
			"ffn_down",
	...
	let tensor_images = tensors.finish()?;
	if let Some(index) = rope_factors {
		for (factor, bytes) in tensor_images[index]
			.bytes
			.as_chunks::<4>()
			.0
			.iter()
			.enumerate()
		{
			...
			if !value.is_finite() || value == 0.0 {
				return Err(GgufLlamaError::new(
					GgufLlamaErrorKind::InvalidTensor,
					format!("rope_freqs.weight[{factor}] must be finite and nonzero"),
				));
			}
		}
	}
	return Ok(GgufLlamaArtifact {
		vocabulary,
		context_length,
		embedding_length,
		feed_forward_length,
		...
		rope_base_bits: rope_base.to_bits(),
		rope_frequency_scale_bits: rope_frequency_scale.to_bits(),
		rope_attention_factor_bits: effective_attention_factor.to_bits(),
		attention_scale_bits: attention_scale.to_bits(),
		clamp_kqv_bits: clamp_kqv.to_bits(),
...
pub fn load_gguf_llama_model_file(path: &Path, limits: GgufLimits) -> InferencePreparationResult<GgufLlamaArtifact> {
	...
	return decode_gguf_llama(snapshot.bytes(), limits).map_err(Into::into);
}

#[inline]
pub fn prepare_gguf_llama_inference_table(artifact: GgufLlamaArtifact, table: &RawTable) -> InferencePreparationResult<PreparedGgufLlamaInference> {
	if table.width() != 1 {
		return Err(GgufLlamaError::new(
	...
	for (row, fields) in table.rows().iter().enumerate() {
		...
		for token in field
			.split(|byte| return byte.is_ascii_whitespace())
			.filter(|token| return !token.is_empty())
		{
			let token = core::str::from_utf8(token).map_err(|error| {
				return GgufLlamaError::new(
					GgufLlamaErrorKind::InvalidTokenStream,
					format!("token row {row} is not UTF-8: {error}"),
				);
			})?;
			let value = parse_contract_i32(token)
				.map_err(|error| {
					return GgufLlamaError::new(
						GgufLlamaErrorKind::InvalidTokenStream,
			...
			if value < 0
				|| u64::try_from(value)
					.ok()
					.is_none_or(|value| return value >= artifact.vocabulary)
			{
				return Err(GgufLlamaError::new(
					GgufLlamaErrorKind::InvalidTokenStream,
					format!(
						"token {} is outside the exact vocabulary domain 0..{}",
						value, artifact.vocabulary
					),
				)
				.into());
			}
	...
	if u64::try_from(tokens.len())
		.ok()
		.is_none_or(|count| return count > artifact.context_length)
	{
		return Err(GgufLlamaError::new(
			GgufLlamaErrorKind::InvalidTokenStream,
			format!(
				...
				tokens.len(),
				artifact.context_length
			),
		)
		.into());
	}
	return Ok(PreparedGgufLlamaInference { artifact, tokens });
}

#[inline]
pub fn compile_prepared_gguf_llama_inference(prepared: &PreparedGgufLlamaInference) -> InferenceCompileResult<CompiledInference> {
	let artifact = &prepared.artifact;
	let sequence = u64::try_from(prepared.tokens.len()).map_err(|error| {
	...
	let token_ids = compiler.external(
		...
		prepared
			.tokens
			.iter()
			.flat_map(|token| return token.to_le_bytes())
			.collect(),
	)?;
	let mut values = BTreeMap::new();
	for index in artifact.execution_tensor_indices() {
		let tensor = artifact.tensor(index);
		let value = compiler.external(
			InferenceInputRole::GgufTensor { tensor: index },
			DType::F32,
			shape(&tensor.shape)?,
			tensor.bytes.clone(),
		)?;
	...
	let mut current = compiler.tensor(DType::F32, shape(&[sequence, artifact.embedding_length])?)?;
	compiler.emit(
		vec![token_embedding, token_ids],
		vec![current],
	...
	let rope = prepare_rope_inputs(&mut compiler, artifact, sequence)?;
	for block in &artifact.blocks {
		let residual = current;
		current = rms_norm(
			&mut compiler,
			current,
			tensor_value(&values, block.attention_norm)?,
			sequence,
			artifact.embedding_length,
			f32::from_bits(artifact.rms_epsilon_bits),
		)?;
		let query = linear(
			&mut compiler,
			current,
		...
		let attention = causal_attention(&mut compiler, artifact, sequence, query, key, value, &rope)?;
		current = linear(
			&mut compiler,
			attention,
			block.attention_output,
		...
		current = compiler.exact_add(current, residual)?;
		let feed_forward_residual = current;
		current = rms_norm(
			&mut compiler,
			current,
			tensor_value(&values, block.feed_forward_norm)?,
			sequence,
			artifact.embedding_length,
			f32::from_bits(artifact.rms_epsilon_bits),
		)?;
		...
		let gate = compiler.apply_activation(
			gate,
			Activation::Silu,
			None,
			shape(&[sequence, artifact.feed_forward_length])?,
		)?;
		let gated = compiler.tensor(
			DType::F32,
			shape(&[sequence, artifact.feed_forward_length])?,
		)?;
		compiler.emit_elementwise(vec![gate, up], vec![gated], multiply_program()?)?;
		current = linear(
			&mut compiler,
			gated,
			block.feed_forward_down,
	...
	current = rms_norm(
		&mut compiler,
		current,
		tensor_value(&values, artifact.output_norm)?,
		sequence,
		artifact.embedding_length,
		f32::from_bits(artifact.rms_epsilon_bits),
	)?;
	let output_weight = tensor_value(&values, artifact.output)?;
	let logits = compiler.tensor(DType::F32, shape(&[sequence, artifact.vocabulary])?)?;
	compiler.emit(
		vec![current, output_weight],
		vec![logits],
...
fn prepare_rope_inputs(compiler: &mut InferenceGraphCompiler, artifact: &GgufLlamaArtifact, sequence: u64) -> InferenceCompileResult<RopeInputs> {
	let rotary_half = artifact.head_dimension / 2;
	let factor_bytes = artifact.rope_factors.map_or_else(
		|| {
			return (0..rotary_half)
				.flat_map(|_| return 1.0f32.to_le_bytes())
				.collect();
		},
		|index| return artifact.tensor(index).bytes.clone(),
	);
	let factor_values = factor_bytes
		.as_chunks::<4>()
		.0
		.iter()
		.map(|bytes| return f32::from_le_bytes(*bytes))
		.collect::<Vec<_>>();
	let rows = sequence.checked_mul(artifact.heads).ok_or_else(|| {
		return InferenceCompileError::new(
	...
	let elements = checked_product(
		&[rows, artifact.head_dimension],
		"GGUF llama RoPE element count",
	)?;
	checked_i32(elements, "GGUF llama RoPE element count")?;
	let capacity = usize::try_from(elements).map_err(|error| {
		return InferenceCompileError::new(
			InferenceCompileErrorKind::UnsupportedExtent,
			format!("GGUF llama RoPE element count does not fit usize: {error}"),
		);
	})?;
	let theta_scale = f32::from_bits(artifact.rope_base_bits).powf(-2.0 / f32_from_u64(artifact.head_dimension));
	...
	let mut partner_indices = Vec::with_capacity(capacity);
	let mut cosine_bits = Vec::with_capacity(capacity);
	let mut signed_sine_bits = Vec::with_capacity(capacity);
	for row in 0..rows {
		let token = row / artifact.heads;
		let mut theta = f32_from_u64(token);
		for pair in 0..rotary_half {
			let factor_index = usize::try_from(pair).map_err(|error| {
				return InferenceCompileError::new(
					InferenceCompileErrorKind::UnsupportedExtent,
					format!("GGUF llama RoPE factor index does not fit usize: {error}"),
				);
			})?;
			let factor = factor_values[factor_index];
			let angle = frequency_scale * theta / factor;
			let cosine = angle.cos() * attention_factor;
			let sine = angle.sin() * attention_factor;
			let base = row
				.checked_mul(artifact.head_dimension)
				.and_then(|base| return base.checked_add(pair * 2))
				.ok_or_else(|| {
					return InferenceCompileError::new(
						InferenceCompileErrorKind::ArithmeticOverflow,
			...
			signed_sine_bits.extend([(-sine).to_bits(), sine.to_bits()]);
			theta *= theta_scale;
		}
	}
	let flat_shape = shape(&[elements])?;
	let partners = compiler.external_i32(
		InferenceInputRole::GgufRopePartnerIndices,
		flat_shape.clone(),
		&partner_indices,
	)?;
	let cosines = compiler.external(
	...
	let signed_sines = compiler.external(
...
fn causal_attention(compiler: &mut InferenceGraphCompiler, artifact: &GgufLlamaArtifact, sequence: u64, query: ValueId, key: ValueId, value: ValueId, rope: &RopeInputs) -> InferenceCompileResult<ValueId> {
	...
	let scores = compiler.tensor(DType::F32, score_shape.clone())?;
	compiler.emit(
		vec![query, key],
		vec![scores],
		PrimitiveKind::Contraction(Contraction {
	...
	let scaled = compiler.tensor(DType::F32, score_shape)?;
	compiler.emit_elementwise(
		vec![scores],
		vec![scaled],
		multiply_constant_program(f32::from_bits(artifact.attention_scale_bits))?,
	)?;
	let probabilities = compiler.causal_softmax(
		scaled,
		1,
		artifact.heads,
		sequence,
	...
	let context = compiler.tensor(
		DType::F32,
		shape(&[1, artifact.heads, sequence, artifact.head_dimension])?,
	)?;
	compiler.emit(
	...
	let context = compiler.head_major_to_sequence(
		context,
		1,
		sequence,
		artifact.heads,
		artifact.head_dimension,
	)?;
	return compiler.reinterpret_f32(context, shape(&[sequence, artifact.embedding_length])?);
}

fn apply_rope(compiler: &mut InferenceGraphCompiler, artifact: &GgufLlamaArtifact, sequence: u64, input: ValueId, rope: &RopeInputs) -> InferenceCompileResult<ValueId> {
	let rows = sequence.checked_mul(artifact.heads).ok_or_else(|| {
		return InferenceCompileError::new(
			InferenceCompileErrorKind::ArithmeticOverflow,
			"GGUF llama RoPE row count overflowed u64",
	...
	let rotated = compiler.tensor(DType::F32, shape(&[elements])?)?;
	let parameters = [
	...
	return compiler.reinterpret_f32(
		rotated,
		shape(&[1, sequence, artifact.heads, artifact.head_dimension])?,
	);
}

fn linear(compiler: &mut InferenceGraphCompiler, input: ValueId, linear: GgufLlamaLinear, values: &BTreeMap<usize, ValueId>, dimensions: [u64; 3], scale_before_bias: bool, clamp: f32) -> InferenceCompileResult<ValueId> {
	...
	let weight = tensor_value(values, linear.weight)?;
	compiler.require_tensor(
		weight,
		DType::F32,
	...
	let mut current = compiler.tensor(DType::F32, output_shape.clone())?;
	compiler.emit(
		vec![input, weight],
		vec![current],
		PrimitiveKind::Contraction(Contraction {
	...
	if let Some(bias) = linear.bias {
		...
		compiler.emit_elementwise(vec![current, bias], vec![output], add_program()?)?;
		current = output;
	}
	if !scale_before_bias {
		current = apply_optional_scale(compiler, current, linear.scale, values, &output_shape)?;
	...
	if clamp > 0.0 {
		let output = compiler.tensor(DType::F32, output_shape)?;
		compiler.emit_elementwise(vec![current], vec![output], symmetric_clamp_program(clamp)?)?;
		current = output;
	}
	return Ok(current);
}

fn apply_optional_scale(compiler: &mut InferenceGraphCompiler, input: ValueId, scale_index: Option<usize>, values: &BTreeMap<usize, ValueId>, output_shape: &recipe_language::Shape) -> InferenceCompileResult<ValueId> {
	let Some(scale_index) = scale_index else {
		return Ok(input);
	};
	let scale = tensor_value(values, scale_index)?;
	compiler.require_tensor(scale, DType::F32, &[1], "GGUF llama linear scale")?;
	let output = compiler.tensor(DType::F32, output_shape.clone())?;
	compiler.emit_elementwise(vec![input, scale], vec![output], multiply_program()?)?;
	return Ok(output);
}

fn rms_norm(compiler: &mut InferenceGraphCompiler, input: ValueId, scale: ValueId, rows: u64, columns: u64, epsilon: f32) -> InferenceCompileResult<ValueId> {
	compiler.require_tensor(
		input,
		DType::F32,
		&[rows, columns],
	...
	let output = compiler.tensor(DType::F32, shape(&[rows, columns])?)?;
	let parameters = [
		(
			"epsilon".to_owned(),
		...
		(
			"tree_lanes".to_owned(),
			PreparedParameter::U64(u64::from(MAXIMUM_REDUCTION_TREE_LANES)),
		),
	]
	.into_iter()
	.collect::<PreparedParameters>();
...
fn symmetric_clamp_program(limit: f32) -> InferenceCompileResult<recipe_core::ScalarProgram> {
	...
	let lower = builder.f32(-limit)?;
	let upper = builder.f32(limit)?;
	let value = builder.binary(ScalarOpcode::Maximum, value, lower)?;
	let value = builder.binary(ScalarOpcode::Minimum, value, upper)?;
	return Ok(builder.finish(&[value])?);
}
...
impl<'archive, 'data> ArtifactTensorBuilder<'archive, 'data> {
	const fn new(archive: &'archive GgufArchive<'data>) -> Self {
		return Self {
			archive,
			images: Vec::new(),
	...
	fn required(&mut self, name: &str, dimensions: &[u64]) -> GgufLlamaResult<usize> {
		return self.capture(name, dimensions)?.ok_or_else(|| {
			return GgufLlamaError::new(
				GgufLlamaErrorKind::MissingTensor,
				format!("required llama tensor {name:?} is absent"),
			);
		});
	}

	fn optional(&mut self, name: &str, dimensions: &[u64]) -> GgufLlamaResult<Option<usize>> { return self.capture(name, dimensions); }
	...
	fn linear(&mut self, prefix: &str, stem: &str, weight_dimensions: &[u64], output_width: u64) -> GgufLlamaResult<GgufLlamaLinear> {
		return Ok(GgufLlamaLinear {
	...
	fn capture(&mut self, name: &str, dimensions: &[u64]) -> GgufLlamaResult<Option<usize>> {
		...
		self.validate(name, dimensions)?;
		let bytes = self.archive.raw_tensor(name).ok_or_else(|| {
			return GgufLlamaError::new(
				GgufLlamaErrorKind::InvalidTensor,
		...
		self.images.push(GgufLlamaTensorImage {
			name: name.to_owned(),
			shape: tensor.dimensions().iter().rev().copied().collect(),
			bytes: bytes.to_vec(),
		});
		self.consumed.insert(name.to_owned());
	...
	fn validate(&self, name: &str, dimensions: &[u64]) -> GgufLlamaResult<()> {
		let tensor = self.archive.tensor(name).ok_or_else(|| {
			return GgufLlamaError::new(
				GgufLlamaErrorKind::MissingTensor,
				format!("llama tensor {name:?} is absent"),
			);
		})?;
		if tensor.tensor_type() != GgufTensorType::F32 || tensor.dimensions() != dimensions {
			return Err(GgufLlamaError::new(
				GgufLlamaErrorKind::InvalidTensor,
				format!(
	...
	fn finish(self) -> GgufLlamaResult<Vec<GgufLlamaTensorImage>> {
		if let Some(tensor) = self
			...
			.tensors()
			.iter()
			.find(|tensor| return !self.consumed.contains(tensor.name()))
		{
			return Err(GgufLlamaError::new(
				GgufLlamaErrorKind::UnsupportedVariant,
				format!(
...
fn metadata_value<'archive, 'data>(archive: &'archive GgufArchive<'data>, key: &str) -> GgufLlamaResult<&'archive GgufMetadataValue<'data>> {
	return archive
		.metadata_entry(key)
...
fn metadata_string<'data>(archive: &GgufArchive<'data>, key: &str) -> GgufLlamaResult<&'data str> {
	let value = metadata_value(archive, key)?;
	if let GgufMetadataValue::String(string) = value {
		return Ok(string);
	}
	return Err(metadata_type_error(key, "string", value));
}

fn metadata_optional_string<'data>(archive: &GgufArchive<'data>, key: &str) -> GgufLlamaResult<Option<&'data str>> {
	match archive
		.metadata_entry(key)
		.map(|entry| return entry.value())
	{
		None => return Ok(None),
		Some(GgufMetadataValue::String(value)) => return Ok(Some(value)),
		Some(value) => return Err(metadata_type_error(key, "string", value)),
	}
}

fn metadata_u32(archive: &GgufArchive<'_>, key: &str) -> GgufLlamaResult<u64> {
	let value = metadata_value(archive, key)?;
	if let GgufMetadataValue::U32(integer) = value {
		return Ok(u64::from(*integer));
	}
	return Err(metadata_type_error(key, "u32", value));
}
...
fn metadata_optional_u32(archive: &GgufArchive<'_>, key: &str) -> GgufLlamaResult<Option<u64>> {
	...
	{
		None => return Ok(None),
		Some(GgufMetadataValue::U32(value)) => return Ok(Some(u64::from(*value))),
		Some(value) => return Err(metadata_type_error(key, "u32", value)),
	}
}

fn metadata_f32(archive: &GgufArchive<'_>, key: &str) -> GgufLlamaResult<f32> {
	let value = metadata_value(archive, key)?;
	if let GgufMetadataValue::F32Bits(bits) = value {
		return Ok(f32::from_bits(*bits));
	}
	return Err(metadata_type_error(key, "f32", value));
}
...
fn metadata_optional_f32(archive: &GgufArchive<'_>, key: &str) -> GgufLlamaResult<Option<f32>> {
	...
	{
		None => return Ok(None),
		Some(GgufMetadataValue::F32Bits(bits)) => return Ok(Some(f32::from_bits(*bits))),
		Some(value) => return Err(metadata_type_error(key, "f32", value)),
	}
}
...
fn metadata_optional_bool(archive: &GgufArchive<'_>, key: &str) -> GgufLlamaResult<Option<bool>> {
	...
	{
		None => return Ok(None),
		Some(GgufMetadataValue::Bool(value)) => return Ok(Some(*value)),
		Some(value) => return Err(metadata_type_error(key, "bool", value)),
	}
}
...
fn require_zero_swiglu_clamps(archive: &GgufArchive<'_>, block_count: u64) -> GgufLlamaResult<()> {
	...
	let GgufMetadataValue::Array(values) = entry.value() else {
	...
	for (block, value) in values.values().iter().take(count).enumerate() {
		let GgufMetadataValue::F32Bits(bits) = value else {
			return Err(metadata_type_error(
				"llama.swiglu_clamp_shexp",
				"f32 array",
				value,
		...
		if f32::from_bits(*bits).abs() > 1.0e-6 {
			return Err(GgufLlamaError::new(
				GgufLlamaErrorKind::UnsupportedVariant,
...
fn to_usize(value: u64, role: &str) -> GgufLlamaResult<usize> {
	return usize::try_from(value).map_err(|error| {
		return GgufLlamaError::new(
			GgufLlamaErrorKind::ArithmeticOverflow,
