...
use crate::{
	Error, Result,
	abi::{
		HsaAgentGetInfo, HsaAgentIterateIsas, HsaAmdAgentIterateMemoryPools, HsaAmdAgentsAllowAccess, HsaAmdMemoryAsyncCopy, HsaAmdMemoryPoolAllocate, HsaAmdMemoryPoolFree, HsaAmdMemoryPoolGetInfo, HsaCodeObjectReaderCreateFromMemory, HsaCodeObjectReaderDestroy, HsaExecutableCreateAlt, HsaExecutableDestroy, HsaExecutableFreeze, HsaExecutableGetSymbolByName, HsaExecutableLoadAgentCodeObject, HsaExecutableSymbolGetInfo, HsaInit, HsaIsaGetInfoAlt, HsaIterateAgents, HsaQueueCreate, HsaQueueDestroy, HsaQueueLoadReadIndexScAcquire, HsaQueueLoadWriteIndexRelaxed,
		HsaQueueStoreWriteIndexScRelease, HsaShutDown, HsaSignalCreate, HsaSignalDestroy, HsaSignalLoadScAcquire, HsaSignalStoreScRelease, HsaSignalWaitScAcquire, HsaStatus, HsaStatusString, HsaSystemGetInfo, STATUS_SUCCESS,
	},
};
...
impl Library {
	fn open(path: &OsStr) -> Result<Self> {
		...
		let Some(handle) = NonNull::new(handle) else {
			return Err(Error::LibraryOpen {
				...
				detail: unsafe { take_dlerror() }.unwrap_or_else(|| "dynamic loader returned a null handle".to_owned()),
			});
		};
	...
	fn symbol(&self, name: &'static str, name_with_nul: &'static [u8]) -> Result<*mut c_void> {
		...
		let address = unsafe {
			...
			libc::dlsym(
				self.handle.as_ptr(),
				name_with_nul.as_ptr().cast::<c_char>(),
			)
		};
		...
		if let Some(detail) = unsafe { take_dlerror() } {
			return Err(Error::MissingSymbol {
				library: self.display_name.clone(),
				symbol: name,
				detail,
			});
		}
...
pub(crate) struct Api {
	_library: Library,
	pub(crate) init: HsaInit,
	pub(crate) shut_down: HsaShutDown,
	pub(crate) status_string: HsaStatusString,
	pub(crate) system_get_info: HsaSystemGetInfo,
	pub(crate) iterate_agents: HsaIterateAgents,
	pub(crate) agent_get_info: HsaAgentGetInfo,
	pub(crate) agent_iterate_isas: HsaAgentIterateIsas,
	pub(crate) isa_get_info_alt: HsaIsaGetInfoAlt,
	pub(crate) queue_create: HsaQueueCreate,
	pub(crate) queue_destroy: HsaQueueDestroy,
	...
	pub(crate) executable_create_alt: HsaExecutableCreateAlt,
	pub(crate) executable_destroy: HsaExecutableDestroy,
	...
	pub(crate) amd_memory_pool_allocate: HsaAmdMemoryPoolAllocate,
	pub(crate) amd_memory_pool_free: HsaAmdMemoryPoolFree,
	pub(crate) amd_agents_allow_access: HsaAmdAgentsAllowAccess,
	pub(crate) amd_memory_async_copy: HsaAmdMemoryAsyncCopy,
	pub(crate) signal_create: HsaSignalCreate,
	pub(crate) signal_destroy: HsaSignalDestroy,
	pub(crate) signal_load_scacquire: HsaSignalLoadScAcquire,
	pub(crate) signal_store_screlease: HsaSignalStoreScRelease,
...
impl Api {
	pub(crate) fn load(path: &OsStr) -> Result<Self> {
		...
		let queue_load_write_index_relaxed = symbol!(
			"hsa_queue_load_write_index_relaxed",
			HsaQueueLoadWriteIndexRelaxed
		);
		let queue_store_write_index_screlease = symbol!(
		...
		Ok(Self {
			...
			queue_create,
			queue_destroy,
			amd_agent_iterate_memory_pools,
			amd_memory_pool_get_info,
			code_object_reader_create_from_memory,
			code_object_reader_destroy,
			executable_create_alt,
			executable_destroy,
			executable_load_agent_code_object,
			executable_freeze,
			executable_get_symbol_by_name,
			executable_symbol_get_info,
			amd_memory_pool_allocate,
			amd_memory_pool_free,
			amd_agents_allow_access,
			amd_memory_async_copy,
			signal_create,
			signal_destroy,
			signal_load_scacquire,
			signal_store_screlease,
			signal_wait_scacquire,
			queue_load_read_index_scacquire,
			queue_load_write_index_relaxed,
	...
	fn status_message(&self, status: HsaStatus) -> Option<String> {
		...
		if query_status != STATUS_SUCCESS || pointer.is_null() {
			return None;
		}
