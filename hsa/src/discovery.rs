...
use crate::{AgentUuid, DeviceType, Error, IsaTarget, MemoryLocation, MemoryPoolFlags, MemorySegment, PciAddress, Profile, QueueKind, Result, RoundingModes, Runtime, abi::*, identity::c_bool};
...
fn validate_queue_capabilities(queue: QueueCapabilities) -> Result<()> {
	let valid = queue.maximum_queues != 0 && queue.minimum_packets != 0 && queue.minimum_packets.is_power_of_two() && queue.maximum_packets != 0 && queue.maximum_packets.is_power_of_two() && queue.minimum_packets <= queue.maximum_packets;
	if valid {
		Ok(())
	} else {
...
fn agent_info<T: Copy>(api: &crate::loader::Api, agent: HsaAgent, attribute: HsaInfo, operation: &'static str) -> Result<T> {
	let mut value = MaybeUninit::<T>::uninit();
...
fn pool_info<T: Copy>(api: &crate::loader::Api, pool: HsaMemoryPool, attribute: HsaInfo, operation: &'static str) -> Result<T> {
	let mut value = MaybeUninit::<T>::uninit();
	// SAFETY: every call site pairs the reviewed attribute with its exact
	// public-header output type.
...
fn pool_info_optional<T: Copy>(api: &crate::loader::Api, pool: HsaMemoryPool, attribute: HsaInfo, operation: &'static str) -> Result<Option<T>> {
	let mut value = MaybeUninit::<T>::uninit();
	// SAFETY: every call site pairs the reviewed attribute with its exact
	// public-header output type.
	let status = unsafe { (api.amd_memory_pool_get_info)(pool, attribute, value.as_mut_ptr().cast::<c_void>()) };
...
fn agent_string<const N: usize>(api: &crate::loader::Api, agent: HsaAgent, attribute: HsaInfo, field: &'static str) -> Result<String> {
