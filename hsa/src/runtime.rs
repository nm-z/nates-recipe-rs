...
use crate::{ Error, Result, abi::STATUS_SUCCESS, discovery::{self, Discovery}, loader::Api, };
...
/// to it.
pub struct Runtime { pub(crate) api: Arc<Api>, active: bool, }

impl Runtime {
	/// Load the first normal ROCr soname that is installed and initialize it.
	pub fn open_default() -> Result<Self> { let mut last_open_error = None;
		for candidate in ["libhsa-runtime64.so.1", "libhsa-runtime64.so"] {
			match Self::open(candidate) { Ok(runtime) => return Ok(runtime),
				Err(error @ Error::LibraryOpen { .. }) => last_open_error = Some(error), Err(error) => return Err(error), } }
	...
	pub fn open(path: impl AsRef<OsStr>) -> Result<Self> { let api = Arc::new(Api::load(path.as_ref())?);
		// SAFETY: the function pointer was resolved with the reviewed ABI and
		// `api` keeps its defining library loaded.
		let status = unsafe { (api.init)() }; if status != STATUS_SUCCESS {
	...
	pub fn discover(&self) -> Result<Discovery<'_>> {
		self.ensure_active()?; discovery::discover(self) }
	...
	pub fn close(mut self) -> Result<()> { self.ensure_active()?; self.active = false;
	...
	pub(crate) fn ensure_active(&self) -> Result<()> { if self.active { Ok(()) } else { Err(Error::RuntimeClosed) } } }

impl Drop for Runtime { fn drop(&mut self) { if !self.active { return; } self.active = false;
		// SAFETY: same invariant as `close`; Drop cannot return the status.
		unsafe { (self.api.shut_down)(); } } }
