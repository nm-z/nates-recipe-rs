...
pub(crate) type HsaAgentGetInfo = unsafe extern "C" fn(agent: HsaAgent, attribute: HsaInfo, value: *mut c_void) -> HsaStatus;
pub(crate) type HsaAgentIterateIsas = unsafe extern "C" fn(agent: HsaAgent, callback: IsaCallback, data: *mut c_void) -> HsaStatus;
pub(crate) type HsaIsaGetInfoAlt = unsafe extern "C" fn(isa: HsaIsa, attribute: HsaInfo, value: *mut c_void) -> HsaStatus;
pub(crate) type HsaQueueCreate = unsafe extern "C" fn(agent: HsaAgent, size: u32, kind: u32, callback: Option<QueueErrorCallback>, data: *mut c_void, private_segment_size: u32, group_segment_size: u32, queue: *mut *mut HsaQueue) -> HsaStatus;
pub(crate) type HsaQueueDestroy = unsafe extern "C" fn(queue: *mut HsaQueue) -> HsaStatus;
pub(crate) type HsaAmdAgentIterateMemoryPools = unsafe extern "C" fn(agent: HsaAgent, callback: MemoryPoolCallback, data: *mut c_void) -> HsaStatus;
pub(crate) type HsaAmdMemoryPoolGetInfo = unsafe extern "C" fn(pool: HsaMemoryPool, attribute: HsaInfo, value: *mut c_void) -> HsaStatus;
pub(crate) type HsaCodeObjectReaderCreateFromMemory = unsafe extern "C" fn(*const c_void, usize, *mut HsaCodeObjectReader) -> HsaStatus;
...
pub(crate) type HsaExecutableLoadAgentCodeObject = unsafe extern "C" fn(HsaExecutable, HsaAgent, HsaCodeObjectReader, *const c_char, *mut HsaLoadedCodeObject) -> HsaStatus;
pub(crate) type HsaExecutableFreeze = unsafe extern "C" fn(HsaExecutable, *const c_char) -> HsaStatus;
pub(crate) type HsaExecutableGetSymbolByName = unsafe extern "C" fn(HsaExecutable, *const c_char, *const HsaAgent, *mut HsaExecutableSymbol) -> HsaStatus;
pub(crate) type HsaExecutableSymbolGetInfo = unsafe extern "C" fn(HsaExecutableSymbol, HsaInfo, *mut c_void) -> HsaStatus;
pub(crate) type HsaAmdMemoryPoolAllocate = unsafe extern "C" fn(HsaMemoryPool, usize, u32, *mut *mut c_void) -> HsaStatus;
pub(crate) type HsaAmdMemoryPoolFree = unsafe extern "C" fn(*mut c_void) -> HsaStatus;
pub(crate) type HsaAmdAgentsAllowAccess = unsafe extern "C" fn(u32, *const HsaAgent, *const u32, *const c_void) -> HsaStatus;
pub(crate) type HsaAmdMemoryAsyncCopy = unsafe extern "C" fn(*mut c_void, HsaAgent, *const c_void, HsaAgent, usize, u32, *const HsaSignal, HsaSignal) -> HsaStatus;
