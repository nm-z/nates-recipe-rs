...
use crate::{
	...
	abi::{AGENT_FEATURE_KERNEL_DISPATCH, AMD_AGENT_INFO_MEMORY_AVAIL, HsaAgent, HsaQueue, HsaStatus, SYSTEM_INFO_TIMESTAMP_FREQUENCY},
...
impl SharedFault {
	pub(crate) fn new() -> Self {
		Self {
			...
			source_queue_id: AtomicU64::new(0),
			source_known: AtomicBool::new(false),
			epoch: AtomicU64::new(0),
			_wait_lock: Mutex::new(()),
			wake: Condvar::new(),
		}
	}

	pub(crate) fn record(&self, status: i32, source_queue_id: Option<u64>) {
		self.status.store(status, Ordering::Relaxed);
		if let Some(source_queue_id) = source_queue_id {
			self.source_queue_id
				.store(source_queue_id, Ordering::Relaxed);
			self.source_known.store(true, Ordering::Relaxed);
		} else {
			self.source_known.store(false, Ordering::Relaxed);
		}
	...
	pub(crate) fn snapshot(&self) -> Option<QueueFault> {
		if !self.poisoned.load(Ordering::Acquire) {
			return None;
		}
		Some(QueueFault {
			status: self.status.load(Ordering::Relaxed),
			source_queue_id: self
				.source_known
				.load(Ordering::Relaxed)
				.then(|| self.source_queue_id.load(Ordering::Relaxed)),
			epoch: self.epoch.load(Ordering::Relaxed),
		})
	}

	pub(crate) fn check(&self) -> Result<()> {
		if let Some(fault) = self.snapshot() {
			Err(Error::SessionPoisoned {
				status: fault.status,
				source_queue_id: fault.source_queue_id,
				epoch: fault.epoch,
			})
		} else {
			Ok(())
...
unsafe extern "C" fn queue_error_callback(status: HsaStatus, source: *mut HsaQueue, data: *mut c_void) {
	...
	let context = unsafe { &*data.cast::<QueueCallbackContext>() };
	let source_queue_id = if source.is_null() {
		None
	} else {
		// SAFETY: ROCr supplies a live source queue for the callback duration.
		Some(unsafe { (*source).id })
	};
	context.fault.record(status, source_queue_id);
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct QueueConfig {
	pub size_packets: u32,
	pub kind: QueueKind,
...
impl QueueConfig {
	pub const fn new(size_packets: u32, kind: QueueKind) -> Self {
		Self {
			size_packets,
			kind,
			private_segment_size: u32::MAX,
			group_segment_size: u32::MAX,
		}
	}
}
...
pub struct Session<'runtime> {
	...
	pub(crate) fault: Arc<SharedFault>,
	pub(crate) signals: SignalPool,
}
...
impl<'runtime> DiscoveredAgent<'runtime> {
	...
	pub fn into_session(self) -> Result<Session<'runtime>> {
		...
		if self.description.device_type != DeviceType::Gpu {
			return Err(Error::UnsupportedAgent {
				reason: "the agent is not a GPU",
			});
		}
		if !self.description.supports_kernel_dispatch() {
			return Err(Error::UnsupportedAgent {
				reason: "the GPU does not report kernel-dispatch support",
			});
		}
		if self.description.isas.is_empty()
			|| self
				.description
				.isas
				.iter()
				.any(|isa| isa.amd_target.is_none())
		{
			return Err(Error::UnsupportedAgent {
		...
		let status = unsafe {
			(self.runtime.api.system_get_info)(
				SYSTEM_INFO_TIMESTAMP_FREQUENCY,
				timestamp_frequency_hz.as_mut_ptr().cast::<c_void>(),
			)
		};
		self.runtime
			.api
		...
		let fault = Arc::new(SharedFault::new());
		let signals = SignalPool::new(
			Arc::clone(&self.runtime.api),
			Arc::clone(&fault),
...
impl Session<'_> {
	...
	pub fn available_memory_bytes(&self) -> Result<u64> {
		...
		let status = unsafe {
			(self.runtime.api.agent_get_info)(
				self.raw_agent,
	...
	pub fn create_queue(&self, config: QueueConfig) -> Result<Queue<'_, '_>> {
		...
		if config.size_packets < capabilities.minimum_packets || config.size_packets > capabilities.maximum_packets || !config.size_packets.is_power_of_two() {
			return Err(Error::InvalidQueueSize {
				requested: config.size_packets,
				minimum: capabilities.minimum_packets,
		...
		if !kind_allowed {
			return Err(Error::UnsupportedQueueKind {
				requested: config.kind,
				advertised: capabilities.advertised_kind,
			});
		}

		let callback = Box::new(QueueCallbackContext {
			fault: Arc::clone(&self.fault),
		});
		let callback_data = (&*callback as *const QueueCallbackContext)
			.cast_mut()
			.cast::<c_void>();
		...
		let status = unsafe {
			(self.runtime.api.queue_create)(
				self.raw_agent,
				config.size_packets,
				config.kind.as_raw(),
				Some(queue_error_callback),
				callback_data,
				config.private_segment_size,
				config.group_segment_size,
				&mut raw,
			)
		};
		self.runtime.api.check("hsa_queue_create", status)?;
		if raw.is_null() {
			return Err(Error::NullQueue);
		}
		// SAFETY: a successful queue creation returns a readable queue object.
		let (base_is_null, actual_size, actual_kind_raw, features) = unsafe {
			(
				(*raw).base_address.is_null(),
				(*raw).size,
				(*raw).kind,
				(*raw).features,
			)
		};
		let actual_kind = QueueKind::from_raw(actual_kind_raw);
		let invalid = base_is_null || actual_size == 0 || !actual_size.is_power_of_two() || features & AGENT_FEATURE_KERNEL_DISPATCH == 0 || actual_kind.is_err();
		if invalid {
			...
			if destroy_status == crate::abi::STATUS_SUCCESS {
				drop(callback);
			} else {
				// Destruction failed, so ROCr may still retain callback data.
				Box::leak(callback);
			}
			return Err(Error::InvalidQueueReturned {
				kind: actual_kind_raw,
				features,
				size: actual_size,
				base_is_null,
			});
...
pub(crate) struct QueueCore {
	...
	pub(crate) raw: *mut HsaQueue,
	callback: Option<Box<QueueCallbackContext>>,
...
impl QueueCore {
	fn destroy(&mut self) -> Result<()> {
		...
		let status = unsafe { (self.api.queue_destroy)(raw) };
		if status == crate::abi::STATUS_SUCCESS {
...
impl Queue<'_, '_> {
	...
	pub fn close(mut self) -> Result<()> {
		...
		match Rc::try_unwrap(core) {
			Ok(mut core) => core.destroy(),
			Err(core) => {
				drop(core);
