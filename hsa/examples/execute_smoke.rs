...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let mut cpu = None;
	let mut gpu_agent = None;
	for agent in discovery.into_agents() {
		match agent.description().device_type {
			DeviceType::Cpu if cpu.is_none() => cpu = Some(agent),
			DeviceType::Gpu if gpu_agent.is_none() => gpu_agent = Some(agent),
			_ => {}
		}
	}
	let cpu = cpu.ok_or_else(|| io::Error::other("no ROCr CPU agent"))?;
	let gpu_agent = gpu_agent.ok_or_else(|| io::Error::other("no ROCr GPU agent"))?;
	let session = gpu_agent.into_session()?;
	...
	if let Some(path) = std::env::var_os("RECIPE_HSA_SMOKE_COPY_HSACO") {
		let symbol = std::env::var("RECIPE_HSA_SMOKE_SYMBOL").map_err(|_| io::Error::other("RECIPE_HSA_SMOKE_SYMBOL is required"))?;
		...
		kernarg.close()?;
		output.close()?;
		line(gpu, "optional HSACO kernel dispatch smoke passed");
	}
	...
	drop(cpu);
	runtime.close()?;
	line(gpu, "ROCr fine→coarse→fine asynchronous copy smoke passed");
	Ok(())
}
