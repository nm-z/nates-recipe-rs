...
use super::{
	...
	emit_batch_normalization_running_update, emit_gradient_clip_norm, emit_lamb_phase_one, emit_lamb_phase_two,
...
pub(super) fn dispatch(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> FamilyDispatch {
	...
	let result = match request.descriptor.symbol {
		...
		"gpu_layernorm_f32" | "gpu_layernorm_into" | "gpu_layernorm_opt_into" => {
			emit_layer_normalization(request, emitter)
		}
		"gpu_layernorm_backward_f32" | "gpu_layernorm_backward_full_into" => {
			emit_layer_normalization_backward(request, emitter)
		}
