...
pub enum ScanMode {
	Inclusive,
	Exclusive { identity: ScalarLiteral },
}
...
pub struct Contraction {
	pub batch_axes: Vec<(usize, usize)>,
...
pub struct PrimitiveKernel {
	...
	pub alias_rules: Vec<PrimitiveAliasRule>,
	pub kind: PrimitiveKind,
}

impl PrimitiveKernel {
	pub fn validate(&self, tensors: &BTreeMap<ValueId, &Tensor>) -> LanguageResult<()> {
		let inputs = self
			.inputs
			.iter()
			.map(|id| {
				tensors.get(id).copied().ok_or_else(|| {
					LanguageError::new(
		...
		match &self.kind {
			PrimitiveKind::Elementwise(spec) => validate_elementwise(self.id, spec, &inputs, &outputs),
	...
	pub fn work(&self, tensors: &BTreeMap<ValueId, &Tensor>) -> LanguageResult<FlopCount> {
		self.validate(tensors)?;
		let tensor = |id: ValueId| {
			tensors
				.get(&id)
				.copied()
		...
		let work = match &self.kind {
			PrimitiveKind::Elementwise(spec) => {
				let elements = tensor(self.outputs[0]).shape.elements();
				let per_element = spec
					.program
					.instructions
					.iter()
					.try_fold(0_u64, |total, instruction| {
						total.checked_add(scalar_work(instruction.opcode))
					})
					.ok_or_else(|| work_overflow(self.id))?;
				elements.checked_mul(per_element)
			}
			PrimitiveKind::Reduce(spec) => {
				let input = tensor(self.inputs[0]).shape.elements();
				let output = tensor(self.outputs[0]).shape.elements();
				let combine = input.saturating_sub(output);
				let multiplier = if spec.result == ReduceResult::ValueAndIndex {
					2
				} else {
					1
				};
				combine.checked_mul(multiplier)
			}
			PrimitiveKind::Scan(_) => tensor(self.inputs[0]).shape.elements().checked_mul(2),
			PrimitiveKind::Contraction(spec) => {
				let left = tensor(self.inputs[0]);
				let contracted = spec
					.contract_axes
					.iter()
					.try_fold(1_u64, |count, (axis, _)| {
						count.checked_mul(left.shape.extents()[*axis])
					});
				contracted
					.and_then(|count| count.checked_mul(tensor(self.outputs[0]).shape.elements()))
					.and_then(|count| count.checked_mul(2))
			}
			PrimitiveKind::Gather(_) | PrimitiveKind::Scatter(_) => {
				Some(tensor(self.outputs[0]).shape.elements())
			}
			PrimitiveKind::Histogram(_) => Some(tensor(self.inputs[0]).shape.elements()),
			PrimitiveKind::Sort(spec) => {
				let input = tensor(self.inputs[0]);
				let axis = input.shape.extents()[spec.axis];
				let comparisons = if axis <= 1 {
					0
				} else {
					u64::from(u64::BITS - (axis - 1).leading_zeros())
						.checked_mul(axis)
						.ok_or_else(|| work_overflow(self.id))?
				};
	...
	fn validate_alias_matrix(&self, input_count: usize, output_count: usize) -> LanguageResult<()> {
		...
		for rule in &self.alias_rules {
			...
			if !pairs.insert((rule.input, rule.output)) {
				return Err(LanguageError::new(
					LanguageErrorKind::InvalidPrimitive,
					format!(
		...
		if pairs.len() != input_count.saturating_mul(output_count) {
			return Err(LanguageError::new(
				LanguageErrorKind::InvalidPrimitive,
...
fn validate_elementwise(
	...
	spec.program.validate().map_err(|errors| {
		LanguageError::new(LanguageErrorKind::InvalidScalarProgram, errors.to_string()).for_kernel(id)
	})?;
	require_arity(
		id,
		"elementwise input",
		inputs.len(),
		spec.program.inputs.len(),
	)?;
	require_arity(
		id,
		"elementwise output",
		outputs.len(),
		spec.program.outputs.len(),
	)?;
	if inputs.is_empty() {
		return Err(LanguageError::new(
	...
	let shape = Shape::broadcast_result(
		&inputs
			.iter()
	...
	for (index, (tensor, scalar)) in outputs.iter().zip(&spec.program.outputs).enumerate() {
		let expected_dtype = spec.program.dtype_of(*scalar);
		if expected_dtype != Some(tensor.dtype) {
			return Err(dtype_error(
				id,
				format!(
...
fn validate_reduce(id: KernelTemplateId, spec: &Reduce, inputs: &[&Tensor], outputs: &[&Tensor]) -> LanguageResult<()> {
	...
	let expected_outputs = match spec.result {
	...
	spec.axes
		.validate_rank(inputs[0].shape.rank())
		.map_err(|error| error.for_kernel(id))?;
	...
	if matches!(
		spec.result,
		ReduceResult::Index | ReduceResult::ValueAndIndex
	) && !matches!(
		spec.operator,
	...
	let reduced = inputs[0]
		.shape
		.reduced(&spec.axes, spec.keep_dimensions)
		.map_err(|error| error.for_kernel(id))?;
	match spec.result {
		ReduceResult::Value => {
	...
	let empty_axis = spec
		...
		.as_slice()
		.iter()
		.any(|axis| inputs[0].shape.extents()[*axis] == 0);
	if empty_axis
		&& matches!(
			spec.operator,
			ReduceOperator::Minimum | ReduceOperator::Maximum
...
fn validate_scan(id: KernelTemplateId, spec: &Scan, inputs: &[&Tensor], outputs: &[&Tensor]) -> LanguageResult<()> {
	...
	if spec.axis >= inputs[0].shape.rank() {
		return Err(LanguageError::new(
	...
	if let ScanMode::Exclusive { identity } = spec.mode
		&& identity.dtype() != inputs[0].dtype
	{
		return Err(dtype_error(
			id,
...
fn validate_contraction(
	...
	if spec.contract_axes.is_empty() {
		return Err(LanguageError::new(
			LanguageErrorKind::InvalidPrimitive,
	...
	let left_rank = inputs[0].shape.rank();
	let right_rank = inputs[1].shape.rank();
	...
	for (class, pairs) in [
		...
		for (left, right) in pairs {
			...
			if !used_left.insert(*left) || !used_right.insert(*right) {
				return Err(LanguageError::new(
			...
			let left_extent = inputs[0].shape.extents()[*left];
			let right_extent = inputs[1].shape.extents()[*right];
			if left_extent != right_extent {
				return Err(LanguageError::new(
					LanguageErrorKind::ShapeMismatch,
	...
	for (axis, extent) in inputs[0].shape.extents().iter().copied().enumerate() {
		if !used_left.contains(&axis) {
			output_extents.push(extent);
		}
	}
	for (axis, extent) in inputs[1].shape.extents().iter().copied().enumerate() {
		if !used_right.contains(&axis) {
			output_extents.push(extent);
		}
	}
	if output_extents.is_empty() {
		output_extents.push(1);
	}
...
fn validate_gather(id: KernelTemplateId, spec: Gather, inputs: &[&Tensor], outputs: &[&Tensor]) -> LanguageResult<()> {
	...
	let expected = inputs[0]
		.shape
		.gather_result(spec.axis, &inputs[1].shape)
		.map_err(|error| error.for_kernel(id))?;
...
fn validate_sort(id: KernelTemplateId, spec: Sort, inputs: &[&Tensor], outputs: &[&Tensor]) -> LanguageResult<()> {
	...
	if inputs[0].shape.extents()[spec.axis] > i32::MAX as u64 {
		return Err(LanguageError::new(
			LanguageErrorKind::InvalidPrimitive,
...
fn dtype_error(id: KernelTemplateId, detail: impl Into<String>) -> LanguageError {
	LanguageError::new(LanguageErrorKind::DTypeMismatch, detail).for_kernel(id)
}

fn work_overflow(id: KernelTemplateId) -> LanguageError {
	LanguageError::new(
		LanguageErrorKind::WorkOverflow,
		"primitive work count overflowed u64",
	)
	.for_kernel(id)
}

const fn scalar_work(opcode: ScalarOpcode) -> u64 {
	opcode.flops()
}

#[cfg(test)]
mod tests {
	use recipe_core::{AliasPermission, DType, ValueId};
	...
	#[test]
	fn validates_fixed_tree_value_and_index_reduction() {
		let tensors = [
			tensor(1, DType::F32, &[8, 16]),
			tensor(2, DType::F32, &[8]),
			tensor(3, DType::I32, &[8]),
		];
		let kernel = PrimitiveKernel {
			id: KernelTemplateId::new(1),
			inputs: vec![ValueId::new(1)],
			outputs: vec![ValueId::new(2), ValueId::new(3)],
			alias_rules: forbidden_aliases(1, 2),
		...
		kernel.validate(&tensors).unwrap();
		assert_eq!(kernel.work(&tensors).unwrap().get(), 240);
	}

	#[test]
	fn validates_batched_general_contraction_and_work() {
		let tensors = [
		...
		let kernel = PrimitiveKernel {
			...
			outputs: vec![ValueId::new(3)],
			alias_rules: forbidden_aliases(2, 1),
		...
		let tensors = index(&tensors);
		kernel.validate(&tensors).unwrap();
		assert_eq!(kernel.work(&tensors).unwrap().get(), 420);
	}

	#[test]
	fn scatter_requires_the_exact_gather_shaped_update() {
		let tensors = [
		...
		let kernel = PrimitiveKernel {
			id: KernelTemplateId::new(3),
			inputs: vec![ValueId::new(1), ValueId::new(2), ValueId::new(3)],
			outputs: vec![ValueId::new(4)],
			alias_rules: forbidden_aliases(3, 1),
		...
		kernel.validate(&index(&tensors)).unwrap();
	}

	#[test]
	fn random_contract_fixes_owned_counter_algorithm() {
		let tensors = [tensor(1, DType::I32, &[1024])];
		let valid = PrimitiveKernel {
			id: KernelTemplateId::new(4),
			inputs: Vec::new(),
		...
		assert_eq!(valid.work(&tensors).unwrap().get(), 40_960);
	...
	#[test]
	fn alias_matrix_is_complete_and_explicit() {
		let tensors = [tensor(1, DType::F32, &[4]), tensor(2, DType::F32, &[4])];
		let kernel = PrimitiveKernel {
			id: KernelTemplateId::new(5),
			inputs: vec![ValueId::new(1)],
		...
		assert_eq!(
			kernel.validate(&index(&tensors)).unwrap_err().kind,
			LanguageErrorKind::InvalidPrimitive
		);
	}
}
