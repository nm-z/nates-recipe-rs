...
impl Shape {
	pub fn new(extents: Vec<u64>) -> LanguageResult<Self> {
		if extents.is_empty() {
		...
		for extent in &extents {
			elements = elements.checked_mul(*extent).ok_or_else(|| {
				LanguageError::new(
					LanguageErrorKind::ShapeOverflow,
	...
	pub fn broadcast_result(inputs: &[&Self]) -> LanguageResult<Self> {
		let rank = inputs.iter().map(|shape| shape.rank()).max().unwrap_or(0);
		if rank == 0 {
			return Err(LanguageError::new(
				LanguageErrorKind::EmptyShape,
		...
		let mut extents = vec![1_u64; rank];
		for shape in inputs {
			let leading = rank - shape.rank();
			for (index, extent) in shape.extents.iter().copied().enumerate() {
				let result = &mut extents[leading + index];
				if *result == 1 {
					*result = extent;
				} else if extent != 1 && extent != *result {
					return Err(LanguageError::new(
	...
	pub fn reduced(&self, axes: &AxisSet, keep_dimensions: bool) -> LanguageResult<Self> {
		axes.validate_rank(self.rank())?;
		let mut result = Vec::with_capacity(self.rank());
		for (axis, extent) in self.extents.iter().copied().enumerate() {
	...
	pub fn gather_result(&self, axis: usize, indices: &Self) -> LanguageResult<Self> {
		if axis >= self.rank() {
			return Err(LanguageError::new(
		...
		extents.extend_from_slice(&self.extents[..axis]);
		extents.extend_from_slice(indices.extents());
		extents.extend_from_slice(&self.extents[axis + 1..]);
		Self::new(extents)
	}
}
...
impl AxisSet {
	pub fn new(mut axes: Vec<usize>) -> LanguageResult<Self> {
		if axes.is_empty() {
			return Err(LanguageError::new(
				LanguageErrorKind::InvalidAxis,
