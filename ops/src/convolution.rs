...
pub struct ChannelwiseConvolution1dPreparation { batch: u64, input_length: u64, input_channels: u64, filters: u64,
	kernel_size: u64, output_length: u64, input_elements: u64, output_elements: u64, window_indices: Vec<i32>,
	input_gradient_indices: Vec<i32>, input_gradient_validity: Vec<u32>, forward_workspace: ByteCount,
...
impl ChannelwiseConvolution1dPreparation {
	...
	#[must_use]
	pub const fn window_indices_shape(&self) -> [u64; 4] { [ self.batch, self.output_length, self.kernel_size,
			self.input_channels, ] }
	...
	#[must_use]
	pub const fn input_gradient_indices_shape(&self) -> [u64; 4] { [ self.batch, self.input_length, self.kernel_size,
			self.filters, ] }
...
pub fn prepare_channelwise_convolution_1d( batch: u64, input_length: u64, input_channels: u64, filters: u64,
	kernel_size: u64, ) -> OperationResult<ChannelwiseConvolution1dPreparation> { for (name, value) in [
		("batch", batch),
	...
	if kernel_size > input_length { return Err(OperationError::new( OperationErrorKind::UnsupportedConcreteShape,
			format!("kernel_size {kernel_size} exceeds input_length {input_length}"),
		)); }
	let output_length = input_length - kernel_size + 1; let input_elements = checked_product(
		&[batch, input_length, input_channels],
		"convolution input elements",
	)?; let output_elements = checked_product( &[batch, output_length, filters],
		"convolution output elements",
	)?; if input_elements > MAX_I32_INDEX || output_elements > MAX_I32_INDEX { return Err(OperationError::new(
			OperationErrorKind::UnsupportedConcreteShape,
			"convolution input and output must fit the checked int32 index domain",
		)); }
	let window_elements = checked_product( &[batch, output_length, kernel_size, input_channels],
		"convolution receptive-field elements",
	)?; let input_gradient_elements = checked_product( &[batch, input_length, kernel_size, filters],
		"convolution input-gradient contributions",
	)?; let window_capacity = usize::try_from(window_elements).map_err(|error| { OperationError::new(
			OperationErrorKind::UnsupportedConcreteShape,
			format!("convolution receptive-field table does not fit host usize: {error}"),
		) })?; let input_gradient_capacity = usize::try_from(input_gradient_elements).map_err(|error| { OperationError::new(
			OperationErrorKind::UnsupportedConcreteShape,
			format!("convolution input-gradient table does not fit host usize: {error}"),
		) })?;

	let mut window_indices = Vec::with_capacity(window_capacity); for batch_index in 0..batch {
		for output_index in 0..output_length { for kernel_index in 0..kernel_size { for channel in 0..input_channels {
					let input_index = (((batch_index * input_length + output_index + kernel_index) * input_channels) + channel) as i32;
					window_indices.push(input_index); } } } }

	let mut input_gradient_indices = Vec::with_capacity(input_gradient_capacity);
	let mut input_gradient_validity = Vec::with_capacity(input_gradient_capacity); for batch_index in 0..batch {
		for input_index in 0..input_length { for kernel_index in 0..kernel_size {
				let output_index = input_index.checked_sub(kernel_index);
				let valid = output_index.is_some_and(|output_index| output_index < output_length); for filter in 0..filters {
					...
					input_gradient_indices.push(gradient_index); input_gradient_validity.push(if valid { 1.0_f32.to_bits() } else {
						0.0_f32.to_bits() }); } } } }

	let forward_workspace = workspace_bytes(window_elements, "convolution forward workspace")?;
	let backward_workspace = workspace_bytes( window_elements .checked_add(input_gradient_elements)
			.ok_or_else(|| overflow("convolution backward workspace elements"))?,
		"convolution backward workspace",
	)?; Ok(ChannelwiseConvolution1dPreparation { batch, input_length, input_channels, filters, kernel_size, output_length,
		input_elements, output_elements, window_indices, input_gradient_indices, input_gradient_validity, forward_workspace,
		backward_workspace, }) }

fn checked_product(values: &[u64], role: &str) -> OperationResult<u64> {
	values.iter().try_fold(1_u64, |product, value| { product.checked_mul(*value).ok_or_else(|| overflow(role)) }) }

fn workspace_bytes(elements: u64, role: &str) -> OperationResult<ByteCount> { elements .checked_mul(4)
		.map(ByteCount::new) .ok_or_else(|| overflow(role)) }

fn overflow(role: &str) -> OperationError { OperationError::new( OperationErrorKind::UnsupportedConcreteShape,
