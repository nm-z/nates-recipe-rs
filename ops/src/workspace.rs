...
impl WorkspaceFormula {
	pub(crate) fn for_symbol(symbol: &str) -> Option<Self> {
		let formula = match symbol {
			"gpu_sum_all_workspace_bytes" | "gpu_max_all_workspace_bytes" | "gpu_min_all_workspace_bytes" | "gpu_mean_all_workspace_bytes" => {
			...
			"gpu_count_distinct_workspace_bytes" | "gpu_run_length_workspace_bytes" => Self::SortRunEncoding { dimensions: 1 },
	...
	pub const fn definition(self) -> &'static str {
		match self {
			Self::NoPersistentScratch { .. } => "zero persistent bytes; the canonical vector contraction uses only fixed shared-memory tiles",
			Self::FixedTreeReduction { .. } => "sum of every non-final 64-lane fixed-tree f32 reduction level",
			Self::FixedTreeScan { .. } => "f32 block totals and hierarchy outputs for every 64-lane fixed-tree scan level",
			Self::StableSort { .. } => "power-of-two-padded f32 values plus int32 original indexes for the stable sorting network",
			Self::SortRunEncoding { .. } => "stable-sort scratch plus int32 transition flags and their fixed-tree prefix scan",
			Self::MapThenReduction { .. } => "one f32 scalar-map image plus every non-final 64-lane fixed-tree reduction level",
			Self::RandomKeySort { .. } => "one f32 Philox key image plus padded f32 values and int32 indexes for the stable sorting network",
			Self::CholeskyFactor => "one 32-column f32 Cholesky panel, clipped to matrix order, plus one int32 fault word",
			...
			Self::SymmetricEigensolver => "one dense f32 Jacobi work matrix, one f32 diagonal vector, and one int32 fault word",
			Self::SingularValueDecomposition => "one f32 bidiagonalization image, two reflector panels, singular values, and one fault word",
	...
	const fn dimension_count(self) -> usize {
		match self {
			Self::NoPersistentScratch { dimensions } | Self::FixedTreeReduction { dimensions, .. } | Self::FixedTreeScan { dimensions } | Self::StableSort { dimensions } | Self::SortRunEncoding { dimensions } | Self::MapThenReduction { dimensions } | Self::RandomKeySort { dimensions } => dimensions,
