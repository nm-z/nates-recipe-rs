...
use recipe_language::{AxisSet, CalculationGraph, CalculationNode, Contraction, Elementwise, Gather, IndexBounds, IndexMap, PrimitiveAliasRule, PrimitiveKernel, PrimitiveKind, Reduce, ReduceOperator, ReduceResult, ScalarExpression, ScalarProgramBuilder, Shape, Tensor};
...
pub fn materialize_kmeans_initialization(request: &KMeansInitializationRequest) -> OperationResult<KMeansInitializationMaterialization> {
...
pub fn materialize_kmeans_lloyd_step(request: &KMeansLloydRequest) -> OperationResult<KMeansLloydMaterialization> {
	...
	validate_dimensions(rows, dimensions, clusters)?;
	if request.prior_centroids.shape.extents()[1] != dimensions || request.updated_centroids.shape.extents() != [clusters, dimensions] {
		return Err(kmeans_error(
			OperationErrorKind::UnsupportedConcreteShape,
	...
	if request.updated_centroids.id == request.points.id || request.updated_centroids.id == request.prior_centroids.id || request.updated_centroids.id == request.distances.id || request.distances.id == request.points.id || request.distances.id == request.prior_centroids.id {
		return Err(kmeans_error(
			OperationErrorKind::InvalidMaterializationRequest,
...
impl KMeansEmitter {
	fn new<'a>(boundaries: impl IntoIterator<Item = (&'a Tensor, bool, bool)>, namespace: IdentityNamespace, context: &'static str) -> OperationResult<Self> {
	...
	fn emit_with_aliases(&mut self, inputs: Vec<ValueId>, outputs: Vec<ValueId>, kind: PrimitiveKind, alias_rules: Vec<PrimitiveAliasRule>) -> OperationResult<KernelTemplateId> {
	...
	fn finish(self, requirements: KMeansRequirements) -> OperationResult<KMeansParts> {
		if self.intermediate_values.len() as u64 != requirements.intermediate_values || self.kernels.len() as u64 != requirements.kernels {
			return Err(kmeans_error(
...
fn validate_resources(namespace: IdentityNamespace, workspace_limit: ByteCount, requirements: KMeansRequirements, context: &str) -> OperationResult<()> {
...
fn elementwise(program: recipe_core::ScalarProgram) -> PrimitiveKind { PrimitiveKind::Elementwise(Elementwise { program }) }
...
fn scalar_ternary(builder: &mut ScalarProgramBuilder, opcode: ScalarOpcode, first: ScalarExpression, second: ScalarExpression, third: ScalarExpression) -> OperationResult<ScalarExpression> {
	builder
