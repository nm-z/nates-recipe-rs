...
use recipe_language::{AxisSet, CalculationGraph, CalculationNode, Elementwise, Gather, IndexBounds, IndexMap, PrimitiveAliasRule, PrimitiveKernel, PrimitiveKind, Reduce, ReduceOperator, ReduceResult, ScalarExpression, ScalarProgramBuilder, Shape, Tensor};
...
pub fn tree_ensemble_inference_requirements(rows: u64, feature_width: u64, trees: u64, depth: u32, outputs: u64) -> OperationResult<TreeEnsembleInferenceRequirements> {
	for (name, value) in [
		("row count", rows),
...
pub fn materialize_tree_ensemble_inference(request: &TreeEnsembleInferenceRequest) -> OperationResult<TreeEnsembleInferenceMaterialization> {
...
fn validate_resources(request: &TreeEnsembleInferenceRequest, requirements: TreeEnsembleInferenceRequirements) -> OperationResult<()> {
...
impl TreeEmitter {
	fn new(request: &TreeEnsembleInferenceRequest, requirements: TreeEnsembleInferenceRequirements) -> OperationResult<Self> {
...
fn leaf_base_program(trees: u64, internal_nodes: u64, leaves: u64, outputs: u64) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder()?;
	let pair = scalar_input(&mut builder, DType::I32)?;
	let node = scalar_input(&mut builder, DType::I32)?;
	let trees = scalar_i32(&mut builder, trees, "tree count")?;
...
fn require_i32_range(builder: &mut ScalarProgramBuilder, value: ScalarExpression, upper_exclusive: ScalarExpression) -> OperationResult<()> {
	let zero = builder.i32(0).map_err(graph_error)?;
	require_i32_interval(builder, value, zero, upper_exclusive)
}

fn require_i32_interval(builder: &mut ScalarProgramBuilder, value: ScalarExpression, lower_inclusive: ScalarExpression, upper_exclusive: ScalarExpression) -> OperationResult<()> {
...
fn insert_boundary(boundaries: &mut BTreeMap<ValueId, Tensor>, tensor: Tensor) -> OperationResult<()> {
	match boundaries.get(&tensor.id) {
		Some(existing) if existing.dtype == tensor.dtype && existing.shape == tensor.shape && existing.layout == tensor.layout && existing.storage_bytes == tensor.storage_bytes => Ok(()),
		Some(_) => {
			Err(tree_error(
				OperationErrorKind::InvalidMaterializationRequest,
				format!(
