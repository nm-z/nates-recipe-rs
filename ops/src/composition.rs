use crate::{CanonicalDTypeContract, LoweringAvailability, OperationDescriptor, OperationError, OperationErrorKind, OperationFamily, OperationResult, PrimitiveFamily};
...
impl CompositionStep {
	...
	#[must_use]
	pub const fn repeat(bound: IterationBound, role: &'static str, body: &'static [CompositionStep]) -> Self { Self::Repeat { bound, role, body } }
...
impl CompositionRecipe {
	const fn new(name: &'static str, definition: &'static str, steps: &'static [CompositionStep], payload: CompositionPayload, family: OperationFamily) -> Self {
		Self {
			name,
			definition,
			steps,
		...
		match step {
			...
			CompositionStep::Repeat { bound, body, .. } => {
				...
				match bound {
					...
					IterationBound::Fixed(_) | IterationBound::ShapeExtent { .. } | IterationBound::MinimumShapeExtent | IterationBound::CeilingLog2ShapeExtent { .. } | IterationBound::PreparedParameter { .. } => {}
				}
...
const fn recipe(name: &'static str, definition: &'static str, steps: &'static [CompositionStep], payload: CompositionPayload, family: OperationFamily) -> CompositionRecipe { CompositionRecipe::new(name, definition, steps, payload, family) }
...
fn composition_for_entry(symbol: &str, source: &str) -> Option<CompositionRecipe> {
	let value = match symbol {
		...
		"gpu_oblivious_route_full" | "gpu_oblivious_route_step" | "gpu_oblivious_route_step_dev" | "gpu_tree_build_into" => {
			recipe(
		...
		"gpu_rope" | "gpu_rope_partial" | "gpu_rope_partial_factors" | "gpu_rope_partial_factors_pos" | "gpu_rope_partial_pos" | "gpu_rope_qk" | "gpu_rope_qk_heads_inplace" => {
			recipe(
