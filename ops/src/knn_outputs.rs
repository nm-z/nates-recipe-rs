...
use recipe_language::{AtomicOrdering, AxisSet, CalculationGraph, CalculationNode, Contraction, Elementwise, Gather, Histogram, IndexBounds, IndexMap, PrimitiveAliasRule, PrimitiveKernel, PrimitiveKind, Reduce, ReduceOperator, ReduceResult, ScalarExpression, ScalarProgramBuilder, Shape, Sort, SortDirection, Tensor};
...
pub fn knn_all_output_requirements(queries: u64, reference_rows: u64, dimensions: u64, neighbors: u64, outputs: &[KnnOutputRequest]) -> OperationResult<KnnAllOutputRequirements> {
	for (name, value) in [
	...
	for (output_index, output) in outputs.iter().enumerate() {
		...
		if known == 0 || known > reference_rows {
			return Err(knn_error(
				OperationErrorKind::UnsupportedConcreteShape,
				format!("KNN output {output_index} known-reference count must be in 1..={reference_rows}, got {known}"),
			));
		}
		...
		let (classes, extra, values, output_kernels) = match output {
			KnnOutputRequest::Numeric { .. } => {
				if effective_neighbors > MAXIMUM_EXACT_F32_INTEGER {
					return Err(knn_error(
						OperationErrorKind::UnsupportedConcreteShape,
						format!("KNN output {output_index} numeric neighbor count exceeds the exact f32 integer domain"),
					));
				}
				(
					None,
			...
			KnnOutputRequest::Categorical { classes, .. } => {
...
pub fn materialize_knn_all_outputs(request: &KnnAllOutputRequest) -> OperationResult<KnnAllOutputMaterialization> {
	...
	for (output, output_requirements) in request.outputs.iter().zip(&requirements.outputs) {
		...
		emitter.elementwise(
			vec![known_count],
			vec![validated_count],
			known_count_program(output_requirements.known_references)?,
		)?;
				...
				emitter.emit(
					...
					PrimitiveKind::Histogram(Histogram {
						bins: u32::try_from(bins).map_err(|error| {
							knn_error(
								OperationErrorKind::UnsupportedConcreteShape,
								format!("KNN histogram bin count cannot be represented by u32: {error}"),
							)
						})?,
						weighted: false,
...
pub fn append_knn_all_outputs(graph: &mut CalculationGraph, request: &KnnAllOutputRequest) -> OperationResult<KnnAllOutputMaterialization> {
...
fn validate_request(request: &KnnAllOutputRequest) -> OperationResult<KnnAllOutputRequirements> {
	...
	for (output_index, output) in request.outputs.iter().enumerate() {
		...
		if output.reference_values().dtype != value_dtype || output.reference_values().shape.extents() != [reference_rows] {
			return Err(knn_error(
				OperationErrorKind::UnsupportedConcreteShape,
				format!("KNN output {output_index} reference values must be {value_dtype:?} [{reference_rows}]"),
			));
		}
...
impl KnnAllOutputEmitter {
	...
	fn elementwise(&mut self, inputs: Vec<ValueId>, outputs: Vec<ValueId>, program: recipe_core::ScalarProgram) -> OperationResult<()> {
		self.emit(
			inputs,
			outputs,
	...
	fn reduce(&mut self, input: ValueId, output: ValueId, axes: &[usize], keep_dimensions: bool, tree_lanes: u32) -> OperationResult<()> {
		self.emit(
			vec![input],
			vec![output],
	...
	fn finish(self) -> OperationResult<KnnAllOutputMaterialization> {
		if self.intermediate_values.len() as u64 != self.requirements.intermediate_values || self.kernels.len() as u64 != self.requirements.kernels || self.workspace_bytes != self.requirements.workspace_bytes.get() {
			return Err(knn_error(
...
fn same_storage_contract(left: &Tensor, right: &Tensor) -> bool { left.id == right.id && left.dtype == right.dtype && left.shape == right.shape && left.layout == right.layout && left.storage_bytes == right.storage_bytes }
...
fn require_categorical_code(builder: &mut ScalarProgramBuilder, code: ScalarExpression, classes: u64) -> OperationResult<()> {
	let zero = builder.i32(0).map_err(graph_error)?;
	let classes = builder
...
fn scalar_unary(builder: &mut ScalarProgramBuilder, opcode: ScalarOpcode, value: ScalarExpression) -> OperationResult<ScalarExpression> { builder.unary(opcode, value).map_err(graph_error) }

fn scalar_binary(builder: &mut ScalarProgramBuilder, opcode: ScalarOpcode, left: ScalarExpression, right: ScalarExpression) -> OperationResult<ScalarExpression> { builder.binary(opcode, left, right).map_err(graph_error) }

fn scalar_finish(builder: ScalarProgramBuilder, outputs: &[ScalarExpression]) -> OperationResult<recipe_core::ScalarProgram> { builder.finish(outputs).map_err(graph_error) }
