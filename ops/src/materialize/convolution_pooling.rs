...
use recipe_language::{AtomicOperation, AtomicOrdering, AxisSet, Elementwise, Gather, IndexBounds, PrimitiveKind, Reduce, ReduceOperator, ReduceResult, Scatter, ScatterConflict};

use super::{Emitter, FamilyDispatch, MAX_I32_INDEX, MaterializationRequest, emit_checked_gather_identity, exact_f32_extent, identity_program, input, language_error, operation_error, output, prepared_tree_lanes, prepared_u64, request_error, require_dtype, require_exact_abi, require_same_tensor_contract, require_shape, require_true, scalar_binary, scalar_builder, scalar_f32, scalar_finish, scalar_input};
...
pub(super) fn dispatch(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> FamilyDispatch {
	...
	let result = match request.descriptor.symbol {
		...
		"gpu_avg_pool_2d_backward" | "gpu_avg_pool_2d_backward_f32" => emit_average_pool_2d_backward(request, emitter),
...
fn emit_im2col_2d(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, extended: bool) -> OperationResult<()> {
	let geometry = match extended {
		true => {
			require_exact_abi(
				request,
	...
	let patch_elements = checked_product(
		request,
...
fn emit_col2im_1d(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let image_elements = checked_product(
		request,
		&[batch, input_length],
		"col2im image element count",
	)?;
	let contribution_count = checked_product(
		request,
		&[batch, output_length, kernel_size],
...
fn emit_col2im_2d(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, extended: bool) -> OperationResult<()> {
	let (geometry, valid_contributions) = match extended {
		true => {
			require_exact_abi(
				request,
				&[
		...
		false => {
			...
			let geometry = convolution_2d_geometry(request, false)?;
			let valid = checked_product(
				request,
				&[geometry.output_elements, geometry.window_elements],
				"col2im contribution count",
			)?;
	...
	emit_column_scatter_with_patch_shape(
		request,
		emitter,
		patch_elements,
		valid_contributions,
...
fn emit_column_scatter(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, contribution_count: u64, image_elements: u64, index_fact: &str) -> OperationResult<()> {
	emit_column_scatter_with_patch_shape(
		request,
		emitter,
		contribution_count,
...
fn emit_column_scatter_with_patch_shape(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, patch_elements: u64, contribution_count: u64, image_elements: u64, index_fact: &str) -> OperationResult<()> {
	...
	let gathered = emitter.intermediate(DType::F32, patch_indices.shape.clone())?;
	emitter.emit(
		vec![patches.id, patch_indices.id],
		vec![gathered],
	...
	let mapped = emitter.intermediate(DType::F32, patch_indices.shape.clone())?;
	emitter.emit(
		vec![gathered],
		vec![mapped],
	...
	emitter.emit(
		vec![image_base.id, destinations.id, mapped],
		vec![image.id],
...
fn emit_average_pool_1d(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let geometry = PoolGeometry {
		input_elements: checked_product(
			request,
			&[batch, window, filters],
...
fn emit_average_pool(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, geometry: PoolGeometry) -> OperationResult<()> {
	...
	let sums = emitter.intermediate(DType::F32, pooled.shape.clone())?;
	emitter.emit(
		vec![windows],
		vec![sums],
		PrimitiveKind::Reduce(Reduce {
			operator: ReduceOperator::Sum,
			axes: pool_axis(request)?,
	...
	emitter.emit(
		...
		vec![pooled.id],
		PrimitiveKind::Elementwise(Elementwise {
			program: divide_by_program(request, divisor)?,
		}),
	)
}
...
fn emit_channelwise_max_pool_1d(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	require_exact_abi(
		request,
...
fn emit_max_pool_2d(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let geometry = pooling_2d_geometry(request)?;
	emit_max_pool(request, emitter, geometry, true)
}

fn emit_max_pool(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, geometry: PoolGeometry, two_dimensional: bool) -> OperationResult<()> {
	require_true(request, "window_indices_verified")?;
	let tree_lanes = prepared_tree_lanes(request)?;
	let values = input(request, "values")?;
	let window_indices = input(request, "window_indices")?;
	...
	match two_dimensional {
		true => {
			...
			emitter.emit(
				vec![maxima, local_indices, bases.id],
				vec![pooled.id, winning_indices.id],
				PrimitiveKind::Elementwise(Elementwise {
					program: max_pool_2d_result_program(request, geometry)?,
				}),
			)
		}
		false => {
...
fn emit_average_pool_2d_backward(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	require_exact_abi(
		request,
		&[
			"output_gradient",
	...
	let geometry = pooling_2d_geometry(request)?;
	let contributions = checked_product(
		request,
		&[geometry.output_elements, geometry.window_elements],
		"average-pool backward contribution count",
	)?;
	emit_average_backward(
		request,
		emitter,
		geometry.input_elements,
		geometry.output_elements,
		contributions,
		geometry.window_elements,
		false,
	)
}
...
fn emit_average_backward(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, input_elements: u64, output_elements: u64, contributions: u64, divisor: u64, destinations_unique: bool) -> OperationResult<()> {
	...
	let gathered = emitter.intermediate(DType::F32, gradient_indices.shape.clone())?;
	emitter.emit(
		vec![output_gradient.id, gradient_indices.id],
		vec![gathered],
		PrimitiveKind::Gather(Gather {
			axis: 0,
			bounds: IndexBounds::Reject,
	...
	let mapped = emitter.intermediate(DType::F32, gradient_indices.shape.clone())?;
	emitter.emit(
		vec![gathered],
		vec![mapped],
		PrimitiveKind::Elementwise(Elementwise {
...
fn emit_legacy_max_pool_1d_backward(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	require_exact_abi(
		request,
		&[
			"output_gradient",
...
fn emit_channelwise_max_pool_1d_backward(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	require_exact_abi(
		request,
		&[
			"output_gradient",
...
fn emit_max_backward(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, input_elements: u64, output_elements: u64, index_stride: u64, destinations_unique: bool) -> OperationResult<()> {
	require_true(request, "gradient_indices_verified")?;
	require_true(request, "input_gradient_base_zero")?;
	...
	for (name, tensor) in [
		...
		require_shape(request, tensor, &[output_elements], name)?;
	}
	require_shape(
		request,
		gradient_base,
	...
	let destinations = emitter.intermediate(DType::I32, winning_indices.shape.clone())?;
	emitter.emit(
		vec![gathered, winning_indices.id, bases.id],
		vec![mapped, destinations],
		PrimitiveKind::Elementwise(Elementwise {
			program: max_pool_backward_program(request, index_stride)?,
		}),
	)?;
	let conflict = match destinations_unique {
		true => ScatterConflict::UniqueIndices,
...
fn emit_checked_flat_gather(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, values: &recipe_language::Tensor, indices: &recipe_language::Tensor, result: &recipe_language::Tensor) -> OperationResult<()> {
...
fn convolution_2d_geometry(request: &MaterializationRequest<'_>, extended: bool) -> OperationResult<PoolGeometry> {
	...
	if effective_height > padded_height || effective_width > padded_width {
		return Err(operation_error(
			request.descriptor.id,
			OperationErrorKind::UnsupportedConcreteShape,
	...
	let output_width = (padded_width - effective_width) / stride_width + 1;
	let input_elements = checked_product(
		request,
		&[batch, channels, input_height, input_width],
		"image element count",
...
fn pooling_2d_geometry(request: &MaterializationRequest<'_>) -> OperationResult<PoolGeometry> {
	...
	Ok(PoolGeometry {
		...
		window_elements: checked_product(
			request,
...
fn checked_effective_kernel(request: &MaterializationRequest<'_>, kernel: u64, dilation: u64, axis: &str) -> OperationResult<u64> {
	(kernel - 1)
...
fn divide_by_program(request: &MaterializationRequest<'_>, divisor: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(request.descriptor.id)?;
	let value = scalar_input(request.descriptor.id, &mut builder, DType::F32)?;
	let divisor = scalar_f32(request.descriptor.id, &mut builder, divisor)?;
	let divided = scalar_binary(
		request.descriptor.id,
		&mut builder,
		ScalarOpcode::Divide,
		value,
...
fn channelwise_pool_result_program(request: &MaterializationRequest<'_>, channels: u64) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(request.descriptor.id)?;
	let value = scalar_input(request.descriptor.id, &mut builder, DType::F32)?;
...
fn max_pool_2d_result_program(request: &MaterializationRequest<'_>, geometry: PoolGeometry) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(request.descriptor.id)?;
	let value = scalar_input(request.descriptor.id, &mut builder, DType::F32)?;
	let local = scalar_input(request.descriptor.id, &mut builder, DType::I32)?;
	let base = scalar_input(request.descriptor.id, &mut builder, DType::I32)?;
	let kernel_width = builder
		.i32(i32_extent(request, geometry.kernel_width, "kernel_width")?)
		.map_err(|error| language_error(request.descriptor.id, error.to_string()))?;
	let input_width = builder
		.i32(i32_extent(request, geometry.input_width, "input_width")?)
		.map_err(|error| language_error(request.descriptor.id, error.to_string()))?;
	let row = scalar_binary(
		request.descriptor.id,
		&mut builder,
		ScalarOpcode::Divide,
		local,
...
fn max_pool_backward_program(request: &MaterializationRequest<'_>, index_stride: u64) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(request.descriptor.id)?;
...
fn i32_extent(request: &MaterializationRequest<'_>, value: u64, name: &str) -> OperationResult<i32> { i32::try_from(value).map_err(|error| request_error(request, format!("{name} does not fit int32: {error}"))) }
