...
use recipe_language::{AtomicOperation, AtomicOrdering, AxisSet, Elementwise, Gather, IndexBounds, PrimitiveKind, Reduce, ReduceOperator, ReduceResult, Scatter, ScatterConflict, Shape};
use recipe_math::MathFunction;

use super::{Emitter, FamilyDispatch, KernelEmission, MAX_I32_INDEX, MaterializationRequest, emit_checked_gather_identity, identity_program, input, language_error, operation_error, output, prepared_f32, prepared_u64, request_error, require_dtype, require_exact_abi, require_same_tensor_contract, require_shape, require_true, scalar_binary, scalar_builder, scalar_f32, scalar_finish, scalar_input, scalar_ternary, scalar_unary};
...
pub(super) fn dispatch(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> FamilyDispatch {
	...
	let result = match request.descriptor.symbol {
		...
		"gpu_mha_split" => emit_checked_gather_identity(request, emitter, "packed", "indices", "heads", "axis", None),
		"gpu_positional_encoding" => emit_positional_encoding(request, emitter),
		"gpu_repeat_rows" => emit_repeat_rows(request, emitter),
		"gpu_rope" | "gpu_rope_partial" | "gpu_rope_partial_factors" | "gpu_rope_partial_factors_pos" | "gpu_rope_partial_pos" => emit_single_tensor_rope(request, emitter),
		symbol => {
			Err(operation_error(
...
fn emit_positional_encoding(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let sine_program = recipe_core::ScalarProgram::try_from(MathFunction::Sin).map_err(|error| language_error(request.descriptor.id, error.to_string()))?;
	let cosine_program = recipe_core::ScalarProgram::try_from(MathFunction::Cos).map_err(|error| language_error(request.descriptor.id, error.to_string()))?;
	emitter.emit_stage([
		KernelEmission {
...
fn emit_causal_softmax(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let exponential_program = recipe_core::ScalarProgram::try_from(MathFunction::Exp).map_err(|error| language_error(request.descriptor.id, error.to_string()))?;
	emitter.emit_stage([
		KernelEmission {
...
fn embedding_blend_program(request: &MaterializationRequest<'_>, scale: f32) -> OperationResult<recipe_core::ScalarProgram> {
