...
use recipe_language::{AxisSet, Elementwise, PrimitiveKind, Reduce, ReduceOperator, ReduceResult, ScalarProgramBuilder, Shape, Tensor};
use recipe_math::MathFunction;

use super::{Emitter, FamilyDispatch, KernelEmission, MaterializationRequest, exact_f32_extent, input, language_error, output, prepared_f32, prepared_tree_lanes, request_error, require_dtype, require_exact_abi, require_same_tensor_contract, scalar_binary, scalar_builder, scalar_f32, scalar_finish, scalar_input, scalar_ternary, scalar_unary};
...
fn emit_multiclass_correct_count(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let tree_lanes = prepared_tree_lanes(request)?;
	let row_shape = Shape::new(vec![rows]).map_err(|error| language_error(request.descriptor.id, error.to_string()))?;
...
fn emit_dense_multiclass_accuracy(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	require_exact_abi(request, &["predictions", "targets"], &["accuracy"], &[
		"tree_lanes",
	])?;
	let predictions = input(request, "predictions")?;
...
fn emit_finite_metric(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, all_finite: bool) -> OperationResult<()> {
...
fn emit_column_variance(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let divisor = exact_f32_extent(request, rows, "row count")?;
	let tree_lanes = prepared_tree_lanes(request)?;
	let column_shape = Shape::new(vec![columns]).map_err(|error| language_error(request.descriptor.id, error.to_string()))?;
	let kept_column_shape = Shape::new(vec![1, columns]).map_err(|error| language_error(request.descriptor.id, error.to_string()))?;
...
fn require_equal_nonempty_f32<'a>(request: &MaterializationRequest<'_>, reference: &Tensor, others: impl IntoIterator<Item = (&'a str, &'a Tensor)>) -> OperationResult<()> {
...
fn require_f32_matrix(request: &MaterializationRequest<'_>, tensor: &Tensor, name: &str) -> OperationResult<(u64, u64)> {
...
fn require_paired_loss_tensors(request: &MaterializationRequest<'_>, left: &Tensor, right: &Tensor, labels: &Tensor, losses: &Tensor) -> OperationResult<(u64, u64)> {
...
fn require_shape(request: &MaterializationRequest<'_>, tensor: &Tensor, shape: &[u64], name: &str) -> OperationResult<()> {
...
fn all_axes(request: &MaterializationRequest<'_>, shape: &Shape) -> OperationResult<AxisSet> { AxisSet::new((0..shape.rank()).collect()).map_err(|error| language_error(request.descriptor.id, error.to_string())) }

fn reduction(request: &MaterializationRequest<'_>, operator: ReduceOperator, axes: AxisSet, keep_dimensions: bool, result: ReduceResult, tree_lanes: u32) -> OperationResult<PrimitiveKind> {
...
fn require_finite(operation: OperationId, builder: &mut ScalarProgramBuilder, value: recipe_language::ScalarExpression) -> OperationResult<()> {
...
fn require_finite_nonnegative(operation: OperationId, builder: &mut ScalarProgramBuilder, value: recipe_language::ScalarExpression) -> OperationResult<()> {
