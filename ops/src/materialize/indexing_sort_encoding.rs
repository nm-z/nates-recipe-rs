...
use super::{ Emitter, FamilyDispatch, KernelEmission, MAX_I32_INDEX, MaterializationRequest, exact_f32_extent,
	...
	scalar_input, scalar_ternary, scalar_unary, }; use crate::{OperationDescriptor, OperationErrorKind, OperationResult};
...
enum ColumnUpdate { Add, ScaledAdd, }

#[derive(Clone, Copy, Debug)]
struct SortedPrefix<'a> {
	elements: u64, prefix: u64, axis: usize,
	prefix_name: &'a str,
	output_name: &'a str,
...
fn emit_column_update(
	...
	update: ColumnUpdate, ) -> OperationResult<()> { let input_names: &[&str] = match update {
	...
	match selected < columns { true => Ok(()), false => { Err(request_error( request,
	...
	let selected_values = emitter.intermediate(DType::F32, column.shape.clone())?; emitter.emit(
		vec![matrix.id, indices.id], vec![selected_values], checked_gather(0), )?;
	let updates = emitter.intermediate(DType::F32, column.shape.clone())?; let program = match update {
		ColumnUpdate::Add => add_program(request)?, ColumnUpdate::ScaledAdd => scaled_add_program(request)?, };
	let map_inputs = match update { ColumnUpdate::Add => vec![selected_values, column.id], ColumnUpdate::ScaledAdd => {
			let scale = input(request, "scale")?;
			require_dtype(request, scale, DType::F32, "scale")?;
			require_shape(request, scale, &[1], "scale")?;
			vec![selected_values, column.id, scale.id] } }; emitter.emit( map_inputs, vec![updates],
		PrimitiveKind::Elementwise(Elementwise { program }), )?; emitter.emit( vec![matrix.id, indices.id, updates],
		vec![updated.id], checked_unique_scatter(0), ) }
...
fn emit_diagonal_add(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let diagonal = emitter.intermediate(DType::F32, indices.shape.clone())?; emitter.emit( vec![matrix.id, indices.id],
		vec![diagonal], checked_gather(0), )?; let updates = emitter.intermediate(DType::F32, indices.shape.clone())?;
	emitter.emit( vec![diagonal, value.id], vec![updates], PrimitiveKind::Elementwise(Elementwise {
			program: add_program(request)?, }), )?; emitter.emit( vec![matrix.id, indices.id, updates], vec![updated.id],
...
fn emit_argsort(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	emit_sorted_prefix(request, emitter, SortedPrefix { elements, prefix: elements, axis: 0,
...
fn emit_partial_argsort(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	match prefix == elements { true => { emit_sorted_prefix(request, emitter, SortedPrefix { elements, prefix, axis: 0,
				prefix_name: "prefix_indices",
				output_name: "indices",
				verification_fact: "prefix_indices_verified",
			}) }
		false => { Err(operation_error( request.descriptor.id, OperationErrorKind::UnsupportedConcreteShape,
...
fn emit_segment_sum(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	if maximum_segment_length > elements { return Err(request_error( request,
			"maximum_segment_length exceeds the input element count",
		)); }
	let grouped_elements = checked_product( request, segments, maximum_segment_length,
	...
	require_i32_indexable( request, grouped_elements,
	...
	let sorted_original_indices = emitter.intermediate(DType::I32, segment_ids.shape.clone())?; emitter.emit(
		vec![segment_ids.id], vec![sorted_ids, sorted_original_indices], ascending_stable_sort(0), )?;
	...
	let segment_starts = emitter.intermediate(DType::I32, segment_ids.shape.clone())?; emitter.emit_stage([
		KernelEmission { inputs: Vec::new(), outputs: vec![positions], kind: PrimitiveKind::IndexMap(IndexMap { start: 0,
				element_step: 1, iteration_step: 0, modulus: None, }), }, KernelEmission { inputs: Vec::new(),
			outputs: vec![previous_positions], kind: PrimitiveKind::IndexMap(IndexMap { start: -1, element_step: 1,
				iteration_step: 0, modulus: None, }), }, KernelEmission { inputs: vec![sorted_ids, previous_positions],
			outputs: vec![previous_ids], kind: PrimitiveKind::Gather(Gather { axis: 0, bounds: IndexBounds::Clamp, }), },
		KernelEmission { inputs: vec![sorted_ids, previous_ids, positions], outputs: vec![boundary_positions],
			kind: PrimitiveKind::Elementwise(Elementwise { program: segment_boundary_program(request)?, }), }, KernelEmission {
			inputs: vec![boundary_positions], outputs: vec![segment_starts], kind: PrimitiveKind::Scan(Scan {
				operator: ReduceOperator::Maximum, axis: 0, mode: ScanMode::Inclusive, reverse: false, tree_lanes, }), }, ])?;
	...
	let grouped = emitter.intermediate(DType::F32, matrix_shape)?; let reduction_axes =
		AxisSet::new(vec![1]).map_err(|error| language_error(request.descriptor.id, error.to_string()))?; emitter.emit_stage([
		KernelEmission { inputs: vec![values.id, sorted_original_indices], outputs: vec![sorted_values],
			kind: checked_gather(0), }, KernelEmission { inputs: vec![sorted_ids, positions, segment_starts],
			outputs: vec![destinations], kind: PrimitiveKind::Elementwise(Elementwise {
				program: segment_destination_program(request, segments, maximum_segment_length)?, }), }, KernelEmission {
			inputs: Vec::new(), outputs: vec![packed_positions], kind: PrimitiveKind::IndexMap(IndexMap { start: 0,
				element_step: 1, iteration_step: 0, modulus: None, }), }, KernelEmission { inputs: vec![packed_positions],
			outputs: vec![zero_base], kind: PrimitiveKind::Elementwise(Elementwise { program: zero_from_i32_program(request)?,
			}), }, KernelEmission { inputs: vec![zero_base, destinations, sorted_values], outputs: vec![packed],
			kind: checked_unique_scatter(0), }, KernelEmission { inputs: Vec::new(), outputs: vec![matrix_positions],
			kind: PrimitiveKind::IndexMap(IndexMap { start: 0, element_step: 1, iteration_step: 0, modulus: None, }), },
		KernelEmission { inputs: vec![packed, matrix_positions], outputs: vec![grouped], kind: checked_gather(0), },
		KernelEmission { inputs: vec![grouped], outputs: vec![sums.id], kind: PrimitiveKind::Reduce(Reduce {
				operator: ReduceOperator::Sum, axes: reduction_axes, keep_dimensions: false, result: ReduceResult::Value,
				tree_lanes, }), }, ]) }
...
fn emit_topk_per_row(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let sorted_values = emitter.intermediate(DType::F32, values.shape.clone())?;
	let sorted_indices = emitter.intermediate(DType::I32, values.shape.clone())?; emitter.emit( vec![values.id],
		vec![sorted_values, sorted_indices], ascending_stable_sort(1), )?; emitter.emit(
		vec![sorted_indices, prefix_indices.id], vec![indices.id], checked_gather(1), ) }
...
fn emit_sorted_prefix(
	...
	match specification.prefix <= specification.elements { true => Ok(()), false => { Err(request_error( request,
				"sorted prefix exceeds the input extent",
			)) } }?; require_true(request, specification.verification_fact)?;
	require_dtype(request, values, DType::F32, "values")?;
	require_dtype( request, prefix_indices, DType::I32, specification.prefix_name, )?;
	...
	require_shape( request, prefix_indices, &[specification.prefix], specification.prefix_name, )?; require_shape( request,
		indices, &[specification.prefix], specification.output_name, )?;
	let sorted_values = emitter.intermediate(DType::F32, values.shape.clone())?;
	let sorted_indices = emitter.intermediate(DType::I32, values.shape.clone())?; emitter.emit( vec![values.id],
		vec![sorted_values, sorted_indices], ascending_stable_sort(specification.axis), )?; emitter.emit(
		vec![sorted_indices, prefix_indices.id], vec![indices.id], checked_gather(specification.axis), ) }
...
fn emit_uniform_edges_single_row(
	...
	match rows { 1 => Ok(()), other_rows => { Err(operation_error( request.descriptor.id,
				...
				), )) } }?; let edge_count = bins.checked_add(1).ok_or_else(|| { operation_error( request.descriptor.id,
	...
	emitter.emit( vec![values.id, bin_indices.id], vec![edges.id], PrimitiveKind::Elementwise(Elementwise {
...
fn emit_row_concatenation(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let total_columns = left_columns.checked_add(right_columns).ok_or_else(|| { operation_error( request.descriptor.id,
			OperationErrorKind::WorkspaceArithmeticOverflow,
			"concatenated column count overflowed u64",
		) })?;
	let total = checked_product(request, rows, total_columns, "concatenated element count")?;
	emit_concatenation( request, emitter, left_elements, right_elements, total,
		"concatenation_tables_verified",
	) }
...
fn emit_vector_concatenation(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let total = left_elements.checked_add(right_elements).ok_or_else(|| { operation_error( request.descriptor.id,
			OperationErrorKind::WorkspaceArithmeticOverflow,
...
fn emit_concatenation(
	request: &MaterializationRequest<'_>,
	emitter: &mut Emitter<'_>,
	left_elements: u64, right_elements: u64, total: u64, verification_fact: &str, ) -> OperationResult<()> {
	...
	let gathered_right = emitter.intermediate(right.dtype, result_shape)?; emitter.emit_stage([ KernelEmission {
			inputs: vec![left.id, left_indices.id], outputs: vec![gathered_left], kind: checked_gather(0), }, KernelEmission {
			inputs: vec![right.id, right_indices.id], outputs: vec![gathered_right], kind: checked_gather(0), }, ])?;
	emitter.emit( vec![select_left.id, gathered_left, gathered_right], vec![concatenated.id],
		PrimitiveKind::Elementwise(Elementwise { program: select_program(request, left.dtype)?, }), ) }
...
fn emit_one_hot(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let updates = emitter.intermediate(DType::F32, labels.shape.clone())?; emitter.emit_stage([ KernelEmission {
			inputs: vec![base.id], outputs: vec![cleared], kind: PrimitiveKind::Elementwise(Elementwise {
				program: clear_program(request)?, }), }, KernelEmission { inputs: vec![labels.id], outputs: vec![updates],
			kind: PrimitiveKind::Elementwise(Elementwise { program: checked_one_program(request, classes_i32)?, }), }, ])?;
	emitter.emit( vec![cleared, destinations.id, updates], vec![encoded.id], checked_unique_scatter(0), ) }
...
fn emit_dense_upper_triangle(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let dimension = positive_parameter(request, "dimension")?;
	match factor_rows >= dimension { true => Ok(()), false => { Err(request_error( request,
				"factor_rows must be at least dimension",
			)) } }?; let factor_elements = checked_product( request, factor_rows, dimension,
		"upper-triangle factor element count",
	)?; let output_elements = checked_product( request, dimension, dimension,
	...
	let gathered = emitter.intermediate(DType::F32, upper.shape.clone())?; emitter.emit( vec![factor.id, indices.id],
		vec![gathered], checked_gather(0), )?; emitter.emit( vec![mask.id, gathered, zero.id], vec![upper.id],
		PrimitiveKind::Elementwise(Elementwise { program: select_program(request, DType::F32)?, }), ) }
...
fn emit_transpose(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	emit_flat_gather_map( request, emitter, elements, elements,
...
fn emit_flat_gather_map(
	request: &MaterializationRequest<'_>,
	emitter: &mut Emitter<'_>,
	source_elements: u64, result_elements: u64, verification_fact: &str, ) -> OperationResult<()> {
	let values = input(request, "values")?;
	let indices_name = match request.descriptor.symbol {
		...
		symbol => { return Err(request_error( request,
		...
		symbol => { return Err(request_error( request,
			...
			)); } }; let indices = input(request, indices_name)?; let result = output(request, output_name)?;
	...
	let gathered = emitter.intermediate(DType::F32, result.shape.clone())?; emitter.emit( vec![values.id, indices.id],
		vec![gathered], checked_gather(0), )?; emitter.emit( vec![gathered], vec![result.id],
...
fn emit_triangular_mask(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let dimension = positive_parameter(request, "dimension")?;
	let elements = checked_product( request, dimension, dimension,
	...
	match elements { 0 => { Err(request_error( request,
				"triangular mask dimension must be positive",
			)) }
		positive_elements => { let confirmed_elements = positive_elements; match confirmed_elements == mask.shape.elements() {
				true => { emitter.emit( vec![rows.id, columns.id, fill.id], vec![mask.id], PrimitiveKind::Elementwise(Elementwise {
...
fn checked_unique_scatter(axis: usize) -> PrimitiveKind { PrimitiveKind::Scatter(Scatter { axis,
		bounds: IndexBounds::Reject, conflict: ScatterConflict::UniqueIndices, }) }

fn ascending_stable_sort(axis: usize) -> PrimitiveKind { PrimitiveKind::Sort(Sort { axis,
...
fn positive_parameter(request: &MaterializationRequest<'_>, name: &str) -> OperationResult<u64> {
	match prepared_u64(request.descriptor.id, request.parameters, name)? { 0 => { Err(request_error( request,
				format!("{name} must be greater than zero"),
			)) }
		value => Ok(value), } }

fn checked_product(request: &MaterializationRequest<'_>, left: u64, right: u64, label: &str) -> OperationResult<u64> {
	left.checked_mul(right).ok_or_else(|| { operation_error( request.descriptor.id,
			OperationErrorKind::WorkspaceArithmeticOverflow,
			format!("{label} overflowed u64"),
		) }) }

fn materialized_shape(request: &MaterializationRequest<'_>, extents: &[u64]) -> OperationResult<Shape> { Shape::new(extents.to_vec()).map_err(|error| language_error(request.descriptor.id, error.to_string())) }

fn segment_boundary_program(request: &MaterializationRequest<'_>) -> OperationResult<recipe_core::ScalarProgram> {
	let operation = request.descriptor.id; let mut builder = scalar_builder(operation)?;
	let current = scalar_input(operation, &mut builder, DType::I32)?;
	let previous = scalar_input(operation, &mut builder, DType::I32)?;
	let position = scalar_input(operation, &mut builder, DType::I32)?; let zero = builder .i32(0)
		.map_err(|error| language_error(operation, error.to_string()))?;
	let first = scalar_binary(operation, &mut builder, ScalarOpcode::Equal, position, zero)?; let changed = scalar_binary(
		operation, &mut builder, ScalarOpcode::NotEqual, current, previous, )?;
	let boundary = scalar_binary(operation, &mut builder, ScalarOpcode::BitOr, first, changed)?;
	let start = scalar_ternary( operation, &mut builder, ScalarOpcode::Select, boundary, position, zero, )?;
	scalar_finish(operation, builder, &[start]) }

fn segment_destination_program(
	request: &MaterializationRequest<'_>,
	segments: u64, maximum_segment_length: u64, ) -> OperationResult<recipe_core::ScalarProgram> {
	...
	let mut builder = scalar_builder(operation)?; let segment = scalar_input(operation, &mut builder, DType::I32)?;
	let position = scalar_input(operation, &mut builder, DType::I32)?;
	let start = scalar_input(operation, &mut builder, DType::I32)?; let zero = builder .i32(0)
		.map_err(|error| language_error(operation, error.to_string()))?; let segment_limit = builder .i32(segments)
		.map_err(|error| language_error(operation, error.to_string()))?; let length_limit = builder
		.i32(maximum_segment_length) .map_err(|error| language_error(operation, error.to_string()))?;
	let local = scalar_binary( operation, &mut builder, ScalarOpcode::Subtract, position, start, )?;
	let segment_nonnegative = scalar_binary( operation, &mut builder, ScalarOpcode::GreaterThanOrEqual, segment, zero, )?;
	let segment_bounded = scalar_binary( operation, &mut builder, ScalarOpcode::LessThan, segment, segment_limit, )?;
	let local_nonnegative = scalar_binary( operation, &mut builder, ScalarOpcode::GreaterThanOrEqual, local, zero, )?;
	let local_bounded = scalar_binary( operation, &mut builder, ScalarOpcode::LessThan, local, length_limit, )?;
	let valid_segment = scalar_binary( operation, &mut builder, ScalarOpcode::BitAnd, segment_nonnegative, segment_bounded,
	)?; let valid_local = scalar_binary( operation, &mut builder, ScalarOpcode::BitAnd, local_nonnegative, local_bounded,
	)?; let valid = scalar_binary( operation, &mut builder, ScalarOpcode::BitAnd, valid_segment, valid_local, )?;
	scalar_unary(operation, &mut builder, ScalarOpcode::Require, valid)?; let base = scalar_binary( operation,
		&mut builder, ScalarOpcode::Multiply, segment, length_limit, )?;
...
fn zero_from_i32_program(request: &MaterializationRequest<'_>) -> OperationResult<recipe_core::ScalarProgram> {
	let operation = request.descriptor.id; let mut builder = scalar_builder(operation)?;
	let position = scalar_input(operation, &mut builder, DType::I32)?; let consumed = scalar_unary( operation,
		&mut builder, ScalarOpcode::ConvertI32ToF32, position, )?; let zero = scalar_f32(operation, &mut builder, 0.0)?;
	let always = builder .i32(1) .map_err(|error| language_error(operation, error.to_string()))?;
	let cleared = scalar_ternary( operation, &mut builder, ScalarOpcode::Select, always, zero, consumed, )?;
...
fn require_i32_indexable(request: &MaterializationRequest<'_>, extent: u64, label: &str) -> OperationResult<()> {
	match (extent > 0, extent <= MAX_I32_INDEX) { (true, true) => Ok(()), (false, true) | (false, false) => {
			Err(operation_error( request.descriptor.id, OperationErrorKind::UnsupportedConcreteShape,
				format!("{label} must be positive"),
			)) }
		(true, false) => { Err(operation_error( request.descriptor.id, OperationErrorKind::UnsupportedConcreteShape,
				format!("{label} must fit checked int32 indexes"),
			)) } } }

fn require_range(
	request: &MaterializationRequest<'_>,
	start: u64, count: u64, extent: u64, label: &str, ) -> OperationResult<()> { let end = start .checked_add(count)
		.ok_or_else(|| request_error(request, format!("{label} start plus count overflowed u64")))?;
	match end <= extent { true => Ok(()), false => { Err(operation_error( request.descriptor.id,
...
fn add_program(request: &MaterializationRequest<'_>) -> OperationResult<recipe_core::ScalarProgram> {
	...
	let right = scalar_input(request.descriptor.id, &mut builder, DType::F32)?; let sum = scalar_binary(
		request.descriptor.id, &mut builder, ScalarOpcode::Add, left, right, )?;
...
fn scaled_add_program(request: &MaterializationRequest<'_>) -> OperationResult<recipe_core::ScalarProgram> {
	...
	let scale = scalar_input(request.descriptor.id, &mut builder, DType::F32)?; let scaled = scalar_binary(
		request.descriptor.id, &mut builder, ScalarOpcode::Multiply, column, scale, )?; let updated = scalar_binary(
		request.descriptor.id, &mut builder, ScalarOpcode::Add, old, scaled, )?;
	scalar_finish(request.descriptor.id, builder, &[updated]) }

fn uniform_single_row_program(
	request: &MaterializationRequest<'_>,
	bins: f32, ) -> OperationResult<recipe_core::ScalarProgram> { let mut builder = scalar_builder(request.descriptor.id)?;
	let value = scalar_input(request.descriptor.id, &mut builder, DType::F32)?;
	let bin_index = scalar_input(request.descriptor.id, &mut builder, DType::F32)?; let difference = scalar_binary(
		request.descriptor.id, &mut builder, ScalarOpcode::Subtract, value, value, )?;
	let divisor = scalar_f32(request.descriptor.id, &mut builder, bins)?; let width = scalar_binary( request.descriptor.id,
		&mut builder, ScalarOpcode::Divide, difference, divisor, )?; let offset = scalar_binary( request.descriptor.id,
		&mut builder, ScalarOpcode::Multiply, bin_index, width, )?; let edge = scalar_binary( request.descriptor.id,
		&mut builder, ScalarOpcode::Add, value, offset, )?; scalar_finish(request.descriptor.id, builder, &[edge]) }
...
fn select_program(request: &MaterializationRequest<'_>, dtype: DType) -> OperationResult<recipe_core::ScalarProgram> {
	...
	let alternate = scalar_input(request.descriptor.id, &mut builder, dtype)?; let result = scalar_ternary(
		request.descriptor.id, &mut builder, ScalarOpcode::Select, condition, selected, alternate, )?;
...
fn clear_program(request: &MaterializationRequest<'_>) -> OperationResult<recipe_core::ScalarProgram> {
	...
	let zero = scalar_f32(request.descriptor.id, &mut builder, 0.0)?; let always = builder .i32(1)
		.map_err(|error| language_error(request.descriptor.id, error.to_string()))?; let cleared = scalar_ternary(
		request.descriptor.id, &mut builder, ScalarOpcode::Select, always, zero, consumed, )?;
	scalar_finish(request.descriptor.id, builder, &[cleared]) }

fn checked_one_program(
	request: &MaterializationRequest<'_>,
	classes: i32, ) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(request.descriptor.id)?;
	let label = scalar_input(request.descriptor.id, &mut builder, DType::I32)?; let zero = builder .i32(0)
		.map_err(|error| language_error(request.descriptor.id, error.to_string()))?; let upper = builder .i32(classes)
		.map_err(|error| language_error(request.descriptor.id, error.to_string()))?; let nonnegative = scalar_binary(
		request.descriptor.id, &mut builder, ScalarOpcode::GreaterThanOrEqual, label, zero, )?;
	let below_upper = scalar_binary( request.descriptor.id, &mut builder, ScalarOpcode::LessThan, label, upper, )?;
	let valid = scalar_binary( request.descriptor.id, &mut builder, ScalarOpcode::BitAnd, nonnegative, below_upper, )?;
	scalar_unary( request.descriptor.id, &mut builder, ScalarOpcode::Require, valid, )?;
	let one = scalar_f32(request.descriptor.id, &mut builder, 1.0)?; scalar_finish(request.descriptor.id, builder, &[one])
}
...
fn triangular_mask_program(request: &MaterializationRequest<'_>) -> OperationResult<recipe_core::ScalarProgram> {
	...
	let fill = scalar_input(request.descriptor.id, &mut builder, DType::F32)?; let retained = scalar_binary(
		request.descriptor.id, &mut builder, ScalarOpcode::LessThanOrEqual, column, row, )?;
	let zero = scalar_f32(request.descriptor.id, &mut builder, 0.0)?; let value = scalar_ternary( request.descriptor.id,
		&mut builder, ScalarOpcode::Select, retained, zero, fill, )?; scalar_finish(request.descriptor.id, builder, &[value])
}
