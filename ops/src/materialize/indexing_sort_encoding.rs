...
use recipe_language::{AxisSet, Elementwise, Gather, IndexBounds, IndexMap, PrimitiveKind, Reduce, ReduceOperator, ReduceResult, Scan, ScanMode, Scatter, ScatterConflict, Shape, Sort, SortDirection};

use super::{Emitter, FamilyDispatch, KernelEmission, MAX_I32_INDEX, MaterializationRequest, exact_f32_extent, identity_program, input, language_error, operation_error, output, prepared_u64, request_error, require_dtype, require_exact_abi, require_shape, require_true, scalar_binary, scalar_builder, scalar_f32, scalar_finish, scalar_input, scalar_ternary, scalar_unary};
...
fn emit_column_update(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, update: ColumnUpdate) -> OperationResult<()> {
...
fn emit_segment_sum(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let reduction_axes = AxisSet::new(vec![1]).map_err(|error| language_error(request.descriptor.id, error.to_string()))?;
	emitter.emit_stage([
		KernelEmission {
...
fn emit_sorted_prefix(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, specification: SortedPrefix<'_>) -> OperationResult<()> {
	let values = input(request, "values")?;
...
fn emit_uniform_edges_single_row(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	match rows {
		...
		other_rows => {
			Err(operation_error(
				request.descriptor.id,
				OperationErrorKind::UnsupportedConcreteShape,
				format!("gpu_bin_edges_uniform needs a reduction stage for {other_rows} rows; the normative recipe currently contains only Elementwise"),
			))
		}
	}?;
...
fn emit_concatenation(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, left_elements: u64, right_elements: u64, total: u64, verification_fact: &str) -> OperationResult<()> {
...
fn emit_flat_gather_map(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, source_elements: u64, result_elements: u64, verification_fact: &str) -> OperationResult<()> {
	let values = input(request, "values")?;
...
fn segment_destination_program(request: &MaterializationRequest<'_>, segments: u64, maximum_segment_length: u64) -> OperationResult<recipe_core::ScalarProgram> {
	let operation = request.descriptor.id;
	let segments = i32::try_from(segments).map_err(|error| request_error(request, error.to_string()))?;
	let maximum_segment_length = i32::try_from(maximum_segment_length).map_err(|error| request_error(request, error.to_string()))?;
	let mut builder = scalar_builder(operation)?;
...
fn require_range(request: &MaterializationRequest<'_>, start: u64, count: u64, extent: u64, label: &str) -> OperationResult<()> {
	let end = start
		.checked_add(count)
...
fn uniform_single_row_program(request: &MaterializationRequest<'_>, bins: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(request.descriptor.id)?;
...
fn checked_one_program(request: &MaterializationRequest<'_>, classes: i32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(request.descriptor.id)?;
