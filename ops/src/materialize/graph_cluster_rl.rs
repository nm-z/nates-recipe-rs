...
fn emit_degree(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let bins =
		u32::try_from(nodes).map_err(|conversion_error| request_error(request, conversion_error.to_string()))?;
	emitter.emit(
		vec![mapped],
		vec![degrees.id],
...
fn emit_centroid_update(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let bins = u32::try_from(centroid_count)
		.map_err(|conversion_error| request_error(request, conversion_error.to_string()))?;
	emitter.emit_stage([
		KernelEmission {
			inputs: vec![contributions],
			outputs: vec![sums],
...
fn checked_flat_action_program(
	operation: OperationId,
	action_count: u64,
