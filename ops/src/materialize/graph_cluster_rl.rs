use recipe_core::{DType, ScalarLiteral, ScalarOpcode}; use recipe_language::{
	...
	SortDirection, }; use recipe_math::MathFunction;

use super::{ Emitter, FamilyDispatch, KernelEmission, MAX_I32_INDEX, MaterializationRequest, identity_program, input,
...
pub(super) fn dispatch(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> FamilyDispatch {
	match supports(request.descriptor) { false => FamilyDispatch::NotOwned, true => {
...
fn emit_degree(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let mapped = emitter.intermediate(DType::I32, edge_destinations.shape.clone())?; emitter.emit(
		vec![edge_destinations.id], vec![mapped], PrimitiveKind::Elementwise(Elementwise {
			program: identity_program(request.descriptor.id, DType::I32)?, }), )?; let bins =
		u32::try_from(nodes).map_err(|conversion_error| request_error(request, conversion_error.to_string()))?; emitter.emit(
		vec![mapped], vec![degrees.id], PrimitiveKind::Histogram(Histogram { bins, weighted: false,
			ordering: AtomicOrdering::Relaxed, }), ) }

fn emit_temporal_difference_targets(
	...
	emitter.emit(vec![values.id, indices.id], vec![next_values], gather())?; emitter.emit(
		vec![rewards.id, next_values, done_mask.id], vec![targets.id], PrimitiveKind::Elementwise(Elementwise {
...
fn emit_gcn_normalization(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	] { require_dtype(request, tensor, DType::F32, name)?; require_shape(request, tensor, shape, name)?; }
	require_dtype(request, indices, DType::I32, "degree_indices")?;
	require_shape(request, indices, &[elements], "degree_indices")?;

	let gathered_degrees = emitter.intermediate(DType::F32, features.shape.clone())?; emitter.emit(
		vec![degrees.id, indices.id], vec![gathered_degrees], gather(), )?; emitter.emit( vec![features.id, gathered_degrees],
		vec![normalized.id], PrimitiveKind::Elementwise(Elementwise {
...
fn emit_discounted_returns(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let gamma = finite_parameter(request, "gamma")?;
	require_supported( request, gamma == 1.0,
	...
	for (name, tensor) in [("rewards", rewards), ("returns", returns)] {
		require_dtype(request, tensor, DType::F32, name)?; require_shape(request, tensor, &[length], name)?; }

	let mapped_rewards = emitter.intermediate(DType::F32, rewards.shape.clone())?; emitter.emit( vec![rewards.id],
		vec![mapped_rewards], PrimitiveKind::Elementwise(Elementwise {
			program: identity_program(request.descriptor.id, DType::F32)?, }), )?; emitter.emit( vec![mapped_rewards],
		vec![returns.id], PrimitiveKind::Scan(Scan { operator: ReduceOperator::Sum, axis: 0, mode: ScanMode::Inclusive,
			reverse: true, tree_lanes, }), ) }
...
fn emit_generalized_advantage(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	require_supported( request, gamma * lambda == 1.0,
	...
	for (name, tensor) in [("next_value_indices", indices), ("has_next", has_next)] {
		require_dtype(request, tensor, DType::I32, name)?; require_shape(request, tensor, &[length], name)?; }

	let next_values = emitter.intermediate(DType::F32, values.shape.clone())?;
	let deltas = emitter.intermediate(DType::F32, values.shape.clone())?; emitter.emit_stage([ KernelEmission {
			inputs: vec![values.id, indices.id], outputs: vec![next_values], kind: gather(), }, KernelEmission {
			inputs: vec![rewards.id, values.id, next_values, has_next.id], outputs: vec![deltas],
			kind: PrimitiveKind::Elementwise(Elementwise { program: advantage_delta_program(request.descriptor.id, gamma)?, }),
		}, ])?; emitter.emit( vec![deltas], vec![advantages.id], PrimitiveKind::Scan(Scan { operator: ReduceOperator::Sum,
			axis: 0, mode: ScanMode::Inclusive, reverse: true, tree_lanes, }), ) }

fn emit_gaussian_log_probability(
	request: &MaterializationRequest<'_>,
	emitter: &mut Emitter<'_>,
) -> OperationResult<()> { require_exact_abi( request,
	...
	let tree_lanes = prepared_tree_lanes(request)?; for (name, tensor) in [
		("means", means),
		("log_standard_deviations", log_standard_deviations),
		("actions", actions),
	] { require_dtype(request, tensor, DType::F32, name)?; require_shape(request, tensor, &[rows, dimensions], name)?; }
	...
	let exponential = recipe_core::ScalarProgram::try_from(MathFunction::Exp)
		.map_err(|error| language_error(request.descriptor.id, error.to_string()))?; emitter.emit_stage([ KernelEmission {
			inputs: vec![log_standard_deviations.id], outputs: vec![standard_deviations],
			kind: PrimitiveKind::Elementwise(Elementwise { program: exponential, }), }, KernelEmission { inputs: vec![ means.id,
				log_standard_deviations.id, actions.id, standard_deviations, ], outputs: vec![terms],
			kind: PrimitiveKind::Elementwise(Elementwise { program: gaussian_term_program(request.descriptor.id)?, }), }, ])?;
	emitter.emit( vec![terms], vec![log_probabilities.id], reduction( request.descriptor.id, ReduceOperator::Sum, &[1],
			false, ReduceResult::Value, tree_lanes, )?, ) }

fn emit_categorical_log_probability(
	request: &MaterializationRequest<'_>,
	emitter: &mut Emitter<'_>,
) -> OperationResult<()> { require_exact_abi( request,
	...
	for (name, tensor) in [("actions", actions), ("row_bases", row_bases)] {
		require_dtype(request, tensor, DType::I32, name)?; require_shape(request, tensor, &[rows], name)?; }
	require_dtype(request, log_probabilities, DType::F32, "log_probabilities")?;
	require_shape(request, log_probabilities, &[rows], "log_probabilities")?;

	let row_shape = shape(request, &[rows])?; let kept_row_shape = shape(request, &[rows, 1])?;
	...
	let exponential = recipe_core::ScalarProgram::try_from(MathFunction::Exp)
		.map_err(|error| language_error(request.descriptor.id, error.to_string()))?; emitter.emit_stage([ KernelEmission {
			inputs: vec![logits.id], outputs: vec![row_maxima_kept], kind: reduction( request.descriptor.id,
				ReduceOperator::Maximum, &[1], true, ReduceResult::Value, tree_lanes, )?, }, KernelEmission {
			inputs: vec![logits.id], outputs: vec![row_maxima], kind: reduction( request.descriptor.id, ReduceOperator::Maximum,
				&[1], false, ReduceResult::Value, tree_lanes, )?, }, KernelEmission { inputs: vec![logits.id, row_maxima_kept],
			outputs: vec![shifted], kind: PrimitiveKind::Elementwise(Elementwise {
				program: subtract_program(request.descriptor.id)?, }), }, KernelEmission { inputs: vec![shifted],
			outputs: vec![exponentials], kind: PrimitiveKind::Elementwise(Elementwise { program: exponential, }), },
		KernelEmission { inputs: vec![exponentials], outputs: vec![row_sums], kind: reduction( request.descriptor.id,
				ReduceOperator::Sum, &[1], false, ReduceResult::Value, tree_lanes, )?, }, KernelEmission {
			inputs: vec![row_bases.id, actions.id], outputs: vec![selected_indices],
			kind: PrimitiveKind::Elementwise(Elementwise {
				program: checked_flat_action_program(request.descriptor.id, action_count)?, }), }, KernelEmission {
			inputs: vec![flat_logits.id, selected_indices], outputs: vec![selected_logits], kind: gather(), }, ])?;

	let log_sums = emitter.intermediate(DType::F32, row_shape)?;
	let logarithm = recipe_core::ScalarProgram::try_from(MathFunction::Log)
		.map_err(|error| language_error(request.descriptor.id, error.to_string()))?; emitter.emit_stage([ KernelEmission {
			inputs: vec![row_sums], outputs: vec![log_sums],
			kind: PrimitiveKind::Elementwise(Elementwise { program: logarithm }), }, KernelEmission {
			inputs: vec![selected_logits, row_maxima, log_sums], outputs: vec![log_probabilities.id],
			kind: PrimitiveKind::Elementwise(Elementwise { program: categorical_finish_program(request.descriptor.id)?, }), }, ])
}
...
fn emit_csr_sparse_matrix(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let outputs = checked_product( request, &[rows, features],
...
fn emit_csr_padded(
	request: &MaterializationRequest<'_>,
	emitter: &mut Emitter<'_>,
	operand_name: &str, operand_elements: u64, output_elements: u64, ) -> OperationResult<()> {
	let values = input(request, "values")?;
	...
	let tree_lanes = prepared_tree_lanes(request)?; for fact in [
		"tables_verified",
		"output_base_zero",
		"output_indices_unique",
	...
	require_shape( request, output_indices, &[output_elements],
		"output_indices",
	)?;
	...
	let gathered_operand = emitter.intermediate(DType::F32, table_shape.clone())?; emitter.emit_stage([ KernelEmission {
			inputs: vec![values.id, value_indices.id], outputs: vec![gathered_values], kind: gather(), }, KernelEmission {
			inputs: vec![operand.id, operand_indices.id], outputs: vec![gathered_operand], kind: gather(), }, ])?;
	let products = emitter.intermediate(DType::F32, table_shape)?; emitter.emit(
		vec![gathered_values, gathered_operand, active_mask.id], vec![products], PrimitiveKind::Elementwise(Elementwise {
			program: masked_product_program(request.descriptor.id)?, }), )?;
	let row_sums = emitter.intermediate(DType::F32, shape(request, &[output_elements])?)?; emitter.emit_stage([
		KernelEmission { inputs: vec![products], outputs: vec![row_sums], kind: reduction( request.descriptor.id,
				ReduceOperator::Sum, &[1], false, ReduceResult::Value, tree_lanes, )?, }, KernelEmission {
			inputs: vec![output_base.id, output_indices.id, row_sums], outputs: vec![product.id],
...
fn emit_neighbor_aggregate(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	] { require_dtype(request, tensor, DType::F32, name)?; require_shape(request, tensor, expected, name)?; }
	for (name, tensor, expected) in [ (
			"feature_indices",
			feature_indices, &[output_elements, row_width][..], ), (
			"active_mask",
			active_mask, &[output_elements, row_width][..], ),
		("degree_indices", degree_indices, &[output_elements][..]),
		("output_indices", output_indices, &[output_elements][..]),
	] { require_dtype(request, tensor, DType::I32, name)?; require_shape(request, tensor, expected, name)?; }
	...
	let gathered_features = emitter.intermediate(DType::F32, feature_table_shape.clone())?; emitter.emit_stage([
		KernelEmission { inputs: vec![degrees.id, degree_indices.id], outputs: vec![gathered_degrees], kind: gather(), },
		KernelEmission { inputs: vec![features.id, feature_indices.id], outputs: vec![gathered_features], kind: gather(), },
	])?; let contributions = emitter.intermediate(DType::F32, feature_table_shape)?; emitter.emit(
		vec![gathered_features, active_mask.id], vec![contributions], PrimitiveKind::Elementwise(Elementwise {
			program: masked_value_program(request.descriptor.id)?, }), )?;
	let sums = emitter.intermediate(DType::F32, output_shape.clone())?;
	let normalized = emitter.intermediate(DType::F32, output_shape)?; emitter.emit_stage([ KernelEmission {
			inputs: vec![contributions], outputs: vec![sums], kind: reduction( request.descriptor.id, ReduceOperator::Sum, &[1],
				false, ReduceResult::Value, tree_lanes, )?, }, KernelEmission { inputs: vec![sums, gathered_degrees],
			outputs: vec![normalized], kind: PrimitiveKind::Elementwise(Elementwise {
				program: neighbor_normalize_program(request.descriptor.id, mean)?, }), }, KernelEmission {
			inputs: vec![output_base.id, output_indices.id, normalized], outputs: vec![aggregate.id],
			kind: scatter(ScatterConflict::UniqueIndices), }, ]) }
...
fn emit_centroid_update(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let centroid_elements = checked_product( request, &[centroid_count, dimensions],
	...
	for (name, tensor) in [
		...
		("active_mask", active_mask),
	] { require_dtype(request, tensor, DType::I32, name)?; require_shape( request, tensor,
			&[centroid_elements, points_per_centroid], name, )?; }
	...
	] { require_dtype(request, tensor, DType::I32, name)?; require_shape(request, tensor, &[centroid_elements], name)?; }
	for (name, tensor) in [("centroid_base", centroid_base), ("centroids", centroids)] {
		require_dtype(request, tensor, DType::F32, name)?; require_shape(request, tensor, &[centroid_elements], name)?; }
	...
	let gathered_points = emitter.intermediate(DType::F32, table_shape.clone())?; emitter.emit(
		vec![points.id, point_indices.id], vec![gathered_points], gather(), )?;
	let contributions = emitter.intermediate(DType::F32, table_shape)?; emitter.emit(
		vec![gathered_points, active_mask.id], vec![contributions], PrimitiveKind::Elementwise(Elementwise {
			program: masked_value_program(request.descriptor.id)?, }), )?;
	...
	let normalized = emitter.intermediate(DType::F32, flat_shape)?; let bins = u32::try_from(centroid_count)
		.map_err(|conversion_error| request_error(request, conversion_error.to_string()))?; emitter.emit_stage([
		KernelEmission { inputs: vec![contributions], outputs: vec![sums], kind: reduction( request.descriptor.id,
				ReduceOperator::Sum, &[1], false, ReduceResult::Value, tree_lanes, )?, }, KernelEmission {
			inputs: vec![assignments.id], outputs: vec![counts.id], kind: PrimitiveKind::Histogram(Histogram { bins,
				weighted: false, ordering: AtomicOrdering::Relaxed, }), }, KernelEmission {
			inputs: vec![counts.id, count_indices.id], outputs: vec![divisors], kind: gather(), }, KernelEmission {
			inputs: vec![sums, divisors], outputs: vec![normalized], kind: PrimitiveKind::Elementwise(Elementwise {
				program: centroid_normalize_program(request.descriptor.id)?, }), }, KernelEmission {
			inputs: vec![centroid_base.id, centroid_indices.id, normalized], outputs: vec![centroids.id],
			kind: scatter(ScatterConflict::UniqueIndices), }, ]) }
...
fn emit_core_distance(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	require_supported( request, points_count == 2 && minimum_points == 1,
	...
	] { require_dtype(request, tensor, DType::I32, name)?; require_shape(request, tensor, &[2, dimensions], name)?; }
	...
	let pair_shape = shape(request, &[2, dimensions])?; let left = emitter.intermediate(DType::F32, pair_shape.clone())?;
	...
	let distances = emitter.intermediate(DType::F32, shape(request, &[2, 1])?)?; emitter.emit_stage([ KernelEmission {
			inputs: vec![points.id, left_indices.id], outputs: vec![left], kind: gather(), }, KernelEmission {
			inputs: vec![points.id, right_indices.id], outputs: vec![right], kind: gather(), }, KernelEmission {
			inputs: vec![left, right], outputs: vec![squared_differences], kind: PrimitiveKind::Elementwise(Elementwise {
				program: squared_difference_program(request.descriptor.id)?, }), }, KernelEmission {
			inputs: vec![squared_differences], outputs: vec![squared_distances], kind: reduction( request.descriptor.id,
				ReduceOperator::Sum, &[1], true, ReduceResult::Value, tree_lanes, )?, }, KernelEmission {
			inputs: vec![squared_distances], outputs: vec![distances], kind: PrimitiveKind::Elementwise(Elementwise {
				program: nonnegative_square_root_program(request.descriptor.id)?, }), }, ])?;
	let sorted = emitter.intermediate(DType::F32, shape(request, &[2, 1])?)?; emitter.emit( vec![distances], vec![sorted],
		PrimitiveKind::Sort(Sort { axis: 1, direction: SortDirection::Ascending, stable: true, emit_indices: false, }), )?;
	emitter.emit( vec![sorted, rank_indices.id], vec![core_distances.id], PrimitiveKind::Gather(Gather { axis: 1,
...
fn emit_pairwise_l2(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let training_squared = emitter.intermediate(DType::F32, training.shape.clone())?; emitter.emit_stage([ KernelEmission {
			inputs: vec![query.id], outputs: vec![query_squared], kind: PrimitiveKind::Elementwise(Elementwise {
				program: square_program(request.descriptor.id)?, }), }, KernelEmission { inputs: vec![training.id],
			outputs: vec![training_squared], kind: PrimitiveKind::Elementwise(Elementwise {
				program: square_program(request.descriptor.id)?, }), }, ])?;
	let query_norms = emitter.intermediate(DType::F32, shape(request, &[queries, 1])?)?;
	let training_norms = emitter.intermediate(DType::F32, shape(request, &[training_rows])?)?; emitter.emit_stage([
		KernelEmission { inputs: vec![query_squared], outputs: vec![query_norms], kind: reduction( request.descriptor.id,
				ReduceOperator::Sum, &[1], true, ReduceResult::Value, tree_lanes, )?, }, KernelEmission {
			inputs: vec![training_squared], outputs: vec![training_norms], kind: reduction( request.descriptor.id,
				ReduceOperator::Sum, &[1], false, ReduceResult::Value, tree_lanes, )?, }, ])?;
	let products = emitter.intermediate(DType::F32, distances.shape.clone())?; emitter.emit( vec![query.id, training.id],
		vec![products], PrimitiveKind::Contraction(Contraction { batch_axes: Vec::new(), contract_axes: vec![(1, 1)], }), )?;
	emitter.emit( vec![query_norms, training_norms, products], vec![distances.id], PrimitiveKind::Elementwise(Elementwise {
...
fn emit_fixed_radius_singleton(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let offsets = emitter.intermediate(DType::I32, counts.shape.clone())?; emitter.emit( vec![counts.id], vec![offsets],
		PrimitiveKind::Scan(Scan { operator: ReduceOperator::Sum, axis: 0, mode: ScanMode::Exclusive {
				identity: ScalarLiteral::I32(0), }, reverse: false, tree_lanes, }), )?;
	let mapped_offsets = emitter.intermediate(DType::I32, counts.shape.clone())?; emitter.emit( vec![offsets],
		vec![mapped_offsets], PrimitiveKind::Elementwise(Elementwise {
			program: identity_program(request.descriptor.id, DType::I32)?, }), )?; emitter.emit_stage([ KernelEmission {
			inputs: vec![ row_pointer_base.id, row_pointer_indices.id, row_pointer_updates.id, ], outputs: vec![row_pointers.id],
			kind: scatter(ScatterConflict::UniqueIndices), }, KernelEmission {
			inputs: vec![neighbor_base.id, neighbor_indices.id, mapped_offsets], outputs: vec![neighbors.id],
			kind: scatter(ScatterConflict::UniqueIndices), }, ]) }
...
fn emit_union_find_two_nodes(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let nodes = prepared_dimension(request, "nodes")?;
	require_supported( request, nodes == 2,
	...
	let destination_roots = emitter.intermediate(DType::I32, edge_sources.shape.clone())?; emitter.emit_stage([
		KernelEmission { inputs: vec![parent_base.id, edge_sources.id], outputs: vec![source_roots], kind: gather(), },
		KernelEmission { inputs: vec![parent_base.id, edge_destinations.id], outputs: vec![destination_roots], kind: gather(),
		}, ])?; let hook_destinations = emitter.intermediate(DType::I32, edge_sources.shape.clone())?;
	let hook_updates = emitter.intermediate(DType::I32, edge_sources.shape.clone())?; emitter.emit(
		vec![source_roots, destination_roots], vec![hook_destinations, hook_updates], PrimitiveKind::Elementwise(Elementwise {
	...
	let compressed = emitter.intermediate(DType::I32, parent_base.shape.clone())?; emitter.emit_stage([ KernelEmission {
			inputs: vec![parent_base.id, hook_destinations, hook_updates], outputs: vec![hooked],
			kind: scatter(ScatterConflict::Atomic { operation: AtomicOperation::Minimum,
				ordering: AtomicOrdering::SequentiallyConsistent, }), }, KernelEmission { inputs: vec![hooked, node_indices.id],
			outputs: vec![compressed], kind: gather(), }, KernelEmission {
			inputs: vec![labels_base.id, node_indices.id, compressed], outputs: vec![labels.id],
			kind: scatter(ScatterConflict::UniqueIndices), }, ]) }
...
fn emit_boruvka(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let gathered_weights = emitter.intermediate(DType::F32, edge_weights.shape.clone())?; emitter.emit_stage([
		KernelEmission { inputs: vec![parent.id, edge_sources.id], outputs: vec![source_components], kind: gather(), },
		KernelEmission { inputs: vec![parent.id, edge_destinations.id], outputs: vec![destination_components], kind: gather(),
		}, KernelEmission { inputs: vec![edge_weights.id, edge_indices.id], outputs: vec![gathered_weights], kind: gather(),
		}, ])?; let minimum_weight = emitter.intermediate(DType::F32, shape(request, &[1])?)?;
	let minimum_edge = emitter.intermediate(DType::I32, shape(request, &[1])?)?; emitter.emit( vec![gathered_weights],
		vec![minimum_weight, minimum_edge], reduction( request.descriptor.id, ReduceOperator::Minimum, &[0], false,
			ReduceResult::ValueAndIndex, tree_lanes, )?, )?; emitter.emit_stage([ KernelEmission { inputs: vec![minimum_weight],
			outputs: vec![total_weight.id], kind: PrimitiveKind::Elementwise(Elementwise {
				program: identity_program(request.descriptor.id, DType::F32)?, }), }, KernelEmission {
			inputs: vec![mask_base.id, minimum_edge, unit_update.id], outputs: vec![in_mst.id],
			kind: scatter(ScatterConflict::UniqueIndices), }, ]) }

fn gather() -> PrimitiveKind { PrimitiveKind::Gather(Gather { axis: 0, bounds: IndexBounds::Reject, }) }

fn scatter(conflict: ScatterConflict) -> PrimitiveKind { PrimitiveKind::Scatter(Scatter { axis: 0,
		bounds: IndexBounds::Reject, conflict, }) }

fn reduction( operation: OperationId, operator: ReduceOperator, axes: &[usize], keep_dimensions: bool,
	result: ReduceResult, tree_lanes: u32, ) -> OperationResult<PrimitiveKind> {
	let axes = AxisSet::new(axes.to_vec()).map_err(|error| language_error(operation, error.to_string()))?;
	Ok(PrimitiveKind::Reduce(Reduce { operator, axes, keep_dimensions, result, tree_lanes, })) }
...
fn prepared_dimension(request: &MaterializationRequest<'_>, name: &str) -> OperationResult<u64> {
	let value = prepared_u64(request.descriptor.id, request.parameters, name)?; match value > 0 && value <= MAX_I32_INDEX {
		true => Ok(value), false => { Err(unsupported( request,
...
fn prepared_count(request: &MaterializationRequest<'_>, name: &str) -> OperationResult<u64> {
	let value = prepared_u64(request.descriptor.id, request.parameters, name)?; match value <= MAX_I32_INDEX {
		true => Ok(value), false => { Err(unsupported( request,
...
fn checked_product(request: &MaterializationRequest<'_>, factors: &[u64], role: &str) -> OperationResult<u64> {
	let product = factors.iter().try_fold(1_u64, |product, factor| { product.checked_mul(*factor).ok_or_else(|| {
			...
			) }) })?; match product <= MAX_I32_INDEX { true => Ok(product), false => { Err(unsupported( request,
...
fn finite_parameter(request: &MaterializationRequest<'_>, name: &str) -> OperationResult<f32> {
	let value = prepared_f32(request, name)?; match value.is_finite() { true => Ok(value), false => { Err(request_error(
				request,
				format!("{name} must be a finite f32"),
			)) } } }

fn prepared_bool(request: &MaterializationRequest<'_>, name: &str) -> OperationResult<bool> {
	match request.parameters.get(name) { Some(super::PreparedParameter::Bool(value)) => Ok(*value), Some(value) => {
...
fn require_iteration_input(request: &MaterializationRequest<'_>, expected: &str) -> OperationResult<()> {
	match request.iteration_shape_input == expected { true => Ok(()), false => { Err(request_error( request,
...
fn unsupported(request: &MaterializationRequest<'_>, detail: impl Into<String>) -> crate::OperationError {
	operation_error( request.descriptor.id, OperationErrorKind::UnsupportedConcreteShape, detail, ) }

fn require_supported(
	request: &MaterializationRequest<'_>,
	condition: bool,
	detail: &'static str,
) -> OperationResult<()> { match condition { true => Ok(()), false => Err(unsupported(request, detail)), } }

fn require_request_fact(
	request: &MaterializationRequest<'_>,
	condition: bool,
	detail: &'static str,
) -> OperationResult<()> { match condition { true => Ok(()), false => Err(request_error(request, detail)), } }

fn bool_mask( operation: OperationId, builder: &mut recipe_language::ScalarProgramBuilder,
	mask: recipe_language::ScalarExpression, ) -> OperationResult<recipe_language::ScalarExpression> { let zero = builder
		.i32(0) .map_err(|error| language_error(operation, error.to_string()))?; let one = builder .i32(1)
	...
	scalar_unary(operation, builder, ScalarOpcode::Require, valid)?; Ok(mask) }

fn temporal_difference_program(operation: OperationId, gamma: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?; let reward = scalar_input(operation, &mut builder, DType::F32)?;
	let next_value = scalar_input(operation, &mut builder, DType::F32)?;
	let done = scalar_input(operation, &mut builder, DType::I32)?; let done = bool_mask(operation, &mut builder, done)?;
	let done = scalar_unary(operation, &mut builder, ScalarOpcode::ConvertI32ToF32, done)?;
	let one = scalar_f32(operation, &mut builder, 1.0)?; let gamma = scalar_f32(operation, &mut builder, gamma)?;
	let live = scalar_binary(operation, &mut builder, ScalarOpcode::Subtract, one, done)?; let discounted = scalar_binary(
		operation, &mut builder, ScalarOpcode::Multiply, next_value, gamma, )?; let continuation = scalar_binary( operation,
		&mut builder, ScalarOpcode::Multiply, discounted, live, )?; let target = scalar_binary( operation, &mut builder,
		ScalarOpcode::Add, reward, continuation, )?; scalar_finish(operation, builder, &[target]) }
...
fn gcn_normalization_program(operation: OperationId) -> OperationResult<recipe_core::ScalarProgram> {
	...
	let degree = scalar_input(operation, &mut builder, DType::F32)?; let zero = scalar_f32(operation, &mut builder, 0.0)?;
	let nonnegative = scalar_binary( operation, &mut builder, ScalarOpcode::GreaterThanOrEqual, degree, zero, )?;
	scalar_unary(operation, &mut builder, ScalarOpcode::Require, nonnegative)?; let positive = scalar_binary( operation,
		&mut builder, ScalarOpcode::GreaterThan, degree, zero, )?;
	...
	let scaled = scalar_binary( operation, &mut builder, ScalarOpcode::Multiply, feature, inverse_root, )?;
	let normalized = scalar_ternary( operation, &mut builder, ScalarOpcode::Select, positive, scaled, feature, )?;
...
fn advantage_delta_program(operation: OperationId, gamma: f32) -> OperationResult<recipe_core::ScalarProgram> {
	...
	let has_next = bool_mask(operation, &mut builder, has_next)?; let zero = scalar_f32(operation, &mut builder, 0.0)?;
	let gamma = scalar_f32(operation, &mut builder, gamma)?; let next = scalar_ternary( operation, &mut builder,
		ScalarOpcode::Select, has_next, next_value, zero, )?;
	let discounted = scalar_binary(operation, &mut builder, ScalarOpcode::Multiply, gamma, next)?;
	let reward_plus_next = scalar_binary( operation, &mut builder, ScalarOpcode::Add, reward, discounted, )?;
	let delta = scalar_binary( operation, &mut builder, ScalarOpcode::Subtract, reward_plus_next, value, )?;
...
fn gaussian_term_program(operation: OperationId) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?; let mean = scalar_input(operation, &mut builder, DType::F32)?;
	...
	let standard_deviation = scalar_input(operation, &mut builder, DType::F32)?; let difference = scalar_binary( operation,
		&mut builder, ScalarOpcode::Subtract, action, mean, )?; let z = scalar_binary( operation, &mut builder,
		ScalarOpcode::Divide, difference, standard_deviation, )?;
	let negative_half = scalar_f32(operation, &mut builder, -0.5)?; let negative_half_z = scalar_binary( operation,
		&mut builder, ScalarOpcode::Multiply, negative_half, z, )?; let quadratic = scalar_binary( operation, &mut builder,
		ScalarOpcode::Multiply, negative_half_z, z, )?; let without_scale = scalar_binary( operation, &mut builder,
		ScalarOpcode::Subtract, quadratic, log_standard_deviation, )?;
	let half_log_two_pi = scalar_f32(operation, &mut builder, 0.918_938_5)?; let term = scalar_binary( operation,
		&mut builder, ScalarOpcode::Subtract, without_scale, half_log_two_pi, )?; scalar_finish(operation, builder, &[term]) }
...
fn checked_flat_action_program( operation: OperationId, action_count: u64,
) -> OperationResult<recipe_core::ScalarProgram> { let action_count = i32::try_from(action_count).map_err(|error| {
		operation_error( operation, OperationErrorKind::InvalidMaterializationRequest, error.to_string(), ) })?;
	let mut builder = scalar_builder(operation)?; let row_base = scalar_input(operation, &mut builder, DType::I32)?;
	let action = scalar_input(operation, &mut builder, DType::I32)?; let zero = builder .i32(0)
		.map_err(|error| language_error(operation, error.to_string()))?; let limit = builder .i32(action_count)
		.map_err(|error| language_error(operation, error.to_string()))?; let nonnegative = scalar_binary( operation,
		&mut builder, ScalarOpcode::GreaterThanOrEqual, action, zero, )?; let below_limit = scalar_binary( operation,
		&mut builder, ScalarOpcode::LessThan, action, limit, )?; let valid = scalar_binary( operation, &mut builder,
		ScalarOpcode::BitAnd, nonnegative, below_limit, )?;
...
fn categorical_finish_program(operation: OperationId) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?; let selected = scalar_input(operation, &mut builder, DType::F32)?;
	...
	let log_probability = scalar_binary( operation, &mut builder, ScalarOpcode::Subtract, selected, log_normalizer, )?;
...
fn masked_product_program(operation: OperationId) -> OperationResult<recipe_core::ScalarProgram> {
	...
	let zero = scalar_f32(operation, &mut builder, 0.0)?; let masked = scalar_ternary( operation, &mut builder,
		ScalarOpcode::Select, active, product, zero, )?; scalar_finish(operation, builder, &[masked]) }

fn neighbor_normalize_program(operation: OperationId, mean: bool) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?; let sum = scalar_input(operation, &mut builder, DType::F32)?;
	let degree = scalar_input(operation, &mut builder, DType::F32)?; let zero = scalar_f32(operation, &mut builder, 0.0)?;
	let normalized = match mean { true => { let nonnegative = scalar_binary( operation, &mut builder,
				ScalarOpcode::GreaterThanOrEqual, degree, zero, )?;
			scalar_unary(operation, &mut builder, ScalarOpcode::Require, nonnegative)?; let positive = scalar_binary( operation,
				&mut builder, ScalarOpcode::GreaterThan, degree, zero, )?;
			let quotient = scalar_binary(operation, &mut builder, ScalarOpcode::Divide, sum, degree)?; scalar_ternary( operation,
				&mut builder, ScalarOpcode::Select, positive, quotient, sum, )? }
		false => sum, }; scalar_finish(operation, builder, &[normalized]) }
...
fn masked_value_program(operation: OperationId) -> OperationResult<recipe_core::ScalarProgram> {
	...
	let active = scalar_input(operation, &mut builder, DType::I32)?;
	let active = bool_mask(operation, &mut builder, active)?; let zero = scalar_f32(operation, &mut builder, 0.0)?;
	let value = scalar_ternary( operation, &mut builder, ScalarOpcode::Select, active, value, zero, )?;
...
fn centroid_normalize_program(operation: OperationId) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?; let sum = scalar_input(operation, &mut builder, DType::F32)?;
	let count = scalar_input(operation, &mut builder, DType::I32)?; let zero_i32 = builder .i32(0)
		.map_err(|error| language_error(operation, error.to_string()))?; let nonnegative = scalar_binary( operation,
		&mut builder, ScalarOpcode::GreaterThanOrEqual, count, zero_i32, )?;
	scalar_unary(operation, &mut builder, ScalarOpcode::Require, nonnegative)?; let positive = scalar_binary( operation,
		&mut builder, ScalarOpcode::GreaterThan, count, zero_i32, )?; let count = scalar_unary( operation, &mut builder,
		ScalarOpcode::ConvertI32ToF32, count, )?;
	let quotient = scalar_binary(operation, &mut builder, ScalarOpcode::Divide, sum, count)?;
	let zero = scalar_f32(operation, &mut builder, 0.0)?; let centroid = scalar_ternary( operation, &mut builder,
		ScalarOpcode::Select, positive, quotient, zero, )?; scalar_finish(operation, builder, &[centroid]) }
...
fn squared_difference_program(operation: OperationId) -> OperationResult<recipe_core::ScalarProgram> {
	...
	let squared = scalar_binary( operation, &mut builder, ScalarOpcode::Multiply, difference, difference, )?;
...
fn nonnegative_square_root_program(operation: OperationId) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?; let value = scalar_input(operation, &mut builder, DType::F32)?;
	let zero = scalar_f32(operation, &mut builder, 0.0)?; let valid = scalar_binary( operation, &mut builder,
		ScalarOpcode::GreaterThanOrEqual, value, zero, )?;
	scalar_unary(operation, &mut builder, ScalarOpcode::Require, valid)?;
...
fn pairwise_l2_result_program(operation: OperationId) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?; let query_norm = scalar_input(operation, &mut builder, DType::F32)?;
	let training_norm = scalar_input(operation, &mut builder, DType::F32)?;
	let product = scalar_input(operation, &mut builder, DType::F32)?; let two = scalar_f32(operation, &mut builder, 2.0)?;
	let twice_product = scalar_binary( operation, &mut builder, ScalarOpcode::Multiply, product, two, )?;
	let norm_sum = scalar_binary( operation, &mut builder, ScalarOpcode::Add, query_norm, training_norm, )?;
	let squared_distance = scalar_binary( operation, &mut builder, ScalarOpcode::Subtract, norm_sum, twice_product, )?;
	let zero = scalar_f32(operation, &mut builder, 0.0)?; let nonnegative = scalar_binary( operation, &mut builder,
		ScalarOpcode::Maximum, squared_distance, zero, )?; let distance = scalar_unary( operation, &mut builder,
		ScalarOpcode::SquareRoot, nonnegative, )?; scalar_finish(operation, builder, &[distance]) }

fn minimum_representative_hook_program(operation: OperationId) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?; let source = scalar_input(operation, &mut builder, DType::I32)?;
	let destination = scalar_input(operation, &mut builder, DType::I32)?; let hook_destination = scalar_binary( operation,
		&mut builder, ScalarOpcode::Maximum, source, destination, )?; let hook_update = scalar_binary( operation,
		&mut builder, ScalarOpcode::Minimum, source, destination, )?;
