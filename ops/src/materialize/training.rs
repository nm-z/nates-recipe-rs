...
use super::{Emitter, FamilyDispatch, MaterializationRequest, input as requested_input, language_error, output, prepared_tree_lanes, request_error, require_dtype, require_exact_abi, require_shape, scalar_binary, scalar_builder, scalar_finish, scalar_input};
...
fn emit_linear_backward_weights_only(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	require_exact_abi(
		request,
...
fn emit_weight_gradient(_request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, input: &Tensor, output_gradient: &Tensor, weight_gradient: &Tensor) -> OperationResult<()> {
	emitter.emit(
...
fn emit_bias_gradient(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, output_gradient: &Tensor, bias_gradient: &Tensor, tree_lanes: u32) -> OperationResult<()> {
...
fn emit_binary_cross_entropy_with_logits(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	require_exact_abi(
		request,
		&["logits", "targets"],
		&["losses", "gradients"],
	...
	let program = crate::scalar::binary_cross_entropy_with_logits_program().map_err(|error| error.for_operation(request.descriptor.id))?;
	emitter.emit(
...
fn require_f32_shape(request: &MaterializationRequest<'_>, tensor: &Tensor, shape: &[u64], name: &str) -> OperationResult<()> {
	require_dtype(request, tensor, DType::F32, name)?;
