...
use super::{ Emitter, FamilyDispatch, MaterializationRequest, input as requested_input, language_error, output,
	prepared_tree_lanes, request_error, require_dtype, require_exact_abi, require_shape, scalar_binary,
	scalar_builder, scalar_finish, scalar_input, }; use crate::{OperationDescriptor, OperationId, OperationResult};
...
fn emit_linear(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	if weight_inner != inner { return Err(request_error( request,
	...
	let contracted = emitter.intermediate(DType::F32, output.shape.clone())?; emitter.emit( vec![input.id, weight.id],
		vec![contracted], PrimitiveKind::Contraction(Contraction { batch_axes: Vec::new(), contract_axes: vec![(1, 0)], }),
	)?; emitter.emit( vec![contracted, bias.id], vec![output.id], PrimitiveKind::Elementwise(Elementwise {
...
fn emit_linear_backward_full(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	if input_rows != rows { return Err(request_error( request,
	...
	require_f32_shape( request, weight_gradient, &[inner, columns],
		"weight_gradient",
	)?;
	require_f32_shape(request, bias_gradient, &[columns], "bias_gradient")?;
	let tree_lanes = prepared_tree_lanes(request)?;

	emitter.emit( vec![output_gradient.id, weight.id], vec![input_gradient.id], PrimitiveKind::Contraction(Contraction {
			batch_axes: Vec::new(), contract_axes: vec![(1, 1)], }), )?;
...
fn emit_weight_gradient(
	...
	emitter: &mut Emitter<'_>,
	input: &Tensor, output_gradient: &Tensor, weight_gradient: &Tensor, ) -> OperationResult<()> { emitter.emit(
		vec![input.id, output_gradient.id], vec![weight_gradient.id], PrimitiveKind::Contraction(Contraction {
			batch_axes: Vec::new(), contract_axes: vec![(0, 0)], }), ) }

fn emit_bias_gradient(
	request: &MaterializationRequest<'_>,
	emitter: &mut Emitter<'_>,
	output_gradient: &Tensor, bias_gradient: &Tensor, tree_lanes: u32, ) -> OperationResult<()> {
	let axes = AxisSet::new(vec![0]).map_err(|error| language_error(request.descriptor.id, error.to_string()))?;
	emitter.emit( vec![output_gradient.id], vec![bias_gradient.id], PrimitiveKind::Reduce(Reduce {
			operator: ReduceOperator::Sum, axes, keep_dimensions: false, result: ReduceResult::Value, tree_lanes, }), ) }
...
fn emit_binary_cross_entropy_with_logits(
	...
	let program = crate::scalar::binary_cross_entropy_with_logits_program()
		.map_err(|error| error.for_operation(request.descriptor.id))?; emitter.emit( vec![logits.id, targets.id],
		vec![losses.id, gradients.id], PrimitiveKind::Elementwise(Elementwise { program }), ) }

fn require_iteration_input(request: &MaterializationRequest<'_>, expected: &str) -> OperationResult<()> {
	if request.iteration_shape_input == expected { Ok(()) } else { Err(request_error( request, format!(
				"iteration shape input must be {expected:?}, got {:?}",
				request.iteration_shape_input ), )) } }

fn require_f32_matrix(
	request: &MaterializationRequest<'_>,
	tensor: &Tensor, name: &str, ) -> OperationResult<(u64, u64)> { require_dtype(request, tensor, DType::F32, name)?;
	match tensor.shape.extents() { [rows, columns] => Ok((*rows, *columns)), shape => { Err(request_error( request,
				format!("{name} has shape {shape:?}, expected a rank-two f32 matrix"),
			)) } } }

fn require_f32_shape(
	request: &MaterializationRequest<'_>,
	tensor: &Tensor, shape: &[u64], name: &str, ) -> OperationResult<()> {
