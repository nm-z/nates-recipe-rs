...
use recipe_language::{AtomicOperation, AtomicOrdering, AxisSet, Elementwise, Gather, Histogram, IndexBounds, PrimitiveKind, RandomDistribution, RandomKey, RandomMap, Reduce, ReduceOperator, ReduceResult, Scan, ScanMode, Scatter, ScatterConflict, Shape, Sort, SortDirection, Tensor};

use super::{Emitter, FamilyDispatch, KernelEmission, MAX_I32_INDEX, MaterializationRequest, emit_tree_leaf_split, identity_program, input, language_error, operation_error, output, prepared_f32, prepared_tree_lanes, prepared_u64, request_error, require_dtype, require_exact_abi, require_shape, require_true, scalar_binary, scalar_builder, scalar_f32, scalar_finish, scalar_input, scalar_ternary, scalar_unary};
	...
	match supports(request.descriptor) {
		...
		true => {
			let result = match request.descriptor.symbol {
				...
				"gpu_lgbm_best_split" | "gpu_oblivious_split_eval" | "gpu_split_eval" | "gpu_tb_split_eval" => emit_two_bin_split(request, emitter),
				...
				"gpu_scatter_add_by_leaf" | "gpu_scatter_add_by_leaf_col" => emit_leaf_prediction(request, emitter),
...
fn emit_bootstrap_sample(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let high_exclusive = i32::try_from(rows).map_err(|conversion_error| request_error(request, conversion_error.to_string()))?;
...
fn emit_tree_histogram(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, include_counts: bool) -> OperationResult<()> {
	...
	let histogram_bins = u32::try_from(bins).map_err(|conversion_error| request_error(request, conversion_error.to_string()))?;
...
fn emit_regularized_leaf(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, expose_sums: bool) -> OperationResult<()> {
...
fn emit_ordered_target_statistics(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	require_exact_abi(
		request,
...
fn reduction(request: &MaterializationRequest<'_>, operator: ReduceOperator, axes: &[usize], keep_dimensions: bool, result: ReduceResult, tree_lanes: u32) -> OperationResult<PrimitiveKind> {
	let axes = AxisSet::new(axes.to_vec()).map_err(|error| language_error(request.descriptor.id, error.to_string()))?;
...
fn require_scalar(request: &MaterializationRequest<'_>, tensor: &Tensor, dtype: DType, name: &str) -> OperationResult<()> {
...
fn regularized_leaf_program(operation: OperationId, lambda: f32, expose_sums: bool) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
	let gradient = scalar_input(operation, &mut builder, DType::F32)?;
	let hessian = scalar_input(operation, &mut builder, DType::F32)?;
...
fn two_bin_gain_program(operation: OperationId, lambda: f32, minimum_child_weight: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
...
fn ordered_statistic_program(operation: OperationId, prior: f32, smoothing: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
