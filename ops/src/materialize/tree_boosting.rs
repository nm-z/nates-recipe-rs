...
use super::{ Emitter, FamilyDispatch, KernelEmission, MAX_I32_INDEX, MaterializationRequest, emit_tree_leaf_split,
...
fn emit_argmax_split(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let best_index = emitter.intermediate(DType::I32, scalar_shape(request)?)?; emitter.emit( vec![gains.id],
		vec![best_gain.id, best_index], reduction( request, ReduceOperator::Maximum, &[0, 1], false,
			ReduceResult::ValueAndIndex, tree_lanes, )?, )?; emitter.emit( vec![best_index], vec![best_feature.id, best_bin.id],
		PrimitiveKind::Elementwise(Elementwise { program: split_index_program(request.descriptor.id, bins)?, }), ) }
...
fn emit_bootstrap_sample(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let random_indices = emitter.intermediate(DType::I32, sampled_rows.shape.clone())?; emitter.emit( Vec::new(),
		vec![random_indices], PrimitiveKind::Random(RandomMap { distribution: RandomDistribution::UniformI32 { low: 0,
				high_exclusive, }, key, philox_rounds: 10, }), )?; emitter.emit( vec![row_ids.id, random_indices],
		vec![sampled_rows.id], gather(), ) }
...
fn emit_feature_subset(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	require_supported( request, selected <= features,
	...
	let gathered = emitter.intermediate(DType::I32, selected_features.shape.clone())?; emitter.emit(
		vec![feature_ids.id, selection_indices.id], vec![gathered], gather(), )?; emitter.emit( vec![gathered],
		vec![selected_features.id], PrimitiveKind::Elementwise(Elementwise {
...
fn emit_gradient_hessian(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	for (name, tensor) in [
		...
		("gradients", gradients),
		("hessians", hessians),
	] { require_dtype(request, tensor, DType::F32, name)?; require_shape(request, tensor, &[rows], name)?; }
	...
	emitter.emit( vec![probabilities.id, targets.id, weights.id, mask.id], vec![gradients.id, hessians.id],
		PrimitiveKind::Elementwise(Elementwise { program: gradient_hessian_program(request.descriptor.id, class_index)?, }), )
}

fn emit_tree_histogram(
	request: &MaterializationRequest<'_>,
	emitter: &mut Emitter<'_>,
	include_counts: bool, ) -> OperationResult<()> { let output_names = match include_counts {
	...
	] { require_dtype(request, tensor, DType::F32, name)?; require_shape(request, tensor, &[contributions], name)?; }
	for (name, tensor) in [
		("gradient_histogram", gradient_histogram),
		("hessian_histogram", hessian_histogram),
	] { require_dtype(request, tensor, DType::F32, name)?; require_shape(request, tensor, &[bins], name)?; }
	...
	let mapped_hessians = emitter.intermediate(DType::F32, hessians.shape.clone())?; emitter.emit(
		vec![bin_indices.id, gradients.id, hessians.id], vec![mapped_bins, mapped_gradients, mapped_hessians],
		PrimitiveKind::Elementwise(Elementwise { program: histogram_contribution_program(request.descriptor.id)?, }), )?;
	let histogram_bins =
		u32::try_from(bins).map_err(|conversion_error| request_error(request, conversion_error.to_string()))?;
	let weighted = |weights, destination| { KernelEmission { inputs: vec![mapped_bins, weights],
			outputs: vec![destination], kind: PrimitiveKind::Histogram(Histogram { bins: histogram_bins, weighted: true,
				ordering: AtomicOrdering::SequentiallyConsistent, }), } }; let mut kernels = vec![
		weighted(mapped_gradients, gradient_histogram.id), weighted(mapped_hessians, hessian_histogram.id), ];
	kernels.extend(count_histogram.map(|tensor| { KernelEmission { inputs: vec![mapped_bins], outputs: vec![tensor.id],
			kind: PrimitiveKind::Histogram(Histogram { bins: histogram_bins, weighted: false,
				ordering: AtomicOrdering::SequentiallyConsistent, }), } })); emitter.emit_stage(kernels) }

fn emit_regularized_leaf(
	request: &MaterializationRequest<'_>,
	emitter: &mut Emitter<'_>,
	expose_sums: bool, ) -> OperationResult<()> { let output_names = match expose_sums {
	...
	let sum_outputs = match expose_sums { true => { Some((
	...
	require_supported( request, leaves == 1,
	...
	sum_outputs .into_iter() .try_for_each(|(leaf_gradient, leaf_hessian)| {
	...
	let histogram = |weights, destination| { KernelEmission { inputs: vec![leaf_indices.id, weights],
			outputs: vec![destination], kind: PrimitiveKind::Histogram(Histogram { bins: 1, weighted: true,
				ordering: AtomicOrdering::SequentiallyConsistent, }), } }; emitter.emit_stage([
		histogram(gradients.id, gradient_histogram), histogram(hessians.id, hessian_histogram), ])?;

	let reduced_gradient = emitter.intermediate(DType::F32, scalar_shape(request)?)?;
	let reduced_hessian = emitter.intermediate(DType::F32, scalar_shape(request)?)?; emitter.emit_stage([ KernelEmission {
			inputs: vec![gradient_histogram], outputs: vec![reduced_gradient], kind: reduction( request, ReduceOperator::Sum,
				&[0], false, ReduceResult::Value, tree_lanes, )?, }, KernelEmission { inputs: vec![hessian_histogram],
			outputs: vec![reduced_hessian], kind: reduction( request, ReduceOperator::Sum, &[0], false, ReduceResult::Value,
				tree_lanes, )?, }, ])?; let mut outputs = match sum_outputs {
		Some((gradient, hessian)) => vec![gradient.id, hessian.id], None => Vec::with_capacity(1), };
	outputs.push(leaf_value.id); emitter.emit( vec![reduced_gradient, reduced_hessian], outputs,
...
fn emit_leaf_reduce(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	] { require_dtype(request, tensor, DType::F32, name)?; require_shape(request, tensor, &[leaves], name)?; }

	let mapped_indices = emitter.intermediate(DType::I32, leaf_indices.shape.clone())?;
	let mapped_gradients = emitter.intermediate(DType::F32, gradients.shape.clone())?;
	let mapped_hessians = emitter.intermediate(DType::F32, hessians.shape.clone())?; emitter.emit(
		vec![leaf_indices.id, gradients.id, hessians.id], vec![mapped_indices, mapped_gradients, mapped_hessians],
		PrimitiveKind::Elementwise(Elementwise { program: histogram_contribution_program(request.descriptor.id)?, }), )?;
	let conflict = ScatterConflict::Atomic { operation: AtomicOperation::Add,
		ordering: AtomicOrdering::SequentiallyConsistent, }; emitter.emit_stage([ KernelEmission {
			inputs: vec![gradient_base.id, mapped_indices, mapped_gradients], outputs: vec![leaf_gradients.id],
			kind: scatter(conflict), }, KernelEmission { inputs: vec![hessian_base.id, mapped_indices, mapped_hessians],
			outputs: vec![leaf_hessians.id], kind: scatter(conflict), }, ]) }
...
fn emit_histogram_subtraction(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	emitter.emit( vec![ parent_gradients.id, parent_hessians.id, parent_counts.id, child_gradients.id, child_hessians.id,
			child_counts.id, ], vec![ remaining_gradients.id, remaining_hessians.id, remaining_counts.id, ],
		PrimitiveKind::Elementwise(Elementwise { program: histogram_subtraction_program(request.descriptor.id)?, }), ) }
...
fn emit_two_bin_split(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	require_supported( request, features == 1 && bins == 2 && nodes == 1,
	...
	] { require_dtype(request, tensor, DType::F32, name)?; require_shape(request, tensor, &[2], name)?; }
	...
	] { require_dtype(request, tensor, DType::I32, name)?; require_shape(request, tensor, &[1], name)?; }
	require_scalar(request, best_gain, DType::F32, "best_gain")?;
	require_scalar(request, best_feature, DType::I32, "best_feature")?;
	require_scalar(request, best_bin, DType::I32, "best_bin")?;
	...
	let weighted_histogram = |weights, destination| { KernelEmission { inputs: vec![bin_indices.id, weights],
			outputs: vec![destination], kind: PrimitiveKind::Histogram(Histogram { bins: 2, weighted: true,
				ordering: AtomicOrdering::SequentiallyConsistent, }), } }; emitter.emit_stage([
		weighted_histogram(gradients.id, copied_gradients), weighted_histogram(hessians.id, copied_hessians), ])?;
	...
	let prefix_counts = emitter.intermediate(DType::I32, counts.shape.clone())?; emitter.emit_stage([ KernelEmission {
			inputs: vec![copied_gradients], outputs: vec![prefix_gradients], kind: inclusive_sum_scan(tree_lanes), },
		KernelEmission { inputs: vec![copied_hessians], outputs: vec![prefix_hessians], kind: inclusive_sum_scan(tree_lanes),
		}, KernelEmission { inputs: vec![counts.id], outputs: vec![prefix_counts], kind: inclusive_sum_scan(tree_lanes), },
	])?;
	...
	let gains = emitter.intermediate(DType::F32, gradients.shape.clone())?; emitter.emit_stage([ KernelEmission {
			inputs: vec![prefix_gradients, total_index.id], outputs: vec![total_gradient], kind: gather(), }, KernelEmission {
			inputs: vec![prefix_hessians, total_index.id], outputs: vec![total_hessian], kind: gather(), }, KernelEmission {
			inputs: vec![ prefix_gradients, prefix_hessians, total_gradient, total_hessian, bin_indices.id, ],
			outputs: vec![gains], kind: PrimitiveKind::Elementwise(Elementwise {
				program: two_bin_gain_program(request.descriptor.id, lambda, minimum_child_weight)?, }), }, ])?;
	emitter.emit_stage([ KernelEmission { inputs: vec![feature_zero.id], outputs: vec![best_feature.id],
			kind: PrimitiveKind::Elementwise(Elementwise { program: identity_program(request.descriptor.id, DType::I32)?, }), },
		KernelEmission { inputs: vec![prefix_counts, left_count_index.id], outputs: vec![best_left_count.id], kind: gather(),
		}, KernelEmission { inputs: vec![gains], outputs: vec![best_gain.id, best_bin.id], kind: reduction( request,
				ReduceOperator::Maximum, &[0], false, ReduceResult::ValueAndIndex, tree_lanes, )?, }, ]) }
...
fn emit_ordered_target_statistics(
	...
	let sorted_positions = emitter.intermediate(DType::I32, ordering_keys.shape.clone())?; emitter.emit(
		vec![ordering_keys.id], vec![sorted_keys, sorted_positions], PrimitiveKind::Sort(Sort { axis: 0,
			direction: SortDirection::Ascending, stable: true, emit_indices: true, }), )?;
	...
	let prefix_counts = emitter.intermediate(DType::F32, targets.shape.clone())?; emitter.emit_stage([ KernelEmission {
			inputs: vec![targets.id, sorted_positions], outputs: vec![sorted_targets], kind: gather(), }, KernelEmission {
			inputs: vec![sorted_targets], outputs: vec![ones], kind: PrimitiveKind::Elementwise(Elementwise {
				program: constant_one_program(request.descriptor.id)?, }), }, KernelEmission { inputs: vec![sorted_targets],
			outputs: vec![prefix_sums], kind: exclusive_sum_scan(DType::F32, tree_lanes), }, KernelEmission { inputs: vec![ones],
			outputs: vec![prefix_counts], kind: exclusive_sum_scan(DType::F32, tree_lanes), }, ])?;
	let original_sums = emitter.intermediate(DType::F32, targets.shape.clone())?;
	let original_counts = emitter.intermediate(DType::F32, targets.shape.clone())?; emitter.emit_stage([ KernelEmission {
			inputs: vec![sum_base.id, sorted_positions, prefix_sums], outputs: vec![original_sums],
			kind: scatter(ScatterConflict::UniqueIndices), }, KernelEmission {
			inputs: vec![count_base.id, sorted_positions, prefix_counts], outputs: vec![original_counts],
			kind: scatter(ScatterConflict::UniqueIndices), }, KernelEmission { inputs: vec![original_sums, original_counts],
			outputs: vec![encoded.id], kind: PrimitiveKind::Elementwise(Elementwise {
...
fn emit_random_threshold(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let key = prepared_random_key(request)?; for (name, tensor) in [
		("minimum", minimum),
		("maximum", maximum),
		("threshold", threshold),
	] { require_scalar(request, tensor, DType::F32, name)?; }
	let uniform = emitter.intermediate(DType::F32, scalar_shape(request)?)?; emitter.emit( Vec::new(), vec![uniform],
		PrimitiveKind::Random(RandomMap { distribution: RandomDistribution::UniformF32, key, philox_rounds: 10, }), )?;
	emitter.emit( vec![minimum.id, maximum.id, uniform], vec![threshold.id], PrimitiveKind::Elementwise(Elementwise {
...
fn emit_leaf_prediction(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	] { require_dtype(request, tensor, DType::F32, name)?; require_shape(request, tensor, &[prediction_elements], name)?; }
	...
	] { require_dtype(request, tensor, DType::I32, name)?; require_shape(request, tensor, &[updates], name)?; }
	...
	let mapped_updates = emitter.intermediate(DType::F32, shape(request, &[updates])?)?; emitter.emit_stage([
		KernelEmission { inputs: vec![leaf_values.id, leaf_indices.id], outputs: vec![selected_values], kind: gather(), },
		KernelEmission { inputs: vec![prediction_base.id, destination_indices.id], outputs: vec![current_predictions],
			kind: gather(), }, KernelEmission { inputs: vec![current_predictions, selected_values],
			outputs: vec![mapped_updates], kind: PrimitiveKind::Elementwise(Elementwise {
				program: leaf_prediction_program(request.descriptor.id, learning_rate)?, }), }, ])?; emitter.emit(
		vec![prediction_base.id, destination_indices.id, mapped_updates], vec![predictions.id],
...
fn emit_write_split(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	] { require_dtype(request, tensor, DType::I32, name)?; require_shape(request, tensor, &[slots], name)?; }
	emitter.emit( vec![feature_base.id, bin_base.id, write_mask.id], vec![split_features.id, split_bins.id],
		PrimitiveKind::Elementwise(Elementwise { program: write_split_program(request.descriptor.id, feature, bin)?, }), ) }
...
fn reduction(
	request: &MaterializationRequest<'_>,
	operator: ReduceOperator, axes: &[usize], keep_dimensions: bool, result: ReduceResult, tree_lanes: u32,
) -> OperationResult<PrimitiveKind> { let axes =
...
fn exclusive_sum_scan(dtype: DType, tree_lanes: u32) -> PrimitiveKind { let identity = match dtype {
		DType::F32 => recipe_core::ScalarLiteral::F32Bits(0.0_f32.to_bits()),
		DType::I32 => recipe_core::ScalarLiteral::I32(0), }; PrimitiveKind::Scan(Scan { operator: ReduceOperator::Sum,
		axis: 0, mode: ScanMode::Exclusive { identity }, reverse: false, tree_lanes, }) }
...
fn finite_parameter(request: &MaterializationRequest<'_>, name: &str) -> OperationResult<f32> {
	let value = prepared_f32(request, name)?; match value.is_finite() { true => Ok(value),
...
fn finite_nonnegative(request: &MaterializationRequest<'_>, name: &str) -> OperationResult<f32> {
	let value = finite_parameter(request, name)?; match value >= 0.0 { true => Ok(value), false => { Err(request_error(
				request,
				format!("{name} must be nonnegative"),
			)) } } }

fn finite_positive(request: &MaterializationRequest<'_>, name: &str) -> OperationResult<f32> {
	let value = finite_parameter(request, name)?; match value > 0.0 { true => Ok(value),
		false => Err(request_error(request, format!("{name} must be positive"))),
	} }

fn require_scalar(
	request: &MaterializationRequest<'_>,
	tensor: &Tensor, dtype: DType, name: &str, ) -> OperationResult<()> { require_dtype(request, tensor, dtype, name)?;
...
fn split_index_program(operation: OperationId, bins: u64) -> OperationResult<recipe_core::ScalarProgram> {
	let bins = i32::try_from(bins).map_err(|conversion_error| { operation_error( operation,
			OperationErrorKind::InvalidMaterializationRequest, conversion_error.to_string(), ) })?;
	let mut builder = scalar_builder(operation)?; let index = scalar_input(operation, &mut builder, DType::I32)?;
	let divisor = builder .i32(bins) .map_err(|error| language_error(operation, error.to_string()))?;
	let feature = scalar_binary( operation, &mut builder, ScalarOpcode::Divide, index, divisor, )?;
	let bin = scalar_binary( operation, &mut builder, ScalarOpcode::Remainder, index, divisor, )?;
...
fn gradient_hessian_program(operation: OperationId, class_index: i32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?; let probability = scalar_input(operation, &mut builder, DType::F32)?;
	let target = scalar_input(operation, &mut builder, DType::I32)?;
	let weight = scalar_input(operation, &mut builder, DType::F32)?;
	let mask = scalar_input(operation, &mut builder, DType::I32)?; let mask = bool_mask(operation, &mut builder, mask)?;
	let class_index = builder .i32(class_index) .map_err(|error| language_error(operation, error.to_string()))?;
	let target_matches = scalar_binary( operation, &mut builder, ScalarOpcode::Equal, target, class_index, )?;
	let target_matches = scalar_unary( operation, &mut builder, ScalarOpcode::ConvertI32ToF32, target_matches, )?;
	let mask = scalar_unary(operation, &mut builder, ScalarOpcode::ConvertI32ToF32, mask)?; let residual = scalar_binary(
		operation, &mut builder, ScalarOpcode::Subtract, probability, target_matches, )?;
	let weighted_residual = scalar_binary( operation, &mut builder, ScalarOpcode::Multiply, residual, weight, )?;
	let gradient = scalar_binary( operation, &mut builder, ScalarOpcode::Multiply, weighted_residual, mask, )?;
	let one = scalar_f32(operation, &mut builder, 1.0)?; let complement = scalar_binary( operation, &mut builder,
		ScalarOpcode::Subtract, one, probability, )?; let variance = scalar_binary( operation, &mut builder,
		ScalarOpcode::Multiply, probability, complement, )?; let weighted_variance = scalar_binary( operation, &mut builder,
		ScalarOpcode::Multiply, variance, weight, )?; let minimum = scalar_f32(operation, &mut builder, 0.001)?;
	let maximum = scalar_f32(operation, &mut builder, 1_000_000.0)?; let floored = scalar_binary( operation, &mut builder,
		ScalarOpcode::Maximum, weighted_variance, minimum, )?; let clamped = scalar_binary( operation, &mut builder,
		ScalarOpcode::Minimum, floored, maximum, )?; let hessian = scalar_binary( operation, &mut builder,
		ScalarOpcode::Multiply, clamped, mask, )?; scalar_finish(operation, builder, &[gradient, hessian]) }

fn histogram_contribution_program(operation: OperationId) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?; let bin = scalar_input(operation, &mut builder, DType::I32)?;
...
fn regularized_leaf_program( operation: OperationId, lambda: f32, expose_sums: bool,
) -> OperationResult<recipe_core::ScalarProgram> { let mut builder = scalar_builder(operation)?;
	let gradient = scalar_input(operation, &mut builder, DType::F32)?;
	let hessian = scalar_input(operation, &mut builder, DType::F32)?; let zero = scalar_f32(operation, &mut builder, 0.0)?;
	let nonnegative_hessian = scalar_binary( operation, &mut builder, ScalarOpcode::GreaterThanOrEqual, hessian, zero, )?;
	scalar_unary( operation, &mut builder, ScalarOpcode::Require, nonnegative_hessian, )?;
	...
	let positive_denominator = scalar_binary( operation, &mut builder, ScalarOpcode::GreaterThan, denominator, zero, )?;
	scalar_unary( operation, &mut builder, ScalarOpcode::Require, positive_denominator, )?;
	let negative_gradient = scalar_unary(operation, &mut builder, ScalarOpcode::Negate, gradient)?;
	let value = scalar_binary( operation, &mut builder, ScalarOpcode::Divide, negative_gradient, denominator, )?;
	let outputs = match expose_sums { true => vec![gradient, hessian, value], false => vec![value], };
...
fn histogram_subtraction_program(operation: OperationId) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?; let parent_gradient = scalar_input(operation, &mut builder, DType::F32)?;
	...
	let child_count = scalar_input(operation, &mut builder, DType::I32)?; let gradient = scalar_binary( operation,
		&mut builder, ScalarOpcode::Subtract, parent_gradient, child_gradient, )?; let hessian = scalar_binary( operation,
		&mut builder, ScalarOpcode::Subtract, parent_hessian, child_hessian, )?; let count = scalar_binary( operation,
		&mut builder, ScalarOpcode::Subtract, parent_count, child_count, )?;
	scalar_finish(operation, builder, &[gradient, hessian, count]) }

fn two_bin_gain_program( operation: OperationId, lambda: f32, minimum_child_weight: f32,
) -> OperationResult<recipe_core::ScalarProgram> { let mut builder = scalar_builder(operation)?;
	...
	let zero_f32 = scalar_f32(operation, &mut builder, 0.0)?; let total_hessian_nonnegative = scalar_binary( operation,
		&mut builder, ScalarOpcode::GreaterThanOrEqual, total_hessian, zero_f32, )?; scalar_unary( operation, &mut builder,
		ScalarOpcode::Require, total_hessian_nonnegative, )?; let zero_i32 = builder .i32(0)
		.map_err(|error| language_error(operation, error.to_string()))?; let one_i32 = builder .i32(1)
		.map_err(|error| language_error(operation, error.to_string()))?;
	...
	let valid_bin = scalar_binary( operation, &mut builder, ScalarOpcode::BitOr, is_left_candidate, is_terminal_bin, )?;
	scalar_unary(operation, &mut builder, ScalarOpcode::Require, valid_bin)?; let right_gradient = scalar_binary(
		operation, &mut builder, ScalarOpcode::Subtract, total_gradient, left_gradient, )?; let right_hessian = scalar_binary(
		operation, &mut builder, ScalarOpcode::Subtract, total_hessian, left_hessian, )?;
	let minimum_child_weight = scalar_f32(operation, &mut builder, minimum_child_weight)?; let left_legal = scalar_binary(
		operation, &mut builder, ScalarOpcode::GreaterThanOrEqual, left_hessian, minimum_child_weight, )?;
	let right_legal = scalar_binary( operation, &mut builder, ScalarOpcode::GreaterThanOrEqual, right_hessian,
		minimum_child_weight, )?; let legal_split = scalar_binary( operation, &mut builder, ScalarOpcode::BitAnd, left_legal,
		right_legal, )?; let valid_candidate = scalar_binary( operation, &mut builder, ScalarOpcode::BitOr, is_terminal_bin,
		legal_split, )?; scalar_unary( operation, &mut builder, ScalarOpcode::Require, valid_candidate, )?;
	let lambda = scalar_f32(operation, &mut builder, lambda)?; let left_denominator = scalar_binary( operation,
		&mut builder, ScalarOpcode::Add, left_hessian, lambda, )?; let right_denominator = scalar_binary( operation,
		&mut builder, ScalarOpcode::Add, right_hessian, lambda, )?; let total_denominator = scalar_binary( operation,
		&mut builder, ScalarOpcode::Add, total_hessian, lambda, )?; let left_square = scalar_binary( operation, &mut builder,
		ScalarOpcode::Multiply, left_gradient, left_gradient, )?; let right_square = scalar_binary( operation, &mut builder,
		ScalarOpcode::Multiply, right_gradient, right_gradient, )?; let total_square = scalar_binary( operation, &mut builder,
		ScalarOpcode::Multiply, total_gradient, total_gradient, )?; let left_score = scalar_binary( operation, &mut builder,
		ScalarOpcode::Divide, left_square, left_denominator, )?; let right_score = scalar_binary( operation, &mut builder,
		ScalarOpcode::Divide, right_square, right_denominator, )?; let parent_score = scalar_binary( operation, &mut builder,
		ScalarOpcode::Divide, total_square, total_denominator, )?; let child_score = scalar_binary( operation, &mut builder,
		ScalarOpcode::Add, left_score, right_score, )?; let gain = scalar_binary( operation, &mut builder,
		ScalarOpcode::Subtract, child_score, parent_score, )?;
	let terminal_sentinel = scalar_f32(operation, &mut builder, -f32::MAX)?; let selected = scalar_ternary( operation,
		&mut builder, ScalarOpcode::Select, is_left_candidate, gain, terminal_sentinel, )?;
...
fn constant_one_program(operation: OperationId) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?; let consumed = scalar_input(operation, &mut builder, DType::F32)?;
	let one = scalar_f32(operation, &mut builder, 1.0)?; let always = builder .i32(1)
		.map_err(|error| language_error(operation, error.to_string()))?; let result = scalar_ternary( operation, &mut builder,
		ScalarOpcode::Select, always, one, consumed, )?; scalar_finish(operation, builder, &[result]) }

fn ordered_statistic_program( operation: OperationId, prior: f32, smoothing: f32,
) -> OperationResult<recipe_core::ScalarProgram> { let mut builder = scalar_builder(operation)?;
	...
	let smoothing = scalar_f32(operation, &mut builder, smoothing)?; let smoothed_prior = scalar_binary( operation,
		&mut builder, ScalarOpcode::Multiply, prior, smoothing, )?; let numerator = scalar_binary( operation, &mut builder,
		ScalarOpcode::Add, prefix_sum, smoothed_prior, )?; let denominator = scalar_binary( operation, &mut builder,
		ScalarOpcode::Add, prefix_count, smoothing, )?; let statistic = scalar_binary( operation, &mut builder,
		ScalarOpcode::Divide, numerator, denominator, )?; scalar_finish(operation, builder, &[statistic]) }

fn threshold_program(operation: OperationId) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?; let minimum = scalar_input(operation, &mut builder, DType::F32)?;
	let maximum = scalar_input(operation, &mut builder, DType::F32)?;
	let uniform = scalar_input(operation, &mut builder, DType::F32)?; for value in [minimum, maximum, uniform] {
		let finite = scalar_unary(operation, &mut builder, ScalarOpcode::IsFinite, value)?;
		scalar_unary(operation, &mut builder, ScalarOpcode::Require, finite)?; }
	let ordered = scalar_binary( operation, &mut builder, ScalarOpcode::LessThanOrEqual, minimum, maximum, )?;
	scalar_unary(operation, &mut builder, ScalarOpcode::Require, ordered)?; let width = scalar_binary( operation,
		&mut builder, ScalarOpcode::Subtract, maximum, minimum, )?; let offset = scalar_binary( operation, &mut builder,
		ScalarOpcode::Multiply, uniform, width, )?;
...
fn leaf_prediction_program(operation: OperationId, learning_rate: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?; let current = scalar_input(operation, &mut builder, DType::F32)?;
	let leaf_value = scalar_input(operation, &mut builder, DType::F32)?;
	let learning_rate = scalar_f32(operation, &mut builder, learning_rate)?; let update = scalar_binary( operation,
		&mut builder, ScalarOpcode::Multiply, leaf_value, learning_rate, )?;
...
fn write_split_program(operation: OperationId, feature: i32, bin: i32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?; let feature_base = scalar_input(operation, &mut builder, DType::I32)?;
	let bin_base = scalar_input(operation, &mut builder, DType::I32)?;
	let mask = scalar_input(operation, &mut builder, DType::I32)?; let mask = bool_mask(operation, &mut builder, mask)?;
	let feature = builder .i32(feature) .map_err(|error| language_error(operation, error.to_string()))?; let bin = builder
		.i32(bin) .map_err(|error| language_error(operation, error.to_string()))?; let selected_feature = scalar_ternary(
		operation, &mut builder, ScalarOpcode::Select, mask, feature, feature_base, )?; let selected_bin = scalar_ternary(
		operation, &mut builder, ScalarOpcode::Select, mask, bin, bin_base, )?;
