...
use recipe_language::{AtomicOrdering, AxisSet, CalculationGraph, CalculationNode, Elementwise, Gather, Histogram, IndexBounds, IndexMap, PrimitiveAliasRule, PrimitiveKernel, PrimitiveKind, Reduce, ReduceOperator, ReduceResult, ScalarExpression, ScalarProgramBuilder, Shape, Tensor};
...
pub fn categorical_bayes_inference_requirements(reference_rows: u64, query_rows: u64, parent_count: u64, parent_configurations: u64, child_classes: u64) -> OperationResult<CategoricalBayesInferenceRequirements> {
	for (name, value) in [
...
pub fn materialize_categorical_bayes_inference(request: &CategoricalBayesInferenceRequest) -> OperationResult<CategoricalBayesInferenceMaterialization> {
...
pub fn append_categorical_bayes_inference(graph: &mut CalculationGraph, request: &CategoricalBayesInferenceRequest) -> OperationResult<CategoricalBayesInferenceMaterialization> {
...
fn validate_request(request: &CategoricalBayesInferenceRequest) -> OperationResult<CategoricalBayesInferenceRequirements> {
	for tensor in [
...
fn validate_boundary_storage(graph: &CalculationGraph, request: &CategoricalBayesInferenceRequest) -> OperationResult<()> {
...
fn validate_resources(request: &CategoricalBayesInferenceRequest, requirements: CategoricalBayesInferenceRequirements) -> OperationResult<()> {
...
impl BayesEmitter {
	fn new(request: &CategoricalBayesInferenceRequest, requirements: CategoricalBayesInferenceRequirements) -> OperationResult<Self> {
	...
	fn reduce_sum(&mut self, input: ValueId, output: ValueId, axis: usize, keep_dimensions: bool, tree_lanes: u32) -> OperationResult<()> {
		self.emit(
			vec![input],
			vec![output],
