...
const F32_1: &[DType] = &[DType::F32]; const F32_2: &[DType] = &[DType::F32, DType::F32];
const F32_3: &[DType] = &[DType::F32, DType::F32, DType::F32];
const F32_4: &[DType] = &[DType::F32, DType::F32, DType::F32, DType::F32];
const I32_F32_F32: &[DType] = &[DType::I32, DType::F32, DType::F32]; const F32_OUTPUT: &[DType] = &[DType::F32];
...
pub enum ScalarRecipe { Opcode { opcode: ScalarOpcode,
		inputs: &'static [DType],
	}, Math(MathFunction), Composite(CompositeScalar), }

/// Multi-instruction elementwise calculations owned by Recipe.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CompositeScalar { Identity, ReverseAdd, ReverseMultiply, DifferenceScaled, ReverseSubtract, ReverseDivide,
	Clamp, ClampLegacyOrder, BinaryCrossEntropyGradient, Dropout, Relu, ReluBackward, LeakyRelu, LeakyReluBackward, Elu,
	EluBackward, Selu, SeluBackward, SigmoidBackward, TanhBackward, Silu, SiluBackward, GeluTanh, GeluTanhBackward,
	GeluTanhMultiply, GluGeluTanh, GluSilu, ScaledExp, Reparameterize, KullbackLeiblerElement, MeanAbsoluteErrorGradient,
	HuberGradient, SgdAdd, SgdSubtract, }

impl ScalarRecipe { pub(crate) fn for_symbol(symbol: &str) -> Option<Self> { let recipe = match symbol {
	...
	#[must_use]
	pub const fn dtype_contract(self) -> CanonicalDTypeContract { match self { Self::Opcode { opcode, inputs } => {
				CanonicalDTypeContract::Exact { inputs, outputs: if matches!( opcode, ScalarOpcode::Equal
							| ScalarOpcode::NotEqual | ScalarOpcode::LessThan
							| ScalarOpcode::LessThanOrEqual | ScalarOpcode::GreaterThan
							| ScalarOpcode::GreaterThanOrEqual ) { I32_OUTPUT } else { F32_OUTPUT }, } }
			Self::Math(function) => { CanonicalDTypeContract::Exact { inputs: if function.arity() == 1 { F32_1 } else { F32_2 },
					outputs: F32_OUTPUT, } }
			Self::Composite(composite) => { CanonicalDTypeContract::Exact { inputs: composite.inputs(), outputs: F32_OUTPUT, } }
		} }

	#[must_use]
	pub const fn definition(self) -> &'static str {
		match self { Self::Opcode { opcode: ScalarOpcode::Add, ..
			} => "per-element typed addition",
			Self::Opcode { opcode: ScalarOpcode::Subtract, ..
			} => "per-element typed subtraction",
			Self::Opcode { opcode: ScalarOpcode::Multiply, ..
			} => "per-element typed multiplication",
			Self::Opcode { opcode: ScalarOpcode::Divide, ..
			} => "per-element IEEE binary32 division",
			Self::Opcode { opcode: ScalarOpcode::Absolute, ..
			} => "per-element binary32 absolute value",
			Self::Opcode { opcode: ScalarOpcode::Negate, ..
			} => "per-element binary32 negation",
			Self::Opcode { opcode: ScalarOpcode::Minimum, ..
			} => "per-element binary32 minimum",
			Self::Opcode { opcode: ScalarOpcode::Maximum, ..
			} => "per-element binary32 maximum",
			Self::Opcode { opcode: ScalarOpcode::Fma, ..
			} => "per-element one-rounding binary32 fused multiply-add",
			Self::Opcode { opcode: ScalarOpcode::Equal, ..
			} => "per-element ordered equality yielding an int32 mask",
			Self::Opcode { opcode: ScalarOpcode::LessThan, ..
			} => "per-element ordered less-than yielding an int32 mask",
			Self::Opcode { opcode: ScalarOpcode::GreaterThan, ..
			} => "per-element ordered greater-than yielding an int32 mask",
			Self::Opcode { opcode: ScalarOpcode::Select, ..
			} => "per-element int32-mask selection",
			Self::Opcode { opcode: ScalarOpcode::SquareRoot, ..
			} => "per-element binary32 square root",
			Self::Opcode { .. } => "typed scalar opcode",
			Self::Math(function) => function.algorithm().name, Self::Composite(composite) => composite.definition(), } } }

impl CompositeScalar {
	const fn inputs(self) -> &'static [DType] {
		match self { Self::Identity | Self::Relu | Self::Silu | Self::GeluTanh => F32_1, Self::ReverseAdd
			...
			| Self::MeanAbsoluteErrorGradient => F32_2, Self::DifferenceScaled
			| Self::Clamp
			...
			| Self::HuberGradient
			| Self::SgdAdd
			| Self::SgdSubtract => F32_3, Self::Dropout | Self::SeluBackward => F32_4, } }
...
pub fn lower_scalar(descriptor: OperationDescriptor) -> OperationResult<ScalarProgram> {
	let recipe = match descriptor.lowering { LoweringAvailability::Scalar(recipe) => recipe,
		LoweringAvailability::Primitive(_)
		| LoweringAvailability::Composition(_)
		| LoweringAvailability::Workspace(_)
		| LoweringAvailability::NonCalculation(_) => { return Err(wrong_kind( descriptor,
				"operation does not have a scalar elementwise recipe",
			)); }
		LoweringAvailability::Unsupported(reason) => { return Err(OperationError::new(
				OperationErrorKind::UnsupportedLowering,
				format!("{}: {reason:?}", descriptor.definition),
			) .for_operation(descriptor.id)); } }; let program = match recipe { ScalarRecipe::Opcode { opcode, inputs } => {
			let mut composer = Composer::new(); let inputs = composer.inputs(inputs)?;
			let output = composer.apply(opcode, &inputs)?; composer.finish(&[output])? }
		...
		ScalarRecipe::Composite(composite) => {
			lower_composite(composite).map_err(|error| error.for_operation(descriptor.id))? } }; program .validate()
		.map_err(|errors| invalid_program(descriptor, errors.to_string()))?; Ok(program) }

/// Canonical parameterless leaky-ReLU forward program used by Recipe models.
pub fn canonical_leaky_relu_program() -> OperationResult<ScalarProgram> { let mut composer = Composer::new();
	let value = composer.input(DType::F32)?; let alpha = composer.f32(0.01)?;
	let output = leaky_relu(&mut composer, value, alpha)?; composer.finish(&[output]) }

/// Canonical parameterless leaky-ReLU backward program used by Recipe models.
pub fn canonical_leaky_relu_backward_program() -> OperationResult<ScalarProgram> { let mut composer = Composer::new();
	let gradient = composer.input(DType::F32)?; let value = composer.input(DType::F32)?; let alpha = composer.f32(0.01)?;
	let output = leaky_relu_backward(&mut composer, gradient, value, alpha)?; composer.finish(&[output]) }
...
pub fn canonical_prelu_program() -> OperationResult<ScalarProgram> { let mut composer = Composer::new();
	let value = composer.input(DType::F32)?; let alpha = composer.input(DType::F32)?;
	let output = leaky_relu(&mut composer, value, alpha)?; composer.finish(&[output]) }
...
pub fn canonical_prelu_backward_program() -> OperationResult<ScalarProgram> { let mut composer = Composer::new();
	let gradient = composer.input(DType::F32)?; let value = composer.input(DType::F32)?;
	let alpha = composer.input(DType::F32)?;
	let input_gradient = leaky_relu_backward(&mut composer, gradient, value, alpha)?; let zero = composer.f32(0.0)?;
...
pub fn canonical_elu_program() -> OperationResult<ScalarProgram> { let mut composer = Composer::new();
	let value = composer.input(DType::F32)?; let alpha = composer.f32(1.0)?;
	let output = elu(&mut composer, value, alpha)?; composer.finish(&[output]) }

/// Canonical parameterless ELU backward program with alpha equal to one.
pub fn canonical_elu_backward_program() -> OperationResult<ScalarProgram> { let mut composer = Composer::new();
	let gradient = composer.input(DType::F32)?; let value = composer.input(DType::F32)?; let alpha = composer.f32(1.0)?;
	let output = elu_backward(&mut composer, gradient, value, alpha)?; composer.finish(&[output]) }

/// Canonical self-normalizing SELU forward program.
pub fn canonical_selu_program() -> OperationResult<ScalarProgram> { let mut composer = Composer::new();
	let value = composer.input(DType::F32)?; let alpha = composer.f32(1.673_263_2)?; let lambda = composer.f32(1.050_701)?;
	let activated = elu(&mut composer, value, alpha)?;
	let output = composer.binary(ScalarOpcode::Multiply, lambda, activated)?; composer.finish(&[output]) }

/// Canonical self-normalizing SELU backward program.
pub fn canonical_selu_backward_program() -> OperationResult<ScalarProgram> { let mut composer = Composer::new();
	let gradient = composer.input(DType::F32)?; let value = composer.input(DType::F32)?;
	let alpha = composer.f32(1.673_263_2)?; let lambda = composer.f32(1.050_701)?;
	let derivative = elu_derivative(&mut composer, value, alpha)?;
	let scaled = composer.binary(ScalarOpcode::Multiply, lambda, derivative)?;
	let output = composer.binary(ScalarOpcode::Multiply, gradient, scaled)?; composer.finish(&[output]) }

pub(crate) fn binary_cross_entropy_with_logits_program() -> OperationResult<ScalarProgram> {
	let mut composer = Composer::new(); let logit = composer.input(DType::F32)?; let target = composer.input(DType::F32)?;

	let target_is_finite = composer.unary(ScalarOpcode::IsFinite, target)?;
	composer.unary(ScalarOpcode::Require, target_is_finite)?; let zero = composer.f32(0.0)?; let one = composer.f32(1.0)?;
	...
	let gradient = composer.binary(ScalarOpcode::Subtract, sigmoid, target)?; composer.finish(&[loss, gradient]) }
...
pub fn canonical_focal_with_logits_program() -> OperationResult<ScalarProgram> { let mut composer = Composer::new();
	let logit = composer.input(DType::F32)?; let target = composer.input(DType::F32)?;
	...
	let loss = composer.binary( ScalarOpcode::Multiply, weighted_focus, negative_log_probability, )?;

	let probability_log = composer.binary( ScalarOpcode::Multiply, target_probability, negative_log_probability, )?;
	...
	let negative_gradient = composer.unary(ScalarOpcode::Negate, gradient_magnitude)?; let gradient = composer.ternary(
		ScalarOpcode::Select, target_is_one, negative_gradient, gradient_magnitude, )?; composer.finish(&[loss, gradient]) }

fn lower_composite(composite: CompositeScalar) -> OperationResult<ScalarProgram> { let mut composer = Composer::new();
	let values = composer.inputs(composite.inputs())?; let output = match composite {
		...
		CompositeScalar::Clamp => { let lower = composer.binary(ScalarOpcode::Maximum, values[0], values[1])?;
			composer.binary(ScalarOpcode::Minimum, lower, values[2])? }
		CompositeScalar::ClampLegacyOrder => { let lower = composer.binary(ScalarOpcode::Maximum, values[2], values[0])?;
		...
		CompositeScalar::Dropout => { let drop = composer.binary(ScalarOpcode::LessThan, values[1], values[2])?;
			let zero = composer.f32(0.0)?; let scaled = composer.binary(ScalarOpcode::Multiply, values[0], values[3])?;
		...
		CompositeScalar::Selu => { let activated = elu(&mut composer, values[0], values[1])?;
			composer.binary(ScalarOpcode::Multiply, values[2], activated)? }
		CompositeScalar::SeluBackward => { let derivative = elu_derivative(&mut composer, values[1], values[2])?;
		...
		CompositeScalar::SigmoidBackward => { let one = composer.f32(1.0)?;
		...
		CompositeScalar::TanhBackward => { let square = composer.binary(ScalarOpcode::Multiply, values[1], values[1])?;
			let one = composer.f32(1.0)?; let slope = composer.binary(ScalarOpcode::Subtract, one, square)?;
			composer.binary(ScalarOpcode::Multiply, values[0], slope)? }
		...
		CompositeScalar::GeluTanhMultiply | CompositeScalar::GluGeluTanh => {
			let activated = gelu_tanh(&mut composer, values[0])?; composer.binary(ScalarOpcode::Multiply, activated, values[1])?
		}
		CompositeScalar::GluSilu => { let activated = silu(&mut composer, values[0])?;
			composer.binary(ScalarOpcode::Multiply, activated, values[1])? }
		CompositeScalar::ScaledExp => { let scaled = composer.binary(ScalarOpcode::Multiply, values[0], values[1])?;
			composer.inline_math(MathFunction::Exp, &[scaled])? }
		CompositeScalar::Reparameterize => { let half = composer.f32(0.5)?;
		...
		CompositeScalar::HuberGradient => { let difference = composer.binary(ScalarOpcode::Subtract, values[0], values[1])?;
			...
			composer.binary(ScalarOpcode::Minimum, lower, values[2])? }
		CompositeScalar::SgdAdd => { let update = composer.binary(ScalarOpcode::Multiply, values[1], values[0])?;
			composer.binary(ScalarOpcode::Add, values[2], update)? }
		CompositeScalar::SgdSubtract => { let update = composer.binary(ScalarOpcode::Multiply, values[1], values[0])?;
			composer.binary(ScalarOpcode::Subtract, values[2], update)? } }; composer.finish(&[output]) }

fn binary_cross_entropy_gradient( composer: &mut Composer, prediction: Value, target: Value, inverse_count: Value,
) -> OperationResult<Value> { let epsilon = composer.f32(1.0e-7)?; let one = composer.f32(1.0)?;
...
fn relu(composer: &mut Composer, value: Value) -> OperationResult<Value> { let zero = composer.f32(0.0)?;
...
fn positive_gradient(composer: &mut Composer, gradient: Value, activation: Value) -> OperationResult<Value> {
	let zero = composer.f32(0.0)?; let positive = composer.binary(ScalarOpcode::GreaterThan, activation, zero)?;
...
fn leaky_relu(composer: &mut Composer, value: Value, alpha: Value) -> OperationResult<Value> {
	let zero = composer.f32(0.0)?; let positive = composer.binary(ScalarOpcode::GreaterThan, value, zero)?;
...
fn leaky_relu_backward( composer: &mut Composer, gradient: Value, activation: Value, alpha: Value,
) -> OperationResult<Value> { let zero = composer.f32(0.0)?;
...
fn elu(composer: &mut Composer, value: Value, alpha: Value) -> OperationResult<Value> { let zero = composer.f32(0.0)?;
	let positive = composer.binary(ScalarOpcode::GreaterThan, value, zero)?;
	let exponential = composer.inline_math(MathFunction::Exp, &[value])?; let one = composer.f32(1.0)?;
...
fn elu_derivative(composer: &mut Composer, value: Value, alpha: Value) -> OperationResult<Value> {
	let zero = composer.f32(0.0)?; let positive = composer.binary(ScalarOpcode::GreaterThan, value, zero)?;
	let one = composer.f32(1.0)?; let exponential = composer.inline_math(MathFunction::Exp, &[value])?;
...
fn elu_backward(composer: &mut Composer, gradient: Value, value: Value, alpha: Value) -> OperationResult<Value> {
	let derivative = elu_derivative(composer, value, alpha)?; composer.binary(ScalarOpcode::Multiply, gradient, derivative)
}
...
fn silu_backward(composer: &mut Composer, gradient: Value, value: Value) -> OperationResult<Value> {
	let sigmoid = composer.inline_math(MathFunction::Sigmoid, &[value])?; let one = composer.f32(1.0)?;
...
fn gelu_tanh(composer: &mut Composer, value: Value) -> OperationResult<Value> {
	...
	let cube = composer.binary(ScalarOpcode::Multiply, square, value)?; let cubic_coefficient = composer.f32(0.044_715)?;
	let cubic = composer.binary(ScalarOpcode::Multiply, cubic_coefficient, cube)?;
	let polynomial = composer.binary(ScalarOpcode::Add, value, cubic)?; let coefficient = composer.f32(0.797_884_6)?;
	let inner = composer.binary(ScalarOpcode::Multiply, coefficient, polynomial)?;
	let hyperbolic = composer.inline_math(MathFunction::Tanh, &[inner])?; let one = composer.f32(1.0)?;
	let shifted = composer.binary(ScalarOpcode::Add, one, hyperbolic)?; let half = composer.f32(0.5)?;
...
fn gelu_tanh_backward(composer: &mut Composer, gradient: Value, value: Value) -> OperationResult<Value> {
	...
	let inner = composer.binary(ScalarOpcode::Multiply, coefficient, polynomial)?;
	let hyperbolic = composer.inline_math(MathFunction::Tanh, &[inner])?; let one = composer.f32(1.0)?;
	let half = composer.f32(0.5)?; let shifted = composer.binary(ScalarOpcode::Add, one, hyperbolic)?;
...
fn kl_divergence(composer: &mut Composer, mean: Value, log_variance: Value) -> OperationResult<Value> {
	...
	let difference = composer.binary(ScalarOpcode::Subtract, sum, log_variance)?; let one = composer.f32(1.0)?;
	let shifted = composer.binary(ScalarOpcode::Subtract, difference, one)?; let half = composer.f32(0.5)?;
	composer.binary(ScalarOpcode::Multiply, half, shifted) }

fn wrong_kind(descriptor: OperationDescriptor, detail: &'static str) -> OperationError { OperationError::new(OperationErrorKind::WrongLoweringKind, detail).for_operation(descriptor.id) }

fn invalid_program(descriptor: OperationDescriptor, detail: String) -> OperationError { OperationError::new(OperationErrorKind::InvalidScalarProgram, detail).for_operation(descriptor.id) }

#[derive(Clone, Copy, Debug)]
struct Value { id: ScalarValueId, dtype: DType, }

#[derive(Debug)]
struct Composer { next_value: u64, inputs: Vec<ScalarInput>, constants: Vec<ScalarConstant>,
	instructions: Vec<ScalarInstruction>, }

impl Composer { const fn new() -> Self { Self { next_value: 1, inputs: Vec::new(), constants: Vec::new(),
			instructions: Vec::new(), } }

	fn inputs(&mut self, dtypes: &[DType]) -> OperationResult<Vec<Value>> { dtypes.iter() .copied()
			.map(|dtype| self.input(dtype)) .collect() }

	fn input(&mut self, dtype: DType) -> OperationResult<Value> { let value = self.next(dtype)?;
		self.inputs.push(ScalarInput { id: value.id, dtype, }); Ok(value) }

	fn f32(&mut self, value: f32) -> OperationResult<Value> { self.constant(ScalarLiteral::F32Bits(value.to_bits())) }

	fn constant(&mut self, literal: ScalarLiteral) -> OperationResult<Value> { let value = self.next(literal.dtype())?;
		self.constants.push(ScalarConstant { id: value.id, value: literal, }); Ok(value) }

	fn unary(&mut self, opcode: ScalarOpcode, operand: Value) -> OperationResult<Value> { self.apply(opcode, &[operand]) }

	fn binary(&mut self, opcode: ScalarOpcode, left: Value, right: Value) -> OperationResult<Value> { self.apply(opcode, &[left, right]) }

	fn ternary(&mut self, opcode: ScalarOpcode, first: Value, second: Value, third: Value) -> OperationResult<Value> { self.apply(opcode, &[first, second, third]) }

	fn apply(&mut self, opcode: ScalarOpcode, operands: &[Value]) -> OperationResult<Value> { let dtypes = operands .iter()
			.map(|operand| operand.dtype) .collect::<Vec<_>>(); let dtype = opcode.result_dtype(&dtypes).ok_or_else(|| {
			OperationError::new( OperationErrorKind::InvalidScalarProgram,
				format!("{opcode:?} does not accept {dtypes:?}"),
			) })?; let result = self.next(dtype)?; self.instructions.push(ScalarInstruction { result: result.id, dtype, opcode,
			operands: operands.iter().map(|operand| operand.id).collect(), }); Ok(result) }
	...
	fn inline_math(&mut self, function: MathFunction, arguments: &[Value]) -> OperationResult<Value> {
		...
		if program.inputs.len() != arguments.len() { return Err(OperationError::new( OperationErrorKind::InvalidScalarProgram,
				format!(
					"{function:?} has {} inputs, but {} arguments were supplied",
					program.inputs.len(), arguments.len() ), )); }
		let mut values = BTreeMap::new(); for (input, argument) in program.inputs.iter().zip(arguments) {
			if input.dtype != argument.dtype { return Err(OperationError::new( OperationErrorKind::InvalidScalarProgram, format!(
						"{function:?} input requires {:?}, received {:?}",
						input.dtype, argument.dtype ), )); }
			values.insert(input.id, *argument); }
		for constant in program.constants { let replacement = self.constant(constant.value)?;
			values.insert(constant.id, replacement); }
		for instruction in program.instructions { let operands = instruction .operands .iter() .map(|operand| {
					values.get(operand).copied().ok_or_else(|| { OperationError::new( OperationErrorKind::InvalidScalarProgram,
							format!("{function:?} references unknown scalar value {operand}"),
						) }) }) .collect::<OperationResult<Vec<_>>>()?; let replacement = self.apply(instruction.opcode, &operands)?;
			if replacement.dtype != instruction.dtype { return Err(OperationError::new( OperationErrorKind::InvalidScalarProgram,
					format!("{function:?} changed an instruction result dtype"),
				)); }
			values.insert(instruction.result, replacement); }
		let output = program.outputs.first().ok_or_else(|| { OperationError::new( OperationErrorKind::InvalidScalarProgram,
				format!("{function:?} exposes no scalar output"),
			) })?; values.get(output).copied().ok_or_else(|| { OperationError::new( OperationErrorKind::InvalidScalarProgram,
				format!("{function:?} exposes an unknown scalar output"),
			) }) }

	fn finish(self, outputs: &[Value]) -> OperationResult<ScalarProgram> { let program = ScalarProgram {
			inputs: self.inputs, constants: self.constants, instructions: self.instructions,
			outputs: outputs.iter().map(|output| output.id).collect(), }; program.validate().map_err(|errors| {
			OperationError::new(OperationErrorKind::InvalidScalarProgram, errors.to_string()) })?; Ok(program) }

	fn next(&mut self, dtype: DType) -> OperationResult<Value> { let id = self.next_value;
		self.next_value = self.next_value.checked_add(1).ok_or_else(|| { OperationError::new(
				OperationErrorKind::InvalidScalarProgram,
				"scalar value identity space exhausted",
			) })?; Ok(Value { id: ScalarValueId::new(id), dtype, }) } }
