...
use recipe_core::{DType, ScalarConstant, ScalarInput, ScalarInstruction, ScalarLiteral, ScalarOpcode, ScalarProgram, ScalarValueId};
use recipe_math::MathFunction;

use crate::{CanonicalDTypeContract, LoweringAvailability, OperationDescriptor, OperationError, OperationErrorKind, OperationResult};
...
impl ScalarRecipe {
	pub(crate) fn for_symbol(symbol: &str) -> Option<Self> {
		let recipe = match symbol {
			...
			"gpu_add_into" | "gpu_add_scalar" | "gpu_bias_add" | "gpu_bias_add_f32" | "gpu_add_f16" => Self::opcode(ScalarOpcode::Add, F32_2),
			...
			"gpu_broadcast_mul" | "gpu_mul" | "gpu_mul_f16" | "gpu_row_scale" => Self::opcode(ScalarOpcode::Multiply, F32_2),
			"gpu_mul_inplace" | "gpu_scale_inplace" | "gpu_scale_f64_inplace" => Self::Composite(CompositeScalar::ReverseMultiply),
			"gpu_broadcast_sub_into" | "gpu_sub" | "gpu_sub_scalar" => Self::opcode(ScalarOpcode::Subtract, F32_2),
			...
			"gpu_gelu_backward_f32" | "gpu_gelu_backward_into" => Self::Composite(CompositeScalar::GeluTanhBackward),
		...
		match self {
			Self::Opcode { opcode, inputs } => {
				CanonicalDTypeContract::Exact {
					...
					outputs: if matches!(
						opcode,
						ScalarOpcode::Equal | ScalarOpcode::NotEqual | ScalarOpcode::LessThan | ScalarOpcode::LessThanOrEqual | ScalarOpcode::GreaterThan | ScalarOpcode::GreaterThanOrEqual
					) {
						I32_OUTPUT
					} else {
						F32_OUTPUT
...
impl CompositeScalar {
	const fn inputs(self) -> &'static [DType] {
		match self {
			...
			Self::ReverseAdd | Self::ReverseMultiply | Self::ReverseSubtract | Self::ReverseDivide | Self::ReluBackward | Self::LeakyRelu | Self::Elu | Self::SigmoidBackward | Self::TanhBackward | Self::SiluBackward | Self::GeluTanhBackward | Self::GeluTanhMultiply | Self::GluGeluTanh | Self::GluSilu | Self::ScaledExp | Self::KullbackLeiblerElement | Self::MeanAbsoluteErrorGradient => F32_2,
			Self::DifferenceScaled | Self::Clamp | Self::ClampLegacyOrder | Self::BinaryCrossEntropyGradient | Self::LeakyReluBackward | Self::EluBackward | Self::Selu | Self::Reparameterize | Self::HuberGradient | Self::SgdAdd | Self::SgdSubtract => F32_3,
	...
	const fn definition(self) -> &'static str {
		match self {
			...
			Self::BinaryCrossEntropyGradient => "((clamp(prediction) - target) / (prediction * (1 - prediction))) * inverse_count",
...
pub fn lower_scalar(descriptor: OperationDescriptor) -> OperationResult<ScalarProgram> {
	let recipe = match descriptor.lowering {
		...
		LoweringAvailability::Primitive(_) | LoweringAvailability::Composition(_) | LoweringAvailability::Workspace(_) | LoweringAvailability::NonCalculation(_) => {
	...
	let program = match recipe {
		...
		ScalarRecipe::Math(function) => ScalarProgram::try_from(function).map_err(|error| invalid_program(descriptor, error.to_string()))?,
		ScalarRecipe::Composite(composite) => lower_composite(composite).map_err(|error| error.for_operation(descriptor.id))?,
	};
	program
		.validate()
...
fn lower_composite(composite: CompositeScalar) -> OperationResult<ScalarProgram> {
	...
	let output = match composite {
		...
		CompositeScalar::BinaryCrossEntropyGradient => binary_cross_entropy_gradient(&mut composer, values[0], values[1], values[2])?,
...
fn binary_cross_entropy_gradient(composer: &mut Composer, prediction: Value, target: Value, inverse_count: Value) -> OperationResult<Value> {
...
fn leaky_relu_backward(composer: &mut Composer, gradient: Value, activation: Value, alpha: Value) -> OperationResult<Value> {
	let zero = composer.f32(0.0)?;
	let positive = composer.binary(ScalarOpcode::GreaterThan, activation, zero)?;
...
impl Composer {
	...
	fn inline_math(&mut self, function: MathFunction, arguments: &[Value]) -> OperationResult<Value> {
		let program = ScalarProgram::try_from(function).map_err(|error| OperationError::new(OperationErrorKind::InvalidScalarProgram, error.to_string()))?;
	...
	fn finish(self, outputs: &[Value]) -> OperationResult<ScalarProgram> {
		...
		program
			.validate()
			.map_err(|errors| OperationError::new(OperationErrorKind::InvalidScalarProgram, errors.to_string()))?;
		Ok(program)
	}
