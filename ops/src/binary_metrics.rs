...
use recipe_language::{AtomicOrdering, AxisSet, CalculationGraph, CalculationNode, Elementwise, Gather, Histogram, IndexBounds, IndexMap, PrimitiveAliasRule, PrimitiveKernel, PrimitiveKind, Reduce, ReduceOperator, ReduceResult, ScalarExpression, ScalarProgramBuilder, Scan, ScanMode, Shape, Sort, SortDirection, Tensor};
...
pub fn binary_metric_requirements(examples: u64, recall_thresholds: usize, calibration_bins: u32) -> OperationResult<BinaryMetricRequirements> {
...
pub fn materialize_binary_classification_metrics(request: &BinaryClassificationMetricRequest) -> OperationResult<BinaryMetricMaterialization> {
...
impl MetricEmitter {
	fn new(request: &BinaryClassificationMetricRequest, requirements: BinaryMetricRequirements) -> OperationResult<Self> {
...
fn scalar_unary(builder: &mut ScalarProgramBuilder, opcode: ScalarOpcode, value: ScalarExpression) -> OperationResult<ScalarExpression> {
...
fn scalar_binary(builder: &mut ScalarProgramBuilder, opcode: ScalarOpcode, left: ScalarExpression, right: ScalarExpression) -> OperationResult<ScalarExpression> {
...
fn scalar_finish(builder: ScalarProgramBuilder, outputs: &[ScalarExpression]) -> OperationResult<recipe_core::ScalarProgram> {
