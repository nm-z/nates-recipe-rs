...
use recipe_language::{AxisSet, CalculationGraph, CalculationNode, Elementwise, Gather, IndexBounds, PrimitiveAliasRule, PrimitiveKernel, PrimitiveKind, Reduce, ReduceOperator, ReduceResult, ScalarProgramBuilder, Scatter, ScatterConflict, Shape, Tensor};

use crate::{CompositionStep, IterationBound, LoweringAvailability, OperationDescriptor, OperationError, OperationErrorKind, OperationId, OperationResult, PrimitiveFamily};
...
impl IdentityNamespace {
	#[must_use]
	pub const fn new(first_value: ValueId, value_capacity: u64, first_kernel: KernelTemplateId, kernel_capacity: u64) -> Self {
		Self {
			first_value,
			value_capacity,
			first_kernel,
...
impl<'a> MaterializationRequest<'a> {
	#[must_use]
	pub const fn new(descriptor: OperationDescriptor, inputs: &'a [NamedTensor<'a>], outputs: &'a [NamedTensor<'a>], iteration_shape_input: &'a str, parameters: &'a PreparedParameters, identity_namespace: IdentityNamespace, workspace_limit: ByteCount) -> Self {
		Self {
			descriptor,
			inputs,
			outputs,
...
pub fn expand_composition(descriptor: OperationDescriptor, iteration_shape: &Shape, parameters: &PreparedParameters) -> OperationResult<ResolvedComposition> {
...
impl<'a> Expansion<'a> {
	...
	fn expand(&mut self, steps: &[CompositionStep], iterations: &[ResolvedIteration]) -> OperationResult<()> {
		for step in steps {
			match step {
				CompositionStep::Primitive { family, role } => self.push_primitive(*family, role, iterations)?,
	...
	fn push_primitive(&mut self, family: PrimitiveFamily, role: &'static str, iterations: &[ResolvedIteration]) -> OperationResult<()> {
...
impl GraphBuilder {
	fn new(request: &MaterializationRequest<'_>) -> OperationResult<Self> {
		let tensors = request
			...
			.collect::<Vec<_>>();
		let (first_value, value_end, first_kernel, kernel_end) = identity_ranges(request.identity_namespace).map_err(|error| error.for_operation(request.descriptor.id))?;
	...
	fn intermediate(&mut self, dtype: DType, shape: Shape) -> OperationResult<ValueId> {
		...
		self.next_value += 1;
		let tensor = Tensor::contiguous(value, dtype, shape.clone(), false, false).map_err(|error| language_error(self.operation, error.to_string()))?;
	...
	fn emit(&mut self, inputs: Vec<ValueId>, outputs: Vec<ValueId>, kind: PrimitiveKind) -> OperationResult<KernelTemplateId> {
...
fn emit_triangular_solve(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	...
	let scalar_shape = Shape::new(vec![1]).map_err(|error| language_error(request.descriptor.id, error.to_string()))?;
...
fn emit_checked_gather_identity(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, values_name: &str, indices_name: &str, output_name: &str, axis_parameter: &str, required_fact: Option<&str>) -> OperationResult<()> {
...
fn emit_batch_normalization_inference(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	require_exact_abi(
		request,
		&[
			"values",
	...
	let expected = Shape::broadcast_result(&inputs.map(|tensor| &tensor.shape)).map_err(|error| language_error(request.descriptor.id, error.to_string()))?;
...
fn emit_batch_normalization_forward(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	require_exact_abi(
		request,
		&["values", "scale", "bias"],
		&[
	...
	let channel_shape = Shape::new(vec![columns]).map_err(|error| language_error(request.descriptor.id, error.to_string()))?;
	...
	let channel_axis = AxisSet::new(vec![0]).map_err(|error| language_error(request.descriptor.id, error.to_string()))?;
	emitter.emit(
		vec![statistic_values],
		vec![sums],
...
fn emit_batch_normalization_backward(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	require_exact_abi(
		request,
		&[
			"output_gradient",
...
fn emit_batch_normalization_running_update(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	require_exact_abi(
		request,
		&[
			"saved_mean",
...
fn emit_layer_normalization_backward(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	require_exact_abi(
		request,
...
fn emit_rms_normalization_backward(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>) -> OperationResult<()> {
	require_exact_abi(
		request,
		&["output_gradient", "values", "scale"],
...
fn emit_adam_update(request: &MaterializationRequest<'_>, emitter: &mut Emitter<'_>, decoupled_weight_decay: bool) -> OperationResult<()> {
...
fn normalization_statistics_program(operation: OperationId, divisor: f32, epsilon: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
	let sum = scalar_input(operation, &mut builder, DType::F32)?;
...
fn batch_normalization_gradient_program(operation: OperationId, divisor: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
	let output_gradient = scalar_input(operation, &mut builder, DType::F32)?;
	let value = scalar_input(operation, &mut builder, DType::F32)?;
	let mean = scalar_input(operation, &mut builder, DType::F32)?;
...
fn running_statistics_update_program(operation: OperationId, momentum: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
...
fn layer_normalization_program(operation: OperationId, divisor: f32, epsilon: f32, has_scale: bool, has_bias: bool) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
	let value = scalar_input(operation, &mut builder, DType::F32)?;
	let sum = scalar_input(operation, &mut builder, DType::F32)?;
	let variance_sum = scalar_input(operation, &mut builder, DType::F32)?;
...
fn layer_normalization_backward_terms_program(operation: OperationId, divisor: f32, epsilon: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
	let output_gradient = scalar_input(operation, &mut builder, DType::F32)?;
	let value = scalar_input(operation, &mut builder, DType::F32)?;
	let scale = scalar_input(operation, &mut builder, DType::F32)?;
...
fn layer_normalization_input_gradient_program(operation: OperationId, divisor: f32, epsilon: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
	let output_gradient = scalar_input(operation, &mut builder, DType::F32)?;
	let value = scalar_input(operation, &mut builder, DType::F32)?;
	let scale = scalar_input(operation, &mut builder, DType::F32)?;
...
fn rms_normalization_program(operation: OperationId, divisor: f32, epsilon: f32, has_scale: bool) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
	let value = scalar_input(operation, &mut builder, DType::F32)?;
...
fn rms_dot_term_program(operation: OperationId, divisor: f32, epsilon: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
	let output_gradient = scalar_input(operation, &mut builder, DType::F32)?;
	let value = scalar_input(operation, &mut builder, DType::F32)?;
	let square_sum = scalar_input(operation, &mut builder, DType::F32)?;
...
fn rms_normalization_backward_program(operation: OperationId, divisor: f32, epsilon: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
	let output_gradient = scalar_input(operation, &mut builder, DType::F32)?;
	let value = scalar_input(operation, &mut builder, DType::F32)?;
	let scale = scalar_input(operation, &mut builder, DType::F32)?;
...
fn inverse_standard_deviation(operation: OperationId, builder: &mut ScalarProgramBuilder, sum: recipe_language::ScalarExpression, divisor: recipe_language::ScalarExpression, epsilon: f32) -> OperationResult<recipe_language::ScalarExpression> {
...
fn momentum_program(operation: OperationId, momentum: f32, learning_rate: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
...
fn adaptive_weight_program(operation: OperationId, learning_rate: f32, epsilon: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
...
fn adam_state_program(operation: OperationId, beta_one: f32, beta_two: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
...
fn adam_weight_program(operation: OperationId, parameters: AdamMomentParameters, learning_rate: f32, weight_decay: f32, decoupled_weight_decay: bool) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
	let weight = scalar_input(operation, &mut builder, DType::F32)?;
	let first_moment = scalar_input(operation, &mut builder, DType::F32)?;
	let second_moment = scalar_input(operation, &mut builder, DType::F32)?;
...
fn nadam_weight_program(operation: OperationId, parameters: AdamMomentParameters, learning_rate: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
	let weight = scalar_input(operation, &mut builder, DType::F32)?;
	let gradient = scalar_input(operation, &mut builder, DType::F32)?;
	let first_moment = scalar_input(operation, &mut builder, DType::F32)?;
...
fn lion_state_program(operation: OperationId, beta_one: f32, beta_two: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
...
fn lion_weight_program(operation: OperationId, learning_rate: f32, weight_decay: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
	let weight = scalar_input(operation, &mut builder, DType::F32)?;
...
fn lamb_phase_one_program(operation: OperationId, parameters: AdamMomentParameters, weight_decay: f32) -> OperationResult<recipe_core::ScalarProgram> {
	let mut builder = scalar_builder(operation)?;
	let weight = scalar_input(operation, &mut builder, DType::F32)?;
	let moment = scalar_input(operation, &mut builder, DType::F32)?;
	let velocity = scalar_input(operation, &mut builder, DType::F32)?;
...
fn normalization_matrix(request: &MaterializationRequest<'_>, tensor: &Tensor, name: &str) -> OperationResult<(u64, u64, f32, f32)> {
	require_dtype(request, tensor, DType::F32, name)?;
...
fn adam_moment_parameters(request: &MaterializationRequest<'_>) -> OperationResult<AdamMomentParameters> {
	...
	let step = i32::try_from(step).map_err(|error| request_error(request, format!("step does not fit int32: {error}")))?;
	let next_step = step
		.checked_add(1)
...
fn require_equal_f32_tensors<'a>(request: &MaterializationRequest<'_>, reference: &Tensor, tensors: impl IntoIterator<Item = (&'a str, &'a Tensor)>) -> OperationResult<()> {
...
fn unit_interval_parameter(request: &MaterializationRequest<'_>, name: &str, include_one: bool) -> OperationResult<f32> {
	let value = prepared_f32(request, name)?;
...
fn sum_reduction(request: &MaterializationRequest<'_>, axes: AxisSet, keep_dimensions: bool, tree_lanes: u32) -> OperationResult<PrimitiveKind> {
	let rank = request
		.inputs
		.iter()
...
fn scalar_f32(operation: OperationId, builder: &mut ScalarProgramBuilder, value: f32) -> OperationResult<recipe_language::ScalarExpression> {
	builder
		.f32(value)
		.map_err(|error| language_error(operation, error.to_string()))
}

fn scalar_unary(operation: OperationId, builder: &mut ScalarProgramBuilder, opcode: ScalarOpcode, value: recipe_language::ScalarExpression) -> OperationResult<recipe_language::ScalarExpression> {
	builder
		.unary(opcode, value)
		.map_err(|error| language_error(operation, error.to_string()))
}

fn scalar_ternary(operation: OperationId, builder: &mut ScalarProgramBuilder, opcode: ScalarOpcode, first: recipe_language::ScalarExpression, second: recipe_language::ScalarExpression, third: recipe_language::ScalarExpression) -> OperationResult<recipe_language::ScalarExpression> {
	builder
		.ternary(opcode, first, second, third)
		.map_err(|error| language_error(operation, error.to_string()))
}

fn require_nonnegative(operation: OperationId, builder: &mut ScalarProgramBuilder, value: recipe_language::ScalarExpression) -> OperationResult<()> {
...
fn require_positive(operation: OperationId, builder: &mut ScalarProgramBuilder, value: recipe_language::ScalarExpression) -> OperationResult<()> {
	let zero = scalar_f32(operation, builder, 0.0)?;
...
fn bias_correct(operation: OperationId, builder: &mut ScalarProgramBuilder, value: recipe_language::ScalarExpression, correction: f32) -> OperationResult<recipe_language::ScalarExpression> {
...
fn require_exact_abi(request: &MaterializationRequest<'_>, input_names: &[&str], output_names: &[&str], parameter_names: &[&str]) -> OperationResult<()> {
	...
	if actual_parameters != expected_parameters {
		return Err(request_error(
			request,
			format!("prepared parameter ABI mismatch: expected {expected_parameters:?}, got {actual_parameters:?}"),
		));
	}
	Ok(())
}
...
fn require_dtype(request: &MaterializationRequest<'_>, tensor: &Tensor, expected: DType, name: &str) -> OperationResult<()> {
...
fn require_shape(request: &MaterializationRequest<'_>, tensor: &Tensor, expected: &[u64], name: &str) -> OperationResult<()> {
...
fn require_same_tensor_contract(request: &MaterializationRequest<'_>, expected: &Tensor, actual: &Tensor, name: &str) -> OperationResult<()> {
...
fn prepared_tree_lanes(request: &MaterializationRequest<'_>) -> OperationResult<u32> {
	...
	let lanes = u32::try_from(value).map_err(|error| request_error(request, format!("tree_lanes does not fit u32: {error}")))?;
...
fn scalar_input(operation: OperationId, builder: &mut ScalarProgramBuilder, dtype: DType) -> OperationResult<recipe_language::ScalarExpression> {
	builder
		.input(dtype)
		.map_err(|error| language_error(operation, error.to_string()))
}

fn scalar_binary(operation: OperationId, builder: &mut ScalarProgramBuilder, opcode: ScalarOpcode, left: recipe_language::ScalarExpression, right: recipe_language::ScalarExpression) -> OperationResult<recipe_language::ScalarExpression> {
	builder
		.binary(opcode, left, right)
		.map_err(|error| language_error(operation, error.to_string()))
}

fn scalar_finish(operation: OperationId, builder: ScalarProgramBuilder, outputs: &[recipe_language::ScalarExpression]) -> OperationResult<recipe_core::ScalarProgram> {
	builder
		.finish(outputs)
		.map_err(|error| language_error(operation, error.to_string()))
}
...
fn has_concrete_materializer(descriptor: OperationDescriptor) -> bool { optimizer_normalization::supports(descriptor) || solver_fft::supports(descriptor) || attention_sequence_embedding::supports(descriptor) || convolution_pooling::supports(descriptor) || loss_metrics::supports(descriptor) || indexing_sort_encoding::supports(descriptor) || graph_cluster_rl::supports(descriptor) || tree_boosting::supports(descriptor) || inference_quantization_diffusion::supports(descriptor) || creation_shape_misc::supports(descriptor) || training::supports(descriptor) }
