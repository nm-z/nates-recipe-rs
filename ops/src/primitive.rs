...
use recipe_language::{AtomicOperation, AtomicOrdering, IndexBounds, PrimitiveKernel, PrimitiveKind, ReduceOperator, ReduceResult, ScanMode, ScatterConflict, SortDirection, Tensor};
use recipe_primitives::{LoweredProgram, LoweringHardware};

use crate::{CanonicalDTypeContract, LoweringAvailability, OperationDescriptor, OperationError, OperationErrorKind, OperationFamily, OperationResult};
...
impl PrimitiveRecipe {
	...
	pub const fn operation_family(self) -> OperationFamily {
		match self.family() {
			...
			PrimitiveFamily::Gather | PrimitiveFamily::Scatter | PrimitiveFamily::Sort | PrimitiveFamily::IndexMap => OperationFamily::ShapeAndIndexing,
	...
	pub const fn definition(self) -> &'static str {
		match self {
			...
			Self::Contraction(ContractionClass::LeftTransposedMatrix) => "binary32 left-transposed matrix contraction",
			Self::Contraction(ContractionClass::RightTransposedMatrix) => "binary32 right-transposed matrix contraction",
			...
			Self::IndexMap => "checked iteration-aware affine int32 index map with optional positive Euclidean modulus",
	...
	fn matches(self, kernel: &PrimitiveKernel, tensors: &BTreeMap<ValueId, &Tensor>) -> bool {
		match (self, &kernel.kind) {
			...
			) => operator == spec.operator && result == spec.result && axes_match(axes, spec.axes.as_slice(), kernel, tensors),
			(
				Self::Scan {
					operator,
					exclusive,
			...
			) => operator == spec.operator && exclusive == matches!(spec.mode, ScanMode::Exclusive { .. }) && axis_matches(axis, spec.axis, kernel, tensors) && !spec.reverse && scan_identity_matches(spec),
			(Self::Contraction(class), PrimitiveKind::Contraction(spec)) => contraction_matches(class, spec, kernel, tensors),
			(Self::Gather { axis }, PrimitiveKind::Gather(spec)) => axis_matches(axis, spec.axis, kernel, tensors) && spec.bounds == IndexBounds::Reject,
			(Self::ScatterAdd { axis }, PrimitiveKind::Scatter(spec)) => {
				axis_matches(axis, spec.axis, kernel, tensors)
					...
					.alias_rules
					.iter()
					.any(|rule| rule.input == 0 && rule.output == 0 && rule.permission == AliasPermission::MustAliasExact)
			}
			(
				Self::Sort {
					direction,
			...
			) => direction == spec.direction && stable == spec.stable && axis_matches(axis, spec.axis, kernel, tensors) && !spec.emit_indices,
			...
			(Self::Reduce { .. } | Self::Scan { .. } | Self::Contraction(_) | Self::Gather { .. } | Self::ScatterAdd { .. } | Self::Sort { .. } | Self::IndexMap | Self::Random(_), PrimitiveKind::Elementwise(_) | PrimitiveKind::Reduce(_) | PrimitiveKind::Scan(_) | PrimitiveKind::Contraction(_) | PrimitiveKind::Gather(_) | PrimitiveKind::Scatter(_) | PrimitiveKind::Histogram(_) | PrimitiveKind::Sort(_) | PrimitiveKind::IndexMap(_) | PrimitiveKind::Random(_)) => false,
		}
	}
}
...
pub fn lower_primitive(descriptor: OperationDescriptor, request: PrimitiveRequest<'_>, hardware: LoweringHardware) -> OperationResult<LoweredProgram> {
	let recipe = match descriptor.lowering {
		LoweringAvailability::Primitive(recipe) => recipe,
		LoweringAvailability::Scalar(_) | LoweringAvailability::Composition(_) | LoweringAvailability::Workspace(_) | LoweringAvailability::NonCalculation(_) => {
...
fn lower_recipe(recipe: PrimitiveRecipe, request: PrimitiveRequest<'_>, hardware: LoweringHardware) -> OperationResult<LoweredProgram> {
...
fn axes_match(requirement: AxisRequirement, axes: &[usize], kernel: &PrimitiveKernel, tensors: &BTreeMap<ValueId, &Tensor>) -> bool {
...
fn axis_matches(requirement: AxisRequirement, axis: usize, kernel: &PrimitiveKernel, tensors: &BTreeMap<ValueId, &Tensor>) -> bool {
	match requirement {
...
fn scan_identity_matches(spec: &recipe_language::Scan) -> bool {
	match (spec.operator, spec.mode) {
		...
		(ReduceOperator::Minimum | ReduceOperator::Maximum | ReduceOperator::Any | ReduceOperator::All, ScanMode::Exclusive { .. })
		| (
...
fn contraction_matches(class: ContractionClass, spec: &recipe_language::Contraction, kernel: &PrimitiveKernel, tensors: &BTreeMap<ValueId, &Tensor>) -> bool {
	let ranks = kernel
		.inputs
		.first()
	...
	match class {
		ContractionClass::Vector => ranks == Some((1, 1)) && spec.batch_axes.is_empty() && spec.contract_axes == [(0, 0)],
		ContractionClass::Matrix => ranks == Some((2, 2)) && spec.batch_axes.is_empty() && spec.contract_axes == [(1, 0)],
		ContractionClass::LeftTransposedMatrix => ranks == Some((2, 2)) && spec.batch_axes.is_empty() && spec.contract_axes == [(0, 0)],
		ContractionClass::RightTransposedMatrix => ranks == Some((2, 2)) && spec.batch_axes.is_empty() && spec.contract_axes == [(1, 1)],
		ContractionClass::Batched => ranks.is_some_and(|(left, right)| left >= 3 && right >= 3) && !spec.batch_axes.is_empty() && spec.contract_axes.len() == 1,
	}
}
