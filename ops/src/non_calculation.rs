	...
	pub const fn definition(self) -> &'static str {
		match self { Self::FacadeDeclaration => {
			...
			Self::TextTokenization => {
				"deterministic raw-text to int32 token-ID metadata transformation performed before payload admission"
			}
			Self::ModelContainerParsing => {
				"validated container parsing that constructs tensor metadata and opaque byte ranges before admission"
			}
			Self::ChatTemplateRendering => {
				"deterministic raw-text template rendering performed before tokenization and payload admission"
			}
			Self::RunShutdown => {
				"exit lifecycle action replaced by the typed one-free-per-device executor transition"
			}
