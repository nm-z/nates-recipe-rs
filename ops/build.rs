...
fn main() {
	...
	let mut parsed = Vec::new(); let mut totals = BTreeMap::<String, u16>::new();
	for (index, raw_line) in source.lines().enumerate() { let line_number = index + 1; let line = raw_line.trim_end();
		...
		); assert!( !symbol.is_empty() && !source.is_empty(),
			"operation-surface.txt:{line_number}: fields must be nonempty"
		); let total = totals.entry(symbol.to_owned()).or_default(); *total = total .checked_add(1)
	...
	for (ordinal, (line, symbol, source)) in parsed.iter().enumerate() {
		let occurrence = seen.entry(symbol.clone()).or_default(); *occurrence = occurrence .checked_add(1)
			.expect("symbol occurrence count fits u16");
		let total = totals[symbol]; generated.push_str(&format!(
