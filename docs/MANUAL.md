...
Widest-steady, not narrowest, is deliberate. A spurious separator that happens to appear a constant number of times per row (one `;` inside a text cell on both probe rows) yields FEWER fields than the real one, so widest ignores it; narrowest would hijack a comma CSV that parses correctly today. The cost is the European decimal-comma dialect — `1,5;2,5;3,5` is steady at 3 on `;` and at 4 on `,`, so comma wins and the file loads as four columns. Raw counting picked comma there too; no dataset in `datasets/` is affected. Whitespace-run delimiters are NOT sniffed: `uci-ecoli/ecoli.data`, `uci-yeast/yeast.data`, `uci-german-numeric`, `uci-satimage/sat.trn`, `uci-shuttle/shuttle.trn` and both `uci-har-sensor` matrices collapse to one column, and would need `\s+` added to the candidate set.
