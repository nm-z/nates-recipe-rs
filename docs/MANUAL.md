...
The separator is a property of the file, never an argument — `Data::load` takes no delimiter and never will. `sniff_delimiter` takes the first two non-blank lines, parses them quote-aware at each of `;` `\t` `,`, and keeps every delimiter that yields the SAME field count on both rows with width > 1. **Comma wins whenever it is steady; a non-comma delimiter must be the only steady candidate.** A table has equal fields on every row, so steadiness identifies the separator where counting cannot: `uci-sms-tab/SMSSpamCollection` is a TSV whose message text carries commas (tab 2,2 vs comma 2,1), so tab is the only steady candidate. When nothing is steady — one record, one column, ragged rows — the first-line majority count decides, ties to comma. Both `csv::ReaderBuilder` sites sniff (`read_raw_csv`, which funnels file/dir/zip loading, and `detect.rs::prefix_columns`), so column-type detection and encoding can never disagree about a file's shape. ARFF is exempt: `@data` rows and `{a,b,c}` nominal specs are comma-delimited by the format, so `split_fields` stays comma.

The comma bias is the whole point, and it is asymmetric on purpose. Picking the WIDEST steady delimiter instead looks reasonable and is wrong: `1,"a;b;c;d",2` repeated is steady at 4 on `;` and at 3 on `,`, so widest shatters a comma CSV that parsed correctly before any sniffing existed. No rule on field counts alone can also get the European decimal-comma dialect right — `1,5;2,5;3,5` is steady at 3 on `;` and 4 on `,`, and comma wins, loading four columns. That dialect needs an explicit hint; raw counting got it wrong too, and no dataset in `datasets/` is affected. Two delimiters steady at the same width is genuinely ambiguous (`a\tb, c` / `d\te, f`), and comma takes it.

Whitespace-run delimiters are NOT sniffed: `uci-ecoli/ecoli.data`, `uci-yeast/yeast.data`, `uci-german-numeric`, `uci-satimage/sat.trn`, `uci-shuttle/shuttle.trn` and both `uci-har-sensor` matrices collapse to one column, and would need `\s+` added to the candidate set.
