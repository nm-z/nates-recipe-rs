...
use recipe_core::{ArtifactBuildRecipe, ArtifactId, ArtifactIdentity, CapacityLedger, Device, Digest, DiscoveredDevice, DiscoveryProfile, Label, ReservationEntry, ReservationEvidence, ReservationLedger, ReservationMechanism, TargetIdentity, ToolchainIdentity, Topology};
use recipe_cuda::{ComputeCapability, DriverSymbol, DriverVersion, ToolchainIdentity as CudaToolchainIdentity};
use recipe_kernel::{ArtifactBuilder, BuildPhase, KernelTarget, LoweringError, LoweringOptions, inspect_cubin, inspect_hsaco_bundle};
use recipe_language::CalculationGraph;
use recipe_native_executor::{CandidateArtifact as ExecutorCandidateArtifact, CandidateFailure as ExecutorCandidateFailure, CandidateRealizationRequest as ExecutorCandidateRequest, CandidateSessionFactory, RuntimeArtifact, RuntimeArtifactKind, RuntimeImage, ValidatedCandidateFactory, ValidatedCandidateSession};
use recipe_planner::PlannedCandidate;
use recipe_probe::{MeasuredProfile, validate_profile};

use crate::{ArtifactProvider, CandidateRealizer, PreparationPolicy, RealizationObservation, RealizeFailure, RealizedCandidate};
...
impl ArtifactProvider for NativeArtifactProvider {
	...
	fn resolve(&mut self, graph: &CalculationGraph, topology: &Topology, discovery: &DiscoveryProfile) -> Result<Self::Catalog, Self::Error> {
		graph.validate()
	...
	fn validate(&self) -> Result<(), NativePrepareError<Infallible>> {
		...
		match (&self.target, &self.runtime_policy) {
			...
			(KernelTarget::Amd(target), RuntimeArtifactPolicy::Hsa) => {
				...
				let architecture_matches = architecture == target.target_id || architecture == full_architecture;
				require(
					self.target_identity.backend.as_str() == HSA_BACKEND && self.target_identity.abi.as_str() == expected_abi && architecture_matches,
					NativePrepareError::InvalidConfiguration(format!(
			...
			(KernelTarget::Nvidia(_), RuntimeArtifactPolicy::Hsa) | (KernelTarget::Amd(_), RuntimeArtifactPolicy::Cuda(_)) => {
...
impl DeferredArtifactCompiler {
	...
	pub fn with_prebuilt_bundle(mut self, target: TargetIdentity, bytes: Arc<[u8]>) -> Result<Self, NativePrepareError<Infallible>> {
		require(
			!bytes.is_empty(),
	...
	fn materialize(&self, candidate: &PlannedCandidate, catalog: &NativeArtifactCatalog, discovery: &DiscoveryProfile) -> Result<Vec<NativeArtifact>, NativePrepareError<Infallible>> {
		let mut artifacts = Vec::with_capacity(
			candidate
				...
				.ok_or_else(|| NativePrepareError::InvalidCandidate("artifact count overflowed usize".to_owned()))?,
		);
		...
		for (spec_index, mut builds) in deferred_by_target {
			...
			let spec = self
				.targets
				.get(spec_index)
				.ok_or_else(|| NativePrepareError::InvalidConfiguration("grouped build specification index is outside the target table".to_owned()))?;
			match &spec.target {
				KernelTarget::Amd(_) => {
	...
	fn build_spec_index(&self, candidate: &PlannedCandidate, discovery: &DiscoveryProfile, build: &ArtifactBuildRecipe) -> Result<usize, NativePrepareError<Infallible>> {
	...
	fn build_nvidia_bundle(&self, candidate: &PlannedCandidate, spec: &TargetBuildSpec, builds: &[&ArtifactBuildRecipe]) -> Result<Vec<NativeArtifact>, NativePrepareError<Infallible>> {
		...
		if let Some(bytes) = self.prebuilt_bundles.get(&spec.target_identity) {
			let expected_sm = target
				...
				.ok_or_else(|| NativePrepareError::InvalidConfiguration("NVIDIA SM identity overflowed u8".to_owned()))?;
			for kernel in &lowered {
				inspect_cubin(bytes, expected_sm, &kernel.abi.entry_symbol).map_err(NativePrepareError::Lowering)?;
			}
		...
		require(
			...
			NativePrepareError::InvalidArtifact("NVIDIA cubin bundle inspection count differs from its lowered entries".to_owned()),
		)?;
	...
	fn build_amd_bundle(&self, candidate: &PlannedCandidate, spec: &TargetBuildSpec, builds: &[&ArtifactBuildRecipe]) -> Result<Vec<NativeArtifact>, NativePrepareError<Infallible>> {
		...
		require(
			built.inspections.len() == lowered.len(),
			NativePrepareError::InvalidArtifact("AMD code-object bundle inspection count differs from its lowered entries".to_owned()),
		)?;
		for (inspection, kernel) in built.inspections.iter().zip(&lowered) {
			require(
...
fn lower_deferred_stage(candidate: &PlannedCandidate, spec: &TargetBuildSpec, build: &ArtifactBuildRecipe) -> Result<recipe_kernel::LoweredKernel, NativePrepareError<Infallible>> {
	let program = candidate
		.lowered_programs
		.iter()
		.find(|program| program.source_kernel == build.source_kernel && Digest::new(program.digest.bytes()) == build.provenance.program_digest)
		.ok_or_else(|| {
			NativePrepareError::InvalidCandidate(format!(
...
fn native_artifact_from_image(spec: &TargetBuildSpec, build: &ArtifactBuildRecipe, lowered: recipe_kernel::LoweredKernel, image: RuntimeImage) -> Result<NativeArtifact, NativePrepareError<Infallible>> {
...
fn runtime_kind(spec: &TargetBuildSpec, digest: [u8; 32]) -> Result<RuntimeArtifactKind, NativePrepareError<Infallible>> {
...
fn candidate_target(candidate: &PlannedCandidate, discovery: &DiscoveryProfile, artifact: ArtifactId) -> Result<TargetIdentity, NativePrepareError<Infallible>> {
...
fn validate_native_artifact(identity: &ArtifactIdentity, runtime: &RuntimeArtifact) -> Result<(), NativePrepareError<Infallible>> {
	identity
		.validate()
	...
	match runtime.kind() {
		RuntimeArtifactKind::Cuda { identity: cuda } => {
			require(
				identity.target.backend.as_str() == CUDA_BACKEND && identity.target.abi.as_str() == CUDA_ABI && identity.target.architecture.as_str() == cuda.target.to_string() && cuda.sha256 == digest,
				NativePrepareError::InvalidArtifact(format!(
		...
		RuntimeArtifactKind::Hsa {
			...
			require(
				identity.target.backend.as_str() == HSA_BACKEND && identity.target.abi.as_str() == abi && identity.target.architecture.as_str() == target_id,
				NativePrepareError::InvalidArtifact(format!(
...
pub trait CandidateDriver {
	...
	fn reservation_mechanism(&self, device: &Device, discovered: &DiscoveredDevice) -> Result<ReservationMechanism, Self::Error>;

	fn reservation_evidence(&self, device: &Device, discovered: &DiscoveredDevice) -> Result<ReservationEvidence, Self::Error>;

	fn realize_candidate(&mut self, request: CandidateRealizationRequest<'_>) -> Result<Self::Session, CandidateDriverFailure<Self::Error>>;

	fn warm_maximum_concurrency(&mut self, session: &mut Self::Session, candidate: &PlannedCandidate, pass: u32) -> Result<(), CandidateDriverFailure<Self::Error>>;

	fn capacity_snapshot(&mut self, session: &mut Self::Session, topology: &Topology, discovery: &DiscoveryProfile) -> Result<CapacityLedger, CandidateDriverFailure<Self::Error>>;
...
{
	pub fn new(profile: &MeasuredProfile, compiler: DeferredArtifactCompiler, driver: D) -> Result<Self, NativePrepareError<D::Error>> {
...
where D: CandidateDriver
{
	...
	fn reservation_plan(&self, topology: &Topology, discovery: &DiscoveryProfile) -> Result<ReservationLedger, Self::Error> {
		self.require_profile(topology, discovery)?;
		exact_reservation_plan(&self.driver, topology, discovery)
	}

	fn realize(&mut self, candidate: &PlannedCandidate, catalog: &NativeArtifactCatalog, reservations: &ReservationLedger, policy: PreparationPolicy) -> Result<RealizedCandidate<Self::Session>, RealizeFailure<Self::Error>> {
		require(
		...
		reservations
			...
			.map_err(|errors| RealizeFailure::Fatal(NativePrepareError::InvalidReservation(errors.to_string())))?;
		let artifacts = self
			.compiler
		...
		let mut snapshots = Vec::with_capacity(
			usize::try_from(policy.stabilization_passes.get()).map_err(|conversion_error| {
...
fn exact_reservation_plan<D>(driver: &D, topology: &Topology, discovery: &DiscoveryProfile) -> Result<ReservationLedger, NativePrepareError<D::Error>>
where D: CandidateDriver {
...
where D: CandidateDriver
{
	fn require_profile(&self, topology: &Topology, discovery: &DiscoveryProfile) -> Result<(), NativePrepareError<D::Error>> {
		require(
	...
	fn reject_after_session<T>(&mut self, session: D::Session, failure: CandidateDriverFailure<D::Error>) -> Result<T, RealizeFailure<NativePrepareError<D::Error>>> {
...
fn map_driver_failure<E>(failure: CandidateDriverFailure<E>) -> RealizeFailure<NativePrepareError<E>> {
	match failure {
		...
		CandidateDriverFailure::PreFinalRealizationUnavailable { backend } => RealizeFailure::Fatal(NativePrepareError::PreFinalRealizationUnavailable { backend }),
	}
}
...
impl NativePrepareError<Infallible> {
	fn erase_driver<F>(self) -> NativePrepareError<F> {
		match self {
			...
			Self::PreFinalRealizationUnavailable { backend } => NativePrepareError::PreFinalRealizationUnavailable { backend },
		}
	}
}
...
impl<E: fmt::Display> fmt::Display for NativePrepareError<E> {
	fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			Self::ProfileMismatch => formatter.write_str("preparer inputs differ from the validated measured profile"),
...
where E: Error + 'static
{
	fn source(&self) -> Option<&(dyn Error + 'static)> {
		match self {
			...
			Self::InvalidConfiguration(_) | Self::InvalidProfile(_) | Self::ProfileMismatch | Self::InvalidCatalog(_) | Self::InvalidCandidate(_) | Self::InvalidArtifact(_) | Self::InvalidReservation(_) | Self::MissingBuildTarget(_) | Self::PreFinalRealizationUnavailable { .. } => None,
		}
	}
}
