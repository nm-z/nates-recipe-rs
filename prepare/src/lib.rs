...
use recipe_core::{ArtifactIdentity, BundleIdentity, ByteCount, CandidateIdentity, CapacityLedger, CapacityLedgerEntry, DeviceId, DiscoveryProfile, DraftPlan, EXACT_USER_RESERVATION, FinalizedBundle, LoopIterations, LoopTaskDomain, Property, PropertyProvenance, RealizationIdentity, RealizationProfile, ReservationLedger, ResourceManifest, TaskId, Topology, ValueId};
use recipe_language::CalculationGraph;
use recipe_planner::{PlannedCandidate, PlannedExternalOutput, PlannedProgramCandidate, ProgramPlannerSearch, plan_program_candidates};
...
pub use production::{CandidateDriver, CandidateDriverFailure, CandidateRealizationRequest, CudaArtifactPolicy, DeferredArtifactCompiler, NativeArtifact, NativeArtifactCatalog, NativeArtifactProvider, NativeCandidateRealizer, NativeExecutorDriver, NativePrepareError, PreparedNativeSession, RuntimeArtifactPolicy, TargetBuildSpec};
...
pub trait CandidateRealizer<C> {
	...
	fn realize(&mut self, candidate: &PlannedCandidate, catalog: &C, reservations: &ReservationLedger, policy: PreparationPolicy) -> Result<RealizedCandidate<Self::Session>, RealizeFailure<Self::Error>>;
	...
	R: CandidateRealizer<A::Catalog>,
{
	...
	pub fn prepare(&mut self, graph: &CalculationGraph, profile: &MeasuredProfile) -> Result<PreparedSystem<A::Catalog, R::Session>, PrepareError> {
		let program = StaticCalculationProgram::every_iteration(graph.clone(), LoopIterations::ONE).map_err(|error| PrepareError::new(PrepareErrorKind::Planning, error.to_string()))?;
		self.prepare_program(&program, profile)
	}

	/// Prepares a bounded calculation program decoded from Recipe OGDL or
	/// constructed directly.
	pub fn prepare_program(&mut self, program: &StaticCalculationProgram, profile: &MeasuredProfile) -> Result<PreparedSystem<A::Catalog, R::Session>, PrepareError> {
		self.policy.validate()?;
		validate_profile(profile).map_err(|error| PrepareError::new(PrepareErrorKind::InvalidMeasuredProfile, error.to_string()))?;
		self.prepare_program_validated(program, &profile.topology, &profile.discovery)
	}

	fn prepare_program_validated(&mut self, program: &StaticCalculationProgram, topology: &Topology, discovery: &DiscoveryProfile) -> Result<PreparedSystem<A::Catalog, R::Session>, PrepareError> {
...
fn reject_candidate(search: &mut ProgramPlannerSearch, rejections: &mut Vec<CandidateRejection>, candidate: CandidateIdentity, stage: CandidateRejectionStage, detail: String) -> Result<(), PrepareError> {
...
fn optimistic_planning_capacity(topology: &Topology, discovery: &DiscoveryProfile, reservations: &ReservationLedger) -> Result<CapacityLedger, PrepareError> {
...
fn validate_observation(topology: &Topology, discovery: &DiscoveryProfile, draft: &DraftPlan, observation: &RealizationObservation, policy: PreparationPolicy) -> Result<CapacityLedger, String> {
	let expected_passes = usize::try_from(policy.stabilization_passes.get()).map_err(|_| "stabilization pass count does not fit usize".to_owned())?;
	...
	let stable_tail = usize::try_from(policy.stable_tail.get()).map_err(|_| "stable-tail length does not fit usize".to_owned())?;
...
fn capacity_stability_deltas(topology: &Topology, snapshots: &[CapacityLedger], stable_start: usize, final_snapshot: &CapacityLedger) -> Vec<String> {
...
fn hash_realization(draft: &DraftPlan, observation: &RealizationObservation, policy: PreparationPolicy) -> RealizationIdentity {
...
fn hash_bundle_with_loop_domains(topology: &Topology, discovery: &DiscoveryProfile, draft: &DraftPlan, realization: &RealizationProfile, layouts: &[recipe_core::ArenaLayout], loop_iterations: LoopIterations, loop_domains: &[LoopTaskDomain]) -> BundleIdentity {
