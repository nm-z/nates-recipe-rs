...
impl Accumulator {
	...
	fn finish(self) -> DatasetSourceResult<DistilledDataset> {
		...
		let table = RawTable::from_parts(self.headers, self.rows)
			.map_err(|error| DatasetSourceError::new(DatasetSourceErrorKind::Ingest, error.to_string()))?;
		Ok(DistilledDataset {
			table,
...
pub fn distill_dataset(path: &Path, limits: IngestLimits) -> DatasetSourceResult<DistilledDataset> {
	distill_datasets([path], limits)
}
...
fn parse_delimited(
	...
	context: &FileContext,
	limits: IngestLimits,
) -> DatasetSourceResult<LogicalTable> {
