...
impl SafeTensorDType {
	fn parse(value: &str) -> SafeTensorResult<Self> {
		match value {
			"BOOL" => Ok(Self::Bool),
			"U8" => Ok(Self::U8),
			"I8" => Ok(Self::I8),
...
pub struct SafeTensorLimits {
	header_bytes: NonZeroU64,
	data_bytes: NonZeroU64,
	tensors: NonZeroU32,
	rank: NonZeroU32,
...
impl<'a> SafeTensorArchive<'a> {
	...
	pub fn encoded_tensor(&self, name: &str) -> Option<&'a [u8]> {
		...
		let begin = usize::try_from(entry.begin).ok()?;
...
pub struct SafeTensorError {
	pub kind: SafeTensorErrorKind,
	pub detail: String,
}

impl SafeTensorError {
	fn new(kind: SafeTensorErrorKind, detail: impl Into<String>) -> Self {
		Self {
			kind,
			detail: detail.into(),
		}
...
pub fn parse_safetensors(bytes: &[u8], limits: SafeTensorLimits) -> SafeTensorResult<SafeTensorArchive<'_>> {
	let length_prefix = bytes.get(..8).ok_or_else(|| {
		SafeTensorError::new(
			SafeTensorErrorKind::Truncated,
			format!(
	...
	if header_bytes > limits.header_bytes.get() {
		return Err(SafeTensorError::new(
	...
	let data_start = 8_u64.checked_add(header_bytes).ok_or_else(|| {
		SafeTensorError::new(
			SafeTensorErrorKind::ArithmeticOverflow,
			"safetensors data offset overflowed u64",
		)
	})?;
	let data_start_usize = usize::try_from(data_start).map_err(|error| {
		SafeTensorError::new(
			SafeTensorErrorKind::ArithmeticOverflow,
			format!("safetensors data offset cannot address host memory: {error}"),
		)
	})?;
	let header = bytes.get(8..data_start_usize).ok_or_else(|| {
		SafeTensorError::new(
			SafeTensorErrorKind::Truncated,
			format!(
	...
	if data_bytes > limits.data_bytes.get() {
		return Err(SafeTensorError::new(
			SafeTensorErrorKind::DataLimitExceeded,
			format!(
	...
	let tensor_count = u32::try_from(raw.tensors.len()).map_err(|error| {
		SafeTensorError::new(
			SafeTensorErrorKind::ArithmeticOverflow,
	...
	if tensor_count > limits.tensors.get() {
		return Err(SafeTensorError::new(
			SafeTensorErrorKind::TensorLimitExceeded,
			format!(
	...
	for (name, tensor) in raw.tensors {
		...
		let rank = u32::try_from(tensor.shape.len()).map_err(|error| {
			SafeTensorError::new(
				SafeTensorErrorKind::ArithmeticOverflow,
		...
		if rank > limits.rank.get() {
			return Err(SafeTensorError::new(
				SafeTensorErrorKind::RankLimitExceeded,
		...
		let expected_bytes = elements.checked_mul(dtype.element_bytes()).ok_or_else(|| {
			SafeTensorError::new(
				SafeTensorErrorKind::ArithmeticOverflow,
		...
		let encoded_bytes = end.checked_sub(begin).ok_or_else(|| {
			SafeTensorError::new(
				SafeTensorErrorKind::InvalidOffset,
	...
	validate_contiguous_data(&entries, data_bytes)?;
	entries.sort_by(|left, right| left.name.cmp(&right.name));
	Ok(SafeTensorArchive {
		data_start,
		data,
...
fn validate_name(name: &str, limits: SafeTensorLimits) -> SafeTensorResult<()> {
	if name.is_empty() {
		return Err(SafeTensorError::new(
			SafeTensorErrorKind::MalformedHeader,
			"safetensors tensor name is empty",
		));
	}
	let bytes = u64::try_from(name.len()).map_err(|error| {
		SafeTensorError::new(
			SafeTensorErrorKind::ArithmeticOverflow,
			format!("tensor name length cannot be represented as u64: {error}"),
		)
	})?;
	if bytes > limits.name_bytes.get() {
		Err(SafeTensorError::new(
...
fn validate_contiguous_data(entries: &[SafeTensorEntry], data_bytes: u64) -> SafeTensorResult<()> {
	let mut spans = entries
		.iter()
		.map(|entry| (entry.begin, entry.end, entry.name.as_str()))
		.collect::<Vec<_>>();
	spans.sort_unstable();
...
impl<'de> Visitor<'de> for HeaderVisitor {
	...
	fn expecting(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
		formatter.write_str("a safetensors JSON object")
	}
	...
	where A: MapAccess<'de> {
		...
		while let Some(name) = map.next_key::<String>()? {
			...
			if name == "__metadata__" {
				metadata = Some(map.next_value::<Metadata>()?.0);
			} else {
...
impl<'de> Visitor<'de> for TensorVisitor {
	...
	fn expecting(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
		formatter.write_str("a safetensors tensor object")
	}
...
fn nonzero_u64(name: &str, value: u64) -> SafeTensorResult<NonZeroU64> {
	NonZeroU64::new(value).ok_or_else(|| invalid_limit(name))
}

fn invalid_limit(name: &str) -> SafeTensorError {
	SafeTensorError::new(
