...
impl IngestLimits {
	...
	pub fn new(source_bytes: u64, records: u64, fields_per_record: u32, field_bytes: u64) -> IngestResult<Self> {
		Ok(Self {
...
impl TableRequest {
	#[must_use]
	pub const fn new(delimiter: Delimiter, header: HeaderMode, limits: IngestLimits) -> Self {
		Self {
			delimiter,
			header,
			limits,
...
impl RawTable {
	...
	pub fn width(&self) -> usize {
		self.headers
			.len()
			.max(self.rows.first().map_or(0, Vec::len))
	}
}
...
impl IngestError {
	fn new(kind: IngestErrorKind, detail: impl Into<String>) -> Self {
		Self {
			kind,
			path: None,
			detail: detail.into(),
...
///
/// This function is a preparation operation. The runtime loop does not own a
/// filesystem handle or callback capable of invoking it.
///
/// # Errors
///
/// Returns an I/O, arithmetic, or configured source-limit error without a
/// partial table.
pub fn read_table(path: &Path, request: TableRequest) -> IngestResult<RawTable> {
	let bytes = read_bounded(path, request.limits)?;
	parse_table(&bytes, request).map_err(|error| error.for_path(path))
}

/// Frame already-admitted source bytes without performing payload
/// calculations.
///
/// # Errors
///
/// Returns a configured-bound, malformed-record, or inconsistent-width error
...
pub fn parse_table(bytes: &[u8], request: TableRequest) -> IngestResult<RawTable> {
	let byte_count = u64::try_from(bytes.len()).map_err(|error| {
		IngestError::new(
			IngestErrorKind::ArithmeticOverflow,
			format!("source length cannot be represented as u64: {error}"),
		)
	})?;
	if byte_count > request.limits.source_bytes.get() {
		return Err(IngestError::new(
	...
	let delimiter = match request.delimiter {
		Delimiter::Auto => sniff_delimiter(bytes),
		explicit => explicit,
	};
	let records = match delimiter {
		Delimiter::AsciiWhitespace => parse_whitespace_records(bytes, request.limits)?,
		_ => parse_csv_records(bytes, delimiter, request.limits)?,
	};
	assemble_table(records, delimiter, request.header)
}

fn read_bounded(path: &Path, limits: IngestLimits) -> IngestResult<Vec<u8>> {
	let metadata = path.metadata().map_err(|error| {
		IngestError::new(IngestErrorKind::Io, format!("inspect source: {error}")).for_path(path)
	})?;
	if metadata.len() > limits.source_bytes.get() {
		return Err(IngestError::new(
			IngestErrorKind::SourceLimitExceeded,
			format!(
	...
	let file = File::open(path)
		.map_err(|error| IngestError::new(IngestErrorKind::Io, format!("open source: {error}")).for_path(path))?;
	let read_limit = limits.source_bytes.get().checked_add(1).ok_or_else(|| {
		IngestError::new(
			IngestErrorKind::ArithmeticOverflow,
	...
	reader.read_to_end(&mut bytes)
		.map_err(|error| IngestError::new(IngestErrorKind::Io, format!("read source: {error}")).for_path(path))?;
	let count = u64::try_from(bytes.len()).map_err(|error| {
		IngestError::new(
			IngestErrorKind::ArithmeticOverflow,
			format!("read length cannot be represented as u64: {error}"),
		)
		.for_path(path)
	})?;
	if count > limits.source_bytes.get() {
		return Err(IngestError::new(
			IngestErrorKind::SourceLimitExceeded,
			format!(
...
fn parse_csv_records(bytes: &[u8], delimiter: Delimiter, limits: IngestLimits) -> IngestResult<Vec<Vec<Vec<u8>>>> {
	let separator = match delimiter {
		...
		Delimiter::Tab => b'\t',
		Delimiter::Auto | Delimiter::AsciiWhitespace => {
			return Err(IngestError::new(
				IngestErrorKind::MalformedTable,
	...
	for result in reader.byte_records() {
		...
		check_record_bound(records.len(), record.len(), limits)?;
		let mut fields = Vec::with_capacity(record.len());
		for field in &record {
			check_field_bound(field, limits)?;
...
fn parse_whitespace_records(bytes: &[u8], limits: IngestLimits) -> IngestResult<Vec<Vec<Vec<u8>>>> {
	...
	for line in bytes.split(|byte| *byte == b'\n') {
		...
		check_record_bound(records.len(), fields.len(), limits)?;
...
fn check_record_bound(records_seen: usize, field_count: usize, limits: IngestLimits) -> IngestResult<()> {
	let record_number = u64::try_from(records_seen)
		.ok()
...
fn check_field_bound(field: &[u8], limits: IngestLimits) -> IngestResult<()> {
	let bytes = u64::try_from(field.len()).map_err(|error| {
		IngestError::new(
			IngestErrorKind::ArithmeticOverflow,
			format!("field length cannot be represented as u64: {error}"),
		)
	})?;
	if bytes > limits.field_bytes.get() {
		Err(IngestError::new(
...
fn sniff_delimiter(bytes: &[u8]) -> Delimiter {
	let line = bytes
		.split(|byte| *byte == b'\n')
		.map(|line| line.strip_suffix(b"\r").unwrap_or(line))
		.find(|line| line.iter().any(|byte| !byte.is_ascii_whitespace()))
		.unwrap_or_default();
	let mut counts = [(Delimiter::Comma, 0_u64); 3];
	counts[1].0 = Delimiter::Semicolon;
	counts[2].0 = Delimiter::Tab;
	let mut quoted = false;
	let mut unquoted_space = false;
	...
	counts.sort_by(|left, right| {
		right.1
			.cmp(&left.1)
			.then_with(|| delimiter_rank(left.0).cmp(&delimiter_rank(right.0)))
	});
	if unquoted_space
		&& counts
			.iter()
			.filter(|(delimiter, _)| matches!(delimiter, Delimiter::Comma | Delimiter::Semicolon))
			.all(|(_, count)| *count == 0)
		&& line
			.split(|byte| byte.is_ascii_whitespace())
			.filter(|field| !field.is_empty())
			...
			.count() == 2
	{
		Delimiter::AsciiWhitespace
	} else if counts[0].1 != 0 {
		counts[0].0
	} else if line
		.split(|byte| byte.is_ascii_whitespace())
		.filter(|field| !field.is_empty())
...
const fn delimiter_rank(delimiter: Delimiter) -> u8 {
	match delimiter {
		Delimiter::Comma => 0,
		Delimiter::Semicolon => 1,
		Delimiter::Tab => 2,
		Delimiter::Auto => 3,
		Delimiter::AsciiWhitespace => 4,
	}
}

fn nonzero_u64(name: &str, value: u64) -> IngestResult<NonZeroU64> {
	NonZeroU64::new(value).ok_or_else(|| invalid_limit(name))
}

fn invalid_limit(name: &str) -> IngestError {
	IngestError::new(
