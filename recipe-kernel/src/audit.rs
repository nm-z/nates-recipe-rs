...
/// library cannot be smuggled in through an unresolved call. Repository,
/// dependency, linker, dynamic-load, and final-ELF policy is enforced by the
/// separate `recipe-audit` gate so this compiler does not carry a second,
/// potentially divergent list of prohibited names.
#[must_use]
pub fn audit_llvm_ir(ir: &str) -> Vec<AuditFinding> {
	...
	for (line_index, line) in ir.lines().enumerate() {
		...
		if !symbol.starts_with("llvm.") {
			findings.push(AuditFinding {
				kind: AuditKind::ExternalIrDeclaration,
				line: line_index + 1,
