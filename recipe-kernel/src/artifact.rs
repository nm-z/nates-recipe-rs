...
fn raw_section(
	...
	entry_size: usize,
	index: usize,
) -> Result<RawSection, LoweringError> {
	let offset = index
...
fn audit_symbols(symbols: &[ElfSymbol]) -> Result<(), LoweringError> {
	...
	for symbol in symbols {
		if symbol.section == ELF_UNDEFINED_SECTION
			&& matches!(symbol.binding, SYMBOL_BIND_GLOBAL | SYMBOL_BIND_WEAK)
		{
...
fn artifact_mismatch(message: impl Into<String>) -> LoweringError {
	LoweringError::new(LoweringErrorKind::ArtifactMismatch, message)
}
...
fn usize_from_u32(value: u32) -> Result<usize, LoweringError> {
	usize::try_from(value).map_err(|_| artifact_format("u32 does not fit usize"))
}

fn usize_from_u64(value: u64) -> Result<usize, LoweringError> {
	usize::try_from(value).map_err(|_| artifact_format("u64 does not fit usize"))
}
