...
#[non_exhaustive]
pub enum Error { DuplicateArtifact { artifact: ArtifactId, }, MissingArtifact { artifact: ArtifactId, },
	UnexpectedArtifact { artifact: ArtifactId, }, ArtifactMismatch { artifact: ArtifactId, detail: String, },
	ResourceContention { task: TaskId,
		detail: &'static str,
	}, ArenaMismatch { device: DeviceId,
		detail: &'static str,
	}, BackendState {
		backend: &'static str,
		detail: &'static str,
	...
	HsaSymbolLookup { artifact: ArtifactId, abi_entry: String, runtime_symbol: String, source: recipe_hsa::Error, },
	Hsa(recipe_hsa::Error), Kernel(recipe_kernel::LoweringError), }

impl fmt::Display for Error {
	fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self { Self::DuplicateArtifact { artifact } => { write!( formatter,
			...
			Self::ArtifactMismatch { artifact, detail } => { write!( formatter,
					"runtime artifact {artifact} is incompatible: {detail}"
				) }
			Self::ResourceContention { task, detail } => { write!( formatter,
					"task {task} has native resource contention: {detail}"
				) }
			Self::ArenaMismatch { device, detail } => { write!( formatter,
			...
			Self::HsaSymbolLookup { artifact, abi_entry, runtime_symbol, source, } => { write!( formatter,
...
impl From<recipe_cuda::CudaError> for Error { fn from(error: recipe_cuda::CudaError) -> Self { Self::Cuda(error) } }

impl From<recipe_hsa::Error> for Error { fn from(error: recipe_hsa::Error) -> Self { Self::Hsa(error) } }
