...
use recipe_core::{ArenaLayout, ArtifactId, BundleIdentity, ByteCount, CompletionSlotId, DeviceId, DraftPlan, FinalizedBundle, InitDataImage, KernelResourceBounds, LinkId, LoopIteration, QueueSlotId, ReservationLedger, ReservationMechanism, ResolvedTransferEndpoint, ResolvedValueLocation, ResourceManifest, RunPhase, SubmissionSlots, Task, TaskId, TaskKind, TransferEndpoint, TransferLaneClaim, ValueId};
...
use recipe_hsa::{Allocation, DeviceType, DiscoveredAgent, DispatchGeometry, Executable, Kernel, MemoryPoolFlags, MemorySegment, PollStatus, PreparedPending, Queue, QueueConfig, QueueKind, QueueProgress, Session};
use recipe_kernel::{ArtifactDigest, KernelArgument, inspect_hsaco_bundle};

use crate::{Error, ExecutionPlan, NativeBackendKind, NativeDeviceExecutionEvidence, Result, RuntimeArtifactKind, plan::InitImageContract};
...
impl HsaBindingProperties {
	#[must_use]
	pub fn new(target_id: String, code_object_version: u8, queue_packets: u32, maximum_submission_queues: u32, enabled_display_connectors: u32) -> Self {
		Self {
			target_id,
			code_object_version,
			queue_packets,
...
impl<'scope> HsaBinding<'scope> {
	#[must_use]
	pub fn new(device: DeviceId, session: &'scope Session<'scope>, host_allocator: &'scope DiscoveredAgent<'scope>, properties: HsaBindingProperties) -> Self {
		Self {
			device,
			session,
			host_allocator,
...
impl<'scope> HsaBackend<'scope> {
	...
	pub(crate) fn bind_partition(&mut self, bundle: &FinalizedBundle, tasks: &BTreeSet<TaskId>) -> Result<HsaResources<'scope>> {
...
impl<'scope> HsaResources<'scope> {
	pub(crate) fn realize(bundle: &FinalizedBundle, plan: ExecutionPlan, bindings: Vec<HsaBinding<'scope>>, tasks: Option<&BTreeSet<TaskId>>) -> Result<Self> {
	...
	pub(crate) fn prepare_pending(&mut self, request: PendingRequest) -> Result<HsaPending<'scope>> {
		...
		debug_assert!(device.queues.contains_key(&planned.slots.queue) && device.completions.contains_key(&planned.slots.completion));
	...
	pub(crate) fn submit(&mut self, arenas: &impl HsaArenaLookup<'scope>, pending: &mut HsaPending<'scope>, work: BackendWork<'_>) -> Result<()> {
		self.ensure_healthy()?;
		let task = work.task();
		...
		self.devices
			...
			.map(|(&device, resources)| {
				NativeDeviceExecutionEvidence {
					...
					persistent_allocations: resources.kernargs.len() + resources.metric_buffers.len() + usize::from(resources.scratch.is_some()) + 1,
				}
			})
			.collect()
	}

	pub(crate) fn collect_exit(&mut self, arenas: &impl HsaArenaLookup<'scope>, pending: &HsaPending<'scope>, work: TransferWork<'_>, destination: &mut [u8]) -> Result<()> {
		self.ensure_healthy()?;
	...
	fn submit_admission(&mut self, arenas: &impl HsaArenaLookup<'scope>, planned: crate::PlannedSubmission, work: InitAdmissionWork<'_>, native: &mut PreparedPending<'scope, 'scope>) -> Result<PendingAction> {
	...
	fn submit_internal_transfer(&mut self, arenas: &impl HsaArenaLookup<'scope>, planned: crate::PlannedSubmission, work: &TransferWork, native: &mut PreparedPending<'scope, 'scope>) -> Result<PendingAction> {
	...
	fn submit_exit_transfer(&mut self, arenas: &impl HsaArenaLookup<'scope>, planned: crate::PlannedSubmission, work: &TransferWork, native: &mut PreparedPending<'scope, 'scope>) -> Result<PendingAction> {
	...
	fn submit_calculation(&mut self, arenas: &impl HsaArenaLookup<'scope>, planned: crate::PlannedSubmission, work: &CalculationWork, native: &mut PreparedPending<'scope, 'scope>) -> Result<PendingAction> {
		...
		let geometry = {
			...
			debug_assert!(geometry.grid[0] == grid && geometry.workgroup[0] == lanes && geometry.dynamic_private_bytes == dynamic_private_bytes && u64::from(grid) >= artifact.abi.elements.get());
			geometry
		};
		let device = self.device_mut(planned.device);
		let artifact = &device.artifacts[&work.artifact];
	...
	fn submit_metric(&mut self, arenas: &impl HsaArenaLookup<'scope>, planned: crate::PlannedSubmission, work: MetricWork, native: &mut PreparedPending<'scope, 'scope>) -> Result<PendingAction> {
...
impl<'scope> HsaPreparedResources<'scope> {
	pub(crate) fn realize(draft: &DraftPlan, runtime_artifacts: Vec<crate::RuntimeArtifact>, reservations: &ReservationLedger, bindings: Vec<HsaBinding<'scope>>, tasks: &BTreeSet<TaskId>) -> Result<Self> {
		let mut binding_by_device = BTreeMap::new();
		for binding in bindings {
			let device = binding.device;
			let prior = binding_by_device.insert(device, binding);
	...
	pub(crate) fn bind_candidate(self, bundle: &FinalizedBundle, tasks: &BTreeSet<TaskId>) -> Result<HsaResources<'scope>> {
		let Self {
			handoff,
			mut devices,
			pending,
...
impl<'scope> Backend for HsaBackend<'scope> {
	...
	fn poll(&mut self, resource: &mut Self::Resource, pending: &mut Self::Pending, physical_calls: &mut PhysicalCallBatch) -> std::result::Result<BackendPoll, Self::Error> {
		...
		let physical_status = match &result {
			...
			Err(error) => {
				match error {
					Error::DuplicateArtifact { .. } | Error::MissingArtifact { .. } | Error::UnexpectedArtifact { .. } | Error::ArtifactMismatch { .. } | Error::ResourceContention { .. } | Error::ArenaMismatch { .. } | Error::BackendState { .. } | Error::BackendPoisoned { .. } | Error::IntegerOverflow { .. } | Error::CudaContract(_) | Error::Cuda(_) | Error::HsaSymbolLookup { .. } | Error::Hsa(_) | Error::Kernel(_) => PhysicalPollStatus::Failed,
				}
			}
		};
...
impl<'scope> HsaPending<'scope> {
	fn ready(request: PendingRequest, planned: crate::PlannedSubmission, native: PreparedPending<'scope, 'scope>) -> Self {
		Self {
			task: request.task,
			phase: request.phase,
	...
	fn validate_ready(&self, work: &BackendWork<'_>, planned: crate::PlannedSubmission) {
		debug_assert!(self.state == HsaPendingState::Ready && self.task == work.task() && self.class == work.class() && self.device == planned.device && self.queue == planned.slots.queue && self.completion == planned.slots.completion);
	}
	...
	fn validate_collected_exit(&self, work: &TransferWork<'_>, destination_bytes: usize) {
		let bytes = bytes_to_usize(work.bytes.get());
		debug_assert!(self.task == work.task && self.class == WorkClass::ExitTransfer && self.state == HsaPendingState::Terminal && self.queue == work.submission.queue && self.completion == work.submission.completion && self.action == PendingAction::Egress { bytes } && destination_bytes == bytes,);
	}
...
fn validate_binding(binding: &HsaBinding<'_>) -> Result<()> {
	...
	let has_fine_pool = allocator.memory_pools.iter().any(|pool| {
		pool.segment == MemorySegment::Global
			...
			&& pool
				.global_flags
				.is_some_and(|flags| flags.contains(MemoryPoolFlags::FINE_GRAINED) || flags.contains(MemoryPoolFlags::EXTENDED_SCOPE_FINE_GRAINED))
	});
...
fn prepare_pending_pool<'scope>(devices: &BTreeMap<DeviceId, DeviceResources<'scope>>, tasks: &[Task], value_devices: &BTreeMap<ValueId, DeviceId>) -> Result<BTreeMap<TaskId, PreparedPending<'scope, 'scope>>> {
...
fn realize_device<'scope>(resources: &ResourceManifest, reservations: &ReservationLedger, tasks: &[Task], value_devices: &BTreeMap<ValueId, DeviceId>, init_images: &[InitDataImage], runtime_artifacts: &BTreeMap<ArtifactId, crate::RuntimeArtifact>, binding: &HsaBinding<'scope>) -> Result<DeviceResources<'scope>> {
	let device = binding.device;
	...
	{
		Some(0) | None => None,
		Some(bytes) => Some(binding.session.allocate_coarse(bytes_to_usize(bytes))?),
	};
	...
	for (digest, entries) in bundles {
		...
		for ((artifact_id, runtime), inspection) in entries.into_iter().zip(inspections) {
			...
			let kernel = executable.kernel(&runtime_symbol).map_err(|source| {
				...
				kernel.metadata().kernarg_segment_size >= runtime.abi.argument_bytes && kernel.metadata().kernarg_segment_alignment >= runtime.abi.argument_alignment,
				Error::ArtifactMismatch {
					artifact: artifact_id,
...
fn bind_finalized_artifact_resources(devices: &mut BTreeMap<DeviceId, DeviceResources<'_>>, plan: &ExecutionPlan) -> Result<()> {
...
fn hsa_artifact_resource_envelope(artifact: ArtifactId, finalized: &KernelResourceBounds, dynamic_callstack: bool) -> Result<HsaArtifactResourceEnvelope> {
...
fn task_contracts(bundle: &FinalizedBundle, selected: Option<&BTreeSet<TaskId>>) -> BTreeMap<TaskId, HsaTaskContract> {
...
fn kernarg_sizes(tasks: &[Task], device: DeviceId, artifacts: &BTreeMap<ArtifactId, LoadedArtifact<'_>>) -> Result<BTreeMap<CompletionSlotId, usize>> {
...
fn fill_kernarg<'scope>(slot: &mut KernargSlot<'_>, arenas: &impl HsaArenaLookup<'scope>, work: &CalculationWork, abi: &recipe_kernel::KernelAbi) -> Result<()> {
...
fn checked_arena<'arena, 'scope>(arenas: &'arena impl HsaArenaLookup<'scope>, location: ResolvedValueLocation, bytes: u64) -> &'arena HsaArena<'scope> {
...
fn claim_completion(completions: &mut BTreeMap<CompletionSlotId, CompletionState>, task: TaskId, completion: CompletionSlotId) {
...
fn release_completion(completions: &mut BTreeMap<CompletionSlotId, CompletionState>, task: TaskId, completion: CompletionSlotId) {
	let slot = completions.get_mut(&completion).unwrap();
	debug_assert!(completion_owned(slot, task));
	*slot = CompletionState::Available;
}

fn finish_submission_claim(completions: &mut BTreeMap<CompletionSlotId, CompletionState>, task: TaskId, completion: CompletionSlotId, submission: recipe_hsa::Result<()>) -> Result<()> {
	match submission {
		Ok(()) => Ok(()),
		Err(error) => {
...
fn submission_error_requires_poison(error: &Error) -> bool {
	match error {
		Error::BackendPoisoned { .. } | Error::Hsa(recipe_hsa::Error::SessionPoisoned { .. }) | Error::Hsa(recipe_hsa::Error::DeferredRetirement { poisoned: true, .. }) => true,
