...
use recipe_core::{ArtifactBuildRecipe, ArtifactId, ArtifactIdentity, CapacityLedger, DeviceId, DiscoveryIdentity, DiscoveryProfile, ReservationEvidence, ReservationLedger, Topology, TopologyIdentity, ValidationErrors};
...
impl fmt::Display for CandidateRequestError {
	fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			Self::ArtifactSetOverlap => formatter.write_str("candidate artifact and deferred-build identities overlap"),
			...
			Self::RuntimeArtifactSetMismatch => formatter.write_str("runtime artifact set differs from the candidate requirement"),
...
impl StdError for CandidateRequestError {
	fn source(&self) -> Option<&(dyn StdError + 'static)> {
		match self {
			Self::InvalidTopology(errors) | Self::InvalidDiscovery(errors) | Self::InvalidDraft(errors) | Self::InvalidReservations(errors) => Some(errors),
			Self::RuntimeArtifact { source, .. } => Some(source),
			Self::ArtifactSetOverlap | Self::InvalidArtifact { .. } | Self::DuplicateArtifact { .. } | Self::StaticArtifactMismatch { .. } | Self::UnexpectedArtifact { .. } | Self::MissingRuntimeArtifact { .. } | Self::RuntimeArtifactSetMismatch | Self::DeferredArtifactMismatch { .. } | Self::MissingStagePlacement { .. } | Self::MissingGpuCapability { .. } | Self::TargetMismatch { .. } => None,
		}
	}
}
...
impl CandidateSessionFactory for UnavailableCandidateFactory {
	...
	fn realize_candidate(&mut self, request: CandidateRealizationRequest<'_>) -> Result<Self::Session, CandidateFailure<Self::Error>> { Err(unavailable_candidate(request)) }

	fn warm_maximum_concurrency(&mut self, session: &mut Self::Session, candidate: &PlannedCandidate, pass: u32) -> Result<(), CandidateFailure<Self::Error>> { Err(unavailable_candidate((session, candidate, pass))) }

	fn capacity_snapshot(&mut self, session: &mut Self::Session, topology: &Topology, discovery: &DiscoveryProfile) -> Result<CapacityLedger, CandidateFailure<Self::Error>> { Err(unavailable_candidate((session, topology, discovery))) }
...
fn validate_built_identity(request: &CandidateRealizationRequest<'_>, build: &ArtifactBuildRecipe, identity: &ArtifactIdentity) -> Result<(), CandidateRequestError> {
	match identity.kernel_template == build.kernel_template && identity.resources == build.resources && identity.build == Some(build.provenance) {
		true => Ok(()),
		false => {
