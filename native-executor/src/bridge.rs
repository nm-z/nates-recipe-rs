...
use recipe_core::{ByteCount, DeviceId, FinalizedBundle, ResolvedTransferEndpoint, RunPhase, SubmissionSlots, Task, TaskId, TaskKind, TransferEndpoint, TransferLaneClaim, ValueId};
use recipe_cuda::{CompletionStatus as CudaCompletionStatus, DeviceBuffer, Event, Pending as CudaPending, PinnedHostBuffer, Stream};
...
pub enum StagedBridgeError {
	State { task: TaskId, detail: &'static str },
	...
	ThreadPanicked,
	WorkerDisconnected,
	WorkerState { task: TaskId, status: u8 },
}
...
impl StdError for StagedBridgeError {
	fn source(&self) -> Option<&(dyn StdError + 'static)> {
		match self {
			...
			Self::State { .. } | Self::ThreadSpawn(_) | Self::ThreadPanicked | Self::WorkerDisconnected | Self::WorkerState { .. } => None,
		}
	}
}
...
impl<'cuda, 'hsa> StagedCrossBackend<'cuda, 'hsa> {
	...
	fn realize_tasks<'tasks>(&self, tasks: impl Iterator<Item = &'tasks Task>, selected: &BTreeSet<TaskId>, devices: &BTreeMap<DeviceId, LocalDeviceClass>) -> Result<StagedBridgeResources<'cuda, 'hsa>, StagedBridgeError> {
		...
		for task in tasks.filter(|task| selected.contains(&task.id)) {
			...
			let prior = resources.insert(task_id, resource);
			debug_assert!(prior.is_none());
		}
		debug_assert!(resources.keys().copied().eq(selected.iter().copied()));
		Ok(StagedBridgeResources { tasks: resources })
	}

	fn realize_task(&self, contract: TransferContract) -> Result<StagedTaskResources<'cuda, 'hsa>, StagedBridgeError> {
	...
	fn realize_leg(&self, endpoint: EndpointContract, bytes: usize) -> Result<(LegResources<'cuda, 'hsa>, PreparedLegToken<'cuda, 'hsa>), StagedBridgeError> {
...
impl TransferContract {
	fn from_task(task: &Task, devices: &BTreeMap<DeviceId, LocalDeviceClass>) -> Self {
		...
		let source_class = devices[&source_device];
		let destination_class = devices[&destination_device];
		let class = transfer_class(task.phase);
		debug_assert_eq!(transfer.route.len(), 1);
		debug_assert!(!(source_class == destination_class && !(source_class == LocalDeviceClass::Cuda && source_device != destination_device)));
		Self {
			task: task.id,
			phase: task.phase,
			class,
...
impl LegResources<'_, '_> {
	fn host_address(&mut self) -> usize {
		match self {
...
enum WorkerSenderState {
	Active(SyncSender<HostStageJob>),
	Closed,
}
...
impl<'cuda, 'hsa> CrossBackendTransfer<'cuda, 'hsa> for StagedCrossBackend<'cuda, 'hsa> {
	...
	fn bind(&mut self, bundle: &FinalizedBundle, tasks: &BTreeSet<TaskId>, devices: &BTreeMap<DeviceId, LocalDeviceClass>) -> Result<Self::Resource, Self::Error> { self.realize_tasks(bundle.tasks().iter(), tasks, devices) }

	fn prepare_pending(&mut self, resource: &mut Self::Resource, request: PendingRequest) -> Result<Self::Pending, Self::Error> {
		let task = resource.tasks.get_mut(&request.task).unwrap();
		debug_assert!(!(request.phase != task.contract.phase || request.class != task.contract.class || request.submission != Some(task.contract.submission)));
	...
	fn submit(&mut self, resource: &mut Self::Resource, arenas: LocalArenaSet<'_, '_, 'cuda, 'hsa>, pending: &mut Self::Pending, class: WorkClass, work: TransferWork<'_>) -> Result<(), Self::Error> {
		let task = resource.tasks.get_mut(&work.task).unwrap();
		...
		let source_offset = endpoint_offset(work.source);
		let destination_offset = endpoint_offset(work.destination);
	...
	fn poll(&mut self, resource: &mut Self::Resource, pending: &mut Self::Pending) -> Result<BackendPoll, Self::Error> {
		let task = resource.tasks.get_mut(&pending.task).unwrap();
		match pending.state {
			PendingState::Source => {
						...
						match pending.target {
							...
							DestinationTarget::Cuda { .. } | DestinationTarget::Hsa { .. } => {
								submit_destination(
									...
									&pending.target,
									byte_count_to_usize(task.contract.bytes),
								)?;
...
impl<'cuda, 'hsa> CandidateCrossBackendTransfer<'cuda, 'hsa> for StagedCrossBackend<'cuda, 'hsa> {
	fn realize_candidate(&mut self, candidate: &PlannedCandidate, tasks: &BTreeSet<TaskId>, devices: &BTreeMap<DeviceId, LocalDeviceClass>) -> Result<Self::Resource, Self::Error> { self.realize_tasks(candidate.draft.tasks.iter(), tasks, devices) }
	...
	fn validate_handoff(&mut self, resource: &Self::Resource, bundle: &FinalizedBundle, tasks: &BTreeSet<TaskId>, devices: &BTreeMap<DeviceId, LocalDeviceClass>) -> Result<(), Self::Error> {
		...
		debug_assert!(
			resource
				.tasks
				.keys()
				.all(|task| bundle.tasks().iter().any(|candidate| candidate.id == *task))
		);
		Ok(())
	}
...
fn recycle_leg<'cuda, 'hsa>(task: TaskId, leg: ActiveLeg<'cuda, 'hsa>) -> Result<PreparedLegToken<'cuda, 'hsa>, StagedBridgeError> {
	match leg {
...
fn middle_job(source_leg: &mut LegResources<'_, '_>, destination_leg: &mut LegResources<'_, '_>, source: LocalArenaRef<'_, '_, '_>, destination: LocalArenaRef<'_, '_, '_>, range: StagedCopyRange) -> HostStageJob {
	match (source_leg, destination_leg, source, destination) {
		(LegResources::Host, destination_leg @ (LegResources::Cuda { .. } | LegResources::Hsa { .. }), LocalArenaRef::Host(source), LocalArenaRef::Cuda(_) | LocalArenaRef::Hsa(_)) => {
			HostStageJob::Read {
				arena: source.clone(),
				offset: usize_to_u64(range.source_offset),
				destination: destination_leg.host_address(),
				bytes: range.bytes,
			}
		}
		(source_leg @ (LegResources::Cuda { .. } | LegResources::Hsa { .. }), LegResources::Host, LocalArenaRef::Cuda(_) | LocalArenaRef::Hsa(_), LocalArenaRef::Host(destination)) => {
			HostStageJob::Write {
				source: source_leg.host_address(),
				arena: destination.clone(),
				offset: usize_to_u64(range.destination_offset),
				bytes: range.bytes,
			}
		}
		(source_leg @ (LegResources::Cuda { .. } | LegResources::Hsa { .. }), destination_leg @ (LegResources::Cuda { .. } | LegResources::Hsa { .. }), LocalArenaRef::Cuda(_) | LocalArenaRef::Hsa(_), LocalArenaRef::Cuda(_) | LocalArenaRef::Hsa(_)) => {
			HostStageJob::Copy {
				source: source_leg.host_address(),
				destination: destination_leg.host_address(),
				bytes: range.bytes,
		...
		(LegResources::Host | LegResources::Cuda { .. } | LegResources::Hsa { .. }, LegResources::Host | LegResources::Cuda { .. } | LegResources::Hsa { .. }, LocalArenaRef::Host(_) | LocalArenaRef::Cuda(_) | LocalArenaRef::Hsa(_), LocalArenaRef::Host(_) | LocalArenaRef::Cuda(_) | LocalArenaRef::Hsa(_)) => unreachable!("finalized endpoint owners select matching bridge resources"),
	}
}

fn destination_target<'cuda, 'hsa>(destination: LocalArenaRef<'_, 'cuda, 'hsa>, offset: usize) -> DestinationTarget<'cuda, 'hsa> {
	match destination {
...
fn submit_source<'cuda, 'hsa>(task: TaskId, resources: &mut LegResources<'cuda, 'hsa>, pending: &mut ActiveLeg<'cuda, 'hsa>, arena: LocalArenaRef<'_, 'cuda, 'hsa>, offset: usize, bytes: usize) -> Result<(), StagedBridgeError> {
	match (resources, pending, arena) {
		...
		(LegResources::Host | LegResources::Cuda { .. } | LegResources::Hsa { .. }, ActiveLeg::Host | ActiveLeg::CudaReady(_) | ActiveLeg::CudaActive(_) | ActiveLeg::Hsa(_) | ActiveLeg::Transition, LocalArenaRef::Host(_) | LocalArenaRef::Cuda(_) | LocalArenaRef::Hsa(_)) => unreachable!("finalized source route selects matching staging resources"),
	}
}

fn submit_destination<'cuda, 'hsa>(task: TaskId, resources: &LegResources<'cuda, 'hsa>, pending: &mut ActiveLeg<'cuda, 'hsa>, target: &DestinationTarget<'cuda, 'hsa>, bytes: usize) -> Result<(), StagedBridgeError> {
	match (resources, pending, target) {
		(LegResources::Cuda { stream, staging }, pending @ ActiveLeg::CudaReady(_), DestinationTarget::Cuda { buffer, offset }) => {
		...
		(LegResources::Host | LegResources::Cuda { .. } | LegResources::Hsa { .. }, ActiveLeg::Host | ActiveLeg::CudaReady(_) | ActiveLeg::CudaActive(_) | ActiveLeg::Hsa(_) | ActiveLeg::Transition, DestinationTarget::Host | DestinationTarget::Cuda { .. } | DestinationTarget::Hsa { .. }) => unreachable!("finalized destination route selects matching staging resources"),
	}
}

fn consume_cuda_event<'cuda, 'hsa>(task: TaskId, pending: &mut ActiveLeg<'cuda, 'hsa>, detail: &'static str) -> Result<Event<'cuda>, StagedBridgeError> {
...
fn first_error(first: Result<(), StagedBridgeError>, second: Result<(), StagedBridgeError>) -> Result<(), StagedBridgeError> {
	match first {
		Err(error) => Err(error),
		Ok(()) => second,
	}
...
fn erase_cuda_pending_lifetime<'operation, 'context>(pending: CudaPending<'operation, 'context>) -> CudaPending<'static, 'context> {
	...
	let widen = |value| unsafe { core::mem::transmute::<CudaPending<'operation, 'context>, CudaPending<'static, 'context>>(value) };
	widen(pending)
}
