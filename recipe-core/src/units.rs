...
impl ByteOffset {
	...
	pub fn checked_end(self, size: ByteCount) -> Option<ByteCount> {
		self.0.checked_add(size.get()).map(ByteCount::new)
	}
}
...
pub fn transfer_time_ceil(bytes: ByteCount, rate: BytesPerSecond) -> Result<Nanoseconds, UnitError> {
	ratio_time_ceil(bytes.get(), rate.get())
}

pub fn calculation_time_ceil(flops: FlopCount, rate: FlopsPerSecond) -> Result<Nanoseconds, UnitError> {
	ratio_time_ceil(flops.get(), rate.get())
}
