...
use recipe_core::{ArenaLayout, BundleIdentity, ByteCount, DType, DeviceId, DraftPlan, FinalizedBundle, LoopIteration, MetricId, MetricSlotId, ReservationLedger, ReservationMechanism, ResolvedTransferEndpoint, ResolvedValueLocation, RunPhase, SubmissionSlots, TaskId, TaskKind, TransferEndpoint, TransferLaneClaim, ValueId};
use recipe_executor::{ArenaSet, Backend, BackendPoll, BackendWork, MetricValue, PendingRequest, PhysicalCall, PhysicalCallBatch, PhysicalPollStatus, TransferWork, WorkClass, sealed};
...
impl HostBackendConfig {
	pub fn new(worker_threads: usize, staging_bytes_per_worker: usize, bindings: Vec<HostDeviceBinding>) -> Result<Self> {
		if worker_threads == 0 {
...
impl HostBackend {
	...
	#[doc(hidden)]
	pub fn prepare_partition(&mut self, resource: &mut HostResources, request: PendingRequest) -> Result<HostPending> { resource.prepare_pending(request) }
	...
	#[doc(hidden)]
	pub fn submit_partition<L: HostArenaLookup + ?Sized>(&mut self, resource: &mut HostResources, arenas: &L, pending: &mut HostPending, work: BackendWork<'_>) -> Result<()> { resource.submit(arenas, pending, work) }
	...
	#[doc(hidden)]
	pub fn collect_partition(&mut self, resource: &mut HostResources, pending: &HostPending, work: TransferWork<'_>, destination: &mut [u8]) -> Result<()> { resource.collect_exit(pending, work, destination) }
	...
	#[doc(hidden)]
	pub fn prepare_candidate(draft: &DraftPlan, reservations: &ReservationLedger, config: HostBackendConfig, tasks: &BTreeSet<TaskId>) -> Result<HostPreparedResources> { HostPreparedResources::realize(draft, reservations, config, tasks) }
}
...
impl ExpectedWork {
	...
	const fn submission(&self) -> Option<SubmissionSlots> {
		match self {
			Self::Init { submission, .. } | Self::Transfer { submission, .. } | Self::Metric { submission, .. } => Some(*submission),
		}
	}
...
impl HostPreparedResources {
	fn realize(draft: &DraftPlan, reservations: &ReservationLedger, config: HostBackendConfig, tasks: &BTreeSet<TaskId>) -> Result<Self> {
		for task in tasks {
			debug_assert!(draft.tasks.iter().any(|candidate| candidate.id == *task),);
		}
...
impl HostResources {
	...
	fn realize_partition(bundle: &FinalizedBundle, config: HostBackendConfig, tasks: &BTreeSet<TaskId>) -> Result<Self> { Self::realize_scoped(bundle, config, Some(tasks)) }

	fn realize_scoped(bundle: &FinalizedBundle, config: HostBackendConfig, tasks: Option<&BTreeSet<TaskId>>) -> Result<Self> {
		let bindings = index_bindings(config.bindings)?;
	...
	pub fn prepare_pending(&mut self, request: PendingRequest) -> Result<HostPending> {
		...
		debug_assert!(request.class == expected.class() && request.submission == expected.submission() && phase_accepts(request.phase, expected.class()));
		...
		match &mut self.pending {
			PendingResources::Prepared(pending) => {
				...
				debug_assert!(prepared.task == request.task && prepared.class == request.class && prepared.admission == expected.admission());
				Ok(prepared)
			}
	...
	#[doc(hidden)]
	pub fn submit<L: HostArenaLookup + ?Sized>(&mut self, arenas: &L, pending: &mut HostPending, work: BackendWork<'_>) -> Result<()> {
		self.ensure_healthy()?;
		let task = work.task();
		...
		let result = match (expected, work) {
			(
				...
				debug_assert!(work.destination.value == *image && work.destination == *destination && work.bytes == *bytes && work.submission == *submission && image_bytes == bytes.get() && pending.admission == expected.admission());
			...
			(
				...
				debug_assert!(submitted_class == *class && work.source == *source && work.destination == *destination && work.bytes == *bytes && work.route == route && work.lane_claims == lane_claims && work.submission == *submission);
			...
			(
				...
				debug_assert!(work.metric == *metric && work.slot == *slot && work.value == *value && work.submission == *submission);
	...
	pub fn collect_exit(&self, pending: &HostPending, work: TransferWork<'_>, destination: &mut [u8]) -> Result<()> {
		...
		debug_assert!(pending.terminal && pending.task == work.task && pending.action == PendingAction::Egress && destination_bytes == work.bytes.get());
		...
		debug_assert!(*class == WorkClass::ExitTransfer && work.source == *source && work.destination == *expected_destination && work.bytes == *bytes && work.route == route && work.lane_claims == lane_claims && work.submission == *submission && matches!(work.destination, ResolvedTransferEndpoint::External));
		pending.staging.as_ref().unwrap().read_exact(0, destination)
	}
...
impl Backend for HostBackend {
	...
	fn bind_resources(&mut self, bundle: &FinalizedBundle, physical_calls: &mut PhysicalCallBatch) -> Result<Self::Resource> {
	...
	fn prepare_pending(&mut self, resource: &mut Self::Resource, request: PendingRequest, physical_calls: &mut PhysicalCallBatch) -> Result<Self::Pending> {
	...
	fn allocate_arena(&mut self, resource: &mut Self::Resource, layout: &ArenaLayout, physical_calls: &mut PhysicalCallBatch) -> Result<Self::Arena> {
	...
	fn submit(&mut self, resource: &mut Self::Resource, arenas: ArenaSet<'_, Self::Arena>, pending: &mut Self::Pending, work: BackendWork<'_>, physical_calls: &mut PhysicalCallBatch) -> Result<()> {
		record(physical_calls, submission_call(&work))?;
		resource.submit(&arenas, pending, work)
	}

	fn submit_loop_iteration(&mut self, resource: &mut Self::Resource, arenas: ArenaSet<'_, Self::Arena>, pending: &mut Self::Pending, _iteration: LoopIteration, work: BackendWork<'_>, physical_calls: &mut PhysicalCallBatch) -> Result<()> {
		record(physical_calls, submission_call(&work))?;
		resource.prepare_loop_pending(pending)?;
		resource.submit(&arenas, pending, work)
	}

	fn poll(&mut self, resource: &mut Self::Resource, pending: &mut Self::Pending, physical_calls: &mut PhysicalCallBatch) -> Result<BackendPoll> {
		...
		let status = match &result {
			...
			Err(error) => {
				match error {
					Error::InvalidConfiguration(_) | Error::InvalidPath | Error::DuplicateDevice(_) | Error::MissingDevice(_) | Error::WrongDeviceKind(_) | Error::BackendState(_) | Error::Protocol { .. } | Error::UnsupportedWork { .. } | Error::PhysicalReportOverflow | Error::RangeOverflow | Error::OutOfBounds { .. } | Error::SlotCapacityExhausted | Error::SlotBusy | Error::InvalidPendingState | Error::WorkerFailed { .. } | Error::Io { .. } | Error::ThreadSpawn(_) | Error::ThreadPanicked | Error::Poisoned => PhysicalPollStatus::Failed,
				}
			}
		};
	...
	fn collect_exit(&mut self, resource: &mut Self::Resource, arenas: ArenaSet<'_, Self::Arena>, pending: &mut Self::Pending, work: TransferWork<'_>, destination: &mut [u8], physical_calls: &mut PhysicalCallBatch) -> Result<()> {
	...
	fn release_arena(&mut self, resource: &mut Self::Resource, device: DeviceId, arena: Self::Arena, physical_calls: &mut PhysicalCallBatch) -> Result<()> {
...
fn validate_bundle_devices(bundle: &FinalizedBundle, bindings: &BTreeMap<DeviceId, HostDeviceBinding>, require_complete_coverage: bool) -> Result<()> {
	let layouts = bundle
		.arena_layouts()
		.iter()
...
fn prepare_candidate_pending(runtime: &Runtime, draft: &DraftPlan, selected: &BTreeSet<TaskId>) -> Result<BTreeMap<TaskId, HostPending>> {
	let values = draft
		.values
		.iter()
		...
		let (class, staging, admission) = match &task.kind {
			...
			TaskKind::Transfer(transfer) => {
				...
				let admission = match (class, transfer.destination) {
					...
					(WorkClass::InternalTransfer | WorkClass::ExitTransfer, TransferEndpoint::External | TransferEndpoint::Device { .. }) => None,
					(WorkClass::Calculation | WorkClass::Metric, TransferEndpoint::External | TransferEndpoint::Device { .. }) => unreachable!(),
				};
...
fn candidate_transfer_class(phase: RunPhase, source: TransferEndpoint, destination: TransferEndpoint) -> WorkClass {
	match (phase, source, destination) {
		(RunPhase::Init, TransferEndpoint::External, TransferEndpoint::Device { .. }) => WorkClass::InitAdmission,
		(RunPhase::Init | RunPhase::Loop, TransferEndpoint::Device { .. }, TransferEndpoint::Device { .. }) => WorkClass::InternalTransfer,
		(RunPhase::Exit, TransferEndpoint::Device { .. }, TransferEndpoint::Device { .. } | TransferEndpoint::External) => WorkClass::ExitTransfer,
		(RunPhase::Init, TransferEndpoint::External, TransferEndpoint::External) | (RunPhase::Init, TransferEndpoint::Device { .. }, TransferEndpoint::External) | (RunPhase::Loop, TransferEndpoint::External, TransferEndpoint::External | TransferEndpoint::Device { .. }) | (RunPhase::Loop, TransferEndpoint::Device { .. }, TransferEndpoint::External) | (RunPhase::Exit, TransferEndpoint::External, TransferEndpoint::External | TransferEndpoint::Device { .. }) => unreachable!(),
	}
}

fn candidate_transfer_staging(class: WorkClass, source: TransferEndpoint, destination: TransferEndpoint, bytes: ByteCount) -> Option<(DeviceId, ByteCount, PendingAction)> {
	match (class, source, destination) {
		(WorkClass::InitAdmission, TransferEndpoint::External, TransferEndpoint::Device { device, .. }) => Some((device, bytes, PendingAction::None)),
		(WorkClass::ExitTransfer, TransferEndpoint::Device { device, .. }, TransferEndpoint::External) => Some((device, bytes, PendingAction::Egress)),
		(WorkClass::InternalTransfer | WorkClass::ExitTransfer, TransferEndpoint::Device { .. }, TransferEndpoint::Device { .. }) => None,
		(WorkClass::Calculation | WorkClass::Metric, TransferEndpoint::External | TransferEndpoint::Device { .. }, TransferEndpoint::External | TransferEndpoint::Device { .. })
		| (WorkClass::InitAdmission | WorkClass::InternalTransfer, TransferEndpoint::External | TransferEndpoint::Device { .. }, TransferEndpoint::External)
		| (WorkClass::InitAdmission, TransferEndpoint::Device { .. }, TransferEndpoint::Device { .. })
		| (WorkClass::InternalTransfer | WorkClass::ExitTransfer, TransferEndpoint::External, TransferEndpoint::Device { .. })
		| (WorkClass::ExitTransfer, TransferEndpoint::External, TransferEndpoint::External) => unreachable!(),
	}
}

fn task_contracts(bundle: &FinalizedBundle, selected: Option<&BTreeSet<TaskId>>) -> Result<BTreeMap<TaskId, ExpectedWork>> {
...
fn transfer_class(phase: RunPhase, source: ResolvedTransferEndpoint, destination: ResolvedTransferEndpoint) -> WorkClass {
	...
	match (phase, source, destination) {
		...
		(RunPhase::Init, External, External) | (RunPhase::Init, Device(_), External) | (RunPhase::Loop, External, External | Device(_)) | (RunPhase::Loop, Device(_), External) | (RunPhase::Exit, External, External | Device(_)) => unreachable!(),
	}
}
...
fn submit_transfer(arenas: &(impl HostArenaLookup + ?Sized), pending: &mut HostPending, work: TransferWork<'_>) -> Result<()> {
	match (work.source, work.destination) {
		...
		(ResolvedTransferEndpoint::External, ResolvedTransferEndpoint::External) | (ResolvedTransferEndpoint::External, ResolvedTransferEndpoint::Device(_)) => unreachable!(),
	}
}
