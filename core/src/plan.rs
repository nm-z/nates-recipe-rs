...
use crate::{
	...
	identity::{BundleIdentity, CandidateIdentity, DiscoveryIdentity, DraftIdentity, Label, RealizationIdentity, TopologyIdentity},
	ids::{ArenaObjectId, ArtifactId, DeviceId, TaskId, ValueId},
	scalar::{AliasPermission, DType, KernelTemplate},
	schedule::{ArenaLayout, ArenaObject, ArenaRelease, InitDataImage, IterationDomain, LoopIterations, LoopSchedule, LoopTaskDomain, MetricPurpose, ResolvedTransferEndpoint, ResolvedTransferEndpoints, ResolvedValueLocation, ResourceManifest, RunPhase, Task, TaskKind, TransferEndpoint, TransferLaneClaim, ValueAliasContract, ValueBinding, ValueSpec},
...
fn validate_tasks(validator: &mut Validator, draft: &DraftPlan, topology: &Topology, discovery: &DiscoveryProfile, indexes: &DraftIndexes<'_>) {
		...
		match &task.kind {
			TaskKind::Calculation(calculation) => {
				...
				if let Some(fault_flag) = calculation.fault_flag {
					...
					if let Some(value) = value {
						validator.require(
							value.device == calculation.device && value.dtype == DType::I32 && value.bytes == ByteCount::new(4),
							ValidationCode::ResourceMismatch,
							format!("{path}.fault_flag"),
				...
				if metric.purpose == MetricPurpose::FaultReadback {
					validator.require(
						indexes
							.values
							.get(&metric.value)
							.is_some_and(|value| value.dtype == DType::I32 && value.bytes == ByteCount::new(4)),
...
fn validate_fault_readbacks(validator: &mut Validator, draft: &DraftPlan, tasks: &BTreeMap<TaskId, &Task>) {
	...
	for (fault_flag, calculations) in &checked_by_flag {
		...
		for published in draft
			.tasks
			.iter()
			.filter(|task| task.phase == RunPhase::Exit || matches!(&task.kind, TaskKind::Metric(metric) if metric.purpose == MetricPurpose::User))
		{
			validator.require(
...
fn validate_transfer_endpoint<'a>(validator: &mut Validator, task_path: &str, field: &str, endpoint: TransferEndpoint, bytes: ByteCount, values: &BTreeMap<ValueId, &'a ValueSpec>) -> Option<&'a ValueSpec> {
...
fn validate_transfer_lane_claims(validator: &mut Validator, task_path: &str, transfer: &crate::schedule::TransferTask, topology: &Topology, discovery: &DiscoveryProfile) {
	validator.require(
		transfer
			.lane_claims
			.windows(2)
	...
	match (transfer.source, transfer.destination) {
		...
		(TransferEndpoint::External, TransferEndpoint::Device { device, .. }) | (TransferEndpoint::Device { device, .. }, TransferEndpoint::External) => {
			validator.require(
...
fn validate_resource_contention(validator: &mut Validator, draft: &DraftPlan, topology: &Topology, discovery: &DiscoveryProfile) {
	...
	for link in &topology.links {
		let events = draft
			...
			.filter_map(|task| {
				match &task.kind {
					TaskKind::Transfer(transfer) if transfer.route.contains(&link.id) => Some([(task.window.start, 1i8), (task.window.end, -1i8)]),
				...
				match &task.kind {
					TaskKind::Transfer(transfer)
						if transfer.route.is_empty()
							&& matches!( (transfer.source, transfer.destination),
	...
	for discovered in &discovery.devices {
		...
		let mut events = draft
			...
			.filter_map(|task| {
				match &task.kind {
					TaskKind::Calculation(candidate) if candidate.device == discovered.device => Some([(task.window.start, 1i8), (task.window.end, -1i8)]),
					_ => None,
				}
			})
			.flatten()
...
fn validate_concurrency_events(validator: &mut Validator, mut events: Vec<(Nanoseconds, i8)>, maximum: u32, name: String) {
	events.sort_by_key(|(time, delta)| (*time, *delta));
	let mut active = 0u32;
	for (_, delta) in events {
		if delta < 0 {
...
fn validate_compute_transfer_overlap(validator: &mut Validator, calculation_index: usize, calculation_task: &Task, transfer_index: usize, transfer_task: &Task, topology: &Topology, discovery: &DiscoveryProfile) {
		...
		(
			...
			source == calculation.device
				...
				|| transfer.route.iter().any(|link| {
					topology
						.link(*link)
						.is_some_and(|link| link.from == calculation.device || link.to == calculation.device)
				})
		}
		(TransferEndpoint::External, TransferEndpoint::Device { device, .. }) | (TransferEndpoint::Device { device, .. }, TransferEndpoint::External) => device == calculation.device,
...
fn validate_submission(validator: &mut Validator, path: &str, device: DeviceId, submission: crate::schedule::SubmissionSlots, resources: &ResourceManifest) {
	validator.require(
		resources
...
fn validate_value_bindings(validator: &mut Validator, draft: &DraftPlan, values: &BTreeMap<ValueId, &ValueSpec>, kernels: &BTreeMap<crate::ids::KernelTemplateId, &KernelTemplate>) {
	let objects = draft
		.arena_objects
		.iter()
	...
	for (index, binding) in draft.value_bindings.iter().enumerate() {
		...
		validator.require(
			object.alignment.get() >= value_alignment && object.alignment.get() % value_alignment == 0 && binding.object_offset.get() % value_alignment == 0,
...
fn validate_init_images(validator: &mut Validator, draft: &DraftPlan, topology: &Topology, values: &BTreeMap<ValueId, &ValueSpec>) {
	let bindings = draft
		.value_bindings
		.iter()
	...
	for (image_index, image) in draft.init_images.iter().enumerate() {
		...
		for (member_index, member) in image.members.iter().enumerate() {
			...
			if let Some(physical_spec) = physical_spec {
				validator.require(
					physical_spec.device == image.device && physical_spec.dtype == member.dtype && physical_spec.bytes == member.bytes,
					ValidationCode::ResourceMismatch,
					member_path.clone(),
...
fn validate_alias_bindings(validator: &mut Validator, draft: &DraftPlan, values: &BTreeMap<ValueId, &ValueSpec>, kernels: &BTreeMap<crate::ids::KernelTemplateId, &KernelTemplate>, bindings: &BTreeMap<ValueId, &ValueBinding>) {
	...
	for (alias_index, alias) in draft.value_aliases.iter().enumerate() {
		...
		if let (Some(input_binding), Some(output_binding), Some(input_spec), Some(output_spec)) = (input_binding, output_binding, input_spec, output_spec) {
			validate_alias_pair(
				validator,
				&path,
				alias.permission,
...
fn validate_alias_pair(validator: &mut Validator, path: &str, permission: AliasPermission, input: AliasValue<'_>, output: AliasValue<'_>) {
	let exact = input.binding.object == output.binding.object && input.binding.object_offset == output.binding.object_offset && input.spec.bytes == output.spec.bytes;
...
fn binding_ranges_overlap(left: &ValueBinding, left_bytes: ByteCount, right: &ValueBinding, right_bytes: ByteCount) -> bool {
...
fn task_references_value(task: &Task, value: ValueId) -> bool {
	match &task.kind {
		...
		TaskKind::Transfer(transfer) => transfer.source.value() == Some(value) || transfer.destination.value() == Some(value),
...
fn validate_realized_artifacts(validator: &mut Validator, realization: &RealizationProfile, draft: &DraftPlan, discovery: &DiscoveryProfile) {
...
fn validate_loop_domains(validator: &mut Validator, tasks: &[Task], loop_iterations: LoopIterations, loop_domains: &[LoopTaskDomain]) {
	...
	for (index, task) in tasks.iter().enumerate() {
		...
		for (consumer_index, consumer) in tasks.iter().enumerate() {
			if consumer.id == task.id || consumer.phase != RunPhase::Loop || !task_reads_value(consumer, destination) {
				continue;
			}
...
impl FinalizedBundle {
	pub fn finalize(identity: BundleIdentity, topology: &Topology, discovery: &DiscoveryProfile, draft: DraftPlan, realization: RealizationProfile, arena_layouts: Vec<ArenaLayout>) -> ValidationResult<Self> {
	...
	pub fn finalize_with_loop_iterations(identity: BundleIdentity, topology: &Topology, discovery: &DiscoveryProfile, draft: DraftPlan, realization: RealizationProfile, arena_layouts: Vec<ArenaLayout>, loop_iterations: LoopIterations) -> ValidationResult<Self> {
	...
	/// loop task.
	pub fn finalize_with_loop_schedule(identity: BundleIdentity, topology: &Topology, discovery: &DiscoveryProfile, draft: DraftPlan, realization: RealizationProfile, arena_layouts: Vec<ArenaLayout>, loop_schedule: LoopSchedule) -> ValidationResult<Self> {
...
fn validate_layouts(validator: &mut Validator, topology: &Topology, objects: &[ArenaObject], capacity: &CapacityLedger, layouts: &[ArenaLayout]) {
	...
	for (layout_index, layout) in layouts.iter().enumerate() {
		...
		for left_index in 0..layout.allocations.len() {
			for right_index in (left_index + 1)..layout.allocations.len() {
				...
				let memory_overlaps = ByteCount::new(left_allocation.offset.get()) < right_end && ByteCount::new(right_allocation.offset.get()) < left_end;
				validator.require(
...
fn resolve_value_locations(validator: &mut Validator, values: &[ValueSpec], bindings: &[ValueBinding], objects: &[ArenaObject], layouts: &[ArenaLayout]) -> Vec<ResolvedValueLocation> {
