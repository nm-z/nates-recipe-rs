...
impl ByteCount { pub const ZERO: Self = Self(0);
	...
	pub fn checked_align_up(self, alignment: Self) -> Result<Self, UnitError> {
		if alignment.0 == 0 || !alignment.0.is_power_of_two() { return Err(UnitError::InvalidAlignment(alignment.0)); }
		let mask = alignment.0 - 1; self.0.checked_add(mask) .map(|value| Self(value & !mask)) .ok_or(UnitError::Overflow) } }
...
impl BytesPerSecond { pub fn new(value: u64) -> Result<Self, UnitError> { if value == 0 { Err(UnitError::ZeroRate)
		} else { Ok(Self(value)) } }
...
impl TransferLaneCount { pub fn new(value: u32) -> Result<Self, UnitError> { if value == 0 {
			Err(UnitError::ZeroTransferLaneCount) } else { Ok(Self(value)) } }
...
impl FlopCount { pub const ZERO: Self = Self(0);
...
impl FlopsPerSecond { pub fn new(value: u64) -> Result<Self, UnitError> { if value == 0 { Err(UnitError::ZeroRate)
		} else { Ok(Self(value)) } }
...
impl Nanoseconds { pub const ZERO: Self = Self(0);
...
impl ElementCount { pub fn new(value: u64) -> Result<Self, UnitError> { if value == 0 { Err(UnitError::ZeroElementCount)
		} else { Ok(Self(value)) } }
...
pub enum UnitError { Overflow, ZeroRate, ZeroTransferLaneCount, ZeroElementCount, InvalidAlignment(u64), }
...
impl fmt::Display for UnitError {
	fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			Self::InvalidAlignment(value) => { write!( formatter,
...
fn ratio_time_ceil(work: u64, rate: u64) -> Result<Nanoseconds, UnitError> { let numerator = u128::from(work)
		.checked_mul(NANOS_PER_SECOND) .ok_or(UnitError::Overflow)?; let denominator = u128::from(rate); let value = numerator
		.checked_add(denominator - 1) .ok_or(UnitError::Overflow)?
		/ denominator; u64::try_from(value) .map(Nanoseconds::new) .map_err(|_| UnitError::Overflow) }
