...
fn build_cpu_tree(
	...
	for d in 0..max_depth {
		...
		for node_off in 0..n_level {
			...
			for (jc, &j) in col_map.iter().enumerate() {
				...
				for i in 0..n {
					if node_assign[i] as usize != node_id {
						continue;
					}
			...
			sf[node_id] = if best_f[node_off] >= 0 {
				best_f[node_off]
			} else {
				-1
			};
			sb[node_id] = if best_b[node_off] >= 0 {
				best_b[node_off]
			} else {
				-1
			};
	...
	let leaf_val: Vec<f64> = (0..max_nodes)
		.map(|node| -sum_g[node] / (sum_h[node] + lambda))
		.collect();
...
pub fn train(x: &[f64], y: &[f64], n: usize, p: usize, params: &Params) -> Result<Model, Error> {
	...
	for t in 0..params.n_estimators {
		...
		let col_map = sample_cols(
			p,
			params.colsample_bytree,
			&mut (ChaCha8Rng::seed_from_u64(
				params.seed ^ (t as u64).wrapping_mul(6364136223846793005).wrapping_add(1),
			)),
		);
...
pub fn predict(model: &Model, x: &[f64], n: usize) -> Result<Vec<f64>, Error> {
	...
	for round_trees in &model.trees {
		...
		for i in 0..n {
			pred[i] += model.learning_rate * tree.leaf_val[leaves[i] as usize];
		}
	}
	Ok(pred)
}
...
pub fn train_multiclass(
	...
	for t in 0..params.n_estimators {
		...
		if (t + 1) % 100 == 0 {
			...
			let acc = (0..n)
				.filter(|&i| {
					(1..n_classes).fold(0usize, |best, k| {
						if probs_log[i * n_classes + k]
							.partial_cmp(&probs_log[i * n_classes + best])
