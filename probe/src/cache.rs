use std::{ fs::{self, File, OpenOptions}, io::{Read, Write},
	os::unix::fs::{MetadataExt, OpenOptionsExt, PermissionsExt}, path::{Path, PathBuf},
	sync::atomic::{AtomicU64, Ordering}, };

use rustix::{ fs::{CWD, RenameFlags, renameat_with}, process::geteuid, };

use crate::{ codec::{MAXIMUM_PROFILE_BYTES, MeasuredProfileCodec}, error::{ProbeError, ProbeResult},
	model::{CacheIdentity, MeasuredProfile, ProfileCache}, };

const PRIVATE_FILE_MODE: u32 = 0o600; const PRIVATE_DIRECTORY_MASK: u32 = 0o077; const O_CLOEXEC: i32 = 0o2_000_000;
...
pub struct ExplicitPathProfileCache { path: PathBuf, }

impl ExplicitPathProfileCache { pub fn new(path: impl Into<PathBuf>) -> ProbeResult<Self> { let path = path.into();
		if !path.is_absolute() {
	...
	fn parent(&self) -> ProbeResult<&Path> { let parent = self .path .parent()
		...
		if canonical != parent { return Err(cache_error(format!(
		...
		if metadata.mode() & PRIVATE_DIRECTORY_MASK != 0 { return Err(cache_error(format!(
				"cache directory {} must not grant group or other permissions",
				parent.display() ))); }
		if metadata.uid() != geteuid().as_raw() { return Err(cache_error(format!(
				"cache directory {} is not owned by the effective user",
				parent.display() ))); }
		Ok(parent) }

	fn load_existing(&self, identity: CacheIdentity) -> ProbeResult<Option<MeasuredProfile>> { self.parent()?;
		let metadata = match fs::symlink_metadata(&self.path) { Ok(metadata) => metadata,
			Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(None),
			Err(error) => return Err(ProbeError::io("inspect cache", &self.path, error)),
		}; if metadata.file_type().is_symlink() || !metadata.is_file() { return Err(cache_error(format!(
				"cache target {} must be a regular non-symlink file",
				self.path.display() ))); }
		if metadata.mode() & PRIVATE_DIRECTORY_MASK != 0 { return Err(cache_error(format!(
				"cache file {} has insecure permissions {:o}",
				self.path.display(), metadata.mode() & 0o777 ))); }
		if metadata.uid() != geteuid().as_raw() { return Err(cache_error(format!(
				"cache file {} is not owned by the effective user",
				self.path.display() ))); }
		let mut file = OpenOptions::new() .read(true) .custom_flags(O_NOFOLLOW | O_CLOEXEC) .open(&self.path)
		...
		if opened.len() > MAXIMUM_PROFILE_BYTES as u64 { return Err(cache_error(format!(
		...
		let profile = MeasuredProfileCodec::decode(&bytes)?; if profile.cache_identity != identity {
...
impl ProfileCache for ExplicitPathProfileCache {
	...
	fn store(&self, profile: &MeasuredProfile) -> ProbeResult<()> { let bytes = MeasuredProfileCodec::encode(profile)?;
		let parent = self.parent()?; if let Some(existing) = self.load_existing(profile.cache_identity)? {
			if existing == *profile { return Ok(()); }
			return Err(cache_error(format!(
				"cache target {} already contains a different profile",
				self.path.display() ))); }

		let mut temporary = PendingFile::create(parent, &self.path)?; temporary .file .as_mut()
		...
		match renameat_with( CWD, &temporary.path, CWD, &self.path, RenameFlags::NOREPLACE, ) {
			Ok(()) => temporary.committed = true, Err(error) => {
				if let Some(existing) = self.load_existing(profile.cache_identity)? && existing == *profile
				{ return Ok(()); }
...
#[derive(Debug)]
struct PendingFile { path: PathBuf, file: Option<File>, committed: bool, }

impl PendingFile { fn create(parent: &Path, target: &Path) -> ProbeResult<Self> {
		static NEXT: AtomicU64 = AtomicU64::new(1); let target_name = target .file_name() .and_then(|value| value.to_str())
		...
		for _ in 0..64 { let nonce = NEXT.fetch_add(1, Ordering::Relaxed);
			...
			match OpenOptions::new() .read(true) .write(true) .create_new(true) .mode(PRIVATE_FILE_MODE)
				.custom_flags(O_NOFOLLOW | O_CLOEXEC) .open(&path)
			{ Ok(file) => { file.set_permissions(fs::Permissions::from_mode(PRIVATE_FILE_MODE))
						.map_err(|error| ProbeError::io("set cache temporary permissions", &path, error))?;
					return Ok(Self { path, file: Some(file), committed: false, }); }
...
impl Drop for PendingFile { fn drop(&mut self) { if !self.committed { let _ = fs::remove_file(&self.path); } } }
