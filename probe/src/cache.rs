...
impl ExplicitPathProfileCache {
	...
	fn parent(&self) -> ProbeResult<&Path> {
		...
		let canonical = fs::canonicalize(parent).map_err(|error| ProbeError::io("canonicalize cache directory", parent, error))?;
		if canonical != parent {
		...
		let metadata = fs::symlink_metadata(parent).map_err(|error| ProbeError::io("inspect cache directory", parent, error))?;
	...
	fn load_existing(&self, identity: CacheIdentity) -> ProbeResult<Option<MeasuredProfile>> {
		...
		let mut bytes = Vec::with_capacity(usize::try_from(opened.len()).map_err(|_| cache_error("cache length does not fit usize"))?);
