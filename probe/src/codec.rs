...
use recipe_core::{ByteCount, BytesPerSecond, CalculationCapability, Device, DeviceId, DeviceKind, Digest, DirectedLink, DiscoveredDevice, DiscoveredLink, DiscoveryIdentity, DiscoveryProfile, DuplexMode, DuplexResourceId, FlopsPerSecond, Label, LinkId, Machine, MachineId, Node, NodeId, NodeRole, Property, PropertyProvenance, TargetIdentity, Topology, TopologyIdentity, TransferCapability, TransferLaneCount, TransportId, TransportKind};
use sha2::{Digest as _, Sha256};

use crate::{
	error::{ProbeError, ProbeResult},
	model::{BenchmarkMetadata, BoundedBenchmarkPlan, CacheIdentity, DirectionalBenchmarkEvidence, MachineFingerprint, MeasuredGpuOrigin, MeasuredMachineOrigin, MeasuredOrigins, MeasuredPeerBenchmark, MeasuredProfile, MeasuredRamOrigin, MeasuredStorageOrigin, PEER_BENCHMARK_PROTOCOL_SCHEMA, PROFILE_CODEC_SCHEMA, PROFILE_SCHEMA, PeerBenchmarkEvidence, PeerDuplexExecution, PeerEndpointEvidence},
...
fn validate_peer_benchmarks(profile: &MeasuredProfile) -> ProbeResult<()> {
	...
	let maximum_duration = u64::try_from(profile.benchmarks.network.maximum_duration.as_nanos()).map_err(|_| codec_error("network benchmark duration exceeds canonical representation"))?;
...
fn validate_directional_evidence(benchmark: &MeasuredPeerBenchmark, direction: &str, evidence: DirectionalBenchmarkEvidence, expected_bytes: u64, expected_iterations: u32, maximum_duration: u64) -> ProbeResult<()> {
	let valid = evidence.total_bytes.get() == expected_bytes && evidence.sample_count != 0 && evidence.sample_count == expected_iterations && evidence.elapsed_nanoseconds != 0 && evidence.elapsed_nanoseconds <= maximum_duration && evidence.minimum_sample_nanoseconds != 0 && evidence.minimum_sample_nanoseconds <= evidence.mean_sample_nanoseconds && evidence.mean_sample_nanoseconds <= evidence.maximum_sample_nanoseconds && evidence.maximum_sample_nanoseconds <= evidence.elapsed_nanoseconds;
	if valid {
		Ok(())
	} else {
...
fn evidence_rate(evidence: DirectionalBenchmarkEvidence) -> ProbeResult<BytesPerSecond> {
	...
	BytesPerSecond::new(u64::try_from(rate).map_err(|_| codec_error("peer benchmark rate does not fit u64"))?).map_err(|error| codec_error(format!("peer benchmark rate is invalid: {error}")))
}
...
fn require_increasing_ref<'a, T: Ord + ?Sized + 'a>(name: &str, values: impl Iterator<Item = &'a T>) -> ProbeResult<()> {
	let mut previous = None;
	for value in values {
		if previous.is_some_and(|previous| previous >= value) {
			return Err(codec_error(format!(
...
fn require_topology_discovery_match(profile: &MeasuredProfile) -> ProbeResult<()> {
	for topology_device in &profile.topology.devices {
		...
		if topology_device.capacity != discovered.total_capacity || topology_device.transfer_rate != discovered.transfer.rate {
			return Err(codec_error(format!(
	...
	for topology_link in &profile.topology.links {
		...
		if topology_link.bandwidth != discovered.bandwidth || topology_link.maximum_inflight_transfers != discovered.maximum_inflight_transfers {
			return Err(codec_error(format!(
...
fn require_peer_topology_match(profile: &MeasuredProfile) -> ProbeResult<()> {
	...
	for (transport, directions) in transports {
		...
		if forward.from != reverse.to || forward.to != reverse.from || forward.kind != reverse.kind || forward.duplex != reverse.duplex {
			return Err(codec_error(format!(
	...
	for benchmark in &profile.peer_benchmarks {
		...
		let matching = peer_transports
			...
			.find(|(index, (forward, reverse))| {
				...
				let rates_match = (forward.bandwidth == benchmark.outbound_rate && reverse.bandwidth == benchmark.inbound_rate) || (reverse.bandwidth == benchmark.outbound_rate && forward.bandwidth == benchmark.inbound_rate);
				let execution_matches = match benchmark.evidence.execution {
					PeerDuplexExecution::Simultaneous => forward.duplex == DuplexMode::Full && forward.capacity_resource != reverse.capacity_resource,
					PeerDuplexExecution::Serialized => forward.duplex == DuplexMode::Half && forward.capacity_resource == reverse.capacity_resource,
				};
...
fn encode_benchmark_metadata(encoder: &mut Encoder, metadata: BenchmarkMetadata) -> ProbeResult<()> {
	...
	for plan in [
		...
		encoder.u64(u64::try_from(plan.maximum_duration.as_nanos()).map_err(|_| codec_error("benchmark duration exceeds canonical representation"))?);
	}
	Ok(())
}
...
fn decode_peer_evidence(decoder: &mut Decoder<'_>, index: usize) -> ProbeResult<PeerBenchmarkEvidence> {
	...
	let mut direction = |name: &str| -> ProbeResult<DirectionalBenchmarkEvidence> {
		Ok(DirectionalBenchmarkEvidence {
			...
			variance_nanoseconds_squared: decoder.u128(&format!("{prefix}.{name}.variance_nanoseconds_squared"))?,
		})
	};
...
fn decode_topology(decoder: &mut Decoder<'_>) -> ProbeResult<Topology> {
	...
	for index in 0..link_count {
		links.push(DirectedLink {
			...
			maximum_inflight_transfers: decoder.property_lanes(&format!("links[{index}].maximum_inflight_transfers"))?,
...
fn decode_discovery(decoder: &mut Decoder<'_>) -> ProbeResult<DiscoveryProfile> {
	...
	for index in 0..device_count {
		...
		let transfer = TransferCapability {
			...
			asynchronous_submission: decoder.bool(&format!("discovery.devices[{index}].transfer.asynchronous"))?,
		...
		let calculation = if decoder.bool(&format!("discovery.devices[{index}].has_calculation"))? {
			Some(CalculationCapability {
				target: TargetIdentity {
					...
					architecture: decoder.label(&format!("discovery.devices[{index}].target.architecture"))?,
	...
	for index in 0..link_count {
		links.push(DiscoveredLink {
			...
			maximum_inflight_transfers: decoder.property_lanes(&format!("discovery.links[{index}].maximum_inflight"))?,
...
impl<'a> Decoder<'a> {
	...
	fn length(&mut self, name: &str) -> ProbeResult<usize> {
		let value = usize::try_from(self.u32(name)?).map_err(|_| codec_error(format!("{name} length does not fit usize")))?;
		if value > MAXIMUM_ITEMS {
			return Err(codec_error(format!(
				"{name} contains {value} items; limit is {MAXIMUM_ITEMS}"
			)));
	...
	fn label(&mut self, name: &str) -> ProbeResult<Label> {
		let length = usize::try_from(self.u32(&format!("{name}.length"))?).map_err(|_| codec_error(format!("{name} length does not fit usize")))?;
		...
		let value = std::str::from_utf8(bytes).map_err(|error| codec_error(format!("{name} is not UTF-8: {error}")))?;
