...
use recipe_core::{ByteCount, BytesPerSecond, CalculationCapability, Device, DeviceId, DeviceKind, DirectedLink, DiscoveredDevice, DiscoveredLink, DiscoveryIdentity, DiscoveryProfile, DuplexMode, DuplexResourceId, Label, LinkId, Machine, MachineId, Node, NodeId, NodeRole, Property, PropertyProvenance, Topology, TopologyIdentity, TransferCapability, TransferLaneCount, TransportId, TransportKind};

use crate::{
	error::{ProbeError, ProbeResult},
	hash::CanonicalDigest,
	model::{BenchmarkMetadata, BoundedBenchmarkPlan, CacheIdentity, GpuBenchmarkIo, GpuDescriptor, GpuDiscovery, GpuMeasurement, HostBenchmarkIo, HostDiscovery, HostInventory, LinkDuplex, MeasuredGpuOrigin, MeasuredMachineOrigin, MeasuredOrigins, MeasuredPeerBenchmark, MeasuredProfile, MeasuredRamOrigin, MeasuredStorageOrigin, PEER_BENCHMARK_PROTOCOL_SCHEMA, PROFILE_SCHEMA, PeerBenchmarkControl, PeerDescriptor, PeerDuplexExecution, PeerMeasurement, PeerSession, RamMeasurement, StorageMeasurement},
...
impl<'a> ProbeEngine<'a> {
	#[must_use]
	pub const fn new(host_discovery: &'a dyn HostDiscovery, gpu_discovery: &'a dyn GpuDiscovery, host_benchmarks: &'a dyn HostBenchmarkIo, gpu_benchmarks: &'a dyn GpuBenchmarkIo) -> Self {
		Self {
			host_discovery,
			gpu_discovery,
			host_benchmarks,
	...
	pub fn probe(&self, seed: &SeedContract, peer_sessions: &[&dyn PeerSession]) -> ProbeResult<MeasuredProfile> {
		...
		topology
			...
			.map_err(|errors| ProbeError::InvalidProfile(format!("topology scheduling validation failed: {errors}")))?;
		discovery
			.validate(&topology)
		...
		crate::codec::validate_profile(&profile).map_err(|error| ProbeError::InvalidProfile(format!("constructed profile validation failed: {error}")))?;
		Ok(profile)
	}

	fn inspect<'b>(&self, seed: &SeedContract, peer_sessions: &'b [&'b dyn PeerSession]) -> ProbeResult<Inspection<'b>> {
	...
	pub fn current_cache_identity(&self, seed: &SeedContract, peer_sessions: &[&dyn PeerSession]) -> ProbeResult<CacheIdentity> { Ok(self.inspect(seed, peer_sessions)?.cache_identity) }

	/// Returns an exact cached profile when valid, otherwise probes and stores a
	/// new one through the explicitly supplied cache.
	pub fn load_or_probe_and_store(&self, seed: &SeedContract, peer_sessions: &[&dyn PeerSession], cache: &dyn crate::model::ProfileCache) -> ProbeResult<MeasuredProfile> {
	...
	pub fn probe_and_store(&self, seed: &SeedContract, peer_sessions: &[&dyn PeerSession], cache: &dyn crate::model::ProfileCache) -> ProbeResult<MeasuredProfile> {
...
fn validate_peer_measurement(descriptor: &PeerDescriptor, measurement: PeerMeasurement, plan: BoundedBenchmarkPlan) -> ProbeResult<()> {
	require_measured(
	...
	if evidence.protocol_schema != PEER_BENCHMARK_PROTOCOL_SCHEMA || evidence.local_endpoint.machine.is_zero() || evidence.local_endpoint.profile.is_zero() || evidence.remote_endpoint.machine.is_zero() || evidence.remote_endpoint.profile.is_zero() || evidence.local_endpoint.machine == evidence.remote_endpoint.machine {
...
fn validate_directional_evidence(descriptor: &PeerDescriptor, direction: &str, evidence: crate::model::DirectionalBenchmarkEvidence, rate: BytesPerSecond, plan: BoundedBenchmarkPlan) -> ProbeResult<()> {
	...
	let maximum_duration = u64::try_from(plan.maximum_duration.as_nanos()).map_err(|_| ProbeError::Benchmark("peer benchmark duration exceeds u64 nanoseconds".to_owned()))?;
	...
	let valid = evidence.total_bytes.get() == total_bytes && evidence.elapsed_nanoseconds != 0 && evidence.elapsed_nanoseconds <= maximum_duration && evidence.sample_count == plan.iterations && evidence.minimum_sample_nanoseconds != 0 && evidence.minimum_sample_nanoseconds <= evidence.mean_sample_nanoseconds && evidence.mean_sample_nanoseconds <= evidence.maximum_sample_nanoseconds && evidence.maximum_sample_nanoseconds <= evidence.elapsed_nanoseconds && u128::from(rate.get()) == derived_rate;
	if valid {
		Ok(())
	} else {
...
fn build_cache_identity(seed: &SeedContract, host: &HostInventory, gpus: &[GpuDescriptor], peers: &[(&dyn PeerSession, PeerDescriptor)]) -> CacheIdentity {
...
fn build_topology(identity: TopologyIdentity, host: &HostInventory, gpus: &[GpuDescriptor], measurements: &Measurements) -> ProbeResult<Topology> {
	let ids = assign_ids(host, gpus, measurements);
...
impl LinkBuilder {
	fn push_pair(&mut self, from: DeviceId, to: DeviceId, forward: DirectionalLinkMeasurement, reverse: DirectionalLinkMeasurement, kind: TransportKind, duplex: LinkDuplex) {
...
fn build_discovery(identity: DiscoveryIdentity, topology: &Topology, host: &HostInventory, gpus: &[GpuDescriptor], measurements: &Measurements) -> ProbeResult<DiscoveryProfile> {
	let ids = assign_ids(host, gpus, measurements);
	let mut devices = Vec::new();
	for domain in &host.ram {
		let measurement = measurements.ram[&domain.key];
...
fn min_lanes(left: TransferLaneCount, right: TransferLaneCount) -> TransferLaneCount { if left <= right { left } else { right } }
