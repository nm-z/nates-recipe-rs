...
impl<'ast> Visit<'ast> for MethodCallVisitor<'_> {
	fn visit_expr_method_call(&mut self, i: &'ast syn::ExprMethodCall) {
		let method = i.method.span().byte_range();
		let open = i.paren_token.span.open().byte_range();
		let close = i.paren_token.span.close().byte_range();
		self.calls.push(MethodCallLocation {
			...
			argument_count: i.args.len(),
			receiver: classify_recipe_expression(&i.receiver, self.bindings),
		});
...
impl<'ast> Visit<'ast> for LocalBindingVisitor<'ast> {
	fn visit_local(&mut self, i: &'ast syn::Local) {
		let (pattern, explicit) = match &i.pat {
		...
		if let syn::Pat::Ident(identifier) = pattern {
			self.bindings.push(LocalBinding {
				...
				explicit,
				initializer: i.init.as_ref().map(|init| return init.expr.as_ref()),
			});
		}
...
fn classify_recipe_type(ty: &syn::Type) -> Option<RecipeReceiver> {
	...
	match path.path.segments.last()?.ident.to_string().as_str() {
		"Data" => return Some(RecipeReceiver::Data),
		"Model" => return Some(RecipeReceiver::Model),
		"Train" => return Some(RecipeReceiver::Train),
		"Infer" => return Some(RecipeReceiver::Infer),
		_ => return None,
	}
}
	...
	loop {
		...
		for binding in &visitor.bindings {
			let kind = binding.explicit.or_else(|| {
				let expression = binding.initializer?;
				return classify_recipe_expression(expression, &resolved);
			});
...
fn classify_recipe_expression(expression: &Expr, bindings: &BTreeMap<String, RecipeReceiver>) -> Option<RecipeReceiver> {
	match expression {
		Expr::Path(path) => {
			...
			return bindings
				.get(&last)
				.copied()
				.or_else(|| return (last == "recipe").then_some(RecipeReceiver::Facade));
		}
		...
		Expr::Group(group) => return classify_recipe_expression(&group.expr, bindings),
		Expr::Paren(parenthesized) => return classify_recipe_expression(&parenthesized.expr, bindings),
		Expr::Reference(reference) => return classify_recipe_expression(&reference.expr, bindings),
		_ => return None,
	}
}
...
impl SourceRewrite {
	pub(crate) fn generated(&self) -> &str { return &self.generated; }
	...
	fn line_column(&self, offset: usize) -> (usize, usize) {
		...
		let line_index = self
			.line_starts
			.partition_point(|start| return *start <= offset)
			.saturating_sub(1);
		let line_start = self
			.line_starts
			.get(line_index)
		...
		let column = self
			...
			.map_or(1, |line| return line.chars().count() + 1);
	...
	fn line(&self, one_based: usize) -> &str {
		...
		let Some(line) = self.original.get(start..end) else {
			return "";
		};
...
impl DiagnosticStream {
	pub(crate) fn parse(bytes: &[u8]) -> Self {
		...
		for line in bytes.split_inclusive(|byte| return *byte == b'\n') {
	...
	pub(crate) fn mapped_rendering(&self, rewrite: &SourceRewrite, compiler_source: &Path, original_source: &Path) -> String { return self.mapped_rendering_chain(&[(rewrite, compiler_source, original_source)]); }
...
pub fn lower_recipe_source(source_path: &Path, source: &str) -> Result<Option<SourceRewrite>, String> {
	...
	let probe_rewrite = if named_grad_candidates.is_empty() {
		...
		.ok_or_else(|| return "could not construct the Recipe syntax-classification source".to_owned())?)
	};
	let classification_source = probe_rewrite
		.as_ref()
		.map_or(source, SourceRewrite::generated);
	let Ok(syntax) = syn::parse_file(classification_source) else {
		return Ok(None);
	};
	...
	let to_original = |offset| {
		return probe_rewrite.as_ref().map_or(offset, |rewrite| {
			return rewrite.generated_to_original(offset);
		});
	};
	let recipe_grad_calls = visitor
		.calls
		.iter()
		.filter(|call| return call.method == "grad" && call.receiver == Some(RecipeReceiver::Model))
		.map(|call| return (to_original(call.method_start), to_original(call.method_end)))
		.collect::<Vec<_>>();
	let mut edits = Vec::new();
	for candidate in &named_grad_candidates {
		if !recipe_grad_calls
			.iter()
			.any(|span| return *span == (candidate.method_start, candidate.method_end))
		{
			continue;
		}
		match &candidate.shape {
			...
			NamedGradShape::Malformed(message) => {
				let diagnostic_source = probe_rewrite
					.as_ref()
					.ok_or_else(|| return "named-gradient classification rewrite is absent".to_owned())?;
	...
	for call in visitor.calls {
		...
		match (call.receiver, call.method.as_str(), call.argument_count) {
			...
			(Some(RecipeReceiver::Train), "run", 2) => {
				edits.push(TextEdit {
					start: method_start,
					end: method_end,
	...
	return build_rewrite(source, edits)
		.ok_or_else(|| return "Recipe syntax edits overlap or address an invalid source boundary".to_owned())
		.map(Some);
}
...
fn classify_named_grad_arguments(tokens: TokenStream) -> NamedGradShape {
	...
	let clip_fields = fields
		.iter()
		.filter(|field| return field.name == "clip")
		.count();
	if clip_fields > 1 {
		return NamedGradShape::Malformed("duplicate named field `clip` in Recipe Model `.grad(...)`; declare `clip: EXPR` once".to_owned());
	}
	if let Some(field) = fields.iter().find(|field| return field.name != "clip") {
		return NamedGradShape::Malformed(format!(
	...
	let [field] = fields.as_slice() else {
		return NamedGradShape::Malformed("Recipe Model `.grad(...)` accepts exactly one named field: `clip: EXPR`".to_owned());
	};
	let name = field.name.span().byte_range();
	let colon = field.colon.span().byte_range();
	return NamedGradShape::Exact {
...
fn render_named_grad_error(source_path: &Path, rewrite: &SourceRewrite, candidate: &NamedGradCandidate, message: &str) -> String {
	...
	let highlight_width = rewrite
		.original
		.get(candidate.arguments_start..highlight_end)
		.unwrap_or_default()
		.chars()
		.count()
		.max(1);
...
fn build_rewrite(source: &str, mut edits: Vec<TextEdit>) -> Option<SourceRewrite> {
	edits.sort_by_key(|edit| return (edit.start, edit.end));
	...
	let mut original_cursor = 0usize;
	for edit in edits {
		if edit.start < original_cursor || edit.end < edit.start || edit.end > source.len() || !source.is_char_boundary(edit.start) || !source.is_char_boundary(edit.end) {
			return None;
		}
		generated.push_str(source.get(original_cursor..edit.start)?);
	...
	generated.push_str(source.get(original_cursor..)?);
...
fn render_diagnostic(diagnostic: &Value, output: &mut String) {
	...
	match code {
		Some(code) => {
			let _ignored = writeln!(output, "{level}[{code}]: {message}");
		}
		None if level == "failure-note" => {
			let _ignored = writeln!(output, "{message}");
		}
		None => {
			let _ignored = writeln!(output, "{level}: {message}");
		}
	}
...
fn preferred_span(diagnostic: &Value) -> Option<&Value> {
	...
	return spans
		.iter()
		.find(|span| return span.get("is_primary").and_then(Value::as_bool) == Some(true))
		.or_else(|| return spans.first());
}
...
fn render_span(span: &Value, output: &mut String) {
	...
	let column = span
		...
		.unwrap_or(1);
	let _ignored = writeln!(output, " --> {file}:{line}:{column}");
	...
	for (offset, source) in lines.iter().enumerate() {
		...
		let end = source
			...
			.and_then(|value| return usize::try_from(value).ok())
			.unwrap_or(start + 1);
		let _ignored = writeln!(output, "{number:>4} | {source_line}");
