...
#[derive(Debug)]
#[non_exhaustive]
pub enum DataPreparationError { Declaration(DeclarationError), MissingTargets, MissingSplit,
	FloatPredicateOutsideF32 { column: String, value: f64 }, Ingest(IngestError), Source(DatasetSourceError),
	Semantic(SemanticError), Prepare(PrepareError), }

impl fmt::Display for DataPreparationError {
	fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			Self::Declaration(error) => write!(formatter, "invalid data declaration: {error}"),
			Self::MissingTargets => { write!( formatter,
					"dataset preparation requires at least one target vector"
				) }
			Self::MissingSplit => { write!( formatter,
					"dataset preparation requires an explicit train split"
				) }
			Self::FloatPredicateOutsideF32 { column, value } => { write!( formatter,
...
impl std::error::Error for DataPreparationError {
	fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
		match self { Self::Declaration(error) => Some(error), Self::Ingest(error) => Some(error),
			Self::Source(error) => Some(error), Self::Semantic(error) => Some(error), Self::Prepare(error) => Some(error),
...
pub fn prepare_data(data: &Data) -> DataPreparationResult<PreparedDataset> { let limits = default_data_limits()?;
...
pub fn distill_data(data: &Data) -> DataPreparationResult<DistilledDataset> { distill_data_with_limits(data, default_data_limits()?) }
...
pub fn select_target_free_data(data: &Data, distilled: &DistilledDataset) -> DataPreparationResult<RawTable> {
	let excluded_columns = data .exclusions() .iter()
		.map(|pattern| ColumnPattern::new(pattern.as_bytes().to_vec()).map_err(DataPreparationError::Prepare))
		.collect::<DataPreparationResult<Vec<_>>>()?; let excluded_rows = data .condition_exclusions() .iter()
		.map(map_condition) .collect::<DataPreparationResult<Vec<_>>>()?;
...
pub fn prepare_data_with_limits(data: &Data, limits: IngestLimits) -> DataPreparationResult<PreparedDataset> {
	data.validate().map_err(DataPreparationError::Declaration)?; if data.targets().is_empty() {
		return Err(DataPreparationError::MissingTargets); }
	let split = data .split_fraction() .ok_or(DataPreparationError::MissingSplit)?;
	let train_fraction = TrainFraction::from_f32(split).map_err(DataPreparationError::Prepare)?;
	let excluded_columns = data .exclusions() .iter()
		.map(|pattern| ColumnPattern::new(pattern.as_bytes().to_vec()).map_err(DataPreparationError::Prepare))
		.collect::<DataPreparationResult<Vec<_>>>()?; let excluded_rows = data .condition_exclusions() .iter()
		.map(map_condition) .collect::<DataPreparationResult<Vec<_>>>()?; let request = PreparationRequest::new(
		data.targets() .iter() .map(|target| target.as_bytes().to_vec()), train_fraction, ) .exclude_columns(excluded_columns)
	.exclude_rows(excluded_rows); let distilled = distill_data_with_limits(data, limits)?; let inferred = distilled
		.infer_vectors(&CategoricalEncodingModel) .map_err(DataPreparationError::Semantic)?;
	prepare_inferred_table(distilled.table(), &inferred, &request).map_err(DataPreparationError::Prepare) }

fn default_data_limits() -> DataPreparationResult<IngestLimits> { IngestLimits::new( DEFAULT_DATA_SOURCE_BYTES,
		DEFAULT_DATA_RECORDS, DEFAULT_DATA_FIELDS_PER_RECORD, DEFAULT_DATA_FIELD_BYTES, )
	.map_err(DataPreparationError::Ingest) }

fn map_condition(condition: &Condition) -> DataPreparationResult<RowPredicate> {
	let operator = match condition.operator() { ComparisonOperator::Equal => IngestComparisonOperator::Equal,
	...
	let literal = match condition.value() { ConditionValue::Signed(value) => PredicateLiteral::Signed(*value),
		ConditionValue::Unsigned(value) => PredicateLiteral::Unsigned(*value), ConditionValue::FloatBits(bits) => {
			let value = f64::from_bits(*bits); let narrowed = value as f32;
			if !narrowed.is_finite() || value != 0.0 && narrowed == 0.0 {
				return Err(DataPreparationError::FloatPredicateOutsideF32 { column: condition.column().to_owned(), value, }); }
			PredicateLiteral::f32(narrowed) }
		ConditionValue::Boolean(value) => PredicateLiteral::Text(value.to_string()),
		ConditionValue::Text(value) => PredicateLiteral::Text(value.clone()), }; Ok(RowPredicate::new(
		condition.column().as_bytes().to_vec(), operator, literal, )) }
