...
use recipe_ingest::{CategoricalEncodingModel, ColumnPattern, ComparisonOperator as IngestComparisonOperator, DatasetSourceError, DistilledDataset, IngestError, IngestLimits, PredicateLiteral, PreparationRequest, PrepareError, PreparedDataset, RawTable, RowPredicate, SemanticError, TrainFraction, distill_datasets, prepare_inferred_table, select_table};
...
impl fmt::Display for DataPreparationError {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			Self::Declaration(error) => return write!(f, "invalid data declaration: {error}"),
			Self::MissingTargets => {
				return write!(f, "dataset preparation requires at least one target vector");
			}
			Self::MissingSplit => {
				return write!(f, "dataset preparation requires an explicit train split");
			}
			...
			Self::Ingest(error) => return write!(f, "load data source: {error}"),
			Self::Source(error) => return write!(f, "distill data source: {error}"),
			Self::Semantic(error) => return write!(f, "infer data vectors: {error}"),
			Self::Prepare(error) => return write!(f, "prepare data source: {error}"),
		}
	}
}
...
impl std::error::Error for DataPreparationError {
	fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
		match self {
			...
			Self::Ingest(error) => return Some(error),
			Self::Source(error) => return Some(error),
			Self::Semantic(error) => return Some(error),
			Self::Prepare(error) => return Some(error),
			Self::MissingTargets | Self::MissingSplit | Self::FloatPredicateOutsideF32 { .. } => return None,
		}
	}
}
...
pub fn prepare_data(data: &Data) -> DataPreparationResult<PreparedDataset> {
	...
	return prepare_data_with_limits(data, limits);
}
...
/// # Errors
///
/// Returns a declaration error for invalid data policy, a limit error when default bounds cannot be formed, or the
/// source error from distilling any declared input.
pub fn distill_data(data: &Data) -> DataPreparationResult<DistilledDataset> { return distill_data_with_limits(data, default_data_limits()?); }

/// Distill one public data declaration under caller-fixed aggregate bounds.
///
/// # Errors
///
/// Returns a declaration error before reading invalid input, or the source error from bounded distillation.
pub fn distill_data_with_limits(data: &Data, limits: IngestLimits) -> DataPreparationResult<DistilledDataset> {
	...
	return distill_datasets(data.sources().iter().map(std::path::Path::new), limits).map_err(DataPreparationError::Source);
}
...
/// # Errors
///
/// Returns the first invalid exclusion pattern, invalid row condition, or table-selection failure.
pub fn select_target_free_data(data: &Data, distilled: &DistilledDataset) -> DataPreparationResult<RawTable> {
	let excluded_columns = data
		...
		.iter()
		.map(|pattern| {
			return ColumnPattern::new(pattern.as_bytes().to_vec()).map_err(DataPreparationError::Prepare);
		})
	...
	let excluded_rows = data
		...
		.map(map_condition)
		.collect::<DataPreparationResult<Vec<_>>>()?;
	return select_table(distilled.table(), excluded_columns, excluded_rows).map_err(DataPreparationError::Prepare);
}
...
pub fn prepare_data_with_limits(data: &Data, limits: IngestLimits) -> DataPreparationResult<PreparedDataset> {
	...
	let request = PreparationRequest::new(
		data.targets()
			.iter()
			.map(|target| return target.as_bytes().to_vec()),
		train_fraction,
	)
	...
	return prepare_inferred_table(distilled.table(), &inferred, &request).map_err(DataPreparationError::Prepare);
}

fn default_data_limits() -> DataPreparationResult<IngestLimits> {
	return IngestLimits::new(
