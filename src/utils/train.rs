...
use recipe_infer::{
	LayerSpec,
	Loss, Metric, PlanMode, Pt, SCRATCH_CONSTS, Scaler, Scratch, concat_layer,
...
extern "C" fn on_sigint(_sig: i32) {
	let Some(_second) = Some(()).filter(|_probe| INTERRUPTED.swap(1, Ordering::SeqCst) != 0) else {
		return;
	};
...
fn fmt_time_axis(secs: f64) -> String {
	Some(())
		...
		.or_else(|| Some(()).filter(|_probe| secs >= 60.0).map(|_probe| format!("{:.1}m", secs / 60.0)))
...
fn fmt_axis(v: f64) -> String {
	...
	Some(())
		...
		.or_else(|| Some(()).filter(|_probe| a >= 1.0).map(|_probe| format!("{v:.1}")))
...
pub fn loss_grad_into(
	...
	match loss {
		...
		Loss::Huber => {
			...
			kernels::gpu_clamp_into(da, &sc.c_neg_one, &sc.c_one, total, da).context("huber clamp")?;
		...
		Loss::Focal => {
			gpu_core::losses::gpu_focal_grad_into(out, y, &sc.c_focal_gamma, &sc.c_focal_alpha, &ss.inv_n, total, da).context("focal")?;
		}
	}
	Ok(())
}
pub(crate) fn metrics_line(metrics: &[Metric], vals: &[f64]) -> String {
	...
	for mi in 0..metrics.len().min(vals.len()) {
		let m = metrics[mi];
		let Some(_w) = m.fmt().width.checked_sub(1) else { continue };
...
impl ModelInner {
	fn render_dashboard(
		...
		for j in 0..ys.len() {
			...
			let real_lo = Some(()).filter(|_probe| lo.is_finite()).map(|_probe| inv_symlog(lo)).unwrap_or(0.0);
			let real_hi = Some(()).filter(|_probe| hi.is_finite()).map(|_probe| inv_symlog(hi)).unwrap_or(1.0);
			let c = palette(j);
		...
		'find: for x in first.left()..first.right() {
			for y in first.top()..first.bottom() {
				...
				let Some(_vert) = Some(()).filter(|_probe| c.symbol() == symbols::line::VERTICAL) else {
					continue;
				};
		...
		for y in first.top()..last.bottom().saturating_sub(1) {
			...
			let fix = Some(())
				...
				.or_else(|| Some(()).filter(|_probe| corner).map(|_probe| CellFix::Corner))
	...
	pub(crate) fn fit(&self, data: &Dataset, cfg: &Train, resume: Option<&str>, net: Option<std::sync::Arc<Vec<crate::wire::Conn>>>) -> anyhow::Result<()> {
		...
		let coll = Some(()).filter(|_probe| embed_cats).map(|_probe| collapse_onehot(data));
		...
		let cat_cols: Vec<usize> = Some(())
			...
			.map(|_probe| Vec::from_iter((0..effective_x.ncols()).filter(|c| !effective_text.contains(c))))
			.unwrap_or_default();
			...
			None => Some(())
				...
				.map(|_probe| {
					Some(())
						...
						.unwrap_or_else(|| xinput.iter().cloned().fold(0.0f64, f64::max) as usize + 1)
				})
				.unwrap_or(0),
		};
		let cat = Some(())
		...
		let d_sc = Some(()).filter(|_probe| embed_first).map(|_probe| c).unwrap_or(d);
		...
		let plan = match plan_layer_params(&self.specs, d, c_cat, vocab, &source, warm) {
			Ok(p) => p,
			Err(what) => {
				let overwrite = matches!(did_resume, Resumed::Yes) && matches!(ask_overwrite(&what), YesNo::Yes);
				match Some(()).filter(|_probe| overwrite) {
					Some(_ow) => {
						did_resume = Resumed::No;
						plan_layer_params(&self.specs, d, c_cat, vocab, &[], PlanMode::Fresh)
							.map_err(|e| anyhow::anyhow!(e))?
					}
					None => match did_resume {
						Resumed::Yes => anyhow::bail!("checkpoint mismatch — user declined overwrite"),
		...
		let k = Some(()).filter(|_probe| expand_ce).map(|_probe| out_dim).unwrap_or(n_targets);
		assert_eq!(
			out_dim, k,
		...
		let stop_metric = Some(()).filter(|_probe| classify).map(|_probe| Metric::Accuracy).unwrap_or(Metric::R2);
		let mut ring_row: Vec<Metric> = vec![stop_metric];
		let ckpt_loss = Some(()).filter(|_probe| checkpointing).map(|_probe| Metric::Loss);
		for m in cfg
			.metrics
			.iter()
			.copied()
		...
		let zf: recipe_infer::ZFit = match Some(()).filter(|_probe| d_sc == 0) {
			...
			None => {
				let src = match Some(()).filter(|_probe| embed_first) {
					Some(_ef) => cat.as_ref().ok_or_else(|| anyhow::anyhow!("cat matrix"))?,
					None => &xinput,
				};
		...
		let scaled_off = Some(()).filter(|_probe| d_sc > 0).map(|_probe| stage.push(&zf.scaled)).unwrap_or(0);
		...
		let ac_pre = match recipe_infer::concat_layer_dims(&plan.dims()) {
		...
		for e in 0..cfg.epochs {
			let Some(_go) = Some(()).filter(|_probe| INTERRUPTED.load(Ordering::SeqCst) == 0) else {
				break;
			};
			...
			epoch_meta.push(EpochMeta {
				...
				logged: Some(()).filter(|_probe| log_now).map(|_probe| Logged::Yes).unwrap_or(Logged::No),
			});
		}
			...
			.map(|_probe| -> anyhow::Result<()> {
				...
				match Some(()).filter(|_p| classify) {
					Some(_cls) => match Some(()).filter(|_p| k == 1) {
						Some(_bin) => kernels::gpu_accuracy_into(&sc.acts[last], &ybuf, n, &end_slot).context("accuracy")?,
						None => kernels::gpu_argmax_accuracy_into(&sc.acts[last], &ybuf, n, k, &end_slot).context("argmax accuracy")?,
					},
					None => kernels::gpu_ss_res_into(&sc.acts[last], &ybuf, n * k, &end_slot).context("ss_res")?,
				};
				Ok(())
			})
			.transpose()?;
		...
		for meta in &epoch_meta {
			...
			fit_score = Some(()).filter(|_probe| score.is_finite()).map(|_probe| score).unwrap_or(fit_score);
			let mut checkpointed: Option<()> = None;
			let nan_stop = match Some(()).filter(|_probe| checkpointing) {
				Some(_ckpt) => {
					...
					match Some(()).filter(|_probe| loss.is_nan()) {
						...
						None => {
							let carve = ckpt_saved.is_none() && e > 0 && loss > loss_prev;
							Some(())
								.filter(|_probe| carve)
								.map(|_probe| -> anyhow::Result<()> {
									ckpt_saved = Some(());
								...
								.transpose()?;
							loss_prev = Some(()).filter(|_probe| loss.is_finite()).map(|_probe| loss).unwrap_or(loss_prev);
							Halt::Continue
						}
					}
				}
			...
			let Some(_go) = Some(()).filter(|_probe| (logged_flag || checkpointed.is_some()) && !plotting) else {
				continue;
			};
				...
				match Some(()).filter(|_probe| was_interrupted) {
					Some(_intr) => {
						...
						match Some(()).filter(|_probe| better_on_disk) {
							Some(_keep) => eprintln!("keeping {path} (better prior {key} on disk)"),
							None => {
	...
	pub(crate) fn prep_eval_input(&self, ds: &Dataset) -> EvalInput {
		...
		let coll = Some(()).filter(|_probe| embed_cats).map(|_probe| collapse_onehot(ds));
		...
		let cat_cols: Vec<usize> = Some(())
			.filter(|_probe| embed_first)
			.map(|_probe| Vec::from_iter((0..eff_x.ncols()).filter(|c| !eff_text.contains(c))))
			.unwrap_or_default();
		let xinput = Some(())
			.filter(|_probe| embed_first)
