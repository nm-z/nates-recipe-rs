    ...
    fn graph(&self, features: usize) -> Result<TrainingGraph<'_>, RecipeError> {
        ...
        for block in &self.blocks {
            ...
            let output = match &block.operation {
                ...
                OperationBlock::Conv(filters, kernel) => filters.saturating_mul(*kernel),
			...
			let block_parameters = match &block.operation { OperationBlock::Embed(dimensions, vocabulary) if *vocabulary != 0 => dimensions.checked_mul(*vocabulary), OperationBlock::Attention(heads) if *heads != 0 => heads.checked_mul(4), OperationBlock::Embed(..) => return Err(RecipeError::new("embedding vocabulary must be positive")), OperationBlock::Attention(_) => return Err(RecipeError::new("attention head count must be positive")), _ => input.checked_mul(output) }.ok_or_else(|| RecipeError::new("model parameter count overflow"))?;
