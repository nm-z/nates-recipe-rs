...
const BALANCE_BYTES_PER_FLOP: &str = env!("RECIPE_BALANCE_BYTES_PER_FLOP");
...
impl Model {
    ...
    fn graph(&self, features: usize) -> Result<TrainingGraph<'_>, RecipeError> {
        ...
        for block in &self.blocks {
			...
			let recurrent = |width: usize, gates: usize| input.checked_mul(width).and_then(|a| width.checked_mul(width).and_then(|b| a.checked_add(b))).and_then(|value| value.checked_add(width)).and_then(|value| value.checked_mul(gates)); let block_parameters = match &block.operation { OperationBlock::Conv(filters, kernel) => filters.checked_mul(*kernel), OperationBlock::Pool(_) => Some(0), OperationBlock::Embed(dimensions, vocabulary) if *vocabulary != 0 => dimensions.checked_mul(*vocabulary), OperationBlock::Attention(heads) if *heads != 0 && input % heads == 0 => input.checked_mul(input).and_then(|value| value.checked_mul(4)), OperationBlock::Rnn(width) => recurrent(*width, 1), OperationBlock::Gru(width) => recurrent(*width, 3), OperationBlock::Lstm(width) => recurrent(*width, 4), OperationBlock::KMeans(_) | OperationBlock::Forest(..) | OperationBlock::Knn(_) => return Err(RecipeError::new("KMeans, Forest, and KNN are standalone estimators outside the gradient graph")), OperationBlock::Embed(..) => return Err(RecipeError::new("embedding vocabulary must be positive")), OperationBlock::Attention(_) => return Err(RecipeError::new("attention heads must divide the input width")), _ => input.checked_mul(output) }.ok_or_else(|| RecipeError::new("model parameter count overflow"))?;
...
#[rustfmt::skip]
impl OperationBlock { fn name(&self) -> &'static str { match self { Self::Layer(_) => "layer", Self::Conv(..) => "conv", Self::Pool(_) => "pool", Self::KMeans(_) => "kmeans", Self::Embed(..) => "embed", Self::Attention(_) => "attn", Self::Rnn(_) => "rnn", Self::Gru(_) => "gru", Self::Lstm(_) => "lstm", Self::Forest(_, 1, _) => "lgbm", Self::Forest(_, 2, _) => "xgbst", Self::Forest(..) => "forest", Self::Knn(_) => "knn", Self::Residual(_) => "residual", Self::Perceptron(_) => "perc" } } }
#[rustfmt::skip]
impl Activation { const fn name(self) -> &'static str { match self { Self::Linear => "linear", Self::Cosine => "cos", Self::Exponential => "exp", Self::Logarithm => "log", Self::NaturalLogarithm => "ln", Self::LegacySignedLogOnePlus => "log1p", Self::Huber => "huber", Self::Tangent => "tan", Self::Relu => "relu", Self::LeakyRelu => "leak", Self::Sigmoid => "sigmoid", Self::Tanh => "tanh", Self::Selu => "selu", Self::Gelu => "gelu", Self::Silu => "silu", Self::Elu => "elu", Self::PRelu => "prelu" } } }
#[rustfmt::skip]
impl BlockNormalization { const fn name(self) -> &'static str { match self { Self::Batch => "bnorm", Self::Layer => "lnorm" } } }
...
fn gpu_metrics(predictions: &DeviceBuffer, targets: &DeviceBuffer, rows: usize, loss: LossFunction, threads: u32) -> Result<[f64; 3], RecipeError> {
    let runtime = hsa()?; let metrics = DeviceBuffer::new(3)?; let (mut predictions, mut targets, mut output) = (predictions.pointer, targets.pointer, metrics.pointer); let (mut rows, mut loss) = (rows as u32, loss.0 as i32); let mut arguments = [&mut predictions as *mut _ as *mut c_void, &mut targets as *mut _ as *mut c_void, &mut output as *mut _ as *mut c_void, &mut rows as *mut _ as *mut c_void, &mut loss as *mut _ as *mut c_void]; hsa_launch(runtime.metrics, 1, threads, &mut arguments)?; let values = metrics.download()?; Ok([values[0], values[1], values[2]])
}

#[rustfmt::skip]
fn gpu_affine(values: &DeviceBuffer, scale: f64, offset: f64, threads: u32) -> Result<(), RecipeError> {
    let runtime = hsa()?; let mut count = values.elements as u32; let mut values = values.pointer; let (mut scale, mut offset, mut threads) = (scale, offset, threads); let mut arguments = [&mut values as *mut _ as *mut c_void, &mut count as *mut _ as *mut c_void, &mut scale as *mut _ as *mut c_void, &mut offset as *mut _ as *mut c_void, &mut threads as *mut _ as *mut c_void]; hsa_launch(runtime.affine, count as usize, threads, &mut arguments)
}

#[rustfmt::skip]
fn gpu_initialize(count: usize, seed: u64, scale: f64, prelu: &[(usize, f64)], threads: u32) -> Result<DeviceBuffer, RecipeError> {
    let runtime = hsa()?; let values = DeviceBuffer::new(count)?; let mut buffer = values.pointer; let (mut count_arg, mut seed, mut scale, mut threads) = (count as u32, seed, scale, threads); let mut arguments = [&mut buffer as *mut _ as *mut c_void, &mut count_arg as *mut _ as *mut c_void, &mut seed as *mut _ as *mut c_void, &mut scale as *mut _ as *mut c_void, &mut threads as *mut _ as *mut c_void]; hsa_launch(runtime.initialize, count, threads, &mut arguments)?; for (index, value) in prelu { let (mut index, mut value) = (*index as u32, *value); let mut set_arguments = [&mut buffer as *mut _ as *mut c_void, &mut index as *mut _ as *mut c_void, &mut value as *mut _ as *mut c_void]; hsa_launch(runtime.set_value, 1, threads, &mut set_arguments)?; } Ok(values)
}
...
impl TrainingReport {
    ...
    pub fn predictions(&self) -> &[f64] { &self.2 }
}
...
impl Operation {
	...
	const fn output(&self) -> Value {
		match self {
			Self::Conv1d(_, _, _, output, _)
			| Self::Matmul(_, _, output)
			| Self::Add(_, _, output)
			| Self::Relu(_, output) => *output,
		}
	}
}
...
impl Recipe {
    #[must_use]
    pub const fn new() -> Self { Self { tensors: Vec::new(), operations: Vec::new() } }
    ...
    pub fn write(&mut self, value: Value, data: impl IntoIterator<Item = f64>) -> Result<(), RecipeError> {
        let tensor = self.tensor_mut(value)?;
        if !tensor.dtype.is_float() {
            return Err(RecipeError::new("host execution currently accepts floating-point tensors"));
        }
        let data = data.into_iter().map(|value| cast(tensor.dtype, value)).collect::<Vec<_>>();
        let expected = usize::try_from(elements(&tensor.shape)?).map_err(|_| RecipeError::new("tensor element count does not fit host memory"))?;
    ...
    pub fn conv1d(&mut self, input: Value, weights: Value, bias: Option<Value>, stride: u64) -> Result<Value, RecipeError> {
        ...
        let input_tensor = self.tensor_ref(input)?.clone();
        let weights_tensor = self.tensor_ref(weights)?.clone();
        if input_tensor.dtype != weights_tensor.dtype || !input_tensor.dtype.is_float() {
            return Err(RecipeError::new("conv1d input and weights must use the same floating-point dtype"));
        }
        let [n, cin, lin] = rank::<3>(&input_tensor.shape, "conv1d input")?;
        let [cout, weight_cin, kernel] = rank::<3>(&weights_tensor.shape, "conv1d weights")?;
        if cin != weight_cin {
        ...
        if let Some(bias_value) = bias {
            let bias_tensor = self.tensor_ref(bias_value)?;
            if bias_tensor.dtype != input_tensor.dtype || bias_tensor.shape != [cout] {
                return Err(RecipeError::new(format!("conv1d bias must have shape [{cout}] and match the input dtype")));
            }
        }
        let lout = (lin - kernel) / stride + 1;
        let output = self.push_output("output", input_tensor.dtype, vec![n, cout, lout])?;
        self.push_operation(Operation::Conv1d(input, weights, bias, output, stride));
        Ok(output)
    }

    pub fn matmul(&mut self, left: Value, right: Value) -> Result<Value, RecipeError> {
        let left_tensor = self.tensor_ref(left)?.clone();
        let right_tensor = self.tensor_ref(right)?.clone();
        if left_tensor.dtype != right_tensor.dtype || !left_tensor.dtype.is_float() {
            return Err(RecipeError::new("matmul operands must use the same floating-point dtype"));
        }
        let [m, k] = rank::<2>(&left_tensor.shape, "matmul left operand")?;
        let [right_k, n] = rank::<2>(&right_tensor.shape, "matmul right operand")?;
        if k != right_k {
            return Err(RecipeError::new(format!("matmul inner dimensions differ: {k} and {right_k}")));
        }
        let output = self.push_output("matmul_output", left_tensor.dtype, vec![m, n])?;
        self.push_operation(Operation::Matmul(left, right, output));
        Ok(output)
    }
    ...
    pub fn add(&mut self, left: Value, right: Value) -> Result<Value, RecipeError> {
        let left_tensor = self.tensor_ref(left)?.clone();
        let right_tensor = self.tensor_ref(right)?.clone();
        if left_tensor.dtype != right_tensor.dtype || left_tensor.shape != right_tensor.shape {
            return Err(RecipeError::new("add operands must have identical dtype and shape"));
        }
        let output = self.push_output("add_output", left_tensor.dtype, left_tensor.shape)?;
        self.push_operation(Operation::Add(left, right, output));
        Ok(output)
    }

    pub fn relu(&mut self, input: Value) -> Result<Value, RecipeError> {
        let tensor = self.tensor_ref(input)?.clone();
        if !tensor.dtype.is_float() {
            return Err(RecipeError::new("relu requires a floating-point tensor"));
        }
        let output = self.push_output("relu_output", tensor.dtype, tensor.shape)?;
        self.push_operation(Operation::Relu(input, output));
        Ok(output)
    }

    pub fn lower(&self, output: Value) -> Result<Lowering, RecipeError> {
        self.tensor_ref(output)?;
        ...
        let transfer_bytes = reads.iter().chain([&output]).try_fold(0_u64, |sum, value| {
            sum.checked_add(self.tensor_bytes(*value)?).ok_or_else(|| RecipeError::new("transfer byte count overflow"))
        })?;
        let flops = calculations
            .iter()
            .try_fold(0_u64, |sum, calculation| sum.checked_add(calculation.flops()).ok_or_else(|| RecipeError::new("FLOP count overflow")))?;
        let balance = BALANCE_BYTES_PER_FLOP.parse::<f64>().map_err(|_| RecipeError::new("configured balance point is invalid"))?;
    ...
    pub fn run(&mut self, output: Value) -> Result<Vec<f64>, RecipeError> {
        ...
        for operation in operations {
            let data = match operation.clone() {
                ...
                Operation::Relu(input, _) => self.tensor_data(input)?.iter().map(|value| value.max(0.0)).collect(),
            };
            let output = operation.output();
            let dtype = self.tensor_ref(output)?.dtype;
            self.tensor_mut(output)?.data = Some(data.into_iter().map(|value| cast(dtype, value)).collect());
        }
    ...
    fn push_operation(&mut self, operation: Operation) {
        ...
        self.tensors[operation.output().0].producer = Some(index);
    ...
    fn tensor_bytes(&self, value: Value) -> Result<u64, RecipeError> {
        ...
        elements(&tensor.shape)?.checked_mul(tensor.dtype.bytes()).ok_or_else(|| RecipeError::new("tensor byte count overflow"))
    }

    fn tensor_data(&self, value: Value) -> Result<&[f64], RecipeError> {
        let tensor = self.tensor_ref(value)?;
        tensor.data.as_deref().ok_or_else(|| RecipeError::new(format!("tensor {:?} has no data", tensor.name)))
    }
    ...
    fn lower_operation(&self, operation: &Operation) -> Result<Vec<Calculation>, RecipeError> {
        match operation {
            Operation::Conv1d(input, weights, bias, _, stride) => {
                let [n, cin, lin] = rank::<3>(&self.tensor_ref(*input)?.shape, "conv1d input")?;
                let [cout, _, kernel] = rank::<3>(&self.tensor_ref(*weights)?.shape, "conv1d weights")?;
                let lout = (lin - kernel) / stride + 1;
                let conv_flops = product(&[n, cout, lout, cin, kernel, 2])?;
                let mut result = vec![Calculation::Conv1d(n, cin, lin, cout, kernel, *stride, conv_flops)];
                if bias.is_some() {
            ...
            Operation::Matmul(left, right, _) => {
                let [m, k] = rank::<2>(&self.tensor_ref(*left)?.shape, "matmul left operand")?;
                let [_, n] = rank::<2>(&self.tensor_ref(*right)?.shape, "matmul right operand")?;
                Ok(vec![Calculation::Matmul(m, k, n, product(&[m, k, n, 2])?)])
            }
    ...
    fn execute_conv1d(&self, input: Value, weights: Value, bias: Option<Value>, stride: u64) -> Result<Vec<f64>, RecipeError> {
        ...
        let [cout, _, kernel] = rank::<3>(&self.tensor_ref(weights)?.shape, "conv1d weights")?;
        let lout = (lin - kernel) / stride + 1;
        let input_data = self.tensor_data(input)?;
        let weights_data = self.tensor_data(weights)?;
        let bias_data = bias.map(|value| self.tensor_data(value)).transpose()?;
        let mut output = vec![0.0; usize_extent(product(&[n, cout, lout])?)?];
        for batch_index in 0..n {
            for out_channel in 0..cout {
                for position in 0..lout {
                    let mut sum = bias_data.map_or(0.0, |values| values[out_channel as usize]);
                    for in_channel in 0..cin {
                        for offset in 0..kernel {
                            let input_index = ((batch_index * cin + in_channel) * lin + position * stride + offset) as usize;
                            let weight_index = ((out_channel * cin + in_channel) * kernel + offset) as usize;
                            sum += input_data[input_index] * weights_data[weight_index];
                        }
                    }
    ...
    fn execute_matmul(&self, left: Value, right: Value) -> Result<Vec<f64>, RecipeError> {
        let [m, k] = rank::<2>(&self.tensor_ref(left)?.shape, "matmul left operand")?;
        let [_, n] = rank::<2>(&self.tensor_ref(right)?.shape, "matmul right operand")?;
        let left_data = self.tensor_data(left)?;
        let right_data = self.tensor_data(right)?;
        let mut output = vec![0.0; usize_extent(product(&[m, n])?)?];
        for row in 0..m {
            for column in 0..n {
                for inner in 0..k {
...
#[rustfmt::skip]
impl Train {
    pub const fn stop(mut self, value: f64) -> Self { self.stop_tolerance = Some(value); self }
    pub fn resume(mut self, value: impl Into<String>) -> Self { self.resume = Some(value.into()); self }
    pub fn save(mut self, value: impl Into<String>) -> Self { self.save = Some(value.into()); self }
    ...
    fn try_run(&self, model: &Model, data: &Data, run: u64) -> Result<TrainingReport, RecipeError> {
        ...
        let prepared = load_training_data(data)?;
        let (samples, targets, rows, features) = (&prepared.samples, &prepared.targets, prepared.rows, prepared.features);
        let training_rows = ((rows as f64) * data.split) as usize;
        if training_rows == 0 || training_rows > rows {
            return Err(RecipeError::new("training split must select at least one row and no more than the dataset"));
        }
        let graph = model.graph(features)?;
        let initial_weight = configured_f64("RECIPE_TRAIN_INITIAL_WEIGHT", env!("RECIPE_TRAIN_INITIAL_WEIGHT"))?;
        let beta1 = configured_f64("RECIPE_TRAIN_ADAMW_BETA1", env!("RECIPE_TRAIN_ADAMW_BETA1"))?;
        let beta2 = configured_f64("RECIPE_TRAIN_ADAMW_BETA2", env!("RECIPE_TRAIN_ADAMW_BETA2"))?;
        let epsilon = configured_f64("RECIPE_TRAIN_ADAMW_EPSILON", env!("RECIPE_TRAIN_ADAMW_EPSILON"))?;
        let decay = configured_f64("RECIPE_TRAIN_ADAMW_WEIGHT_DECAY", env!("RECIPE_TRAIN_ADAMW_WEIGHT_DECAY"))?;
        let threads = env!("RECIPE_GPU_THREADS").parse::<u32>().map_err(|error| RecipeError::new(format!("invalid RECIPE_GPU_THREADS: {error}")))?;
        if threads == 0 { return Err(RecipeError::new("GPU thread count must be positive")); }
        let device = match data.device.get_or_init(|| { let samples = DeviceBuffer::upload(samples)?; let targets = DeviceBuffer::upload(targets)?; let (target_offset, target_scale) = if data.normalize { let sample_stats = DeviceBuffer::new(2 * features)?; gpu_normalize(&samples, &samples, &sample_stats, rows, features, BlockNormalization::Batch, false, threads)?; let target_stats = DeviceBuffer::new(2)?; gpu_normalize(&targets, &targets, &target_stats, rows, 1, BlockNormalization::Batch, false, threads)?; let values = target_stats.download()?; (values[0], values[1]) } else { (0.0, 1.0) }; Ok(DeviceData { samples, targets, target_offset, target_scale }) }) { Ok(device) => device, Err(error) => return Err(error.clone()) };
        let (samples, targets, target_offset, target_scale) = (&device.samples, &device.targets, device.target_offset, device.target_scale);
        let activation_config = activation_config()?;
        let seed = env!("RECIPE_RANDOM_SEED").parse::<u64>().map_err(|error| RecipeError::new(format!("invalid RECIPE_RANDOM_SEED: {error}")))?;
        let prelu = (0..graph.blocks.len()).filter_map(|stage| graph.prelu(stage).map(|index| (index, activation_config.prelu))).collect::<Vec<_>>();
        ...
        let (mut previous_loss, mut checkpoint_saved, epoch_width) = (f64::NAN, false, self.epochs.to_string().len());
        for epoch in 1..=self.epochs {
            let started = Instant::now();
            let [loss, _, r2, checkpoint] = graph.epoch_gpu(&device_graph, &epoch_buffers, samples, targets, &weights, &config, &moments, &variances, training_rows, threads, model.loss, optimizer, epoch as i32, previous_loss, tolerance, !checkpoint_saved && tolerance > 0.0)?;
            let milliseconds = started.elapsed().as_secs_f64() * 1000.0;
            let checkpointed = checkpoint != 0.0;
            if checkpointed {
            ...
            if !self.log_metrics.is_empty() {
                let topology = model.description(self.log_metrics.iter().any(|metric| metric.0 == 5), self.log_metrics.iter().any(|metric| metric.0 == 6), self.log_metrics.iter().any(|metric| metric.0 == 7)); let (mut output, mut items, mut topology_printed) = (String::new(), 0, false); for metric in &self.log_metrics { let (name, value) = match metric.0 { 0 => ("run", format!("{run:>5}")), 1 => (model.loss.name(), format!("{loss:.4}")), 2 => ("r2", format!("{r2:>7.4}")), 3 => ("time", format!("{milliseconds:>9.3} ms")), 4 => ("epoch", format!("{epoch:>epoch_width$}")), 5..=7 if !topology_printed && !topology.is_empty() => { topology_printed = true; ("", topology.clone()) }, 5..=7 => continue, _ => unreachable!() }; if items != 0 { output.push_str("  "); } let color = ["242;40;60", "0;174;107", "39;125;255", "255;194;0", "135;90;251", "255;122;0", "215;46;130"][items % 7]; if name.is_empty() { output.push_str(&value); } else { output.push_str(name); output.push(' '); output.push_str(&format!("\x1b[38;2;{color}m{value}\x1b[0m")); } items += 1; } if checkpointed { output.push_str("  \x1b[1;32m← checkpoint\x1b[0m"); } eprintln!("{output}");
            }
            initial_loss = if epoch == 1 { loss } else { initial_loss };
            previous_loss = loss;
            if !loss.is_finite() {
                break;
            }
        }
        graph.forward_gpu(&device_graph, samples, training_rows, &weights, &config, threads)?;
        let normalized = &device_graph.pass.values[graph.blocks.len() - 1];
        let [final_loss, _, _] = gpu_metrics(normalized, targets, training_rows, model.loss, threads)?;
        if !checkpoint_saved && let Some(path) = &self.save {
            save_weights(path, model, features, &prepared.schema, &weights.download()?)?;
        }
        gpu_affine(normalized, target_scale, target_offset, threads)?; let predictions = normalized.download()?;
        Ok(TrainingReport(initial_loss, final_loss, predictions))
    }
}
...
#[rustfmt::skip]
enum FeatureType { Numeric(&'static str), Categoric(Vec<String>), Text(usize) }

#[rustfmt::skip]
fn load_training_data(data: &Data) -> Result<&Prepared, RecipeError> {
	match data.prepared.get_or_init(|| prepare_data(data)) { Ok(prepared) => Ok(prepared), Err(error) => Err(error.clone()) }
}

#[rustfmt::skip]
fn prepare_data(data: &Data) -> Result<Prepared, RecipeError> {
	let mut paths = Vec::new();
	for source in &data.sources { collect_files(&expand_home(source)?, &mut paths)?; }
	paths.sort(); paths.dedup();
	let mut tables = Vec::new(); let mut images = BTreeMap::<String, usize>::new();
	for path in paths {
		let bytes = fs::read(&path).map_err(|error| RecipeError::new(format!("cannot read {}: {error}", path.display())))?;
		if let Some(extension) = image_extension(&path) {
			if bytes.is_empty() { return Err(RecipeError::new(format!("image {} is empty", path.display()))); }
			*images.entry(extension.to_owned()).or_default() += 1; continue;
		}
		tables.push(parse_table(&path, &bytes)?);
	}
	let mut matches = Vec::new();
	for (table, value) in tables.iter().enumerate() { for (column, header) in value.headers.iter().enumerate() { let qualified = format!("{}.{}", value.name, header); if data.target == *header || data.target == qualified { matches.push((table, column)); } } }
	if matches.is_empty() { return Err(RecipeError::new(format!("target {:?} is absent", data.target))); }
	if matches.len() != 1 { return Err(RecipeError::new(format!("target {:?} is ambiguous, use file.feature", data.target))); }
	let (table_index, target) = matches[0]; let selected = &tables[table_index];
	let fit_rows = ((selected.rows.len() as f64) * data.split).floor().max(1.0) as usize;
	let kinds = selected.headers.iter().enumerate().map(|(column, _)| infer_feature(selected, column, fit_rows)).collect::<Vec<_>>();
	eprintln!("Feature name:                         Dtype:    Samples:");
	for table in &tables { for (column, header) in table.headers.iter().enumerate() { let kind = infer_feature(table, column, ((table.rows.len() as f64) * data.split).floor().max(1.0) as usize); eprintln!("{:<37} {:<9} {}", format!("{}.{}", table.name, header), kind_name(&kind), table.rows.iter().filter(|row| row.get(column).is_some_and(|value| !value.is_empty())).count()); } }
	for (extension, count) in images { eprintln!("{extension:<37} u8 bytes  {count}"); }
	let columns = selected.headers.iter().enumerate().filter_map(|(column, header)| {
	...
	let features = columns.iter().map(|column| feature_width(&kinds[*column])).sum();
	if features == 0 { return Err(RecipeError::new("dataset has no training features")); }
	let target_categories = categories(selected, target, fit_rows); let mut samples = Vec::new(); let mut targets = Vec::new();
	for row in &selected.rows {
		let mut encoded = Vec::with_capacity(features); let mut valid = true;
		for column in &columns { if let Some(value) = row.get(*column) { valid &= encode(value, &kinds[*column], &mut encoded); } else { valid = false; } }
		let target_value = row.get(target).and_then(|value| value.parse::<f64>().ok()).or_else(|| row.get(target).and_then(|value| target_categories.iter().position(|category| category == value).map(|index| index as f64)));
		if valid && let Some(target_value) = target_value && target_value.is_finite() { samples.extend(encoded); targets.push(target_value); }
	}
...
fn expand_home(source: &str) -> Result<PathBuf, RecipeError> {
	if source == "~" || source.starts_with("~/") { let home = std::env::var_os("HOME").ok_or_else(|| RecipeError::new("HOME is absent"))?; return Ok(PathBuf::from(home).join(source.trim_start_matches("~/"))); } Ok(PathBuf::from(source))
}
...
fn collect_files(path: &Path, files: &mut Vec<PathBuf>) -> Result<(), RecipeError> {
	...
	if metadata.is_file() { files.push(path.to_owned()); return Ok(()); }
	let mut children = fs::read_dir(path).map_err(|error| RecipeError::new(format!("cannot read folder {}: {error}", path.display())))?.collect::<Result<Vec<_>, _>>().map_err(|error| RecipeError::new(format!("cannot read folder {}: {error}", path.display())))?;
	children.sort_by_key(fs::DirEntry::path); for child in children { collect_files(&child.path(), files)?; } Ok(())
}

#[rustfmt::skip]
fn parse_table(path: &Path, bytes: &[u8]) -> Result<Table, RecipeError> {
	let first = bytes.split(|byte| *byte == b'\n').next().ok_or_else(|| RecipeError::new("dataset is empty"))?;
	let delimiter = match [b',', b';', b'\t'].into_iter().max_by_key(|delimiter| first.iter().filter(|byte| *byte == delimiter).count()).filter(|delimiter| first.contains(delimiter)) { Some(value) => value, None => b'\t' };
	let mut records = records(bytes, delimiter)?; if records.is_empty() { return Err(RecipeError::new(format!("dataset {} is empty", path.display()))); }
	let headers = records.remove(0); let width = headers.len(); records.retain(|row| row.len() == width);
	let name = path.file_stem().and_then(|name| name.to_str()).ok_or_else(|| RecipeError::new(format!("file {} has no UTF-8 name", path.display())))?.to_owned();
	Ok(Table { name, headers, rows: records })
}

#[rustfmt::skip]
fn records(bytes: &[u8], delimiter: u8) -> Result<Vec<Vec<String>>, RecipeError> {
	let mut rows = Vec::new(); let mut row = Vec::new(); let mut field = Vec::new(); let mut quoted = false; let mut index = 0;
	while index < bytes.len() { let byte = bytes[index]; if byte == b'"' { if quoted && bytes.get(index + 1) == Some(&b'"') { field.push(b'"'); index += 1; } else { quoted = !quoted; } } else if byte == delimiter && !quoted { row.push(String::from_utf8(field).map_err(|_| RecipeError::new("feature text is not UTF-8"))?); field = Vec::new(); } else if byte == b'\n' && !quoted { row.push(String::from_utf8(field).map_err(|_| RecipeError::new("feature text is not UTF-8"))?.trim_end_matches('\r').to_owned()); field = Vec::new(); if row.iter().any(|value| !value.is_empty()) { rows.push(row); } row = Vec::new(); } else { field.push(byte); } index += 1; }
	if quoted { return Err(RecipeError::new("unterminated quoted feature")); } if !field.is_empty() || !row.is_empty() { row.push(String::from_utf8(field).map_err(|_| RecipeError::new("feature text is not UTF-8"))?); rows.push(row); } Ok(rows)
}
...
fn infer_feature(table: &Table, column: usize, rows: usize) -> FeatureType {
	...
	if !values.is_empty() && values.iter().all(|value| value.parse::<f64>().is_ok()) { return FeatureType::Numeric(numeric_name(&values)); }
...
#[rustfmt::skip]
fn numeric_name(values: &[&String]) -> &'static str { let parsed = values.iter().filter_map(|value| value.parse::<f64>().ok()).collect::<Vec<_>>(); if parsed.iter().all(|value| value.fract() == 0.0) { let min = parsed.iter().copied().fold(f64::INFINITY, f64::min); let max = parsed.iter().copied().fold(f64::NEG_INFINITY, f64::max); if min >= 0.0 { if max <= u8::MAX as f64 { "u8" } else if max <= u16::MAX as f64 { "u16" } else if max <= u32::MAX as f64 { "u32" } else { "u64" } } else if min >= i8::MIN as f64 && max <= i8::MAX as f64 { "i8" } else if min >= i16::MIN as f64 && max <= i16::MAX as f64 { "i16" } else if min >= i32::MIN as f64 && max <= i32::MAX as f64 { "i32" } else { "i64" } } else if parsed.iter().all(|value| (*value as f32) as f64 == *value) { "f32" } else { "f64" } }
#[rustfmt::skip]
fn kind_name(kind: &FeatureType) -> &'static str { match kind { FeatureType::Numeric(name) => name, FeatureType::Categoric(_) => "categoric", FeatureType::Text(_) => "string" } }
#[rustfmt::skip]
fn feature_width(kind: &FeatureType) -> usize { match kind { FeatureType::Numeric(_) => 1, FeatureType::Categoric(values) => values.len(), FeatureType::Text(width) => *width } }
#[rustfmt::skip]
fn encode(value: &str, kind: &FeatureType, output: &mut Vec<f64>) -> bool { match kind { FeatureType::Numeric(_) => if let Ok(value) = value.parse::<f64>() { output.push(value); value.is_finite() } else { false }, FeatureType::Categoric(categories) => { let found = categories.iter().position(|category| category == value); output.extend((0..categories.len()).map(|index| f64::from(Some(index) == found))); true }, FeatureType::Text(width) => { output.extend(value.as_bytes().iter().map(|byte| f64::from(*byte)).chain(std::iter::repeat(0.0)).take(*width)); value.len() <= *width } } }
#[rustfmt::skip]
...
#[rustfmt::skip]
impl Lowering {
    #[must_use] pub fn shape(&self, value: Value) -> Option<&[u64]> { self.tensors.get(value.0).map(|tensor| tensor.shape.as_slice()) }
    #[must_use] pub const fn transfer_bytes(&self) -> u64 { self.transfer_bytes }
    #[must_use] pub const fn flops(&self) -> u64 { self.flops }
    #[must_use] pub fn bytes_per_flop(&self) -> f64 { if self.flops == 0 { f64::INFINITY } else { self.transfer_bytes as f64 / self.flops as f64 } }
    #[must_use] pub fn bound(&self) -> Bound { if self.bytes_per_flop() >= self.balance { Bound::Transfer } else { Bound::Calculation } }
}

#[rustfmt::skip]
impl fmt::Display for Lowering {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        let title = if self.operations.len() == 1 { match self.operations[0] { Operation::Conv1d(..) => "conv", Operation::Matmul(..) => "matmul", Operation::Add(..) => "add", Operation::Relu(..) => "relu" } } else { "recipe" };
        writeln!(formatter, "{title}\n\tshape")?;
        for value in &self.reads { let tensor = &self.tensors[value.0]; writeln!(formatter, "\t\t{:<7} {}", tensor.name, Shape(&tensor.shape))?; }
        let output = &self.tensors[self.output.0];
        writeln!(formatter, "\t\t{:<7} {}\n\ttransfer", output.name, Shape(&output.shape))?;
        for value in &self.reads { let tensor = &self.tensors[value.0]; let bytes = elements(&tensor.shape).map_err(|_| fmt::Error)? * tensor.dtype.bytes(); writeln!(formatter, "\t\tread {:<7} {bytes} bytes", tensor.name)?; }
        let output_bytes = elements(&output.shape).map_err(|_| fmt::Error)? * output.dtype.bytes();
        writeln!(formatter, "\t\twrite {:<6} {output_bytes} bytes\n\t\ttotal        {} bytes\n\tcalc", output.name, self.transfer_bytes)?;
        for calculation in &self.calculations { match calculation {
...
struct Shape<'a>(&'a [u64]);

#[rustfmt::skip]
impl fmt::Display for Shape<'_> {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str("[")?;
...
fn unique_name(tensors: &[Tensor], stem: &str) -> String {
    if !tensors.iter().any(|tensor| tensor.name == stem) { return stem.to_owned(); }
    let mut ordinal = 2_u64;
    loop {
        let candidate = format!("{stem}_{ordinal}");
        if !tensors.iter().any(|tensor| tensor.name == candidate) { return candidate; }
        ordinal += 1;
    }
}
...
fn elements(shape: &[u64]) -> Result<u64, RecipeError> {
	if shape.is_empty() { return Err(RecipeError::new("tensor shape must have at least one dimension")); }
	if shape.contains(&0) { return Err(RecipeError::new("tensor dimensions must be positive")); }
	product(shape)
}
...
fn product(values: &[u64]) -> Result<u64, RecipeError> {
	values.iter().try_fold(1_u64, |product, value| {
		product
			.checked_mul(*value)
			.ok_or_else(|| RecipeError::new("dimension product overflow"))
	})
}

fn rank<const N: usize>(shape: &[u64], role: &str) -> Result<[u64; N], RecipeError> {
	shape.try_into()
		.map_err(|_| RecipeError::new(format!("{role} must have rank {N}")))
}
