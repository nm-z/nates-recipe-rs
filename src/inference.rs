...
use recipe_training::{
	BayesModelArtifact, CheckpointArtifact, CheckpointDecodeLimits, CompiledInference, CompiledKnnInference, CompletedInferenceExecution, CompletedKnnInferenceExecution, DecodedMulticlassClass, GgufLlamaArtifact, InferenceCompileError, InferenceExecutionError, InferenceExecutionLimits, InferencePrediction, InferencePredictionKind, InferencePreparationError, KnnInferencePrediction, KnnInferencePredictionKind, KnnLabelValue, KnnModelArtifact, KnnModelDecodeLimits, SemanticModelArtifact, compile_prepared_bayes_inference, compile_prepared_gguf_llama_inference, compile_prepared_inference,
	compile_prepared_knn_inference, load_gguf_llama_model_file, load_semantic_model_file, prepare_and_execute_local_inference, prepare_and_execute_local_knn_inference, prepare_bayes_inference_table, prepare_checkpoint_inference_table, prepare_gguf_llama_inference_table, prepare_knn_inference_table,
};
use write::always;
...
impl fmt::Display for InferenceError {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			Self::Declaration(error) => return write!(f, "invalid inference declaration: {error}"),
			Self::Data(error) => return write!(f, "prepare inference data: {error}"),
			Self::Model(error) => return write!(f, "load inference model: {error}"),
			Self::Compile(error) => return write!(f, "compile inference graph: {error}"),
			Self::Native(error) => return write!(f, "prepare current native system: {error}"),
			Self::Execute(error) => return write!(f, "execute native inference: {error}"),
			Self::Runtime { stage, detail } => return write!(f, "{stage}: {detail}"),
			Self::Unsupported { detail } => {
				return write!(f, "unsupported inference declaration: {detail}");
			}
		}
	}
}
...
impl std::error::Error for InferenceError {
	fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
		match self {
			...
			Self::Model(error) => return Some(error),
			Self::Compile(error) => return Some(error),
			Self::Native(error) => return Some(error),
			Self::Execute(error) => return Some(error),
			Self::Runtime { .. } | Self::Unsupported { .. } => return None,
		}
	}
}
...
impl From<InferencePreparationError> for InferenceError {
	fn from(error: InferencePreparationError) -> Self { return Self::Model(error); }
}

impl From<InferenceCompileError> for InferenceError {
	fn from(error: InferenceCompileError) -> Self { return Self::Compile(error); }
}
...
impl From<InferenceExecutionError> for InferenceError {
	fn from(error: InferenceExecutionError) -> Self { return Self::Execute(error); }
}
...
impl CompiledModelInference {
	...
	pub const fn kind(&self) -> InferenceModelKind {
		match self {
			Self::Dense(_) => return InferenceModelKind::Dense,
			Self::Knn(_) => return InferenceModelKind::Knn,
			Self::Bayes(_) => return InferenceModelKind::Bayes,
			Self::GgufLlama(_) => return InferenceModelKind::GgufLlama,
		}
	}
}
...
impl CompletedModelInference {
	const fn elapsed(&self) -> Duration {
		match self {
			Self::Dense(execution) | Self::Bayes(execution) | Self::GgufLlama(execution) => {
				return execution.elapsed();
			}
			Self::Knn(execution) => return execution.elapsed(),
		}
	}
}
...
impl InferenceReport {
	...
	pub const fn kind(&self) -> InferenceModelKind {
		match self.payload {
			InferenceReportPayload::Dense { .. } => return InferenceModelKind::Dense,
			InferenceReportPayload::Knn { .. } => return InferenceModelKind::Knn,
			InferenceReportPayload::Bayes { .. } => return InferenceModelKind::Bayes,
			InferenceReportPayload::GgufLlama { .. } => return InferenceModelKind::GgufLlama,
		}
	}
	...
	pub const fn run(&self) -> RunId {
		match &self.payload {
			InferenceReportPayload::Dense { execution, .. } => return execution.run(),
			InferenceReportPayload::Knn { execution, .. } => return execution.run(),
			InferenceReportPayload::Bayes { execution, .. } => return execution.run(),
			InferenceReportPayload::GgufLlama { execution, .. } => return execution.run(),
		}
	}
	...
	pub const fn bundle(&self) -> BundleIdentity {
		match &self.payload {
			InferenceReportPayload::Dense { execution, .. } => return execution.bundle(),
			InferenceReportPayload::Knn { execution, .. } => return execution.bundle(),
			InferenceReportPayload::Bayes { execution, .. } => return execution.bundle(),
			InferenceReportPayload::GgufLlama { execution, .. } => return execution.bundle(),
		}
	}
	...
	pub const fn prediction(&self) -> Option<&InferencePrediction> {
		match &self.payload {
			InferenceReportPayload::Dense { execution, .. } => return Some(execution.prediction()),
			InferenceReportPayload::Knn { .. } => return None,
			InferenceReportPayload::Bayes { execution, .. } => return Some(execution.prediction()),
			InferenceReportPayload::GgufLlama { execution, .. } => return Some(execution.prediction()),
		}
	}
	...
	pub fn knn_predictions(&self) -> Option<&[KnnInferencePrediction]> {
		match &self.payload {
			InferenceReportPayload::Dense { .. } | InferenceReportPayload::Bayes { .. } | InferenceReportPayload::GgufLlama { .. } => return None,
			InferenceReportPayload::Knn { execution, .. } => return Some(execution.predictions()),
		}
	}
	...
	pub const fn journal(&self) -> &RunJournal {
		match &self.payload {
			InferenceReportPayload::Dense { execution, .. } => return execution.journal(),
			InferenceReportPayload::Knn { execution, .. } => return execution.journal(),
			InferenceReportPayload::Bayes { execution, .. } => return execution.journal(),
			InferenceReportPayload::GgufLlama { execution, .. } => return execution.journal(),
		}
	}
	...
	pub const fn native_kernels(&self) -> Option<&recipe_training::RealizedNativeKernelSet> {
		match &self.payload {
			InferenceReportPayload::Dense { execution, .. } | InferenceReportPayload::Bayes { execution, .. } | InferenceReportPayload::GgufLlama { execution, .. } => return Some(execution.native_kernels()),
			InferenceReportPayload::Knn { .. } => return None,
		}
	}
	...
	pub const fn native_evidence(&self) -> &recipe_native_executor::NativeExecutionEvidence {
		match &self.payload {
			InferenceReportPayload::Dense { execution, .. } | InferenceReportPayload::Bayes { execution, .. } | InferenceReportPayload::GgufLlama { execution, .. } => return execution.native_evidence(),
			InferenceReportPayload::Knn { execution, .. } => return execution.native_evidence(),
		}
	}
	...
	#[must_use]
	pub fn devices(&self) -> &[String] { return &self.devices; }
	...
	pub fn decode_multiclass_class(&self, class: usize) -> Option<DecodedMulticlassClass<'_>> {
		match &self.payload {
			InferenceReportPayload::Dense { checkpoint, .. } => return checkpoint.decode_multiclass_class(class),
			InferenceReportPayload::Knn { .. } | InferenceReportPayload::Bayes { .. } | InferenceReportPayload::GgufLlama { .. } => return None,
		}
	}
}
...
pub(crate) fn evaluate_inference_declaration(declaration: &InferenceDeclaration) -> InferenceResult<InferenceReport> {
	crate::api::select_log_output(declaration.policy().log_items());
	...
	write_inference_report(&mut rendered, declaration.policy(), &report).map_err(|error| return InferenceError::runtime("write inference report", error.to_string()))?;
	let rendered = String::from_utf8(rendered).map_err(|error| return InferenceError::runtime("encode inference report", error.to_string()))?;
	always(rendered);
	return Ok(report);
}
...
/// # Errors
///
/// Returns the first declaration, target-free data preparation, checkpoint loading, or inference compilation error.
pub fn compile_inference(policy: &Infer, data: &Data, model: &Model) -> InferenceResult<CompiledModelInference> { return compile_inference_package(policy, data, model).map(|package| return package.inference); }

pub(crate) fn compile_inference_declaration(declaration: &InferenceDeclaration) -> InferenceResult<CompiledInferencePackage> {
	let data = declaration
		.data()
		.ok_or_else(|| return InferenceError::unsupported("inference evaluation requires one data declaration"))?;
	return compile_inference_package(declaration.policy(), data, declaration.model());
}
...
fn compile_inference_package(policy: &Infer, data: &Data, model: &Model) -> InferenceResult<CompiledInferencePackage> {
	...
	let inference = match &model {
		LoadedInferenceModel::Semantic(SemanticModelArtifact::Dense(checkpoint)) => {
			let prepared = prepare_checkpoint_inference_table(checkpoint.as_ref().clone(), &selected)?;
	...
	return Ok(CompiledInferencePackage { model, inference });
}
...
fn require_inference_model_source(model: &Model) -> InferenceResult<&Path> {
	let source = model.weights_source().ok_or_else(|| {
		return InferenceError::unsupported("inference requires `recipe.model().load(MODEL_PATH)`");
	})?;
...
fn gguf_limits_for_file(path: &Path) -> InferenceResult<GgufLimits> {
	...
	return GgufLimits::new(bound, bound, bound, 4, bound, bound, bound).map_err(|error| {
		return InferenceError::runtime(
...
fn execute_current_inference_native(inference: &CompiledModelInference) -> InferenceResult<(CompletedModelInference, Vec<String>)> {
	let run = next_run_id();
	let result = with_current_native_preparation(|profile, _config, scope| {
		let devices = scope
			...
			.devices()
			.iter()
			.map(|target| return target.origin().as_str().to_owned())
			.collect::<Vec<_>>();
		...
		let tuning = derive_native_runtime_tuning(graph, profile, host.machine()).map_err(|error| return NativePreparationError::LocalConfiguration(error.to_string()))?;
		...
		let execution = match inference {
			CompiledModelInference::Dense(inference) => prepare_and_execute_local_inference(inference, profile, &mut preparer, run, limits).map(CompletedModelInference::Dense),
			CompiledModelInference::Knn(inference) => prepare_and_execute_local_knn_inference(inference, profile, &mut preparer, run, limits).map(CompletedModelInference::Knn),
			CompiledModelInference::Bayes(inference) => prepare_and_execute_local_inference(inference, profile, &mut preparer, run, limits).map(CompletedModelInference::Bayes),
			CompiledModelInference::GgufLlama(inference) => prepare_and_execute_local_inference(inference, profile, &mut preparer, run, limits).map(CompletedModelInference::GgufLlama),
		};
		return Ok((execution, devices));
	})?;
...
fn write_inference_report(writer: &mut impl Write, policy: &Infer, report: &InferenceReport) -> io::Result<()> {
	...
	return write_prediction_rows(writer, report);
}

fn write_prediction_rows(writer: &mut impl Write, report: &InferenceReport) -> io::Result<()> {
	match &report.payload {
		InferenceReportPayload::Dense { .. } => return write_dense_prediction_rows(writer, report),
		InferenceReportPayload::Knn {
			artifact,
			execution,
		} => return write_knn_prediction_rows(writer, artifact, execution),
		InferenceReportPayload::Bayes {
			artifact,
			execution,
		} => return write_bayes_prediction_rows(writer, artifact, execution),
		InferenceReportPayload::GgufLlama {
			artifact,
			execution,
		} => return write_gguf_llama_prediction_rows(writer, artifact, execution),
	}
}
		...
		match contract.kind() {
			...
			InferencePredictionKind::MulticlassProbabilities => {
				...
				for (index, value) in row_image
					...
					.0
					.iter()
					.map(|bytes| return decode_prediction_f32(bytes))
					.enumerate()
				{
					if index != 0 {
						writer.write_all(b",")?;
...
fn write_gguf_llama_prediction_rows(writer: &mut impl Write, artifact: &GgufLlamaArtifact, execution: &CompletedInferenceExecution) -> io::Result<()> {
	...
	let expected_bytes = usize::try_from(*positions)
		.ok()
		.and_then(|positions| return positions.checked_mul(row_bytes))
		.ok_or_else(|| {
			return io::Error::new(
				io::ErrorKind::InvalidData,
...
fn write_named_dense_values(writer: &mut impl Write, checkpoint: &CheckpointArtifact, row_image: &[u8]) -> io::Result<()> {
	let values = row_image
		.as_chunks::<4>()
		.0
		.iter()
		.map(|bytes| return decode_prediction_f32(bytes));
...
fn write_dense_target_name(writer: &mut impl Write, checkpoint: &CheckpointArtifact, column: usize) -> io::Result<()> {
	...
	let vector = checkpoint
		...
		.iter()
		.find(|vector| {
			return vector.source_index() == source_index && vector.role() == recipe_ingest::VectorRole::Target;
		})
		.ok_or_else(|| {
			return io::Error::new(
				io::ErrorKind::InvalidData,
	...
	return write_quoted_bytes(writer, vector.name());
}

fn write_bayes_prediction_rows(writer: &mut impl Write, artifact: &BayesModelArtifact, execution: &CompletedInferenceExecution) -> io::Result<()> {
	let prediction = execution.prediction();
	let contract = prediction.contract();
	...
	let expected_width = artifact
		.conditionals()
		.iter()
		.try_fold(0usize, |width, conditional| {
			return width.checked_add(conditional.child_classes());
		})
		.ok_or_else(|| {
			return io::Error::new(
				io::ErrorKind::InvalidData,
	...
	let expected_bytes = usize::try_from(*rows)
		.ok()
		.and_then(|rows| return rows.checked_mul(row_bytes))
		.ok_or_else(|| {
			return io::Error::new(
				io::ErrorKind::InvalidData,
...
fn write_knn_prediction_rows(writer: &mut impl Write, artifact: &KnnModelArtifact, execution: &CompletedKnnInferenceExecution) -> io::Result<()> {
	...
	let rows = predictions.first().map_or(0, |prediction| {
		let [rows, 1] = prediction.contract().shape().extents() else {
			unreachable!("validated KNN prediction shape is [rows, 1]");
		};
		return usize::try_from(*rows).expect("validated KNN row count fits usize");
	});
	for row in 0..rows {
		for (output_index, (prediction, output)) in predictions.iter().zip(outputs).enumerate() {
			...
			match prediction.contract().kind() {
				...
				KnnInferencePredictionKind::DiscreteMode => {
					let code = i32::from_le_bytes(scalar.try_into().expect("one validated KNN i32 prediction"));
...
fn write_knn_label(writer: &mut impl Write, label: &KnnLabelValue) -> io::Result<()> {
	match label {
		KnnLabelValue::I32(value) => return write!(writer, "{value}"),
		KnnLabelValue::Bytes(value) => return write_quoted_bytes(writer, value),
	}
}

fn decode_prediction_f32(bytes: &[u8]) -> f32 { return f32::from_le_bytes(bytes.try_into().expect("one validated prediction f32")); }
...
fn write_multiclass_label(writer: &mut impl Write, label: Option<DecodedMulticlassClass<'_>>) -> io::Result<()> {
	match label {
		Some(DecodedMulticlassClass::ReservedUnseen) => return writer.write_all(b"<reserved-unseen>"),
		Some(DecodedMulticlassClass::Label(bytes)) => return write_quoted_bytes(writer, bytes),
		None => return writer.write_all(b"<unavailable>"),
	}
}
