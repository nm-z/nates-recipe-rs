use core::fmt; use std::{ io::{self, Write}, path::Path, time::Duration, };

use recipe_core::{BundleIdentity, RunId}; use recipe_executor::RunJournal; use recipe_ingest::GgufLimits;
...
#[derive(Debug)]
#[non_exhaustive]
pub enum InferenceError { Declaration(DeclarationError), Data(DataPreparationError), Model(InferencePreparationError),
	Compile(InferenceCompileError), Native(NativePreparationError), Execute(InferenceExecutionError),
	Runtime { stage: &'static str, detail: String },
	Unsupported { detail: String }, }

impl InferenceError { fn unsupported(detail: impl Into<String>) -> Self { Self::Unsupported { detail: detail.into(), } }
...
impl std::error::Error for InferenceError {
	fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
		match self { Self::Declaration(error) => Some(error), Self::Data(error) => Some(error),
			Self::Model(error) => Some(error), Self::Compile(error) => Some(error), Self::Native(error) => Some(error),
			Self::Execute(error) => Some(error), Self::Runtime { .. } | Self::Unsupported { .. } => None, } } }

impl From<DeclarationError> for InferenceError { fn from(error: DeclarationError) -> Self { Self::Declaration(error) } }

impl From<DataPreparationError> for InferenceError { fn from(error: DataPreparationError) -> Self { Self::Data(error) }
}
...
pub enum InferenceModelKind { Dense, Knn, Bayes, GgufLlama, }

/// One immutable native inference program selected from a semantic model.
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum CompiledModelInference { Dense(CompiledInference), Knn(CompiledKnnInference), Bayes(CompiledInference),
	GgufLlama(CompiledInference), }

impl CompiledModelInference {
	#[must_use]
	pub const fn kind(&self) -> InferenceModelKind { match self { Self::Dense(_) => InferenceModelKind::Dense,
			Self::Knn(_) => InferenceModelKind::Knn, Self::Bayes(_) => InferenceModelKind::Bayes,
			Self::GgufLlama(_) => InferenceModelKind::GgufLlama, } } }

#[derive(Clone, Debug)]
enum InferenceReportPayload { Dense { checkpoint: CheckpointArtifact, execution: CompletedInferenceExecution, }, Knn {
		artifact: KnnModelArtifact, execution: CompletedKnnInferenceExecution, }, Bayes { artifact: BayesModelArtifact,
		execution: CompletedInferenceExecution, }, GgufLlama { artifact: GgufLlamaArtifact,
		execution: CompletedInferenceExecution, }, }

#[derive(Clone, Debug)]
enum CompletedModelInference { Dense(CompletedInferenceExecution), Knn(CompletedKnnInferenceExecution),
	Bayes(CompletedInferenceExecution), GgufLlama(CompletedInferenceExecution), }

impl CompletedModelInference { const fn elapsed(&self) -> Duration { match self {
			Self::Dense(execution) | Self::Bayes(execution) | Self::GgufLlama(execution) => execution.elapsed(),
			Self::Knn(execution) => execution.elapsed(), } } }

#[derive(Clone, Debug)]
enum LoadedInferenceModel { Semantic(SemanticModelArtifact), GgufLlama(GgufLlamaArtifact), }

/// Fully exited target-free inference result.
#[derive(Clone, Debug)]
pub struct InferenceReport { payload: InferenceReportPayload, elapsed: Duration, devices: Vec<String>, }

impl InferenceReport {
	#[must_use]
	pub const fn kind(&self) -> InferenceModelKind { match self.payload {
	...
	#[must_use]
	pub const fn run(&self) -> RunId { match &self.payload {
	...
	#[must_use]
	pub const fn bundle(&self) -> BundleIdentity { match &self.payload {
	...
	#[must_use]
	pub const fn prediction(&self) -> Option<&InferencePrediction> { match &self.payload {
	...
	/// for a dense report.
	#[must_use]
	pub fn knn_predictions(&self) -> Option<&[KnnInferencePrediction]> { match &self.payload {
	...
	#[must_use]
	pub const fn journal(&self) -> &RunJournal { match &self.payload {
	...
	#[must_use]
	pub const fn native_kernels(&self) -> Option<&recipe_training::RealizedNativeKernelSet> { match &self.payload {
	...
	#[must_use]
	pub const fn native_evidence(&self) -> &recipe_native_executor::NativeExecutionEvidence { match &self.payload {
			InferenceReportPayload::Dense { execution, .. }
			| InferenceReportPayload::Bayes { execution, .. }
	...
	pub fn values(&self) -> impl ExactSizeIterator<Item = f32> + '_ {
		self.prediction() .map_or(&[][..], InferencePrediction::bytes) .chunks_exact(4)
	...
	pub fn decode_multiclass_class(&self, class: usize) -> Option<DecodedMulticlassClass<'_>> {
		match &self.payload { InferenceReportPayload::Dense { checkpoint, .. } => checkpoint.decode_multiclass_class(class),
...
#[derive(Clone, Debug)]
pub(crate) struct CompiledInferencePackage { model: LoadedInferenceModel, inference: CompiledModelInference, }
...
pub(crate) fn evaluate_inference_declaration(declaration: &InferenceDeclaration) -> InferenceResult<InferenceReport> {
	...
	let (execution, devices) = execute_current_inference_native(&package.inference)?; let elapsed = execution.elapsed();
	let payload = match (package.model, execution) { (
			LoadedInferenceModel::Semantic(SemanticModelArtifact::Dense(checkpoint)), CompletedModelInference::Dense(execution),
		) => { InferenceReportPayload::Dense { checkpoint, execution, } }
		( LoadedInferenceModel::Semantic(SemanticModelArtifact::Knn(artifact)), CompletedModelInference::Knn(execution),
		) => { InferenceReportPayload::Knn { artifact, execution, } }
		( LoadedInferenceModel::Semantic(SemanticModelArtifact::Bayes(artifact)), CompletedModelInference::Bayes(execution),
		) => { InferenceReportPayload::Bayes { artifact, execution, } }
		(LoadedInferenceModel::GgufLlama(artifact), CompletedModelInference::GgufLlama(execution)) => {
			InferenceReportPayload::GgufLlama { artifact, execution, } }
		_ => unreachable!("compiled and completed model families remain identical"),
	}; let report = InferenceReport { payload, elapsed, devices, };
...
pub fn compile_inference(policy: &Infer, data: &Data, model: &Model) -> InferenceResult<CompiledModelInference> { compile_inference_package(policy, data, model).map(|package| package.inference) }

pub(crate) fn compile_inference_declaration( declaration: &InferenceDeclaration,
) -> InferenceResult<CompiledInferencePackage> { let data = declaration .data()
...
fn compile_inference_package(policy: &Infer, data: &Data, model: &Model) -> InferenceResult<CompiledInferencePackage> {
	policy.validate()?; data.validate()?; model.validate()?; require_target_free_data_policy(data)?;
	let source = require_inference_model_source(model)?;
	let model = match source.extension().and_then(|extension| extension.to_str()) {
		Some("ogdl") => {
			LoadedInferenceModel::Semantic(load_semantic_model_file( source, CheckpointDecodeLimits::default(),
				KnnModelDecodeLimits::default(), )?) }
		Some("gguf") => {
			LoadedInferenceModel::GgufLlama(load_gguf_llama_model_file( source, gguf_limits_for_file(source)?, )?) }
		_ => unreachable!("model extension was validated before loading"),
	}; let distilled = distill_data(data)?; let selected = select_target_free_data(data, &distilled)?;
	let inference = match &model { LoadedInferenceModel::Semantic(SemanticModelArtifact::Dense(checkpoint)) => {
...
fn require_target_free_data_policy(data: &Data) -> InferenceResult<()> { if !data.targets().is_empty() {
		return Err(InferenceError::unsupported(
			"target-free inference does not consume `.target(...)`; the saved model supplies target interpretation",
		)); }
	if data.split_fraction().is_some() { return Err(InferenceError::unsupported(
			"target-free inference evaluates every declared row and does not accept `.split(...)`",
		)); }
	if data.normalization().is_some() { return Err(InferenceError::unsupported(
			"inference normalization is fixed by the semantic model and cannot be redeclared on data",
		)); }
	Ok(()) }

fn require_inference_model_source(model: &Model) -> InferenceResult<&Path> { let source = model .weights_source()
		.ok_or_else(|| InferenceError::unsupported("inference requires `recipe.model().load(MODEL_PATH)`"))?;
	let path = Path::new(source); match path.extension().and_then(|extension| extension.to_str()) {
...
fn gguf_limits_for_file(path: &Path) -> InferenceResult<GgufLimits> { let file_bytes = std::fs::metadata(path)
		.map_err(|error| { InferenceError::runtime(
				"inspect GGUF inference model length",
				format!("{}: {error}", path.display()),
			) })? .len(); let bound = file_bytes.max(1);
	GgufLimits::new(bound, bound, bound, 4, bound, bound, bound).map_err(|error| { InferenceError::runtime(
			"bound GGUF inference model",
			format!("{}: {error}", path.display()),
		) }) }

fn execute_current_inference_native( inference: &CompiledModelInference,
) -> InferenceResult<(CompletedModelInference, Vec<String>)> { let run = next_run_id();
	let result = with_current_native_preparation(|profile, _config, scope| { let devices = scope .targets() .devices()
			.iter() .map(|target| target.origin().as_str().to_owned()) .collect::<Vec<_>>();
		let (bindings, host, targets) = scope.into_parts(); let graph = match inference {
		...
		let limits = InferenceExecutionLimits::new(tuning.watchdog); let host = host
		...
		let driver = NativeExecutorDriver::new(factory); let compiler = targets .deferred_compiler()
		...
		let mut preparer = Preparer::new(provider, realizer); let execution = match inference {
			...
			CompiledModelInference::GgufLlama(inference) => {
				prepare_and_execute_local_inference(inference, profile, &mut preparer, run, limits)
					.map(CompletedModelInference::GgufLlama) } }; Ok((execution, devices)) })?; let (execution, devices) = result;
	Ok((execution?, devices)) }

fn write_inference_report(writer: &mut impl Write, policy: &Infer, report: &InferenceReport) -> io::Result<()> {
	if policy .log_items() .iter() .any(|item| item.metric() == Metric::Time)
	{
		writeln!(writer, "time\t{:.6}s", report.elapsed().as_secs_f64())?;
	}
	if policy .log_items() .iter() .any(|item| item.metric() == Metric::Device)
	{
		writeln!(writer, "device\t{}", report.devices().join(","))?;
	}
	write_prediction_rows(writer, report) }

fn write_prediction_rows(writer: &mut impl Write, report: &InferenceReport) -> io::Result<()> { match &report.payload {
		InferenceReportPayload::Dense { .. } => write_dense_prediction_rows(writer, report), InferenceReportPayload::Knn {
			artifact, execution, } => write_knn_prediction_rows(writer, artifact, execution), InferenceReportPayload::Bayes {
			artifact, execution, } => write_bayes_prediction_rows(writer, artifact, execution),
		InferenceReportPayload::GgufLlama { artifact, execution,
...
fn write_dense_prediction_rows(writer: &mut impl Write, report: &InferenceReport) -> io::Result<()> {
	let checkpoint = match &report.payload { InferenceReportPayload::Dense { checkpoint, .. } => checkpoint,
	...
	let contract = prediction.contract(); let shape = contract.shape().extents(); let [rows, width] = shape else {
	...
	let row_bytes = width .checked_mul(core::mem::size_of::<f32>())
		.expect("validated prediction row byte count fits usize");
	let bytes = prediction.bytes(); debug_assert_eq!(bytes.len(), usize::try_from(*rows).unwrap() * row_bytes);
	for (row, row_image) in bytes.chunks_exact(row_bytes).enumerate() { match contract.kind() {
			InferencePredictionKind::BinaryProbability => { writeln!( writer,
					"prediction\t{row}\tprobability\t{}",
					decode_prediction_f32(&row_image[..4]) )?; }
			InferencePredictionKind::Regression => { writeln!( writer,
					"prediction\t{row}\tvalue\t{}",
					decode_prediction_f32(&row_image[..4]) )?; }
			InferencePredictionKind::MulticlassProbabilities => { let class = multiclass_argmax(row_image);
				...
				for (index, value) in row_image .chunks_exact(4) .map(decode_prediction_f32) .enumerate()
				{ if index != 0 {
						writer.write_all(b",")?;
					}
					write!(writer, "{value}")?;
			...
			InferencePredictionKind::MultiTargetBinaryProbabilities => {
				...
				write_named_dense_values(writer, checkpoint, row_image)?; writeln!(writer)?; }
			InferencePredictionKind::JointTargetProbabilities => { let class = multiclass_argmax(row_image);
...
fn write_gguf_llama_prediction_rows( writer: &mut impl Write, artifact: &GgufLlamaArtifact,
	execution: &CompletedInferenceExecution, ) -> io::Result<()> { let prediction = execution.prediction();
	let contract = prediction.contract(); if contract.kind() != InferencePredictionKind::TokenLogits {
		return Err(io::Error::new( io::ErrorKind::InvalidData,
			"GGUF llama prediction is not a raw token-logit matrix",
		)); }
	let [positions, vocabulary] = contract.shape().extents() else { return Err(io::Error::new( io::ErrorKind::InvalidData,
			"GGUF llama prediction is not rank two",
		)); }; if *vocabulary != artifact.vocabulary() { return Err(io::Error::new( io::ErrorKind::InvalidData,
			"GGUF llama prediction width differs from the model vocabulary",
		)); }
	let vocabulary = usize::try_from(*vocabulary).map_err(|error| { io::Error::new( io::ErrorKind::InvalidData,
			format!("GGUF llama vocabulary width: {error}"),
		) })?; let row_bytes = vocabulary .checked_mul(core::mem::size_of::<f32>()) .ok_or_else(|| { io::Error::new(
				io::ErrorKind::InvalidData,
				"GGUF llama logit row size overflowed",
			) })?; let expected_bytes = usize::try_from(*positions) .ok() .and_then(|positions| positions.checked_mul(row_bytes))
		.ok_or_else(|| { io::Error::new( io::ErrorKind::InvalidData,
				"GGUF llama logit size overflowed",
			) })?; if prediction.bytes().len() != expected_bytes { return Err(io::Error::new( io::ErrorKind::InvalidData,
	...
	for (position, row_image) in prediction.bytes().chunks_exact(row_bytes).enumerate() {
		let token = multiclass_argmax(row_image); let logit = decode_prediction_f32(&row_image[token * 4..token * 4 + 4]);
		writeln!( writer,
			"prediction\t{position}\ttoken\t{token}\tlogit\t{logit}"
		)?; }
	Ok(()) }

fn write_named_dense_values( writer: &mut impl Write, checkpoint: &CheckpointArtifact, row_image: &[u8],
) -> io::Result<()> { let values = row_image.chunks_exact(4).map(decode_prediction_f32);
	if values.len() != checkpoint.target_source_indices().len() { return Err(io::Error::new( io::ErrorKind::InvalidData,
			"dense prediction width differs from saved target order",
		)); }
	writer.write_all(b"[")?;
	for (column, value) in values.enumerate() { if column != 0 {
			writer.write_all(b",")?;
		}
...
fn write_dense_target_name(writer: &mut impl Write, checkpoint: &CheckpointArtifact, column: usize) -> io::Result<()> {
	let source_index = checkpoint .target_source_indices() .get(column) .copied() .ok_or_else(|| { io::Error::new(
				io::ErrorKind::InvalidData,
				"dense target column is out of range",
			) })?; let vector = checkpoint .vectors() .iter()
...
fn write_bayes_prediction_rows( writer: &mut impl Write, artifact: &BayesModelArtifact,
	execution: &CompletedInferenceExecution, ) -> io::Result<()> { let prediction = execution.prediction();
	let contract = prediction.contract(); if contract.kind() != InferencePredictionKind::BayesProbabilities {
		return Err(io::Error::new( io::ErrorKind::InvalidData,
			"Bayesian prediction is not a categorical probability matrix",
		)); }
	let [rows, width] = contract.shape().extents() else { return Err(io::Error::new( io::ErrorKind::InvalidData,
			"Bayesian prediction is not rank two",
		)); }; let width = usize::try_from(*width).map_err(|error| { io::Error::new( io::ErrorKind::InvalidData,
			format!("Bayesian class width: {error}"),
		) })?; let expected_width = artifact .conditionals() .iter() .try_fold(0_usize, |width, conditional| {
			width.checked_add(conditional.child_classes()) }) .ok_or_else(|| { io::Error::new( io::ErrorKind::InvalidData,
				"Bayesian saved output width overflowed",
			) })?; if width != expected_width { return Err(io::Error::new( io::ErrorKind::InvalidData,
			format!("Bayesian prediction has width {width} for {expected_width} saved child labels"),
		)); }
	let row_bytes = width .checked_mul(core::mem::size_of::<f32>()) .ok_or_else(|| { io::Error::new(
				io::ErrorKind::InvalidData,
				"Bayesian prediction row size overflowed",
			) })?; let expected_bytes = usize::try_from(*rows) .ok() .and_then(|rows| rows.checked_mul(row_bytes))
		.ok_or_else(|| { io::Error::new( io::ErrorKind::InvalidData,
				"Bayesian prediction size overflowed",
			) })?; if prediction.bytes().len() != expected_bytes { return Err(io::Error::new( io::ErrorKind::InvalidData,
			"Bayesian prediction byte count differs from its matrix shape",
		)); }
	for (row, row_image) in prediction.bytes().chunks_exact(row_bytes).enumerate() { let mut start = 0_usize;
		for (output, conditional) in artifact.conditionals().iter().enumerate() {
			let labels = conditional.child().dictionary(); let end = start + labels.len();
			let output_image = &row_image[start * 4..end * 4]; let class = multiclass_argmax(output_image);
			...
			write!(writer, "\tprobabilities\t[")?;
			for (index, value) in output_image .chunks_exact(4) .map(decode_prediction_f32) .enumerate()
			{ if index != 0 {
					writer.write_all(b",")?;
				}
				write!(writer, "{value}")?;
...
fn write_knn_prediction_rows( writer: &mut impl Write, artifact: &KnnModelArtifact,
	execution: &CompletedKnnInferenceExecution, ) -> io::Result<()> { let predictions = execution.predictions();
	let outputs = artifact.references().outputs(); if predictions.len() != outputs.len() { return Err(io::Error::new(
			io::ErrorKind::InvalidData, format!(
				"KNN execution returned {} outputs for {} saved targets",
				predictions.len(), outputs.len() ), )); }
	let rows = predictions .first() .map(|prediction| { let [rows, 1] = prediction.contract().shape().extents() else {
				unreachable!("validated KNN prediction shape is [rows, 1]");
			};
			usize::try_from(*rows).expect("validated KNN row count fits usize")
		}) .unwrap_or(0); for row in 0..rows {
		for (output_index, (prediction, output)) in predictions.iter().zip(outputs).enumerate() {
			if prediction.contract().source_vector() != output.schema().source_index() { return Err(io::Error::new(
					io::ErrorKind::InvalidData,
					format!("KNN prediction {output_index} has the wrong saved target identity"),
				)); }
			let begin = row .checked_mul(4)
...
fn decode_prediction_f32(bytes: &[u8]) -> f32 { f32::from_le_bytes(bytes.try_into().expect("one validated prediction f32")) }

fn multiclass_argmax(row_image: &[u8]) -> usize { let mut values = row_image .chunks_exact(4)
		.map(decode_prediction_f32) .enumerate(); let (mut best_class, mut best_value) = values .next()
		.expect("validated multiclass prediction width is nonzero");
	for (class, value) in values { if value.total_cmp(&best_value).is_gt() { best_class = class; best_value = value; } }
	best_class }
