use core::{ fmt, num::{NonZeroU32, NonZeroU64, NonZeroUsize}, }; use std::{ collections::{BTreeMap, BTreeSet, VecDeque},
	io::{self, Write}, path::Path, sync::{ Arc, atomic::{AtomicU64, Ordering}, mpsc::Receiver, }, thread::JoinHandle,
...
use recipe_ingest::{SourceError, SourceLimit, read_source_snapshot}; use recipe_language::CalculationGraph;
use recipe_native_executor::{LocalCandidateFactory, StagedCrossBackend}; use recipe_prepare::{
...
const LIVE_METRIC_CHANNEL_CAPACITY: usize = 256; const LIVE_METRIC_PALETTE: [(u8, u8, u8); 12] = [ (242, 40, 60),
	(39, 125, 255), (0, 174, 107), (255, 194, 0), (215, 46, 130), (135, 90, 251), (255, 122, 0), (91, 192, 235),
	(157, 121, 188), (46, 83, 57), (3, 252, 186), (194, 1, 20), ];

static NEXT_RUN_SEQUENCE: AtomicU64 = AtomicU64::new(1);

/// The public training boundary failed before or during execution.
#[non_exhaustive]
pub enum TrainingError { Declaration(DeclarationError), Data(DataPreparationError), Compile(TrainingCompileError),
	Checkpoint(CheckpointError), Resume(InferencePreparationError), NativeKernelSource(SourceError),
	Native(NativePreparationError), Unsupported { detail: String },
	Runtime { stage: &'static str, detail: String },
}

impl TrainingError { pub(crate) fn unsupported(detail: impl Into<String>) -> Self { Self::Unsupported {
			detail: detail.into(), } }

	pub(crate) fn runtime(stage: &'static str, detail: impl Into<String>) -> Self {
		Self::Runtime { stage, detail: detail.into(), } } }
...
impl std::error::Error for TrainingError {
	fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
		match self { Self::Declaration(error) => Some(error), Self::Data(error) => Some(error),
			Self::Compile(error) => Some(error), Self::Checkpoint(error) => Some(error), Self::Resume(error) => Some(error),
			Self::NativeKernelSource(error) => Some(error), Self::Native(error) => Some(error),
			Self::Unsupported { .. } | Self::Runtime { .. } => None, } } }

impl From<DeclarationError> for TrainingError { fn from(error: DeclarationError) -> Self { Self::Declaration(error) } }

impl From<DataPreparationError> for TrainingError { fn from(error: DataPreparationError) -> Self { Self::Data(error) } }

impl From<TrainingCompileError> for TrainingError {
	fn from(error: TrainingCompileError) -> Self { Self::Compile(error) } }

impl From<CheckpointError> for TrainingError { fn from(error: CheckpointError) -> Self { Self::Checkpoint(error) } }
...
pub enum TrainingModelKind { Dense, Knn, Bayes, }

#[derive(Clone)]
enum TrainingReportPayload { Dense(CompletedTrainingCheckpoint), Knn(KnnModelArtifact), Bayes(BayesModelArtifact), }
...
#[derive(Clone)]
pub struct TrainingReport { payload: TrainingReportPayload, validation_status: ValidationMetricStatus,
...
impl TrainingReport { fn dense( execution: CompletedTrainingExecution, manifest: CheckpointManifest,
		validation_status: ValidationMetricStatus, ) -> TrainingResult<Self> { let gracefully_stopped = execution .journal()
			.logical_events() .iter() .any(|event| matches!(event, LogicalEvent::LoopStopAccepted { .. })); Ok(Self {
			payload: TrainingReportPayload::Dense(CompletedTrainingCheckpoint::new(execution, manifest)?), validation_status,
			gracefully_stopped, }) }

	const fn knn(model: KnnModelArtifact) -> Self { Self { payload: TrainingReportPayload::Knn(model),
			validation_status: ValidationMetricStatus::NotRequested, gracefully_stopped: false, } }

	const fn bayes(model: BayesModelArtifact) -> Self { Self { payload: TrainingReportPayload::Bayes(model),
			validation_status: ValidationMetricStatus::NotRequested, gracefully_stopped: false, } }

	#[must_use]
	pub const fn kind(&self) -> TrainingModelKind { match self.payload {
			TrainingReportPayload::Dense(_) => TrainingModelKind::Dense, TrainingReportPayload::Knn(_) => TrainingModelKind::Knn,
	...
	#[must_use]
	pub const fn run(&self) -> Option<RunId> { match &self.payload {
	...
	#[must_use]
	pub const fn bundle(&self) -> Option<BundleIdentity> { match &self.payload {
			TrainingReportPayload::Dense(checkpoint) => Some(checkpoint.bundle()),
			TrainingReportPayload::Knn(_) | TrainingReportPayload::Bayes(_) => None, } }

	#[must_use]
	pub fn external_outputs(&self) -> &[ExitImage] { match &self.payload {
	...
	#[must_use]
	pub fn metrics(&self) -> &[FinalTrainingMetric] { match &self.payload {
	...
	#[must_use]
	pub const fn journal(&self) -> Option<&RunJournal> { match &self.payload {
	...
	/// dense training run.
	#[must_use]
	pub const fn native_kernels(&self) -> Option<&RealizedNativeKernelSet> { match &self.payload {
	...
	#[must_use]
	pub const fn native_evidence(&self) -> Option<&recipe_native_executor::NativeExecutionEvidence> { match &self.payload {
	...
	#[must_use]
	pub const fn training_evidence(&self) -> Option<&TrainingExecutionEvidence> { match &self.payload {
	...
	fn save_model(&self, path: impl AsRef<Path>) -> TrainingResult<()> { match &self.payload {
	...
	fn save_native_kernel(&self, path: impl AsRef<Path>, format: NativeKernelFormat) -> TrainingResult<()> {
		match &self.payload { TrainingReportPayload::Dense(checkpoint) => checkpoint .save_native_kernel(path, format)
				.map_err(TrainingError::from), TrainingReportPayload::Knn(_) => Err(TrainingError::unsupported(
...
#[derive(Debug)]
struct CompiledTrainingPackage { training: CompiledTraining, checkpoint: CheckpointManifest,
	resume_native: Option<ResumeNativeBundle>, }

#[derive(Clone, Debug)]
struct ResumeNativeBundle { topology: TopologyIdentity, discovery: DiscoveryIdentity, target: TargetIdentity,
	toolchain: ToolchainIdentity, bytes: Arc<[u8]>, }
...
pub fn compile_knn_model(policy: &Train, data: &Data, model: &Model) -> TrainingResult<KnnModelArtifact> {
	...
	if model.objective().is_some() || model.gradient_clip_value().is_some() { return Err(TrainingError::unsupported(
			"KNN feature reduction has no loss objective or gradient policy",
		)); }
	if policy.optimizer_spec().is_some() || policy.learning_rate().is_some() || policy.learning_rate_schedule().is_some()
	...
	if !policy.log_items().is_empty() || !policy.plot_items().is_empty() { return Err(TrainingError::unsupported(
	...
	let resume = if let Some(source) = policy.resume_source() { let path = Path::new(source); let exists = path
			.try_exists()
			.map_err(|error| TrainingError::runtime("inspect KNN resume model", error.to_string()))?;
		if exists { Some(load_knn_model_file(path, KnnModelDecodeLimits::default())?) } else { None } } else { None };
...
pub fn compile_bayes_model(policy: &Train, data: &Data, model: &Model) -> TrainingResult<BayesModelArtifact> {
	policy.validate()?; data.validate()?; model.validate()?; if model.bayes_dependencies().is_empty() {
		return Err(TrainingError::unsupported(
			"Bayesian preparation requires at least one `.bayes(child, parents)` declaration",
		)); }
	if !model.layers().is_empty() || model.weights_source().is_some() { return Err(TrainingError::unsupported(
	...
	if data.normalization().is_some() { return Err(TrainingError::unsupported(
	...
	let resume = if let Some(source) = policy.resume_source() { let path = Path::new(source); let exists = path
		...
		if exists { Some(load_bayes_model_file( path, BayesModelDecodeLimits::default(), )?) } else { None } } else { None };
	let prepared = prepare_data(data)?; let dependencies = model .bayes_dependencies() .iter()
		.map(|dependency| BayesianDependency::new(dependency.child(), dependency.parents())) .collect::<Vec<_>>();
	let current = BayesModelArtifact::from_conditionals(prepare_categorical_bayesian_reference_sets( &prepared,
		&dependencies, )?)?; match resume { Some(saved) => saved.continue_with(current).map_err(Into::into),
		None => Ok(current), } }
...
fn compile_training_package(policy: &Train, data: &Data, model: &Model) -> TrainingResult<CompiledTrainingPackage> {
	...
	let checkpoint = CheckpointManifest::from_compiled(&training)?; Ok(CompiledTrainingPackage { training, checkpoint,
		resume_native, }) }

fn compile_training_graph( policy: &Train, data: &Data, model: &Model,
	...
	let epochs = policy .epoch_bound() .map(|epochs| {
			u64::try_from(epochs)
				...
				}) }) .transpose()? .unwrap_or(TrainingHorizon::Unbounded);
	...
	let mut resume_checkpoint = None; if let Some(source) = policy.resume_source() { let path = Path::new(source);
		...
		if exists { let checkpoint = load_checkpoint_file(path, CheckpointDecodeLimits::default())?;
			apply_checkpoint_resume(&mut training, &checkpoint)?; resume_checkpoint = Some(checkpoint); } }
	Ok((training, resume_checkpoint)) }

fn load_resume_native_bundle( policy: &Train, training: &CompiledTraining, checkpoint: Option<&CheckpointArtifact>,
) -> TrainingResult<Option<ResumeNativeBundle>> { let Some(source) = policy.resume_kernel_source() else {
		return Ok(None); }; let Some(checkpoint) = checkpoint else { return Ok(None); }; let path = Path::new(source);
	let exists = path .try_exists()
	...
	}; let native = checkpoint.native_realization().ok_or_else(|| { CheckpointError::IncompatibleResume {
			detail: "the semantic model predates native realization metadata, so the supplied kernel cannot be authenticated"
				.to_owned(), } })?; let current_program = compiled_training_program_digest(training)?;
	if native.program() != current_program { return Err(CheckpointError::IncompatibleResume {
			detail: "the supplied native kernel was realized for a different static training program".to_owned(),
		} .into()); }
	let matching = native .kernels() .iter() .filter(|kernel| kernel.format() == format) .collect::<Vec<_>>();
	let kernel = match matching.as_slice() { [kernel] => *kernel, [] => { return Err(CheckpointError::IncompatibleResume {
				detail: format!(
					"the semantic model has no authenticated .{} realization",
					format.extension() ), } .into()); }
		kernels => { return Err(CheckpointError::IncompatibleResume { detail: format!(
					"the semantic model has {} authenticated .{} realizations; one resume path is ambiguous",
					kernels.len(), format.extension() ), } .into()); } };
	...
	let snapshot = read_source_snapshot( path, SourceLimit::new(source_limit).map_err(TrainingError::NativeKernelSource)?,
	) .map_err(TrainingError::NativeKernelSource)?; if snapshot.digest() != kernel.digest() {
		return Err(CheckpointError::IncompatibleResume { detail:
				"the supplied native kernel bytes do not match the digest authenticated by the semantic model"
					.to_owned(), } .into()); }
	Ok(Some(ResumeNativeBundle { topology: native.topology(), discovery: native.discovery(),
		target: kernel.target().clone(), toolchain: kernel.toolchain().clone(), bytes: Arc::from(snapshot.into_bytes()), })) }
...
impl Train {
	...
	#[doc(hidden)]
	pub fn __recipe_run_with(&self, model: &Model, data: &Data) -> TrainingResult<TrainingReport> { self.try_run_with(data, model) }
	...
	fn try_run_with(&self, data: &Data, model: &Model) -> TrainingResult<TrainingReport> {
		...
		{ let report = TrainingReport::knn(compile_knn_model(self, data, model)?);
			if let Some(destination) = self.save_model_destination() { report.save_model(destination)?; }
			return Ok(report); }
		let package = compile_training_package(self, data, model)?; let interrupt = crate::signal::SigintGuard::install()
			.map_err(|error| TrainingError::runtime("install graceful SIGINT handler", error.to_string()))?;
		let execution = execute_current_training( self, &package.training, package.resume_native.as_ref(),
			interrupt.request_flag(), )?; let report = TrainingReport::dense( execution, package.checkpoint,
			package.training.outputs().validation_status, )?; if let Some(destination) = self.save_model_destination() {
			report.save_model(destination)?; }
		if let Some(destination) = self.save_kernel_destination() { let format = match Path::new(destination) .extension()
...
fn execute_current_training( policy: &Train, training: &CompiledTraining, resume_native: Option<&ResumeNativeBundle>,
	stop_requested: &std::sync::atomic::AtomicBool, ) -> TrainingResult<CompletedTrainingExecution> {
	report_unavailable_validation(training)?; let presentations = live_metric_presentations(policy, training);
	if presentations.is_empty() {
		let execution = execute_current_training_native(training, resume_native, None, stop_requested)?;
		report_static_training_metrics(policy, &execution)?; return Ok(execution); }
	let capacity =
		NonZeroUsize::new(LIVE_METRIC_CHANNEL_CAPACITY).expect("the fixed live metric channel capacity is nonzero");
	let (mut observer, receiver) = bounded_training_metric_channel( capacity, presentations.metrics.keys().copied(),
		NonZeroU64::MIN, ); let presenter = spawn_live_metric_presenter(receiver, presentations, training.bounds())?;
	let execution = execute_current_training_native(training, resume_native, Some(&mut observer), stop_requested);
	drop(observer); let presentation = presenter .join()
		.map_err(|panic| TrainingError::runtime("join live metric presenter", thread_panic_detail(panic)))?;
	match (execution, presentation) { (Ok(execution), Ok(())) => { report_static_training_metrics(policy, &execution)?;
			Ok(execution) }
...
fn report_unavailable_validation(training: &CompiledTraining) -> TrainingResult<()> {
	...
	else { return Ok(()); }; let stdout = io::stdout(); write_live_metric_row(&mut stdout.lock(), &line)
...
fn report_static_training_metrics(policy: &Train, execution: &CompletedTrainingExecution) -> TrainingResult<()> {
	let log_device = policy .log_items() .iter() .any(|item| item.metric() == Metric::Device); let plot_device = policy
		.plot_items() .iter() .any(|item| item.metric() == Metric::Device); if !log_device && !plot_device { return Ok(()); }
	let devices = execution .native_kernels() .kernels() .iter() .map(|kernel| { let target = kernel.target();
			format!("{}:{}:{}", target.backend, target.architecture, target.abi)
		}) .collect::<BTreeSet<_>>() .into_iter() .collect::<Vec<_>>()
		.join(",");
	let devices = if devices.is_empty() {
		"unavailable".to_owned()
	} else { devices }; let stdout = io::stdout(); let mut output = stdout.lock(); if log_device {
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) struct NativeRuntimeTuning { pub worker_threads: usize, pub staging_bytes_per_worker: usize,
	pub watchdog: Watchdog, }

fn select_host_worker_threads( available_parallelism: usize, measured_host_lanes: usize, realized_opportunities: usize,
) -> usize { available_parallelism .min(measured_host_lanes) .min(realized_opportunities) .max(1) }

fn select_host_staging_bytes( maximum_value_bytes: u64, measured_staging_scale: u64, local_ram_bytes: u64,
	worker_threads: u64, ) -> u64 { maximum_value_bytes .min(measured_staging_scale)
		.min(local_ram_bytes / worker_threads.max(1)) .max(1) }

pub(crate) fn derive_native_runtime_tuning( graph: &CalculationGraph, profile: &MeasuredProfile, machine: MachineId,
) -> TrainingResult<NativeRuntimeTuning> { let local_devices = profile .discovery .devices .iter()
		.filter(|discovered| { profile .topology .device(discovered.device) .is_some_and(|device| device.machine == machine)
		}) .collect::<Vec<_>>(); if local_devices.is_empty() { return Err(TrainingError::runtime(
			"derive native training tuning",
			"measured local machine has no discovered devices",
		)); }

	let realized_opportunities = graph.tensors.len().max(1); let measured_host_lanes = local_devices .iter()
		.filter(|discovered| { profile .topology .device(discovered.device)
				.is_some_and(|device| matches!(device.kind, DeviceKind::Ram | DeviceKind::Disk)) })
		.try_fold(0_u64, |total, discovered| { let lanes = u64::from( discovered .transfer .maximum_inflight_transfers .value
					.get() .min(discovered.maximum_submission_queues), ); total.checked_add(lanes) }) .ok_or_else(|| {
		...
		.max(1); let available_parallelism = std::thread::available_parallelism() .map(NonZeroUsize::get)
		.map_err(|error| TrainingError::runtime("query host parallelism", error.to_string()))?;
	let worker_threads = select_host_worker_threads( available_parallelism, measured_host_lanes, realized_opportunities, );

	let maximum_value_bytes = graph .tensors .iter() .map(|tensor| tensor.storage_bytes) .max_by_key(|bytes| bytes.get())
		.unwrap_or(ByteCount::new(1)); let external_input_bytes = graph .tensors .iter()
		.filter(|tensor| tensor.external_input) .try_fold(0_u64, |total, tensor| {
			total.checked_add(tensor.storage_bytes.get()) }) .ok_or_else(|| { TrainingError::runtime(
				"derive native training tuning",
				"external input image size overflowed",
			) })?; let external_output_bytes = graph .tensors .iter() .filter(|tensor| tensor.external_output)
		.try_fold(0_u64, |total, tensor| { total.checked_add(tensor.storage_bytes.get()) }) .ok_or_else(|| {
			TrainingError::runtime(
				"derive native training tuning",
				"external output image size overflowed",
			) })?; let maximum_transfer_bytes = ByteCount::new( maximum_value_bytes .get() .max(external_input_bytes)
			.max(external_output_bytes), ); let has_local_disk = profile .topology .devices .iter()
		.any(|device| device.machine == machine && device.kind == DeviceKind::Disk);
	let measured_staging_scale = match has_local_disk { true => profile.benchmarks.storage.buffer_bytes,
		false => profile.benchmarks.ram.buffer_bytes, }; let local_ram_bytes = local_devices .iter() .filter(|discovered| {
			profile .topology .device(discovered.device) .is_some_and(|device| device.kind == DeviceKind::Ram) })
		.try_fold(0_u64, |total, discovered| { total.checked_add(discovered.total_capacity.value.get()) }) .ok_or_else(|| {
			TrainingError::runtime(
				"derive native training tuning",
				"measured RAM capacity overflowed",
			) })?; if local_ram_bytes == 0 { return Err(TrainingError::runtime(
			"derive native training tuning",
			"measured local machine has no RAM capacity for host staging",
		)); }
	let worker_count = u64::try_from(worker_threads)
		.map_err(|error| TrainingError::runtime("derive native training tuning", error.to_string()))?;
	let staging_bytes = select_host_staging_bytes( maximum_value_bytes.get(), measured_staging_scale.get(),
		local_ram_bytes, worker_count, ); let staging_bytes_per_worker = usize::try_from(staging_bytes)
		.map_err(|error| TrainingError::runtime("derive native training tuning", error.to_string()))?;

	let tensor_index = graph .tensors .iter() .map(|tensor| (tensor.id, tensor)) .collect::<BTreeMap<_, _>>();
	let maximum_work = graph .nodes .iter() .map(|node| node.kernel.work(&tensor_index)) .collect::<Result<Vec<_>, _>>()
		.map_err(|error| TrainingError::runtime("derive native training tuning", error.to_string()))?
		.into_iter() .max_by_key(|work| work.get()) .unwrap_or(FlopCount::ZERO); let slowest_calculation_rate = local_devices
		.iter() .filter_map(|discovered| { discovered .calculation .as_ref() .map(|calculation| calculation.rate.value) })
		.min_by_key(|rate| rate.get()) .ok_or_else(|| { TrainingError::runtime(
				"derive native training tuning",
		...
		.map_err(|error| TrainingError::runtime("derive native training tuning", error.to_string()))?;
	let slowest_device_transfer_rate = local_devices .iter() .map(|discovered| discovered.transfer.rate.value)
		.min_by_key(|rate| rate.get()); let slowest_local_link_rate = profile .discovery .links .iter()
		.filter_map(|discovered| { let link = profile.topology.link(discovered.link)?;
			let source = profile.topology.device(link.from)?; let destination = profile.topology.device(link.to)?;
			(source.machine == machine && destination.machine == machine).then_some(discovered.bandwidth.value) })
		.min_by_key(|rate| rate.get()); let slowest_transfer_rate = slowest_device_transfer_rate .into_iter()
		.chain(slowest_local_link_rate) .min_by_key(|rate| rate.get()) .ok_or_else(|| { TrainingError::runtime(
				"derive native training tuning",
		...
		.map_err(|error| TrainingError::runtime("derive native training tuning", error.to_string()))?;
	let expected_operation = Duration::from_nanos( calculation_duration .get() .max(transfer_duration.get()) .max(1), );
	let measured_concurrency = local_devices .iter() .try_fold(0_u64, |total, discovered| { let calculation = discovered
				.calculation .as_ref() .map_or(0_u64, |calculation| { u64::from(calculation.maximum_concurrent_tasks) });
			let transfer = u64::from(discovered.transfer.maximum_inflight_transfers.value.get());
			total.checked_add(calculation)?.checked_add(transfer) }) .ok_or_else(|| { TrainingError::runtime(
				"derive native training tuning",
				"device concurrency overflowed",
			) })? .max(1) .min(u64::from(u32::MAX)); let safety_multiplier = NonZeroU32::new(measured_concurrency as u32)
	...
	Ok(NativeRuntimeTuning { worker_threads, staging_bytes_per_worker, watchdog, }) }

fn execute_current_training_native( training: &CompiledTraining, resume_native: Option<&ResumeNativeBundle>,
	mut observer: Option<&mut TrainingMetricObserver>, stop_requested: &std::sync::atomic::AtomicBool,
) -> TrainingResult<CompletedTrainingExecution> { let run = next_run_id();
	let result = with_current_native_preparation(|profile, _config, scope| { if let Some(native) = resume_native {
			...
			if !matches_current_target { return Err(NativePreparationError::IdentityMismatch {
		...
		let limits = TrainingExecutionLimits::new(tuning.watchdog); let host = host
			.backend_config(run, tuning.worker_threads, tuning.staging_bytes_per_worker)
			.map_err(|error| NativePreparationError::LocalConfiguration(error.to_string()))?;
		let (cuda, hsa) = bindings.into_parts(); let bridge = StagedCrossBackend::new(cuda.clone(), hsa.clone());
		let factory = LocalCandidateFactory::production(Some(host), cuda, hsa, bridge);
		let driver = NativeExecutorDriver::new(factory); let mut compiler = targets .deferred_compiler()
			.map_err(NativePreparationError::TargetSpecification)?; if let Some(native) = resume_native { compiler = compiler
		...
		let mut preparer = Preparer::new(provider, realizer); let execution = prepare_and_execute_local_training_controlled(
			training, profile, &mut preparer, run, limits, observer.as_deref_mut(),
			TrainingExecutionControl::graceful_stop(stop_requested), ); Ok(execution) })?;
	result.map_err(|error| TrainingError::runtime("execute native training", error.to_string()))
}

#[derive(Clone, Debug, PartialEq, Eq)]
struct LiveMetricPresentation { label: String, width: usize, log_cadence: Option<NonZeroU64>, plot: bool, }

#[derive(Clone, Debug, Default, PartialEq, Eq)]
struct LiveHostPresentation { epoch_log_cadence: Option<NonZeroU64>, epoch_plot: bool,
	time_log_cadence: Option<NonZeroU64>, time_plot: bool, }

impl LiveHostPresentation {
	fn requested(&self) -> bool { self.epoch_log_cadence.is_some() || self.epoch_plot || self.time_log_cadence.is_some() || self.time_plot }
}

#[derive(Clone, Debug, Default, PartialEq, Eq)]
struct LiveMetricPlan { metrics: BTreeMap<MetricId, LiveMetricPresentation>, host: LiveHostPresentation, }

impl LiveMetricPlan { fn is_empty(&self) -> bool { self.metrics.is_empty() } }

fn live_metric_presentations(policy: &Train, training: &CompiledTraining) -> LiveMetricPlan {
	let mut plan = LiveMetricPlan { host: LiveHostPresentation {
			epoch_log_cadence: policy.metric_log_interval(Metric::Epoch), epoch_plot: policy .plot_items() .iter()
				.any(|item| item.metric() == Metric::Epoch), time_log_cadence: policy.metric_log_interval(Metric::Time),
			time_plot: policy .plot_items() .iter() .any(|item| item.metric() == Metric::Time), }, ..LiveMetricPlan::default() };
	for binding in &training.outputs().metric_bindings {
		let Some(metric) = log_selects_binding_to_metric(binding.kind) else { continue; }; let logged = policy .log_items()
			.iter() .any(|item| log_selects(item.metric(), binding.kind)); let plotted = policy .plot_items() .iter()
			.any(|item| log_selects(item.metric(), binding.kind)); if !logged && !plotted { continue; }
		plan.metrics.insert( binding.metric, LiveMetricPresentation { label: metric_label(binding.kind),
				width: metric_width(binding.kind), log_cadence: logged.then(|| policy.metric_log_interval(metric)).flatten(),
				plot: plotted, }, ); }
	if plan.host.requested() && let Some(trigger) = training .outputs() .metric_bindings .iter()
			.find(|binding| binding.kind == TrainingMetricKind::TrainingLoss)
	{ plan.metrics .entry(trigger.metric) .or_insert_with(|| LiveMetricPresentation { label: metric_label(trigger.kind),
				width: metric_width(trigger.kind), log_cadence: None, plot: false, }); }
	plan }

fn log_selects_binding_to_metric(binding: TrainingMetricKind) -> Option<Metric> { match binding {
		...
		TrainingMetricKind::Accuracy => Some(Metric::Accuracy), TrainingMetricKind::R2 => Some(Metric::R2),
		TrainingMetricKind::LearningRate => Some(Metric::LearningRate), TrainingMetricKind::AuRoc => Some(Metric::AuRoc),
		TrainingMetricKind::AuPrc => Some(Metric::AuPrc), TrainingMetricKind::BrierScore => Some(Metric::Brier),
		TrainingMetricKind::ExpectedCalibrationError => Some(Metric::CalibrationError),
		TrainingMetricKind::RecallAt { .. } => Some(Metric::Loss), } }

fn log_selects(requested: Metric, available: TrainingMetricKind) -> bool { match requested { Metric::Loss => { matches!(
				available, TrainingMetricKind::TrainingLoss | TrainingMetricKind::ValidationMeanBce
					| TrainingMetricKind::ValidationMeanCrossEntropy ) }
		Metric::Accuracy => available == TrainingMetricKind::Accuracy, Metric::R2 => available == TrainingMetricKind::R2,
		Metric::AuRoc => available == TrainingMetricKind::AuRoc, Metric::AuPrc => available == TrainingMetricKind::AuPrc,
...
fn metric_label(metric: TrainingMetricKind) -> String { match metric {
...
const fn metric_width(metric: TrainingMetricKind) -> usize { match metric { TrainingMetricKind::TrainingLoss
		| TrainingMetricKind::ValidationMeanBce
...
fn spawn_live_metric_presenter( receiver: Receiver<TrainingMetricSample>, presentations: LiveMetricPlan,
	bounds: TrainingBounds, ) -> TrainingResult<JoinHandle<TrainingResult<()>>> { std::thread::Builder::new()
		.name("recipe-live-metrics".to_owned())
		.spawn(move || { let stdout = io::stdout(); let mut output = stdout.lock();
...
fn present_live_metrics( receiver: Receiver<TrainingMetricSample>, presentations: LiveMetricPlan,
	bounds: TrainingBounds, output: &mut impl Write, ) -> TrainingResult<()> {
	let mut rows = LiveMetricRows::new(presentations, bounds.epochs); while let Ok(sample) = receiver.recv() {
		if let Some(row) = rows.push( sample.zero_based_iteration.index(), sample.epoch, sample.metric, sample.value, ) {
			write_live_metric_row(output, &row)
				.map_err(|error| TrainingError::runtime("write live metrics", error.to_string()))?;
		} }
	if let Some(row) = rows.finish() { write_live_metric_row(output, &row)
			.map_err(|error| TrainingError::runtime("write live metrics", error.to_string()))?;
	}
	for plot in rows.plot_lines() { write_live_metric_row(output, &plot)
...
#[derive(Debug)]
struct LiveMetricRows { plan: LiveMetricPlan, epochs: TrainingHorizon, started: Instant,
	pending_sample: Option<(u64, NonZeroU64)>, pending_values: BTreeMap<MetricId, MetricValue>,
	plots: BTreeMap<String, BoundedPlotSeries>, }

impl LiveMetricRows { fn new(plan: LiveMetricPlan, epochs: impl Into<TrainingHorizon>) -> Self { Self { plan,
			epochs: epochs.into(), started: Instant::now(), pending_sample: None, pending_values: BTreeMap::new(),
			plots: BTreeMap::new(), } }

	fn push(&mut self, iteration: u64, epoch: NonZeroU64, metric: MetricId, value: MetricValue) -> Option<String> {
		let completed = if self .pending_sample .is_some_and(|(pending, _)| pending != iteration)
		{ let completed = self .pending_sample .and_then(|(_, epoch)| self.render(epoch, false)); self.pending_sample = None;
			self.pending_values.clear(); completed } else { None }; self.pending_sample = Some((iteration, epoch));
		if self.plan.metrics.contains_key(&metric) { self.pending_values.insert(metric, value); }
		completed }

	fn finish(&mut self) -> Option<String> { let (_, epoch) = self.pending_sample.take()?;
		let row = self.render(epoch, true); self.pending_values.clear(); row }
	...
	fn render(&mut self, epoch: NonZeroU64, final_flush: bool) -> Option<String> {
		...
		let elapsed = self.started.elapsed().as_secs_f64(); if self.plan.host.epoch_plot {
			self.record_plot("epoch", epoch.get() as f64);
		}
		if self.plan.host.time_plot {
			self.record_plot("time", elapsed);
		}
		let plotted = self .plan .metrics .iter() .filter_map(|(metric, presentation)| { presentation .plot .then(|| {
						self.pending_values .get(metric) .copied() .map(|value| (presentation.label.clone(), value)) }) .flatten() })
			.collect::<Vec<_>>(); for (label, value) in plotted { self.record_plot(&label, metric_value_f64(value)); }

		let mut fields = Vec::with_capacity(self.pending_values.len().saturating_add(2)); let epoch_due = self .plan .host
			.epoch_log_cadence .is_some_and(|cadence| epoch.get() % cadence.get() == 0 || final_epoch); let time_due = self .plan
			.host .time_log_cadence .is_some_and(|cadence| epoch.get() % cadence.get() == 0 || final_epoch); if time_due {
			fields.push(live_metric_field("time", 9, &format!("{elapsed:.3}s"), 1));
		}
		for (metric, presentation) in &self.plan.metrics { let Some(value) = self.pending_values.get(metric) else { continue;
			}; let log_due = presentation .log_cadence .is_some_and(|cadence| epoch.get() % cadence.get() == 0 || final_epoch);
			if !log_due { continue; }
			let rendered = render_live_metric_value(*value, presentation.width); fields.push(live_metric_field(
				&presentation.label, presentation.width, &rendered, fields.len().saturating_add(1), )); }
		if fields.is_empty() && !epoch_due { return None; }
	...
	fn plot_lines(&self) -> Vec<String> { self.plots .iter() .map(|(label, series)| series.render(label)) .collect() } }

const LIVE_PLOT_WIDTH: usize = 64;

#[derive(Debug, Default)]
struct BoundedPlotSeries { values: VecDeque<f64>, total: u64, }

impl BoundedPlotSeries { fn push(&mut self, value: f64) { self.total = self.total.saturating_add(1);
		if self.values.len() == LIVE_PLOT_WIDTH { self.values.pop_front(); }
		self.values.push_back(value); }

	fn render(&self, label: &str) -> String { let finite = self .values .iter() .copied()
			.filter(|value| value.is_finite()) .collect::<Vec<_>>(); let (minimum, maximum) = finite .iter() .copied()
			.fold(None, |range, value| match range {
				Some((minimum, maximum)) => Some((f64::min(minimum, value), f64::max(maximum, value))),
				None => Some((value, value)), }) .unwrap_or((f64::NAN, f64::NAN));
		let levels = ['▁', '▂', '▃', '▄', '▅', '▆', '▇', '█'];
		let plot = self .values .iter() .map(|value| { if !value.is_finite() {
					'·'
				} else if minimum == maximum { levels[3] } else {
					let scaled = ((*value - minimum) / (maximum - minimum)) * (levels.len() - 1) as f64;
					levels[scaled.round() as usize] } }) .collect::<String>(); if minimum.is_finite() { format!(
				"plot {label} {plot}  range {minimum:.4}..{maximum:.4}  samples {}",
				self.total ) } else {
			format!("plot {label} {plot}  range N/A  samples {}", self.total)
		} } }

fn metric_value_f64(value: MetricValue) -> f64 { match value { MetricValue::F32(value) => f64::from(value),
		MetricValue::I32(value) => f64::from(value), } }

fn render_live_metric_value(value: MetricValue, width: usize) -> String { match value {
...
pub(crate) fn next_run_id() -> RunId { let sequence = NEXT_RUN_SEQUENCE.fetch_add(1, Ordering::Relaxed);
	let epoch = SystemTime::now() .duration_since(UNIX_EPOCH) .map_or(0, |duration| duration.as_nanos() as u64);
...
fn require_supported_policy(policy: &Train) -> TrainingResult<()> {
	if policy.resume_kernel_source().is_some() && policy.resume_source().is_none() { return Err(TrainingError::unsupported(
			"a native resume kernel requires a semantic .ogdl model",
		)); }
	if policy.optimizer_spec() != Some(Optimizer::AdamW) { return Err(TrainingError::unsupported(
			"dense training requires the Recipe-owned AdamW optimizer on the training declaration",
		)); }
	if policy.learning_rate_schedule().is_none() { return Err(TrainingError::unsupported(
