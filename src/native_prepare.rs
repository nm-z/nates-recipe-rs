...
use recipe_prepare::{CudaArtifactPolicy, DeferredArtifactCompiler, NativePrepareError, RuntimeArtifactPolicy, TargetBuildSpec};
use recipe_probe::{ExplicitPathProfileCache, GpuDescriptor, GpuDiscovery as _, HostInventory, MeasuredProfile, ProbeError, ProfileCache as _, ResolvedLocalInventory};
	...
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			Self::ProfileNotFound { path } => {
				return write!(f, "measured profile does not exist at {}", path.display());
			}
			Self::Probe(error) => return write!(f, "native probe/profile validation failed: {error}"),
			Self::Lowering(error) => return write!(f, "native artifact builder is invalid: {error}"),
			Self::TargetSpecification(error) => {
				return write!(f, "native target specification is invalid: {error}");
			}
			Self::LocalConfiguration(detail) => {
				return write!(f, "local native configuration failed: {detail}");
			}
			Self::IdentityMismatch { detail } => return write!(f, "native identity mismatch: {detail}"),
		}
	}
}
...
impl std::error::Error for NativePreparationError {
	fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
		match self {
			Self::Probe(error) => return Some(error),
			Self::Lowering(error) => return Some(error),
			Self::TargetSpecification(error) => return Some(error),
			Self::ProfileNotFound { .. } | Self::LocalConfiguration(_) | Self::IdentityMismatch { .. } => {
				return None;
			}
		}
	}
...
impl NativeDeviceBuildTarget {
	#[must_use]
	pub const fn device(&self) -> DeviceId { return self.device; }

	#[must_use]
	pub const fn origin(&self) -> &Label { return &self.origin; }
...
impl NativeHostPlan {
	#[must_use]
	pub const fn machine(&self) -> MachineId { return self.machine; }
	...
	/// # Errors
	///
	/// Returns a host configuration error when a measured storage binding cannot form a valid run-scoped path.
	pub fn backend_config(&self, run: RunId, worker_threads: usize, staging_bytes_per_worker: usize) -> recipe_host::Result<HostBackendConfig> {
		let mut bindings = self
			.ram
			.iter()
			.copied()
			.map(|device| return HostDeviceBinding::Ram { device })
			.collect::<Vec<_>>();
		...
		return HostBackendConfig::new(worker_threads, staging_bytes_per_worker, bindings);
	}
}
...
impl NativeTargetPlan {
	...
	#[must_use]
	pub fn devices(&self) -> &[NativeDeviceBuildTarget] { return &self.devices; }
	...
	#[must_use]
	pub fn target_build_specs(&self) -> &[TargetBuildSpec] { return &self.targets; }
	...
	pub fn deferred_compiler(&self) -> Result<DeferredArtifactCompiler, NativePrepareError<Infallible>> { return DeferredArtifactCompiler::new(self.targets.clone()); }
}
...
impl<'cuda, 'hsa> NativePreparationScope<'cuda, 'hsa> {
	#[must_use]
	pub const fn bindings(&self) -> &NativeExecutionBindings<'cuda, 'hsa> { return &self.bindings; }

	#[must_use]
	pub const fn targets(&self) -> &NativeTargetPlan { return &self.targets; }

	#[must_use]
	pub const fn host(&self) -> &NativeHostPlan { return &self.host; }
	...
	pub fn into_parts(
		...
		return (self.bindings, self.host, self.targets);
	}

	#[must_use]
	pub fn into_targets(self) -> NativeTargetPlan { return self.targets; }
}

fn with_native_preparation_from_probe<T>(probe: &NativeGpuProbe, config: &NativeProbeConfig, profile: &MeasuredProfile, host: &HostInventory, operation: impl for<'cuda, 'hsa> FnOnce(NativePreparationScope<'cuda, 'hsa>) -> NativePreparationResult<T>) -> NativePreparationResult<T> {
	let inventory = probe
		.discover_all()
	...
	.map_err(NativePreparationError::Probe)?;
	return callback_result.ok_or_else(|| return identity_error("native preparation callback was not invoked"))?;
}
...
/// # Errors
///
/// Returns the first local-input, cache, profile-identity, native-probe, or callback preparation failure.
pub fn with_current_native_preparation<T>(operation: impl for<'cuda, 'hsa> FnOnce(&MeasuredProfile, &NativeProbeConfig, NativePreparationScope<'cuda, 'hsa>) -> NativePreparationResult<T>) -> NativePreparationResult<T> {
	...
	let profile = cache
		...
		.map_err(NativePreparationError::Probe)?
		.ok_or_else(|| {
			return NativePreparationError::ProfileNotFound {
				path: current.profile_path.clone(),
			};
		})?;
	return CURRENT_NATIVE_PROBE.with(|cached| {
		let mut cached = cached.borrow_mut();
		if let Some((config, _probe)) = cached.as_ref()
			&& config != &current.config
		{
			return Err(NativePreparationError::IdentityMismatch {
				detail: "current native configuration changed after this thread opened its GPU runtime".to_owned(),
			});
		}
		if cached.is_none() {
		...
		let probe = &cached
			...
			.1;
		return with_native_preparation_from_probe(probe, &current.config, &profile, &current.host, |scope| {
			return operation(&profile, &current.config, scope);
		});
	});
}
...
fn owned_gpu_inventory(resolved: &ResolvedLocalInventory<'_>) -> Vec<(DeviceId, GpuDescriptor)> {
	let mut devices = resolved
		.gpu()
		.iter()
		.map(|resolved| return (resolved.device(), resolved.descriptor().clone()))
		.collect::<Vec<_>>();
	devices.sort_by_key(|(device, _)| return *device);
	return devices;
}

fn require_complete_local_gpu_scope(profile: &MeasuredProfile, machine: MachineId, devices: &[(DeviceId, GpuDescriptor)]) -> NativePreparationResult<()> {
	let local = devices
		.iter()
		.map(|(device, _)| return *device)
		.collect::<BTreeSet<_>>();
	let outside = profile
		.discovery
		.devices
		.iter()
		.filter(|discovered| return discovered.calculation.is_some())
		.filter_map(|discovered| {
			return profile
				.topology
				.device(discovered.device)
				.filter(|device| return device.machine != machine || !local.contains(&device.id))
...
fn owned_host_plan(resolved: &ResolvedLocalInventory<'_>) -> NativeHostPlan {
	let mut ram = resolved
		.ram()
		.iter()
		.map(|device| return device.device())
		.collect::<Vec<_>>();
	ram.sort();
	let mut storage = resolved
		.storage()
		.iter()
		.map(|device| return (device.device(), device.domain().benchmark_root.clone()))
		.collect::<Vec<_>>();
	storage.sort_by_key(|(device, _)| return *device);
	return NativeHostPlan {
...
fn build_scope<'cuda, 'hsa>(config: &NativeProbeConfig, machine: MachineId, host: NativeHostPlan, devices: &[(DeviceId, GpuDescriptor)], bindings: NativeExecutionBindings<'cuda, 'hsa>) -> NativePreparationResult<NativePreparationScope<'cuda, 'hsa>> {
	...
	let actual = cuda
		...
		.copied()
		.collect::<BTreeSet<_>>();
	if actual != expected || cuda.keys().any(|device| return hsa.contains_key(device)) {
		return Err(identity_error(format!(
	...
	for (device, descriptor) in devices {
		let spec = match descriptor.target.backend.as_str() {
			CUDA_BACKEND => {
				let binding = cuda.get(device).ok_or_else(|| {
					return identity_error(format!("CUDA device {device} has no exact binding"));
				})?;
				cuda_spec(config, descriptor, binding.deployment(), builder.clone())?
			}
			HSA_BACKEND => {
				let binding = hsa.get(device).ok_or_else(|| {
					return identity_error(format!("HSA device {device} has no exact binding"));
				})?;
	...
	return Ok(NativePreparationScope {
		bindings,
		host,
		targets: NativeTargetPlan {
			machine,
...
fn unique_cuda_bindings<'scope, 'cuda>(bindings: &'scope NativeExecutionBindings<'cuda, '_>) -> NativePreparationResult<BTreeMap<DeviceId, &'scope recipe_native_executor::CudaBinding<'cuda>>> {
...
fn unique_hsa_bindings<'scope, 'hsa>(bindings: &'scope NativeExecutionBindings<'_, 'hsa>) -> NativePreparationResult<BTreeMap<DeviceId, &'scope recipe_native_executor::HsaBinding<'hsa>>> {
	let mut devices = BTreeMap::new();
...
fn cuda_spec(config: &NativeProbeConfig, descriptor: &GpuDescriptor, deployment: &DeploymentIdentity, builder: ArtifactBuilder) -> NativePreparationResult<TargetBuildSpec> {
	let sm_major = u8::try_from(deployment.target.major).map_err(|error| return identity_error(format!("CUDA SM major does not fit u8: {error}")))?;
	let sm_minor = u8::try_from(deployment.target.minor).map_err(|error| return identity_error(format!("CUDA SM minor does not fit u8: {error}")))?;
	...
	if let Some(missing) = required_driver_symbols
		.iter()
		.find(|symbol| return !deployment.driver_capabilities.supports(**symbol))
	{
		return Err(identity_error(format!(
...
fn hsa_spec(config: &NativeProbeConfig, descriptor: &GpuDescriptor, binding: &recipe_native_executor::HsaBinding<'_>, builder: ArtifactBuilder) -> NativePreparationResult<TargetBuildSpec> {
	if descriptor.target.architecture.as_str() != binding.target_id() || binding.code_object_version() != config.hsa.code_object_version {
		return Err(identity_error(format!(
...
fn cuda_toolchain_identity(config: &NativeProbeConfig) -> NativePreparationResult<CudaToolchainIdentity> {
	let assembler = config
		...
		.ptx_assembler
		.as_ref()
		.ok_or_else(|| return identity_error("measured NVIDIA target has no pinned PTX assembler"))?;
	return Ok(CudaToolchainIdentity {
...
fn require_equivalent_specs(existing: &TargetBuildSpec, candidate: &TargetBuildSpec) -> NativePreparationResult<()> {
	if existing.target_identity == candidate.target_identity && existing.target == candidate.target && existing.toolchain_identity == candidate.toolchain_identity && existing.scratch_parent == candidate.scratch_parent && existing.runtime_policy == candidate.runtime_policy {
		return Ok(());
	}
	return Err(identity_error(format!(
...
fn identity_error(detail: impl Into<String>) -> NativePreparationError {
	return NativePreparationError::IdentityMismatch {
		detail: detail.into(),
	};
}
