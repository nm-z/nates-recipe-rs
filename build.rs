...
fn constant(value: f64) -> String { format!("0x{:016X}", value.to_bits()) }
fn math_literal(arithmetic: &str, value: f64) -> String { if arithmetic == "double" { constant(value) } else { constant(f64::from(value as f32)) } }
...
fn main() -> BuildResult<()> {
	...
	for (key, environment) in [
		...
		("topology-calibration-rows", "RECIPE_TOPOLOGY_CALIBRATION_ROWS"),
	] {
	...
	println!("cargo:rustc-env=RECIPE_MULTI_DEVICE={}", match placement { "false" | "true" => placement, "\"auto\"" => "auto", value => return Err(io::Error::other(format!("multi-device must be false, true, or \"auto\", not {value}")).into()) });
