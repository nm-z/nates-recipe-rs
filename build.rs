...
fn parallel_ir(ir: String, width: &str) -> String { let ir = ir
	...
	ir.replacen('\n', &format!("\n{}\n", PARALLEL.replace("declare i32 @recipe.workgroup.size.x()", width)), 1)
		.replace("recipe.local.id.x", "llvm.amdgcn.workitem.id.x") .replace("recipe.group.id.x", "llvm.amdgcn.workgroup.id.x")
		.replace("recipe.local.barrier", "llvm.amdgcn.s.barrier") }
fn setting<'a>(manifest: &'a str, key: &str) -> BuildResult<&'a str> { let prefix = format!("{key} = "); manifest
		.lines() .find_map(|line| line.trim().strip_prefix(&prefix))
...
fn render(command: &mut Command, role: &str, path: &Path) -> BuildResult<()> { let output = command.output()?;
	if !output.status.success() { let detail = String::from_utf8_lossy(&output.stderr);
		return Err(io::Error::other(format!("{role} failed: {}", detail.trim())).into()); }
	fs::write(path, output.stdout)?; Ok(()) } fn compile_amd(manifest: &str, out: &PathBuf) -> BuildResult<()> {
...
fn compile_nvidia(manifest: &str, out: &PathBuf) -> BuildResult<()> {
	...
	let ir = parallel_ir(fs::read_to_string("amd-nv.ll")?, "declare i32 @recipe.workgroup.size.x()")
		...
		.replace("llvm.amdgcn.s.barrier", "llvm.nvvm.barrier0") .replace("__ocml_exp_f64", "__nv_exp")
		.replace("__ocml_tanh_f64", "__nv_tanh") .replace("__ocml_cos_f64", "__nv_cos") .replace("__ocml_sin_f64", "__nv_sin")
		.replace("__ocml_log_f64", "__nv_log") .replace("define protected amdgpu_kernel", "define ptx_kernel") .replace(
			"attributes #0 = { nounwind \"amdgpu-flat-work-group-size\"=\"1,1024\" }", "attributes #0 = { nounwind }", );
	fs::write(&source, ir)?;
	let mut command = Command::new(text(manifest, "nvidia-compiler")?); command.args([ "-target", "nvptx64-nvidia-cuda",
		&architecture, "-Xclang", "-target-feature", "-Xclang", &ptx, "-O2", "-S", "-x", "ir",
		source.to_str().ok_or_else(|| io::Error::other("NVIDIA IR path is not UTF-8"))?, "-Xclang", "-mlink-builtin-bitcode",
		"-Xclang", text(manifest, "nvidia-device-library")?, "-o", ]);
	command.arg(&ptx_output);
	run(&mut command, "NVIDIA LLVM IR compiler")?;
	println!("cargo:rustc-env=RECIPE_NV_PTX={}", ptx_output.display());
	println!("cargo:rustc-link-search=native={}", text(manifest, "nvidia-library")?); Ok(()) }
fn main() -> BuildResult<()> { let manifest = fs::read_to_string("Cargo.toml")?; for (key, environment) in [
		...
		("surrogate-width", "RECIPE_SURROGATE_WIDTH"), ("rat-batch-rows", "RECIPE_RAT_BATCH_ROWS"),
		...
		("ssh-connect-timeout-seconds", "RECIPE_SSH_CONNECT_TIMEOUT"), ("random-seed", "RECIPE_RANDOM_SEED"),
		("normalization-epsilon", "RECIPE_NORMALIZATION_EPSILON"), ("leak-slope", "RECIPE_LEAK_SLOPE"),
		("prelu-slope", "RECIPE_PRELU_SLOPE"), ("elu-alpha", "RECIPE_ELU_ALPHA"), ("selu-alpha", "RECIPE_SELU_ALPHA"),
		("selu-scale", "RECIPE_SELU_SCALE"), ("gelu-scale", "RECIPE_GELU_SCALE"), ("gelu-cubic", "RECIPE_GELU_CUBIC"),
		("huber-threshold", "RECIPE_HUBER_THRESHOLD"), ("output-tolerance", "RECIPE_OUTPUT_TOLERANCE"),
