fn main() {
	match detect_platform() {
		...
		Platform::Amd => {
			...
			let rocm_extra = std::env::var("ROCM_EXTRA_LIB").unwrap_or_else(|_e| format!("{rocm}/lib"));
...
fn detect_platform() -> Platform {
	...
	match configured {
		...
		Err(_e) => {
			let cuda = std::env::var("CUDA_PATH").unwrap_or_else(|_e2| "/opt/cuda".to_string());
	...
	for entry in walkdir("src") {
		...
		for line in text.lines() {
			...
			for pat in &banned {
				...
				let Some(_fire) = Some(pat).filter(|_p| hit) else { continue };
				panic!(
					"{}:{}: synchronous {} banned in training crate — use hipMallocAsync/hipFreeAsync",
					entry, lineno, pat.trim_end_matches('('),
				);
			}
		}
	}
...
fn walkdir(dir: &str) -> Vec<String> {
	...
	for e in rd.flatten() {
		...
		for f in Some(&p).filter(|q| !q.is_dir() && q.extension().is_some_and(|e| e == "rs")).into_iter() {
