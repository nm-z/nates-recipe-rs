...
/// Owned, validated model state for Recipe's first named GGUF execution case.
///
/// This instrument intentionally covers the ordinary dense-F32 `llama` graph:
/// equal query/KV head counts, full-head adjacent-pair RoPE, RMSNorm, causal
/// attention, and parallel SwiGLU. Other GGUF architectures and llama variants
...
pub fn prepare_gguf_llama_inference_table(
	artifact: GgufLlamaArtifact,
	table: &RawTable,
) -> InferencePreparationResult<PreparedGgufLlamaInference> {
	if table.width() != 1 {
		return Err(GgufLlamaError::new(
	...
	for (row, fields) in table.rows().iter().enumerate() {
			...
			.filter(|token| !token.is_empty())
		{
			...
			if value < 0
				|| u64::try_from(value)
					.ok()
					.is_none_or(|value| value >= artifact.vocabulary)
			{
				return Err(GgufLlamaError::new(
					GgufLlamaErrorKind::InvalidTokenStream,
					format!(
	...
	if u64::try_from(tokens.len())
		.ok()
		.is_none_or(|count| count > artifact.context_length)
	{
		return Err(GgufLlamaError::new(
			GgufLlamaErrorKind::InvalidTokenStream,
			format!(
...
fn prepare_rope_inputs(
	...
	artifact: &GgufLlamaArtifact,
	sequence: u64,
) -> InferenceCompileResult<RopeInputs> {
	...
	for row in 0..rows {
		...
		for pair in 0..rotary_half {
			...
			let base = row
				...
				.and_then(|base| base.checked_add(pair * 2))
				.ok_or_else(|| {
					InferenceCompileError::new(
						InferenceCompileErrorKind::ArithmeticOverflow,
...
fn apply_optional_scale(
	...
	values: &BTreeMap<usize, ValueId>,
	output_shape: &recipe_language::Shape,
) -> InferenceCompileResult<ValueId> {
...
impl<'archive, 'data> ArtifactTensorBuilder<'archive, 'data> {
	...
	fn linear(
		...
		prefix: &str,
		stem: &str,
		weight_dimensions: &[u64],
		output_width: u64,
	) -> GgufLlamaResult<GgufLlamaLinear> {
		Ok(GgufLlamaLinear {
