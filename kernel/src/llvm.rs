...
pub enum BufferAccess { Read, Write, }

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum KernelArgument { Buffer { access: BufferAccess, dtype: DType, alignment: u32, },
	...
	LoopIteration, ElementCount, }

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct KernelAbi { pub entry_symbol: String, pub arguments: Vec<KernelArgument>, pub argument_bytes: u32,
	pub argument_alignment: u32, pub elements: ElementCount, pub workgroup_lanes: u32, }

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct LoweringOptions { pub entry_symbol: String, pub workgroup_lanes: u32, }

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct LoweredKernel { pub llvm_ir: String, pub abi: KernelAbi, pub work: FlopCount,
...
#[derive(Clone, Debug)]
struct ValueRef { dtype: DType, operand: String, }

struct Emitter { body: String, next_temporary: u64, values: BTreeMap<ScalarValueId, ValueRef>,
	uses_f32_arithmetic: bool, uses_fma: bool, uses_minimum: bool, uses_maximum: bool, uses_sat_conversion: bool,
	uses_square_root: bool, uses_floor: bool, uses_ceiling: bool, uses_round_nearest_even: bool,
	fault_conditions: Vec<String>, }

impl Emitter { fn new() -> Self { Self { body: String::new(), next_temporary: 0, values: BTreeMap::new(),
			uses_f32_arithmetic: false, uses_fma: false, uses_minimum: false, uses_maximum: false, uses_sat_conversion: false,
			uses_square_root: false, uses_floor: false, uses_ceiling: false, uses_round_nearest_even: false,
			fault_conditions: Vec::new(), } }

	fn temporary(&mut self, purpose: &str) -> String { let id = self.next_temporary; self.next_temporary += 1;
		format!("%{purpose}_{id}")
	}
	...
	fn value(&self, id: ScalarValueId) -> Result<&ValueRef, LoweringError> { self.values.get(&id).ok_or_else(|| {
			LoweringError::new( LoweringErrorKind::UnknownScalarValue,
	...
	fn emit_fault_flag(&mut self) { let Some(first) = self.fault_conditions.first().cloned() else { return; };
		let mut rejected = first; for condition in self .fault_conditions .iter() .skip(1) .cloned() .collect::<Vec<_>>()
		{
			let combined = self.temporary("rejected");
			self.line(format_args!("  {combined} = or i1 {rejected}, {condition}"));
			rejected = combined; }
		let label_id = self.next_temporary; self.next_temporary += 1;
...
pub fn lower_elementwise( template: &KernelTemplate, target: &KernelTarget, options: &LoweringOptions,
) -> Result<LoweredKernel, LoweringError> { template .validate()
		.map_err(|errors| LoweringError::new(LoweringErrorKind::InvalidKernel, errors.to_string()))?; target.validate()?;
	validate_options(options)?;

	let mut emitter = Emitter::new(); emit_global_index(&mut emitter, target, options.workgroup_lanes);
	...
	for (index, (kernel_input, scalar_input)) in template .inputs .iter() .zip(&template.program.inputs) .enumerate()
	{ let element_index = emit_buffer_index( &mut emitter, &template.index_space, &kernel_input.access,
			&format!("input_{index}"),
		);
		...
		emitter.line(format_args!(
			...
			llvm_type(kernel_input.dtype) )); emitter.values.insert(scalar_input.id, ValueRef { dtype: scalar_input.dtype,
			operand: loaded, }); }
	for constant in &template.program.constants { let operand = match constant.value {
			ScalarLiteral::I32(value) => value.to_string(), ScalarLiteral::F32Bits(bits) => {
				let value = emitter.temporary("constant");
				emitter.line(format_args!("  {value} = bitcast i32 {bits} to float"));
				value } }; emitter.values.insert(constant.id, ValueRef { dtype: constant.value.dtype(), operand, }); }

	let mut flops_per_element = 0u64; for instruction in &template.program.instructions {
		let result = lower_instruction(&mut emitter, instruction)?; flops_per_element = flops_per_element
			.checked_add(instruction_flops(instruction.opcode)) .ok_or_else(|| { LoweringError::new(
					LoweringErrorKind::ArithmeticOverflow,
					"per-element FLOP count overflowed",
				) })?; emitter.values.insert(instruction.result, ValueRef { dtype: instruction.dtype, operand: result, }); }
	emitter.emit_fault_flag();

	for (index, (kernel_output, scalar_output)) in template .outputs .iter() .zip(&template.program.outputs) .enumerate()
	{ let value = emitter.value(*scalar_output)?.clone(); if value.dtype != kernel_output.dtype {
		...
		let element_index = emit_buffer_index( &mut emitter, &template.index_space, &kernel_output.access,
		...
		emitter.line(format_args!(
			...
			llvm_type(kernel_output.dtype), )); emitter.line(format_args!(
			"  store {} {}, ptr addrspace(1) {pointer}, align 4",
			llvm_type(kernel_output.dtype), value.operand )); }
	emitter.line(format_args!("  br label %exit"));
	emitter.line(format_args!("exit:"));
	emitter.line(format_args!("  ret void"));

	let llvm_ir = assemble_module(target, options, template, &emitter); let findings = audit_llvm_ir(&llvm_ir);
	if let Some(finding) = findings.first() { return Err(LoweringError::new( LoweringErrorKind::ProhibitedInterface,
			format!(
				"generated LLVM IR failed audit at line {}: {:?} `{}`",
				finding.line, finding.kind, finding.token ), )); }
	let mut pointer_arguments = template .inputs .len() .checked_add(template.outputs.len()) .ok_or_else(|| {
			LoweringError::new( LoweringErrorKind::ArithmeticOverflow,
				"kernel argument count overflowed",
			) })?; if !emitter.fault_conditions.is_empty() { pointer_arguments = pointer_arguments.checked_add(1).ok_or_else(|| {
			LoweringError::new( LoweringErrorKind::ArithmeticOverflow,
				"kernel fault argument count overflowed",
			) })?; }
	let argument_bytes = u32::try_from(pointer_arguments) .ok() .and_then(|count| count.checked_add(1))
		.and_then(|count| count.checked_mul(8)) .ok_or_else(|| { LoweringError::new( LoweringErrorKind::ArithmeticOverflow,
				"kernel ABI size overflowed",
			) })?; let mut arguments = template .inputs .iter() .map(|input| { KernelArgument::Buffer {
				access: BufferAccess::Read, dtype: input.dtype, alignment: 4, } }) .collect::<Vec<_>>();
	arguments.extend(template.outputs.iter().map(|output| { KernelArgument::Buffer { access: BufferAccess::Write,
			dtype: output.dtype, alignment: 4, } })); if !emitter.fault_conditions.is_empty() {
		arguments.push(KernelArgument::FaultFlag); }
	arguments.push(KernelArgument::ElementCount); let work = FlopCount::new(flops_per_element)
		.checked_mul(template.index_space.elements().get()) .ok_or_else(|| { LoweringError::new(
				LoweringErrorKind::ArithmeticOverflow,
				"kernel FLOP count overflowed",
			) })?; Ok(LoweredKernel { llvm_ir, abi: KernelAbi { entry_symbol: options.entry_symbol.clone(), arguments,
			argument_bytes, argument_alignment: 8, elements: template.index_space.elements(),
			workgroup_lanes: options.workgroup_lanes, }, work, target: target.clone(), }) }

fn emit_buffer_index( emitter: &mut Emitter, index_space: &IndexSpace, access: &StaticBufferAccess, label: &str,
) -> String { let mut logical_stride = index_space.elements().get();
	let mut accumulated = if access.offset_elements == 0 { None } else { Some(access.offset_elements.to_string()) };

	for (axis, (extent, physical_stride)) in index_space .dimensions() .iter() .zip(&access.strides) .enumerate()
	{ logical_stride /= extent.get(); if extent.get() == 1 || *physical_stride == 0 { continue; }
		let quotient = if logical_stride == 1 {
			...
			)); quotient }; let coordinate = if logical_stride .checked_mul(extent.get())
...
fn validate_options(options: &LoweringOptions) -> Result<(), LoweringError> {
	...
	if !valid_symbol { return Err(LoweringError::new( LoweringErrorKind::InvalidEntrySymbol,
			"entry symbol must be an ASCII LLVM identifier without punctuation",
		)); }
	if !(1..=1024).contains(&options.workgroup_lanes) { return Err(LoweringError::new(
			LoweringErrorKind::InvalidWorkgroupSize,
			"workgroup size must be between 1 and 1024 lanes",
		)); }
	Ok(()) }

fn emit_global_index(emitter: &mut Emitter, target: &KernelTarget, workgroup_lanes: u32) { match target {
		KernelTarget::Amd(_) => { emitter.line(format_args!(
				"  %local_id_32 = call i32 @llvm.amdgcn.workitem.id.x()"
			)); emitter.line(format_args!(
				"  %group_id_32 = call i32 @llvm.amdgcn.workgroup.id.x()"
			)); }
		KernelTarget::Nvidia(_) => { emitter.line(format_args!(
...
fn lower_instruction(emitter: &mut Emitter, instruction: &ScalarInstruction) -> Result<String, LoweringError> {
	let operands = instruction .operands .iter() .map(|id| emitter.value(*id).cloned()) .collect::<Result<Vec<_>, _>>()?;
	let result = match (instruction.opcode, instruction.dtype) {
		...
		(ScalarOpcode::Fma, DType::F32) => constrained_fma(emitter, &operands), (ScalarOpcode::Fma, DType::I32) => {
			return Err(LoweringError::new( LoweringErrorKind::UnsupportedOperation,
				"Fma is defined only for f32",
			) .for_scalar(instruction.result)); }
		( ScalarOpcode::Equal | ScalarOpcode::NotEqual | ScalarOpcode::LessThan | ScalarOpcode::LessThanOrEqual
			| ScalarOpcode::GreaterThan
			| ScalarOpcode::GreaterThanOrEqual, DType::I32, ) => comparison(emitter, instruction.opcode, &operands),
		...
		( ScalarOpcode::BitAnd | ScalarOpcode::BitOr | ScalarOpcode::BitXor | ScalarOpcode::BitNot | ScalarOpcode::ShiftLeft
			| ScalarOpcode::ShiftRightLogical
			| ScalarOpcode::ShiftRightArithmetic, DType::F32, ) => { return Err(LoweringError::new(
		...
		(ScalarOpcode::ConvertF32ToI32, DType::I32) => { emitter.uses_sat_conversion = true;
			let converted = emitter.temporary("converted");
			emitter.line(format_args!(
				"  {converted} = call i32 @llvm.fptosi.sat.i32.f32(float {})",
				operands[0].operand )); converted }
		(ScalarOpcode::ConvertI32ToF32, DType::F32) => {
			let converted = emitter.temporary("converted");
			emitter.line(format_args!(
				"  {converted} = sitofp i32 {} to float",
				operands[0].operand )); converted }
		(ScalarOpcode::ConvertF32ToI32 | ScalarOpcode::ConvertI32ToF32, _) => { return Err(LoweringError::new(
				LoweringErrorKind::UnsupportedOperation,
				"conversion result type does not match the opcode",
			) .for_scalar(instruction.result)); }
		_ => { return Err(LoweringError::new( LoweringErrorKind::UnsupportedOperation,
				"scalar opcode is newer than this lowering implementation",
			) .for_scalar(instruction.result)); } }; Ok(result) }
...
fn constrained_binary(emitter: &mut Emitter, operation: &str, operands: &[ValueRef]) -> String {
	...
	emitter.line(format_args!(
		...
		operands[0].operand, operands[1].operand )); emitter.canonicalize_nan(raw) }
...
fn constrained_fma(emitter: &mut Emitter, operands: &[ValueRef]) -> String { emitter.uses_fma = true;
	...
	emitter.line(format_args!(
		...
		operands[0].operand, operands[1].operand, operands[2].operand )); emitter.canonicalize_nan(raw) }
...
fn integer_binary(emitter: &mut Emitter, operation: &str, operands: &[ValueRef]) -> String {
	...
	emitter.line(format_args!(
		...
		operands[0].operand, operands[1].operand )); result }
...
fn checked_integer_divrem(emitter: &mut Emitter, operation: &str, operands: &[ValueRef]) -> String {
	...
	)); emitter.reject_when(rejected); result }
...
fn float_intrinsic(emitter: &mut Emitter, operation: &str, operands: &[ValueRef]) -> String { match operation {
		"minimum" => emitter.uses_minimum = true,
		"maximum" => emitter.uses_maximum = true,
		_ => unreachable!("known float intrinsic"),
	}
	let raw = emitter.temporary(operation); emitter.line(format_args!(
...
fn comparison(emitter: &mut Emitter, opcode: ScalarOpcode, operands: &[ValueRef]) -> String {
	...
	match operands[0].dtype { DType::I32 => { emitter.line(format_args!(
...
fn square_root(emitter: &mut Emitter, operands: &[ValueRef]) -> String { emitter.uses_square_root = true;
	let raw = emitter.temporary("sqrt");
	emitter.line(format_args!(
		"  {raw} = call float @llvm.sqrt.f32(float {})",
		operands[0].operand )); emitter.canonicalize_nan(raw) }

fn unary_float_intrinsic(emitter: &mut Emitter, operation: &str, operands: &[ValueRef]) -> String { match operation {
...
fn assemble_module( target: &KernelTarget, options: &LoweringOptions, template: &KernelTemplate, emitter: &Emitter,
) -> String { let mut module = String::new(); match target { KernelTarget::Amd(_) => {
	...
	if emitter.uses_fma { module.push_str(
	...
	.expect("writing to String");
	let mut arguments = template .inputs .iter() .enumerate()
		.map(|(index, _)| format!("ptr addrspace(1) %input_{index}"))
		.chain( template .outputs .iter() .enumerate()
				.map(|(index, _)| format!("ptr addrspace(1) %output_{index}")),
		) .collect::<Vec<_>>(); if !emitter.fault_conditions.is_empty() {
