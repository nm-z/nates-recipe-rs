...
use recipe_core::{DType, ElementCount, FlopCount, IndexSpace, KernelTemplate, ScalarInstruction, ScalarLiteral, ScalarOpcode, ScalarValueId, StaticBufferAccess};
...
pub fn lower_elementwise(template: &KernelTemplate, target: &KernelTarget, options: &LoweringOptions) -> Result<LoweredKernel, LoweringError> {
	template
		.validate()
...
fn emit_buffer_index(emitter: &mut Emitter, index_space: &IndexSpace, access: &StaticBufferAccess, label: &str) -> String {
...
fn lower_instruction(emitter: &mut Emitter, instruction: &ScalarInstruction) -> Result<String, LoweringError> {
	...
	let result = match (instruction.opcode, instruction.dtype) {
		...
		(ScalarOpcode::Equal | ScalarOpcode::NotEqual | ScalarOpcode::LessThan | ScalarOpcode::LessThanOrEqual | ScalarOpcode::GreaterThan | ScalarOpcode::GreaterThanOrEqual, DType::I32) => comparison(emitter, instruction.opcode, &operands),
		...
		(ScalarOpcode::BitAnd | ScalarOpcode::BitOr | ScalarOpcode::BitXor | ScalarOpcode::BitNot | ScalarOpcode::ShiftLeft | ScalarOpcode::ShiftRightLogical | ScalarOpcode::ShiftRightArithmetic, DType::F32) => {
			return Err(LoweringError::new(
				LoweringErrorKind::UnsupportedOperation,
...
fn assemble_module(target: &KernelTarget, options: &LoweringOptions, template: &KernelTemplate, emitter: &Emitter) -> String {
	...
	if emitter.uses_fma {
		module.push_str("declare float @llvm.experimental.constrained.fma.f32(float, float, float, metadata, metadata)\n");
	}
