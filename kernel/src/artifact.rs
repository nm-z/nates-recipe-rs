...
const ELF64_SECTION_BYTES: usize = 64;
...
const ELF_SECTION_NO_BITS: u32 = 8;
const ELF_SECTION_SYMBOLS: u32 = 2;
const ELF_SECTION_DYNAMIC_SYMBOLS: u32 = 11;
const ELF_UNDEFINED_SECTION: u16 = 0;
const SYMBOL_BIND_GLOBAL: u8 = 1;
const SYMBOL_BIND_WEAK: u8 = 2;
const SYMBOL_TYPE_OBJECT: u8 = 1;
const SYMBOL_TYPE_FUNCTION: u8 = 2;
const AMDGPU_METADATA_NOTE: u32 = 32;
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct HsaKernelMetadata {
	pub name: String,
	pub symbol: String,
	pub arguments: Vec<HsaKernelArgument>,
	...
	pub maximum_workgroup_size: u64,
...
impl<'a> ElfFile<'a> {
	fn parse(bytes: &'a [u8]) -> Result<Self, LoweringError> {
		...
		let string_section_index = usize::from(read_u16(bytes, 62)?);
		if section_entry_size != ELF64_SECTION_BYTES {
		...
		let raw_string_section = raw_section(
			bytes,
			section_offset,
			section_entry_size,
			string_section_index,
		)?;
		if raw_string_section.kind == ELF_SECTION_NO_BITS {
			return Err(artifact_format(
		...
		for index in 0..section_count {
			let raw = raw_section(bytes, section_offset, section_entry_size, index)?;
			sections.push(ElfSection {
				name: read_string(strings, usize_from_u32(raw.name)?)?.to_owned(),
				kind: raw.kind,
				flags: raw.flags,
				offset: usize_from_u64(raw.offset)?,
				size: usize_from_u64(raw.size)?,
				link: usize_from_u32(raw.link)?,
				entry_size: usize_from_u64(raw.entry_size)?,
			});
		}
		for section in &sections {
			if section.kind != ELF_SECTION_NO_BITS {
				subslice(bytes, section.offset, section.size)?;
			}
		}
		Ok(Self {
			bytes,
	...
	fn symbols(&self) -> Result<Vec<ElfSymbol>, LoweringError> {
		let mut symbols = Vec::new();
		for section in &self.sections {
			if !matches!(
				section.kind,
				ELF_SECTION_SYMBOLS | ELF_SECTION_DYNAMIC_SYMBOLS
			) {
				continue;
			}
			...
			let table = self.section_data(section)?;
			let strings = self.section_data(&self.sections[section.link])?;
			for entry in table.as_chunks::<ELF64_SYMBOL_BYTES>().0 {
				let name = read_string(strings, usize_from_u32(read_u32(entry, 0)?)?)?;
				if name.is_empty() {
					continue;
				}
				symbols.push(ElfSymbol {
					...
					binding: entry[4] >> 4,
					kind: entry[4] & 0x0f,
					section: read_u16(entry, 6)?,
					size: read_u64(entry, 16)?,
				});
			}
		}
		symbols.sort_by(|left, right| {
			(&left.name, left.section, left.binding, left.kind, left.size).cmp(&(
				&right.name,
				right.section,
				right.binding,
				right.kind,
		...
		symbols.dedup_by(|left, right| left.name == right.name && left.section == right.section && left.binding == right.binding && left.kind == right.kind && left.size == right.size);
		Ok(symbols)
	}
}
...
fn raw_section(bytes: &[u8], table_offset: usize, entry_size: usize, index: usize) -> Result<RawSection, LoweringError> {
	let offset = index
		.checked_mul(entry_size)
		.and_then(|value| table_offset.checked_add(value))
		.ok_or_else(|| artifact_format("section header offset overflowed"))?;
	let entry = subslice(bytes, offset, entry_size)?;
	Ok(RawSection {
		name: read_u32(entry, 0)?,
		kind: read_u32(entry, 4)?,
		flags: read_u64(entry, 8)?,
		offset: read_u64(entry, 24)?,
		size: read_u64(entry, 32)?,
		link: read_u32(entry, 40)?,
		entry_size: read_u64(entry, 56)?,
	})
}

/// Structurally inspect an HSACO and match its complete entry ABI.
pub fn inspect_hsaco(bytes: &[u8], expected_target_id: &str, expected_code_object_version: u8, expected_abi: &KernelAbi) -> Result<InspectedHsaco, LoweringError> {
...
pub fn inspect_hsaco_bundle<'a>(bytes: &[u8], expected_target_id: &str, expected_code_object_version: u8, expected_abis: impl IntoIterator<Item = &'a KernelAbi>) -> Result<Vec<InspectedHsaco>, LoweringError> {
	...
	let code_object_version = elf
		.abi_version
		.checked_add(2)
		.ok_or_else(|| artifact_format("code-object version overflowed"))?;
	if code_object_version != expected_code_object_version {
		return Err(artifact_mismatch(format!(
	...
	let defined_symbols = symbols
		.iter()
		.filter(|symbol| symbol.section != ELF_UNDEFINED_SECTION && matches!(symbol.binding, SYMBOL_BIND_GLOBAL | SYMBOL_BIND_WEAK))
	...
	let target_id = metadata
		...
		.to_owned();
	if !target_ids_match(&target_id, expected_target_id) {
		return Err(artifact_mismatch(format!(
...
fn target_ids_match(actual: &str, expected: &str) -> bool {
	fn components(value: &str) -> Option<(&str, BTreeMap<&str, bool>)> {
		...
		for field in fields {
			let (name, polarity) = field.split_at(field.len().checked_sub(1)?);
			let enabled = match polarity {
				"+" => true,
				"-" => false,
				_ => return None,
			};
			if name.is_empty() || features.insert(name, enabled).is_some() {
				return None;
			}
		}
...
pub fn inspect_cubin(bytes: &[u8], expected_sm: u8, expected_entry_symbol: &str) -> Result<InspectedCubin, LoweringError> {
	let elf = ElfFile::parse(bytes)?;
	...
	if text.size == 0 || text.flags & 0x4 == 0 || symbol.size == 0 {
...
fn validate_hsa_abi(metadata: &HsaKernelMetadata, expected: &KernelAbi) -> Result<(), LoweringError> {
		...
		.enumerate()
	{
		...
		if actual.offset != offset || actual.size != 8 || actual.is_hidden() {
			return Err(artifact_mismatch(format!(
		...
		match expected {
			...
			KernelArgument::RunId => {
				if actual.name.as_deref().is_some_and(|name| name != "run_id") || actual.value_kind != "by_value" {
	...
	if metadata.arguments[expected.arguments.len()..]
		.iter()
		.any(|argument| !argument.is_hidden())
	{
		return Err(artifact_mismatch(
...
fn require_symbol<'a>(symbols: &'a [ElfSymbol], name: &str, kind: u8) -> Result<&'a ElfSymbol, LoweringError> {
	symbols
		.iter()
		.find(|symbol| symbol.name == name && symbol.kind == kind && symbol.section != ELF_UNDEFINED_SECTION && matches!(symbol.binding, SYMBOL_BIND_GLOBAL | SYMBOL_BIND_WEAK))
		.ok_or_else(|| {
			artifact_mismatch(format!(
				"artifact has no defined global symbol `{name}` of kind {kind}"
			))
...
fn audit_symbols(symbols: &[ElfSymbol]) -> Result<(), LoweringError> {
	let mut undefined = BTreeSet::new();
	for symbol in symbols {
		if symbol.section == ELF_UNDEFINED_SECTION && matches!(symbol.binding, SYMBOL_BIND_GLOBAL | SYMBOL_BIND_WEAK) {
...
fn hsa_metadata(elf: &ElfFile<'_>) -> Result<MsgMap, LoweringError> {
	for section in &elf.sections {
		if section.kind != ELF_SECTION_NOTE {
			continue;
		}
		let notes = elf.section_data(section)?;
		let mut cursor = 0usize;
		while cursor < notes.len() {
			let names = usize_from_u32(read_u32(notes, cursor)?)?;
			let descriptions = usize_from_u32(read_u32(notes, cursor + 4)?)?;
			let note_type = read_u32(notes, cursor + 8)?;
			cursor = cursor
				.checked_add(12)
				.ok_or_else(|| artifact_format("note offset overflowed"))?;
			let name = subslice(notes, cursor, names)?;
			cursor = align_four(
				cursor.checked_add(names)
					.ok_or_else(|| artifact_format("note name overflowed"))?,
			)?;
			let description = subslice(notes, cursor, descriptions)?;
			cursor = align_four(
				cursor.checked_add(descriptions)
			...
			if note_type == AMDGPU_METADATA_NOTE && &name[..name_end] == b"AMDGPU" {
				let mut decoder = MsgDecoder::new(description);
...
fn parse_hsa_kernel(map: &MsgMap) -> Result<HsaKernelMetadata, LoweringError> {
	let arguments = map
		.array(".args")
...
fn required_string(map: &MsgMap, key: &str) -> Result<String, LoweringError> {
	map.string(key)
		.map(str::to_owned)
		.ok_or_else(|| artifact_format(format!("kernel metadata omits `{key}`")))
}

fn required_unsigned(map: &MsgMap, key: &str) -> Result<u64, LoweringError> {
	map.unsigned(key)
		.ok_or_else(|| artifact_format(format!("kernel metadata omits `{key}`")))
}
...
fn subslice(bytes: &[u8], offset: usize, size: usize) -> Result<&[u8], LoweringError> {
	let end = offset
		.checked_add(size)
...
fn read_u16(bytes: &[u8], offset: usize) -> Result<u16, LoweringError> {
	let bytes: [u8; 2] = subslice(bytes, offset, 2)?
		.try_into()
...
fn read_u32(bytes: &[u8], offset: usize) -> Result<u32, LoweringError> {
	let bytes: [u8; 4] = subslice(bytes, offset, 4)?
		.try_into()
...
fn read_u64(bytes: &[u8], offset: usize) -> Result<u64, LoweringError> {
	let bytes: [u8; 8] = subslice(bytes, offset, 8)?
		.try_into()
		.map_err(|_| artifact_format("invalid u64 range"))?;
	Ok(u64::from_le_bytes(bytes))
}

fn read_string(bytes: &[u8], offset: usize) -> Result<&str, LoweringError> {
	let rest = bytes
		.get(offset..)
...
fn align_four(value: usize) -> Result<usize, LoweringError> {
	value.checked_add(3)
		.map(|value| value & !3)
...
impl MsgMapExt for MsgMap {
	fn string(&self, key: &str) -> Option<&str> {
		match self.get(key) {
			Some(MsgValue::String(value)) => Some(value),
			_ => None,
		}
	}
	...
	fn unsigned(&self, key: &str) -> Option<u64> {
		match self.get(key) {
			Some(MsgValue::Unsigned(value)) => Some(*value),
			Some(MsgValue::Signed(value)) => u64::try_from(*value).ok(),
			_ => None,
		}
	}
	...
	fn array(&self, key: &str) -> Option<&[MsgValue]> {
		match self.get(key) {
			Some(MsgValue::Array(value)) => Some(value),
			_ => None,
		}
	}
}
...
impl MsgValue {
	...
	fn into_map(self) -> Option<MsgMap> {
		match self {
			Self::Map(value) => Some(value),
			_ => None,
		}
...
impl<'a> MsgDecoder<'a> {
	...
	fn decode(&mut self) -> Result<MsgValue, LoweringError> {
		...
		let value = match marker {
			0x00..=0x7f => MsgValue::Unsigned(u64::from(marker)),
			...
			0xc0 => MsgValue::Nil,
			0xc2 => MsgValue::Boolean(false),
			0xc3 => MsgValue::Boolean(true),
			0xcc => MsgValue::Unsigned(u64::from(self.byte()?)),
			0xcd => MsgValue::Unsigned(u64::from(self.be_u16()?)),
			0xce => MsgValue::Unsigned(u64::from(self.be_u32()?)),
			0xcf => MsgValue::Unsigned(self.be_u64()?),
			...
			0xd3 => MsgValue::Signed(i64::from_be_bytes(self.take_array()?)),
			0xd9 => {
				let size = usize::from(self.byte()?);
				self.string(size)?
			}
			0xda => {
				let size = usize::from(self.be_u16()?);
				self.string(size)?
			}
			0xdb => {
				let size = usize_from_u32(self.be_u32()?)?;
				self.string(size)?
			}
			0xdc => {
				let size = usize::from(self.be_u16()?);
			...
			0xe0..=0xff => MsgValue::Signed(i64::from(i8::from_be_bytes([marker]))),
			_ => {
				return Err(artifact_format(format!(
	...
	fn array(&mut self, size: usize) -> Result<MsgValue, LoweringError> {
	...
	fn map(&mut self, size: usize) -> Result<MsgValue, LoweringError> {
		let mut values = MsgMap::new();
		for _ in 0..size {
			let MsgValue::String(key) = self.decode()? else {
				return Err(artifact_format("MessagePack map key is not a string"));
			};
			if values.insert(key.clone(), self.decode()?).is_some() {
				return Err(artifact_format(format!(
	...
	fn string(&mut self, size: usize) -> Result<MsgValue, LoweringError> {
	...
	fn take(&mut self, size: usize) -> Result<&'a [u8], LoweringError> {
		let bytes = subslice(self.bytes, self.cursor, size)?;
		self.cursor = self
			.cursor
			.checked_add(size)
	...
	fn take_array<const SIZE: usize>(&mut self) -> Result<[u8; SIZE], LoweringError> {
		self.take(SIZE)?
			.try_into()
