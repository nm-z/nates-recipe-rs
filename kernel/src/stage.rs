...
use recipe_core::{
	ArtifactBuildAccess, ArtifactBuildRecipe, ByteCount, DType, Digest, ElementCount, KernelResourceBounds, ScalarLiteral,
}; use recipe_language::{
...
use recipe_primitives::{
	...
	SortTieBreak, StageKind, SynchronizationScope, TreePhase, UniformI32Mapping, }; use sha2::{Digest as _, Sha256};

use crate::{ BufferAccess, KernelAbi, KernelArgument, KernelTarget, LoweredKernel, LoweringError, LoweringErrorKind,
...
pub(crate) fn artifact_build_contract_digest(build: &ArtifactBuildRecipe) -> Digest {
	...
	digest.length(build.bindings.len()); for binding in &build.bindings { digest.u64(binding.value.get());
		digest.u8(match binding.dtype { DType::F32 => 0, DType::I32 => 1, }); digest.u8(match binding.access {
...
pub(crate) fn lower_stage( program: &LoweredProgram, build: &ArtifactBuildRecipe, target: &KernelTarget,
	options: &LoweringOptions, ) -> Result<LoweredKernel, LoweringError> {
	let stage = validate_contract(program, build, target, options)?; match &stage.kind {
		StageKind::ScalarMap { template } => { let mut lowered = crate::lower_elementwise(template, target, options)?;
			rewrite_scalar_fault(&mut lowered, stage.fault)?; validate_realized_kernel(&lowered, stage, build)?; Ok(lowered) }
...
fn rewrite_scalar_fault(lowered: &mut LoweredKernel, fault: Option<FaultContract>) -> Result<(), LoweringError> {
	let fault = match fault { Some(fault) => fault, None => return Ok(()), };
	let old = "atomicrmw or ptr addrspace(1) %fault_flag, i32 1 monotonic, align 4";
	ensure_stage( lowered.llvm_ir.contains(old),
		...
		fault.code ); lowered.llvm_ir = lowered.llvm_ir.replacen(old, &replacement, 1); Ok(()) }

fn validate_contract<'a>(
	program: &'a LoweredProgram,
	build: &ArtifactBuildRecipe, target: &KernelTarget, options: &LoweringOptions,
) -> Result<&'a ProgramStage, LoweringError> {
	ensure_stage( program.validate().is_ok(),
		"lowered program failed canonical validation",
	)?; ensure_stage( build.validate().is_ok(),
		"artifact build recipe failed canonical validation",
	)?; target.validate()?; validate_options(options)?; ensure_stage(
	...
	)?; ensure_stage( build.source_kernel == program.source_kernel,
		"artifact build source kernel differs from the lowered program",
	)?; let stage = program .stages .iter() .find(|stage| stage.id.get() == build.provenance.stage_ordinal)
		.ok_or_else(|| stage_error("artifact build stage ordinal is absent from the lowered program"))?;
	let expected_template = stage_template_identity(program, stage)?; ensure_stage(
		build.kernel_template == expected_template,
		"artifact build stage-scoped kernel identity is not canonical",
	)?; ensure_stage( build.artifact.get() == build.kernel_template.get(),
		"artifact ID differs from the reserved stage-scoped identity",
	)?; ensure_stage( build.provenance.contract_digest == artifact_build_contract_digest(build),
		"artifact build contract digest does not match its contents",
	)?; ensure_stage( build.dispatch.logical_lanes == stage.geometry.logical_lanes
	...
	)?; ensure_stage( options.workgroup_lanes == stage.geometry.workgroup_lanes,
		"lowering options workgroup size differs from the immutable stage geometry",
	)?; ensure_stage( build.work.flops == stage.resources.flops
	...
	)?; let expected_resources = KernelResourceBounds { private_bytes_per_lane: stage.resources.private_bytes_per_lane,
		shared_bytes_per_workgroup: stage.resources.shared_bytes_per_workgroup, scratch_bytes_per_dispatch: ByteCount::ZERO,
		maximum_workgroup_lanes: stage.resources.maximum_workgroup_lanes, }; ensure_stage(
		build.resources == expected_resources,
		"artifact resource envelope differs from the primitive stage",
	)?; ensure_stage( build.bindings.len() == stage.bindings.len(),
		"artifact binding count differs from the primitive stage",
	)?; for (actual, expected) in build.bindings.iter().zip(&stage.bindings) { let expected_access = access(expected.mode);
		ensure_stage( actual.dtype == expected.dtype && actual.access == expected_access
	...
	validate_fault_binding(stage, build)?; Ok(stage) }

fn validate_fault_binding(stage: &ProgramStage, build: &ArtifactBuildRecipe) -> Result<(), LoweringError> {
	let fault = match stage.fault { Some(fault) => fault, None => { return ensure_stage( build.fault_flag.is_none(),
				"artifact supplies a fault value for an unchecked stage",
			); } }; let position = stage .bindings .iter() .position(|binding| binding.buffer == fault.flag)
		.ok_or_else(|| stage_error("stage fault flag has no ordered binding"))?;
	ensure_stage( build.fault_flag == Some(build.bindings[position].value),
...
fn validate_options(options: &LoweringOptions) -> Result<(), LoweringError> {
	let valid_symbol = !options.entry_symbol.is_empty() && options .entry_symbol .bytes() .next()
			.is_some_and(|byte| byte.is_ascii_alphabetic() || byte == b'_')
		&& options .entry_symbol .bytes()
			.all(|byte| byte.is_ascii_alphanumeric() || byte == b'_');
	ensure( valid_symbol, LoweringErrorKind::InvalidEntrySymbol,
		"entry symbol must be an ASCII LLVM identifier without punctuation",
	)?; ensure( (1..=1024).contains(&options.workgroup_lanes), LoweringErrorKind::InvalidWorkgroupSize,
		"workgroup size must be between 1 and 1024 lanes",
	)?; Ok(()) }

fn stage_template_identity( program: &LoweredProgram, stage: &ProgramStage,
	...
	let bytes = digest.finish().bytes(); let mut prefix = [0_u8; 8]; prefix.copy_from_slice(&bytes[..8]);
	let identity = recipe_core::KernelTemplateId::new(u64::from_le_bytes(prefix)); ensure_stage( identity.get() != 0,
		"canonical stage-scoped kernel identity is the reserved zero value",
	)?; Ok(identity) }

const fn access(mode: AccessMode) -> ArtifactBuildAccess { match mode { AccessMode::Read => ArtifactBuildAccess::Read,
		AccessMode::Write => ArtifactBuildAccess::Write, AccessMode::ReadWrite => ArtifactBuildAccess::ReadWrite,
		AccessMode::ReadWriteAtomic => ArtifactBuildAccess::ReadWriteAtomic, } }

fn stage_error(message: impl Into<String>) -> LoweringError { LoweringError::new(LoweringErrorKind::InvalidStageContract, message) }

fn ensure(condition: bool, kind: LoweringErrorKind, message: &'static str) -> Result<(), LoweringError> {
	match condition { true => Ok(()), false => Err(LoweringError::new(kind, message)), } }

fn ensure_stage(condition: bool, message: &'static str) -> Result<(), LoweringError> { ensure(condition, LoweringErrorKind::InvalidStageContract, message) }

#[derive(Debug)]
struct ContractDigest { hasher: Sha256, }

impl ContractDigest { fn new(domain: &str) -> Self { let mut result = Self { hasher: Sha256::new(), };
		result.bytes(domain.as_bytes()); result }

	fn bytes(&mut self, value: &[u8]) { self.hasher.update(value.len().to_string().as_bytes());
	...
	fn finish(self) -> Digest { let finalized = self.hasher.finalize(); let mut bytes = [0_u8; 32];
		bytes.copy_from_slice(&finalized); Digest::new(bytes) } }

#[derive(Clone, Debug)]
struct BoundPointer { dtype: DType, view: recipe_core::ArtifactBuildView, read: Option<String>, write: Option<String>,
	fault: bool, }

#[derive(Debug)]
struct StageSignature { pointers: Vec<BoundPointer>, arguments: Vec<KernelArgument>, parameters: Vec<String>,
	has_run_id: bool, has_loop_iteration: bool, }

impl StageSignature { fn new( stage: &ProgramStage, build: &ArtifactBuildRecipe, has_run_id: bool,
		has_loop_iteration: bool, ) -> Result<Self, LoweringError> { let fault_buffer = stage.fault.map(|fault| fault.flag);
		let mut pointers = build .bindings .iter() .zip(&stage.bindings) .map(|(build, stage)| { BoundPointer {
					dtype: build.dtype, view: build.view.clone(), read: None, write: None, fault: fault_buffer == Some(stage.buffer), }
			}) .collect::<Vec<_>>(); let mut arguments = Vec::new(); let mut parameters = Vec::new();
		for (input, (pointer, binding)) in pointers .iter_mut() .zip(&build.bindings)
			.filter(|(pointer, binding)| !pointer.fault && binding.access.reads()) .enumerate()
		{
			let name = format!("%input_{input}");
			pointer.read = Some(name.clone());
			parameters.push(format!("ptr addrspace(1) {name}"));
			arguments.push(KernelArgument::Buffer { access: BufferAccess::Read, dtype: binding.dtype, alignment: 4, }); }
		for (output, (pointer, binding)) in pointers .iter_mut() .zip(&build.bindings)
			.filter(|(pointer, binding)| !pointer.fault && binding.access.writes()) .enumerate()
		{
			let name = format!("%output_{output}");
			pointer.write = Some(name.clone());
			parameters.push(format!("ptr addrspace(1) {name}"));
			arguments.push(KernelArgument::Buffer { access: BufferAccess::Write, dtype: binding.dtype, alignment: 4, }); }
		let fault_arguments = usize::from(stage.fault.is_some()); parameters.extend(std::iter::repeat_n(
			"ptr addrspace(1) %fault_flag".to_owned(),
			fault_arguments, )); arguments.extend(std::iter::repeat_n( KernelArgument::FaultFlag, fault_arguments, ));
		parameters.extend(std::iter::repeat_n(
			"i64 %run_id".to_owned(),
			usize::from(has_run_id), )); arguments.extend(std::iter::repeat_n( KernelArgument::RunId, usize::from(has_run_id),
		)); parameters.extend(std::iter::repeat_n(
			"i64 %loop_iteration".to_owned(),
			usize::from(has_loop_iteration), )); arguments.extend(std::iter::repeat_n( KernelArgument::LoopIteration,
			usize::from(has_loop_iteration), ));
		parameters.push("i64 %element_count".to_owned());
		arguments.push(KernelArgument::ElementCount); Ok(Self { pointers, arguments, parameters, has_run_id,
			has_loop_iteration, }) }

	fn pointer(&self, index: usize) -> Result<&BoundPointer, LoweringError> { self.pointers .get(index)
	...
	fn abi(&self, entry_symbol: &str, logical_lanes: u64, workgroup_lanes: u32) -> Result<KernelAbi, LoweringError> {
		let argument_count = match u32::try_from(self.arguments.len()) { Ok(count) => count, Err(error) => {
				return Err(LoweringError::new( LoweringErrorKind::ArithmeticOverflow, error.to_string(), )); } };
		let argument_bytes = argument_count.checked_mul(8).ok_or_else(|| { LoweringError::new(
				LoweringErrorKind::ArithmeticOverflow,
				"stage kernel ABI size overflowed",
			) })?; let elements = ElementCount::new(logical_lanes).map_err(|error| { stage_error(format!(
				"stage logical lane count cannot form a kernel ABI: {error}"
			)) })?; Ok(KernelAbi { entry_symbol: entry_symbol.to_owned(), arguments: self.arguments.clone(), argument_bytes,
			argument_alignment: 8, elements, workgroup_lanes, }) } }

#[derive(Debug)]
struct Ir { body: String, next: u64,
	declarations: BTreeMap<&'static str, &'static str>,
	globals: Vec<String>, helpers: BTreeMap<String, String>, }

impl Ir { fn new() -> Self { Self { body: String::default(), next: 0, declarations: BTreeMap::new(),
			globals: Vec::new(), helpers: BTreeMap::new(), } }
	...
	fn emit_global_index(&mut self, target: &KernelTarget, workgroup_lanes: u32) { match target { KernelTarget::Amd(_) => {
				self.declare(
			...
			KernelTarget::Nvidia(_) => { self.declare(
	...
	fn barrier(&mut self, target: &KernelTarget) { match target { KernelTarget::Amd(_) => { self.declare(
	...
	fn coordinates(&mut self, linear: &str, extents: &[u64], purpose: &str) -> Vec<String> {
		let mut divisor = extents.iter().copied().product::<u64>(); extents .iter() .copied() .enumerate()
			.map(|(axis, extent)| { divisor /= extent; let quotient = match divisor {
					0 => unreachable!("tensor extents are nonzero"),
					1 => linear.to_owned(), 2..=u64::MAX => {
	...
	fn index_from_coordinates(&mut self, pointer: &BoundPointer, coordinates: &[String], purpose: &str) -> String {
		let mut accumulated = match pointer.view.offset_elements { 0 => None, offset => Some(offset.to_string()), };
		for (axis, (coordinate, stride)) in coordinates .iter() .zip(&pointer.view.strides) .enumerate()
	...
	fn load(&mut self, pointer: &BoundPointer, index: &str, purpose: &str) -> Result<String, LoweringError> {
		let base = pointer .read .as_deref()
		...
		self.line(format_args!(
			...
			llvm_type(pointer.dtype) )); let value = self.temporary(purpose); self.line(format_args!(
			"  {value} = load {}, ptr addrspace(1) {address}, align 4",
			llvm_type(pointer.dtype) )); Ok(value) }

	fn store( &mut self, pointer: &BoundPointer, index: &str, value: &str, purpose: &str, ) -> Result<(), LoweringError> {
		let base = pointer .write .as_deref()
			.ok_or_else(|| stage_error(format!("{purpose} binding is not writable")))?;
		let address = self.temporary(&format!("{purpose}_address"));
		self.line(format_args!(
			"  {address} = getelementptr inbounds {}, ptr addrspace(1) {base}, i64 {index}",
			llvm_type(pointer.dtype) )); self.line(format_args!(
			"  store {} {value}, ptr addrspace(1) {address}, align 4",
			llvm_type(pointer.dtype) )); Ok(()) }
	...
	fn write_address(&mut self, pointer: &BoundPointer, index: &str, purpose: &str) -> Result<String, LoweringError> {
		...
		self.line(format_args!(
			"  {address} = getelementptr inbounds {}, ptr addrspace(1) {base}, i64 {index}",
			llvm_type(pointer.dtype) )); Ok(address) } }
...
const fn private_address_space(target: &KernelTarget) -> u32 { match target { KernelTarget::Amd(_) => 5,
...
fn lower_owned_stage( program: &LoweredProgram, stage: &ProgramStage, kind: &StageKind, build: &ArtifactBuildRecipe,
	target: &KernelTarget, options: &LoweringOptions, ) -> Result<LoweredKernel, LoweringError> {
	...
	let signature = StageSignature::new(stage, build, has_run_id, has_loop_iteration)?; let mut ir = Ir::new();
	ir.emit_global_index(target, stage.geometry.workgroup_lanes); match kind {
		StageKind::Fill { value } => emit_fill(&mut ir, &signature, *value)?,
		StageKind::Copy { .. } => emit_copy(&mut ir, &signature)?, StageKind::FixedTreeReduce(reduction) => {
		...
		StageKind::ScanUniformCombine(scan) => { let axis = scan_axis(program, stage, scan.level)?;
			emit_scan_uniform(&mut ir, &signature, scan, axis)? }
		StageKind::TiledContraction(contraction) => { emit_contraction(&mut ir, &signature, contraction, target, options)? }
		StageKind::Gather { axis, bounds } => emit_gather(&mut ir, &signature, stage.fault, *axis, *bounds)?,
		StageKind::Scatter { axis, bounds, conflict, } => { emit_scatter( &mut ir, &signature, stage.fault, stage, *axis,
				*bounds, *conflict, )? }
		StageKind::HistogramClear => emit_zero(&mut ir, &signature)?, StageKind::HistogramAccumulate { bins, weighted,
			bin_mapping, ordering, } => { emit_histogram(&mut ir, &signature, stage, HistogramContract { fault: stage .fault
					.ok_or_else(|| stage_error("histogram stage omitted its checked fault contract"))?,
				bins: *bins, weighted: *weighted, mapping: *bin_mapping, ordering: *ordering, })? }
		...
		StageKind::StableSortFinalize { network, emit_indices,
		...
		StageKind::ScalarMap { .. } => { return Err(stage_error(
				"scalar-map stage reached the owned-stage emitter",
			)); } }
	let abi = signature.abi( &options.entry_symbol, stage.geometry.logical_lanes, stage.geometry.workgroup_lanes, )?;
	let llvm_ir = assemble_stage_module(target, options, &signature, &ir); let findings = audit_llvm_ir(&llvm_ir);
	match findings.first() { Some(finding) => { Err(LoweringError::new( LoweringErrorKind::ProhibitedInterface, format!(
					"generated stage LLVM IR failed audit at line {}: {:?} `{}`",
					finding.line, finding.kind, finding.token ), )) }
		None => Ok(()), }?; let lowered = LoweredKernel { llvm_ir, abi, work: build.work.flops, target: target.clone(), };
	validate_realized_kernel(&lowered, stage, build)?; Ok(lowered) }

fn validate_realized_kernel( lowered: &LoweredKernel, stage: &ProgramStage, build: &ArtifactBuildRecipe,
) -> Result<(), LoweringError> { ensure_stage( lowered.abi.elements.get() == stage.geometry.logical_lanes
	...
	)?; let expected_faults = usize::from(stage.fault.is_some()); ensure_stage( lowered .abi .arguments .iter()
			.filter(|argument| matches!(argument, KernelArgument::FaultFlag)) .count() == expected_faults,
		"realized kernel fault ABI differs from the immutable stage",
	)?; Ok(()) }

fn assemble_stage_module( target: &KernelTarget, options: &LoweringOptions, signature: &StageSignature, ir: &Ir,
) -> String { let mut module = String::default(); match target {
	...
	for declaration in ir.declarations.values() { module.push_str(declaration);
		module.push('\n');
	}
	push_section_gap(&mut module, !ir.declarations.is_empty()); for global in &ir.globals { module.push_str(global);
		module.push('\n');
	}
	push_section_gap(&mut module, !ir.globals.is_empty()); for helper in ir.helpers.values() {
...
fn begin_lane(ir: &mut Ir) { ir.line(format_args!(
...
fn emit_fill(ir: &mut Ir, signature: &StageSignature, value: ScalarLiteral) -> Result<(), LoweringError> {
	let output = signature.pointer(0)?; ensure_stage( output.dtype == value.dtype(),
...
fn emit_zero(ir: &mut Ir, signature: &StageSignature) -> Result<(), LoweringError> { let output = signature.pointer(0)?;
	begin_lane(ir);
...
fn emit_copy(ir: &mut Ir, signature: &StageSignature) -> Result<(), LoweringError> { let input = signature.pointer(0)?;
	let output = signature.pointer(1)?; ensure_stage( input.dtype == output.dtype,
...
fn emit_index_map( ir: &mut Ir, signature: &StageSignature, stage: &ProgramStage, spec: IndexMap,
) -> Result<(), LoweringError> { let output = signature.pointer(0)?; let fault = stage .fault
		.ok_or_else(|| stage_error("index-map stage omitted its checked fault contract"))?;
	ensure_stage( output.dtype == DType::I32 && !signature.has_run_id
	...
	)?; ensure_stage( spec.modulus.is_none_or(|modulus| modulus > 0),
		"index-map modulus is not strictly positive",
	)?;

	begin_lane(ir); let mut rejected = Vec::new(); let element = checked_unsigned_scale( ir,
		"%global_id",
		spec.element_step,
		"index_map_element",
		&mut rejected, ); let iteration = match spec.iteration_step {
		0 => "0".to_owned(),
		step => { checked_unsigned_scale( ir,
				"%loop_iteration",
				step,
				"index_map_iteration",
				&mut rejected, ) } }; let with_element = checked_signed_add( ir, &spec.start.to_string(), &element,
		"index_map_start_element",
		&mut rejected, ); let affine = checked_signed_add( ir, &with_element, &iteration,
		"index_map_affine",
		&mut rejected, ); let result = match spec.modulus { Some(modulus) => {
...
fn checked_unsigned_scale( ir: &mut Ir, unsigned: &str, scale: i32, purpose: &str, rejected: &mut Vec<String>,
) -> String { if scale == 0 {
		return "0".to_owned();
	}
	ir.declare(
	...
	ir.line(format_args!(
		...
		i64::MAX )); rejected.push(outside_signed);
...
fn literal_operand(ir: &mut Ir, literal: ScalarLiteral, purpose: &str) -> String { match literal {
		ScalarLiteral::I32(value) => value.to_string(), ScalarLiteral::F32Bits(bits) => { let value = ir.temporary(purpose);
...
fn scan_axis(program: &LoweredProgram, stage: &ProgramStage, level: u32) -> Result<usize, LoweringError> {
	let mut axes = program .stages .iter() .filter(|candidate| candidate.id != stage.id)
		.filter_map(|candidate| scan_local_axis(&candidate.kind, level)) .collect::<Vec<_>>(); axes.sort_unstable();
	axes.dedup(); match axes.as_slice() { [axis] => Ok(*axis), [] => { Err(stage_error(
...
fn scan_local_axis(kind: &StageKind, level: u32) -> Option<usize> { match kind {
		StageKind::FixedTreeScanLocal(local) => (local.level == level).then_some(local.axis), StageKind::ScalarMap { .. }
		| StageKind::Fill { .. }
		| StageKind::Copy { .. }
...
fn emit_reduction( ir: &mut Ir, signature: &StageSignature, stage: &ProgramStage, reduction: &ReductionStage,
	target: &KernelTarget, options: &LoweringOptions, ) -> Result<(), LoweringError> { ensure_stage(
	...
	)?; validate_tree_contract(&reduction.tree, stage, TreePhase::Reduction)?; ensure_stage(
	...
	)?; let reads = signature .pointers .iter() .enumerate()
		.filter(|(_, pointer)| !pointer.fault && pointer.read.is_some()) .map(|(index, _)| index) .collect::<Vec<_>>();
	let writes = signature .pointers .iter() .enumerate() .filter(|(_, pointer)| !pointer.fault && pointer.write.is_some())
		.map(|(index, _)| index) .collect::<Vec<_>>(); let value_input = signature.pointer( *reads.first()
			.ok_or_else(|| stage_error("reduction stage has no readable value binding"))?,
	)?; ensure_stage( value_input.dtype == reduction.dtype,
		"reduction value input type differs from its stage contract",
	)?; let has_index = reduction.result != ReduceResult::Value; let index_input = match has_index && reduction.pass != 0 {
		true => { Some(signature .pointer(*reads.get(1).ok_or_else(|| {
					stage_error("later reduction pass omitted its readable index binding")
				})?)?) }
		false => None, }; let final_pass = reduction.output_width == 1;
	let (value_output, index_output) = match (final_pass, reduction.result) { (_, ReduceResult::Value) => { (
				Some(signature.pointer( *writes .first()
						.ok_or_else(|| stage_error("value reduction omitted its writable result"))?,
				)?), None, ) }
		(true, ReduceResult::Index) => { ( None, Some(signature.pointer( *writes .first()
						.ok_or_else(|| stage_error("index reduction omitted its writable result"))?,
				)?), ) }
		(false, ReduceResult::Index) | (_, ReduceResult::ValueAndIndex) => { ( Some(signature.pointer( *writes .first()
						.ok_or_else(|| stage_error("indexed reduction omitted its writable value"))?,
				)?), Some(signature.pointer( *writes .get(1)
						.ok_or_else(|| stage_error("indexed reduction omitted its writable index"))?,
				)?), ) } }; ensure_stage( index_input .into_iter() .chain(index_output) .all(|pointer| pointer.dtype == DType::I32),
	...
	let identity = reduction_identity( ir, reduction.operator, reduction.dtype,
		"reduction_identity",
	)?;
	let initial = ir.temporary("reduction_initial");
	ir.line(format_args!(
		"  {initial} = select i1 {active}, {} {loaded}, {} {identity}",
		llvm_type(reduction.dtype), llvm_type(reduction.dtype) )); shared_store( ir, &value_global, reduction.tree.lanes,
		"%local_id",
		reduction.dtype, &initial,
		"reduction_shared_value",
	);

	initialize_reduction_index( ir, index_global.as_deref(), index_input, reduction, &sequence, &safe_reduced, &active, )?;
	ir.barrier(target);
	...
	for (step_index, step) in reduction.tree.steps.iter().enumerate() {
		...
		ir.line(format_args!(
			...
			step.stride )); ir.line(format_args!(
		...
		ir.line(format_args!(
			...
			step.stride )); let left_value = shared_load( ir, &value_global, reduction.tree.lanes,
			"%local_id",
			reduction.dtype,
			"reduction_left",
		); let right_value = shared_load( ir, &value_global, reduction.tree.lanes, &right_lane, reduction.dtype,
			"reduction_right",
		); let left_index = index_global.as_ref().map(|global| { shared_load( ir, global, reduction.tree.lanes,
				"%local_id",
				DType::I32,
				"reduction_left_index",
			) }); let right_index = index_global.as_ref().map(|global| { shared_load( ir, global, reduction.tree.lanes,
				&right_lane, DType::I32,
				"reduction_right_index",
			) }); let (combined, combined_index) = emit_reduce_combine( ir, reduction.operator, reduction.dtype, &left_value,
			&right_value, left_index.as_deref(), right_index.as_deref(), )?; shared_store( ir, &value_global,
			reduction.tree.lanes,
			"%local_id",
			reduction.dtype, &combined,
			"reduction_combined",
		); store_reduction_combined_index( ir, index_global.as_deref(), combined_index.as_deref(), reduction.tree.lanes, )?;
		ir.line(format_args!("  br label %{joined}"));
		ir.line(format_args!("{joined}:"));
		ir.barrier(target); let expected_step = match u32::try_from(step_index) { Ok(step) => step,
			Err(error) => return Err(stage_error(error.to_string())), }; ensure_stage(
	...
	)); store_reduction_outputs(ir, ReductionOutputs { value: value_output, index: index_output,
		value_global: &value_global, index_global: index_global.as_deref(), linear: &output_linear,
		lanes: reduction.tree.lanes, dtype: reduction.dtype, })?;
...
fn initialize_reduction_index( ir: &mut Ir, index_global: Option<&str>, index_input: Option<&BoundPointer>,
	reduction: &ReductionStage, sequence: &str, safe_reduced: &str, active: &str, ) -> Result<(), LoweringError> {
	let index_global = match index_global { Some(global) => global, None => return Ok(()), };
	let raw_index = match index_input { Some(pointer) => {
	...
	)); shared_store( ir, index_global, reduction.tree.lanes,
		"%local_id",
		DType::I32, &initial_index,
		"reduction_shared_index",
	); Ok(()) }

fn store_reduction_combined_index( ir: &mut Ir, global: Option<&str>, combined_index: Option<&str>, lanes: u32,
) -> Result<(), LoweringError> { match (global, combined_index) { (Some(global), Some(combined_index)) => {
			shared_store( ir, global, lanes,
				"%local_id",
				DType::I32, combined_index,
				"reduction_combined_index",
			); Ok(()) }
		(None, None) => Ok(()), (Some(_), None) | (None, Some(_)) => { Err(stage_error(
...
fn store_reduction_outputs(ir: &mut Ir, outputs: ReductionOutputs<'_>) -> Result<(), LoweringError> {
	match outputs.value { Some(output) => {
		...
		None => Ok(()), }?; match (outputs.index, outputs.index_global) { (Some(output), Some(global)) => {
			let index = ir.affine_index(output, outputs.linear, "reduction_index_output");
			let value = shared_load( ir, global, outputs.lanes,
				"0",
				DType::I32,
...
fn validate_tree_contract(tree: &FixedTree, stage: &ProgramStage, phase: TreePhase) -> Result<(), LoweringError> {
	ensure_stage( tree.lanes == stage.geometry.workgroup_lanes && tree.steps.len() == stage.synchronization.len(),
		"fixed tree differs from launch geometry or synchronization count",
	)?; for (step, synchronization) in tree.steps.iter().zip(&stage.synchronization) { ensure_stage(
...
fn reduction_input_index( ir: &mut Ir, input: &BoundPointer, reduction: &ReductionStage, sequence: &str, reduced: &str,
) -> Result<String, LoweringError> { match reduction.pass {
		0 => reduction_first_pass_index(ir, input, reduction, sequence, reduced), 1..=u32::MAX => {
...
fn reduction_first_pass_index( ir: &mut Ir, input: &BoundPointer, reduction: &ReductionStage, sequence: &str,
	reduced: &str, ) -> Result<String, LoweringError> { let rank = input.view.logical_extents.len(); ensure_stage(
		reduction.reduced_axes.iter().all(|axis| *axis < rank),
		"reduction axis exceeds its first-pass input rank",
	)?; let free_extents = input .view .logical_extents .iter() .copied() .enumerate()
		.filter(|(axis, _)| !reduction.reduced_axes.contains(axis)) .map(|(_, extent)| extent) .collect::<Vec<_>>();
	let reduced_extents = reduction .reduced_axes .iter() .map(|axis| input.view.logical_extents[*axis])
		.collect::<Vec<_>>();
	...
	let mut free = free_coordinates.into_iter(); let mut reduced = reduced_coordinates.into_iter();
	let mut coordinates = Vec::with_capacity(rank); for axis in 0..rank { let coordinate = match reduction .reduced_axes
			.iter() .position(|candidate| *candidate == axis)
		{ Some(_) => { reduced .next()
...
fn reduction_identity( ir: &mut Ir, operator: ReduceOperator, dtype: DType, purpose: &str,
) -> Result<String, LoweringError> { let literal = match (operator, dtype) {
...
fn shared_store(ir: &mut Ir, global: &str, lanes: u32, index: &str, dtype: DType, value: &str, purpose: &str) {
	let bits = match dtype { DType::I32 => value.to_owned(), DType::F32 => {
...
fn shared_load(ir: &mut Ir, global: &str, lanes: u32, index: &str, dtype: DType, purpose: &str) -> String {
	...
	)); match dtype { DType::I32 => bits, DType::F32 => { let value = ir.temporary(purpose);
			ir.line(format_args!("  {value} = bitcast i32 {bits} to float"));
			value } } }

fn emit_reduce_combine( ir: &mut Ir, operator: ReduceOperator, dtype: DType, left: &str, right: &str,
	left_index: Option<&str>, right_index: Option<&str>, ) -> Result<(String, Option<String>), LoweringError> {
	match operator { ReduceOperator::Minimum | ReduceOperator::Maximum => {
			let choose_right = ordered_choose_right(ir, OrderedComparison { dtype, maximum: operator == ReduceOperator::Maximum,
				left, right, indices: (left_index, right_index),
				purpose: "reduce_order",
			})?;
			let value = ir.temporary("reduce_selected");
			ir.line(format_args!(
				"  {value} = select i1 {choose_right}, {} {right}, {} {left}",
				llvm_type(dtype), llvm_type(dtype) )); let value = match dtype {
				DType::F32 => canonicalize_nan(ir, &value, "reduce_canonical"),
				DType::I32 => value, }; let index = match (left_index, right_index) { (Some(left), Some(right)) => {
		...
		ReduceOperator::Sum | ReduceOperator::Product => { let value = match dtype { DType::I32 => {
					...
					value }
				DType::F32 => { constrained_binary( ir, match operator {
...
fn constrained_binary(ir: &mut Ir, operation: &'static str, left: &str, right: &str, purpose: &str) -> String {
	...
	let raw = ir.temporary(purpose); ir.line(format_args!(
...
fn constrained_fma(ir: &mut Ir, left: &str, right: &str, addend: &str, purpose: &str) -> String { ir.declare(
...
fn plain_fdiv(ir: &mut Ir, left: &str, right: &str, purpose: &str) -> String { let raw = ir.temporary(purpose);
...
fn ordered_choose_right(ir: &mut Ir, comparison: OrderedComparison<'_>) -> Result<String, LoweringError> {
	let OrderedComparison { dtype, maximum, left, right, indices: (left_index, right_index), purpose, } = comparison;
	let right_better = match dtype { DType::I32 => {
		...
		DType::F32 => {
			...
			)); value } }; match (left_index, right_index) { (Some(left_index), Some(right_index)) => { let equal = match dtype {
				DType::I32 => {
...
fn emit_scan_local( ir: &mut Ir, signature: &StageSignature, stage: &ProgramStage, scan: &ScanLocalStage,
	target: &KernelTarget, options: &LoweringOptions, ) -> Result<(), LoweringError> {
	validate_tree_contract(&scan.tree, stage, TreePhase::ScanUpsweep)?; let input = signature.pointer(0)?;
	let output = signature.pointer(1)?; let totals = signature.pointers.get(2); ensure_stage( input.dtype == scan.dtype
	...
	)); let position = match scan.reverse { true => { let last = scan.input_width - 1;
	...
	ir.line(format_args!(
		...
		llvm_type(scan.dtype), llvm_type(scan.dtype) )); shared_store( ir, &shared, scan.tree.lanes,
		"%local_id",
		scan.dtype, &initial,
		"scan_shared_initial",
	); ir.barrier(target);

	let mut root_cleared = false; for (step_index, step) in scan.tree.steps.iter().enumerate() {
		root_cleared = match (step.phase, root_cleared) { (TreePhase::ScanDownsweep, false) => {
				emit_scan_root_reset(ir, &shared, scan, &sequence, &block, totals)?; true }
			(TreePhase::ScanDownsweep, true) | (TreePhase::ScanUpsweep, true) | (TreePhase::Reduction, true) => { true }
			(TreePhase::ScanUpsweep, false) | (TreePhase::Reduction, false) => false, }; let span = u64::from(step.stride) * 2;
		...
		ir.line(format_args!(
			...
			step.stride )); let left = shared_load( ir, &shared, scan.tree.lanes, &left_lane, scan.dtype,
			"scan_left",
		); let right = shared_load( ir, &shared, scan.tree.lanes,
			"%local_id",
			scan.dtype,
			"scan_right",
		); match step.phase { TreePhase::ScanUpsweep => { let (combined, _) =
					emit_reduce_combine(ir, scan.operator, scan.dtype, &left, &right, None, None)?; shared_store( ir, &shared,
					scan.tree.lanes,
					"%local_id",
					scan.dtype, &combined,
					"scan_upsweep",
				); }
			TreePhase::ScanDownsweep => { shared_store( ir, &shared, scan.tree.lanes, &left_lane, scan.dtype, &right,
					"scan_downsweep_left",
				); let (combined, _) = emit_reduce_combine(ir, scan.operator, scan.dtype, &right, &left, None, None)?; shared_store(
					ir, &shared, scan.tree.lanes,
					"%local_id",
					scan.dtype, &combined,
		...
		ir.line(format_args!("  br label %{joined}"));
		ir.line(format_args!("{joined}:"));
		ir.barrier(target); let expected_ordinal = match u32::try_from(step_index) { Ok(step) => step,
			Err(error) => return Err(stage_error(error.to_string())), }; ensure_stage(
			stage.synchronization[step_index].after_step == expected_ordinal,
			"scan synchronization ordinal differs from its tree step",
		)?; }
	match root_cleared { true => Ok(()), false => emit_scan_root_reset(ir, &shared, scan, &sequence, &block, totals), }?;
	...
	ir.line(format_args!("{write}:"));
	let exclusive = shared_load( ir, &shared, scan.tree.lanes,
		"%local_id",
		scan.dtype,
		"scan_exclusive",
	); let result = match scan.mode { ScanStageMode::UserInclusive => { emit_reduce_combine( ir, scan.operator, scan.dtype,
				&exclusive, &loaded, None, None, )? .0 }
...
fn emit_scan_root_reset( ir: &mut Ir, shared: &str, scan: &ScanLocalStage, sequence: &str, block: &str,
	totals: Option<&BoundPointer>, ) -> Result<(), LoweringError> { let root = u64::from(scan.tree.lanes - 1);
	...
	let total = shared_load( ir, shared, scan.tree.lanes, &root.to_string(), scan.dtype,
		"scan_block_total",
	); match totals { Some(totals) => {
		...
		None => Ok(()), }?; let identity = match scan.mode {
		...
		} }; shared_store( ir, shared, scan.tree.lanes, &root.to_string(), scan.dtype, &identity,
		"scan_root_identity",
	);
	ir.line(format_args!("  br label %{joined}"));
	ir.line(format_args!("{joined}:"));
...
fn scan_value_index( ir: &mut Ir, pointer: &BoundPointer, scan: &ScanLocalStage, sequence: &str, position: &str,
	purpose: &str, ) -> Result<String, LoweringError> { match pointer.view.logical_extents.len() { 1 => {
		...
		0 | 2.. => { ensure_stage( scan.axis < pointer.view.logical_extents.len()
					&& pointer.view.logical_extents[scan.axis] == scan.input_width,
				"scan axis or width differs from its tensor binding",
			)? } }
	let free_extents = pointer .view .logical_extents .iter() .copied() .enumerate()
		.filter(|(axis, _)| *axis != scan.axis) .map(|(_, extent)| extent) .collect::<Vec<_>>();
	...
	for axis in 0..pointer.view.logical_extents.len() { coordinates.push(match axis == scan.axis {
			true => position.to_owned(), false => { free.next()
					.ok_or_else(|| stage_error("scan coordinate underflow"))?
			} }); }
	Ok(ir.index_from_coordinates(pointer, &coordinates, purpose)) }

fn emit_scan_uniform( ir: &mut Ir, signature: &StageSignature, scan: &ScanUniformStage, axis: usize,
) -> Result<(), LoweringError> { let target = signature.pointer(0)?; let offsets = signature.pointer(1)?; ensure_stage(
		target.dtype == scan.dtype && offsets.dtype == scan.dtype,
		"uniform scan binding type differs from its contract",
	)?; let active_per_sequence = scan.width.saturating_sub(u64::from(scan.block_lanes)); ensure_stage(
	...
	ir.line(format_args!(
		...
		scan.block_lanes )); let position = match scan.reverse { true => {
	...
	ir.line(format_args!(
		...
		scan.block_lanes )); let blocks = scan.width.div_ceil(u64::from(scan.block_lanes));
...
fn scan_uniform_target_index( ir: &mut Ir, pointer: &BoundPointer, scan: &ScanUniformStage, axis: usize, sequence: &str,
	position: &str, ) -> Result<String, LoweringError> { match pointer.view.logical_extents.len() { 1 => {
	...
	let free_extents = pointer .view .logical_extents .iter() .copied() .enumerate()
		.filter(|(candidate, _)| *candidate != axis) .map(|(_, extent)| extent) .collect::<Vec<_>>(); let mut free = ir
		.coordinates(sequence, &free_extents, "uniform_scan_sequence")
		.into_iter(); let mut coordinates = Vec::with_capacity(pointer.view.logical_extents.len());
	for candidate in 0..pointer.view.logical_extents.len() { coordinates.push(match candidate == axis {
			true => position.to_owned(), false => { free.next()
...
fn emit_contraction( ir: &mut Ir, signature: &StageSignature, contraction: &ContractionStage, target: &KernelTarget,
	options: &LoweringOptions, ) -> Result<(), LoweringError> { let private_address_space = private_address_space(target);
	let left = signature.pointer(0)?; let right = signature.pointer(1)?; let output = signature.pointer(2)?; ensure_stage(
	...
	)?; ensure_stage( contraction.canonical_contracted_order,
		"contraction does not declare canonical contracted order",
	)?; let left_rank = left.view.logical_extents.len(); let right_rank = right.view.logical_extents.len();
	let mut used_left = vec![false; left_rank]; let mut used_right = vec![false; right_rank];
	for (left_axis, right_axis) in contraction .spec .batch_axes .iter() .chain(&contraction.spec.contract_axes)
	{ ensure_stage( *left_axis < left_rank && *right_axis < right_rank && !used_left[*left_axis]
		...
		)?; used_left[*left_axis] = true; used_right[*right_axis] = true; }
	let left_free = (0..left_rank) .filter(|axis| !used_left[*axis]) .collect::<Vec<_>>(); let right_free = (0..right_rank)
		.filter(|axis| !used_right[*axis]) .collect::<Vec<_>>();
	let expected_output_rank = contraction.spec.batch_axes.len() + left_free.len() + right_free.len(); ensure_stage(
	...
	{ return Ok(()); }
	let workgroup_lanes = contraction .tile .output_x .checked_mul(contraction.tile.output_y)
		.ok_or_else(|| stage_error("contraction workgroup width overflowed"))?;
	let staged_global = match contraction.strategy { ContractionStrategy::Direct => None, ContractionStrategy::Staged => {
	...
	)); let output_coordinates = ir.coordinates( &safe_output, &output.view.logical_extents,
		"contraction_output",
	);
	let mut left_coordinates = vec!["0".to_owned(); left_rank];
	let mut right_coordinates = vec!["0".to_owned(); right_rank];
	let mut output_axis = 0_usize; for (left_axis, right_axis) in &contraction.spec.batch_axes {
		left_coordinates[*left_axis] = output_coordinates[output_axis].clone();
		right_coordinates[*right_axis] = output_coordinates[output_axis].clone(); output_axis += 1; }
	for axis in &left_free { left_coordinates[*axis] = output_coordinates[output_axis].clone(); output_axis += 1; }
	for axis in &right_free { right_coordinates[*axis] = output_coordinates[output_axis].clone(); output_axis += 1; }

	let accumulator = ir.temporary("contraction_accumulator_slot");
	ir.line(format_args!(
		"  {accumulator} = alloca {}, align 4, addrspace({private_address_space})",
		llvm_type(contraction.dtype), )); let zero = match contraction.dtype {
		DType::F32 => "0.000000e+00",
		DType::I32 => "0",
	}; ir.line(format_args!(
	...
	ir.line(format_args!(
		...
		contraction.contracted_elements )); ir.line(format_args!(
	...
	let contracted_extents = contraction .spec .contract_axes .iter() .map(|(axis, _)| left.view.logical_extents[*axis])
		.collect::<Vec<_>>();
	let contracted_coordinates = ir.coordinates(&contracted, &contracted_extents, "contraction_k");
	let mut left_iteration = left_coordinates.clone(); let mut right_iteration = right_coordinates.clone();
	for ((left_axis, right_axis), coordinate) in contraction .spec .contract_axes .iter() .zip(contracted_coordinates)
	{ left_iteration[*left_axis] = coordinate.clone(); right_iteration[*right_axis] = coordinate; }
	...
	let product = match contraction.dtype { DType::F32 => None, DType::I32 => {
	...
	ir.line(format_args!(
		...
		llvm_type(contraction.dtype) )); let sum = match contraction.dtype { DType::F32 => { constrained_fma( ir, &left_value,
				&right_value, &accumulated,
				"contraction_sum",
			) }
		DType::I32 => {
	...
	if let Some(staged_global) = &staged_global {
		...
		ir.line(format_args!(
			...
			llvm_type(contraction.dtype) )); shared_store( ir, staged_global, workgroup_lanes,
			"%local_id",
			contraction.dtype, &checkpoint,
			"contraction_staged_checkpoint",
		); ir.barrier(target); let restored = shared_load( ir, staged_global, workgroup_lanes,
			"%local_id",
			contraction.dtype,
			"contraction_staged_restore",
		); ir.line(format_args!(
			"  store {} {restored}, ptr addrspace({private_address_space}) {accumulator}, align 4",
			llvm_type(contraction.dtype) )); ir.barrier(target); }
	let next_tile = ir.temporary("contraction_next_tile");
	ir.line(format_args!(
		"  {next_tile} = add i64 {tile_base}, {}",
		contraction.tile.reduction )); ir.line(format_args!(
...
fn emit_nvidia_tf32_matrix_contraction( ir: &mut Ir, left: &BoundPointer, right: &BoundPointer, output: &BoundPointer,
	contraction: &ContractionStage, ) -> Result<bool, LoweringError> {
	let ([rows, reduction], [columns, right_reduction], [output_rows, output_columns]) = (
		left.view.logical_extents.as_slice(), right.view.logical_extents.as_slice(), output.view.logical_extents.as_slice(),
	) else { return Ok(false); }; let eligible = contraction.dtype == DType::F32 && contraction.spec.batch_axes.is_empty()
		...
		&& output.view.strides.as_slice() == [*columns, 1]; if !eligible { return Ok(false); }

	let column_tiles = columns / 8; let tile_count = (rows / 16) .checked_mul(column_tiles)
		.ok_or_else(|| stage_error("NVIDIA TF32 contraction tile count overflowed"))?;
	let warps_per_group = u64::from( contraction .tile .output_x .checked_mul(contraction.tile.output_y)
			.ok_or_else(|| stage_error("NVIDIA TF32 contraction workgroup width overflowed"))?
			/ 32, ); ensure_stage( warps_per_group > 0,
	...
	let mut accumulators = [
		...
		accumulators = tf32_mma( ir, [&left_first, &left_second], &right_value, accumulators.each_ref(), ); }

	for (index, accumulator) in accumulators.iter().enumerate() { let row = match index < 2 { true => {
...
fn tf32_fragment(ir: &mut Ir, value: &str, purpose: &str) -> String { let converted = ir.temporary(purpose);
	ir.line(format_args!(
...
fn tf32_mma(ir: &mut Ir, left: [&str; 2], right: &str, accumulators: [&String; 4]) -> [String; 4] {
	...
	ir.line(format_args!(
		...
		left[0], left[1], accumulators[0], accumulators[1], accumulators[2], accumulators[3] )); std::array::from_fn(|index| {
		let value = ir.temporary("mma_accumulator");
		ir.line(format_args!(
			"  {value} = extractvalue {{ float, float, float, float }} {result}, {index}"
		)); value }) }

fn emit_gather( ir: &mut Ir, signature: &StageSignature, fault: Option<FaultContract>, axis: usize, bounds: IndexBounds,
) -> Result<(), LoweringError> { let input = signature.pointer(0)?; let indices = signature.pointer(1)?;
	let output = signature.pointer(2)?; ensure_stage( indices.dtype == DType::I32 && input.dtype == output.dtype,
	...
	let mapping = emit_gather_mapping( ir, input, indices, &output_coordinates, axis, bounds,
		"gather",
	)?;
	let output_index = ir.index_from_coordinates(output, &output_coordinates, "gather_output");
	match (bounds, mapping.valid.as_deref(), fault) { (IndexBounds::Reject, Some(valid), Some(fault)) => {
...
fn emit_scatter( ir: &mut Ir, signature: &StageSignature, fault: Option<FaultContract>, stage: &ProgramStage,
	axis: usize, bounds: IndexBounds, conflict: ScatterConflict, ) -> Result<(), LoweringError> {
	let indices = signature.pointer(0)?; let updates = signature.pointer(1)?; let output = signature.pointer(2)?;
	ensure_stage( indices.dtype == DType::I32 && updates.dtype == output.dtype,
		"scatter binding types are invalid",
	)?; begin_lane(ir); let update_coordinates = ir.coordinates(
		"%global_id",
		&updates.view.logical_extents,
		"scatter_update",
	); let mapping = emit_gather_mapping( ir, output, indices, &update_coordinates, axis, bounds,
		"scatter",
	)?;
	...
	match conflict { ScatterConflict::UniqueIndices => {
			ir.store(output, &output_index, &update, "scatter_output")?;
		}
		ScatterConflict::Atomic { operation, ordering, } => { let atomic = stage .atomics .iter()
			...
			ensure_stage( atomic.operation == OwnedAtomicOperation::from(operation)
			...
			)?; emit_atomic_update( ir, output, &output_index, atomic.operation, atomic.ordering, &update,
				"scatter",
			)?; } }
	finish_lane(ir); Ok(()) }

#[derive(Debug)]
struct GatherMapping { payload_coordinates: Vec<String>, valid: Option<String>, }

fn emit_gather_mapping( ir: &mut Ir, payload: &BoundPointer, indices: &BoundPointer, result_coordinates: &[String],
	axis: usize, bounds: IndexBounds, purpose: &str, ) -> Result<GatherMapping, LoweringError> {
	let payload_rank = payload.view.logical_extents.len(); ensure_stage(
		axis < payload_rank && result_coordinates.len() + 1 >= payload_rank,
		"indexed operation axis or result rank is invalid",
	)?; let index_rank = result_coordinates.len() + 1 - payload_rank; ensure_stage(
	...
	let mut payload_coordinates = Vec::with_capacity(payload_rank); for payload_axis in 0..payload_rank {
		let coordinate = match payload_axis.cmp(&axis) { std::cmp::Ordering::Less => result_coordinates[payload_axis].clone(),
	...
	Ok(GatherMapping { payload_coordinates, valid, }) }

fn normalize_index( ir: &mut Ir, raw: &str, extent: u64, bounds: IndexBounds, purpose: &str,
) -> Result<(String, Option<String>), LoweringError> { ensure_stage(
	...
	match bounds { IndexBounds::Reject => {
...
fn emit_fault_publish(ir: &mut Ir, fault: FaultContract) -> Result<(), LoweringError> { ensure_stage(
		fault.guard_before_address && fault.publish.operation == OwnedAtomicOperation::Exchange
...
fn emit_atomic_update( ir: &mut Ir, pointer: &BoundPointer, index: &str, operation: OwnedAtomicOperation,
	ordering: OwnedAtomicOrdering, operand: &str, purpose: &str, ) -> Result<(), LoweringError> {
	let address = ir.write_address(pointer, index, purpose)?; match pointer.dtype { DType::I32 => {
		...
		DType::F32 => { match operation { OwnedAtomicOperation::Exchange => {
					let previous = ir.temporary(&format!("{purpose}_atomic_previous"));
					ir.line(format_args!(
				...
				OwnedAtomicOperation::Add | OwnedAtomicOperation::Minimum | OwnedAtomicOperation::Maximum => {
					let helper = ensure_f32_atomic_helper(ir, operation, ordering); ir.line(format_args!(
...
fn ensure_f32_atomic_helper(ir: &mut Ir, operation: OwnedAtomicOperation, ordering: OwnedAtomicOrdering) -> String {
	...
	declare_atomic_intrinsic(ir, operation); let desired = match operation { OwnedAtomicOperation::Add => { String::from(
  ...
  %desired = select i1 %raw_nan, float 0x7FF8000000000000, float %raw\n",
		...
		OwnedAtomicOperation::Minimum | OwnedAtomicOperation::Maximum => { let predicate = match operation {
...
exit:\n\
  ret void\n\
		...
		atomic_ordering(ordering), atomic_failure_ordering(ordering) ); ir.helpers.entry(name.clone()).or_insert(helper); name
}

fn declare_atomic_intrinsic(ir: &mut Ir, operation: OwnedAtomicOperation) { let declaration = match operation {
		OwnedAtomicOperation::Add => { Some((
				"llvm.experimental.constrained.fadd.f32",
				"declare float @llvm.experimental.constrained.fadd.f32(float, float, metadata, metadata)",
			)) }
		OwnedAtomicOperation::Exchange | OwnedAtomicOperation::Minimum | OwnedAtomicOperation::Maximum => None, }; declaration
		.into_iter() .for_each(|(name, signature)| ir.declare(name, signature)); }

#[derive(Clone, Copy, Debug)]
struct HistogramContract { fault: FaultContract, bins: u32, weighted: bool, mapping: HistogramBinMapping,
	ordering: OwnedAtomicOrdering, }

fn emit_histogram( ir: &mut Ir, signature: &StageSignature, stage: &ProgramStage, contract: HistogramContract,
) -> Result<(), LoweringError> { let HistogramContract { fault, bins, weighted, mapping, ordering, } = contract;
	let input = signature.pointer(0)?; let output = signature.pointer(1)?; let weight = match weighted {
		true => Some(signature.pointer(2)?), false => None, }; let expected_output = match weighted { true => DType::F32,
		false => DType::I32, }; ensure_stage( bins != 0 && output.view.logical_extents.as_slice() == [u64::from(bins)]
	...
	)?; let atomic = stage .atomics .iter() .find(|atomic| atomic.domain == AtomicAddressDomain::HistogramBins)
		.ok_or_else(|| stage_error("histogram omitted its bin atomic contract"))?;
	ensure_stage( atomic.operation == OwnedAtomicOperation::Add && atomic.ordering == ordering
			&& atomic.dtype == output.dtype,
	...
	let (bin, valid) = match (mapping, input.dtype) { (HistogramBinMapping::I32Direct, DType::I32) => {
			...
			ir.line(format_args!("  {valid} = and i1 {nonnegative}, {below}"));
			(signed, valid) }
		(HistogramBinMapping::F32TruncateTowardZero, DType::F32) => { ir.declare(
	...
	let operand = match weight { Some(weight) => {
	...
	emit_atomic_update( ir, output, &output_index, OwnedAtomicOperation::Add, ordering, &operand,
		"histogram",
	)?; finish_lane(ir); Ok(()) }

fn validate_sort_network(network: SortNetwork) -> Result<(), LoweringError> { ensure_stage( network.axis_length != 0
...
fn emit_sort_initialize(ir: &mut Ir, signature: &StageSignature, network: SortNetwork) -> Result<(), LoweringError> {
	validate_sort_network(network)?; let input = signature.pointer(0)?; let values = signature.pointer(1)?;
	let indices = signature.pointer(2)?; ensure_stage( values.dtype == input.dtype && indices.dtype == DType::I32
	...
	ir.line(format_args!(
		...
		network.padded_axis_length )); ir.line(format_args!(
...
fn emit_sort_compare(ir: &mut Ir, signature: &StageSignature, compare: SortCompareStage) -> Result<(), LoweringError> {
	validate_sort_network(compare.network)?; let values = signature.pointer(0)?; let indices = signature.pointer(1)?;
	ensure_stage( indices.dtype == DType::I32 && values.view.logical_extents.as_slice()
	...
	)?; let comparisons_per_slice = compare.network.padded_axis_length / 2; begin_lane(ir);
	...
	ir.line(format_args!(
		"  {group} = udiv i64 {pair}, {}",
		compare.compare_distance )); ir.line(format_args!(
	...
	let right_before_left = sort_before(ir, SortComparison { dtype: values.dtype, direction: compare.network.direction,
		left: (&right_value, &right_index), right: (&left_value, &left_index),
		purpose: "sort_right_before",
	}); let left_before_right = sort_before(ir, SortComparison { dtype: values.dtype, direction: compare.network.direction,
		left: (&left_value, &left_index), right: (&right_value, &right_index),
	...
	ir.line(format_args!(
		...
		llvm_type(values.dtype), llvm_type(values.dtype) )); ir.line(format_args!(
...
fn emit_sort_finalize( ir: &mut Ir, signature: &StageSignature, network: SortNetwork, emit_indices: bool,
) -> Result<(), LoweringError> { validate_sort_network(network)?; let values = signature.pointer(0)?;
	let indices = signature.pointer(1)?; let output = signature.pointer(2)?; let index_output = match emit_indices {
		true => Some(signature.pointer(3)?), false => None, }; ensure_stage( values.dtype == output.dtype
	...
	)?; let position = output_coordinates[network.axis].clone(); let free_extents = output .view .logical_extents .iter()
		.copied() .enumerate() .filter(|(axis, _)| *axis != network.axis) .map(|(_, extent)| extent) .collect::<Vec<_>>();
	let free_coordinates = output_coordinates .iter() .enumerate() .filter(|(axis, _)| *axis != network.axis)
		.map(|(_, coordinate)| coordinate.clone()) .collect::<Vec<_>>();
	...
	match index_output { Some(index_output) => {
		...
		None => Ok(()), }?; finish_lane(ir); Ok(()) }

fn sort_tensor_index( ir: &mut Ir, pointer: &BoundPointer, network: SortNetwork, slice: &str, position: &str,
	purpose: &str, ) -> Result<String, LoweringError> { ensure_stage( network.axis < pointer.view.logical_extents.len()
			&& pointer.view.logical_extents[network.axis] == network.axis_length,
		"sort input axis differs from its network",
	)?; let free_extents = pointer .view .logical_extents .iter() .copied() .enumerate()
		.filter(|(axis, _)| *axis != network.axis) .map(|(_, extent)| extent) .collect::<Vec<_>>();
	let mut free = ir.coordinates(slice, &free_extents, purpose).into_iter();
	let mut coordinates = Vec::with_capacity(pointer.view.logical_extents.len());
	for axis in 0..pointer.view.logical_extents.len() { coordinates.push(match axis == network.axis {
			true => position.to_owned(), false => { free.next()
...
struct SortComparison<'a> {
	dtype: DType, direction: SortDirection,
	left: (&'a str, &'a str),
	right: (&'a str, &'a str),
	purpose: &'a str,
}
...
fn sort_before(ir: &mut Ir, comparison: SortComparison<'_>) -> String {
	let SortComparison { dtype, direction, left: (left_value, left_index), right: (right_value, right_index), purpose,
	} = comparison;
	...
	ir.line(format_args!(
	...
	)); let values_before = match dtype { DType::I32 => {
		...
		DType::F32 => {
			...
			)); value } }; let values_equal = match dtype { DType::I32 => {
			let value = ir.temporary(&format!("{purpose}_equal"));
			ir.line(format_args!(
...
fn emit_linear_from_coordinates(ir: &mut Ir, coordinates: &[String], extents: &[u64], purpose: &str) -> String {
	let mut result = None; for (axis, (coordinate, extent)) in coordinates.iter().zip(extents).enumerate() {
		result = Some(match result { Some(previous) => {
...
fn emit_philox( ir: &mut Ir, signature: &StageSignature, build: &ArtifactBuildRecipe, contract: &Philox10Contract,
	target: &KernelTarget, ) -> Result<(), LoweringError> { ensure_stage( signature.has_run_id
			...
			&& contract.counter == [ PhiloxCounterWord::ElementLow, PhiloxCounterWord::ElementHigh,
					PhiloxCounterWord::IterationXorStreamLow, PhiloxCounterWord::IterationXorStreamHigh,
	...
	)?; let output = signature.pointer(0)?; let expected_dtype = match contract.distribution {
		RandomDistribution::UniformF32 | RandomDistribution::NormalF32 => DType::F32,
		RandomDistribution::BernoulliI32 { .. } | RandomDistribution::UniformI32 { .. } => DType::I32, }; ensure_stage(
		output.dtype == expected_dtype,
		"Philox output dtype differs from its distribution",
	)?; let helper = ensure_philox_helper(ir, contract); begin_lane(ir);
	...
	)); let kernel = build.source_kernel.get(); let seed_low = split_u64(contract.key.seed_low);
	let seed_high = split_u64(contract.key.seed_high); let kernel_words = split_u64(kernel);
	...
	let result = match contract.distribution { RandomDistribution::UniformF32 => {
			let words = emit_philox_call(ir, PhiloxCall { helper: &helper,
				counter: [&element_low, &element_high, &stream_low, &stream_high], key: [&key_0, &key_1],
				purpose: "philox_uniform",
			});
			uniform_f32(ir, &words[0], "philox_uniform")
		}
		RandomDistribution::BernoulliI32 { probability_bits } => { let words = emit_philox_call(ir, PhiloxCall {
				helper: &helper, counter: [&element_low, &element_high, &stream_low, &stream_high], key: [&key_0, &key_1],
				purpose: "philox_bernoulli",
			}); let probability = f32::from_bits(probability_bits); let threshold = bernoulli_threshold(probability)?;
			match threshold {
		...
		RandomDistribution::UniformI32 { low, high_exclusive, } => { emit_uniform_i32( ir, PhiloxInputs { helper: &helper,
					element_low: &element_low, element_high: &element_high, stream_low: &stream_low, stream_high: &stream_high,
					key: [&key_0, &key_1], }, low, high_exclusive, target, )? }
		RandomDistribution::NormalF32 => { let words = emit_philox_call(ir, PhiloxCall { helper: &helper,
				counter: [&element_low, &element_high, &stream_low, &stream_high], key: [&key_0, &key_1],
...
fn split_u64(value: u64) -> [u32; 2] { let bytes = value.to_le_bytes(); [
		u32::from_le_bytes([bytes[0], bytes[1], bytes[2], bytes[3]]),
		u32::from_le_bytes([bytes[4], bytes[5], bytes[6], bytes[7]]), ] }

fn bernoulli_threshold(probability: f32) -> Result<u64, LoweringError> { ensure_stage(
		probability.is_finite() && (0.0..=1.0).contains(&probability),
		"Bernoulli probability is outside the closed unit interval",
	)?; match probability.to_bits() { 0x3f80_0000 => Ok(4_294_967_296), bits => {
			let exponent_raw = (bits >> 23).to_le_bytes()[0]; match exponent_raw { 0 => Ok(0), 1..=126 => {
					let significand = u64::from((bits & 0x007f_ffff) | 0x0080_0000); let shift = i32::from(exponent_raw) - 118;
					match shift { i32::MIN..=-1 => Ok(significand >> shift.unsigned_abs().min(63)),
...
fn ensure_philox_helper(ir: &mut Ir, contract: &Philox10Contract) -> String {
	...
	for round in [0_u8, 1, 2, 3, 4, 5, 6, 7, 8, 9] {
		...
		); counters = [next_0, low_1, next_2, low_0]; keys = match round { 0..=8 => {
	...
	ir.helpers.entry(name.clone()).or_insert(body); name }
...
fn uniform_f32(ir: &mut Ir, word: &str, purpose: &str) -> String {
	...
	ir.line(format_args!("  {value} = bitcast i32 {bits} to float"));
	let one = literal_operand( ir, ScalarLiteral::F32Bits(1.0_f32.to_bits()),
		&format!("{purpose}_one"),
	);
...
fn emit_uniform_i32( ir: &mut Ir,
	inputs: PhiloxInputs<'_>,
	low: i32, high_exclusive: i32, target: &KernelTarget, ) -> Result<String, LoweringError> {
	let private_address_space = private_address_space(target); let range_i64 = i64::from(high_exclusive) - i64::from(low);
	let range = match u32::try_from(range_i64) { Ok(range) => range,
		Err(error) => return Err(stage_error(error.to_string())), }; let threshold = range.wrapping_neg() % range;
	...
	ir.line(format_args!(
		...
		inputs.stream_high )); let words = emit_philox_call(ir, PhiloxCall { helper: inputs.helper, counter: [
			inputs.element_low, inputs.element_high, inputs.stream_low, &retry_counter, ], key: inputs.key,
		purpose: "uniform_i32",
	});
...
fn owned_box_muller_v1(ir: &mut Ir, first: &str, second: &str) -> String {
	...
	let log_u1 = owned_log_v1(ir, &u1); let negative_two = literal_operand( ir,
	...
	let two_pi = literal_operand( ir, ScalarLiteral::F32Bits(std::f32::consts::TAU.to_bits()),
		"normal_two_pi",
	);
...
fn positive_uniform_f32(ir: &mut Ir, word: &str, purpose: &str) -> String {
	...
	)); let scale = literal_operand( ir, ScalarLiteral::F32Bits((1.0_f32 / 16_777_216.0_f32).to_bits()),
...
fn owned_log_v1(ir: &mut Ir, value: &str) -> String {
	...
	let mut power = y.clone(); let mut sum = y; for divisor in [3_u16, 5, 7, 9, 11, 13, 15] {
		power = constrained_binary(ir, "fmul", &power, &y2, "owned_log_power");
		let reciprocal = literal_operand( ir, ScalarLiteral::F32Bits((1.0_f32 / f32::from(divisor)).to_bits()),
	...
	let two = literal_operand( ir, ScalarLiteral::F32Bits(2.0_f32.to_bits()),
		"owned_log_two",
	);
	let log_mantissa = constrained_binary(ir, "fmul", &sum, &two, "owned_log_mantissa");
	let ln2 = literal_operand( ir, ScalarLiteral::F32Bits(std::f32::consts::LN_2.to_bits()),
		"owned_log_ln2",
	);
	let exponent_term = constrained_binary(ir, "fmul", &exponent, &ln2, "owned_log_exponent_term");
	constrained_binary( ir,
		"fadd",
		&log_mantissa, &exponent_term,
		"owned_log_result",
	) }

fn owned_cos_v1(ir: &mut Ir, angle: &str) -> String { let pi = literal_operand( ir,
		ScalarLiteral::F32Bits(std::f32::consts::PI.to_bits()),
		"owned_cos_pi",
	); let tau = literal_operand( ir, ScalarLiteral::F32Bits(std::f32::consts::TAU.to_bits()),
		"owned_cos_tau",
	);
	...
	)); let half_pi = literal_operand( ir, ScalarLiteral::F32Bits(std::f32::consts::FRAC_PI_2.to_bits()),
		"owned_cos_half_pi",
	);
	...
	let mut polynomial = literal_operand( ir, ScalarLiteral::F32Bits((-1.0_f32 / 3_628_800.0_f32).to_bits()),
		"owned_cos_c10",
	); for coefficient in [ 1.0_f32 / 40_320.0_f32, -1.0_f32 / 720.0_f32, 1.0_f32 / 24.0_f32, -0.5_f32, ] {
		polynomial = constrained_binary(ir, "fmul", &polynomial, &x2, "owned_cos_horner_mul");
		let coefficient = literal_operand( ir, ScalarLiteral::F32Bits(coefficient.to_bits()),
			"owned_cos_coefficient",
		); polynomial = constrained_binary( ir,
			"fadd",
