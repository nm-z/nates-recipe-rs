...
use recipe_core::{ArtifactBuildAccess, ArtifactBuildRecipe, ByteCount, DType, Digest, ElementCount, KernelResourceBounds, ScalarLiteral};
use recipe_language::{IndexBounds, IndexMap, RandomDistribution, ReduceOperator, ReduceResult, ScatterConflict, SortDirection};
use recipe_primitives::{AccessMode, AtomicAddressDomain, ContractionStage, ContractionStrategy, FaultContract, FixedTree, FloatSortOrder, HistogramBinMapping, LoweredProgram, MemorySemantics, NormalF32Mapping, OwnedAtomicOperation, OwnedAtomicOrdering, Philox10Contract, PhiloxCounterWord, ProgramStage, ReductionPadding, ReductionStage, ReductionTieBreak, ScanLocalStage, ScanStageMode, ScanUniformStage, SortCompareStage, SortNetwork, SortPadding, SortTieBreak, StageKind, SynchronizationScope, TreePhase, UniformI32Mapping};
use sha2::{Digest as _, Sha256};

use crate::{BufferAccess, KernelAbi, KernelArgument, KernelTarget, LoweredKernel, LoweringError, LoweringErrorKind, LoweringOptions, audit_llvm_ir};
...
pub(crate) fn lower_stage(program: &LoweredProgram, build: &ArtifactBuildRecipe, target: &KernelTarget, options: &LoweringOptions) -> Result<LoweredKernel, LoweringError> {
...
fn validate_contract<'a>(program: &'a LoweredProgram, build: &ArtifactBuildRecipe, target: &KernelTarget, options: &LoweringOptions) -> Result<&'a ProgramStage, LoweringError> {
	ensure_stage(
	...
	ensure_stage(
		build.dispatch.logical_lanes == stage.geometry.logical_lanes && build.dispatch.workgroup_lanes == stage.geometry.workgroup_lanes && build.dispatch.workgroups == stage.geometry.workgroups,
	...
	ensure_stage(
		build.work.flops == stage.resources.flops && build.work.integer_operations == stage.resources.integer_operations && build.work.atomic_operations == stage.resources.atomic_operations,
	...
	for (actual, expected) in build.bindings.iter().zip(&stage.bindings) {
		...
		ensure_stage(
			actual.dtype == expected.dtype && actual.access == expected_access && actual.view.logical_extents == expected.view.logical_extents && actual.view.offset_elements == expected.view.offset_elements && actual.view.strides == expected.view.strides && actual.view.storage_bytes == expected.view.storage_bytes,
...
fn stage_template_identity(program: &LoweredProgram, stage: &ProgramStage) -> Result<recipe_core::KernelTemplateId, LoweringError> {
...
impl StageSignature {
	fn new(stage: &ProgramStage, build: &ArtifactBuildRecipe, has_run_id: bool, has_loop_iteration: bool) -> Result<Self, LoweringError> {
...
impl Ir {
	...
	fn declare(&mut self, name: &'static str, declaration: &'static str) { self.declarations.insert(name, declaration); }
...
fn lower_owned_stage(program: &LoweredProgram, stage: &ProgramStage, kind: &StageKind, build: &ArtifactBuildRecipe, target: &KernelTarget, options: &LoweringOptions) -> Result<LoweredKernel, LoweringError> {
	let has_run_id = matches!(kind, StageKind::Philox4x32_10(_));
	let has_loop_iteration = matches!(kind, StageKind::Philox4x32_10(_)) || matches!(kind, StageKind::IndexMap(spec) if spec.iteration_step != 0);
	...
	match kind {
		...
		StageKind::FixedTreeReduce(reduction) => emit_reduction(&mut ir, &signature, stage, reduction, target, options)?,
...
fn validate_realized_kernel(lowered: &LoweredKernel, stage: &ProgramStage, build: &ArtifactBuildRecipe) -> Result<(), LoweringError> {
	ensure_stage(
		lowered.abi.elements.get() == stage.geometry.logical_lanes && lowered.abi.workgroup_lanes == stage.geometry.workgroup_lanes && lowered.work == build.work.flops,
...
fn assemble_stage_module(target: &KernelTarget, options: &LoweringOptions, signature: &StageSignature, ir: &Ir) -> String {
	...
	module.push_str("}\n\n");
	module.push_str("attributes #0 = { nounwind strictfp \"denormal-fp-math-f32\"=\"ieee\" \"no-trapping-math\"=\"false\" }\n");
	module
}

fn push_section_gap(module: &mut String, present: bool) { module.extend(std::iter::repeat_n('\n', usize::from(present))); }
...
fn emit_index_map(ir: &mut Ir, signature: &StageSignature, stage: &ProgramStage, spec: IndexMap) -> Result<(), LoweringError> {
	let output = signature.pointer(0)?;
	let fault = stage
		.fault
		.ok_or_else(|| stage_error("index-map stage omitted its checked fault contract"))?;
	ensure_stage(
		output.dtype == DType::I32 && !signature.has_run_id && signature.has_loop_iteration == (spec.iteration_step != 0) && fault.reason == recipe_primitives::FaultReason::ArithmeticDomain && fault.code == 4,
...
fn checked_unsigned_scale(ir: &mut Ir, unsigned: &str, scale: i32, purpose: &str, rejected: &mut Vec<String>) -> String {
	if scale == 0 {
		return "0".to_owned();
	}
	ir.declare(
...
fn scan_local_axis(kind: &StageKind, level: u32) -> Option<usize> {
	match kind {
		...
		StageKind::ScalarMap { .. } | StageKind::Fill { .. } | StageKind::Copy { .. } | StageKind::FixedTreeReduce(_) | StageKind::ScanUniformCombine(_) | StageKind::TiledContraction(_) | StageKind::Gather { .. } | StageKind::Scatter { .. } | StageKind::HistogramClear | StageKind::HistogramAccumulate { .. } | StageKind::StableSortInitialize { .. } | StageKind::StableSortCompareExchange(_) | StageKind::StableSortFinalize { .. } | StageKind::IndexMap(_) | StageKind::Philox4x32_10(_) => None,
	}
}

fn emit_reduction(ir: &mut Ir, signature: &StageSignature, stage: &ProgramStage, reduction: &ReductionStage, target: &KernelTarget, options: &LoweringOptions) -> Result<(), LoweringError> {
	ensure_stage(
	...
	ensure_stage(
		reduction.padding == ReductionPadding::OperatorIdentity && reduction.tie_break == ReductionTieBreak::LowestLogicalIndex,
	...
	let index_input = match has_index && reduction.pass != 0 {
		true => {
			Some(signature.pointer(
				*reads.get(1)
					.ok_or_else(|| stage_error("later reduction pass omitted its readable index binding"))?,
			)?)
		}
		false => None,
	};
...
fn initialize_reduction_index(ir: &mut Ir, index_global: Option<&str>, index_input: Option<&BoundPointer>, reduction: &ReductionStage, sequence: &str, safe_reduced: &str, active: &str) -> Result<(), LoweringError> {
...
fn store_reduction_combined_index(ir: &mut Ir, global: Option<&str>, combined_index: Option<&str>, lanes: u32) -> Result<(), LoweringError> {
...
fn validate_tree_contract(tree: &FixedTree, stage: &ProgramStage, phase: TreePhase) -> Result<(), LoweringError> {
	...
	for (step, synchronization) in tree.steps.iter().zip(&stage.synchronization) {
		ensure_stage(
			(phase != TreePhase::Reduction || step.phase == TreePhase::Reduction) && synchronization.scope == SynchronizationScope::Workgroup && synchronization.memory == MemorySemantics::SharedAcquireRelease,
...
fn reduction_input_index(ir: &mut Ir, input: &BoundPointer, reduction: &ReductionStage, sequence: &str, reduced: &str) -> Result<String, LoweringError> {
...
fn reduction_first_pass_index(ir: &mut Ir, input: &BoundPointer, reduction: &ReductionStage, sequence: &str, reduced: &str) -> Result<String, LoweringError> {
...
fn reduction_identity(ir: &mut Ir, operator: ReduceOperator, dtype: DType, purpose: &str) -> Result<String, LoweringError> {
...
fn emit_reduce_combine(ir: &mut Ir, operator: ReduceOperator, dtype: DType, left: &str, right: &str, left_index: Option<&str>, right_index: Option<&str>) -> Result<(String, Option<String>), LoweringError> {
	match operator {
			...
			let value = match dtype {
				DType::I32 => {
					...
					let operation = match operator {
						...
						ReduceOperator::Minimum | ReduceOperator::Maximum | ReduceOperator::Any | ReduceOperator::All => unreachable!("arithmetic reduction operator"),
					};
		...
		ReduceOperator::Any | ReduceOperator::All => {
			...
			let operation = match operator {
				...
				ReduceOperator::Sum | ReduceOperator::Product | ReduceOperator::Minimum | ReduceOperator::Maximum => unreachable!("logical reduction operator"),
			};
			ir.line(format_args!(
...
fn emit_scan_local(ir: &mut Ir, signature: &StageSignature, stage: &ProgramStage, scan: &ScanLocalStage, target: &KernelTarget, options: &LoweringOptions) -> Result<(), LoweringError> {
	validate_tree_contract(&scan.tree, stage, TreePhase::ScanUpsweep)?;
	let input = signature.pointer(0)?;
	let output = signature.pointer(1)?;
	let totals = signature.pointers.get(2);
	ensure_stage(
		input.dtype == scan.dtype && output.dtype == scan.dtype && totals.is_none_or(|pointer| pointer.dtype == scan.dtype),
	...
	for (step_index, step) in scan.tree.steps.iter().enumerate() {
		...
		match step.phase {
			TreePhase::ScanUpsweep => {
				let (combined, _) = emit_reduce_combine(ir, scan.operator, scan.dtype, &left, &right, None, None)?;
				shared_store(
					ir,
					&shared,
					scan.tree.lanes,
			...
			TreePhase::ScanDownsweep => {
				...
				let (combined, _) = emit_reduce_combine(ir, scan.operator, scan.dtype, &right, &left, None, None)?;
				shared_store(
					ir,
					&shared,
					scan.tree.lanes,
...
fn emit_scan_root_reset(ir: &mut Ir, shared: &str, scan: &ScanLocalStage, sequence: &str, block: &str, totals: Option<&BoundPointer>) -> Result<(), LoweringError> {
	...
	let identity = match scan.mode {
		...
		ScanStageMode::UserInclusive | ScanStageMode::HierarchyExclusiveIdentity => reduction_identity(ir, scan.operator, scan.dtype, "scan_identity")?,
	};
	shared_store(
		ir,
		shared,
...
fn scan_value_index(ir: &mut Ir, pointer: &BoundPointer, scan: &ScanLocalStage, sequence: &str, position: &str, purpose: &str) -> Result<String, LoweringError> {
	match pointer.view.logical_extents.len() {
		...
		0 | 2.. => {
			ensure_stage(
				scan.axis < pointer.view.logical_extents.len() && pointer.view.logical_extents[scan.axis] == scan.input_width,
...
fn emit_scan_uniform(ir: &mut Ir, signature: &StageSignature, scan: &ScanUniformStage, axis: usize) -> Result<(), LoweringError> {
...
fn scan_uniform_target_index(ir: &mut Ir, pointer: &BoundPointer, scan: &ScanUniformStage, axis: usize, sequence: &str, position: &str) -> Result<String, LoweringError> {
	match pointer.view.logical_extents.len() {
		1 => {
...
fn emit_contraction(ir: &mut Ir, signature: &StageSignature, contraction: &ContractionStage, target: &KernelTarget, options: &LoweringOptions) -> Result<(), LoweringError> {
	...
	{
		ensure_stage(
			*left_axis < left_rank && *right_axis < right_rank && !used_left[*left_axis] && !used_right[*right_axis] && left.view.logical_extents[*left_axis] == right.view.logical_extents[*right_axis],
	...
	if matches!(target, KernelTarget::Nvidia(nvidia) if nvidia.sm_major >= 8) && emit_nvidia_tf32_matrix_contraction(ir, left, right, output, contraction)? {
		return Ok(());
	}
...
fn emit_nvidia_tf32_matrix_contraction(ir: &mut Ir, left: &BoundPointer, right: &BoundPointer, output: &BoundPointer, contraction: &ContractionStage) -> Result<bool, LoweringError> {
	...
	let eligible = contraction.dtype == DType::F32 && contraction.spec.batch_axes.is_empty() && contraction.spec.contract_axes.as_slice() == [(1, 1)] && rows == output_rows && columns == output_columns && reduction == right_reduction && rows % 16 == 0 && columns % 8 == 0 && reduction % 4 == 0 && left.view.offset_elements == 0 && right.view.offset_elements == 0 && output.view.offset_elements == 0 && left.view.strides.as_slice() == [*reduction, 1] && right.view.strides.as_slice() == [*reduction, 1] && output.view.strides.as_slice() == [*columns, 1];
	if !eligible {
		return Ok(false);
	}
...
fn emit_gather(ir: &mut Ir, signature: &StageSignature, fault: Option<FaultContract>, axis: usize, bounds: IndexBounds) -> Result<(), LoweringError> {
	let input = signature.pointer(0)?;
	...
	match (bounds, mapping.valid.as_deref(), fault) {
		...
		(IndexBounds::Reject, None, None) | (IndexBounds::Reject, Some(_), None) | (IndexBounds::Reject, None, Some(_)) => Err(stage_error("rejecting gather lacks its exact fault guard")),
		(IndexBounds::Clamp | IndexBounds::Wrap, None, None) => Ok(()),
		(IndexBounds::Clamp | IndexBounds::Wrap, Some(_), None) | (IndexBounds::Clamp | IndexBounds::Wrap, None, Some(_)) | (IndexBounds::Clamp | IndexBounds::Wrap, Some(_), Some(_)) => {
			Err(stage_error(
...
fn emit_scatter(ir: &mut Ir, signature: &StageSignature, fault: Option<FaultContract>, stage: &ProgramStage, axis: usize, bounds: IndexBounds, conflict: ScatterConflict) -> Result<(), LoweringError> {
	...
	match (bounds, mapping.valid.as_deref(), fault) {
		...
		(IndexBounds::Reject, None, None) | (IndexBounds::Reject, Some(_), None) | (IndexBounds::Reject, None, Some(_)) => Err(stage_error("rejecting scatter lacks its exact fault guard")),
		(IndexBounds::Clamp | IndexBounds::Wrap, None, None) => Ok(()),
		(IndexBounds::Clamp | IndexBounds::Wrap, Some(_), None) | (IndexBounds::Clamp | IndexBounds::Wrap, None, Some(_)) | (IndexBounds::Clamp | IndexBounds::Wrap, Some(_), Some(_)) => {
			Err(stage_error(
	...
	match conflict {
		...
		ScatterConflict::Atomic {
			...
			ensure_stage(
				atomic.operation == OwnedAtomicOperation::from(operation) && atomic.ordering == OwnedAtomicOrdering::from(ordering) && atomic.dtype == output.dtype,
...
fn emit_gather_mapping(ir: &mut Ir, payload: &BoundPointer, indices: &BoundPointer, result_coordinates: &[String], axis: usize, bounds: IndexBounds, purpose: &str) -> Result<GatherMapping, LoweringError> {
...
fn normalize_index(ir: &mut Ir, raw: &str, extent: u64, bounds: IndexBounds, purpose: &str) -> Result<(String, Option<String>), LoweringError> {
	ensure_stage(
...
fn emit_fault_publish(ir: &mut Ir, fault: FaultContract) -> Result<(), LoweringError> {
	ensure_stage(
		fault.guard_before_address && fault.publish.operation == OwnedAtomicOperation::Exchange && fault.publish.ordering == OwnedAtomicOrdering::Release && fault.publish.domain == AtomicAddressDomain::SingleFaultFlag && fault.publish.dtype == DType::I32,
...
fn emit_atomic_update(ir: &mut Ir, pointer: &BoundPointer, index: &str, operation: OwnedAtomicOperation, ordering: OwnedAtomicOrdering, operand: &str, purpose: &str) -> Result<(), LoweringError> {
...
fn emit_histogram(ir: &mut Ir, signature: &StageSignature, stage: &ProgramStage, contract: HistogramContract) -> Result<(), LoweringError> {
	...
	ensure_stage(
		bins != 0 && output.view.logical_extents.as_slice() == [u64::from(bins)] && output.dtype == expected_output && weight.is_none_or(|pointer| pointer.dtype == DType::F32),
	...
	ensure_stage(
		atomic.operation == OwnedAtomicOperation::Add && atomic.ordering == ordering && atomic.dtype == output.dtype,
...
fn validate_sort_network(network: SortNetwork) -> Result<(), LoweringError> {
	ensure_stage(
		network.axis_length != 0 && network.padded_axis_length.is_power_of_two() && network.padded_axis_length == network.axis_length.next_power_of_two() && network.tie_break == SortTieBreak::OriginalAxisIndexAscending && network.float_order == FloatSortOrder::Ieee754TotalOrder && network.padding == SortPadding::AfterAllValidElements,
...
fn emit_sort_initialize(ir: &mut Ir, signature: &StageSignature, network: SortNetwork) -> Result<(), LoweringError> {
	...
	ensure_stage(
		values.dtype == input.dtype && indices.dtype == DType::I32 && values.view.logical_extents.as_slice() == [network.slices * network.padded_axis_length] && indices.view.logical_extents == values.view.logical_extents,
...
fn emit_sort_compare(ir: &mut Ir, signature: &StageSignature, compare: SortCompareStage) -> Result<(), LoweringError> {
	...
	let indices = signature.pointer(1)?;
	ensure_stage(
		indices.dtype == DType::I32 && values.view.logical_extents.as_slice() == [compare.network.slices * compare.network.padded_axis_length] && indices.view.logical_extents == values.view.logical_extents && compare.merge_width.is_power_of_two() && compare.compare_distance.is_power_of_two() && compare.compare_distance < compare.merge_width,
...
fn emit_sort_finalize(ir: &mut Ir, signature: &StageSignature, network: SortNetwork, emit_indices: bool) -> Result<(), LoweringError> {
	validate_sort_network(network)?;
	let values = signature.pointer(0)?;
	let indices = signature.pointer(1)?;
	let output = signature.pointer(2)?;
	...
	ensure_stage(
		values.dtype == output.dtype && indices.dtype == DType::I32 && index_output.is_none_or(|pointer| pointer.dtype == DType::I32),
...
fn sort_tensor_index(ir: &mut Ir, pointer: &BoundPointer, network: SortNetwork, slice: &str, position: &str, purpose: &str) -> Result<String, LoweringError> {
	ensure_stage(
		network.axis < pointer.view.logical_extents.len() && pointer.view.logical_extents[network.axis] == network.axis_length,
...
fn emit_philox(ir: &mut Ir, signature: &StageSignature, build: &ArtifactBuildRecipe, contract: &Philox10Contract, target: &KernelTarget) -> Result<(), LoweringError> {
	ensure_stage(
...
fn ensure_philox_helper(ir: &mut Ir, contract: &Philox10Contract) -> String {
	...
	let mut body = String::from("define internal { i32, i32, i32, i32 } @recipe_philox4x32_10_v1(i32 %counter_0, i32 %counter_1, i32 %counter_2, i32 %counter_3, i32 %key_0, i32 %key_1) #0 {\nentry:\n");
	let mut counters = [
		"%counter_0".to_owned(),
		"%counter_1".to_owned(),
		"%counter_2".to_owned(),
...
fn emit_uniform_i32(ir: &mut Ir, inputs: PhiloxInputs<'_>, low: i32, high_exclusive: i32, target: &KernelTarget) -> Result<String, LoweringError> {
	let private_address_space = private_address_space(target);
