...
impl Reservation {
	pub fn ram(device: DeviceId, bytes: ByteCount) -> Result<Self> {
		let arena = Arena::ram(device, bytes)?;
		let backing = Arc::try_unwrap(arena.backing).map_err(|_| Error::SlotBusy)?;
		Ok(Self {
			device,
			kind: ReservationKind::Ram,
			backing,
		})
	}

	pub fn disk(device: DeviceId, spec: DiskFileSpec, bytes: ByteCount) -> Result<Self> {
		let (file, path) = create_allocated_file(spec, bytes, "create disk reservation")?;
		Ok(Self {
			device,
			kind: ReservationKind::Disk,
