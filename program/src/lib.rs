...
impl StaticCalculationProgram {
	...
	#[inline]
	pub fn new<I: Into<LoopIterations>>(graph: CalculationGraph, iterations: I, domains: Vec<KernelIterationDomain>) -> ProgramResult<Self> { return Self::new_with_metrics(graph, iterations, domains, Vec::new()); }
	...
	/// inconsistent with the declared loop horizon.
	#[inline]
	pub fn new_with_metrics<I: Into<LoopIterations>>(graph: CalculationGraph, iterations: I, mut domains: Vec<KernelIterationDomain>, mut metrics: Vec<MetricEmission>) -> ProgramResult<Self> {
	...
	pub fn validate(&self) -> ProgramResult<()> {
		...
		for emission in &self.metrics {
			...
			let producer_covers_emission = if emission_domain.first_iteration() < producer_domain.first_iteration() || !(emission_domain.first_iteration() - producer_domain.first_iteration()).is_multiple_of(producer_domain.stride().get()) {
				false
			} else {
				let emission_last = emission_domain.end_exclusive().map(|end| {
					let span = end - 1 - emission_domain.first_iteration();
					return emission_domain.first_iteration() + span / emission_domain.stride().get() * emission_domain.stride().get();
				});
	...
	pub fn from_ogdl_graph(source: &Graph) -> ProgramResult<Self> {
		...
		let version_fields = node(source, program_root, "RecipeProgram")?
			...
			.copied()
			.filter(|child_id| {
				return node(source, *child_id, "RecipeProgram").is_ok_and(|child_node| return child_node.text() == "version");
			})
			.collect::<Vec<_>>();
		...
		if schema != PROGRAM_SCHEMA {
			return Err(ProgramError::new(
				ProgramErrorKind::InvalidDocument,
				format!("RecipeProgram.schema is {schema:?}, expected {PROGRAM_SCHEMA:?}"),
			));
		}
		let iteration_text = leaf_value(source, fields["iterations"], "RecipeProgram.iterations")?;
...
impl fmt::Display for ProgramError {
	...
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		return match self.clone() {
			...
			Self::Ogdl(error) => write!(f, "invalid program OGDL: {error}"),
...
impl From<Errored> for ProgramError {
	#[inline]
	fn from(error: Errored) -> Self { return Self::Ogdl(error); }
}
...
fn copy_subtree(source: &Graph, source_id: NodeId, target: &mut Graph, parent_id: Option<NodeId>) -> ProgramResult<NodeId> {
...
fn exact_fields(graph: &Graph, parent: NodeId, expected: &[&str], path: &str) -> ProgramResult<BTreeMap<String, NodeId>> {
